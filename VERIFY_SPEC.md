...
- native-linkage inspection reads the selected `CARGO_TARGET_DIR` and build profile (`FGIT_LINKAGE_PROFILE`, default `debug`), including the pinned nightly's `build/<package>/<hash>/run/stdout` and the older `output` layouts. `CARGO_BUILD_TARGET`, when set, selects both the metadata platform and its target subdirectory. Run fingerprints must resolve to a compiled build script whose dep-info names an enabled package's exact custom-build source path. A same-name stale version, another target/profile, missing output, or an unrecognized fingerprint cannot establish that observation. Cached observations are explicitly not current source/feature/release attestations; release evidence still requires its revision-bound build and complete target matrix. The real-build positive and planted forbidden-emission cell is `scripts/e2e/suites/checker/native_linkage_observation.sh`;
