...
mod unix {
    ...
    fn text(value: &OsString) -> Result<&str, &'static str> { value.to_str().ok_or(USAGE) }
    fn hex<const N: usize>(value: &OsString) -> Result<[u8; N], &'static str> {
        let value = text(value)?.as_bytes();
        if value.len() != N * 2 { return Err(USAGE); }
        let mut result = [0; N];
        let digit = |byte| match byte {
            b'0'..=b'9' => Ok(byte - b'0'), b'a'..=b'f' => Ok(byte - b'a' + 10), _ => Err(USAGE),
        };
    ...
    fn root(value: &OsString) -> Result<Commitment, &'static str> {
        ...
        let digest = Digest::new(DigestAlgorithm::Sha256.id(), DigestBytes::try_new(&bytes).map_err(|_| USAGE)?);
    ...
    fn number(value: &OsString) -> Result<u64, &'static str> {
        ...
        if parsed.to_string() != input { return Err(USAGE); }
        Ok(parsed)
    }
    fn parse(args: &[OsString]) -> Result<Options, &'static str> {
        if !matches!(args.len(), 6 | 9) { return Err(USAGE); }
        let path = PathBuf::from(&args[0]);
        if !path.is_absolute() { return Err(USAGE); }
        let scope = CheckJournalScope { tenant: TenantId::from_bytes(hex(&args[1])?),
            repository: RepositoryId::from_bytes(hex(&args[2])?), journal_id: root(&args[3])? };
        let batch = root(&args[4])?;
        let fact = usize::try_from(number(&args[5])?).map_err(|_| USAGE)?;
        if fact >= MAX_BATCH_FACTS { return Err(USAGE); }
        let minimum = if args.len() == 9 {
            if text(&args[6])? != "--minimum" { return Err(USAGE); }
            let bytes = number(&args[7])?;
            if bytes < 72 { return Err(USAGE); }
            Some(CheckJournalPin::new(bytes, root(&args[8])?))
        } else { None };
        Ok(Options { path, scope, batch, fact, minimum })
    }
    fn run(options: Options, output: &mut impl Write) -> Result<(), String> {
        let mut journal = FileCheckJournal::open(&options.path, options.scope,
            CheckJournalLimits::default(), options.minimum, &|| true).map_err(|error| error.to_string())?;
        let pin = journal.pin();
        let observation = journal.read_trusted_job(pin, options.batch, options.fact,
            MAX_OBSERVATION_BYTES, &|| true).map_err(|error| error.to_string())?;
        ...
        let json = format!(
            ...
            pin.byte_len(), pin.tail(), options.batch, options.fact, observation.evidence(),
            observation.run_id(), observation.attempt_id(), observation.authority_head(),
            observation.source_commit(), observation.logical_now(), observation.requires_containment(),
            observation.report_json(),
        );
        output.write_all(json.as_bytes()).and_then(|()| output.flush())
    ...
    pub fn main() -> std::process::ExitCode {
        ...
        let result = parse(&args).map_err(str::to_owned)
    ...
    mod tests {
        ...
        fn args() -> Vec<OsString> {
            ["/private/checks".to_owned(), "11".repeat(16), "22".repeat(16),
                "33".repeat(32), "44".repeat(32), "0".to_owned()].into_iter().map(OsString::from).collect()
        }
        #[test]
        fn exact_scope_and_minimum_pin_are_explicit_not_inferred_from_paths() {
            ...
            assert_eq!(options.fact, 0); assert!(options.minimum.is_none());
        ...
        fn malformed_selectors_and_extra_arguments_refuse_before_open() {
            for (index, value) in [(0, "relative"), (1, "00"), (3, "guess"), (5, "128"),
                (5, "-1"), (5, "+1"), (5, "01"), (5, "18446744073709551616")] {
                let mut input = args(); input[index] = value.into(); assert!(parse(&input).is_err());
            }
            for count in [0, 1, 5, 7, 8, 10] {
                let mut input = args(); input.resize(count, "unknown".into()); assert!(parse(&input).is_err());
            }
            let mut input = args(); input.extend(["--minimum".into(), "71".into(), "55".repeat(32).into()]);
...
#[cfg(unix)]
fn main() -> std::process::ExitCode { unix::main() }
