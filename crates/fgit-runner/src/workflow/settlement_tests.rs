...
fn observation(outcome: StepOutcome) -> StepObservation {
    StepObservation {
        outcome,
        exit_code: Some(if outcome == StepOutcome::Succeeded { 0 } else { 1 }),
        stdout: b"output".to_vec(), stderr: Vec::new(), elapsed_millis: 1,
...
impl Executor<'_> {
    fn new(first: StepObservation) -> Self {
        Self {
            ...
            events: Vec::new(), observations: Vec::new(),
            cancel_on_finish: None, close_error: false,
        }
    }
}
impl WorkflowExecutor for Executor<'_> {
    ...
    fn execute_step(&mut self, _: usize, script: &str, _: StepLimits, _: &dyn Fn() -> bool) -> Result<StepObservation, WorkerFailure> {
        ...
        Ok(self.pending.pop_front().expect("one observation per started step"))
    }
    fn finish_job(&mut self, retain: bool) -> Result<(), WorkerFailure> {
        self.events.push(format!("close:{retain}"));
        if let Some(live) = self.cancel_on_finish { live.set(false); }
        if self.close_error { Err(WorkerFailure::new("close failed", false)) }
        else { Ok(()) }
    }
...
fn run(executor: &mut Executor<'_>) -> WorkflowReport {
    WorkflowPlan::compile(WORKFLOW).unwrap()
        .execute(WorkflowLimits::default(), executor, &|| true).unwrap()
}
...
fn output_clamping_cannot_hide_containment_failure_or_bad_success_exit() {
    for outcome in [StepOutcome::ContainmentFailure, StepOutcome::Succeeded] {
        ...
        let report = WorkflowPlan::compile(WORKFLOW).unwrap().execute(
            WorkflowLimits { stream_bytes: 8, ..WorkflowLimits::default() },
            &mut executor, &|| true,
        ).unwrap();
...
fn cancellation_during_cleanup_is_observed_before_terminal_success_notification() {
    ...
    let report = WorkflowPlan::compile(WORKFLOW).unwrap().execute(
        WorkflowLimits::default(), &mut executor, &|| live.get(),
    ).unwrap();
    assert_eq!(report.jobs[0].outcome, JobOutcome::Cancelled);
    assert_eq!(report.jobs[1].outcome, JobOutcome::Cancelled);
...
fn ordinary_failure_permits_independent_work_and_observations_follow_close() {
    ...
    assert_eq!(executor.events, [
        "begin:first", "step:echo first", "close:false", "observe:first",
        "begin:independent", "step:echo independent", "close:false", "observe:independent",
    ]);
    assert_eq!(executor.observations, report.jobs);
}
