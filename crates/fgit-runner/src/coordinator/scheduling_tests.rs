...
fn execution_guard_refuses_unready_jobs_without_any_responsibility() {
    ...
    assert!(matches!(c.require_ready_job(run, "test"), Err(CoordinatorRefusal::InvalidStateTransition { .. })));
    assert!(matches!(c.require_ready_job(run, "missing"), Err(CoordinatorRefusal::JobNotFound(_))));
...
fn failed_dependency_closes_skip_chain_but_runs_failure_diagnostics() {
    ...
    assert_eq!(job(&c, run, "test"), JobStatus::Terminal(JobOutcome::Skipped));
    assert_eq!(job(&c, run, "ship"), JobStatus::Terminal(JobOutcome::Skipped));
    assert_eq!(c.eligible_jobs(run).unwrap(), vec!["diagnostics"]);
    finish(&mut c, run, "diagnostics", JobOutcome::Succeeded);
    assert_eq!(c.active_runs[&run].status, RunStatus::Terminal(RunOutcome::Failed { failed_jobs: vec!["build".to_owned()] }));
    let facts = c.drain_check_facts();
    assert_eq!(facts.iter().filter(|fact| fact.status == CheckRunStatus::Completed).count(), 4);
    assert!(facts.iter().filter(|fact| fact.conclusion == Some(CheckRunConclusion::Neutral)).all(|fact| fact.receipt_commitment.is_none()));
...
fn successful_dependency_skips_failure_only_job_and_allows_the_chain() {
    ...
    assert_eq!(job(&c, run, "diagnostics"), JobStatus::Terminal(JobOutcome::Skipped));
    ...
    assert_eq!(c.active_runs[&run].status, RunStatus::Terminal(RunOutcome::Succeeded));
}
...
fn terminal_job_cannot_run_again_while_another_job_remains_ready() {
    ...
    assert!(matches!(c.require_ready_job(run, "build"), Err(CoordinatorRefusal::InvalidStateTransition { .. })));
    finish(&mut c, run, "build", JobOutcome::Failed);
    c.settle_skipped_jobs(run, 300);
    assert_eq!(before, c.outbox_facts);
    assert_eq!(job(&c, run, "build"), JobStatus::Terminal(JobOutcome::Succeeded));
}

#[test]
fn always_can_follow_ordinary_failure_but_not_lost_containment() {
    for outcome in [JobOutcome::Failed, JobOutcome::Refused, JobOutcome::Cancelled] {
        let mut c = coordinator();
        let mut g = graph();
        g.jobs.iter_mut().find(|j| j.id == "diagnostics").unwrap().condition = Condition::Always;
...
fn root_false_condition_settles_at_enqueue_without_a_runner() {
    ...
    assert_eq!(c.outbox_facts[1].conclusion, Some(CheckRunConclusion::Neutral));
}
...
fn mutated_graphs_refuse_before_outbox_or_idempotency_state() {
    ...
    for g in [duplicate, cycle, empty, missing_steps, huge] {
        let mut c = coordinator();
        let result = c.enqueue_run(TenantId::from_bytes([1; 16]), RepositoryId::from_bytes([2; 16]),
            Commitment::of_bytes(b"head"), GitOid::Sha1(GitOidSha1::from_bytes([3; 20])),
            g, TriggerContext::trusted_push("alice"), 1, 100);
        assert!(matches!(result, Err(CoordinatorRefusal::WorkflowRefusal(_))));
...
fn concurrency_capacity_includes_draining_jobs() {
    ...
    let mut g = graph();
    g.jobs.iter_mut().find(|j| j.id == "diagnostics").unwrap().needs.clear();
    g.jobs.iter_mut().find(|j| j.id == "diagnostics").unwrap().condition = Condition::Always;
    let run = enqueue(&mut c, g);
    assert_eq!(c.eligible_jobs(run).unwrap(), vec!["build"]);
    c.active_runs.get_mut(&run).unwrap().job_statuses.insert("build".to_owned(), JobStatus::Draining {
...
fn terminal_outcomes_survive_recovery_and_late_finalization() {
    let mut c = coordinator();
    let run = enqueue(&mut c, graph());
    c.cancel_run(run, CancellationReason::UserRequested).unwrap();
    let before = c.active_runs[&run].status.clone();
    c.check_and_finalize_run(run);
    assert!(c.recover_from_crash(Commitment::of_bytes(b"new-head")).is_empty());
