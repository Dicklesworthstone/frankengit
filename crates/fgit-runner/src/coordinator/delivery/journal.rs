...
impl CheckJournalScope {
    fn bytes(self) -> Vec<u8> {
        ...
        root(&mut bytes, self.journal_id);
        bytes
    }
}
...
struct StoredBatch {
    frame: Frame,
    delivered: Option<(Commitment, Frame)>,
}
...
impl FileCheckJournal {
    ...
    pub fn open(
        ...
        if header.as_slice() != scope.bytes().as_slice() {
        ...
        while journal.pin.bytes < end {
            ...
            if journal.records >= limits.records {
                return Err(CheckDeliveryRefusal::JournalFull);
            }
            ...
            if hash.digest().bytes().as_bytes() != expected.as_slice() {
                return Err(CheckDeliveryRefusal::CorruptJournal);
            }
            let frame = Frame {
                offset,
                length: count,
                previous: journal.pin.tail,
                hash,
            };
            journal.replay(&payload, frame)?;
            journal.pin = CheckJournalPin {
                bytes: next,
                tail: hash,
            };
            journal.records += 1;
    ...
    fn empty(file: File, scope: CheckJournalScope, limits: CheckJournalLimits) -> Self {
        Self {
            ...
            pin: CheckJournalPin {
                bytes: HEADER_BYTES,
                tail: Commitment::of_bytes(&scope.bytes()),
            },
            records: 0,
            failed: false,
            batches: BTreeMap::new(),
            batch_order: BTreeMap::new(),
    ...
    pub fn store_evidence(
        ...
        self.healthy()?;
        if bytes.is_empty() || bytes.len() > self.limits.evidence_bytes {
        ...
        if self.evidence.contains_key(&expected) {
            if self.read_evidence(expected)? != bytes {
                return Err(CheckDeliveryRefusal::AcknowledgementMismatch);
            }
            return Ok(());
        }
    ...
    pub fn read_evidence(&mut self, id: Commitment) -> Result<Vec<u8>, CheckDeliveryRefusal> {
        ...
        if payload.first() != Some(&3)
            || payload.len() < 34
            || Commitment::of_bytes(&payload[33..]) != id
        {
            self.failed = true;
            return Err(CheckDeliveryRefusal::CorruptJournal);
        }
    ...
    pub fn next_batch(&mut self) -> Result<Option<CheckDeliveryBatch>, CheckDeliveryRefusal> {
        ...
        for root in batch
            .facts
            .iter()
            .filter_map(|fact| fact.receipt_commitment)
            .collect::<BTreeSet<_>>()
        {
    ...
    pub fn record_delivery(
        ...
        let entry = self
            .batches
            .get(&acknowledgement.batch)
            .ok_or(CheckDeliveryRefusal::StaleBatch)?;
        if let Some((receipt, frame)) = entry.delivered {
            if receipt != acknowledgement.receipt {
                return Err(CheckDeliveryRefusal::AcknowledgementMismatch);
            }
            self.read_frame(frame)?;
            return Ok(());
        }
        if self.pending.front() != Some(&acknowledgement.batch) {
            return Err(CheckDeliveryRefusal::OutOfOrder);
        }
        let mut payload = vec![2];
        root(&mut payload, acknowledgement.batch);
        root(&mut payload, acknowledgement.receipt);
        let frame = self.append(&payload, self.pending.len() - 1)?;
        self.batches
            .get_mut(&acknowledgement.batch)
            .expect("validated batch")
            .delivered = Some((acknowledgement.receipt, frame));
    ...
    pub fn forward_next<S: CheckDeliverySink>(
        &mut self,
        destination: &mut S,
        live: &dyn Fn() -> bool,
    ...
    fn check_capacity(
        ...
        let reserve = (pending as u64)
            .checked_mul(DELIVERED_BYTES)
            .ok_or(CheckDeliveryRefusal::JournalFull)?;
        if self
            .pin
            .bytes
            .checked_add(bytes)
            .and_then(|n| n.checked_add(reserve))
            .is_none_or(|n| n > self.limits.journal_bytes)
            || self
                .records
                .checked_add(records)
                .and_then(|n| n.checked_add(pending))
                .is_none_or(|n| n > self.limits.records)
        {
            return Err(CheckDeliveryRefusal::JournalFull);
        }
        Ok(())
    ...
    fn check_batch(&self, batch: &CheckDeliveryBatch) -> Result<(), CheckDeliveryRefusal> {
        if batch.tenant != self.scope.tenant || batch.repository != self.scope.repository {
            return Err(CheckDeliveryRefusal::ScopeMismatch);
        }
        if self
            .bindings
            .get(&batch.run)
            .is_some_and(|binding| *binding != Binding::of(batch))
        {
            return Err(CheckDeliveryRefusal::ScopeMismatch);
        }
        if batch
            .ordinal
            .checked_add(batch.facts.len() as u64)
            .is_none()
            || self
                .run_ends
                .get(&batch.run)
                .is_some_and(|end| batch.ordinal < *end)
        {
        ...
        for fact in &batch.facts {
            ...
            if fact
                .receipt_commitment
                .is_some_and(|id| !self.evidence.contains_key(&id))
            {
    ...
    fn index_batch(&mut self, batch: &CheckDeliveryBatch, frame: Frame) {
        ...
        self.bindings
            .entry(batch.run)
            .or_insert_with(|| Binding::of(batch));
        self.run_ends
            .insert(batch.run, batch.ordinal + batch.facts.len() as u64);
        for fact in &batch.facts {
            self.phases
                .insert((batch.run, fact.job_id.clone()), phase(fact.status));
        }
        ...
        match payload.first() {
            Some(1) => {
                let batch = CheckDeliveryBatch::decode(&payload[1..])
                    .map_err(|_| CheckDeliveryRefusal::CorruptJournal)?;
            ...
            Some(2) if payload.len() == 65 => {
                let mut input = Input(&payload[1..]);
                let id = input.root()?;
                let receipt = input.root()?;
                if self.pending.front() != Some(&id) {
                    return Err(CheckDeliveryRefusal::CorruptJournal);
                }
                self.batches
                    .get_mut(&id)
                    .ok_or(CheckDeliveryRefusal::CorruptJournal)?
                    .delivered = Some((receipt, frame));
                self.pending.pop_front();
            }
            Some(3) if payload.len() > 33 && payload.len() - 33 <= self.limits.evidence_bytes => {
                let id = Input(&payload[1..33]).root()?;
                if Commitment::of_bytes(&payload[33..]) != id || self.evidence.contains_key(&id) {
                    return Err(CheckDeliveryRefusal::CorruptJournal);
                }
    ...
    fn append(
        ...
        payload: &[u8],
        pending_after: usize,
    ) -> Result<Frame, CheckDeliveryRefusal> {
        self.healthy()?;
        ...
        let frame = Frame {
            ...
            hash: frame_hash(self.pin.tail, payload),
        };
        ...
        write_frame(&mut self.file, payload, frame.hash, |file| file.sync_all())
            .map_err(storage)?;
        self.pin = CheckJournalPin {
            bytes: self.pin.bytes + bytes,
            tail: frame.hash,
        };
        self.records += 1;
        self.failed = false;
    ...
    fn read_frame(&mut self, frame: Frame) -> Result<Vec<u8>, CheckDeliveryRefusal> {
        ...
        if frame_hash(frame.previous, &payload) != frame.hash
...
impl CheckDeliverySink for FileCheckJournal {
    fn accept(
        ...
        if let Some(stored) = self.batches.get(&batch.id()) {
            ...
            return Ok(CheckDeliveryAcknowledgement::after_durable_acceptance(
                batch, frame.hash,
            ));
        }
        self.check_batch(batch)?;
        ...
        payload.push(1);
        payload.extend_from_slice(batch.body());
        let frame = self.append(&payload, self.pending.len() + 1)?;
        self.index_batch(batch, frame);
...
impl WorkflowCoordinator {
    ...
    pub fn journal_check_facts(
        ...
        maximum_facts: usize,
        maximum_bytes: usize,
        trusted: Option<&TrustedWorkflowReceipt>,
        live: &dyn Fn() -> bool,
    ) -> Result<Option<CheckDeliveryAcknowledgement>, CheckDeliveryRefusal> {
        if !live() {
            return Err(CheckDeliveryRefusal::Cancelled);
        ...
        for fact in &batch.facts {
            let Some(expected) = fact.receipt_commitment else {
                continue;
            };
            if !live() {
                return Err(CheckDeliveryRefusal::Cancelled);
            }
            if journal.evidence.contains_key(&expected) {
                continue;
            }
            let bytes = match batch.profile {
                ...
                CoordinatorExecutionProfile::TrustedWorkflow { .. } => trusted
                    .filter(|receipt| receipt.run_id() == batch.run)
...
fn storage(_: io::Error) -> CheckDeliveryRefusal {
    CheckDeliveryRefusal::StorageUnavailable
}
