...
impl Directory {
    fn new() -> Self {
        let path = std::env::temp_dir().join(format!("fgit-history-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
...
fn digest(text: &str) -> Commitment { Commitment::of_bytes(text.as_bytes()) }
fn scope() -> CheckJournalScope {
    CheckJournalScope { tenant: TenantId::from_bytes([1; 16]), repository: RepositoryId::from_bytes([2; 16]), journal_id: digest("journal") }
}
fn batch(job: &str, ordinal: u64, completed: bool, evidence: Option<Commitment>) -> CheckDeliveryBatch {
    let run = WorkflowRunId(digest("run"));
    let mut batch = CheckDeliveryBatch {
        tenant: scope().tenant, repository: scope().repository, run,
        attempt: AttemptId::derive(run, 1), head: digest("head"),
        source: GitOid::Sha1(fgit_types::GitOidSha1::from_bytes([3; 20])),
        graph: digest("graph"), trust: TrustDomain::new(RunnerText::parse("trust", "fixture").unwrap()),
        profile: CoordinatorExecutionProfile::CommandOnly, ordinal,
        facts: vec![CheckRunFact {
            run_id: run, job_id: job.to_owned(),
            status: if completed { CheckRunStatus::Completed } else { CheckRunStatus::Queued },
            conclusion: completed.then_some(CheckRunConclusion::Failure),
            receipt_commitment: evidence, timestamp_millis: ordinal,
        }], body: Vec::new(),
    };
...
fn deliver(journal: &mut FileCheckJournal, batch: &CheckDeliveryBatch) {
    journal.record_delivery(CheckDeliveryAcknowledgement::after_durable_acceptance(batch, digest("downstream"))).unwrap();
}
fn page(journal: &mut FileCheckJournal) -> CheckHistoryPage {
    journal.read_history(None, None, MAX_HISTORY_BATCHES, MAX_HISTORY_BYTES, &|| true).unwrap()
}
...
fn delivered_results_remain_readable_without_reopening_pending_custody() {
    ...
    assert_eq!(history.entries()[0].delivery_receipt(), Some(digest("downstream")));
...
fn pages_follow_acceptance_order_not_commitment_sort_order() {
    let directory = Directory::new();
    let mut journal = directory.journal();
    let batches = (0..7).map(|n| accept(&mut journal, &format!("job-{n}"), n)).collect::<Vec<_>>();
    let mut after = None;
    let mut expected = None;
    let mut seen = Vec::new();
    loop {
        let history = journal.read_history(expected, after, 2, MAX_HISTORY_BYTES, &|| true).unwrap();
    ...
    assert_eq!(seen, batches.iter().map(CheckDeliveryBatch::id).collect::<Vec<_>>());
...
fn empty_and_final_pages_do_not_fabricate_continuations() {
    ...
    let last = journal.read_history(None, None, 1, MAX_HISTORY_BYTES, &|| true).unwrap();
    assert_eq!(last.next_after(), None);
    let end = journal.read_history(Some(last.snapshot()), Some(first.id()), 1, MAX_HISTORY_BYTES, &|| true).unwrap();
...
fn byte_budget_is_exact_including_delivery_receipt() {
    ...
    assert_eq!(journal.read_history(None, None, 8, size - 1, &|| true), Err(CheckDeliveryRefusal::BatchTooLarge));
}
...
fn append_or_acknowledgement_invalidates_a_paging_snapshot() {
    ...
    let pin = journal.pin();
    accept(&mut journal, "second", 1);
    assert_eq!(journal.read_history(Some(pin), Some(first.id()), 1, MAX_HISTORY_BYTES, &|| true), Err(CheckDeliveryRefusal::StaleBatch));
    let pin = journal.pin();
    deliver(&mut journal, &first);
    assert_eq!(journal.read_history(Some(pin), None, 1, MAX_HISTORY_BYTES, &|| true), Err(CheckDeliveryRefusal::StaleBatch));
}
...
fn unpinned_or_foreign_cursors_and_invalid_limits_refuse() {
    ...
    let mut journal = directory.journal();
    let first = accept(&mut journal, "first", 0);
    assert_eq!(journal.read_history(None, Some(first.id()), 1, 1024, &|| true), Err(CheckDeliveryRefusal::StaleBatch));
    assert_eq!(journal.read_history(Some(journal.pin()), Some(digest("unknown")), 1, 1024, &|| true), Err(CheckDeliveryRefusal::StaleBatch));
    for (count, bytes) in [(0, 1024), (MAX_HISTORY_BATCHES + 1, 1024), (1, 0), (1, MAX_HISTORY_BYTES + 1)] {
        assert_eq!(journal.read_history(None, None, count, bytes, &|| true), Err(CheckDeliveryRefusal::InvalidLimits));
    }
...
fn restart_retains_delivered_history_and_the_same_page_identity() {
    ...
    drop(journal);
    let mut reopened = FileCheckJournal::open(&directory.file(), scope(), CheckJournalLimits::default(), Some(expected.snapshot()), &|| true).unwrap();
...
fn evidence_selection_requires_membership_in_the_selected_batch() {
    ...
    journal.store_evidence(digest("evidence"), b"evidence").unwrap();
    ...
    assert_eq!(journal.read_batch_evidence(completed.id(), digest("evidence")).unwrap(), b"evidence");
    assert_eq!(journal.read_batch_evidence(first.id(), digest("evidence")), Err(CheckDeliveryRefusal::EvidenceMissing));
    assert_eq!(journal.read_batch_evidence(completed.id(), digest("orphan")), Err(CheckDeliveryRefusal::EvidenceMissing));
    assert_eq!(journal.read_retained_batch(digest("unknown")), Err(CheckDeliveryRefusal::StaleBatch));
}
...
fn cancellation_returns_no_partial_page_and_leaves_custody_unchanged() {
    ...
    assert_eq!(journal.read_history(None, None, 8, MAX_HISTORY_BYTES, &|| {
...
fn corrupt_delivered_proposal_or_acknowledgement_cannot_hide_in_history() {
    for corrupt_ack in [false, true] {
        ...
        let frame = if corrupt_ack { entry.delivered.unwrap().1 } else { entry.frame };
        let mut bytes = fs::read(directory.file()).unwrap();
        bytes[frame.offset as usize + 4] ^= 1;
        fs::write(directory.file(), bytes).unwrap();
        assert_eq!(journal.read_retained_batch(first.id()), Err(CheckDeliveryRefusal::CorruptJournal));
        assert!(journal.is_failed());
    }
}
...
fn empty_history_rechecks_its_on_disk_scope_and_length() {
    ...
    bytes[0] ^= 1;
    fs::write(directory.file(), bytes).unwrap();
    assert_eq!(journal.read_history(None, None, 1, 1024, &|| true), Err(CheckDeliveryRefusal::CorruptJournal));
    assert!(journal.is_failed());
}
