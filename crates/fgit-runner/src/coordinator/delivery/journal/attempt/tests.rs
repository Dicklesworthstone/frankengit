...
fn scope() -> CheckJournalScope {
    CheckJournalScope {
        ...
        journal_id: Commitment::of_bytes(b"attempt-tests"),
    }
}
fn fixture_binding() -> WorkflowAttemptBinding {
    WorkflowAttemptBinding {
        scope: scope(),
        run: WorkflowRunId(Commitment::of_bytes(b"run")),
        attempt: AttemptId(Commitment::of_bytes(b"attempt")),
        request: Commitment::of_bytes(b"request"),
    }
}
fn header_pin() -> CheckJournalPin {
    CheckJournalPin::new(HEADER_BYTES, Commitment::of_bytes(&scope().bytes()))
}
...
fn header_and_start_hash_match_independent_python_golden() {
    ...
    assert_eq!(binding.bytes().len(), 168);
    assert_eq!(
        Commitment::of_bytes(&binding.bytes())
            .digest()
            .bytes()
            .as_bytes(),
        &HEADER_GOLDEN
    ...
    let temp = Temp::new();
    let mut owner = FileWorkflowAttempt::create(&temp.attempt(), binding).unwrap();
    owner.start(header_pin()).unwrap();
    assert_eq!(
        owner.pin().tail().digest().bytes().as_bytes(),
        &START_GOLDEN
    );
}
...
fn started_is_irreversible_after_reopen_and_does_not_grant_retry() {
    ...
    drop(owner);
    let mut owner =
        FileWorkflowAttempt::open(&temp.attempt(), binding, Some(pin), &|| true).unwrap();
    assert_eq!(owner.status(), WorkflowAttemptStatus::Started);
    assert_eq!(
        owner.start(header_pin()).err(),
        Some(WorkflowAttemptRefusal::ReconciliationRequired)
    );
    assert_eq!(owner.pin(), pin);
...
fn completed_bytes_and_containment_flag_survive_lost_response() {
    ...
    owner.start(header_pin()).unwrap();
    owner
        .finish_bytes(b"local-observation-fixture", true, header_pin())
        .unwrap();
    let pin = owner.pin();
    drop(owner);
    let mut owner =
    ...
    assert_eq!(receipt.frame(), b"local-observation-fixture");
...
fn wrong_request_or_journal_instance_cannot_adopt_an_attempt() {
    let temp = Temp::new();
    let binding = fixture_binding();
    drop(FileWorkflowAttempt::create(&temp.attempt(), binding).unwrap());
    for other in [
        WorkflowAttemptBinding {
            request: Commitment::of_bytes(b"changed"),
            ..binding
        },
        WorkflowAttemptBinding {
            scope: CheckJournalScope {
                journal_id: Commitment::of_bytes(b"other"),
                ..scope()
            },
            ..binding
        },
        ...
        assert_eq!(
            FileWorkflowAttempt::open(&temp.attempt(), other, None, &|| true).err(),
            Some(WorkflowAttemptRefusal::IdentityMismatch)
        );
    }
}
...
fn lock_no_overwrite_and_missing_file_are_fail_closed() {
    ...
    assert_eq!(
        FileWorkflowAttempt::open(&temp.attempt(), binding, None, &|| true).err(),
        Some(WorkflowAttemptRefusal::Custody(
            CheckDeliveryRefusal::LockUnavailable
        ))
    );
    drop(owner);
    assert!(FileWorkflowAttempt::open(&temp.attempt(), binding, None, &|| true).is_ok());
}
...
fn unsafe_file_kinds_and_permissions_are_not_adopted() {
    ...
    std::os::unix::fs::symlink(temp.attempt(), &alias).unwrap();
    assert!(FileWorkflowAttempt::open(&alias, binding, None, &|| true).is_err());
    fs::remove_file(&alias).unwrap();
    fs::hard_link(temp.attempt(), &alias).unwrap();
    assert!(FileWorkflowAttempt::open(&temp.attempt(), binding, None, &|| true).is_err());
    fs::remove_file(&alias).unwrap();
...
fn rollback_to_valid_prepared_prefix_requires_the_independent_witness() {
    ...
    assert_eq!(
        FileWorkflowAttempt::open(&temp.attempt(), binding, Some(pin), &|| true).err(),
        Some(WorkflowAttemptRefusal::Custody(
            CheckDeliveryRefusal::CorruptJournal
        ))
    );
    assert_eq!(
        FileWorkflowAttempt::open(&temp.attempt(), binding, None, &|| true)
            .unwrap()
            .status(),
...
fn every_nonboundary_torn_prefix_is_refused_without_modifying_it() {
    ...
    let mut owner = FileWorkflowAttempt::create(&temp.attempt(), binding).unwrap();
    owner.start(header_pin()).unwrap();
    let start = owner.pin().byte_len() as usize;
    owner
        .finish_bytes(b"fixture result", false, header_pin())
        .unwrap();
    drop(owner);
    let full = fs::read(temp.attempt()).unwrap();
    let path = temp.0.join("damaged");
    for end in 0..full.len() {
        if [168, start].contains(&end) {
            continue;
        }
        write_fixture(&path, &full[..end]);
        assert_eq!(
            FileWorkflowAttempt::open(&path, binding, None, &|| true).err(),
            Some(WorkflowAttemptRefusal::Custody(
                CheckDeliveryRefusal::CorruptJournal
            )),
            "prefix {end}"
...
fn readback_corruption_poisons_the_open_owner() {
    let temp = Temp::new();
    let mut owner = FileWorkflowAttempt::create(&temp.attempt(), fixture_binding()).unwrap();
    owner.start(header_pin()).unwrap();
    owner
        .finish_bytes(b"fixture result", false, header_pin())
        .unwrap();
    let mut bytes = fs::read(temp.attempt()).unwrap();
    let at = bytes.len() - 33;
    bytes[at] ^= 1;
    write_fixture(&temp.attempt(), &bytes);
    assert!(owner.completed_receipt().is_err());
    assert!(owner.is_failed());
    assert_eq!(
        owner.completed_receipt().err(),
        Some(WorkflowAttemptRefusal::Custody(
            CheckDeliveryRefusal::FailedJournal
        ))
    );
}
...
fn duplicate_start_and_unknown_completion_tag_refuse_even_with_valid_hashes() {
    for bad in [1, 3] {
        ...
        let mut owner = FileWorkflowAttempt::create(&temp.attempt(), binding).unwrap();
        owner.start(header_pin()).unwrap();
        let previous = owner.pin().tail();
        drop(owner);
        let mut payload = pin_payload(bad, header_pin());
        if bad == 3 {
            payload.push(0);
        ...
        let mut file = OpenOptions::new()
            ...
            .open(temp.attempt())
            .unwrap();
        write_frame(&mut file, &payload, attempt_hash(previous, &payload), |f| {
            f.sync_all()
        })
        .unwrap();
        drop(file);
...
fn full_execution_replays_after_discarding_all_in_memory_handles_without_work() {
    ...
    let mut p = enqueue(&mut c, 1, SOURCE);
    let binding = c.trusted_attempt_binding(&p, scope(), 200).unwrap();
    let mut journal =
        FileCheckJournal::create(&temp.journal(), scope(), CheckJournalLimits::default()).unwrap();
    let mut owner = FileWorkflowAttempt::create(&temp.attempt(), binding).unwrap();
    let mut executor = Executor::new(temp.attempt());
    let receipt = c
        .execute_trusted_workflow_durable(
            &mut p,
            &mut executor,
            &mut journal,
            &mut owner,
    ...
    assert_eq!(executor.closes, 2);
    assert_eq!(receipt.frame(), p.receipt().unwrap().frame());
...
fn unknown_started_attempt_blocks_rerun_and_fences_other_work_on_the_coordinator() {
    let temp = Temp::new();
    let mut c = coordinator();
    let p = enqueue(&mut c, 1, SOURCE);
    let binding = c.trusted_attempt_binding(&p, scope(), 200).unwrap();
    let mut journal =
        FileCheckJournal::create(&temp.journal(), scope(), CheckJournalLimits::default()).unwrap();
    let mut owner = FileWorkflowAttempt::create(&temp.attempt(), binding).unwrap();
    owner.start(journal.pin()).unwrap();
    drop(owner);
    drop(p);
    drop(c);
    let mut c = coordinator();
    ...
    assert_eq!(
        c.execute_trusted_workflow_durable(
            &mut p,
            &mut executor,
            &mut journal,
            &mut owner,
    ...
    assert_eq!(executor.starts, 0);
    let other = enqueue(&mut c, 2, SOURCE);
...
fn completed_outcome_refuses_a_recreated_or_rolled_back_custody_journal() {
    ...
    .unwrap();
    drop(journal);
    let mut empty = FileCheckJournal::create(
        &temp.0.join("recreated"),
        scope(),
        CheckJournalLimits::default(),
    )
    .unwrap();
...
fn retained_in_process_receipt_can_finish_started_without_reexecution() {
    ...
    owner.start(journal.pin()).unwrap();
    let mut executor = Executor::new(temp.attempt());
    c.execute_trusted_workflow_journaled(&mut p, &mut executor, &mut journal, 200, &|| true)
        .unwrap();
    let receipt = c
        .execute_trusted_workflow_durable(
            &mut p,
...
fn checkpoint_ancestry_is_verified_even_after_later_downstream_acknowledgements() {
    ...
    while let Some(batch) = journal.next_batch().unwrap() {
        journal
            .record_delivery(CheckDeliveryAcknowledgement::after_durable_acceptance(
                &batch,
                Commitment::of_bytes(b"fixture-sink"),
            ))
            .unwrap();
    }
    ...
    assert!(
        journal
            .verify_checkpoint(CheckJournalPin::new(
                receipt.journal_pin().byte_len(),
                Commitment::of_bytes(b"wrong-prefix")
            ))
            .is_err()
    );
}
...
fn changed_actor_limits_source_or_logical_time_changes_the_attempt_binding() {
    let mut c = coordinator();
    let p = enqueue(&mut c, 1, SOURCE);
    let original = c.trusted_attempt_binding(&p, scope(), 200).unwrap();
    assert_ne!(
        original,
        c.trusted_attempt_binding(&p, scope(), 201).unwrap()
    );
    c.active_runs
        .get_mut(&p.run_id())
        .unwrap()
    ...
    assert_ne!(
        original,
        c.trusted_attempt_binding(&p, scope(), 200).unwrap()
    );
    let mut c2 = coordinator();
    let p2 = enqueue(&mut c2, 1, &format!("# different raw source\n{SOURCE}"));
    assert_ne!(
        original,
        c2.trusted_attempt_binding(&p2, scope(), 200).unwrap()
    );
    ...
    assert_ne!(
        original,
        c3.trusted_attempt_binding(&p3, scope(), 200).unwrap()
    );
}
...
fn real_trusted_shell_side_effect_is_not_repeated_after_disk_reopen() {
        ...
        fn execute_step(
            ...
            let open = |name: &str| {
                OpenOptions::new()
                    ...
                    .create_new(true)
                    .mode(0o600)
                    .open(self.directory.join(format!("{name}-{index}")))
                    .unwrap()
            };
    ...
    .unwrap();
    assert_eq!(fs::read(temp.0.join("marker")).unwrap(), b"xx");
    drop(owner);
    drop(p);
    drop(c);
    drop(journal);
...
const HEADER_GOLDEN: [u8; 32] = [
    0xd3, 0xa1, 0x40, 0x30, 0x69, 0xa4, 0xb5, 0xfb, 0x76, 0xe8, 0x5d, 0x8e, 0xc4, 0x04, 0x21, 0x59,
    0x06, 0xc1, 0x1a, 0xca, 0x9b, 0x7e, 0x22, 0xaf, 0x1a, 0x84, 0x3d, 0x74, 0x17, 0xb5, 0xbc, 0xe3,
];
const START_GOLDEN: [u8; 32] = [
    0x33, 0x82, 0xc3, 0x04, 0x16, 0x2a, 0xe3, 0xc0, 0xce, 0x2b, 0xa2, 0x06, 0xa8, 0x6d, 0x2f, 0x94,
    0x14, 0x4e, 0x3f, 0xe5, 0x2f, 0xe7, 0xd2, 0x94, 0x0b, 0x59, 0x70, 0x90, 0x34, 0x2b, 0xda, 0x18,
];
