...
use crate::workflow::{JobOutcome, WorkflowExecutor, MAX_JOBS};
...
impl WorkflowCoordinator {
    ...
    pub fn execute_journaled_trusted_workflow<'a, E: WorkflowExecutor>(
        ...
        let mut custody = JournalCustody { journal, run_id, live, failure: None };
        // Reject wrong scopes before pinning the prepared handle's local mode.
        custody.check_scope(self).map_err(JournaledWorkflowRefusal::Custody)?;
        let result = self.execute_trusted_workflow_with_launch_custody(
            prepared, executor, logical_now, live, &mut |_, _| Ok(()), Some(&mut custody),
        );
        ...
        custody.finish(self, receipt).map_err(JournaledWorkflowRefusal::Custody)?;
        Ok(receipt)
    }
}
...
impl JournalCustody<'_> {
    fn remember(&mut self, result: Result<(), CheckDeliveryRefusal>) -> Result<(), CheckDeliveryRefusal> {
        if let Err(error) = result { self.failure.get_or_insert(error); }
        result
    }

    fn check_scope(&self, coordinator: &WorkflowCoordinator) -> Result<(), CheckDeliveryRefusal> {
        self.journal.healthy()?;
        let run = coordinator.active_runs.get(&self.run_id).ok_or(CheckDeliveryRefusal::UnknownRun)?;
        if run.tenant != self.journal.scope.tenant || run.repository != self.journal.scope.repository {
            return Err(CheckDeliveryRefusal::ScopeMismatch);
        }
        if coordinator.outbox_facts.iter().any(|fact| fact.run_id != self.run_id) {
            return Err(CheckDeliveryRefusal::OutOfOrder);
        }
        let binding = Binding {
            attempt: run.attempt_id, head: run.authority_head, source: run.source_commit,
            graph: run.graph_id, trust: run.trigger_ctx.trust_domain.clone(), profile: run.execution_profile,
        };
        if self.journal.bindings.get(&self.run_id).is_some_and(|known| *known != binding) {
            return Err(CheckDeliveryRefusal::ScopeMismatch);
        }
        Ok(())
    }
    ...
    fn flush_pending(&mut self, coordinator: &mut WorkflowCoordinator,
        receipt: Option<&TrustedWorkflowReceipt>, cancellable: bool)
        -> Result<(), CheckDeliveryRefusal>
    {
        ...
        for _ in 0..=MAX_JOBS * 3 {
            ...
            if coordinator.journal_check_facts(
                self.journal, 1, MAX_BATCH_BYTES, receipt, &keep_settling,
            )?.is_none() { return Ok(()); }
        }
    ...
    fn start(&mut self, coordinator: &mut WorkflowCoordinator) -> Result<(), CheckDeliveryRefusal> {
        self.check_scope(coordinator)?;
        if !(self.live)() { return Err(CheckDeliveryRefusal::Cancelled); }
        ...
        if self.journal.phases.iter().any(|((run, _), phase)| *run == self.run_id && *phase != 1) {
        ...
        if self.journal.phases.iter().any(|((run, _), phase)| *run != self.run_id && *phase == 2) {
            return Err(CheckDeliveryRefusal::OutOfOrder);
        }
        ...
        if run.graph.jobs.iter().any(|job| {
            self.journal.phases.get(&(self.run_id, job.id.clone())) != Some(&1)
        }) {
    ...
    fn finish(&mut self, coordinator: &mut WorkflowCoordinator, receipt: &TrustedWorkflowReceipt)
        -> Result<(), CheckDeliveryRefusal>
    {
        ...
        loop {
            let next = match cursor {
                ...
                Some(id) => self.journal.batches.range((std::ops::Bound::Excluded(id), std::ops::Bound::Unbounded)).next(),
            }.map(|(id, stored)| (*id, stored.frame));
            let Some((id, frame)) = next else { break; };
            cursor = Some(id);
            let payload = self.journal.read_frame(frame)?;
            if payload.first() != Some(&1) { return Err(CheckDeliveryRefusal::CorruptJournal); }
            let batch = CheckDeliveryBatch::decode(&payload[1..])?;
            if batch.id() != id { return Err(CheckDeliveryRefusal::CorruptJournal); }
            if batch.run_id() != self.run_id { continue; }
            for fact in batch.facts().iter().filter(|fact| fact.status == CheckRunStatus::Completed) {
                if completed.len() >= MAX_JOBS || completed.insert(fact.job_id.clone(), fact.clone()).is_some() {
                    return Err(CheckDeliveryRefusal::InvalidBatch);
                }
            }
        }
        if completed.len() != receipt.report().jobs.len() { return Err(CheckDeliveryRefusal::EvidenceMissing); }
        for job in &receipt.report().jobs {
            let expected = receipt.job_commitment(&job.id).ok_or(CheckDeliveryRefusal::EvidenceMissing)?;
            let fact = completed.get(&job.id).ok_or(CheckDeliveryRefusal::EvidenceMissing)?;
...
impl WorkflowCustody for JournalCustody<'_> {
    fn identity(&self) -> Commitment { Commitment::of_bytes(&self.journal.scope.bytes()) }

    fn before_run(&mut self, coordinator: &mut WorkflowCoordinator, run: WorkflowRunId)
        -> Result<(), CheckDeliveryRefusal>
    {
        let result = if run == self.run_id { self.start(coordinator) }
            else { Err(CheckDeliveryRefusal::ScopeMismatch) };
        self.remember(result)
    }

    fn flush(&mut self, coordinator: &mut WorkflowCoordinator, receipt: Option<&TrustedWorkflowReceipt>)
        -> Result<(), CheckDeliveryRefusal>
    {
