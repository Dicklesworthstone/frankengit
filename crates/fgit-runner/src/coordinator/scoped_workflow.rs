...
use super::*;
use crate::workflow::{
    JobReport, StepLimits, WorkerFailure, WorkflowExecutor, WorkflowLimits,
    WorkflowPlan, WorkflowReport,
};
...
pub enum CoordinatorExecutionProfile {
    CommandOnly,
    TrustedWorkflow { source: Commitment, limits: WorkflowLimits },
}
...
impl PreparedTrustedWorkflow {
    pub const fn run_id(&self) -> WorkflowRunId { self.run_id }
    pub const fn limits(&self) -> WorkflowLimits { self.limits }
    pub const fn receipt(&self) -> Option<&TrustedWorkflowReceipt> { self.receipt.as_ref() }
}
...
impl ObservationBinding {
    fn from_run(run: &ActiveRun) -> Self {
        Self {
            run: run.id, attempt: run.attempt_id, tenant: run.tenant, repository: run.repository,
            head: run.authority_head, source: run.source_commit, trust: run.trigger_ctx.trust_domain.clone(),
        }
    }
}
...
impl TrustedWorkflowReceipt {
    pub const fn run_id(&self) -> WorkflowRunId { self.binding.run }
    pub const fn report(&self) -> &WorkflowReport { &self.report }

    /// Versioned local-observation bytes; not a new canonical repository schema.
    pub fn frame(&self) -> Vec<u8> {
        observation_frame(&self.binding, &self.report, &self.attempts, self.logical_now)
    }
...
fn observation_frame(
    binding: &ObservationBinding, report: &WorkflowReport,
    attempts: &BTreeMap<String, u32>, logical_now: u64,
) -> Vec<u8> {
    let mut bytes = OBSERVATION_DOMAIN.to_vec();
    for root in [binding.run.commitment(), binding.attempt.commitment(), binding.head] {
    ...
    bytes.push(match binding.source { GitOid::Sha1(_) => 1, GitOid::Sha256(_) => 2 });
...
impl WorkflowCoordinator {
    ...
    pub fn enqueue_trusted_workflow(
        ...
        source_commit: GitOid, plan: WorkflowPlan, mut limits: WorkflowLimits,
        trigger_ctx: TriggerContext, sequence: u64, logical_now: u64,
    ) -> Result<PreparedTrustedWorkflow, CoordinatorRefusal> {
        limits.validate().map_err(CoordinatorRefusal::WorkflowRefusal)?;
        if trigger_ctx.is_fork {
            return Err(CoordinatorRefusal::UnsupportedExecution {
                job_id: String::new(), reason: "fork workflows require a registered hostile-code executor",
            });
        }
        limits.run_timeout = limits.run_timeout.min(self.limits.run_timeout);
        limits.step_timeout = limits.step_timeout.min(self.limits.step_timeout)
            .min(limits.run_timeout).min(Duration::from_millis(self.ceilings.wall_clock_millis()));
        limits.validate().map_err(CoordinatorRefusal::WorkflowRefusal)?;
        let profile = CoordinatorExecutionProfile::TrustedWorkflow { source: plan.source_commitment(), limits };
        let run_id = self.enqueue_preflighted_run(tenant, repository, authority_head,
    ...
    pub fn execute_trusted_workflow<'a, E: WorkflowExecutor>(
        ...
        logical_now: u64, live: &dyn Fn() -> bool,
        ...
        let expected = CoordinatorExecutionProfile::TrustedWorkflow {
            source: prepared.plan.source_commitment(), limits: prepared.limits,
        };
        if run.execution_profile != expected || &run.graph != prepared.plan.graph()
        ...
        if prepared.attempted || !matches!(run.status, RunStatus::Queued) || !self.has_concurrency_turn(run) {
            return Err(CoordinatorRefusal::InvalidStateTransition {
        ...
        let in_flight = self.active_runs.values().flat_map(|r| r.job_statuses.values())
            .filter(|s| matches!(s, JobStatus::Running | JobStatus::Draining { .. })).count();
        if in_flight >= self.limits.max_concurrent_jobs {
            return Err(CoordinatorRefusal::QueueCapacityExceeded { limit: self.limits.max_concurrent_jobs });
        }
        ...
        let result = {
            ...
            let result = catch_unwind(AssertUnwindSafe(|| {
                prepared.plan.execute(prepared.limits, &mut adapter, live)
            }));
        ...
        let report = match result {
            ...
            Err(_) => {
                self.hold_trusted_workflow(run_id);
                return Err(CoordinatorRefusal::ContainmentFailure("trusted workflow executor unwound; reconcile its scope before reuse".to_owned()));
            }
        };
    ...
    fn hold_trusted_workflow(&mut self, run_id: WorkflowRunId) {
        ...
        if matches!(run.status, RunStatus::Terminal(_)) { return; }
        let reason = DrainReason::WorkerFailure("trusted workflow scope requires containment reconciliation".to_owned());
        run.status = RunStatus::Draining { reason: reason.clone() };
        for status in run.job_statuses.values_mut() {
            if matches!(status, JobStatus::Running) { *status = JobStatus::Draining { reason: reason.clone() }; }
        }
    }
}

struct CoordinatedExecutor<'a, E> {
...
impl<E: WorkflowExecutor> WorkflowExecutor for CoordinatedExecutor<'_, E> {
    fn begin_job(&mut self, index: usize, job: &fgit_schema::workflow::Job, live: &dyn Fn() -> bool) -> Result<(), WorkerFailure> {
        self.coordinator.require_eligible_job(self.binding.run, &job.id)
        ...
        let attempt = run.job_attempts[&job.id].checked_add(1)
        ...
        self.coordinator.outbox_facts.push(CheckRunFact {
            run_id: self.binding.run, job_id: job.id.clone(), status: CheckRunStatus::InProgress,
    ...
    fn observe_job(&mut self, report: &JobReport) {
        ...
        let envelope = WorkflowReport {
            source: self.source, graph: self.graph, limits: self.limits, jobs: Vec::new(),
        };
        let root = job_commitment(self.binding, &envelope, report, run.job_attempts[&report.id], self.logical_now);
        run.job_statuses.insert(report.id.clone(), JobStatus::Terminal(report.outcome));
        run.job_outputs.insert(report.id.clone(), report.steps.iter().map(|step| step.observation.clone()).collect());
        ...
        self.coordinator.outbox_facts.push(CheckRunFact {
            run_id: self.binding.run, job_id: report.id.clone(), status: CheckRunStatus::Completed,
            conclusion: Some(conclusion), receipt_commitment: Some(root), timestamp_millis: self.logical_now,
        });
        self.coordinator.obligations.check_publications_emitted += 1;
