...
impl WorkflowCoordinator {
    pub(super) fn enqueue_concurrency_group(
        ...
        let Some(group) = &trigger.concurrency_group else {
            return;
        };
        ...
        if group.cancel_in_progress {
            for existing_id in queue.iter() {
                let Some(run) = self.active_runs.get_mut(existing_id) else {
                    continue;
                };
    ...
    pub(super) fn has_concurrency_turn(&self, candidate: &ActiveRun) -> bool {
        let Some(group) = &candidate.concurrency_group else {
            return true;
        };
        let key = (candidate.tenant, candidate.repository, group.clone());
        let Some(queue) = self.concurrency_groups.get(&key) else {
            return false;
        };
        queue
            .iter()
...
mod tests {
    ...
    fn non_preempting_group_is_fifo_and_releases_only_terminal_predecessors() {
        ...
        assert_eq!(
            c.active_runs[&first].concurrency_group.as_deref(),
            Some("ci")
        );
        assert!(c.require_ready_job(first, "build").is_ok());
        assert!(c.require_ready_job(second, "build").is_err());
        c.cancel_run(second, CancellationReason::UserRequested)
            .unwrap();
    ...
    fn replacement_waits_for_every_preempted_run_to_drain() {
        ...
        assert!(matches!(
            c.active_runs[&first].status,
            RunStatus::Draining { .. }
        ));
        assert!(matches!(
            c.active_runs[&queued].status,
            RunStatus::Draining { .. }
        ));
    ...
    fn duplicate_trigger_is_refused_before_it_can_preempt_or_emit() {
        ...
        assert!(matches!(
            result,
            Err(CoordinatorRefusal::DuplicateIdempotencyKey(_))
        ));
    ...
    fn recovery_does_not_apply_one_repository_head_to_another_repository() {
        ...
        assert!(
            c.recover_from_crash(Commitment::of_bytes(b"changed"))
                .is_empty()
        );
        assert_eq!(c.active_runs[&first].status, RunStatus::Queued);
        let recovered = c.recover_repository_from_crash(
            TenantId::from_bytes([1; 16]),
            RepositoryId::from_bytes([1; 16]),
            Commitment::of_bytes(b"changed"),
        );
