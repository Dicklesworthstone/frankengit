...
fn argv(command: &BuildCommand) -> Vec<&str> {
    std::iter::once(command.program().as_str()).chain(command.arguments().iter().map(RunnerText::as_str)).collect()
}
...
fn enqueue(c: &mut WorkflowCoordinator, graph: WorkflowGraph) -> Result<WorkflowRunId, CoordinatorRefusal> {
    c.enqueue_run(TenantId::from_bytes([1; 16]), RepositoryId::from_bytes([2; 16]),
        Commitment::of_bytes(b"head"), GitOid::Sha1(GitOidSha1::from_bytes([3; 20])),
        graph, TriggerContext::trusted_push("alice"), 1, 100)
}
fn source() -> Vec<SourceObject> { vec![SourceObject::new(Commitment::of_bytes(b"source"), 6)] }
...
impl Fixture {
    fn success() -> Self { Self { calls: 0, exit: ExitClass::Succeeded, memory: 16, observed_wall_limit: 0 } }
}
impl ContainmentSubstrate for Fixture {
    fn launch(&mut self, plan: &SandboxPlan) -> Result<SubstrateObservation, SubstrateRefusal> {
        ...
        assert!(plan.secret_leases().is_empty(), "no unrequested credential may reach the runner");
        assert!(plan.capsule().environment().iter().any(|binding| binding.name().as_str() == "FGIT_JOB_ATTEMPT"));
        Ok(SubstrateObservation {
            exit: self.exit,
            usage: ResourceUsage { cpu_micros: 2, memory_bytes: self.memory, disk_bytes: 3, network_bytes: 0, processes: 1, wall_clock_millis: 4 },
            reaped: RunnerReaped { processes_reaped: 1, containment: ContainmentClass::Cooperative },
            log_redaction: LogRedactor::new(Vec::new()).unwrap().redact(b"observed output").unwrap().receipt(),
            artifacts: Vec::new(),
        })
    }
}
...
fn command_lowering_preserves_literals_and_never_substitutes_true() {
    ...
    assert_ne!(lowered("cargo check").unwrap().commitment(), lowered("cargo test").unwrap().commitment());
}

#[test]
fn shell_syntax_and_unrepresentable_arguments_refuse_instead_of_approximating() {
    for script in ["", " ", "echo $HOME", "echo `id`", "echo $(id)", "echo *.rs", "echo ~",
        "echo x | cat", "echo x > result", "true && false", "true; false", "true\nfalse",
        "FOO=value cargo test", "cd src", "echo 'unterminated", "echo \"unterminated", "echo \\",
        "echo \"hello world\"", "echo ''", "echo \0", "echo café", "echo # comment", "echo ${{x}}"]
    {
        assert!(lowered(script).is_err(), "must refuse {script:?}");
    }
    assert_eq!(argv(&lowered("echo '$HOME'").unwrap()), vec!["echo", "$HOME"]);
}
...
fn argument_count_and_byte_bounds_have_permitted_boundary_twins() {
    ...
    assert_eq!(lowered(&at_count).unwrap().arguments().len(), MAX_COMMAND_ARGUMENTS);
...
fn entire_graph_is_preflighted_before_any_run_or_proposal() {
    ...
    assert!(matches!(enqueue(&mut c, invalid), Err(CoordinatorRefusal::UnsupportedExecution { .. })));
...
fn multistep_and_unavailable_runner_profiles_fail_explicitly() {
    ...
    assert!(matches!(enqueue(&mut coordinator(), multi), Err(CoordinatorRefusal::UnsupportedExecution { .. })));
    ...
    assert!(matches!(enqueue(&mut coordinator(), unavailable), Err(CoordinatorRefusal::WorkflowRefusal(WorkflowError::UnsupportedRunner(_)))));
}
...
fn invalid_inputs_do_not_start_jobs_issue_secrets_or_leak_slots() {
    ...
    assert!(c.execute_job(run, "build", &mut fixture, 100, source(), Commitment::of_bytes(b"lock"), "bad toolchain").is_err());
    assert!(c.execute_job(run, "build", &mut fixture, 100, Vec::new(), Commitment::of_bytes(b"lock"), "rust-nightly").is_err());
    ...
    let receipt = c.execute_job(run, "build", &mut fixture, 100, source(), Commitment::of_bytes(b"lock"), "rust-nightly").unwrap();
    ...
    assert!(c.execute_job(run, "build", &mut fixture, 101, source(), Commitment::of_bytes(b"lock"), "rust-nightly").is_err());
    assert_eq!(fixture.calls, 1);
    c.drain_check_facts();
...
fn configured_resource_ceilings_and_step_deadline_reach_the_substrate() {
    ...
    let limits = CoordinatorLimits { step_timeout: Duration::from_secs(2), ..CoordinatorLimits::default() };
    let mut c = WorkflowCoordinator::new(limits, limited, 4).unwrap();
    let run = enqueue(&mut c, graph("echo test")).unwrap();
    let mut fixture = Fixture { exit: ExitClass::ResourceCeiling, ..Fixture::success() };
    let receipt = c.execute_job(run, "build", &mut fixture, 100, source(), Commitment::of_bytes(b"lock"), "rust-nightly").unwrap();
    assert_eq!(fixture.observed_wall_limit, 2000);
    assert_eq!(receipt.outcome(), CheckOutcome::ResourceCeiling { dimension: ResourceDimension::MemoryBytes });
    assert!(!matches!(c.active_runs[&run].status, RunStatus::Terminal(RunOutcome::Succeeded)));
}
...
fn check_fact_binds_actual_terminal_evidence_not_just_input_capsule() {
    ...
    for exit in [ExitClass::Succeeded, ExitClass::Failed] {
        ...
        let mut fixture = Fixture { exit, ..Fixture::success() };
        let receipt = c.execute_job(run, "build", &mut fixture, 100, source(), Commitment::of_bytes(b"lock"), "rust-nightly").unwrap();
        receipt.verify_evidence().unwrap();
        let facts = c.drain_check_facts();
        let terminal = facts.iter().find(|fact| fact.status == CheckRunStatus::Completed).unwrap();
...
fn real_trusted_process_runs_declared_command_and_preserves_nonzero_exit() {
    ...
    for (command, outcome) in [("/bin/true", CheckOutcome::Succeeded), ("/bin/false", CheckOutcome::Failed), ("/bin/echo building", CheckOutcome::Succeeded)] {
        let mut c = coordinator();
        let run = enqueue(&mut c, graph(command)).unwrap();
        let receipt = c.execute_job(run, "build", &mut crate::ProcessSubstrate::new(), 100, source(), Commitment::of_bytes(b"lock"), "host-tools").unwrap();
...
fn lost_containment_blocks_independent_jobs_and_transitive_always_paths() {
    ...
    for (name, needs) in [("middle", vec!["build".to_owned()]), ("last", vec!["middle".to_owned()]), ("independent", Vec::new())] {
        let mut job = g.jobs[0].clone();
        job.id = name.to_owned();
        job.needs = needs;
        if name != "middle" { job.condition = Condition::Always; }
        g.jobs.push(job);
    }
    ...
    let mut fixture = Fixture { memory: 4097, ..Fixture::success() };
    let receipt = c.execute_job(run, "build", &mut fixture, 100, source(), Commitment::of_bytes(b"lock"), "rust-nightly").unwrap();
    assert!(matches!(receipt.outcome(), CheckOutcome::ContainmentFailure { .. }));
    assert!(matches!(c.active_runs[&run].status, RunStatus::Draining { reason: DrainReason::WorkerFailure(_) }));
    assert!(c.eligible_jobs(run).unwrap().is_empty());
    assert!(c.execute_job(run, "independent", &mut fixture, 101, source(), Commitment::of_bytes(b"lock"), "rust-nightly").is_err());
    assert_eq!(fixture.calls, 1);
}
...
fn exhausted_run_deadline_never_reserves_or_launches_another_command() {
    ...
    c.active_runs.get_mut(&run).unwrap().started_at = Instant::now().checked_sub(Duration::from_secs(3601));
    assert!(c.active_runs[&run].started_at.is_some());
    let mut fixture = Fixture::success();
    assert!(c.execute_job(run, "build", &mut fixture, 100, source(), Commitment::of_bytes(b"lock"), "rust-nightly").is_err());
    assert_eq!(fixture.calls, 0);
    assert_eq!(c.obligations.runner_slots_reserved, 0);
    assert_eq!(c.active_runs[&run].job_statuses["build"], JobStatus::Terminal(JobOutcome::TimedOut));
    assert!(c.drain_check_facts().iter().any(|fact| fact.conclusion == Some(CheckRunConclusion::TimedOut)));
}
