...
impl CheckDeliveryBatch {
    ...
    pub const fn repository(&self) -> RepositoryId { self.repository }
    pub const fn run_id(&self) -> WorkflowRunId { self.run }
    pub const fn attempt_id(&self) -> AttemptId { self.attempt }
    pub const fn authority_head(&self) -> Commitment { self.head }
    pub const fn source_commit(&self) -> GitOid { self.source }
    pub const fn graph_commitment(&self) -> Commitment { self.graph }
    pub const fn trust_domain(&self) -> &TrustDomain { &self.trust }
    pub const fn execution_profile(&self) -> CoordinatorExecutionProfile { self.profile }
    pub const fn ordinal(&self) -> u64 { self.ordinal }
    pub fn facts(&self) -> &[CheckRunFact] { &self.facts }
    pub fn body(&self) -> &[u8] { &self.body }
    pub fn id(&self) -> Commitment { Commitment::of_bytes(&self.body) }
    ...
    pub fn decode(body: &[u8]) -> Result<Self, CheckDeliveryRefusal> {
        if body.len() > MAX_BATCH_BYTES { return Err(CheckDeliveryRefusal::BatchTooLarge); }
        let mut input = Input(body);
        if input.take(8)? != MAGIC { return Err(CheckDeliveryRefusal::InvalidBatch); }
        ...
        let trust = TrustDomain::new(RunnerText::parse("check_producer_trust", input.text(256)?)
        ...
        let profile = match input.byte()? {
            ...
            2 => {
                ...
                let limits = crate::workflow::WorkflowLimits {
                    step_timeout: input.duration()?, run_timeout: input.duration()?,
                    stream_bytes: input.size()?, total_output_bytes: input.size()?,
                };
                limits.validate().map_err(|_| CheckDeliveryRefusal::InvalidBatch)?;
        ...
        if count == 0 || count > MAX_BATCH_FACTS { return Err(CheckDeliveryRefusal::InvalidBatch); }
        let mut facts = Vec::with_capacity(count);
        for _ in 0..count {
            let job_id = input.text(MAX_JOB_BYTES)?.to_owned();
            let status = match input.byte()? {
                1 => CheckRunStatus::Queued, 2 => CheckRunStatus::InProgress,
                3 => CheckRunStatus::Completed, _ => return Err(CheckDeliveryRefusal::InvalidBatch),
            };
            let conclusion = match input.byte()? {
                0 => None, 1 => Some(CheckRunConclusion::Success),
                2 => Some(CheckRunConclusion::Failure), 3 => Some(CheckRunConclusion::Neutral),
                4 => Some(CheckRunConclusion::Cancelled), 5 => Some(CheckRunConclusion::TimedOut),
                6 => Some(CheckRunConclusion::ActionRequired), _ => return Err(CheckDeliveryRefusal::InvalidBatch),
            };
            let receipt_commitment = match input.byte()? {
                0 => None, 1 => Some(input.root()?), _ => return Err(CheckDeliveryRefusal::InvalidBatch),
            };
            facts.push(CheckRunFact { run_id: run, job_id, status, conclusion,
                receipt_commitment, timestamp_millis: input.u64()? });
        }
        if !input.0.is_empty() { return Err(CheckDeliveryRefusal::InvalidBatch); }
        let mut batch = Self { tenant, repository, run, attempt, head, source, graph,
            trust, profile, ordinal, facts, body: Vec::new() };
        batch.body = batch.encode()?;
        if batch.body != body { return Err(CheckDeliveryRefusal::InvalidBatch); }
        Ok(batch)
    }
    ...
    fn encode(&self) -> Result<Vec<u8>, CheckDeliveryRefusal> {
        ...
        for fact in &self.facts {
            if fact.run_id != self.run || fact.job_id.is_empty() || fact.job_id.len() > MAX_JOB_BYTES
                ...
                || (fact.conclusion == Some(CheckRunConclusion::Success) && fact.receipt_commitment.is_none())
                || (matches!(self.profile, CoordinatorExecutionProfile::TrustedWorkflow { .. })
                    && matches!(fact.conclusion, Some(CheckRunConclusion::Success | CheckRunConclusion::Neutral)))
            { return Err(CheckDeliveryRefusal::InvalidBatch); }
        }
        let mut bytes = MAGIC.to_vec();
        bytes.extend_from_slice(self.tenant.as_bytes());
        bytes.extend_from_slice(self.repository.as_bytes());
        for value in [self.run.commitment(), self.attempt.commitment(), self.head] { root(&mut bytes, value); }
        bytes.push(match self.source { GitOid::Sha1(_) => 1, GitOid::Sha256(_) => 2 });
        ...
        match self.profile {
            ...
            CoordinatorExecutionProfile::TrustedWorkflow { source, limits } => {
                limits.validate().map_err(|_| CheckDeliveryRefusal::InvalidBatch)?;
                bytes.push(2); root(&mut bytes, source);
                duration(&mut bytes, limits.step_timeout); duration(&mut bytes, limits.run_timeout);
                number(&mut bytes, limits.stream_bytes as u64);
                number(&mut bytes, limits.total_output_bytes as u64);
            }
        }
        number(&mut bytes, self.ordinal); number(&mut bytes, self.facts.len() as u64);
        for fact in &self.facts {
            text(&mut bytes, &fact.job_id);
            bytes.push(match fact.status { CheckRunStatus::Queued => 1, CheckRunStatus::InProgress => 2, CheckRunStatus::Completed => 3 });
            ...
            if let Some(value) = fact.receipt_commitment { root(&mut bytes, value); }
            number(&mut bytes, fact.timestamp_millis);
        }
        if bytes.len() > MAX_BATCH_BYTES { return Err(CheckDeliveryRefusal::BatchTooLarge); }
        Ok(bytes)
    }
}
fn number(out: &mut Vec<u8>, value: u64) { out.extend_from_slice(&value.to_be_bytes()); }
fn text(out: &mut Vec<u8>, value: &str) { number(out, value.len() as u64); out.extend_from_slice(value.as_bytes()); }
fn root(out: &mut Vec<u8>, value: Commitment) { out.extend_from_slice(value.digest().bytes().as_bytes()); }
fn duration(out: &mut Vec<u8>, value: Duration) {
    number(out, value.as_secs()); out.extend_from_slice(&value.subsec_nanos().to_be_bytes());
}
struct Input<'a>(&'a [u8]);
impl<'a> Input<'a> {
    fn take(&mut self, count: usize) -> Result<&'a [u8], CheckDeliveryRefusal> {
        let result = self.0.get(..count).ok_or(CheckDeliveryRefusal::InvalidBatch)?;
        self.0 = &self.0[count..]; Ok(result)
    }
    fn array<const N: usize>(&mut self) -> Result<[u8; N], CheckDeliveryRefusal> {
        self.take(N)?.try_into().map_err(|_| CheckDeliveryRefusal::InvalidBatch)
    }
    fn byte(&mut self) -> Result<u8, CheckDeliveryRefusal> { Ok(self.array::<1>()?[0]) }
    fn u64(&mut self) -> Result<u64, CheckDeliveryRefusal> { Ok(u64::from_be_bytes(self.array()?)) }
    ...
    fn text(&mut self, maximum: usize) -> Result<&'a str, CheckDeliveryRefusal> {
        let count = self.size()?;
        if count > maximum { return Err(CheckDeliveryRefusal::InvalidBatch); }
    ...
    fn root(&mut self) -> Result<Commitment, CheckDeliveryRefusal> {
        let bytes = DigestBytes::try_new(self.take(32)?).map_err(|_| CheckDeliveryRefusal::InvalidBatch)?;
    ...
    fn duration(&mut self) -> Result<Duration, CheckDeliveryRefusal> {
        ...
        if nanos >= 1_000_000_000 { return Err(CheckDeliveryRefusal::InvalidBatch); }
...
impl CheckDeliveryAcknowledgement {
    pub fn after_durable_acceptance(batch: &CheckDeliveryBatch, receipt: Commitment) -> Self {
        Self { batch: batch.id(), receipt }
    }
    pub const fn batch_id(&self) -> Commitment { self.batch }
    pub const fn receipt_root(&self) -> Commitment { self.receipt }
}
...
pub trait CheckDeliverySink {
    fn accept(&mut self, batch: &CheckDeliveryBatch)
        -> Result<CheckDeliveryAcknowledgement, CheckDeliveryRefusal>;
}

impl WorkflowCoordinator {
    pub fn pending_check_fact_count(&self) -> usize { self.outbox_facts.len() }
    ...
    pub fn prepare_check_delivery(&self, maximum_facts: usize, maximum_bytes: usize)
        -> Result<Option<CheckDeliveryBatch>, CheckDeliveryRefusal>
    {
        if maximum_facts == 0 || maximum_facts > MAX_BATCH_FACTS || maximum_bytes == 0 || maximum_bytes > MAX_BATCH_BYTES {
            return Err(CheckDeliveryRefusal::InvalidLimits);
        }
        let Some(first) = self.outbox_facts.first() else { return Ok(None); };
        let run = self.active_runs.get(&first.run_id).ok_or(CheckDeliveryRefusal::UnknownRun)?;
        let mut batch = CheckDeliveryBatch {
            tenant: run.tenant, repository: run.repository, run: run.id, attempt: run.attempt_id,
            head: run.authority_head, source: run.source_commit, graph: run.graph_id,
            trust: run.trigger_ctx.trust_domain.clone(), profile: run.execution_profile,
            ordinal: u64::try_from(self.obligations.check_publications_settled).map_err(|_| CheckDeliveryRefusal::InvalidBatch)?,
            facts: Vec::new(), body: Vec::new(),
        };
        for fact in self.outbox_facts.iter().take(maximum_facts).take_while(|fact| fact.run_id == run.id) {
            // Reject excessive text before copying it into the immutable batch.
            if fact.job_id.len() > MAX_JOB_BYTES { return Err(CheckDeliveryRefusal::BatchTooLarge); }
            batch.facts.push(fact.clone());
            let body = batch.encode()?;
            if body.len() > maximum_bytes {
                batch.facts.pop();
                if batch.facts.is_empty() { return Err(CheckDeliveryRefusal::BatchTooLarge); }
                break;
            }
            batch.body = body;
        }
    ...
    pub fn acknowledge_check_delivery(&mut self, batch: &CheckDeliveryBatch, acknowledgement: CheckDeliveryAcknowledgement)
        -> Result<(), CheckDeliveryRefusal>
    {
        if acknowledgement.batch != batch.id() { return Err(CheckDeliveryRefusal::AcknowledgementMismatch); }
        let current = self.prepare_check_delivery(batch.facts.len(), MAX_BATCH_BYTES)?
            .ok_or(CheckDeliveryRefusal::StaleBatch)?;
        if current.body != batch.body { return Err(CheckDeliveryRefusal::StaleBatch); }
        let settled = self.obligations.check_publications_settled.checked_add(batch.facts.len())
            .ok_or(CheckDeliveryRefusal::InvalidBatch)?;
        if settled > self.obligations.check_publications_emitted { return Err(CheckDeliveryRefusal::InvalidBatch); }
    ...
    pub fn deliver_check_facts<S: CheckDeliverySink>(&mut self, sink: &mut S,
        maximum_facts: usize, maximum_bytes: usize, live: &dyn Fn() -> bool)
