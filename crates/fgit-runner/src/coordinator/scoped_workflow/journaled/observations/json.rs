...
use super::{commitment, checkpoint, valid_job, ObservationRefusal, MAX_JOB_BYTES};
use crate::workflow::{
    JobOutcome, JobReport, StepObservation, StepOutcome, StepReport, WorkerFailure,
    WorkflowLimits, WorkflowReport, MAX_JOBS, MAX_STEPS,
};
...
pub(super) fn report(
    bytes: &[u8], step_timeout: Duration, run_timeout: Duration, live: &dyn Fn() -> bool,
) -> Result<WorkflowReport> {
    let mut input = Json { bytes, position: 0, live };
    ...
    if u128::from(input.number()?) != step_timeout.as_millis() { return Err(BAD); }
    input.literal(b",\"run_timeout_millis\":")?;
    if u128::from(input.number()?) != run_timeout.as_millis() { return Err(BAD); }
    ...
    let limits = WorkflowLimits { step_timeout, run_timeout, stream_bytes, total_output_bytes };
    ...
    if input.peek() != Some(b']') {
        loop {
            checkpoint(live)?;
            if jobs.len() == MAX_JOBS { return Err(BAD); }
            jobs.try_reserve(1).map_err(|_| ObservationRefusal::AllocationFailed)?;
    ...
    input.literal(b"]}")?;
    let report = WorkflowReport { source, graph, limits, jobs };
    if input.position != bytes.len() || report.succeeded() != succeeded { return Err(BAD); }
    Ok(report)
}
...
impl Json<'_> {
    ...
    fn literal(&mut self, literal: &[u8]) -> Result<()> {
        if !self.bytes[self.position..].starts_with(literal) { return Err(BAD); }
        self.position += literal.len(); Ok(())
    }
    fn boolean(&mut self) -> Result<bool> {
        if self.peek() == Some(b't') { self.literal(b"true")?; Ok(true) }
        else { self.literal(b"false")?; Ok(false) }
    }
    fn number(&mut self) -> Result<u64> {
        let start = self.position;
        let mut value = 0u64;
        while let Some(byte @ b'0'..=b'9') = self.peek() {
            if self.position - start == 20 { return Err(BAD); }
            value = value.checked_mul(10).and_then(|n| n.checked_add(u64::from(byte - b'0'))).ok_or(BAD)?;
            self.position += 1;
        }
    ...
    fn size(&mut self) -> Result<usize> { usize::try_from(self.number()?).map_err(|_| BAD) }
    fn exit(&mut self) -> Result<Option<i32>> {
        if self.peek() == Some(b'n') { self.literal(b"null")?; return Ok(None); }
        let negative = self.peek() == Some(b'-');
        if negative { self.position += 1; }
    ...
    fn text(&mut self, maximum: usize) -> Result<String> {
        ...
        loop {
            if self.position % 1024 == 0 { checkpoint(self.live)?; }
            ...
            if result.len() == maximum { return Err(BAD); }
            result.try_reserve(1).map_err(|_| ObservationRefusal::AllocationFailed)?;
            result.push(decoded);
        }
        String::from_utf8(result).map_err(|_| BAD)
    }
    fn optional_text(&mut self, maximum: usize) -> Result<Option<String>> {
        if self.peek() == Some(b'n') { self.literal(b"null")?; Ok(None) }
        else { self.text(maximum).map(Some) }
    }
    ...
    fn output(&mut self, maximum: usize, remaining: &mut usize) -> Result<Vec<u8>> {
        ...
        loop {
            ...
            let byte = self.peek().ok_or(BAD)?;
            if byte == b'"' { break; }
            if self.position - start == ceiling { return Err(BAD); }
            nibble(byte)?;
            self.position += 1;
        }
        let length = self.position - start;
        if length % 2 != 0 { return Err(BAD); }
        let mut output = Vec::new();
        output.try_reserve_exact(length / 2).map_err(|_| ObservationRefusal::AllocationFailed)?;
        for (index, pair) in self.bytes[start..self.position].chunks_exact(2).enumerate() {
            if index % 1024 == 0 { checkpoint(self.live)?; }
    ...
    fn job(&mut self, limits: WorkflowLimits, remaining: &mut usize, steps_left: &mut usize) -> Result<JobReport> {
        self.literal(b"{\"id\":")?;
        let id = self.text(MAX_JOB_BYTES)?;
        if !valid_job(&id) { return Err(BAD); }
        self.literal(b",\"outcome\":")?;
        let outcome = match self.text(32)?.as_str() {
            "succeeded" => JobOutcome::Succeeded, "failed" => JobOutcome::Failed,
            "skipped" => JobOutcome::Skipped, "cancelled" => JobOutcome::Cancelled,
            "timed_out" => JobOutcome::TimedOut, "output_limit" => JobOutcome::OutputLimit,
            "refused" => JobOutcome::Refused, _ => return Err(BAD),
        };
        ...
        if self.peek() != Some(b']') {
            loop {
                ...
                if *steps_left == 0 { return Err(BAD); }
                *steps_left -= 1;
                let step = self.step(limits, remaining)?;
                if steps.last().is_some_and(|previous| previous.index >= step.index) { return Err(BAD); }
                steps.try_reserve(1).map_err(|_| ObservationRefusal::AllocationFailed)?;
                steps.push(step);
        ...
        let failure = if self.peek() == Some(b'n') {
            ...
            self.literal(b"}")?;
            Some(WorkerFailure { detail, retain_workspace })
        };
        self.literal(b"}")?;
        if outcome == JobOutcome::Succeeded && (failure.is_some()
            || steps.iter().any(|step| step.observation.outcome != StepOutcome::Succeeded))
        { return Err(BAD); }
        Ok(JobReport { id, outcome, steps, failure })
    }
    fn step(&mut self, limits: WorkflowLimits, remaining: &mut usize) -> Result<StepReport> {
        self.literal(b"{\"index\":")?;
        let index = self.size()?;
        if index >= MAX_STEPS { return Err(BAD); }
        self.literal(b",\"name\":")?;
        ...
        self.literal(b",\"outcome\":")?;
        let outcome = match self.text(32)?.as_str() {
            "succeeded" => StepOutcome::Succeeded, "failed" => StepOutcome::Failed,
            "cancelled" => StepOutcome::Cancelled, "timed_out" => StepOutcome::TimedOut,
            "output_limit" => StepOutcome::OutputLimit,
            "containment_failure" => StepOutcome::ContainmentFailure, _ => return Err(BAD),
        };
        ...
        let retain_workspace = self.boolean()?;
        self.literal(b"}")?;
        if outcome == StepOutcome::Succeeded && (exit_code != Some(0) || !output_complete || retain_workspace) {
            return Err(BAD);
        }
        Ok(StepReport { index, name, script, observation: StepObservation {
            outcome, exit_code, stdout, stderr, elapsed_millis, output_complete, retain_workspace,
        } })
    }
}
fn nibble(byte: u8) -> Result<u8> {
    match byte { b'0'..=b'9' => Ok(byte - b'0'), b'a'..=b'f' => Ok(byte - b'a' + 10), _ => Err(BAD) }
}
