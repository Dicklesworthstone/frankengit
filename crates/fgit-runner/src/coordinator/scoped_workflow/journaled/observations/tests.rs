...
pub(super) fn produce(
    format: bool,
    outcome: StepOutcome,
) -> (CheckDeliveryBatch, TrustedWorkflowReceipt) {
...
pub(super) fn rebind_hash(
    ...
    let positions = batch
        ...
        .windows(32)
        .enumerate()
        .filter_map(|(i, part)| (part == raw.as_bytes()).then_some(i))
        .collect::<Vec<_>>();
    assert_eq!(positions.len(), 1);
    let mut body = batch.body().to_vec();
    body[positions[0]..positions[0] + 32]
        .copy_from_slice(Commitment::of_bytes(bytes).digest().bytes().as_bytes());
    ...
    for sha256 in [false, true] {
        for outcome in [
            ...
            for (index, fact) in batch.facts().iter().enumerate() {
                ...
                let one =
                    verify_trusted_job(&batch, index, &bytes, MAX_OBSERVATION_BYTES, &|| true)
                        .unwrap();
...
fn valid_hash_never_substitutes_for_any_proposal_subject_coordinate() {
    ...
    for coordinate in 0..11 {
        ...
        assert!(
            decode(&bytes).is_ok(),
            "coordinate {coordinate} must be a valid independent record"
        );
        let changed = rebind_hash(&batch, index, &bytes);
        assert_eq!(
            verify_trusted_job(&changed, index, &bytes, MAX_OBSERVATION_BYTES, &|| true),
...
fn full_workflow_wrong_job_and_changed_conclusion_are_not_single_job_proofs() {
    ...
    assert_eq!(
        verify_trusted_job(&batch, 0, &[], MAX_OBSERVATION_BYTES, &|| true),
        Err(ObservationRefusal::FactNotCompleted)
    );
    assert_eq!(
        verify_trusted_job(&batch, usize::MAX, &[], MAX_OBSERVATION_BYTES, &|| true),
        Err(ObservationRefusal::FactNotCompleted)
    );
}
...
fn json_summary_authority_environment_limits_and_unknown_fields_cannot_drift() {
    ...
    for (old, new) in [
        (
            "\"authoritative_check\":false",
            "\"authoritative_check\":true",
        ),
        ("\"schema_version\":1", "\"schema_version\":2"),
        ("\"succeeded\":true", "\"succeeded\":false"),
        (
            "\"step_timeout_millis\":1000",
            "\"step_timeout_millis\":1001",
        ),
        ("/usr/bin:/bin", "/tmp/bin"),
        ...
        let bytes = change_json(&receipt, |text| {
...
fn identity_collections_and_output_bounds_refuse_before_unbounded_growth() {
    ...
    zero.attempts.insert("build".to_owned(), 0);
    assert!(decode(&zero.frame()).is_err());
    let mut backwards = receipt.clone();
    backwards.report.jobs[0].steps[1].index = 0;
    assert!(decode(&backwards.frame()).is_err());
    let mut aggregate = receipt.clone();
    aggregate.report.limits.total_output_bytes = 16;
    ...
    assert!(
        decode_trusted_observation(&bytes, Commitment::of_bytes(&bytes), bytes.len(), &|| true)
            .is_ok()
    );
    for maximum in [0, MAX_OBSERVATION_BYTES + 1] {
        assert_eq!(
            decode_trusted_observation(&bytes, Commitment::of_bytes(&bytes), maximum, &|| true),
            Err(ObservationRefusal::InvalidLimits)
        );
    }
}
...
fn one_job_workflow_full_receipt_is_the_same_exact_job_body() {
    ...
    let verified = verify_trusted_job(
        &batch,
        selected(&batch),
        &receipt.frame(),
        MAX_OBSERVATION_BYTES,
