...
use fgit_runner::{
    BuildCommand, BuildInputCapsule, CheckOutcome, Commitment, ContainmentSubstrate, ForkPolicy,
    JobRequest, LogRedactor, MAX_ARTIFACTS, ResourceCeilings, ResourceUsage, RunnerControlPlane,
    RunnerPolicy, RunnerRefusal, RunnerText, SandboxPlan, SecretBroker, SecretRequest,
    SourceObject, SubstrateObservation, SubstrateRefusal, TrustDomain,
};

fn text(value: &str) -> RunnerText {
    RunnerText::parse("test", value).unwrap()
}
fn hash(value: &[u8]) -> Commitment {
    Commitment::of_bytes(value)
}
fn ceilings() -> ResourceCeilings {
    ResourceCeilings::new(100, 100, 100, 0, 2, 100).unwrap()
}
fn policy() -> RunnerPolicy {
    RunnerPolicy::new(
        TrustDomain::new(text("settlement")),
...
fn capsule() -> BuildInputCapsule {
    BuildInputCapsule::new(
        hash(b"head"),
        vec![SourceObject::new(hash(b"source"), 6)],
        hash(b"lock"),
        text("pinned-toolchain"),
        BuildCommand::new(text("check"), vec![]).unwrap(),
        vec![],
    )
    .unwrap()
}
    ...
    for artifacts in [
        ...
        let admitted = controller
            .admit(
                capsule(),
                policy(),
                JobRequest::new(false, leases.clone(), vec![], 1).unwrap(),
                &mut broker,
                1,
            )
            .unwrap();
        assert!(matches!(
            controller.admit(
                capsule(),
                policy(),
                JobRequest::new(false, vec![], vec![], 2).unwrap(),
                &mut broker,
                1
            ),
        ...
        let expected = if artifacts.len() > MAX_ARTIFACTS {
            ...
            RunnerRefusal::DuplicateArtifactCommitment
        };
        let error = controller
            .execute(admitted, &mut Worker(artifacts), &mut broker)
            .unwrap_err();
        ...
        let next = controller
            .admit(
                capsule(),
                policy(),
                JobRequest::new(false, vec![], vec![], 3).unwrap(),
                &mut broker,
                2,
            )
            .unwrap();
        let receipt = controller
            .execute(next, &mut Worker(vec![hash(b"valid")]), &mut broker)
            .unwrap();
...
fn rejected_secret_batch_does_not_bind_the_valid_prefix_or_consume_capacity() {
    ...
    assert!(matches!(
        controller.admit(
            capsule(),
            policy(),
            JobRequest::new(false, vec![first, expired], vec![], 1).unwrap(),
            &mut broker,
            2
        ),
    ...
    let admitted = controller
        .admit(
            capsule(),
            policy(),
            JobRequest::new(false, vec![first], vec![], 2).unwrap(),
            &mut broker,
            2,
        )
        .unwrap();
    let receipt = controller
        .execute(admitted, &mut Worker(vec![]), &mut broker)
        .unwrap();
    assert_eq!(receipt.outcome(), CheckOutcome::Succeeded);
