...
use fgit_treefs::{BaseView, CandidateManifestRefusal, ObjectSource, ObjectSourceError,
    PathPolicy, ReadGrant, SparseCandidateManifest, SparseLimits, TreeCapability, TreePath, WorkspaceId};
use fgit_types::{ByteCount, CodecVersion, DigestAlgorithmId, DigestBytes, RepositoryCommitId, RepositoryId};

struct Source { objects: BTreeMap<Vec<u8>, (GitObjectKind, Vec<u8>)>, payload_reads: Cell<usize> }
impl Source {
    fn put<A: GitHashAlgorithm>(&mut self, kind: GitObjectKind, bytes: Vec<u8>) -> GitOid<A> {
        let id = GitOid::<A>::of_object(kind, &bytes);
        self.objects.insert(id.digest_bytes().to_vec(), (kind, bytes)); id
    }
    fn tree<A: GitHashAlgorithm>(&mut self, content: &[u8]) -> GitOid<A> {
        let blob = self.put::<A>(GitObjectKind::Blob, content.to_vec());
        self.put::<A>(GitObjectKind::Tree, [b"100755 result\xff\0".as_slice(), blob.digest_bytes()].concat())
    }
    fn commit<A: GitHashAlgorithm>(&mut self, tree: GitOid<A>, parents: &[GitOid<A>], message: &str) -> GitOid<A> {
...
impl<A: GitHashAlgorithm> ObjectSource<A> for Source {
    fn read_object(&self, id: &GitOid<A>, kind: GitObjectKind, _: &ReadGrant) -> Result<Vec<u8>, ObjectSourceError> {
        if kind != GitObjectKind::Commit { self.payload_reads.set(self.payload_reads.get() + 1); }
        let (actual, bytes) = self.objects.get(id.digest_bytes()).ok_or_else(||
            ObjectSourceError::NotFound { oid_hex: hex(id.digest_bytes()) })?;
        if *actual != kind { return Err(ObjectSourceError::Refused { reason: "wrong fixture kind".into() }); }
        Ok(bytes.clone())
    }
}
fn hex(bytes: &[u8]) -> String { bytes.iter().map(|b| format!("{b:02x}")).collect() }
fn path() -> TreePath { TreePath::parse_default(b"result\xff").unwrap() }
fn limits<A: GitHashAlgorithm>() -> ParseLimits { ParseLimits { tree_reference_bytes: A::DIGEST_LEN, ..Default::default() } }
fn capability() -> TreeCapability {
    TreeCapability::new(WorkspaceId::from_bytes([1; 16]), RepositoryId::from_bytes([2; 16]), vec![path()], Vec::new())
        .with_fetch_budget(ByteCount::try_new("test", 1_000_000, 1_000_000).unwrap()).with_file_budget(100)
}
fn fixture<A: GitHashAlgorithm>() -> (Source, BaseView<A>, GitOid<A>, GitOid<A>, GitOid<A>) {
    let mut source = Source { objects: BTreeMap::new(), payload_reads: Cell::new(0) };
...
impl Scratch {
    fn new() -> Self {
        loop {
            let path = std::env::temp_dir().join(format!("fg-merge-inputs-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
...
fn copied<A: GitHashAlgorithm>(cancel: bool) {
    ...
    let manifest = SparseCandidateManifest::build_merge(&base, &source, candidate, incoming,
        &mut capability(), 0, limits::<A>(), SparseLimits::default()).unwrap();
    ...
    assert_eq!(manifest.entries()[0].kind().body(), Some(b"resolved\0\xff\n".as_slice()));
    ...
    let obligation = ledger.reserve(plan.reservation(), ledger.grant(plan.budget()).unwrap()).unwrap();
    let result = SparseWorkspace::materialize(plan, File::open(&root.0).unwrap(), TreePath::parse_default(b"job").unwrap(),
        obligation, &capability(), 0, &|epoch| cancel && matches!(epoch, HostEpoch::Writing(_)));
    if cancel {
        assert!(matches!(result, Err(HostRefusal::Cancelled(HostEpoch::Writing(_)))));
    } else {
...
#[test]
fn both_formats_copy_the_actual_merge_result_and_refuse_canonical_import() { copied::<Sha1>(false); copied::<Sha256>(false); }
#[test]
fn merge_copy_cancellation_drains_the_same_host_obligation() { copied::<Sha1>(true); copied::<Sha256>(true); }

fn wrong_parents<A: GitHashAlgorithm>() {
    let (mut source, base, incoming, candidate, tree) = fixture::<A>();
    assert!(matches!(SparseCandidateManifest::build(&base, &source, candidate, &mut capability(), 0,
    ...
    assert_eq!(source.payload_reads.get(), 0, "no candidate tree or blob read before ordered parent validation");
    let key = candidate.digest_bytes().to_vec();
    source.objects.get_mut(&key).unwrap().1.push(b'x');
    assert!(matches!(SparseCandidateManifest::build_merge(&base, &source, candidate, incoming, &mut capability(), 0,
        limits::<A>(), SparseLimits::default()), Err(CandidateManifestRefusal::Source(ObjectSourceError::IdentityMismatch { .. }))));
    assert_eq!(source.payload_reads.get(), 0);
}
#[test]
fn ordered_parent_binding_and_hash_verification_precede_sparse_discovery() { wrong_parents::<Sha1>(); wrong_parents::<Sha256>(); }
