...
impl WorkflowExecutor for Fake {
    ...
    fn execute_step(
        ...
        Ok(StepObservation {
            ...
            retain_workspace: script == self.retain_on,
        })
    }
...
fn ordinary_failure_runs_failure_and_always_diagnostics_without_turning_green() {
    ...
    assert_eq!(
        report.jobs.iter().map(|j| j.outcome).collect::<Vec<_>>(),
        [
            JobOutcome::Failed,
    ...
    assert_eq!(
        ...
        [
            "begin:build",
            "step:0:build",
            "step:2:same-job-failure-diagnostic",
            "step:3:same-job-always-diagnostic",
            "finish:false",
            "begin:cleanup",
            "step:0:dependent-always",
            "finish:false",
            "begin:failure-report",
            "step:0:dependent-failure",
            "finish:false",
        ]
    );
    assert_eq!(
        report.jobs[0].steps.len(),
        3,
        "default success step after failure is skipped, not fabricated"
    );
}
...
fn success_skips_failure_predicates_but_runs_always() {
    ...
    assert!(
        worker
            .calls
            .contains(&"step:1:forbidden-success".to_owned())
    );
    assert!(
        !worker
            .calls
            .iter()
            .any(|s| s.contains("same-job-failure-diagnostic"))
    );
    assert!(
        worker
            .calls
            .iter()
            .any(|s| s.contains("same-job-always-diagnostic"))
    );
...
fn always_never_overrides_lost_containment() {
    ...
    assert!(
        !worker
            .calls
            .iter()
            .any(|s| s.contains("diagnostic") || s.contains("dependent"))
    );
