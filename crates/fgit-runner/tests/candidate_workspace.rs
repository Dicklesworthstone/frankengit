...
use fgit_treefs::{
    ...
    SparseCandidateManifest, SparseLimits, SparseManifest, TreeCapability, TreePath, WorkspaceId,
};
use fgit_types::{
...
impl Source {
    fn put<A: GitHashAlgorithm>(&mut self, kind: GitObjectKind, bytes: Vec<u8>) -> GitOid<A> {
        ...
        self.objects
            .insert(oid.digest_bytes().to_vec(), (kind, bytes));
        oid
    }
}
    ...
    fn read_object(
        ...
        if self.stop.get() {
            return Err(ObjectSourceError::Refused {
                reason: "cancelled fixture source".into(),
            });
        }
        let (actual, bytes) =
            self.objects
...
fn path() -> TreePath {
    TreePath::parse_default(b"input\xff").unwrap()
}
fn parse_limits<A: GitHashAlgorithm>() -> ParseLimits {
    ParseLimits {
...
fn capability() -> TreeCapability {
    ...
    .with_fetch_budget(ByteCount::try_new("fixture", 1_000_000, 1_000_000).unwrap())
...
fn fixture<A: GitHashAlgorithm>() -> (Source, BaseView<A>, GitOid<A>) {
    ...
    let mut tree = |bytes: &[u8]| {
        ...
        source.put::<A>(
            GitObjectKind::Tree,
            [b"100755 input\xff\0".as_slice(), blob.digest_bytes()].concat(),
        )
    };
...
fn copy_and_close<A: GitHashAlgorithm>() {
    ...
    let manifest = SparseCandidateManifest::build(
        &base,
        &source,
        candidate,
        &mut capability(),
    ...
    let original = SparseManifest::build(
        &base,
        &source,
        &mut capability(),
        0,
    ...
    .unwrap();
    let original = SparseWorkspacePlan::new(
        Arc::new(original),
        Vec::new(),
        &capability(),
        0,
    ...
    let granted = ledger.grant(plan.budget()).unwrap();
    let reservation = ledger.reserve(plan.reservation(), granted).unwrap();
    ...
    assert_eq!(
        fs::read(
            root.0
                .join("job")
                .join(std::ffi::OsStr::from_bytes(b"input\xff"))
        )
        .unwrap(),
        b"\0\xff\r\n"
    );
...
fn parent_integrity_capabilities_and_payload_limits_are_not_bypassed() {
    ...
    .unwrap();
    let wrong = commit::<Sha1>(
        &mut source,
        *good.candidate_tree_oid(),
        &[candidate],
        "wrong parent",
    );
    let merged = commit::<Sha1>(
        &mut source,
        *good.candidate_tree_oid(),
        &[*base.base_commit_oid(), candidate],
        "two parents",
    );
    ...
    source
        .objects
        .get_mut(candidate.digest_bytes())
        .unwrap()
        .1
        .push(b'x');
    assert!(matches!(
...
fn candidate_metadata_is_in_the_host_commitment_and_cancelled_copy_settles() {
    ...
    let first = SparseCandidateManifest::build(
        &base,
        &source,
        candidate,
        &mut capability(),
    ...
    .unwrap();
    let other = commit::<Sha256>(
        &mut source,
        *first.candidate_tree_oid(),
        &[*base.base_commit_oid()],
        "different metadata",
    );
    let second = SparseCandidateManifest::build(
        &base,
        &source,
        other,
        &mut capability(),
    ...
    .unwrap();
    let other_plan = SparseWorkspacePlan::for_candidate(
        Arc::new(second),
        &capability(),
        0,
        SparseLimits::default(),
    ...
    let result = SparseWorkspace::materialize(
        plan,
        File::open(&root.0).unwrap(),
        TreePath::parse_default(b"cancelled").unwrap(),
        reservation,
        &capability(),
        0,
        &|epoch| matches!(epoch, HostEpoch::Writing(_)),
    );
