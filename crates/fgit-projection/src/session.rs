    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::OutOfRange {
                ...
                write!(
                    f,
                    "projection {field} {value} exceeds storage maximum {maximum}"
                )
            }
...
pub fn flatten<T>(
    ...
    match outcome {
        ...
        asupersync::Outcome::Panicked(payload) => Err(ProjectionError::Panicked { step, payload }),
    }
}
    ...
    C::Tx<'a>: TransactionOps,
{
    ...
    let head_generation = checked_sql_counter(
        ...
        record.authority_head_generation,
    )?;
...
mod tests {
    ...
    fn runtime_outcomes_keep_their_distinct_payloads() {
        assert_eq!(
            flatten(asupersync::Outcome::Ok(42), "read").expect("success"),
            42
        );
        let sql = flatten::<()>(
        ...
        assert!(
            matches!(sql, ProjectionError::Sql(sqlmodel_core::Error::Custom(ref message))
        ...
        assert!(
            matches!(cancelled, ProjectionError::Cancelled { step: "select", reason: actual }
            if *actual == reason)
        );
        ...
        let panicked = flatten::<()>(asupersync::Outcome::Panicked(payload.clone()), "commit")
            .expect_err("panic");
        assert!(
            matches!(panicked, ProjectionError::Panicked { step: "commit", payload: actual }
            if actual == payload)
        );
    }
    ...
    fn integer_bounds_never_saturate_or_wrap() {
        ...
        assert_eq!(
            checked_sql_counter("seq", 9_223_372_036_854_775_807).expect("max"),
            i64::MAX
        );
        assert!(matches!(
            checked_sql_counter("seq", u64::MAX),
        ...
        assert!(matches!(
            next_after(Some(ProjectionPosition::new(u64::MAX))),
        ...
        let result: Result<(), ProjectionError> = {
            ...
            rt.block_on(async {
                ...
                flatten(
                    session
                        .connection_ref()
                        .execute(&cx, "DROP TABLE fgit_projection_watermark", &[])
                        .await,
                    "test_drop",
                )?;
                ...
                .await
                .expect_err("constraint fails after inserting the decision");
    ...
    fn full_generations_and_stale_empty_snapshots_are_checked_under_the_transaction() {
        ...
        let result: Result<(), ProjectionError> = {
            ...
            rt.block_on(async {
                ...
                .await
                .expect_err("another folder advanced an expected-empty snapshot");
                assert!(matches!(
                    stale,
                    ProjectionError::Refusal(WatermarkRefusal::Gap { .. })
                ));
                let foreign = identity(u32::MAX, 8);
                let offered = DecisionRecord {
                    authority_head_generation: 8,
                    ..record(2)
                };
                let error = advance_within_transaction(
                    session.connection_ref(),
                ...
                .await
                .expect_err("stored head generation must match, not just its text");
                assert!(matches!(
                    error,
                    ProjectionError::Identity(IdentityAdvanceError::BindingMismatch {
                        field: "authority_head_generation",
                        ..
                    })
                ));
                assert_eq!(apply_batch(&session, &cx, &[record(2)]).await?.applied, 1);
                let overflow = apply_batch(&session, &cx, &[record(u64::MAX)])
                    .await
                    .expect_err("typed range error");
                assert!(matches!(
                    overflow,
