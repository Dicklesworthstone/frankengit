...
fn generation(text: &str) -> Result<GraphGenerationId, String> {
    ...
    let raw = text
        .strip_prefix(&prefix)
        .ok_or("Invalid generation token domain.")?;
    if raw.len() != width * 2
        || !raw
            .bytes()
    ...
    if bytes.iter().all(|b| *b == 0) {
        return Err("Zero candidate is not allowed.".to_owned());
    }
...
fn parse(args: &[OsString]) -> Result<Options, String> {
    if !(7..=8).contains(&args.len()) || args.iter().any(|s| s.len() > 4096) {
        return Err(USAGE.to_owned());
    }
    let text = |i: usize| {
        args[i]
            .to_str()
            .ok_or_else(|| "Only the root path may contain non-UTF-8 bytes.".to_owned())
    };
    ...
    if !reference.as_bytes().starts_with(b"refs/") {
        return Err("A full reference is required.".to_owned());
    }
    let action = match text(5)? {
        ...
        "query" if args.len() == 8 => {
            let mode = match text(6)? {
                "exact" => SymbolMatchMode::Exact,
                "prefix" => SymbolMatchMode::Prefix,
                _ => return Err(USAGE.to_owned()),
            };
            Action::Query(
                SymbolQuery::new(text(7)?.as_bytes(), mode, &[], &[], MAX_SYMBOL_WORK)
                    .map_err(|e| e.to_string())?,
            )
        }
        _ => return Err(USAGE.to_owned()),
...
fn execute(node: &OneNode, options: &Options) -> Result<String, Failure> {
    ...
    match &options.action {
        Action::Build(predecessor) => {
            ...
            Ok(format!(
                "{{\"type\":\"symbol_index_activation\",\"index_token\":\"{}\",\"index_number\":{},\"snapshot_token\":\"{}\",\"source_commit\":\"{}\",\"root_tree\":\"{}\",\"repository_transaction_created\":false}}",
                token(activation.generation_id.as_internal_object_id()),
                activation.authority_generation.get(),
                token(source.head.as_internal_object_id()),
                source.commit,
                source.tree
            ))
        }
        Action::Refresh(predecessor) => {
            ...
            Ok(format!(
                "{{\"type\":\"symbol_index_refresh\",\"index_token\":\"{}\",\"index_number\":{},\"snapshot_token\":\"{}\",\"source_commit\":\"{}\",\"root_tree\":\"{}\",\"reused_files\":{},\"source_blobs_read\":{},\"source_bytes_read\":{},\"predecessor_tables_read\":{},\"predecessor_payload_bytes\":{},\"repository_transaction_created\":false}}",
                token(activation.generation_id.as_internal_object_id()),
                activation.authority_generation.get(),
                token(source.head.as_internal_object_id()),
                source.commit,
                source.tree,
                stats.reused_files,
                stats.source_blobs_read,
                stats.source_bytes_read,
                stats.predecessor_tables_read,
                stats.predecessor_payload_bytes
            ))
        }
        Action::Query(query) => {
            let report =
        ...
        Action::Recover(candidate) => {
            ...
            let state = match report {
                GenerationRecovery::Uninitialized => "uninitialized",
                GenerationRecovery::Active { .. } => "active",
                GenerationRecovery::Superseded { .. } => "superseded",
                GenerationRecovery::NotInSelectedHistory { .. } => "not_in_selected_history",
            };
            Ok(format!(
                "{{\"type\":\"symbol_index_recovery\",\"candidate_token\":\"{}\",\"state\":\"{state}\",\"read_only\":true,\"negative_observation_proves_rollback\":false}}",
...
fn main() -> ExitCode {
    ...
    let result = execute(&node, &options);
    let shutdown = node.shutdown();
    let ok = match result {
        Ok(body) => match writeln!(io::stdout().lock(), "{body}") {
            Ok(()) => true,
            Err(e) => {
                eprintln!("Operation succeeded, but output failed: {e}. Do not infer rollback.");
                false
            }
        },
        Err(AccessError::Publication { candidate, cause }) => {
            eprintln!(
                "Publication was not confirmed: {cause}\nCandidate: {}\nRecover the original candidate; this error does not prove rollback.",
                token(&candidate)
            );
            false
        }
        Err(e) => {
            eprintln!("Symbol index operation failed: {e}");
            false
        }
    };
    if let Err(e) = shutdown {
        eprintln!("Shutdown failed: {e}. Confirmed index activations are not undone.");
        return ExitCode::FAILURE;
    }
    if ok {
        ExitCode::SUCCESS
...
mod tests {
    ...
    fn args(tail: &[&str]) -> Vec<OsString> {
        ...
        .map(OsString::from)
        .collect();
        out.extend(tail.iter().map(|s| OsString::from(*s)));
        out
    }
    #[test]
    fn build_and_recovery_never_guess_or_refresh_predecessors() {
        ...
        assert!(parse(&args(&["build", &id])).is_ok());
        assert!(parse(&args(&["recover", &id])).is_ok());
        for tail in [
            &["build", "latest"][..],
            &["build", "genesis", "force"],
            &["recover", "genesis"],
    ...
    fn refresh_requires_an_exact_existing_generation_token() {
        let id = format!("alg:2:{}", "a".repeat(64));
        assert!(matches!(
            parse(&args(&["refresh", &id])).unwrap().action,
            Action::Refresh(_)
        ));
        for tail in [
            &["refresh"][..],
    ...
    fn query_grammar_is_explicit_and_bounded() {
        assert!(parse(&args(&["query", "prefix", "Thing"])).is_ok());
        assert!(parse(&args(&["query", "exact", "type"])).is_ok());
        for tail in [
            &["query", "regex", "Thing"][..],
            &["query", "prefix"],
            &["query", "exact", "r#type"],
    ...
    fn namespace_tokens_and_full_refs_are_checked_before_open() {
        for i in [1, 2, 3, 4] {
            let mut input = args(&["build", "genesis"]);
            input[i] = OsString::from("bad");
        ...
        for token in [
            ...
            assert!(generation(&token).is_err());
        }
    }
}
