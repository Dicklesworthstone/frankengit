...
fn opaque_image(custody: &Custody, wal: bool) -> PathBuf {
    ...
    if wal {
        fs::write(path.join(WAL), b"closed-WAL-fixture").unwrap();
    }
    path
}
...
fn intent_is_exactly_framed_and_cannot_change_format_pin_or_instance() {
    ...
    trailing.push(0);
    fs::write(&marker, trailing).unwrap();
    assert!(resume(&scratch.root()).is_err());
...
fn wal_move_interruption_normalizes_before_database_open_and_can_repeat() {
    ...
    let error = custody
        .publish(|| Err("stop after WAL".into()))
        .unwrap_err();
    ...
    assert_eq!(
        fs::read(quarantine.join(WAL)).unwrap(),
...
fn no_replace_publication_preserves_a_racing_destination_and_private_image() {
    ...
    assert!(
        custody
            .publish(|| {
                fs::write(scratch.root().join(DATABASE), b"winner").unwrap();
                Ok(())
            })
            .is_err()
    );
...
fn cleanup_never_erases_unknown_files_and_missing_both_images_cannot_reinitialize() {
    ...
    assert_eq!(
        fs::read(quarantine.join("unrecognized-evidence")).unwrap(),
        b"retain"
    );
...
fn unowned_legacy_or_source_restore_directories_are_never_adopted() {
    ...
    fs::write(
        root.join(".restore-intent"),
        binding([7; 32], StoreInstanceId::from_raw(42)),
    )
    .unwrap();
    assert!(
        resume(&root)
            .unwrap_err()
            .contains("missing original intent")
    );
