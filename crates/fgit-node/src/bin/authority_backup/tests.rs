...
impl Scratch {
    fn new() -> Self {
        ...
        let path =
            std::env::temp_dir().join(format!("fg-authority-command-{}-{id}", std::process::id()));
...
fn strict_command_grammar_requires_local_authority_and_independent_restore_pins() {
    assert!(matches!(
        parse(&args(&["export", "source", "backup", "--trusted-local"]))
            .unwrap()
            .mode,
        Mode::Export
    ));
    ...
    assert!(
        matches!(parse(&restore).unwrap().mode, Mode::Restore { instance, .. } if instance.raw() == 52)
    );
    for bad in [
        vec![],
        vec!["export", "source", "backup"],
...
fn checksum_codec_and_same_instance_refusals_precede_destination_creation() {
    ...
    assert!(
        restore(&input, &target, [0; 32], StoreInstanceId::from_raw(52))
            .unwrap_err()
            .contains("checksum")
    );
    ...
    assert!(
        restore(
            &input,
            &target,
            sha256(b"invalid portable bytes"),
