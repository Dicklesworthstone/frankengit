...
enum Stage { Intent, Imported, Reopened, WalPrepared, Published, FinalVerified, Cleaned }

pub(super) fn execute(input: &Path, root: &Path, pin: [u8; 32], instance: StoreInstanceId,
    resume: bool,
) -> Result<String, String> {
    execute_with(input, root, pin, instance, resume, |_| Ok(()))
}

fn image(root: &Path, source: &MultiHeadSnapshot, instance: StoreInstanceId,
    import: bool, resume: bool,
) -> Result<Vec<HeadReadReceipt>, String> {
    let database = root.join("authority.fsqlite");
    if !import { regular(&database)?; }
    with_store(&database, instance, !import, |runtime, store, cx| {
        ...
        let checked = runtime.block_on(store.verify_multi_head_import(cx, source, Default::default()))
            .map_err(|error| format!("all-heads whole-image verification refused: {error}"))?;
        if checked.len() != source.heads.len() || imported.as_ref().is_some_and(|prior| *prior != checked) {
            return Err("all-heads publication receipts disagree with the complete image".into());
        }
        for receipt in &checked {
            runtime.block_on(store.authenticate_head_receipt(cx, receipt))
...
fn execute_with(input: &Path, root: &Path, pin: [u8; 32], instance: StoreInstanceId,
    resume: bool, mut checkpoint: impl FnMut(Stage) -> Result<(), String>,
) -> Result<String, String> {
    if !resume { require_absent(root)?; }
    let bytes = read_backup(input)?;
    if sha256(&bytes) != pin { return Err("backup checksum mismatch; destination untouched".into()); }
    ...
    let outcome = (|| {
        let expected = if already_published {
            ...
            if reopened != imported { return Err("quarantined receipts changed across reopen".into()); }
        ...
        if final_image != expected { return Err("final-location receipts changed across reopen".into()); }
    ...
    outcome.map_err(|error: String| match custody.published() {
        Ok(false) => format!("{error}; final authority absent; private restore state retained at {}", root.display()),
        Ok(true) => format!("{error}; final authority is visible; verification/cleanup incomplete at {}", root.display()),
        Err(inspect) => format!("{error}; publication state could not be inspected: {inspect}; preserve {}", root.display()),
    })?;
    Ok(format!(concat!(
        ...
        quote(&hex(&pin)), source.bodies.len(), source.heads.len(), source.issuance.len(),
        source.instance, instance.raw(), quote(&root.join("authority.fsqlite").to_string_lossy()),
        resume, already_published))
}
