...
use fgit_authority_fsqlite::{ExportedBody, ExportedHead, ExportedIssuance,
    IssuanceSequence, SCHEMA_VERSION, encode_multi_head_snapshot, mint_token};

static NEXT: AtomicU64 = AtomicU64::new(0);
struct Scratch(PathBuf);
impl Scratch {
    fn new() -> Self {
        let path = std::env::temp_dir().join(format!("fg-all-heads-recovery-{}-{}",
            std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
        fs::create_dir(&path).unwrap(); Self(path)
    }
    fn archive(&self, source: &MultiHeadSnapshot) -> (PathBuf, [u8; 32]) {
        let path = self.0.join("backup");
        let bytes = encode_multi_head_snapshot(source, Default::default(), Default::default()).unwrap();
        fs::write(&path, &bytes).unwrap(); (path, sha256(&bytes))
    }
}
impl Drop for Scratch { fn drop(&mut self) { let _ = fs::remove_dir_all(&self.0); } }
fn target() -> StoreInstanceId { StoreInstanceId::from_raw(42) }
fn source() -> MultiHeadSnapshot {
    let mut issuance = Vec::new();
    for (index, (key, generation)) in [(b"a", 5), (b"b", 1), (b"a", 9), (b"b", 2)].into_iter().enumerate() {
        let sequence = index as u64 + 1;
        issuance.push(ExportedIssuance {
            token: mint_token(StoreInstanceId::from_raw(41), IssuanceSequence::new(sequence).unwrap()).to_opaque_bytes().to_vec(),
            sequence, head_key: key.to_vec(), generation, body: vec![index as u8, 0, 255],
        });
    }
    let heads = issuance[2..].iter().map(|row| ExportedHead {
        key: row.head_key.clone(), token: row.token.clone(), generation: row.generation, body: row.body.clone(),
    }).collect();
    MultiHeadSnapshot { schema_version: SCHEMA_VERSION, instance: 41,
        bodies: vec![ExportedBody { key: b"original".to_vec(), body: b"immutable\0bytes".to_vec() }], heads, issuance }
}
fn observed(root: &Path) -> MultiHeadSnapshot {
    with_store(&root.join("authority.fsqlite"), target(), true, |runtime, store, cx|
        runtime.block_on(store.export_multi_head_portable(cx, Default::default())).map_err(|e| e.to_string())).unwrap()
}

#[test]
fn every_restore_phase_resumes_from_archive_alone_without_minting_again() {
    for stopped in [Stage::Intent, Stage::Imported, Stage::Reopened, Stage::WalPrepared,
        Stage::Published, Stage::FinalVerified, Stage::Cleaned]
    {
        let scratch = Scratch::new(); let source = source();
        let (archive, pin) = scratch.archive(&source); let root = scratch.0.join("target");
        let error = execute_with(&archive, &root, pin, target(), false, |stage| {
            if stage == stopped { Err("deliberate interruption".into()) } else { Ok(()) }
        }).unwrap_err();
        assert!(error.contains("deliberate interruption"));
        let was_published = matches!(stopped, Stage::Published | Stage::FinalVerified | Stage::Cleaned);
        assert_eq!(root.join("authority.fsqlite").exists(), was_published, "{stopped:?}");
        ...
        assert_eq!(observed(&root), first, "retry must not issue tokens: {stopped:?}");
...
fn advanced_secondary_head_and_extra_body_refuse_without_rewriting_any_state() {
    for change in 0..2 {
        ...
        let root = scratch.0.join("target"); execute(&archive, &root, pin, target(), false).unwrap();
        with_store(&root.join("authority.fsqlite"), target(), true, |runtime, store, cx| {
            if change == 0 {
                let snapshot = runtime.block_on(store.export_multi_head_portable(cx, Default::default())).unwrap();
                let head = &snapshot.heads[1];
                let key = fgit_authority::HeadKey::new(head.key.clone()).unwrap();
                let fgit_authority::HeadRead::Present(receipt) = runtime.block_on(store.read_head(cx, &key)).unwrap()
                    else { panic!("second head exists") };
                assert!(matches!(runtime.block_on(store.compare_exchange_head(cx, &key, receipt.token(),
                    HeadGeneration::try_new(head.generation + 1).unwrap(), b"new accepted work")).unwrap(), CasOutcome::Committed(_)));
            } else {
                runtime.block_on(store.put_if_absent(cx, &ImmutableKey::new(b"extra".to_vec()).unwrap(), b"new evidence")).unwrap();
            }
            Ok(())
        }).unwrap();
        let before = observed(&root);
        assert!(execute(&archive, &root, pin, target(), true).unwrap_err().contains("whole-image"));
...
fn partial_occupied_quarantine_is_not_filled_in_as_a_retry() {
    ...
    with_store(&quarantine.join("authority.fsqlite"), target(), false, |runtime, store, cx| {
        runtime.block_on(store.put_if_absent(cx, &ImmutableKey::new(b"unexpected".to_vec()).unwrap(), b"retained")).unwrap();
        Ok(())
    }).unwrap();
    let before = observed(&quarantine); drop(custody);
...
fn empty_whole_store_uses_the_same_publication_and_retry_path() {
    let scratch = Scratch::new(); let mut empty = source();
    empty.bodies.clear(); empty.heads.clear(); empty.issuance.clear();
    let (archive, pin) = scratch.archive(&empty); let root = scratch.0.join("target");
    execute(&archive, &root, pin, target(), false).unwrap();
    let first = observed(&root); assert!(first.heads.is_empty());
