...
impl Encoder {
    pub fn new(identity: Identity, authority: &[u8], objects: usize) -> Result<Self, String> {
        ...
        if authority.is_empty() {
            return Err("missing authority transport".into());
        }
        let mut out = Self {
            bytes: Vec::new(),
            identity,
        ...
        out.push(&[match identity.format {
    ...
    fn object_inner(
        ...
        if self.remaining == 0
            || oid.algorithm() != self.identity.format
            || oid.is_zero()
    ...
    fn push(&mut self, part: &[u8]) -> Result<(), String> {
        ...
        self.bytes
            ...
            .map_err(|_| "repository backup allocation refused")?;
...
impl<'a> Reader<'a> {
    fn take(&mut self, count: usize) -> Result<&'a [u8], String> {
        let part = self
            .bytes
            .get(..count)
            .ok_or("truncated repository backup")?;
    ...
    fn array<const N: usize>(&mut self) -> Result<[u8; N], String> {
        self.take(N)?
            .try_into()
            .map_err(|_| "truncated repository backup field".into())
    }
...
pub(super) fn decode(
    bytes: &[u8],
    mut live: impl FnMut() -> Result<(), String>,
) -> Result<Archive<'_>, String> {
    live()?;
    if bytes.len() > MAX_ARCHIVE_BYTES {
        return Err("repository backup input exceeds 64 MiB".into());
    }
    ...
    let authority = import_bundle(input.take(size)?)
    ...
    if count > input.bytes.len() / minimum {
        return Err("truncated repository backup object inventory".into());
    }
    let mut records = Vec::new();
    records
        .try_reserve_exact(count)
        .map_err(|_| "repository backup record allocation refused")?;
    let mut previous = None;
    for _ in 0..count {
        live()?;
        let raw = input.take(format.digest_len())?;
        let oid = GitOid::from_hex(format, &super::super::hex(raw))
    ...
    if !input.bytes.is_empty() {
        return Err("trailing repository backup bytes".into());
    }
    live()?;
    Ok(Archive {
        identity: Identity {
