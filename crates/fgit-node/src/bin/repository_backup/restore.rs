...
The 300 second default deadline is shared across ALL passes, not reset per file
or object. Bounds can be explicit; codec/runtime limits can refuse earlier.

Metadata and objects are staged under .restore-quarantine; graph/selection,
original commitments and disk readback must verify before authority.fsqlite
is published at the root. The closed database's WAL, if any, is installed BEFORE
that database. A remaining nonempty rollback journal refuses publication.
Shared-memory caches are rebuilt. A fresh reopen is checked before success.
Failed/interrupted directories remain for investigation; existence never implies
completion. Do not concurrently open or modify the reserved root/quarantine.
Trusted parent paths are required. Deadlines cannot interrupt blocking I/O.
...
#[derive(Debug)]
struct Options { input: PathBuf, output: PathBuf, expected: [u8; 32], instance: StoreInstanceId, profile: Profile }
...
fn parse(args: &[String]) -> Result<Options, String> {
    if !(8..=12).contains(&args.len()) || args[0] != "restore" || args.iter().any(|arg| arg.len() > 8192)
    ...
    Ok(Options { input: args[1].clone().into(), output,
        ...
        instance: instance.ok_or("restore requires a new --destination-instance")?, profile: profile.finish() })
}
...
fn graph_from_archive(node: &OneNode, archive: &mut PinnedArchive, expected: &HeadReadReceipt,
    install: bool, profile: Profile, deadline: Deadline,
    ...
    let mut graph = ObjectGraphAudit::new(objects, archive.header().identity.format,
        limits(profile.transfer), &mut checkpoint).map_err(|e| e.to_string())?;
    ...
    archive.scan(deadline, |record| {
        ...
        if !install { verify_stored(node, &record)?; }
        deadline.in_request(&request)
    })?;
    ...
    if install {
        archive.scan(deadline, |record| {
            ...
            if !objects.contains(&record.oid) { return Err("archive inventory changed before placement".into()); }
...
fn execute(options: &Options) -> Result<String, String> {
    ...
    let prepared = (|| {
        ...
        let expected = with_store(&quarantine.join("authority.fsqlite"), options.instance, false, |runtime, store, cx| {
            deadline.check()?;
            let head = runtime.block_on(store.import_portable(cx, &archive.header().authority, Default::default()))
            ...
            let source = archive.header().authority.head.as_ref().ok_or("missing source head")?;
    ...
    publication.publish()?;
    let reopened = with_node(config(&options.output, archive.header()), |node|
        graph_from_archive(node, &mut archive, &expected, false, options.profile, deadline))
