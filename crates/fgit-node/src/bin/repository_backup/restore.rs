...
pub(super) const USAGE: &str =
    "usage: fg-repository-backup restore <backup-file> <new-storage-root> --trusted-local
...
fn parse(args: &[String]) -> Result<Options, String> {
    ...
    if output.file_name().is_none() {
        return Err("restore requires a new named storage root".into());
    }
    Ok(Options {
...
impl PreparedPublication {
        ...
        match fs::symlink_metadata(&wal) {
            Ok(metadata) if metadata.is_file() => {
                File::open(&wal)
                    .and_then(|file| file.sync_all())
                    .map_err(|e| e.to_string())?;
                fs::hard_link(&wal, destination.join("authority.fsqlite-wal"))
                    .map_err(|e| e.to_string())?;
            }
    ...
    fn publish(self) -> Result<(), String> {
        ...
        fs::remove_dir_all(&self.quarantine).map_err(|e| {
            format!("destination authority is visible; quarantine cleanup failed: {e}")
        })?;
        sync_directory(&self.destination)
            .map_err(|e| format!("destination authority is visible; cleanup sync failed: {e}"))
    }
}
    ...
    let prepared = (|| {
        ...
        let expected = with_store(
            ...
            |runtime, store, cx| {
                deadline.check()?;
                let head = runtime
                    .block_on(store.import_portable(
                        cx,
