...
impl Scratch {
    fn new() -> Self {
        let root = std::env::temp_dir().join(format!("fg-source-publication-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
        fs::create_dir(&root).unwrap(); Self(root)
    }
    fn stage(&self) -> (PathBuf, PathBuf) {
        let target = self.0.join("target"); create_private(&target).unwrap();
        let stage = target.join(".restore-quarantine"); create_private(&stage).unwrap();
...
fn args() -> Vec<String> {
    ["restore", "/unused", "/new-root", "--trusted-local", "--expected-sha256",
        &"ab".repeat(32), "--destination-instance", "12"].into_iter().map(str::to_owned).collect()
}
#[test]
fn restore_requires_exact_trust_pin_and_fresh_positive_instance() {
    ...
    for bad in ["", "0", "01", "+1", "-1", "9223372036854775808"] {
        let mut input = args(); input[7] = bad.into(); assert!(parse(&input).is_err(), "{bad}");
    }
    for bad in ["", "a", "AB", &"AB".repeat(32), &"a".repeat(65)] {
        let mut input = args(); input[5] = bad.into(); assert!(parse(&input).is_err());
    }
    let mut untrusted = args(); untrusted[3] = "--untrusted".into(); assert!(parse(&untrusted).is_err());
    let mut duplicate = args(); duplicate[6] = "--expected-sha256".into(); assert!(parse(&duplicate).is_err());
    assert!(parse(&[]).is_err());
}
#[test]
fn interruption_after_data_preparation_leaves_no_public_authority() {
    ...
    let scratch = Scratch::new(); let (stage, target) = scratch.stage();
    ...
    assert_eq!(fs::read(target.join("objects/body")).unwrap(), b"already-verified-body");
    assert_eq!(fs::read(target.join("authority.fsqlite-wal")).unwrap(), b"closed-WAL");
    assert!(!target.join("authority.fsqlite-shm").exists());
    drop(ready);
    assert!(!target.join("authority.fsqlite").exists(), "abandonment is not publication");
    assert!(stage.join("authority.fsqlite").exists(), "retain interruption evidence");
}
#[test]
fn publication_is_no_replace_and_preserves_both_images_on_collision() {
    ...
    assert_eq!(fs::read(target.join("authority.fsqlite")).unwrap(), b"existing-winner");
    assert_eq!(fs::read(stage.join("authority.fsqlite")).unwrap(), b"already-closed-image");
}
#[test]
fn successful_file_publication_keeps_data_and_reaps_only_owned_quarantine() {
    let scratch = Scratch::new(); let (stage, target) = scratch.stage();
    fs::write(stage.join("authority.fsqlite-wal"), b"closed-WAL").unwrap();
    PreparedPublication::prepare(&stage, &target).unwrap().publish().unwrap();
    assert!(!stage.exists());
    assert_eq!(fs::read(target.join("authority.fsqlite")).unwrap(), b"already-closed-image");
    assert_eq!(fs::read(target.join("authority.fsqlite-wal")).unwrap(), b"closed-WAL");
