...
use super::archive::{Record, stream::{Seal, StreamDecoder, StreamHeader, TransferLimits}};
...
impl PinnedArchive<File> {
    pub fn open(path: &Path, expected: [u8; 32], limits: TransferLimits, deadline: Deadline) -> Result<Self, String> {
        limits.validate()?; deadline.check()?;
        if regular(path)? > limits.max_archive_bytes { return Err("repository backup archive-byte limit exceeded".into()); }
...
impl<R: Read + Seek> PinnedArchive<R> {
    fn new(mut input: R, expected: [u8; 32], limits: TransferLimits, deadline: Deadline) -> Result<Self, String> {
        limits.validate()?;
        ...
        if seal != initial { return Err("repository backup changed between checksum and validation".into()); }
        Ok(Self { input, header, seal, limits })
    }
    ...
    pub fn scan(&mut self, deadline: Deadline,
        mut visit: impl for<'a> FnMut(Record<'a>) -> Result<(), String>,
    ) -> Result<(), String> {
        deadline.check()?;
        self.input.seek(SeekFrom::Start(0)).map_err(|e| format!("backup input seek failed: {e}"))?;
        let mut live = || deadline.check();
        let mut decoded = StreamDecoder::new(&mut self.input, self.limits, &mut live)?;
        if decoded.header() != &self.header { return Err("repository backup metadata changed between passes".into()); }
        while let Some(record) = decoded.record(&mut live)? { visit(record)?; }
        let (_, seal) = decoded.finish(self.seal.digest, &mut live)?;
        if seal != self.seal { return Err("repository backup length changed between passes".into()); }
        Ok(())
    }
}
fn check_checksum(input: &mut impl Read, expected: [u8; 32], limits: TransferLimits,
    deadline: Deadline,
) -> Result<Seal, String> {
    ...
    loop {
        ...
        bytes = bytes.checked_add(count as u64).filter(|n| *n <= limits.max_archive_bytes)
            .ok_or("repository backup archive-byte limit exceeded")?;
        hash.update(&buffer[..count]);
    }
    deadline.check()?;
    let seal = Seal { digest: hash.finish(), bytes };
    if seal.digest != expected { return Err("repository backup checksum mismatch; no destination created".into()); }
    Ok(seal)
}
