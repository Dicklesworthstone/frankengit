...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct Seal { pub digest: [u8; 32], pub bytes: u64 }
...
impl<W: Write> StreamEncoder<W> {
    pub fn new(output: W, identity: Identity, authority: &[u8], objects: usize,
        limits: TransferLimits, live: &mut impl FnMut() -> Result<(), String>,
    ) -> Result<Self, String> {
        limits.validate()?;
        live()?;
        ...
        let mut encoder = Self { output, hash: Sha256Hasher::new(), bytes: 0, limits,
            format: identity.format, remaining: objects, previous: None, failed: false };
        ...
        encoder.push(&[match identity.format { GitHashAlgorithm::Sha1 => 1, GitHashAlgorithm::Sha256 => 2 }], live)?;
    ...
    pub fn object(&mut self, oid: GitOid, kind: GitObjectKind, payload: &[u8],
        commitment: &[u8; 32], live: &mut impl FnMut() -> Result<(), String>,
    ) -> Result<(), String> {
    ...
    fn object_inner(&mut self, oid: GitOid, kind: GitObjectKind, payload: &[u8],
        ...
        live()?;
        if self.remaining == 0 || oid.algorithm() != self.format || oid.is_zero()
    ...
    fn push(&mut self, bytes: &[u8], live: &mut impl FnMut() -> Result<(), String>) -> Result<(), String> {
        ...
        while !pending.is_empty() {
            ...
            match self.output.write(&pending[..offered]) {
                ...
                Ok(_) => return Err("repository backup writer returned an invalid byte count".into()),
    ...
    pub fn finish(mut self, live: &mut impl FnMut() -> Result<(), String>) -> Result<Seal, String> {
        ...
        live()?;
        self.output.flush().map_err(|e| format!("repository backup flush failed: {e}"))?;
        live()?;
        Ok(Seal { digest: self.hash.finish(), bytes: self.bytes })
    }
}

fn check_room(used: u64, amount: u64, limits: TransferLimits) -> Result<(), String> {
    used.checked_add(amount).filter(|total| *total <= limits.max_archive_bytes)
        .ok_or_else(|| "repository backup archive-byte limit exceeded".to_owned()).map(|_| ())
}

struct Input<R> { reader: R, hash: Sha256Hasher, bytes: u64, limits: TransferLimits }
impl<R: Read> Input<R> {
    fn read_exact(&mut self, output: &mut [u8], live: &mut impl FnMut() -> Result<(), String>) -> Result<(), String> {
        ...
        while !remaining.is_empty() {
            ...
            match self.reader.read(&mut remaining[..offered]) {
                ...
                Ok(_) => return Err("repository backup reader returned an invalid byte count".into()),
                Err(error) if error.kind() == io::ErrorKind::Interrupted => continue,
    ...
    fn array<const N: usize>(&mut self, live: &mut impl FnMut() -> Result<(), String>) -> Result<[u8; N], String> {
        let mut output = [0; N]; self.read_exact(&mut output, live)?; Ok(output)
    }
    fn length(&mut self, maximum: usize, live: &mut impl FnMut() -> Result<(), String>) -> Result<usize, String> {
        usize::try_from(u64::from_be_bytes(self.array(live)?)).ok().filter(|n| *n <= maximum)
            .ok_or_else(|| "repository backup declared length/count exceeds its limit".into())
    }
    fn finish(mut self, expected: [u8; 32], live: &mut impl FnMut() -> Result<(), String>) -> Result<Seal, String> {
        ...
        live()?;
        let seal = Seal { digest: self.hash.finish(), bytes: self.bytes };
        if seal.digest != expected { return Err("repository backup checksum mismatch or input changed during verification".into()); }
        Ok(seal)
    }
}
...
impl<R: Read> StreamDecoder<R> {
    pub fn new(reader: R, limits: TransferLimits, live: &mut impl FnMut() -> Result<(), String>) -> Result<Self, String> {
        limits.validate()?; live()?;
        let mut input = Input { reader, hash: Sha256Hasher::new(), bytes: 0, limits };
        if &input.array::<8>(live)? != MAGIC { return Err("unsupported repository backup transport".into()); }
        ...
        bytes.try_reserve_exact(size).map_err(|_| "repository authority allocation refused")?;
        bytes.resize(size, 0);
        input.read_exact(&mut bytes, live)?;
        let authority = import_bundle(&bytes).map_err(|error| format!("invalid repository authority transport: {error}"))?;
        drop(bytes); live()?;
        ...
        Ok(Self { input, header: StreamHeader { identity: Identity { tenant, repository, incarnation, format }, authority, objects },
            remaining: objects, previous: None, payload: Vec::new(), failed: false })
    }
    pub fn header(&self) -> &StreamHeader { &self.header }
    ...
    pub fn record(&mut self, live: &mut impl FnMut() -> Result<(), String>) -> Result<Option<Record<'_>>, String> {
        if self.failed { return Err("repository backup decoder previously failed".into()); }
        ...
        match result {
            Ok(Some((oid, kind))) => Ok(Some(Record { oid, kind, payload: &self.payload })),
            Ok(None) => Ok(None),
            Err(error) => { self.failed = true; Err(error) }
        }
    }
    fn record_inner(&mut self, live: &mut impl FnMut() -> Result<(), String>) -> Result<Option<(GitOid, GitObjectKind)>, String> {
        live()?;
        if self.remaining == 0 { return Ok(None); }
        let format = self.header.identity.format;
        let mut raw = [0; 32];
        self.input.read_exact(&mut raw[..format.digest_len()], live)?;
        let text = raw[..format.digest_len()].iter().map(|byte| format!("{byte:02x}")).collect::<String>();
        ...
        self.payload.clear();
        self.payload.try_reserve_exact(length).map_err(|_| "repository backup object allocation refused")?;
        ...
        live()?;
        self.remaining -= 1; self.previous = Some(oid);
        Ok(Some((oid, kind)))
    }
    pub fn finish(self, expected: [u8; 32], live: &mut impl FnMut() -> Result<(), String>) -> Result<(StreamHeader, Seal), String> {
        if self.failed || self.remaining != 0 { return Err("incomplete repository backup object set".into()); }
