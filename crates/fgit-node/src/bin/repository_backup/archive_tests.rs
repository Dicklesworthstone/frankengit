...
fn commitment(kind: GitObjectKind, body: &[u8]) -> [u8; 32] {
    git_payload_commitment(kind, body, CANONICAL_CODEC_VERSION)
        .digest()
        .as_bytes()
        .try_into()
        .unwrap()
...
fn sample(format: GitHashAlgorithm) -> Vec<u8> {
    ...
    encoder
        .object(id, kind, body, &commitment(kind, body))
        .unwrap();
...
fn wrong_identity_and_original_commitment_are_independent_refusals() {
    ...
    *payload.last_mut().unwrap() ^= 1;
    ...
    commitment[record + 32 + 1 + 8] ^= 1;
    assert!(
        decode(&commitment, || Ok(()))
            .unwrap_err()
            .contains("commitment mismatch")
    );
    let mut oid = original.clone();
    oid[record] ^= 1;
    assert!(
        decode(&oid, || Ok(()))
            .unwrap_err()
...
fn unsupported_versions_formats_counts_and_sizes_refuse_before_reservation() {
    ...
    bytes[57..65].copy_from_slice(&u64::MAX.to_be_bytes());
    assert!(decode(&bytes, || Ok(())).is_err());
    let count_at = 65 + authority().len();
    let mut count = original.clone();
    count[count_at..count_at + 8].copy_from_slice(&u64::MAX.to_be_bytes());
