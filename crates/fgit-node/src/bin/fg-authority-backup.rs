...
use fgit_authority_fsqlite::{
    ExportBundle, FsqliteAuthorityStore, PortableStoreLimits, export_bundle, import_bundle,
};
...
const USAGE: &str = "usage: fg-authority-backup export <existing-authority.fsqlite> <new-backup-file> --trusted-local [--all-heads]
       fg-authority-backup restore <backup-file> <new-directory> --trusted-local
         --expected-sha256 <64-lowercase-hex> --destination-instance <positive-integer> [--all-heads [--resume]]
...
All-heads restore stages into a private quarantine and publishes authority.fsqlite
last, after complete-image verification and reopen. To continue an owned attempt,
use restore --all-heads --resume with its ORIGINAL checksum and destination
instance. Exact completed retries verify only; changed/newer state refuses.
Legacy directories without the original all-heads intent cannot be adopted.
Keep the destination offline: restore locks do not exclude ordinary node tools.
...
fn main() -> ExitCode {
    ...
    match run(&args, &mut std::io::stdout().lock()) {
        ...
        Err(error) => {
            eprintln!(
                "{{\"type\":\"authority_backup_error\",\"schema_version\":1,\"complete\":false,\"error\":{}}}",
                quote(&error)
            );
            ExitCode::from(2)
        }
...
fn parse(args: &[String]) -> Result<Options, String> {
    ...
    if !trusted {
        return Err("--trusted-local and whole-store operator authorization are required".into());
    }
    if resume && (args[0] != "restore" || !all_heads) {
        return Err("--resume is supported only by restore --all-heads".into());
    }
    let mode = match args[0].as_str() {
        "export" if expected.is_none() && instance.is_none() => Mode::Export,
        "restore" => Mode::Restore {
            expected: expected
                .ok_or("restore requires an independently trusted --expected-sha256")?,
    ...
    if output.file_name().is_none() {
        return Err("destination must have a final path component".into());
    }
    Ok(Options {
...
fn read_backup(path: &Path) -> Result<Vec<u8>, String> {
    if regular(path)? > MAX_BYTES as u64 {
        return Err("backup exceeds the 64 MiB input limit".into());
    }
    ...
    if bytes.is_empty() || bytes.len() > MAX_BYTES {
        return Err("empty or oversized backup".into());
    }
    Ok(bytes)
}
...
fn run(args: &[String], output: &mut impl Write) -> Result<(), String> {
    ...
    let receipt = match options.mode {
        ...
        Mode::Restore { expected, instance } if options.all_heads => {
            if options.resume {
                multihead::resume(&options.input, &options.output, expected, instance)?
            } else {
...
fn export(input: &Path, destination: &Path) -> Result<String, String> {
    ...
    let (bytes, bodies, issuance, generation, instance) = with_store(
        input,
        ...
        |runtime, store, cx| {
            ...
            if bytes.len() > MAX_BYTES {
                return Err("serialized backup exceeds 64 MiB".into());
            }
            ...
            let decoded = import_bundle(&bytes)
                .map_err(|e| format!("export is outside the restore codec envelope: {e}"))?;
            if decoded != bundle {
                return Err("portable encoding changed its source snapshot".into());
            }
            Ok((
                bytes,
                bundle.bodies.len(),
    ...
    Ok(format!(
        concat!(
            "{{\"type\":\"authority_backup_export\",\"schema_version\":1,\"sha256\":{},",
...
fn restore(
    ...
    let bundle = import_bundle(&bytes)
        .map_err(|e| format!("invalid backup; no destination created: {e}"))?;
    if bundle.instance == instance.raw() {
    ...
    builder
        .create(destination)
        .map_err(|e| format!("cannot create new restore directory: {e}"))?;
    ...
    sync_directory(destination)
        .map_err(|e| format!("restore committed and store closed; directory sync failed: {e}"))?;
    sync_directory(parent(destination))
        .map_err(|e| format!("restore committed and store closed; parent sync failed: {e}"))?;
    Ok(format!(
        concat!(
            "{{\"type\":\"authority_backup_restore\",\"schema_version\":1,\"sha256\":{},",
...
fn publish_new(destination: &Path, bytes: &[u8]) -> Result<(), String> {
    ...
    for _ in 0..16 {
        ...
        let path = parent(destination).join(format!(
            ".fg-authority-backup-{}-{sequence}.tmp",
            std::process::id()
        ));
    ...
    if let Err(error) = written {
        return Err(remove_failed_stage(
            &temporary,
            format!("backup staging failed: {error}"),
        ));
    }
    if let Err(error) = fs::hard_link(&temporary, destination) {
        return Err(remove_failed_stage(
            &temporary,
            format!("backup publication failed: {error}"),
        ));
    }
    fs::remove_file(&temporary).map_err(|e| {
        format!(
            "backup is visible at {}; staging cleanup failed at {}: {e}",
            destination.display(),
            temporary.display()
        )
    })?;
    sync_directory(parent(destination))
        .map_err(|e| format!("backup is visible; parent sync failed: {e}"))?;
    Ok(())
}
