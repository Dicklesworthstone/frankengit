...
fn generation(text: &str) -> Result<GraphGenerationId, String> {
    ...
    let raw = text.strip_prefix(&prefix).ok_or("Wrong generation algorithm/token.")?;
    ...
    if bytes.iter().all(|b| *b == 0) { return Err("Zero generation is not a candidate.".to_owned()); }
...
fn parse(args: &[OsString]) -> Result<Command, String> {
    ...
    if args.iter().any(|a| a.len() > 4096) { return Err("Argument exceeds 4096 bytes.".to_owned()); }
    let text = |i: usize| args[i].to_str().ok_or_else(|| "Only the node root may contain non-UTF-8 bytes.".to_owned());
    let tenant = TenantId::from_bytes(decode(text(1)?, 16)?.try_into().map_err(|_| "Invalid tenant.")?);
    let repository = RepositoryId::from_bytes(decode(text(2)?, 16)?.try_into().map_err(|_| "Invalid repository.")?);
    ...
    let action = match text(5)? {
        ...
        "query" if args.len() >= 8 => {
            ...
            let terms = (7..args.len()).map(|i| text(i).map(|s| s.as_bytes().to_vec())).collect::<Result<Vec<_>, _>>()?;
            Action::Query(LexicalQuery::new(channel, &terms, &[]).map_err(|e| e.to_string())?)
        }
        _ => return Err(USAGE.to_owned()),
    };
    Ok(Command { root: PathBuf::from(&args[0]), tenant, repository, format, reference, action })
}
fn activation_fields(activation: &GenerationActivation) -> String {
    format!("\"index_token\":\"{}\",\"index_number\":{}", token(activation.generation_id.as_internal_object_id()), activation.authority_generation.get())
}
fn execute(node: &OneNode, command: &Command) -> Result<String, NodeWorkspaceRefusal> {
    ...
    match &command.action {
        Action::Build(predecessor) => {
            let (source, activation) = node.runtime().block_on(node.build_source_index_local_in(&context,
        ...
        Action::Recover(candidate) => {
            ...
            let (state, selected) = match recovered {
                ...
                GenerationRecovery::NotInSelectedHistory { selected } => ("not_in_selected_history", Some(selected)),
            };
            let selected = selected.map_or_else(|| "null".to_owned(), |s| format!("{{{}}}", activation_fields(s.activation())));
            Ok(format!("{{\"type\":\"index_recovery\",\"candidate_token\":\"{}\",\"state\":\"{state}\",\"selected\":{selected},\"read_only\":true}}", token(candidate.as_internal_object_id())))
        }
        Action::Query(query) => {
            let report = node.runtime().block_on(node.search_source_index_local_in(&context, &command.reference,
                None, None, None, None, query, None, Default::default(), Default::default()))?;
            let hits = report.results.hits.iter().map(|h| format!("{{\"document_id\":{},\"path_hex\":\"{}\",\"blob\":\"{}\"}}", h.document_id, hex(&h.path), h.blob))
                .collect::<Vec<_>>().join(",");
            Ok(format!("{{\"type\":\"index_query\",{},\"snapshot_token\":\"{}\",\"source_commit\":\"{}\",\"complete\":{},\"next_after\":{},\"hits\":[{}],\"read_only\":true}}",
                activation_fields(&report.generation), token(report.source.source_head.as_internal_object_id()), report.source.commit,
                report.results.complete, report.results.next_after.map_or_else(|| "null".to_owned(), |n| n.to_string()), hits))
        }
    }
}
fn main() -> ExitCode {
    let args: Vec<_> = std::env::args_os().skip(1).take(40).collect();
    let command = match parse(&args) { Ok(c) => c, Err(e) => { eprintln!("{e}"); return ExitCode::FAILURE; } };
    let config = NodeConfig::new(command.root.clone(), command.tenant, command.repository).with_object_format(command.format).with_worker_threads(2);
    let mut node = match OneNode::open_existing(config) { Ok(n) => n, Err(e) => { eprintln!("Open failed: {e}"); return ExitCode::FAILURE; } };
    let ready = node.runtime().block_on(node.authenticate_authority_head())
        .map_err(|e| e.to_string()).and_then(|selected| node.bring_into_service(selected.receipt().generation()).map_err(|e| e.to_string()));
    if let Err(error) = ready {
        eprintln!("Source authority unavailable: {error}");
        if let Err(close) = node.shutdown() { eprintln!("Shutdown also failed: {close}"); }
    ...
    let output_ok = match result {
        Ok(body) => match writeln!(io::stdout().lock(), "{body}") {
            Ok(()) => true,
            Err(e) => { eprintln!("Operation returned successfully but its receipt could not be written: {e}. Do not infer rollback."); false }
        },
        Err(NodeWorkspaceRefusal::SourceIndexPublication { candidate, error }) => {
            eprintln!("Index publication was not confirmed: {error}\nCandidate: {}\nRecover this candidate; this error does not establish rollback.", token(candidate.as_internal_object_id())); false
        }
        Err(e) => { eprintln!("Operation failed: {e}"); false }
    };
    if let Err(error) = shutdown {
        eprintln!("Node shutdown failed: {error}. This does not undo any reported index activation.");
        return ExitCode::FAILURE;
    }
    if output_ok { ExitCode::SUCCESS } else { ExitCode::FAILURE }
}
#[cfg(test)]
mod tests {
    use super::*;
    fn args(action: &[&str]) -> Vec<OsString> {
        ...
        out.extend(action.iter().map(|s| OsString::from(*s))); out
    }
    #[test]
    fn build_requires_explicit_genesis_or_exact_predecessor_before_open() {
        ...
        assert!(matches!(parse(&args(&["build", &token])).unwrap().action, Action::Build(Some(_))));
        for tail in [&["build"][..], &["build", "latest"], &["build", "genesis", "force"], &["init", "genesis"]] { assert!(parse(&args(tail)).is_err()); }
    }
    #[test]
    fn queries_are_bounded_whole_tokens_and_recovery_cannot_refresh_a_candidate() {
        ...
        for tail in [&["query", "content"][..], &["query", "symbol", "thing"], &["query", "content", "a.*"], &["recover", "latest"], &["recover", "genesis"]] { assert!(parse(&args(tail)).is_err()); }
        let mut many = args(&["query", "path"]); many.extend((0..33).map(|_| OsString::from("a"))); assert!(parse(&many).is_err());
    }
    #[test]
