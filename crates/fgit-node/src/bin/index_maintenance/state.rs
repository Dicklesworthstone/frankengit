...
pub fn decimal(text: &str, maximum: u64) -> io::Result<u64> {
    ...
    {
        return Err(invalid("invalid decimal"));
    }
    text.parse::<u64>()
        .ok()
        .filter(|n| *n <= maximum)
        .ok_or_else(|| invalid("decimal exceeds limit"))
}
fn digest(text: &str) -> io::Result<[u8; 32]> {
    let value: [u8; 32] = unhex(text, 32)?
        .try_into()
        .map_err(|_| invalid("invalid digest width"))?;
    if value == [0; 32] {
        return Err(invalid("zero generation digest"));
    }
    Ok(value)
}
...
impl State {
    pub fn new(binding: String, refs: &[Vec<u8>]) -> io::Result<Self> {
        if binding.is_empty()
            ...
            || refs.is_empty()
            || refs.len() > MAX_REFS
            || refs.iter().map(Vec::len).sum::<usize>() > 16 * 1024
        {
        ...
        for reference in refs {
            if reference.is_empty()
                || reference.len() > 1024
                || rows.insert(reference.clone(), Row::default()).is_some()
            {
    ...
    fn encode_version(&self, legacy: bool) -> io::Result<Vec<u8>> {
        ...
        for (reference, row) in &self.rows {
            if usize::from(row.running)
                + usize::from(row.preparing)
                + usize::from(row.pending.is_some())
                > 1
                || (legacy && row.preparing)
            {
                return Err(invalid("contradictory attempt state"));
            }
            let (floor, number) = row
                .floor
                .as_ref()
                .map_or(("-".to_owned(), 0), |p| (hex(&p.digest), p.number));
            if row
                .floor
                .as_ref()
                .is_some_and(|p| p.number == 0 || p.digest == [0; 32])
                || row.pending == Some([0; 32])
            {
            ...
            let pending = row
                .pending
                .as_ref()
                .map_or_else(|| "-".to_owned(), |d| hex(d));
            let phase = if row.preparing {
                "p"
            } else if row.running {
                "1"
            } else {
            ...
            out.push_str(&format!(
                "{} {floor} {number} {pending} {phase}\n",
                hex(reference)
            ));
            if out.len() > MAX_STATE_BYTES {
                return Err(invalid("progress size limit"));
            }
        }
        Ok(out.into_bytes())
    }
    pub fn decode(bytes: &[u8], expected: &Self) -> io::Result<Self> {
        ...
        let header = lines
            .next()
            .ok_or_else(|| invalid("missing progress header"))?;
        let legacy = if header == format!("{MAGIC} {}", expected.binding) {
            false
        } else if header == format!("{LEGACY_MAGIC} {}", expected.binding) {
            true
        } else {
            return Err(invalid("progress namespace mismatch"));
        };
        ...
        for line in lines {
            ...
            if fields.len() != 5 {
                return Err(invalid("invalid progress row"));
            }
            ...
            let floor = match (fields[1], number) {
                ...
                (value, number) => Some(Pin {
                    digest: digest(value)?,
                    number,
                }),
            };
            let pending = if fields[3] == "-" {
                None
            } else {
                Some(digest(fields[3])?)
            };
            let (running, preparing) = match fields[4] {
                "0" => (false, false),
                "1" => (true, false),
                "p" if !legacy => (false, true),
        ...
        if seen.len() != result.rows.len() || result.encode_version(legacy)? != bytes {
            return Err(invalid("noncanonical or incomplete progress"));
        }
        Ok(result)
    }
    fn row(&mut self, reference: &[u8]) -> io::Result<&mut Row> {
        self.rows
            .get_mut(reference)
            .ok_or_else(|| invalid("unconfigured reference"))
    }
    ...
    pub fn begin(&mut self, reference: &[u8]) -> io::Result<()> {
        ...
        if row.running || row.preparing || row.pending.is_some() {
            return Err(invalid("unresolved previous attempt"));
        }
        row.running = true;
        Ok(())
    }
    pub fn refuse(&mut self, reference: &[u8]) -> io::Result<()> {
        let row = self.row(reference)?;
        if (!row.running && !row.preparing) || row.pending.is_some() {
            return Err(invalid("no running attempt to refuse"));
        }
        row.running = false;
        row.preparing = false;
        Ok(())
    ...
    pub fn uncertain(&mut self, reference: &[u8], candidate: [u8; 32]) -> io::Result<()> {
        let row = self.row(reference)?;
        if !row.running || row.preparing || row.pending.is_some() || candidate == [0; 32] {
            return Err(invalid("invalid pending transition"));
        }
        row.running = false;
        row.pending = Some(candidate);
        Ok(())
    }
    ...
    pub fn abandon_preparation(&mut self, reference: &[u8]) -> io::Result<()> {
        let row = self.row(reference)?;
        if !row.preparing || row.running || row.pending.is_some() {
            return Err(invalid("not an effect-free preparation"));
        }
        row.preparing = false;
        Ok(())
    }
    ...
    pub fn publication_refused(&mut self, reference: &[u8], candidate: [u8; 32]) -> io::Result<()> {
        ...
        if row.running || row.preparing || row.pending != Some(candidate) {
            return Err(invalid(
                "failed publication does not match recorded candidate",
            ));
        }
        row.pending = None;
        Ok(())
    ...
    pub fn completed(&mut self, reference: &[u8], pin: Pin) -> io::Result<()> {
        let row = self.row(reference)?;
        if row.running
            || (row.pending.is_none() && !row.preparing)
    ...
    pub fn acknowledge(
        ...
        if !allowed
            || pin.number == 0
            || pin.digest == [0; 32]
            || row.floor.as_ref().is_some_and(|old| {
...
impl ProgressFile {
    pub fn open(directory: &Path, initialize: bool, expected: State) -> io::Result<Self> {
        ...
        if !metadata.is_dir() || metadata.file_type().is_symlink() {
            return Err(invalid("progress directory must be a real directory"));
        }
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            if metadata.permissions().mode() & 0o077 != 0 {
                return Err(invalid("progress directory must be private (0700)"));
            }
        }
        if !cfg!(unix) {
            return Err(io::Error::new(
                io::ErrorKind::Unsupported,
                "durable maintenance progress requires the Unix profile",
            ));
        }
        ...
        let load = (|| {
            ...
            match fs::symlink_metadata(directory.join("checkpoint.next")) {
                ...
                Err(e) => Err(e),
                Ok(_) => Err(invalid(
                    "interrupted checkpoint replacement requires inspection",
                )),
            }
        })();
