...
use fgit_authority::{ExpectedOld, OutcomeLookup, ProposedNew, SealAttempt, SemanticRequest, RECEIVE_ADMISSION_SCHEMA};
use fgit_crypto::GitObjectKind;
use fgit_forge::tags::{TagAnnotation, TagCommand, TagRead, TagReadLimits, TagRefusal, TagSignatureState, validate_tag_name};
use fgit_git_object::{AcceptanceProfile, ObjectType, ParseLimits, TagSignature, TagTargetType, parse_annotated_tag};
use fgit_pack::{CanonicalObjectSource, CanonicalPackObject, PackPlanner, PackWriteError, PackWriteProfile, PackWriter};
...
use crate::{LoopbackReceiveSession, NodeReceiveTransportRefusal, NodeRequestContext, OneNode, VerifiedFabricPackSource, crypto_object_kind};
use super::{NodeWorkspaceRefusal, workspace_request_live};
use super::publication::receive_error;

fn refused(reason: TagRefusal) -> NodeWorkspaceRefusal { NodeWorkspaceRefusal::Tag(reason) }
...
impl CanonicalObjectSource for NewTagObject {
    fn load(&self, id: &GitOid) -> Result<CanonicalPackObject, PackWriteError> {
        self.0.as_ref().filter(|object| object.id() == *id).cloned()
...
impl OneNode {
    ...
    pub async fn admit_tag_durable_in(
        ...
        command: &TagCommand, limits: AdmissionLimits,
        ...
        let semantic = SemanticRequest::build(RECEIVE_ADMISSION_SCHEMA, self.object_format, true,
            vec![prepared.command.clone()], vec![], vec![]).map_err(|_| refused(TagRefusal::InvalidObject))?;
        ...
        if selected.snapshot().hidden_refs.hides(command.reference().as_bytes()) {
        ...
        let old = match prepared.command.expected_old { ExpectedOld::Exactly(oid) => oid.to_string(), _ => zero.clone() };
        let new = match prepared.command.proposed_new { ProposedNew::Update(oid) => oid.to_string(), ProposedNew::Delete => zero };
        let mut line = format!("{old} {new} ").into_bytes();
        line.extend_from_slice(command.reference().as_bytes()); line.push(0);
        ...
        if matches!(prepared.command.proposed_new, ProposedNew::Update(_)) {
            let ids = prepared.object.as_ref().map(|object| vec![object.id]).unwrap_or_default();
            let source = NewTagObject(prepared.object.map(|object| CanonicalPackObject::new(
                object.id, ObjectType::Tag, object.body, vec![], 0, 0,
            )));
            let plan = PackPlanner::new(self.object_format, PackWriteProfile::STORED_V1, receive_limits.pack.clone())
                .plan_selected(&source, &ids, &mut live).map_err(|e| NodeWorkspaceRefusal::MergePack(Box::new(e)))?;
            let (pack, _) = PackWriter::new(receive_limits.pack.clone()).write(&plan, &mut live)
        ...
        let context = ReceiveContext::new(format, capabilities, receive_limits, SignedPushProfile::Refuse).map_err(receive_error)?;
        self.receive_loopback_pack_durable_in(request, session, &selected, context, &input,
            parse_limits, limits, &mut live).await.map_err(receive_error)
    }
    ...
    pub async fn read_tag_in(
        ...
        expected_head: Option<RepositoryAuthorityHeadId>, limits: TagReadLimits,
    ) -> Result<TagRead, NodeWorkspaceRefusal> {
        validate_tag_name(reference).map_err(refused)?; limits.validate().map_err(refused)?;
        ...
        let selected = self.materialize_admission_in(request).await
            .map_err(|e| NodeWorkspaceRefusal::Authority(Box::new(e)))?;
        if selected.snapshot().hidden_refs.hides(reference.as_bytes()) { return Err(NodeWorkspaceRefusal::RefUnavailable); }
        let head = selected.basis().id();
        if expected_head.is_some_and(|expected| expected != head) { return Err(refused(TagRefusal::SnapshotMoved)); }
        ...
        let mut seen = BTreeSet::new(); let mut annotations = Vec::new();
        let exhaustion = Cell::new(None);
        loop {
            checkpoint(request)?;
            if !seen.insert(id) { return Err(refused(TagRefusal::Cycle)); }
            if !selected.selected_closure().closure().objects().contains(&id) { return Err(refused(TagRefusal::InvalidObject)); }
            ...
            let source = VerifiedFabricPackSource { fabric: &self.fabric, object_format: self.object_format,
                maximum_object_bytes: limits.max_object_bytes.min(remaining)
                    .min(usize::try_from(self.max_object_bytes).unwrap_or(usize::MAX)),
                database_context: request.authority(), database_exhaustion: &exhaustion, session_is_live: None };
            let read = source.read_object(&id); checkpoint(request)?;
            let (kind, body) = read.map_err(|e| NodeWorkspaceRefusal::MergePack(Box::new(e)))?;
            remaining = remaining.checked_sub(body.len()).ok_or_else(|| refused(TagRefusal::Budget("total bytes")))?;
            let actual = crypto_object_kind(kind);
            if expected_kind.is_some_and(|expected| expected != actual) { return Err(refused(TagRefusal::TargetKindMismatch)); }
            if kind != ObjectType::Tag {
                return Ok(TagRead { head, reference: reference.clone(), tip, peeled: id, peeled_kind: actual, annotations });
            }
            if annotations.len() == limits.max_tags { return Err(refused(TagRefusal::Budget("tag depth"))); }
            let parsed = parse_annotated_tag(&body, self.object_format, AcceptanceProfile::GitCompatibleImport,
                &source.parse_limits()).map_err(|_| refused(TagRefusal::InvalidObject))?;
            let target = parsed.target();
            if target.oid.is_zero() { return Err(refused(TagRefusal::InvalidObject)); }
            let target_kind = match target.object_type { TagTargetType::Blob => GitObjectKind::Blob,
                TagTargetType::Tree => GitObjectKind::Tree, TagTargetType::Commit => GitObjectKind::Commit,
            ...
            let signature = match parsed.signature() {
                ...
                TagSignature::Absent if parsed.message().windows(b"-----BEGIN ".len())
                    .any(|part| part == b"-----BEGIN ") => TagSignatureState::OpaqueUnverifiable,
                TagSignature::Absent => TagSignatureState::Absent,
            };
            annotations.push(TagAnnotation { id, target: target.oid, target_kind, body, signature });
            id = target.oid; expected_kind = Some(target_kind);
        }
    }
}
