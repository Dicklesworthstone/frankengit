...
use crate::{
    AdmissionMaterializationRefusal, NodeRequestContext, OneNode, VerifiedFabricPackSource,
};
...
use fgit_pack::{
    PackLimits, PackPlanner, PackWriteError, PackWriteProfile, PackWriter, verify_native_object,
};
...
impl std::fmt::Display for RebasePreparationRefusal {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "rebase preparation refused: {self:?}")
    }
}
...
fn original_metadata(
    ...
    body: &[u8],
    limits: &ParseLimits,
) -> Result<RebaseCommitMetadata, MergeSourceError> {
    ...
    for header in commit.headers() {
        match header.name.as_slice() {
            ...
            b"author" => {
                if author.is_some() || !header.continuations.is_empty() {
                    return Err(invalid());
                }
            ...
            b"committer" => {
                committers += 1;
                if committers != 1 || !header.continuations.is_empty() {
                    return Err(invalid());
                }
            }
            b"encoding" => {
                if encoding.is_some() || !header.continuations.is_empty() {
                    return Err(invalid());
                }
...
impl OneNode {
    ...
    pub async fn prepare_rebase_bundle_in(
        ...
        if source_ref == onto_ref
            || [source_ref, onto_ref]
                .iter()
                .any(|r| !r.as_bytes().starts_with(b"refs/heads/"))
            || [inputs.source_tip, inputs.upstream, inputs.onto]
                .iter()
                .any(|id| id.is_zero() || id.algorithm() != self.object_format)
        {
            return Err(RebasePreparationRefusal::InvalidInput(
                "two distinct native branches and exact same-format tips required",
            ));
        }
        admits_read(self.cell_state(), ReadMode::Current)
            .map_err(RebasePreparationRefusal::Cell)?;
        ...
        if expected_head.is_some_and(|head| head != selected.basis().id()) {
            return Err(RebasePreparationRefusal::SnapshotMoved);
        }
        if selected.snapshot().hidden_refs.hides(source_ref.as_bytes())
            || selected.snapshot().hidden_refs.hides(onto_ref.as_bytes())
        {
            return Err(RebasePreparationRefusal::RefUnavailable);
        }
        let source_tip = selected
            .snapshot()
            .refs
            .get(source_ref)
            .ok_or(RebasePreparationRefusal::RefUnavailable)?;
        let onto = selected
            .snapshot()
            .refs
            .get(onto_ref)
            .ok_or(RebasePreparationRefusal::RefUnavailable)?;
        if *source_tip != inputs.source_tip || *onto != inputs.onto {
            return Err(RebasePreparationRefusal::TipMoved);
        }
...
fn verify_chain(
    original: &SelectedSource<'_>,
    plan: &PreparedRebase,
) -> Result<(), RebasePreparationRefusal> {
    let invalid = || {
        RebasePreparationRefusal::InvalidInput(
            "rewritten native chain differs from the complete rebase receipt",
        )
    };
    ...
    for step in &plan.steps {
        original.checkpoint()?;
        if !seen.insert(step.original) {
            return Err(invalid());
        }
        if step.kind == RebaseStepKind::DroppedEmpty {
            if step.rewritten != parent || step.tree != tree {
                return Err(invalid());
            }
            continue;
        }
        ...
        let parents = commit
            .parent_references()
            .map(|p| original.oid(p, object.id))
            .collect::<Result<Vec<_>, _>>()?;
        let actual_tree = original.oid(commit.tree_reference().ok_or_else(invalid)?, object.id)?;
        if parents != [parent] || actual_tree != step.tree {
            return Err(invalid());
        }
        parent = step.rewritten;
        tree = step.tree;
    }
    if plan.commit != parent
        || plan.tree != tree
        || plan
            .steps
            .last()
            .is_some_and(|s| s.original != plan.request.source_tip)
        || (plan.steps.is_empty() && plan.request.source_tip != plan.request.upstream)
    {
        return Err(invalid());
    }
    Ok(())
...
fn rebase_bundle(
    ...
    let candidate = CandidateSource {
        original,
        generated: plan.objects.iter().map(|o| (o.id, o)).collect(),
    };
    for object in &plan.objects {
        ...
        .map_err(|_| {
            RebasePreparationRefusal::InvalidInput("constructed object failed strict validation")
        })?;
    }
    let validation = MergeObjectLimits {
        max_object_bytes: original.limits.max_object_bytes,
        ..MergeObjectLimits::default()
    };
    let empty = CandidateSource {
        original,
    ...
    if !all.objects.contains(&plan.request.onto) {
        return Err(RebasePreparationRefusal::InvalidInput(
            "onto history absent",
        ));
    }
    let needed: Vec<_> = all.objects.difference(&base.objects).copied().collect();
    if needed.len() > limits.max_objects {
        return Err(RebasePreparationRefusal::BudgetExceeded);
    }
    let borrowed = needed
        .iter()
        .filter(|id| !candidate.generated.contains_key(*id))
        .count();
    let pack_limits = PackLimits {
        max_entries: u32::try_from(limits.max_objects)
            .map_err(|_| RebasePreparationRefusal::BudgetExceeded)?,
        max_total_expanded_bytes: limits.max_output_bytes,
        max_cached_bytes: limits.max_output_bytes,
        ..PackLimits::default()
    };
    let packed = PackPlanner::new(
        original.inner.object_format,
