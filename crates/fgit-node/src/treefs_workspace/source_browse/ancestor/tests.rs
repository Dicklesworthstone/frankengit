...
impl Fixture {
    fn new(format: Format) -> Self { Self { format, objects: BTreeMap::new() } }
    ...
    fn run(&self, tip: Oid, wanted: Oid, limits: Limits) -> Result<Receipt, NodeWorkspaceRefusal> {
        walk(self.format, tip, wanted, limits,
...
fn ancestry_reaches_non_first_parents_in_both_native_formats() {
    for format in [Format::Sha1, Format::Sha256] {
        ...
        let result = walk(format, tip, right, Limits::default(), &mut |id, maximum| {
            reads.push((id, maximum));
            f.objects.get(&id).cloned().ok_or_else(invalid_source)
        }, &mut || Ok(())).unwrap();
        assert_eq!(&result.body, expected);
        assert_eq!(reads.iter().map(|(id, _)| *id).collect::<Vec<_>>(), vec![tip, left, right]);
        assert_eq!(result.bytes, reads.iter().map(|(id, _)| f.objects[id].1.len() as u64).sum::<u64>());
        assert_eq!(f.run(tip, root, Limits::default()).unwrap().body, f.objects[&root].1);
    }
}
...
fn an_unrelated_admitted_commit_is_never_read_by_its_supplied_id() {
    for format in [Format::Sha1, Format::Sha256] {
        ...
        let mut reads = Vec::new();
        let result = walk(format, tip, secret, Limits::default(), &mut |id, _| {
            assert_ne!(id, secret);
            reads.push(id);
            f.objects.get(&id).cloned().ok_or_else(invalid_source)
        }, &mut || Ok(()));
...
fn missing_corrupt_wrong_kind_and_ambiguous_metadata_are_not_not_found() {
    ...
    assert!(matches!(f.run(tip, root, Limits::default()), Err(NodeWorkspaceRefusal::Object(_))));
    ...
    for extra in [format!("tree {tree}\n"), " continued\n".to_owned()] {
        ...
        assert!(matches!(f.run(bad, bad, Limits::default()), Err(NodeWorkspaceRefusal::Object(_))));
    }
    let continued = f.body(format!("tree {tree}\nparent {tip}\n continued\nauthor A <a@invalid> 1 +0000\ncommitter A <a@invalid> 1 +0000\n\nbad parent\n").into_bytes());
    assert!(matches!(f.run(continued, continued, Limits::default()), Err(NodeWorkspaceRefusal::Object(_))));
}
...
fn every_work_and_byte_limit_refuses_instead_of_truncating_ancestry() {
    ...
    let root = f.commit(&[], "root");
    let tip = f.commit(&[root], "tip");
    assert!(is_budget(f.run(tip, root, Limits { commits: 1, ..Limits::default() })));
    assert!(is_budget(f.run(tip, root, Limits { edges: 0, ..Limits::default() })));
    assert!(is_budget(f.run(tip, root, Limits { object_bytes: 1, ..Limits::default() })));
    assert!(is_budget(f.run(tip, root, Limits { bytes: 1, ..Limits::default() })));
    let first_bytes = f.objects[&tip].1.len();
    let mut maxima = Vec::new();
    let result = walk(f.format, tip, root, Limits { bytes: first_bytes + 7, ..Limits::default() },
        &mut |id, maximum| {
            maxima.push(maximum);
            f.objects.get(&id).cloned().ok_or_else(invalid_source)
        }, &mut || Ok(()));
...
fn duplicate_parent_edges_do_not_duplicate_reads_but_still_consume_work() {
    ...
    let mut reads = Vec::new();
    walk(f.format, tip, root, Limits::default(), &mut |id, _| {
        reads.push(id);
        f.objects.get(&id).cloned().ok_or_else(invalid_source)
    }, &mut || Ok(())).unwrap();
    assert_eq!(reads, vec![tip, root]);
    assert!(is_budget(f.run(tip, root, Limits { edges: 1, ..Limits::default() })));
}
...
fn cancellation_at_every_checkpoint_prevents_a_success_receipt() {
    ...
    let tip = f.commit(&[root], "tip");
    let mut total = 0;
    walk(f.format, tip, root, Limits::default(),
        &mut |id, _| f.objects.get(&id).cloned().ok_or_else(invalid_source),
        &mut || { total += 1; Ok(()) }).unwrap();
    for stop in 1..=total {
        let mut checks = 0;
        let result = walk(f.format, tip, root, Limits::default(),
            &mut |id, _| f.objects.get(&id).cloned().ok_or_else(invalid_source),
            &mut || {
                checks += 1;
                if checks >= stop { Err(NodeWorkspaceRefusal::Cancelled { exhaustion: None }) }
                else { Ok(()) }
            });
        assert!(matches!(result, Err(NodeWorkspaceRefusal::Cancelled { .. })), "checkpoint {stop}");
    }
}
...
fn selection_needs_both_head_and_exact_target_without_oid_domain_aliases() {
    ...
    let selection = Selection { expected_ref_tip: target, commit: target };
    let mut query = SourceBrowseQuery { path: None, expected_head: None,
        expected_commit: Some(target), action: SourceBrowseAction::List { after: None, limit: 1 } };
    assert!(selection.validate(f.format, &query).is_err());
    use fgit_types::{CANONICAL_CODEC_VERSION, DigestAlgorithmId, DigestBytes};
    query.expected_head = Some(RepositoryAuthorityHeadId::from_digest(DigestAlgorithmId::try_new(1).unwrap(),
        CANONICAL_CODEC_VERSION, DigestBytes::try_new(&[1; 32]).unwrap()));
    ...
    assert!(matches!(f.run(target, zero, Limits::default()), Err(NodeWorkspaceRefusal::ObjectFormatMismatch)));
}
