...
fn review_walk_retains_votes_and_historical_branch_tips_after_coupled_merge() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            page(&f.node, None, 100, Some(reference.source_head)).unwrap(),
            reference,
        ...
        reopened
            ...
            .unwrap();
        assert_eq!(
            page(&reopened, None, 100, Some(reference.source_head)).unwrap(),
            reference
        );
...
fn retained_review_reads_preserve_current_visibility_and_cancellation_gates() {
    ...
    assert_eq!(
        page(&f.node, None, 100, Some(pinned.source_head)).unwrap(),
        pinned
    );
