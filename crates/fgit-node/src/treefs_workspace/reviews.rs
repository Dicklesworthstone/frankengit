...
impl OneNode {
    ...
    pub async fn admit_candidate_review_durable_in(
        ...
        let owner = ReviewOwner {
            inner: NodeNativeMergeProjection {
                node: self,
                inner: self
        ...
        let terminal = reviews::admit_candidate_review_async(
            &self.authority,
            request.authority(),
            &context,
            command,
    ...
    pub async fn read_reviews_in(
        ...
        if limit == 0 || limit > 100 {
            return Err(ReviewReadRefusal::InvalidLimit);
        }
        if after.is_some() && expected_head.is_none() {
            return Err(ReviewReadRefusal::UnpinnedContinuation);
        }
        ...
        .await
        .map_err(|error| ReviewReadRefusal::Admission(Box::new(error)))
    }
    ...
    pub async fn apply_reviewed_merge_bundle_durable_in(
        ...
        required_reviewers: &[PrincipalId],
        ...
        let required = ReviewRequirements::new(policy_epoch, required_reviewers.to_vec())
            .map_err(map_error)?;
        let context = AdmissionContext {
            head_key: self.head_key.clone(),
            tenant_id: self.tenant_id,
            ...
            idempotency_key: IdempotencyKey::new(key_bytes.to_vec()).map_err(|_| {
                NodeWorkspaceRefusal::InvalidWorkspaceCandidate("invalid bounded idempotency key")
            })?,
            object_format: self.object_format,
        };
        let intent = NativeMergeIntent::new(number, version, merge.clone()).map_err(map_error)?;
        let attempt =
            gate::reviewed_seal_attempt(&context, &intent, &required).map_err(map_error)?;
        let tx_id = attempt
            .derive()
            .map_err(|_| {
                NodeWorkspaceRefusal::InvalidWorkspaceCandidate("reviewed merge seal refused")
            })?
            .0;
        if let OutcomeLookup::Decided(terminal) = fgit_authority::resolve_outcome_async(
            &self.authority,
        ...
        let terminal = gate::admit_reviewed_merge_async(
            &self.authority,
            request.authority(),
            &context,
            &intent,
...
impl std::fmt::Display for ReviewReadRefusal {
    fn fmt(&self, out: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(out, "review read refused: {self:?}")
    }
}
...
impl AsyncAdmissionProjection<FsqliteAuthorityStore> for ReviewOwner<'_> {
    ...
    fn materialize_refusal_async<'a>(
        ...
        self.inner
            .materialize_refusal_async(store, cx, basis, tx, code)
    }
}
impl NativeMergeProjection<FsqliteAuthorityStore> for ReviewOwner<'_> {
    ...
    fn validate_merge_async<'a>(
        ...
        self.inner
            .validate_merge_async(store, cx, basis, head, intent)
    }
}
            ...
            if command.decision != ReviewDecision::Withdraw {
                {
                    ...
                    for tip in [command.subject.source_tip, command.subject.target_tip] {
                        ...
                        let (kind, bytes) = value.map_err(|_| {
                            ProjectionFailure::Unavailable(RefusalCode::EvidenceMissing)
                        })?;
                ...
                if let Some(binding) = candidate {
                    ...
                    result.map_err(|error| match error {
                        ...
                        E::SnapshotMoved | E::ParentMoved => {
                            ProjectionFailure::Unavailable(RefusalCode::AuthorityReceiptStale)
                        }
                        E::InvalidCandidate(_) | E::Pack(_) | E::Envelope(_) => {
