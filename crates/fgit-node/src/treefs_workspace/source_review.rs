...
use super::{
    Cell, MergeObjectSource, MergeSourceError, ParseLimits, RefVisibility, SelectedSource,
...
use fgit_forge::review::{
    ReviewError, ReviewOptions, ReviewSelection, SourceReview, compare_source,
};
...
impl NodeReviewRefusal {
    ...
    pub(crate) fn review_error(&self) -> Option<&ReviewError> {
        match self {
            Self::Review(error) => Some(error.as_ref()),
            _ => None,
        }
    }
}
fn failed(error: ReviewError) -> NodeReviewRefusal {
    NodeReviewRefusal::Review(Box::new(error))
}
fn source_failed(error: MergeSourceError) -> NodeReviewRefusal {
    failed(ReviewError::Source(error))
}
        ...
        let (before_reference, after_reference, before, after, pull_request) = match selection {
            ReviewSelection::References { before, after } => {
                if !visible(before, after) {
                    return Err(NodeReviewRefusal::Unavailable);
                }
                let a = *selected
                    .snapshot()
                    .refs
            ...
            ReviewSelection::PullRequest {
                ...
                .await
                .map_err(|error| NodeReviewRefusal::History(Box::new(error)))?;
                let view = page
                    .pull_requests
                    .first()
                    .filter(|view| view.number == *number)
                    .ok_or(NodeReviewRefusal::Unavailable)?;
                ...
                let (a_ref, b_ref, a, b) = match &view.event.payload {
                    ForgeEventPayload::PullRequestChangedNative(change) => (
                        change.data.target_ref.clone(),
                        change.data.source_ref.clone(),
                        change.data.target_tip,
                        change.data.source_tip,
                    ),
                    ForgeEventPayload::MergeCommittedNative(merge) => (
                        merge.target_ref.clone(),
                        merge.source_ref.clone(),
...
mod tests {
    ...
    fn both_hash_formats_review_real_objects_without_changing_authority() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            ...
            let selection = ReviewSelection::References {
                before: a_ref,
                after: b_ref.clone(),
            };
            ...
            assert!(matches!(
                &result.comparison.entries[0].content,
                ReviewContent::Text {
                    additions: 1,
                    deletions: 1,
                    ..
            ...
            hidden
                .push_rule(b_ref.as_bytes(), &Default::default())
                .unwrap();
            assert!(matches!(
                node.runtime().block_on(
                    node.review_source_in(&request, &selection, &hidden, None, &options,)
                ),
                Err(NodeReviewRefusal::Unavailable)
            ));
            let mut bounded = options.clone();
            bounded.limits.max_blob_bytes = 1;
            assert!(
                node.runtime()
                    .block_on(node.review_source_in(
                        &request,
    ...
    fn pr_review_uses_recorded_tips_after_merge_and_enforces_both_pins() {
        ...
        let command = PullRequestCommand {
            ...
            data: PullRequestData {
                source_ref: b_ref.clone(),
                target_ref: a_ref.clone(),
                source_tip: b,
                target_tip: a,
                title: "Review original tips".into(),
                body: String::new(),
            },
        };
        let session = LoopbackReceiveSession::authenticated(
            actor,
            IdempotencyKey::new(b"review-open".to_vec()).unwrap(),
        );
        let terminal = node
            .runtime()
        ...
        let selection = ReviewSelection::PullRequest {
        ...
        let prepared = node
            ...
            .unwrap();
        let fgit_forge::preparation::MergePreparation::Clean(plan) = prepared.outcome else {
            panic!("clean");
        };
        let merge = fgit_forge::event::NativeMerge {
            source_ref: b_ref,
            source_tip: b,
            target_ref: a_ref,
            target_tip_before: a,
        ...
        assert!(matches!(
            node.runtime().block_on(node.review_source_in(
                &request,
                &selection,
                &RefVisibility::new(),
                Some(original.source_head),
                &options
            )),
            Err(NodeReviewRefusal::SnapshotMoved)
        ));
        assert!(matches!(
            node.runtime().block_on(node.review_source_in(
                &request,
            ...
            Err(NodeReviewRefusal::VersionMoved)
        ));
        let selection = ReviewSelection::PullRequest {
            number: command.number,
