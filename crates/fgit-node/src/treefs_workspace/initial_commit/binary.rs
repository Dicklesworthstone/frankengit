...
use fgit_forge::initial_commit::{
    InitialCommitError, InitialCommitPlan, prepare_initial_commit_with_binary_decoder,
};
...
fn target(index: &IndexExpectation, format: GitHashAlgorithm) -> Result<GitOid, PatchError> {
    ...
    {
        return Err(invalid(
            "initial binary index requires full absent/source and native target identities",
        ));
    }
    let value =
        std::str::from_utf8(&index.new).map_err(|_| invalid("invalid initial binary target"))?;
    let id =
        GitOid::from_hex(format, value).map_err(|_| invalid("invalid initial binary target"))?;
    if id.is_zero() {
        return Err(invalid("initial binary target must be present"));
    }
    Ok(id)
}
...
pub(super) fn prepare_initial_commit(
    ...
    if cancelled() {
        return Err(PatchError::Cancelled.into());
    }
    limits.validate()?;
    ...
    for file in patch.files() {
        ...
        if file.binary_hunks().is_some() {
            target(
                file.index().ok_or(InitialCommitError::IndexMismatch)?,
                format,
            )?;
            count += 1;
        }
    ...
    let plan = prepare_initial_commit_with_binary_decoder(
        ...
        |hunks, index, base| {
            if !base.is_empty() {
                return Err(invalid("initial binary source must be absent"));
            }
            let id = target(index, format)?;
            let batch = batch
                .as_mut()
                .ok_or_else(|| invalid("initial binary budget not reserved"))?;
            batch
                .apply(hunks, None, Some(id), base, &mut || !cancelled())
                .map_err(refusal)
        },
    )?;
    if batch
        .as_ref()
        .is_some_and(|batch| batch.usage().files != count)
    {
