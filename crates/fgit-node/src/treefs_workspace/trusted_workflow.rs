...
use fgit_runner::workflow::{
    StepLimits, StepObservation, WorkerFailure, WorkflowExecutor, WorkflowJob, WorkflowLimits,
    WorkflowPlan, WorkflowReport, run_trusted_step,
};
use fgit_treefs::{
    BaseView, SparseEntryKind, SparseLimits, SparseManifest, TreeCapability, TreePath, WorkspaceId,
};
use fgit_types::{
    ByteCount, GitHashAlgorithm as Format, GitOid, RefName, RepositoryAuthorityHeadId,
    RepositoryCommitId, RepositoryId, RepositoryIncarnationId, TenantId,
};
...
impl TrustedWorkflowRun {
    ...
    pub fn executed_commit(&self) -> GitOid {
        self.candidate.map_or(self.source_commit, |c| c.commit)
    }
    pub fn executed_tree(&self) -> GitOid {
        self.candidate.map_or(self.source_tree, |c| c.tree)
    }
    fn identity_json(&self) -> String {
        ...
        format!(
            ...
            self.source_head,
            self.source_rcr,
            self.source_commit.algorithm().as_str(),
            self.source_commit,
            self.source_tree,
            hex(&self.source_reference),
            self.workflow_blob,
            hex(&self.workflow_path),
            prefixes,
            hex(&self.run_id),
            hex(self.run_directory.as_os_str().as_bytes()),
    ...
    pub fn to_json(&self) -> String {
        format!(
            concat!(
                "{{\"type\":\"trusted_workflow_run\",{},\"succeeded\":{},",
                "\"workspaces_closed\":{},\"request_interrupted\":{},\"execution\":{}}}"
            ),
            self.identity_json(),
            self.succeeded(),
            self.workspaces_closed,
            self.request_interrupted,
...
impl OneNode {
    ...
    pub async fn run_trusted_workflow_in(
        ...
        match self.object_format {
            ...
            Format::Sha256 => {
                self.run_workflow_format::<Sha256>(
                    request,
                    reference,
                    workflow_path,
                    run_id,
    ...
    async fn run_workflow_format<A: GitHashAlgorithm>(
        ...
        let observed = observed
            .into_inner()
            .map_err(|_| TrustedWorkflowFailure::InvalidInput("result lock poisoned"))?;
        match (selected, observed) {
            ...
            (Err(error), Some(Ok(report))) => Err(TrustedWorkflowFailure::Journal {
                ...
                detail: format!(
                    "source finalization failed after execution: {error}; inspect report.json, do not replay"
                ),
            }),
            (Err(error), None) => Err(TrustedWorkflowFailure::Source(error)),
            (Ok(()), None) => Err(TrustedWorkflowFailure::InvalidInput(
                "source consumer did not complete",
            )),
        }
    }
}
...
fn validate_inputs(
    ...
    if run_id == [0; 16] || !parent.is_absolute() || prefixes.is_empty() || prefixes.len() > 1024 {
        return Err(invalid(
            "nonzero run ID, absolute private parent and 1..1024 input prefixes required",
        ));
    }
    if prefixes
        .iter()
        .try_fold(0_usize, |n, p| n.checked_add(p.len()))
        .is_none_or(|n| n > 64 * 1024)
    {
        return Err(invalid("input prefix bytes exceed 64 KiB"));
    }
    let workflow =
        TreePath::parse_default(workflow).map_err(|_| invalid("invalid workflow path"))?;
...
fn run_at<A: GitHashAlgorithm>(
    ...
    if !workspace_request_live(request) || started.elapsed() >= limits.run_timeout {
        return Err(TrustedWorkflowFailure::InvalidInput(
            "request stopped before source discovery",
        ));
    }
    let manifest = Arc::new(
        SparseManifest::build(base, source, capability, 0, SOURCE_LIMITS).map_err(|error| {
            TrustedWorkflowFailure::Source(NodeWorkspaceRefusal::Manifest(error))
        })?,
    );
    run_inputs(
        node,
        request,
        WorkflowInputs::Canonical(manifest),
        capability,
        head,
        reference,
        workflow_path,
...
fn run_inputs<A: GitHashAlgorithm>(
    node: &OneNode,
    request: &NodeRequestContext,
    ...
    if !live() {
        return Err(TrustedWorkflowFailure::InvalidInput(
            "request stopped before workflow preflight",
        ));
    }
    let entry = inputs
        .entries()
        .iter()
        .find(|entry| entry.path().as_bytes() == workflow_path)
        .ok_or(TrustedWorkflowFailure::InvalidInput(
            "workflow is not in the selected inputs",
        ))?;
    ...
    if inputs
        .payload_bytes()
        .checked_mul(plan.graph().jobs.len())
    ...
    {
        return Err(TrustedWorkflowFailure::InvalidInput(
            "aggregate job materialization exceeds profile",
        ));
    }
    ...
    let mut report = TrustedWorkflowRun {
        ...
        source_tree,
        candidate,
        merge: inputs.merge().cloned(),
        source_reference: reference.as_bytes().to_vec(),
        workflow_blob: native(entry.source_oid().digest_bytes())?,
        workflow_path: workflow_path.to_vec(),
        read_prefixes: declared.to_vec(),
        run_id,
        run_directory: parent.join(format!("workflow-{}", hex(&run_id))),
        workspaces_closed: false,
        request_interrupted: false,
        execution: WorkflowReport {
            source: plan.source_commitment(),
            graph: plan.graph_commitment(),
            limits,
            jobs: Vec::new(),
        },
    };
    ...
    if !metadata.is_dir() || metadata.permissions().mode() & 0o777 != 0o700 {
        return Err(TrustedWorkflowFailure::InvalidInput(
            "run parent must be a nonsymlink 0700 directory",
        ));
    }
    ...
    match builder.create(&report.run_directory) {
        ...
        Err(source) => {
            return Err(TrustedWorkflowFailure::Io {
                operation: "reserve unique workflow attempt",
                source,
            });
        }
    }
    let run_directory = report.run_directory.clone();
    let journal_error = |detail| TrustedWorkflowFailure::Journal {
    ...
    .map_err(|e| {
        journal_error(format!(
            "attempt marker durability failed before any job: {e}"
        ))
    })?;
    let mut worker = Executor {
        ...
        capability,
        parent: root,
        directory: report.run_directory.clone(),
        current: None,
    ...
    publish_report(&worker.parent, &report.run_directory, json.as_bytes()).map_err(|e| {
        journal_error(format!(
            "execution finished but report durability failed: {e}; do not replay this attempt"
        ))
    })?;
    Ok(report)
}
...
impl<A: GitHashAlgorithm> WorkflowExecutor for Executor<'_, A> {
    fn begin_job(
        ...
        if self.current.is_some() || self.retained {
            return Err(WorkerFailure::new("previous job is not quiescent", true));
        }
        ...
        let reserved = ledger
            ...
            .map_err(|e| WorkerFailure::new(e.to_string(), false))?;
        match SparseWorkspace::materialize(
            plan,
            parent,
            slot,
            reserved,
            ...
            Ok(workspace) => {
                self.current = Some((workspace, ledger));
                Ok(())
            }
            Err(error) => {
    ...
    fn execute_step(
        ...
        let (workspace, _) = self
            .current
            .as_ref()
            .ok_or_else(|| WorkerFailure::new("no active job", true))?;
        let mut open = |stream: &str| {
            let path = self.directory.join(format!(
                "job-{:03}-step-{index:03}-{stream}",
                self.job_index
            ));
    ...
    fn finish_job(&mut self, retain: bool) -> Result<(), WorkerFailure> {
        let Some((workspace, ledger)) = self.current.take() else {
            ...
            return Err(WorkerFailure::new(
                "job responsibility missing during close",
                true,
            ));
        };
        if retain {
            ...
            return Err(WorkerFailure::new(
                "workspace and captures retained: descendant quiescence is unproved",
                true,
            ));
        }
