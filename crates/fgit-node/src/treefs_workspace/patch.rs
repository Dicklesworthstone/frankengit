...
use fgit_treefs::{BaseEntry, BaseError, BaseView, EntryClass, ExportLimits, FileMode,
    IntentLog, TreeCapability, TreeEditIntent, TreePath, WorkspaceId};
use fgit_types::{ByteCount, GitHashAlgorithm as ObjectFormat, GitOid, RefName, RepositoryCommitId};
...
impl WorkspacePatchCandidate {
    #[must_use]
    pub fn bundle_bytes(&self) -> &[u8] { &self.bundle }
}
...
fn preflight(reference: &RefName, expected: GitOid, format: ObjectFormat,
    ...
    if !reference.as_bytes().starts_with(b"refs/heads/") || reference.as_bytes().len() > 4096 {
        return Err(invalid("patch preparation requires a bounded full branch reference"));
    }
    if expected.is_zero() || expected.algorithm() != format {
        return Err(NodeWorkspaceRefusal::ObjectFormatMismatch);
    }
    metadata.validate().map_err(|_| invalid("patch commit metadata is invalid"))
}
...
impl OneNode {
    ...
    pub async fn prepare_trusted_patch_in(
        ...
        match self.object_format {
            ObjectFormat::Sha1 => self.prepare_trusted_patch_format::<Sha1>(request, reference,
                expected_commit, workspace_id, patch_bytes, metadata, limits).await,
            ObjectFormat::Sha256 => self.prepare_trusted_patch_format::<Sha256>(request, reference,
                expected_commit, workspace_id, patch_bytes, metadata, limits).await,
        }
    }
    ...
    async fn prepare_trusted_patch_format<A: GitHashAlgorithm>(
        ...
        self.with_workspace_base_in::<A, _>(request, reference, &RefVisibility::new(), &mut seed, 0,
            |base, source, seed| {
                ...
                let root = base.read_object(source, base.base_tree_oid(), GitObjectKind::Tree, &grant)
                    .map_err(NodeWorkspaceRefusal::Object)?;
                seed.charge_fetch(root.len() as u64).map_err(capability_error)?;
                let entries = parse_tree(&root, AcceptanceProfile::GitCompatibleImport, &source.inner.parse_limits())
                ...
                prepare_at_base(base, source, &mut owner, request, reference, expected_commit,
                    &patch, &paths, patch_bytes, 0, metadata)
            },
        ).await
    }
}

fn budgeted(id: WorkspaceId, repository: fgit_types::RepositoryId, reads: Vec<TreePath>,
    writes: Vec<TreePath>, bytes: u64, objects: u64) -> Result<TreeCapability, NodeWorkspaceRefusal> {
    let bytes = ByteCount::try_new("patch_fetch_bytes", bytes, FETCH_BYTES)
        .map_err(|_| patch_error(PatchError::Budget("read bytes")))?;
    Ok(TreeCapability::new(id, repository, reads, writes).with_fetch_budget(bytes).with_file_budget(objects))
}

fn prepare_at_base<A: GitHashAlgorithm>(
    base: &BaseView<A>, source: &NodeTreeSource<'_>, capability: &mut TreeCapability,
    request: &NodeRequestContext, reference: &RefName, expected_commit: GitOid,
    patch: &UnifiedPatch<'_>, paths: &[TreePath], patch_bytes: &[u8], now: u64,
    ...
    for (file, path) in patch.files().iter().zip(paths) {
        ...
        if let Some(index) = file.index() {
            ...
            if !IndexExpectation::matches(&index.old, old_hex.as_deref().map(str::as_bytes)) {
                return Err(invalid("patch old index does not match the verified source blob"));
            }
        }
        let input = match existing {
            Some((oid, _, mode)) => {
                ...
                let bounded = NodeTreeSource { inner: VerifiedFabricPackSource {
                    fabric: source.inner.fabric, object_format: source.inner.object_format,
                    maximum_object_bytes: source.inner.maximum_object_bytes.min(limits.max_file_bytes),
                ...
                }, selected: source.selected, workspace: source.workspace };
                let body = base.read_object(&bounded, &oid, GitObjectKind::Blob, &grant)
                    .map_err(NodeWorkspaceRefusal::Object)?;
                capability.charge_fetch(body.len() as u64).map_err(capability_error)?;
                Some((mode, body))
            }
            None => None,
        };
        ...
        match output {
            Some(file) => {
                total = total.checked_add(file.content.len()).filter(|bytes| *bytes <= limits.max_output_bytes)
                    .ok_or_else(|| patch_error(PatchError::Budget("aggregate result bytes")))?;
                log.push(TreeEditIntent::Write { path: path.clone(), content: file.content,
                    mode: if file.mode == 0o100755 { FileMode::Executable } else { FileMode::Regular },
    ...
    if export.plan.root_tree() == base.base_tree_oid() {
        return Err(invalid("patch has no net tree change; no candidate was created"));
    }
    pack(source.inner.object_format, reference, export, receipts, sha256_digest(patch_bytes),
        metadata, &|| workspace_request_live(request))
}

fn native_id(format: ObjectFormat, bytes: &[u8]) -> Result<GitOid, NodeWorkspaceRefusal> {
    GitOid::from_hex(format, &bytes.iter().map(|b| format!("{b:02x}")).collect::<String>())
...
fn pack<A: GitHashAlgorithm>(format: ObjectFormat, reference: &RefName,
    export: candidate::WorkspaceEditExport<A>, paths: Vec<PatchPathReceipt>, patch_sha256: [u8; 32],
    metadata: &MergeMetadata, live: &dyn Fn() -> bool,
) -> Result<WorkspacePatchCandidate, NodeWorkspaceRefusal> {
    let check = || if live() { Ok(()) } else { Err(patch_error(PatchError::Cancelled)) };
    check()?;
    ...
    let parsing = ParseLimits { tree_reference_bytes: format.digest_len(), max_object_bytes: EXPORT_BYTES,
        ..ParseLimits::default() };
    parse_object_body(ObjectType::Commit, &body, AcceptanceProfile::StrictCreate, &parsing)
    ...
    for object in export.plan.objects() {
        ...
        let refs = if object.kind() == GitObjectKind::Tree {
            parse_tree(object.body(), AcceptanceProfile::StrictCreate, &parsing)
                .map_err(|_| invalid("patch candidate tree is invalid"))?.into_iter()
                .filter(|entry| entry.mode != b"160000")
                .map(|entry| native_id(format, &entry.object_id)).collect::<Result<Vec<_>, _>>()?
        } else { Vec::new() };
        objects.insert(id, CanonicalPackObject::new(id, object.kind(), object.body().to_vec(), refs, 0, 0));
    }
    objects.insert(candidate_commit, CanonicalPackObject::new(candidate_commit, ObjectType::Commit,
        body, vec![root_tree, source_commit], 0, 0));
    ...
    let (pack, _) = PackWriter::new(limits).write(&plan, &mut running)
        .map_err(|error| NodeWorkspaceRefusal::MergePack(Box::new(error)))?;
    let mut bundle = match format {
        ObjectFormat::Sha1 => b"# v2 git bundle\n".to_vec(),
        ObjectFormat::Sha256 => b"# v3 git bundle\n@object-format=sha256\n".to_vec(),
    };
    bundle.extend_from_slice(format!("-{source_commit} patch base\n{candidate_commit} ").as_bytes());
    bundle.extend_from_slice(reference.as_bytes());
    bundle.extend_from_slice(b"\n\n");
    bundle.extend_from_slice(&pack);
    check()?;
    Ok(WorkspacePatchCandidate { object_format: format, source_commit, source_rcr: export.source_rcr,
        candidate_commit, root_tree, patch_sha256, paths, object_count: selected.len(), bundle })
}
