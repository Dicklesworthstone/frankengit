...
fn check_reference(reference: &RefName) -> Result<(), NodeWorkspaceRefusal> {
    if reference.as_bytes().len() > 4096 || !reference.as_bytes().starts_with(b"refs/heads/") {
        return Err(invalid(
            "initial commit requires a bounded full branch reference",
        ));
    }
    Ok(())
}
...
impl OneNode {
    ...
    ) -> Result<(RepositoryAuthorityHeadId, InitialCommitPlan, FullBundle), NodeWorkspaceRefusal>
    {
        ...
        .map_err(NodeWorkspaceRefusal::InitialCommit)?;
            ...
            let references = match object.kind {
                ...
                GitObjectKind::Tree => {
                    parse_tree(&object.body, AcceptanceProfile::StrictCreate, &parse_limits)
                        .map_err(|_| invalid("initial tree failed native validation"))?
                        .into_iter()
                        .map(|entry| {
                            GitOid::from_hex(
                                self.object_format,
                ...
                GitObjectKind::Commit => {
                    ...
                    .map_err(|_| invalid("initial commit failed native validation"))?
                        ...
                        return Err(invalid("initial object kind mismatch"));
                    };
        ...
        let pack = PackPlanner::new(
            self.object_format,
            PackWriteProfile::COMPRESSED_NO_DELTA_V1,
            pack_limits.clone(),
        )
        .plan_selected(&Objects(objects), &ids, &mut live)
        .map_err(pack_error)?;
        let bundle = FullBundle::write(
            &[BundleReference::new(plan.commit, reference.clone())],
            None,
            &pack,
    ...
    pub async fn apply_initial_patch_bundle_durable_in(
        ...
        let semantic = SemanticRequest::build(
            ...
            vec![RefCommand {
                ...
                proposed_new: ProposedNew::Update(expected_commit),
                force: false,
            }],
            vec![],
            vec![],
        ...
        let admission_error =
            |e| receive_error(NodeReceiveTransportRefusal::Admission(Box::new(e)));
    ...
    fn initial_commit_pack_limits(&self) -> PackLimits {
        ...
        limits.max_object_bytes = limits
            .max_object_bytes
