...
use fgit_admission::merge::native::objects::{
    MergeObjectLimits, validate_commit_closure, validate_workspace_objects,
};
use fgit_forge::preparation::replay::{
    PreparedReplay, ReplayError, ReplayPreparation, ReplayRequest, prepare_replay,
...
use fgit_forge::preparation::{
    MergeMetadata, MergeObjectSource, MergeSourceError, PreparationLimits,
};
...
impl ReplayPreparationRefusal {
    ...
    pub(crate) const fn is_tip_moved(&self) -> bool {
    ...
    pub(crate) const fn is_resource_refusal(&self) -> bool {
        matches!(
            self,
            Self::BudgetExceeded
    ...
    pub(crate) const fn source_refusal(&self) -> Option<&MergeSourceError> {
    ...
    pub(crate) fn preparation_refusal(&self) -> Option<&ReplayError> {
        if let Self::Preparation(error) = self {
            Some(error)
        } else {
            None
        }
...
impl OneNode {
    ...
    pub async fn prepare_replay_bundle_in(
        ...
        self.prepare_replay_bundle_inner_in(
            request,
            target,
            source_ref,
            inputs,
    ...
    async fn prepare_replay_bundle_inner_in(
        ...
        if let Some(choices) = resolutions {
            validate_resolutions(choices, limits).map_err(preparation)?;
        }
        metadata.validate().map_err(preparation)?;
        if [target, source_ref]
            .iter()
            .any(|name| !name.as_bytes().starts_with(b"refs/heads/"))
            || [inputs.target, inputs.source_tip, inputs.selected_commit]
                .iter()
                .any(|id| id.is_zero() || id.algorithm() != self.object_format)
            || inputs.mainline == Some(0)
        {
            return Err(ReplayPreparationRefusal::InvalidInput(
                "exact native branch/commit/mainline inputs required",
            ));
        }
        admits_read(self.cell_state(), ReadMode::Current)
            .map_err(ReplayPreparationRefusal::Cell)?;
        ...
        let actual_target = selected
            .snapshot()
            .refs
            .get(target)
            .ok_or(ReplayPreparationRefusal::RefUnavailable)?;
        let actual_source = selected
            .snapshot()
            .refs
            .get(source_ref)
            .ok_or(ReplayPreparationRefusal::RefUnavailable)?;
        ...
        let max_bytes = usize::try_from(self.max_object_bytes)
            .unwrap_or(usize::MAX)
            .min(32 * 1024 * 1024);
        let original = SelectedSource {
            inner: VerifiedFabricPackSource {
                ...
                maximum_object_bytes: max_bytes,
        ...
        let (outcome, resolutions) = match resolutions {
            Some(choices) => {
                let result = prepare_resolved_replay(
                    &original,
                    self.object_format,
                    inputs,
                    choices,
            ...
            None => (
                prepare_replay(&original, self.object_format, inputs, metadata, limits)
                    .map_err(preparation)?,
                Vec::new(),
            ),
        };
        let (bundle, pack_objects, borrowed_objects) = match &outcome {
            ...
            ReplayPreparation::Conflicted { .. } | ReplayPreparation::NoChange { .. } => {
                (None, 0, 0)
            }
        };
        original.checkpoint()?;
        Ok(ResolvedReplayBundle {
            artifact: PreparedReplayBundle {
...
fn replay_bundle(
    ...
    for object in &plan.objects {
        ...
        .map_err(|_| {
            ReplayPreparationRefusal::InvalidInput("constructed object failed strict validation")
        })?;
    }
    let verified = validate_workspace_objects(
        &candidate,
        plan.commit,
        plan.coordinates.request.target,
        validation_limits,
    ...
    let needed: Vec<_> = verified
        .objects
        .difference(&parent.objects)
        .copied()
        .collect();
    ...
    bundle.extend_from_slice(
        format!(
            "-{} target\n{} ",
            plan.coordinates.request.target, plan.commit
        )
        .as_bytes(),
    );
