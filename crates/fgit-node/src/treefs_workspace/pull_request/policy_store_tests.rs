...
use fgit_admission::policy_bridge::persisted::{
    PolicyFrame, PolicyStageDisposition, PolicyStoreError, PolicyStoreLimits, read_policy_async,
    stage_policy_async,
};
...
fn live() -> Result<(), fgit_types::RefusalCode> {
    Ok(())
}
fn frame(allow: bool) -> PolicyFrame {
    PolicyFrame::compile(
        if allow {
            "policy durable { default allow }"
        } else {
            "policy durable { default deny \"review required\" }"
        },
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let before = {
            ...
            assert_eq!(
                after.basis(),
                before.basis(),
                "storing a policy is not policy activation"
            );
            before.basis().id()
        };
...
fn file_backed_wrong_slots_and_cancelled_reads_cannot_return_a_policy() {
    ...
    {
        let request = node.request_context();
        let deny = frame(false);
        let allow = frame(true);
        let before = node
            .runtime()
            .block_on(node.materialize_admission_in(&request))
            .unwrap();
        ...
        node.runtime()
            .block_on(
                node.authority
                    .put_if_absent(request.authority(), &key, allow.bytes()),
            )
            .unwrap();
        assert!(matches!(
            node.runtime().block_on(read_policy_async(
                &node.authority,
                request.authority(),
                deny.id(),
                PolicyStoreLimits::default(),
        ...
        assert!(matches!(
            node.runtime().block_on(stage_policy_async(
                &node.authority,
                request.authority(),
                &deny,
                &live
        ...
        let stop_after_read = || {
            if count.fetch_add(1, Ordering::Relaxed) == 0 {
                ...
                Err(fgit_types::RefusalCode::CancellationInProgress)
            }
        };
        assert!(matches!(
            node.runtime().block_on(read_policy_async(
