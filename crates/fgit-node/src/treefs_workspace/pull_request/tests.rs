...
fn fixture(node: &OneNode, scratch: &Scratch, format: GitHashAlgorithm) -> Fixture {
    ...
    Fixture {
        ...
        data: PullRequestData {
            ...
            source_tip: theirs,
            target_tip: ours,
            title: "A native pull request".into(),
...
fn command(fixture: &Fixture, number: u64) -> PullRequestCommand {
    PullRequestCommand {
        number: PullRequestNumber::try_new(number).unwrap(),
...
fn accepted(
    result: Result<(TxId, TerminalOutcome), NodeReceiveTransportRefusal>,
...
fn page(
    ...
    let request = node.request_context();
    node.runtime().block_on(node.read_pull_requests_in(
        &request,
        &RefVisibility::new(),
        after,
        limit,
...
fn unchanged_code(before: &MaterializedAdmission, after: &MaterializedAdmission) {
    ...
    assert_eq!(
        before.basis().body().policy_epoch,
        after.basis().body().policy_epoch
    );
}
...
fn real_open_update_close_and_reopen_preserve_code_and_original_outcomes() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let open = command(&f, 7);
        let before = snapshot(&node);
        let opening = accepted(apply(&node, &open, "open"));
        let opened = snapshot(&node);
        unchanged_code(&before, &opened);
        assert_eq!(
            opened.basis().generation().get(),
            before.basis().generation().get() + 1
        );
        assert_ne!(
            opened.basis().body().forge_position_root,
            before.basis().body().forge_position_root
        );
        assert_ne!(
            opened.basis().body().outbox_root,
            before.basis().body().outbox_root
        );
        assert_eq!(
            opened.snapshot().outbox.len(),
        ...
        assert_eq!(
            updated.pull_requests[0].data.as_ref().unwrap().title,
            "Updated title"
        );
        ...
        close.action = PullRequestAction::Close;
        ...
        let closed = snapshot(&node);
        unchanged_code(&before, &closed);
...
fn stale_version_competitor_and_reused_key_cannot_overwrite_or_revive() {
    ...
    accepted(apply(&node, &open, "open"));
    let mut winner = open.clone();
    winner.action = PullRequestAction::Update;
    winner.expected_version = ExpectedVersion::Exactly(AggregateVersion::FIRST);
    winner.data.title = "Winner".into();
    let mut loser = winner.clone();
    loser.data.title = "Loser".into();
    accepted(apply(&node, &winner, "winner"));
    let before = snapshot(&node);
    assert!(apply(&node, &loser, "winner").is_err());
    ...
    let mut close = winner.clone();
    close.action = PullRequestAction::Close;
    close.expected_version = ExpectedVersion::Exactly(AggregateVersion::try_new(2).unwrap());
    accepted(apply(&node, &close, "close"));
    let mut resurrection = close.clone();
    resurrection.action = PullRequestAction::Update;
    ...
    assert_eq!(
        page(&node, 0, 100, None).unwrap().pull_requests[0]
            .data
            .as_ref()
            .unwrap()
            .title,
...
fn terminal_retries_survive_quota_containment_and_stopped_intake() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let scratch = Scratch::new();
        let mut node = node(&scratch, format);
        let f = fixture(&node, &scratch, format);
        let open = command(&f, 1);
        ...
        assert!(matches!(
            apply(&node, &command(&f, 2), "new-open"),
            Err(NodeReceiveTransportRefusal::QuotaContained { .. })
        ));
        let mut changed = open.clone();
        changed.data.title = "Different request".into();
        ...
        assert!(matches!(
            apply(&reopened, &command(&f, 2), "new-open"),
...
fn pagination_is_numeric_pinned_and_filters_both_branch_names() {
    ...
    let before = snapshot(&node);
    let first = page(&node, 0, 2, None).unwrap();
    assert_eq!(
        first
            .pull_requests
            .iter()
            .map(|view| view.number.get())
            .collect::<Vec<_>>(),
        [1, 2]
    );
    assert_eq!(first.next_after, Some(2));
    let second = page(&node, 2, 2, Some(first.source_head)).unwrap();
    assert_eq!(second.pull_requests[0].number.get(), 10);
    ...
    for hidden in [target(), source_ref()] {
        ...
        let result = node
            .runtime()
            .block_on(node.read_pull_requests_in(&request, &visibility, 0, 100, None))
            .unwrap();
    ...
    assert!(matches!(
        page(&node, 0, 0, None),
        Err(PullRequestReadRefusal::InvalidLimit)
    ));
    node.shutdown().unwrap();
}
...
fn merge_uses_active_pr_coordinates_and_retains_title_and_opener() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let f = fixture(&node, &scratch, format);
        let open = command(&f, 1);
        accepted(apply(&node, &open, "open-merge"));
        let candidate = node
            .put_git_object(
                GitObjectKind::Commit,
                commit(
                    f.tree,
                    &[f.data.target_tip, f.data.source_tip],
                    "reviewed merge",
                ),
            )
            .unwrap()
        ...
        let intent = NativeMergeIntent::new(
            ...
            NativeMerge {
                source_ref: source_ref(),
                source_tip: f.data.source_tip,
                base_tip: f.base,
                target_ref: target(),
                target_tip_before: f.data.target_tip,
        ...
        let mut mutation = open;
        mutation.action = PullRequestAction::Update;
        ...
        reopened.bring_into_service(HeadGeneration::FIRST).unwrap();
        assert_eq!(
            page(&reopened, 0, 100, None).unwrap().pull_requests,
...
fn closed_pr_cannot_authorize_a_merge_and_serving_gate_precedes_mutation() {
    ...
    let open = command(&f, 1);
    accepted(apply(&stopped, &open, "open"));
    ...
    let refused = stopped
        .runtime()
        .block_on(stopped.admit_native_merge_durable_in(
            &request,
            &session("closed-merge"),
            &intent,
            AdmissionLimits::default(),
            MergeObjectLimits::default(),
        ))
