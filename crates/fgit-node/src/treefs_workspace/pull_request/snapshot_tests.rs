...
fn retained_pr_walk_excludes_later_insertions_and_edits_even_after_restart() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for number in [9, 3, 1] {
            accepted(apply(&node, &command(&fixture, number), &format!("open-{number}")));
        }
        ...
        assert_eq!(fresh.pull_requests.iter().map(|row| row.number.get()).collect::<Vec<_>>(), [1, 2, 3, 9]);
        assert_eq!(fresh.pull_requests[2].data.as_ref().unwrap().title, updated.data.title);
        ...
        let last = page(&node, second.next_after.unwrap(), 1, Some(first.source_head)).unwrap();
        ...
        assert_eq!(walked, reference.pull_requests, "no duplicate, omission, newer title or newer lifecycle state");
        ...
        assert_eq!(page(&reopened, 0, 100, Some(reference.source_head)).unwrap(), reference);
...
fn a_retained_pr_token_never_overrides_the_callers_current_ref_visibility() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for hidden in [source_ref(), target()] {
            ...
            visibility.push_rule(hidden.as_bytes(), &Default::default()).unwrap();
            let request = node.request_context();
            let redacted = node.runtime().block_on(node.read_pull_requests_in(&request, &visibility,
                0, 10, Some(pinned.source_head))).unwrap();
        ...
        assert_eq!(page(&node, 0, 10, Some(pinned.source_head)).unwrap(), pinned);
        let cancelled = node.request_context();
        cancelled.authority().cancel();
        assert!(node.runtime().block_on(node.read_pull_requests_in(&cancelled, &RefVisibility::new(),
            0, 10, Some(pinned.source_head))).is_err());
        node.shutdown().unwrap();
    }
}
