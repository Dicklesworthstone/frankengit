...
use fgit_types::{
    DecisionOutcome, GitHashAlgorithm, HeadGeneration, RefName, RepositoryId, TenantId,
};
use std::fs;
...
fn loose(
    ...
    fs::write(dir.join(&hex[2..]), frame).unwrap();
    id
}
...
fn committed(value: (TxId, TerminalOutcome)) -> (TxId, TerminalOutcome) {
    assert!(
        matches!(value.1.outcome, DecisionOutcome::Committed { .. }),
        "{value:?}"
    );
    value
}
...
fn fixture(scratch: &Scratch, format: GitHashAlgorithm) -> Fixture {
    ...
    let data = PullRequestData {
        ...
        source_tip: theirs,
        target_tip: ours,
        title: "Candidate review".into(),
        body: "Review the actual artifact".into(),
    };
    ...
    let metadata = MergeMetadata {
        ...
        timestamp: 1,
        message: b"actual reviewed candidate\n".to_vec(),
    };
    let prepared = node
        .runtime()
    ...
    let command = CandidateReviewCommand {
        ...
        review: ReviewCommand {
            ...
            reason: "Reviewed exact candidate bytes".into(),
        },
    };
    Fixture {
        node,
        command,
        bundle: prepared.bundle.unwrap(),
        data,
    }
}
fn vote(
...
fn publish(
    ...
    command: &CandidateReviewCommand,
    bundle: &[u8],
    reviewers: &[PrincipalId],
    key: &str,
) -> Result<(TxId, TerminalOutcome), NodeWorkspaceRefusal> {
    let request = node.request_context();
    node.runtime()
        .block_on(node.apply_reviewed_merge_bundle_durable_in(
            &request,
            actor(2),
            key.as_bytes(),
            command.review.subject.pull_request,
            ExpectedVersion::Exactly(command.review.subject.pull_request_version),
            &command.candidate.merge(&command.review.subject),
            bundle,
            command.review.subject.policy_epoch,
            reviewers,
        ))
}
fn page(
...
fn review_then_publish_both_formats_preserves_code_until_the_coupled_merge() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let reviewed = snapshot(&f.node);
        code_unchanged(&before, &reviewed);
        assert_ne!(
            reviewed.basis().body().forge_position_root,
            before.basis().body().forge_position_root
        );
        assert_ne!(
            reviewed.basis().body().outbox_root,
            before.basis().body().outbox_root
        );
        assert_eq!(
            reviewed.snapshot().outbox.len(),
            before.snapshot().outbox.len() + 1
        );
        assert!(
            f.node.read_git_object(f.command.candidate.commit).is_err(),
            "review never imports candidate objects"
        );
        let rows = page(&f.node, None, 100, None).unwrap();
        ...
        assert_eq!(
            vote(&f.node, &f.command, None, 3, "approve").unwrap(),
            accepted
        );
        ...
        assert_eq!(
            vote(&reopened, &f.command, None, 3, "approve").unwrap(),
            accepted
        );
        reopened.bring_into_service(HeadGeneration::FIRST).unwrap();
        let terminal =
            committed(publish(&reopened, &f.command, &f.bundle, &[actor(3)], "publish").unwrap());
        ...
        let prs = reopened
            .runtime()
            .block_on(reopened.read_pull_requests_in(&request, &RefVisibility::new(), 0, 100, None))
            .unwrap();
        assert!(matches!(
            prs.pull_requests[0].event.payload,
            ForgeEventPayload::MergeCommittedNative(_)
        ));
        assert_eq!(prs.pull_requests[0].data, Some(f.data));
        assert_eq!(
            publish(&reopened, &f.command, &[], &[actor(3)], "publish").unwrap(),
            terminal
        );
...
fn later_approval_cannot_rewrite_an_earlier_canonical_refusal() {
    ...
    assert_eq!(
        publish(&f.node, &f.command, &[], &[actor(3)], "missing-vote").unwrap(),
        refused
    );
...
fn withdrawal_and_changed_candidate_cannot_satisfy_required_reviewers() {
    ...
    withdrawn.review.reason = "Candidate needs another pass".into();
    committed(vote(&f.node, &withdrawn, None, 3, "withdraw").unwrap());
    assert_eq!(
        page(&f.node, None, 100, None).unwrap().reviews[0].freshness,
        ReviewFreshness::Withdrawn
    );
    let before = snapshot(&f.node);
    let refused = publish(
        &f.node,
    ...
    renewed.review.expected_version =
    ...
    let other = f
        .node
        .runtime()
        .block_on(f.node.prepare_merge_bundle_in(
            &request,
            &target(),
            &incoming(),
            &RefVisibility::new(),
            &MergeMetadata {
                ...
                committer: "Fixture <fixture@example.invalid>".into(),
                timestamp: 1,
                message: b"different actual commit\n".to_vec(),
            },
            PreparationLimits::default(),
        ))
        .unwrap();
    let MergePreparation::Clean(plan) = other.outcome else {
        panic!("second candidate");
    };
    let mut different = f.command.clone();
    different.candidate.commit = plan.commit;
    let stale = publish(
        &f.node,
        &different,
        &other.bundle.unwrap(),
...
fn reviewer_keys_competing_versions_and_snapshot_pages_do_not_alias() {
    ...
    let first = page(&f.node, None, 1, None).unwrap();
    ...
    assert_eq!(second.reviews[0].event.reviewer, actor(4));
    assert!(second.next_after.is_none());
    let request = f.node.request_context();
    assert!(matches!(
        f.node.runtime().block_on(f.node.read_reviews_in(
            &request,
            &RefVisibility::new(),
            PullRequestNumber::FIRST,
            first.next_after,
        ...
        Err(ReviewReadRefusal::UnpinnedContinuation)
    ));
    let mut changed = f.command.clone();
    changed.review.reason.push('!');
    assert!(vote(&f.node, &changed, Some(&f.bundle), 3, "first-reviewer").is_err());
    let mut update = f.command.clone();
    update.review.expected_version = ExpectedVersion::Exactly(AggregateVersion::FIRST);
    update.review.decision = ReviewDecision::RequestChanges;
    update.review.reason = "Blocking problem".into();
...
fn opener_votes_and_stale_pr_metadata_do_not_authorize_publication() {
    ...
    data.title = "New metadata version".into();
    ...
    committed(
        f.node
            .runtime()
            .block_on(f.node.admit_pull_request_durable_in(
                &request,
                &session(1, "update"),
                &command,
                AdmissionLimits::default(),
    ...
    assert!(
        page(&f.node, None, 100, None)
            .unwrap()
            .reviews
            .iter()
            .all(|v| v.freshness == ReviewFreshness::PullRequestChanged)
    );
    let mut new_version = f.command.clone();
    new_version.review.subject.pull_request_version = AggregateVersion::try_new(2).unwrap();
    let refused = publish(
        &f.node,
        &new_version,
        &f.bundle,
