...
fn path_history_is_snapshot_pinned_read_only_and_excludes_other_ref_ancestry() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (head, first) = node
            .runtime()
            .block_on(node.read_path_history_in(
                &request,
                &reference,
                &RefVisibility::new(),
                None,
        ...
        assert_eq!(first.tip, target);
        assert_eq!(first.total_commits, 2);
        assert_eq!(first.commits[0].id, target);
        assert_eq!(first.next_after, Some(1));
        query.log.after = 1;
        let (last_head, last) = node
            .runtime()
            .block_on(node.read_path_history_in(
                &request,
                &reference,
            ...
            .unwrap();
        assert_eq!(last_head, head);
        assert_eq!(last.next_after, None);
        assert_eq!(last.commits.len(), 1);
        assert_ne!(last.commits[0].id, incoming);
        query.path = b"path-that-never-existed".to_vec();
        query.log.after = 0;
        let (empty_head, empty) = node
            .runtime()
            .block_on(node.read_path_history_in(
                &request,
                &reference,
            ...
            .unwrap();
        assert_eq!(empty_head, head);
        assert_eq!(empty.tip, target);
        assert_eq!(empty.total_commits, 0);
...
fn path_history_preserves_visibility_pinning_and_resource_refusals() {
    ...
    hidden
        .push_rule(b"refs/heads/main", &fgit_wire::WireLimits::default())
        .unwrap();
    assert!(matches!(
        node.runtime()
            .block_on(node.read_path_history_in(&request, &reference, &hidden, None, &query)),
        Err(NodeHistoryRefusal::Unavailable)
    ));
    let missing = RefName::try_new(b"refs/heads/missing").unwrap();
    assert!(matches!(
        node.runtime().block_on(node.read_path_history_in(
            &request,
            &missing,
            &RefVisibility::new(),
            None,
    ...
    let wrong = RepositoryAuthorityHeadId::from_digest(
        DigestAlgorithmId::try_new(1).unwrap(),
        CANONICAL_CODEC_VERSION,
        DigestBytes::try_new(&[123; 32]).unwrap(),
    );
    assert!(matches!(
        node.runtime().block_on(node.read_path_history_in(
            &request,
        ...
        Err(NodeHistoryRefusal::SnapshotMoved)
    ));
    query.log.after = 1;
    assert!(matches!(
        node.runtime().block_on(node.read_path_history_in(
        ...
        Err(NodeHistoryRefusal::UnpinnedContinuation)
    ));
    query.log.after = 0;
    query.log.limits.max_commits = 1;
    assert!(matches!(
        node.runtime().block_on(node.read_path_history_in(
            &request,
            &reference,
        ...
        Err(NodeHistoryRefusal::History(_))
    ));
    query.log = LogOptions::default();
    query.path = b"../text".to_vec();
    let refusal = node
        .runtime()
        .block_on(node.read_path_history_in(
            &request,
...
fn historical_browsing_opens_old_bytes_but_never_other_branch_commits() {
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let mut query = SourceBrowseQuery {
            path: Some(b"text".to_vec()),
            expected_head: Some(head),
            expected_commit: Some(base),
        ...
        let report = node
            .runtime()
            .block_on(
                node.browse_source_ancestor_local_in(&request, &reference, target, base, &query),
            )
            .unwrap();
        ...
        query.action = SourceBrowseAction::Read {
            offset: 2,
            limit: 2,
        };
        let range = node
        ...
        query.path = None;
        query.action = SourceBrowseAction::List {
            after: None,
            limit: 10,
        };
        assert!(
            node.runtime()
                .block_on(
                    node.browse_source_ancestor_local_in(
                        &request, &reference, target, base, &query
                    )
                )
                .is_ok()
        );
        ...
        assert!(matches!(
            node.runtime().block_on(
                node.browse_source_ancestor_local_in(
                    &request, &reference, target, incoming, &query
                )
            ),
...
fn historical_reads_refuse_unpinned_stale_missing_and_cancelled_selections() {
    ...
    let (_scratch, node, target, incoming) =
        super::super::tests::fixture(GitHashAlgorithm::Sha1, false);
    let request = node.request_context();
    let reference = RefName::try_new(b"refs/heads/main").unwrap();
    let head = node
        .runtime()
    ...
    assert!(
        node.runtime()
            .block_on(
                node.browse_source_ancestor_local_in(&request, &reference, target, target, &query)
            )
            .is_err()
    );
    ...
    assert!(
        node.runtime()
            .block_on(
                node.browse_source_ancestor_local_in(
                    &request, &reference, incoming, target, &query
                )
            )
            .is_err()
    );
    ...
    assert!(matches!(
        node.runtime().block_on(
            node.browse_source_ancestor_local_in(&request, &missing, target, target, &query)
        ),
        Err(NodeWorkspaceRefusal::RefUnavailable)
    ));
    let wrong = RepositoryAuthorityHeadId::from_digest(
