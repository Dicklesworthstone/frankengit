...
use super::{
    Cell, MergeObjectSource, MergeSourceError, ObjectType, ParseLimits, RefVisibility,
...
use fgit_forge::history::{
    BlameOptions, BlameResult, HistoryError, HistoryLimits, HistoryPage, HistorySource, LogOptions,
    blame, commit_history,
};
...
fn failed(error: HistoryError) -> NodeHistoryRefusal {
    NodeHistoryRefusal::History(Box::new(error))
}
impl HistorySource for SelectedSource<'_> {
    fn commit_body(&self, id: GitOid) -> Result<Vec<u8>, MergeSourceError> {
        self.read(id, Some(ObjectType::Commit))
            .map(|(_, body)| body)
    }
}
...
impl OneNode {
    ...
    pub async fn read_commit_history_in(
        ...
        self.with_history_source_in(
            request,
            reference,
            visibility,
            expected_head,
            options.limits,
            |source, tip| commit_history(source, self.object_format, tip, options),
        )
        .await
    }
    ...
    pub async fn read_path_history_in(
        ...
        self.with_history_source_in(
            ...
            expected_head,
            options.log.limits,
            |source, tip| path_history(source, self.object_format, tip, options),
        )
        .await
    }
    ...
    pub async fn blame_source_in(
        ...
        self.with_history_source_in(
            ...
            expected_head,
            options.limits,
            |source, tip| blame(source, self.object_format, tip, options),
        )
        .await
    }
    ...
    async fn with_history_source_in<T>(
        ...
        let tip = *selected
            ...
            .refs
            .get(reference)
            .ok_or(NodeHistoryRefusal::Unavailable)?;
        ...
        let source = SelectedSource {
            ...
            limits: ParseLimits {
                ...
                max_header_lines: HistoryLimits::default().max_edges,
        ...
        source
            .checkpoint()
            .map_err(|error| failed(HistoryError::Source(error)))?;
...
mod tests {
    ...
    fn authenticated_history_and_blame_are_read_only_in_both_formats() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            ...
            let (blame_head, lines) = node
                ...
                .unwrap();
            assert_eq!(blame_head, head);
            assert_eq!(lines.content, b"A\nb\nc\nd\ne\n");
            assert_eq!(lines.lines[0].origin_commit, target);
            assert!(
                lines.lines[1..]
                    .iter()
                    .all(|line| line.origin_commit == base)
            );
            assert_eq!(
                node.runtime()
                    .block_on(node.materialize_admission_in(&request))
    ...
    fn hidden_refs_unpinned_pages_stale_heads_and_resource_limits_refuse() {
        ...
        assert!(matches!(
            node.runtime().block_on(node.read_commit_history_in(
                &request,
                &reference,
                &RefVisibility::new(),
                None,
        ...
        assert!(matches!(
            node.runtime().block_on(node.blame_source_in(
                &request,
                &reference,
                &RefVisibility::new(),
                None,
