...
use fgit_authority::{
    ExpectedOld, IdempotencyKey, OutcomeLookup, ProposedNew, RECEIVE_ADMISSION_SCHEMA, RefCommand,
    SealAttempt, SemanticRequest,
};
    ...
    pub async fn apply_rebase_bundle_durable_in(
        ...
        let semantic = SemanticRequest::build(
            ...
            vec![RefCommand {
                ...
                proposed_new: ProposedNew::Update(expected_candidate),
                force: false,
            }],
            vec![],
            vec![],
        )
        .map_err(|_| invalid("invalid rebase ref transaction"))?;
        ...
        let admission_error = |error| {
            receive_error(crate::NodeReceiveTransportRefusal::Admission(Box::new(
                error,
            )))
        };
        let (tx_id, _) = attempt
        ...
        let (validated, mut parse_limits) = self
            .quarantine_bound_envelope_in(request, reference, expected_source, &envelope, &[])
            .await?;
        ...
        let source = VerifiedFabricPackSource {
            ...
            maximum_object_bytes: parse_limits.max_object_bytes,
        ...
        while cursor != onto {
            ...
            if seen.len() == MAX_REBASE_COMMITS || !seen.insert(cursor) {
                return Err(invalid(
                    "rebase candidate exceeds the bounded linear series",
                ));
            }
            ...
            let (kind, body) =
                read.map_err(|_| invalid("rebase commit bytes could not be verified"))?;
            bytes = bytes
            ...
            let parent = parents
                .next()
                .ok_or_else(|| invalid("rebase series does not reach its onto prerequisite"))?;
            if parents.next().is_some() {
                return Err(invalid(
                    "rebase publication accepts only a single-parent series",
                ));
            }
            cursor = std::str::from_utf8(parent)
                .ok()
                .and_then(|text| {
                    GitOid::from_hex(self.object_format, &text.to_ascii_lowercase()).ok()
                })
