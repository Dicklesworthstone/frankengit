...
impl OneNode {
    ...
    pub async fn reconcile_source_symbol_index_local_in(
        &self,
        ...
        limits: SearchLimits,
        read_limits: GenerationReadLimits,
    ) -> Result<(data::Source, GenerationActivation), Failure> {
        self.reconcile_source_symbol_index_guarded_local_in(
            request,
            reference,
            expected_head,
            minimum,
    ...
    pub async fn reconcile_source_symbol_index_guarded_local_in(
        &self,
        request: &NodeRequestContext,
        reference: &RefName,
        expected_head: Option<RepositoryAuthorityHeadId>,
        ...
        let predecessor = if let Some(generation) = generation {
            ...
            let mut bytes = 0;
            let raw = self
                .read_symbol_payload(request, root, &mut bytes, data::MAX_INDEX_BYTES)
                .await?;
        ...
        match predecessor {
            ...
            Some(predecessor) => self
                .refresh_source_symbol_index_guarded_local_in(
                    request,
                    reference,
                    Some(head),
                    Some(commit),
                ...
                .await
                .map(|(source, activation, _)| (source, activation)),
        }
