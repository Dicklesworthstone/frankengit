...
impl OneNode {
    ...
    pub async fn refresh_source_symbol_index_local_in(
        &self,
        ...
        self.refresh_source_symbol_index_guarded_local_in(
            request,
            reference,
            expected_head,
            expected_commit,
    ...
    pub async fn refresh_source_symbol_index_guarded_local_in(
        &self,
        request: &NodeRequestContext,
        reference: &RefName,
        expected_head: Option<RepositoryAuthorityHeadId>,
        ...
        let raw = self
            .read_symbol_payload(
                request,
                root,
                &mut stats.predecessor_payload_bytes,
        ...
        let mut verifier =
            data::ReuseVerifier::new(&manifest, &cancelled).map_err(Failure::Index)?;
        drop(raw);
        drop(manifest);
        drop(generation);
        ...
        let (source, activation) = self
            .publish_symbol_corpus_in(
                request,
                reference,
                (head, forge, corpus),
                Some(predecessor),
