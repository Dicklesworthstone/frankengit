...
use fgit_graph::{
    ...
    GenerationReadLimits, GenerationRecovery, GraphAuthorityClass, GraphGenerationBody,
...
fn live(request: &NodeRequestContext) -> Result<(), Failure> {
    if workspace_request_live(request) {
        ...
        Err(Failure::Index(data::Error::Cancelled))
    }
}
...
impl OneNode {
    fn symbol_key_prefix(&self, kind: &[u8]) -> Vec<u8> {
        ...
        key.extend_from_slice(self.tenant_id.as_bytes());
        key.extend_from_slice(self.repository_id.as_bytes());
        key.extend_from_slice(self.repository_incarnation_id().as_bytes());
        key.push(match self.object_format {
            Format::Sha1 => 1,
            Format::Sha256 => 2,
        });
    ...
    fn symbol_payload_key(&self, root: Digest) -> Result<ImmutableKey, Failure> {
        ...
        key.extend_from_slice(&root.algorithm().code_point().to_be_bytes());
        key.extend_from_slice(root.bytes().as_bytes());
    ...
    pub async fn build_source_symbol_index_local_in(
        &self,
        ...
        self.build_source_symbol_index_guarded_local_in(
            request,
            reference,
            expected_head,
            expected_commit,
    ...
    pub async fn build_source_symbol_index_guarded_local_in(
        &self,
        request: &NodeRequestContext,
        reference: &RefName,
        expected_head: Option<RepositoryAuthorityHeadId>,
        ...
        let query = Build(
            SourceQuery::new(b"symbols", SearchCase::Exact, &[])
    ...
    async fn publish_symbol_corpus_in(
        &self,
        request: &NodeRequestContext,
        reference: &RefName,
        selected: (RepositoryAuthorityHeadId, Digest, data::Corpus),
        predecessor: Option<GraphGenerationId>,
        barrier: &mut (impl FnMut(GraphGenerationId) -> Result<(), NodeWorkspaceRefusal> + Send),
    ) -> Result<(data::Source, GenerationActivation), Failure> {
        live(request)?;
        ...
        if tables
            ...
            .any(|p| p.bytes.len() > self.authority.limits().body_bytes)
        {
        ...
        live(request)?;
        barrier(candidate).map_err(Failure::Source)?;
        let result: Result<GenerationActivation, Failure> = async {
            ...
            {
                live(request)?;
                match AsyncAuthorityStore::put_if_absent(
                    &self.authority,
                    request.authority(),
                    &self.symbol_payload_key(payload.root)?,
                    &payload.bytes,
                )
                .await
            ...
            live(request)?;
            GenerationAuthority::new(&self.authority, head_key)
                .stage_and_activate_async(request.authority(), &body)
                .await
                .map_err(Failure::Generation)
        }
        .await;
    ...
    pub async fn search_source_symbols_index_snapshot_local_in(
        ...
        if expected_head.is_some_and(|expected| expected != head) {
            return Err(Failure::Source(NodeWorkspaceRefusal::SourceBrowse(
                Box::new(fgit_forge::source_browse::SourceBrowseError::SnapshotMoved),
            )));
        }
        if expected_commit.is_some_and(|expected| expected != commit) {
            return Err(Failure::Source(NodeWorkspaceRefusal::SourceBrowse(
                Box::new(fgit_forge::source_browse::SourceBrowseError::CommitMoved),
            )));
        }
        ...
        let mut bytes = 0;
        let raw = self
            .read_symbol_payload(request, root, &mut bytes, maximum_payload_bytes)
            .await?;
        let cancelled = || !workspace_request_live(request);
        ...
        let candidates = if let Some(root) = symbol_directory_root(body)? {
            let raw = self
                ...
                .await?;
            search
                .directory_candidates(&manifest, &raw, root, &cancelled)
                .map_err(Failure::Index)?
        } else {
            manifest
                .documents()
                .iter()
                .enumerate()
                .filter_map(|(i, doc)| search.includes(doc).then_some(i))
                .collect()
        };
        ...
        for ordinal in candidates {
            ...
            if files > limits.max_files {
                return Err(Failure::Index(data::Error::Limit("table reads")));
            }
            source_bytes = source_bytes
                .checked_add(doc.source_bytes)
                .filter(|n| *n <= limits.max_total_bytes)
                .ok_or(Failure::Index(data::Error::Limit(
                    "referenced source bytes",
                )))?;
            if doc.source_bytes > limits.max_file_bytes
                || bytes
                    .checked_add(doc.encoded_bytes)
                    .is_none_or(|n| n > maximum_payload_bytes)
            {
                return Err(Failure::Index(data::Error::Limit(
                    "table/source read budget",
                )));
            }
            let raw = self
                .read_symbol_payload(request, doc.root, &mut bytes, maximum_payload_bytes)
                .await?;
            if search
                .observe(doc, &raw, &cancelled)
                .map_err(Failure::Index)?
            {
                break;
            }
    ...
    fn validate_symbol_source(
        ...
        if source.tenant != self.tenant_id
            ...
            || source.rcr != body.source().source_rcr_id
            || source.forge != body.source().source_forge_position_root
        {
    ...
    async fn read_symbol_payload(
        ...
        let raw = AsyncAuthorityStore::read_immutable(
            &self.authority,
            request.authority(),
            &self.symbol_payload_key(root)?,
        )
        .await
        .map_err(Failure::Authority)?;
        live(request)?;
        let ImmutableRead::Present(raw) = raw else {
            return Err(Failure::Missing(root));
        };
        if raw.len() > data::MAX_PAYLOAD {
            return Err(Failure::Index(data::Error::Limit("payload bytes")));
        }
        *bytes = bytes
            .checked_add(raw.len())
            .filter(|n| *n <= maximum)
            .ok_or(Failure::Index(data::Error::Limit("index read bytes")))?;
        Ok(raw)
    }
    /// Original-candidate observation, not a retry or an inference from staged tables.
    pub async fn recover_source_symbol_index_local_in(
        &self,
        request: &NodeRequestContext,
        reference: &RefName,
