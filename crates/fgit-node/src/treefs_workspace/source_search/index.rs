...
mod inventory;
use inventory::{InventoryRequest, Document};
use super::*;
use fgit_graph::{GenerationActivation, GenerationRecovery, GraphGenerationId, GenerationReadLimits};
use fgit_graph::lexical::{IndexError, IndexedLexicalReport, LexicalError, LexicalIndexStore,
    LexicalNamespace, LexicalQuery, LexicalQueryLimits, LexicalReadLimits, LexicalSegment,
    PreparedLexicalIndex, SourceDocument, LexicalSource, MAX_DOCUMENTS};
...
fn live(request: &NodeRequestContext) -> Result<(), NodeWorkspaceRefusal> {
    ...
    else { Err(search_error(SearchError::Cancelled)) }
}
...
fn segments(namespace: LexicalNamespace, documents: &[Document],
    request_live: &mut impl FnMut() -> bool,
) -> Result<Vec<LexicalSegment>, IndexError> {
    if !request_live() { return Err(LexicalError::Cancelled.into()); }
    ...
    for (group, chunk) in documents.chunks(MAX_DOCUMENTS).enumerate() {
        ...
        while let Some((offset, inputs)) = pending.pop() {
            if !request_live() { return Err(LexicalError::Cancelled.into()); }
            attempts += 1;
            if attempts > 256 { return Err(LexicalError::Limit("segment build attempts").into()); }
            let bytes: usize = inputs.iter().map(|input| input.bytes.len() + input.path.len()).sum();
            rescanned = rescanned.checked_add(bytes).filter(|n| *n <= 256 * 1024 * 1024)
                .ok_or(LexicalError::Limit("segment build byte work"))?;
            let built = LexicalSegment::build(namespace, offset as u64 + 1, inputs.iter().map(|input|
                SourceDocument { path: &input.path, blob: input.blob, content: &input.bytes }), request_live);
            match built {
                Ok(segment) => {
                    if output.len() == 128 { return Err(LexicalError::Limit("index segments").into()); }
                    encoded_bytes = encoded_bytes.checked_add(segment.encode(request_live)?.len())
                        .filter(|n| *n <= 32 * 1024 * 1024)
                        .ok_or(LexicalError::Limit("index bytes"))?;
                    output.push(segment);
                }
                Err(LexicalError::Limit("documents" | "dictionary terms" | "postings" | "segment bytes")) if inputs.len() > 1 => {
...
impl OneNode {
    fn lexical_namespace(&self) -> LexicalNamespace {
        LexicalNamespace { tenant: self.tenant_id, repository: self.repository_id,
            incarnation: self.repository_incarnation_id(), object_format: self.object_format }
    }
    ...
    pub async fn build_source_index_local_in(
        ...
        let query = InventoryRequest(SourceQuery::new(b"index", SearchCase::Exact, &[]).map_err(search_error)?);
        let (head, forge_position_root, inventory) = match self.object_format {
            Format::Sha1 => self.select_source_local_format::<Sha1, _>(request, reference,
                expected_head, expected_commit, &query, limits).await?,
        ...
        let parts = segments(source.namespace, &inventory.documents, &mut request_live).map_err(index_error)?;
    ...
    #[expect(clippy::too_many_arguments, reason = "source pins, index pins, pagination and independent resource budgets are distinct contracts")]
    pub async fn search_source_index_local_in(
        &self, request: &NodeRequestContext, reference: &RefName,
        expected_head: Option<RepositoryAuthorityHeadId>, expected_commit: Option<GitOid>,
        generation: Option<&GenerationActivation>, minimum: Option<&GenerationActivation>,
        query: &LexicalQuery, after: Option<u64>, query_limits: LexicalQueryLimits, read_limits: LexicalReadLimits,
        ...
        if after.is_some() && (generation.is_none() || expected_head.is_none() || expected_commit.is_none()) {
            return Err(index_error(LexicalError::Invalid("continuation requires exact source and generation")));
        }
        if expected_commit.is_some_and(|id| id.is_zero() || id.algorithm() != self.object_format) {
            return Err(NodeWorkspaceRefusal::ObjectFormatMismatch);
        }
        ...
        let mut request_live = || workspace_request_live(request);
        let index = store.select_async(request.authority(), generation, minimum, read_limits, &mut request_live).await.map_err(index_error)?;
        let source = index.source();
        if source.source_head != head || source.commit != commit || source.source_rcr != rcr
            || source.forge_position_root != selected.basis().body().forge_position_root
        { return Err(NodeWorkspaceRefusal::SourceIndexStale); }
        let report = store.search_async(request.authority(), &index, query, after, read_limits, query_limits, &mut request_live)
    ...
    pub async fn recover_source_index_local_in(
        &self, request: &NodeRequestContext, reference: &RefName, candidate: GraphGenerationId,
        ...
        let store = LexicalIndexStore::new(&self.authority, self.lexical_namespace(), reference.clone()).map_err(index_error)?;
        store.recover_async(request.authority(), candidate, minimum, limits, &mut || workspace_request_live(request))
            .await.map_err(index_error)
    }
}
...
mod tests {
    ...
    fn namespace() -> LexicalNamespace {
        LexicalNamespace { tenant: fgit_types::TenantId::from_bytes([1;16]),
    ...
    fn document(path: &[u8], bytes: Vec<u8>) -> Document {
        Document { path: path.to_vec(), blob: git_object_id(namespace().object_format, GitObjectKind::Blob, &bytes), bytes }
    }
    #[test]
    fn partitioning_preserves_every_document_and_absolute_id_under_dictionary_pressure() {
        let documents: Vec<_> = (0..3).map(|n| document(format!("file{n}").as_bytes(),
            (0..20_000).map(|i| format!("word{:05} ", n * 20_000 + i)).collect::<String>().into_bytes())).collect();
        let parts = segments(namespace(), &documents, &mut || true).unwrap();
        assert_eq!(parts.len(), 3);
        assert_eq!(parts.iter().flat_map(|p| p.documents()).map(|d| d.id).collect::<Vec<_>>(), vec![1,2,3]);
        ...
        for (left, right) in parts.iter().zip(again) {
            assert_eq!(left.encode(&mut || true).unwrap(), right.encode(&mut || true).unwrap());
        }
    }
    #[test]
    fn unsupported_document_never_becomes_an_omission_or_a_different_segmentation() {
        let docs = vec![document(b"a", b"small".to_vec()), document(b"b", vec![b'x';129])];
        assert!(matches!(segments(namespace(), &docs, &mut || true),
            Err(IndexError::Lexical(LexicalError::Limit("token bytes")))));
        let mut docs = vec![document(b"a", b"small".to_vec())]; docs[0].bytes[0] = b'S';
        assert!(matches!(segments(namespace(), &docs, &mut || true),
    ...
    fn empty_inventory_is_distinct_from_cancelled_build() {
        ...
        assert!(matches!(segments(namespace(), &docs, &mut || false),
            Err(IndexError::Lexical(LexicalError::Cancelled))));
        assert_eq!(segments(namespace(), &docs, &mut || true).unwrap()[0].documents().len(), 1);
    }
}
