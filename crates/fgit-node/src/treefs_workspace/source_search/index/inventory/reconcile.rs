...
impl OneNode {
    ...
    pub async fn reconcile_source_index_guarded_local_in(
        ...
        let store = LexicalIndexStore::new(&self.authority, self.lexical_namespace(), reference.clone())
            .map_err(index_error)?;
        let mut request_live = || workspace_request_live(request);
        let previous = match store.select_async(request.authority(), None, minimum, read_limits, &mut request_live).await {
            Ok(index) => Some(index),
        ...
        if let Some(index) = &previous {
            ...
            if source.source_head == head && source.commit == commit
                && source.source_rcr == rcr && source.forge_position_root == forge_position
            {
                return Ok((source.clone(), index.activation().clone()));
            }
        }
        let predecessor = previous.as_ref().map(|index| index.activation().generation_id);
        drop(previous);
        drop(selected);
