...
use super::super::{
    LocalSearch, SearchCompletion, SearchError, SearchLimits, SourceQuery, SourceSearchReport,
};
...
impl LocalSearch for InventoryRequest {
    ...
    fn run<A: GitHashAlgorithm, S: ObjectSource<A>>(
        ...
        for (path, blob) in discovery.files {
            ...
            inventory.source.bytes_read = inventory
                .source
                .bytes_read
            ...
            inventory.documents.push(Document {
...
fn discover<A: GitHashAlgorithm, S: ObjectSource<A>>(
    ...
    if depth > limits.max_depth {
        return Err(SearchError::Budget("index tree depth"));
    }
    let children = base
    ...
    for (name, entry) in children {
        check(cancelled)?;
        found.entries += 1;
        if found.entries > limits.max_entries {
            return Err(SearchError::Budget("index tree entries"));
        }
        ...
        match entry {
            ...
            BaseEntry::File { oid, mode } => {
                ...
                if found.files.len() == limits.max_files {
                    return Err(SearchError::Budget("index files"));
                }
                found.path_bytes = found
                    .path_bytes
                    .checked_add(path.as_bytes().len())
                    .filter(|n| *n <= 8 * 1024 * 1024)
                    .ok_or(SearchError::Budget("index path bytes"))?;
                if found.files.insert(path, oid).is_some() {
