...
use fgit_forge::source_symbols::{
    SymbolQuery, SymbolReadError, SymbolSearchReport, search_source_symbols,
};
...
impl LocalSearch for SymbolQuery {
    ...
    fn empty(&self, source: SourceSearchReport) -> Self::Report {
        Ok(SymbolSearchReport::empty(source))
    }
    fn run<A: GitHashAlgorithm, S: ObjectSource<A>>(
        ...
        Ok(search_source_symbols(
            base, source, capability, now, self, limits, cancelled,
        ))
    }
}
impl OneNode {
    ...
    #[expect(
        ...
        reason = "caller capability, visibility and source budgets are independent"
    )]
    ...
    pub async fn search_source_symbols_snapshot_local_in(
        ...
        (RepositoryAuthorityHeadId, SymbolSearchReport),
        SymbolReadError<NodeWorkspaceRefusal>,
    > {
        if expected_commit.is_some_and(|id| id.is_zero() || id.algorithm() != self.object_format) {
            return Err(SymbolReadError::Source(search_error(
                SearchError::InvalidObjectFormat,
            )));
        }
        ...
        .map_err(SymbolReadError::Source)?;
        result
            .map(|report| (head, report))
            .map_err(|error| error.map_source(search_error))
    }
}
