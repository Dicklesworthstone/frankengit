...
fn prepare(node: &OneNode, profile: MergeProfile) -> PreparedMergeBundle {
    ...
    node.runtime()
        .block_on(node.prepare_merge_bundle_with_profile_in(
            &request,
            &target,
            &incoming,
            &RefVisibility::new(),
...
fn session(key: &[u8]) -> crate::LoopbackReceiveSession {
    crate::LoopbackReceiveSession::authenticated(
        principal(),
...
fn open_subject(
    ...
    target_tip: GitOid,
    source_tip: GitOid,
) -> (ReviewSubject, PullRequestCommand) {
...
fn rename_profile_produces_verified_read_only_candidates_without_losing_edits() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let MergePreparation::Clean(plan) = renamed.outcome else {
            panic!("rename must be clean");
        };
        assert_eq!((plan.target, plan.source), (target, incoming));
        assert!(
            node.read_git_object(plan.commit).is_err(),
            "preparation staged its candidate"
        );
        ...
        let root = plan
            .objects
            .iter()
            .find(|object| object.id == plan.tree)
            .unwrap();
        ...
        let commit = plan
            ...
            .unwrap();
        assert!(commit.body.starts_with(
            format!("tree {}\nparent {target}\nparent {incoming}\n", plan.tree).as_bytes()
        ));
...
fn renamed_bundle_publishes_through_existing_authority_and_exact_retry() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            node.read_git_object(plan.tree).unwrap().payload(),
            plan.objects
                .iter()
                .find(|object| object.id == plan.tree)
                .unwrap()
...
fn pr_profile_requires_the_same_open_subject_and_cannot_refresh_stale_metadata() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let artifact = node
            .runtime()
            .block_on(node.prepare_pull_request_bundle_with_profile_in(
                &request,
                &subject,
                &RefVisibility::new(),
                &metadata(),
...
fn explicit_renames_do_not_bypass_visibility_resource_or_cancellation_gates() {
    ...
    hidden
        .push_rule(incoming.as_bytes(), &fgit_wire::WireLimits::default())
        .unwrap();
    assert!(matches!(
        node.runtime()
            .block_on(node.prepare_merge_bundle_with_profile_in(
