...
use crate::{
    LoopbackReceiveSession, NodeRequestContext, OneNode, PackContextCheckpoint,
...
impl OneNode {
    ...
    pub async fn recover_transaction_in(
        ...
        recover_request_async(
            &self.authority,
            request.authority(),
            &self.head_key,
            scope,
            authenticated.client_idempotency_key(),
            &|| recovery_checkpoint(request),
        )
        .await
    }
