...
impl OneNode {
    ...
    pub async fn sparse_candidate_manifest_in<A: Hash>(
        ...
        self.sparse_bound_candidate_in(request, reference, expected_base, candidate, None,
            input, visibility, expected_head, capability, now, sparse).await
    }
    ...
    pub async fn sparse_merge_candidate_manifest_in<A: Hash>(
        ...
        merge.validate().map_err(|_| invalid("invalid merge input coordinates"))?;
        ...
        { return Err(invalid("merge workflow requires two distinct bounded branches")); }
        self.sparse_bound_candidate_in(request, &merge.target_ref, merge.target_tip_before,
            merge.merge_commit, Some(merge), input, visibility, expected_head, capability, now, sparse).await
    }

    async fn sparse_bound_candidate_in<A: Hash>(
        &self, request: &NodeRequestContext, reference: &RefName,
        expected_base: GitOid, candidate: GitOid, merge: Option<&NativeMerge>, input: &[u8],
        visibility: &RefVisibility, expected_head: Option<RepositoryAuthorityHeadId>,
        capability: &mut TreeCapability, now: u64, sparse: SparseLimits,
    ) -> Result<(RepositoryAuthorityHeadId, SparseCandidateManifest<A>, [u8; 32]), BundleInspectionRefusal> {
        ...
        envelope.bind(self.object_format, reference, expected_base, candidate)
            .map_err(|e| BundleInspectionRefusal::Envelope(Box::new(e)))?;
        if merge.is_some_and(|m| envelope.prerequisites.len() != 2
            || !envelope.prerequisites.contains(&m.source_tip))
        { return Err(invalid("merge inputs require exact target and source prerequisites")); }
        ...
        if let Some(m) = merge { selections.push((&m.source_ref, m.source_tip)); }
        if selections.iter().any(|(name, _)| visibility.hides(name.as_bytes())) {
        ...
        if selections.iter().any(|(name, _)| selected.snapshot().hidden_refs.hides(name.as_bytes())) {
            return Err(BundleInspectionRefusal::RefUnavailable);
        }
        for (name, expected) in &selections {
            let actual = selected.snapshot().refs.get(*name).ok_or(BundleInspectionRefusal::RefUnavailable)?;
            if actual != expected { return Err(BundleInspectionRefusal::ParentMoved); }
        }
        ...
        let manifest = match merge {
            Some(m) => SparseCandidateManifest::build_merge(&base_view, &tree_source, typed(candidate)?,
                typed(m.source_tip)?, capability, now, source.parse_limits.clone(), sparse),
            None => SparseCandidateManifest::build(&base_view, &tree_source, typed(candidate)?,
