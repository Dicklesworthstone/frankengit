...
impl OneNode {
    ...
    pub async fn read_pull_requests_in(
        ...
        if limit == 0 || limit > 100 {
            return Err(PullRequestReadRefusal::InvalidLimit);
        }
        ...
        .map_err(|error| match error {
            ...
            snapshots::SnapshotReadRefusal::Admission(error) => {
                PullRequestReadRefusal::Admission(error)
            }
        })?;
        ...
        .await
        .map_err(|error| PullRequestReadRefusal::Admission(Box::new(error)))
    }
}
...
impl PullRequestProjection<FsqliteAuthorityStore> for NodeNativeMergeProjection<'_> {
    #[expect(
        ...
        reason = "explicit Send is the native projection contract"
    )]
