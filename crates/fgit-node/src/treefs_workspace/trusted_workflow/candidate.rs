...
impl<A: GitHashAlgorithm> WorkflowInputs<A> {
    pub(super) fn entries(&self) -> &[SparseEntry<A>] {
        match self { Self::Canonical(m) => m.entries(), Self::Candidate { manifest, .. } => manifest.entries() }
    }
    pub(super) fn payload_bytes(&self) -> usize {
        match self { Self::Canonical(m) => m.receipt().payload_bytes(), Self::Candidate { manifest, .. } => manifest.payload_bytes() }
    }
    pub(super) fn host_plan(&self, capability: &TreeCapability) -> Result<SparseWorkspacePlan<A>, HostRefusal> {
        match self {
            Self::Canonical(m) => SparseWorkspacePlan::new(Arc::clone(m), Vec::new(), capability, 0, SOURCE_LIMITS),
...
impl OneNode {
    ...
    pub async fn run_trusted_candidate_workflow_in(
        ...
        coordinates: (GitOid, GitOid), bundle: &[u8], workflow_path: &[u8],
        ...
        match self.object_format {
            Format::Sha1 => self.run_candidate_format::<Sha1>(request, reference, coordinates, bundle,
                workflow_path, run_id, parent, paths, expected_head, limits, started).await,
            Format::Sha256 => self.run_candidate_format::<Sha256>(request, reference, coordinates, bundle,
                workflow_path, run_id, parent, paths, expected_head, limits, started).await,
        }
    }
    async fn run_candidate_format<A: GitHashAlgorithm>(
        ...
        let (head, manifest, bundle_sha256) = self.sparse_candidate_manifest_in::<A>(request,
