...
fn local_record() -> WorkflowCheckRecord {
    ...
    report.execution = prepared
        .execute(&temp.0, &mut Executor::new(&temp), &|| true)
        .unwrap();
    let mut journal = FileCheckJournal::open(
        &temp.0.join("check-proposals.journal"),
        report.check_journal_scope(),
        Default::default(),
        None,
        &|| true,
    )
    ...
    let record =
        OneNode::workflow_check_record_from_batch(source_ref(), &batch, index, &bytes, &|| true)
            .unwrap();
    assert!(
        OneNode::workflow_check_record_from_batch(
            source_ref(),
            &batch,
            usize::MAX,
            &bytes,
    ...
    corrupt[0] ^= 1;
    assert!(
        OneNode::workflow_check_record_from_batch(source_ref(), &batch, index, &corrupt, &|| true)
            .is_err()
    );
    assert!(
        OneNode::workflow_check_record_from_batch(source_ref(), &batch, index, &bytes, &|| false)
            .is_err()
    );
    record
}
...
fn every_claimed_coordinate_and_scope_is_checked_against_the_retained_frame() {
    ...
    for change in 0..7 {
        ...
        match change {
            ...
            4 => {
                bad.source_commit =
                    GitOid::from_hex(GitHashAlgorithm::Sha1, &"ff".repeat(20)).unwrap()
            }
            5 => bad.conclusion = WorkflowCheckConclusion::Failure,
            _ => bad.evidence.push(0),
        }
        assert!(
            validate_record(&bad, tenant, repository, &|| true).is_err(),
            "coordinate {change}"
        );
    }
...
fn run_fixture(
    ...
    format: GitHashAlgorithm,
    fail: bool,
) -> (OneNode, NodeConfig, TrustedWorkflowRun) {
...
fn submitted_record(run: &TrustedWorkflowRun) -> WorkflowCheckRecord {
    ...
    OneNode::workflow_check_record_from_batch(source_ref(), &batch, index, &evidence, &|| true)
        .unwrap()
}
...
fn actual_publication_survives_reopen_with_exact_evidence_and_no_branch_or_policy_change() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let temp = Temp::new();
        let (mut node, config, run) = run_fixture(&temp, format, false);
        let record = submitted_record(&run);
        let before = materialize(&node);
        ...
        let before_head = before.basis().body().clone();
        let after_head = after.basis().body().clone();
        assert_eq!(before_head.ref_root, after_head.ref_root);
        assert_eq!(before_head.policy_epoch, after_head.policy_epoch);
        assert_eq!(before_head.retention_root, after_head.retention_root);
        assert_ne!(
            before_head.forge_position_root,
            after_head.forge_position_root
        );
        ...
        let fgit_forge::AggregateId::WorkflowCheck(id) = event.aggregate else {
            panic!("workflow aggregate")
        };
        let basis = after.basis().clone();
        let saved = node
            .runtime()
            .block_on(workflow_checks::read_at(
                &node.authority,
        ...
        assert_eq!(saved.actor, publisher());
        assert_eq!(
            saved.record.conclusion,
            WorkflowCheckConclusion::ActionRequired
        );
        assert_eq!(
...
fn occupied_job_and_evidence_substitution_cannot_create_a_second_delivery() {
    let temp = Temp::new();
    let (mut node, _, run) = run_fixture(&temp, GitHashAlgorithm::Sha1, false);
    let record = submitted_record(&run);
    let first = publish(&node, &record, b"publish-once");
    ...
    assert_eq!(
        materialize(&node).basis().body().clone().outbox_root,
        outbox
    );
    let mut bad = record.clone();
    bad.job.push_str("-forged");
    ...
    assert!(
        node.runtime()
            .block_on(node.admit_trusted_workflow_check_in(
                &node.request_context(),
                &session(b"publish-once"),
                &bad,
                Default::default()
            ))
            .is_err(),
        "a changed body cannot reuse the successful key"
    );
    node.shutdown().unwrap();
}
...
fn historical_retry_is_not_rewritten_when_the_current_branch_moves() {
    let temp = Temp::new();
    let (mut node, _, run) = run_fixture(&temp, GitHashAlgorithm::Sha256, false);
    let record = submitted_record(&run);
    let first = publish(&node, &record, b"old-observation");
    let body = format!(
    ...
    assert_eq!(
        materialize(&node).snapshot().refs.get(&source_ref()),
        Some(&next)
    );
    node.shutdown().unwrap();
}
...
fn actual_failed_job_is_recorded_as_failure_without_a_green_conclusion() {
    let temp = Temp::new();
    let (mut node, _, run) = run_fixture(&temp, GitHashAlgorithm::Sha1, true);
    let record = submitted_record(&run);
    assert_eq!(record.conclusion, WorkflowCheckConclusion::Failure);
    assert!(matches!(
        publish(&node, &record, b"failed-job").1.outcome,
        DecisionOutcome::Committed { .. }
    ));
    assert_eq!(
        materialize(&node).snapshot().refs.get(&source_ref()),
