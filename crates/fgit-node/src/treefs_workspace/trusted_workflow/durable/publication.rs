...
use fgit_forge::event::workflow_check::{WorkflowCheckConclusion, WorkflowCheckRecord, MAX_CHECK_EVIDENCE_BYTES};
...
use fgit_runner::coordinator::delivery::journal::history::{decode_trusted_observation, verify_trusted_job};
...
impl OneNode {
    ...
    pub fn workflow_check_record_from_batch(
        ...
        if !live() { return Err(invalid(RefusalCode::CancellationInProgress)); }
        let observed = verify_trusted_job(batch, fact_index, evidence, MAX_CHECK_EVIDENCE_BYTES, live)
            .map_err(|_| invalid(if live() { RefusalCode::EvidenceInvalid } else { RefusalCode::CancellationInProgress }))?;
        let job = &observed.report().jobs[0];
        let record = WorkflowCheckRecord {
            source_ref, source_commit: observed.source_commit(),
            ...
            job: job.id.clone(), conclusion: conclusion(job.outcome), evidence: evidence.to_vec(),
        };
        record.validate().map_err(|_| invalid(RefusalCode::EvidenceInvalid))?;
        if !live() { return Err(invalid(RefusalCode::CancellationInProgress)); }
        Ok(record)
    }
    ...
    pub async fn admit_trusted_workflow_check_in(
        ...
        let context = AdmissionContext {
            head_key: self.head_key.clone(), tenant_id: self.tenant_id, repository_id: self.repository_id,
        ...
        let tx = attempt.derive().map_err(|_| invalid(RefusalCode::EvidenceInvalid))?.0;
        ...
        let projection = NodeNativeMergeProjection {
            node: self, inner: self.durable_admission_projection(&context).map_err(map)?,
        ...
        let terminal = workflow_checks::admit_async(
            &self.authority, request.authority(), &context, record, limits, &projection,
        ).await.map_err(map)?;
        Ok((tx, terminal))
    }
}
...
impl workflow_checks::WorkflowCheckProjection<FsqliteAuthorityStore> for NodeNativeMergeProjection<'_> {
    fn validate_workflow_check_async<'a>(
        ...
        async move {
            ...
            validate_record(record, self.node.tenant_id, self.node.repository_id,
            ...
            let ParsedObject::Commit(commit) = parse_object_body(
                kind, &body, AcceptanceProfile::GitCompatibleImport, &source.parse_limits(),
            ).map_err(|_| ProjectionFailure::Refuse(RefusalCode::EvidenceInvalid))? else {
                return Err(ProjectionFailure::Refuse(RefusalCode::EvidenceInvalid));
            };
            ...
            self.merge_checkpoint(cx).map_err(ProjectionFailure::Unavailable)?;
            Ok(ValidatedClosure {
                object_closure_root: fgit_admission::permitted_object_closure_root(closure).map_err(ProjectionFailure::Unavailable)?,
...
fn validate_record(record: &WorkflowCheckRecord, tenant: TenantId, repository: RepositoryId,
    live: &dyn Fn() -> bool) -> Result<(), RefusalCode> {
    record.validate().map_err(|_| RefusalCode::EvidenceInvalid)?;
    ...
    let observed = decode_trusted_observation(&record.evidence, Commitment::of_bytes(&record.evidence),
        MAX_CHECK_EVIDENCE_BYTES, live).map_err(|_| RefusalCode::EvidenceInvalid)?;
    let report = observed.report();
    if observed.tenant() != tenant || observed.repository() != repository
        ...
        || bytes32(report.graph)? != record.graph_root || report.jobs.len() != 1
        || report.jobs[0].id != record.job || conclusion(report.jobs[0].outcome) != record.conclusion
    { return Err(RefusalCode::EvidenceInvalid); }
    Ok(())
}
fn bytes32(value: Commitment) -> Result<[u8; 32], RefusalCode> {
    value.digest().bytes().as_bytes().try_into().map_err(|_| RefusalCode::EvidenceInvalid)
}
fn conclusion(value: JobOutcome) -> WorkflowCheckConclusion {
    match value {
        ...
        JobOutcome::Failed | JobOutcome::Refused | JobOutcome::OutputLimit => WorkflowCheckConclusion::Failure,
...
fn invalid(code: RefusalCode) -> NodeReceiveTransportRefusal {
    NodeReceiveTransportRefusal::Admission(Box::new(AdmissionError::AsyncProjectionUnavailable(code)))
}
