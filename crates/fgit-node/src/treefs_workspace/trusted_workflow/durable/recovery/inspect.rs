...
impl OneNode {
    ...
    pub fn trusted_workflow_history_json(
        ...
        if limits.0 == 0 || limits.0 > MAX_HISTORY_BATCHES
            || limits.1 == 0 || limits.1 > MAX_HISTORY_BYTES
        {
            return Err(TrustedWorkflowFailure::InvalidInput("invalid custody history page limits"));
        }
        ...
        let page = journal.read_history(snapshot, after, limits.0, limits.1, live)
            .map_err(|e| failure(directory, e))?;
        let mut entries = Vec::new();
        for entry in page.entries() {
            if !live() { return Err(failure(directory, "custody history rendering cancelled")); }
            entries.push(entry_json(entry));
        }
        let entries = entries.join(",");
        if !live() { return Err(failure(directory, "custody history rendering cancelled")); }
        let continuation = page.next_after().map_or_else(|| "null".to_owned(), |id| quoted(&root_hex(id)));
        Ok(format!(
            ...
            scope_json(expected), pin_json(page.snapshot()), journal.retained_batches(), journal.pending_batches(),
        ))
    }
    ...
    pub fn trusted_workflow_artifact(
        ...
        let mut journal = Self::open_trusted_workflow_journal(directory, expected, minimum, live)?;
        let entry = journal.read_retained_batch(batch).map_err(|e| failure(directory, e))?;
        if !live() { return Err(failure(directory, "custody artifact read cancelled")); }
        let (bytes, id, kind) = match evidence {
            Some(id) => (journal.read_batch_evidence(batch, id).map_err(|e| failure(directory, e))?, id, "evidence"),
            None => (entry.batch().body().to_vec(), batch, "proposal_batch"),
        };
        if Commitment::of_bytes(&bytes) != id {
            return Err(failure(directory, "saved custody artifact commitment mismatch"));
        }
        if !live() { return Err(failure(directory, "custody artifact read cancelled")); }
        let metadata = format!(
            ...
            scope_json(expected), pin_json(journal.pin()), root_hex(batch), root_hex(id), bytes.len(),
...
fn checked_root(value: Digest) -> Result<Commitment, TrustedWorkflowFailure> {
    if value.algorithm() != DigestAlgorithm::Sha256.id() || value.bytes().as_bytes().len() != 32 {
        return Err(TrustedWorkflowFailure::InvalidInput("custody recovery requires a 32-byte SHA-256 digest"));
    }
    Commitment::try_from_digest(value)
        .map_err(|_| TrustedWorkflowFailure::InvalidInput("unsupported custody recovery digest"))
}
fn checked_scope(value: (TenantId, RepositoryId, Digest)) -> Result<CheckJournalScope, TrustedWorkflowFailure> {
    Ok(CheckJournalScope { tenant: value.0, repository: value.1, journal_id: checked_root(value.2)? })
}
fn checked_pin(value: (u64, Digest)) -> Result<CheckJournalPin, TrustedWorkflowFailure> {
    if !(72..=4 * 1024 * 1024 * 1024).contains(&value.0) {
        return Err(TrustedWorkflowFailure::InvalidInput("invalid custody recovery pin length"));
    }
...
fn failure(directory: &Path, error: impl std::fmt::Display) -> TrustedWorkflowFailure {
    TrustedWorkflowFailure::Journal { directory: directory.to_path_buf(), detail: error.to_string() }
}
fn hex(bytes: &[u8]) -> String { bytes.iter().map(|b| format!("{b:02x}")).collect() }
fn root_hex(root: Commitment) -> String { hex(root.digest().bytes().as_bytes()) }
...
fn scope_json(scope: CheckJournalScope) -> String {
    format!("{{\"tenant\":\"{}\",\"repository\":\"{}\",\"journal_id\":\"{}\"}}",
        hex(scope.tenant.as_bytes()), hex(scope.repository.as_bytes()), root_hex(scope.journal_id))
}
...
fn entry_json(entry: &CheckHistoryEntry) -> String {
    ...
    let profile = match batch.execution_profile() {
        CoordinatorExecutionProfile::CommandOnly => "{\"name\":\"coordinator-command-only-v1\"}".to_owned(),
        CoordinatorExecutionProfile::TrustedWorkflow { source, limits } => format!(
            ...
            root_hex(source), limits.step_timeout.as_nanos(), limits.run_timeout.as_nanos(), limits.stream_bytes, limits.total_output_bytes,
        ),
    };
    ...
    format!(
        ...
        root_hex(batch.id()), batch.body().len(), batch.ordinal(),
        if entry.delivery_receipt().is_some() { "acknowledged" } else { "pending" },
        optional_root(entry.delivery_receipt()), root_hex(batch.run_id().commitment()), root_hex(batch.attempt_id().commitment()),
        root_hex(batch.authority_head()), batch.source_commit().algorithm().as_str(), batch.source_commit(),
        root_hex(batch.graph_commitment()), quoted(batch.trust_domain().name().as_str()),
    )
}
