...
fn marker(report: &TrustedWorkflowRun) {
    let mut file = fs::OpenOptions::new()
        ...
        .create_new(true)
        .mode(0o600)
        .open(report.run_directory.join("attempt.json"))
        .unwrap();
    file.write_all(report.attempt_marker().as_bytes()).unwrap();
    file.sync_all().unwrap();
    File::open(&report.run_directory)
        .unwrap()
        .sync_all()
        .unwrap();
}
...
fn complete_custody_reopens_without_final_report_source_or_live_node() {
    ...
    let pin = journal.pin();
    let first = journal.next_batch().unwrap().unwrap();
    assert_eq!(journal.pending_batches(), 3);
    drop(journal);
    drop(report);
    let mut journal =
        OneNode::open_trusted_workflow_journal(&temp.0, expected, Some(pin), &|| true).unwrap();
...
fn producer_lock_excludes_recovery_and_partial_queue_is_not_execution_success() {
    ...
    let expected = report.check_journal_scope();
    let journal =
        FileCheckJournal::create(&temp.0.join(JOURNAL_FILE), expected, Default::default()).unwrap();
...
fn missing_marker_missing_journal_and_bad_scope_never_recreate_files() {
    ...
    let mut wrong = scope;
    wrong.repository = fgit_types::RepositoryId::from_bytes([0xff; 16]);
    assert!(OneNode::open_trusted_workflow_journal(&temp.0, wrong, None, &|| true).is_err());
    wrong = scope;
    wrong.journal_id = Commitment::of_bytes(b"different attempt");
    assert!(OneNode::open_trusted_workflow_journal(&temp.0, wrong, None, &|| true).is_err());
...
fn marker_corruption_symlinks_and_excess_size_refuse_before_journal_access() {
    ...
    marker(&report);
    let expected = report.check_journal_scope();
    let path = temp.0.join("attempt.json");
...
fn trusted_checkpoint_detects_rollback_and_cancelled_recovery_does_no_io() {
    ...
    let scope = report.check_journal_scope();
    let mut journal =
        FileCheckJournal::create(&temp.0.join(JOURNAL_FILE), scope, Default::default()).unwrap();
    let prior = fs::read(temp.0.join(JOURNAL_FILE)).unwrap();
    journal
        .store_evidence(Commitment::of_bytes(b"kept"), b"kept")
        .unwrap();
    let pin = journal.pin();
    drop(journal);
