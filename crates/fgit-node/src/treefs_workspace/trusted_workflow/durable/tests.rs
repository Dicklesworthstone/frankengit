...
use fgit_runner::coordinator::delivery::journal::attempt::WorkflowAttemptStatus;
...
use fgit_types::{
    CANONICAL_CODEC_VERSION, DecisionOutcome, GitHashAlgorithm, GitOid, GitOidSha1, PrincipalId,
    RefName, RepositoryAuthorityHeadId, RepositoryCommitId, RepositoryId, RepositoryIncarnationId,
    TenantId,
};
...
pub(super) fn report(temp: &Temp) -> TrustedWorkflowRun {
    ...
    TrustedWorkflowRun {
        ...
        source_reference: b"refs/heads/main".to_vec(),
        candidate: None,
        merge: None,
        workflow_blob: GitOid::Sha1(GitOidSha1::from_bytes([6; 20])),
        workflow_path: b"workflow.yml".to_vec(),
        read_prefixes: vec![b"workflow.yml".to_vec()],
        run_id: [7; 16],
        run_directory: temp.0.clone(),
...
impl WorkflowExecutor for Executor {
    fn begin_job(
        ...
        let bytes = fs::metadata(self.directory.join(JOURNAL_FILE))
            .unwrap()
            .len();
        assert!(bytes > 72);
        assert!(fs::metadata(self.directory.join(OWNER_FILE)).unwrap().len() > 168);
        if let Some(previous) = self.observed_lengths.last() {
...
fn node_adapter_journals_each_job_before_releasing_its_dependent() {
    ...
    assert_eq!(executor.started, ["first", "next"]);
    assert_eq!(executor.closed, 2);
    let mut owner =
        FileWorkflowAttempt::open(&temp.0.join(OWNER_FILE), binding, None, &|| true).unwrap();
    ...
    let mut sink = DiskSink(temp.0.clone());
    let mut count = 0;
    ...
    while let Some(batch) = journal.next_batch().unwrap() {
        ...
        journal.forward_next(&mut sink, &|| true).unwrap().unwrap();
        count += 1;
    }
    assert_eq!((count, completed_jobs), (3, 2));
    drop(journal);
    drop(owner);
    let journal = FileCheckJournal::open(
        &temp.0.join(JOURNAL_FILE),
        scope,
        Default::default(),
        Some(receipt.journal_pin()),
...
fn persistence_failure_after_a_closed_job_fences_all_later_scopes() {
    let temp = Temp::new();
    let prepared = prepare(&report(&temp));
    let binding = prepared.binding;
    let mut executor = Executor::new(&temp);
    executor.corrupt_after_close = true;
    assert!(matches!(
        prepared.execute(&temp.0, &mut executor, &|| true),
        Err(TrustedWorkflowFailure::Journal { .. })
    ));
...
fn cancelled_intake_and_preexisting_owner_never_open_a_scope() {
    ...
    let prepared = prepare(&report(&temp));
    fs::write(
        temp.0.join(OWNER_FILE),
        b"existing ownership is not adopted",
    )
    .unwrap();
    let mut executor = Executor::new(&temp);
    ...
    assert!(executor.started.is_empty());
    assert_eq!(
        fs::read(temp.0.join(OWNER_FILE)).unwrap(),
        b"existing ownership is not adopted"
    );
}
...
fn containment_observations_survive_completion_and_reopen() {
    ...
    assert!(
        completed
            .jobs
            .iter()
            .any(fgit_runner::workflow::JobReport::requires_containment)
    );
    let mut owner =
        FileWorkflowAttempt::open(&temp.0.join(OWNER_FILE), binding, None, &|| true).unwrap();
    assert!(
...
fn identity_preserves_the_whole_nonce_input_scope_and_exact_time_limits() {
    ...
    let other = prepare(&report);
    assert_ne!(original.scope, other.scope);
    assert_ne!(original.binding, other.binding);
    report.read_prefixes.pop();
    report.incarnation = RepositoryIncarnationId::from_bytes([0xff; 16]);
...
fn marker_identity_is_stable_after_jobs_complete_but_mismatched_plans_refuse() {
    let temp = Temp::new();
    let mut report = report(&temp);
    let before = scope(&report);
    let completed = prepare(&report)
        .execute(&temp.0, &mut Executor::new(&temp), &|| true)
        .unwrap();
    report.execution = completed;
    report.workspaces_closed = true;
    assert_eq!(before, scope(&report));
    assert!(Prepared::new(&report, WorkflowPlan::compile(SOURCE).unwrap()).is_err());
    report.execution.jobs.clear();
    report.execution.source = Commitment::of_bytes(b"different source");
    assert!(Prepared::new(&report, WorkflowPlan::compile(SOURCE).unwrap()).is_err());
}
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let imported = node
            ...
            .block_on(node.import_loose_git_directory_durable_in(
                ...
                &source,
                PrincipalId::from_bytes([0x43; 16]),
                b"workflow-custody-fixture",
            ))
            .unwrap();
        assert!(
            imported.commands.iter().all(|command| matches!(
        ...
        assert_eq!(
            report.execution.jobs[0].steps[0].observation.stdout,
            b"first"
        );
        assert_eq!(
            report.execution.jobs[0].steps[1].observation.stdout,
            b"second"
        );
        assert_eq!(
            report.execution.jobs[1].steps[0].observation.stdout,
            b"dependent"
        );
        ...
        let retry = reopened
            .runtime()
            .block_on(reopened.run_trusted_workflow_in(
        ...
        assert!(matches!(
            retry,
            Err(TrustedWorkflowFailure::AttemptExists(_))
        ));
        assert_eq!(generation(&reopened), before);
        reopened.shutdown().unwrap();
        assert_eq!(
            fs::read(report.run_directory.join(OWNER_FILE)).unwrap(),
            owner_bytes
        );
        assert_eq!(
            fs::read(report.run_directory.join(JOURNAL_FILE)).unwrap(),
            journal_bytes
        );
        let jobs = std::mem::take(&mut report.execution.jobs);
        let prepared = prepare(&report);
        report.execution.jobs = jobs;
        let mut owner = FileWorkflowAttempt::open(
            &report.run_directory.join(OWNER_FILE),
            prepared.binding,
            None,
            &|| true,
        )
        ...
        let receipt = owner.completed_receipt().unwrap().unwrap();
        let mut journal = FileCheckJournal::open(
            &report.run_directory.join(JOURNAL_FILE),
            prepared.scope,
            Default::default(),
            Some(receipt.journal_pin()),
            &|| true,
        ...
        let mut sink = DiskSink(destination.0.clone());
        let mut outputs = Vec::new();
        while let Some(batch) = journal.next_batch().unwrap() {
            for fact in batch.facts() {
                assert_ne!(fact.conclusion, Some(CheckRunConclusion::Success));
                if let Some(id) = fact.receipt_commitment {
                    outputs.push(journal.read_evidence(id).unwrap());
                }
            }
        ...
        assert!(
            outputs
                .iter()
                .any(|bytes| bytes.windows(10).any(|part| part == b"6669727374"))
        );
        assert_eq!(
            fs::read(report.run_directory.join("report.json")).unwrap(),
            saved
        );
    }
}
