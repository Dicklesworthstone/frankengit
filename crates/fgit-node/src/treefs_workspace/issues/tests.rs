...
fn apply(
    ...
    node.runtime().block_on(node.admit_issue_durable_in(
        &request,
        &session(key),
        command,
...
fn history(
    ...
    number: u64,
    after: u64,
    pin: Option<RepositoryAuthorityHeadId>,
) -> Result<issues::IssueHistoryPage, IssueReadRefusal> {
    let request = node.request_context();
    node.runtime().block_on(node.read_issue_history_in(
        &request,
...
fn durable_issue_lifecycle_rebuilds_after_reopen_and_keeps_exact_historical_results() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let edited = change(
            7,
            1,
            IssueAction::Edit(IssueEdit {
                title: Some("Edited é".into()),
                labels: Some(vec![]),
                ..Default::default()
            }),
        );
        ...
        no_code_changes(&before, &after);
        assert_eq!(
            after.snapshot().outbox.len(),
            before.snapshot().outbox.len() + 5
        );
        assert_ne!(
        ...
        assert_eq!(issue.opened_by, actor());
        assert_eq!(issue.title, "Edited é");
        assert_eq!(issue.body, "text é\r\nwithout terminal newline");
        assert_eq!(page.next_after, Some(2));
        let next = history(&node, 7, 2, Some(page.source_head)).unwrap();
        assert_eq!(
            next.events[0].payload,
            comment.proposed_event(actor()).unwrap().payload
        );
        let final_page = history(&node, 7, 4, Some(page.source_head)).unwrap();
...
fn genuinely_overlapping_issue_edits_publish_one_winner_and_one_stale_decision() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let scratch = Scratch::new();
        let node = node(&scratch, format);
        accepted(apply(&node, &open(1), "open"));
        let a = change(
            1,
            1,
            IssueAction::Edit(IssueEdit {
        ...
        let mut fa =
            Box::pin(node.admit_issue_durable_in(&ra, &sa, &a, AdmissionLimits::default()));
        let mut fb =
            Box::pin(node.admit_issue_durable_in(&rb, &sb, &b, AdmissionLimits::default()));
        let (mut left, mut right, mut overlap) = (None, None, false);
        let (a_result, b_result) = node.runtime().block_on(std::future::poll_fn(|cx| {
            if left.is_none()
                && let Poll::Ready(value) = fa.as_mut().poll(cx)
            {
                left = Some(value);
            }
            if right.is_none()
                && let Poll::Ready(value) = fb.as_mut().poll(cx)
            {
                right = Some(value);
            }
        ...
        assert!(
            overlap,
            "both real authority operations must be pending together"
        );
        let outcomes = [&a_result.1.outcome, &b_result.1.outcome];
        assert_eq!(
            outcomes
                .iter()
                .filter(|outcome| matches!(outcome, DecisionOutcome::Committed { .. }))
                .count(),
            1
        );
        assert_eq!(
        ...
        assert_eq!(apply(&node, &a, "a").unwrap(), a_result);
        assert_eq!(apply(&node, &b, "b").unwrap(), b_result);
...
fn missing_wrong_state_changed_key_and_cancelled_commands_do_not_fabricate_success() {
    let scratch = Scratch::new();
    let mut node = node(&scratch, GitHashAlgorithm::Sha1);
    ...
    assert!(apply(&node, &open(2), "open").is_err());
    assert_eq!(snapshot(&node).basis(), before.basis());
...
fn issue_and_timeline_pagination_are_numeric_snapshot_pinned_and_read_only() {
    ...
    for number in [10, 2, 1] {
        accepted(apply(&node, &open(number), &format!("open-{number}")));
    }
    let request = node.request_context();
    let first = node
        .runtime()
        .block_on(node.read_issues_in(&request, 0, 2, None))
        .unwrap();
    assert_eq!(
        first
            .issues
    ...
    assert!(matches!(
        node.runtime()
            .block_on(node.read_issues_in(&request, 2, 2, None)),
        Err(IssueReadRefusal::SnapshotRequired)
    ));
    let second = node
        .runtime()
        .block_on(node.read_issues_in(&request, 2, 2, Some(first.source_head)))
        .unwrap();
    assert_eq!(second.issues[0].number.get(), 10);
    assert!(second.next_after.is_none());
    assert!(history(&node, 99, 0, None).unwrap().issue.is_none());
    assert_eq!(snapshot(&node).basis(), before.basis());
    ...
    assert_eq!(
        history(&node, 1, 0, None).unwrap().issue.unwrap().state,
        IssueState::Closed
    );
    assert_eq!(snapshot(&node).basis(), after.basis());
    node.shutdown().unwrap();
