...
fn fixture(scratch: &Scratch, format: GitHashAlgorithm) -> (OneNode, Fixture) {
    ...
    let target_tree = put(
        GitObjectKind::Tree,
        "tree",
        &tree(&[(b"keep", 0o100644, keep), (b"selected", 0o100755, ours)]),
    );
    let target = put(
        GitObjectKind::Commit,
        "commit",
    ...
    (
        ...
        Fixture {
            input: ReplayRequest {
                ...
                target,
                source_tip: source,
                selected_commit: picked,
                mainline: None,
            },
            borrowed,
            source_tree: picked_tree,
            keep_tree: base_tree,
        },
    )
}
fn choice(value: ResolutionChoice) -> ConflictResolution {
    ConflictResolution {
...
fn manual_pick_and_conflicted_revert_pass_inspection_publication_and_reopen_in_both_formats() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (node, f) = fixture(&scratch, format);
        let request = node.request_context();
        let before = node
            .runtime()
        ...
        let choices = [choice(ResolutionChoice::File {
            mode: 0o100755,
            bytes: bytes.to_vec(),
        })];
        let resolved = node
        ...
        let ReplayPreparation::Clean(plan) = &artifact.outcome else {
            panic!("resolved candidate");
        };
        ...
        let inspected = node
            ...
            .unwrap();
        assert_eq!(inspected.parents, vec![f.input.target]);
        ...
        let inverse = node
            ...
            .unwrap();
        let ReplayPreparation::Clean(revert) = &inverse.artifact.outcome else {
            panic!("inverse resolution");
        };
        ...
        let reverted = node
            ...
            .block_on(node.apply_workspace_bundle_durable_in(
                ...
                plan.commit,
                revert.commit,
                inverse.artifact.bundle.as_ref().unwrap(),
            ))
            .unwrap();
        assert!(matches!(
            reverted.commands[0].terminal.outcome,
            DecisionOutcome::Committed { .. }
        ));
        ...
        assert_eq!(
            reopened
                .runtime()
                .block_on(reopened.materialize_admission_in(&context))
                .unwrap()
                .snapshot()
                .refs[&main_ref()],
            final_commit
...
fn side_resolution_packs_source_only_dependencies_and_never_requires_later_source_history() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let resolved = node
            ...
            .unwrap();
        let ReplayPreparation::Clean(plan) = &resolved.artifact.outcome else {
            panic!("side candidate");
        };
        assert_eq!(plan.tree, f.source_tree);
        assert!(!plan.objects.iter().any(|object| object.id == f.borrowed));
        assert_eq!(
            resolved.artifact.borrowed_objects, 2,
            "source-only tree and blob must be packed"
        );
        let inspected = node
            .runtime()
            .block_on(node.inspect_workspace_bundle_in(
                ...
                f.input.target,
                plan.commit,
                resolved.artifact.bundle.as_ref().unwrap(),
                &Default::default(),
                None,
                &ReviewOptions {
                    mode: ComparisonMode::Direct,
...
fn no_change_resolution_retains_receipts_but_never_builds_an_empty_commit() {
    let scratch = Scratch::new();
    let (node, f) = fixture(&scratch, GitHashAlgorithm::Sha1);
    let request = node.request_context();
    let before = node
        .runtime()
        .block_on(node.materialize_admission_in(&request))
    ...
    let resolved = node
        ...
        .unwrap();
    assert!(matches!(
        resolved.artifact.outcome,
        ReplayPreparation::NoChange { .. }
    ));
    assert_eq!(resolved.resolutions.len(), 1);
    assert!(resolved.artifact.bundle.is_none());
    assert_eq!(resolved.artifact.pack_objects, 0);
    assert_eq!(resolved.artifact.borrowed_objects, 0);
    assert_eq!(
        node.runtime()
            .block_on(node.materialize_admission_in(&request))
            .unwrap()
...
fn invalid_incomplete_hidden_stale_and_over_budget_resolution_cannot_mutate_state() {
    let scratch = Scratch::new();
    let (node, f) = fixture(&scratch, GitHashAlgorithm::Sha256);
    let request = node.request_context();
    let before = node
        .runtime()
        .block_on(node.materialize_admission_in(&request))
    ...
    for choices in [
        vec![],
        vec![choice(ResolutionChoice::Base)],
        vec![
            choice(ResolutionChoice::Ours),
            choice(ResolutionChoice::Theirs),
        ],
    ...
    assert!(matches!(
        ...
        Err(ReplayPreparationRefusal::RefUnavailable)
    ));
    let stale = ReplayRequest {
        target: f.input.selected_commit,
        ..f.input
    };
    assert!(matches!(
        node.runtime()
        ...
        Err(ReplayPreparationRefusal::TipMoved)
    ));
    let limited = PreparationLimits {
        max_tree_entries: 5,
        ..PreparationLimits::default()
    };
    assert!(
