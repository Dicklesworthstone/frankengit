...
fn borrowed_source_objects_are_in_the_target_only_bundle_and_replay_round_trips() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let inspected = node
            ...
            .unwrap();
        assert_eq!(inspected.parents, vec![f.target]);
        assert_eq!(inspected.prerequisites, vec![f.target]);
        ...
        let reverted = node
            ...
            .unwrap();
        let ReplayPreparation::Clean(revert) = &reverted.outcome else {
            panic!("inverse candidate required");
        };
...
fn conflict_staleness_visibility_history_and_byte_budgets_never_mutate_the_node() {
    ...
    hidden
        ...
        .unwrap();
    assert!(matches!(
        node.runtime().block_on(node.prepare_replay_bundle_in(
            &request,
            &main_ref(),
            &topic_ref(),
            inputs(&f),
    ...
    let unrelated = ReplayRequest {
        selected_commit: f.target,
        ..inputs(&f)
    };
    assert!(
        matches!(node.runtime().block_on(node.prepare_replay_bundle_in(&request, &main_ref(), &topic_ref(),
...
fn snapshot_pin_rejects_later_authority_movement_even_when_selected_commit_remains_reachable() {
    let scratch = Scratch::new();
    let (node, f) = fixture(&scratch, GitHashAlgorithm::Sha256, false);
    let request = node.request_context();
    let before = node
        .runtime()
        .block_on(node.materialize_admission_in(&request))
    ...
    node.runtime()
        .block_on(node.apply_workspace_bundle_durable_in(
            ...
            f.target,
            plan.commit,
            &artifact.bundle.unwrap(),
        ))
        .unwrap();
    let updated = ReplayRequest {
        target: plan.commit,
        ..inputs(&f)
    };
    assert!(matches!(
    ...
    assert!(no_change.bundle.is_none());
    node.shutdown().unwrap();
}
