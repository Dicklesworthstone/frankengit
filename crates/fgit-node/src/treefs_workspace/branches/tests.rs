...
fn delete(name: &str, oid: GitOid) -> RefCommand {
    RefCommand {
        name: reference(name),
        expected_old: ExpectedOld::Exactly(oid),
...
fn branch_lifecycle_is_atomic_and_exact_retries_survive_reopen_and_deleted_sources() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let rename = [
            delete("refs/heads/topic", child),
            create("refs/heads/renamed", child),
        ];
        let renamed = accepted(apply(&node, &rename, "rename"));
        assert_eq!(renamed.commands.len(), 2);
        assert_eq!(renamed.commands[0], renamed.commands[1]);
        ...
        assert_eq!(
            after.snapshot().refs.get(&reference("refs/heads/renamed")),
            Some(&child)
        );
        assert_eq!(
            after.snapshot().head_target,
        ...
        let removed = accepted(apply(
            &node,
            &[delete("refs/heads/renamed", child)],
            "delete",
        ));
        let settled = snapshot(&node);
        assert_eq!(
            apply(&node, &[create("refs/heads/topic", base)], "create").unwrap(),
            created
        );
        ...
        assert_eq!(
            apply(&reopened, &[delete("refs/heads/renamed", child)], "delete").unwrap(),
            removed
        );
...
fn rename_cannot_overwrite_an_existing_destination_or_delete_a_changed_source() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        accepted(apply(
            &node,
            &[create("refs/heads/destination", child)],
            "destination",
        ));
        let before = snapshot(&node);
        let conflict = [
            delete("refs/heads/source", base),
            create("refs/heads/destination", base),
        ];
        ...
        let stale = [
            delete("refs/heads/source", child),
        ...
        assert!(
            apply(
                &node,
                &[create("refs/heads/changed-input", child)],
                "source"
            )
            .is_err()
        );
        assert_eq!(snapshot(&node).basis(), head.basis());
        node.shutdown().unwrap();
    }
}
...
fn branch_guards_preserve_authority_and_terminal_retries_precede_quota() {
    let scratch = Scratch::new();
    let (mut node, base, child, blob) = fixture(&scratch, GitHashAlgorithm::Sha1);
    let before = snapshot(&node);
    assert!(matches!(
        apply(&node, &[create("refs/heads/blob", blob)], "blob"),
        Err(NodeWorkspaceRefusal::CommitRequired)
    ));
    ...
    assert!(
        apply(
            &node,
            &[
                delete("refs/heads/main", child),
                create("refs/heads/other", child)
            ],
            "default-rename"
        )
        .is_err()
    ...
    assert!(
        node.runtime()
            .block_on(node.admit_branch_updates_durable_in(
                &request,
                &session("cancel"),
                &[create("refs/heads/cancel", base)],
                AdmissionLimits::default()
            ))
            .is_err()
    );
    assert_eq!(snapshot(&node).basis(), before.basis());
    let permitted = accepted(apply(
        &node,
    ...
    assert_eq!(
        apply(&node, &[create("refs/heads/permitted", base)], "permit").unwrap(),
        permitted
    );
    let head = snapshot(&node);
    assert!(apply(&node, &[create("refs/heads/new", base)], "new").is_err());
    assert_eq!(snapshot(&node).basis(), head.basis());
    node.shutdown().unwrap();
}
...
fn branch_pages_are_byte_sorted_visibility_filtered_and_snapshot_pinned() {
    let scratch = Scratch::new();
    let (node, base, _, _) = fixture(&scratch, GitHashAlgorithm::Sha256);
    for name in ["refs/heads/z", "refs/heads/a"] {
        accepted(apply(&node, &[create(name, base)], name));
    }
    let request = node.request_context();
    let empty = RefVisibility::default();
    let (head, first, next) = node
        .runtime()
        .block_on(node.list_branch_refs_in(&request, &empty, None, 1, None))
        .unwrap();
    assert_eq!(first[0].0, reference("refs/heads/a"));
    assert_eq!(next, Some(reference("refs/heads/a")));
    assert!(
        node.runtime()
            .block_on(node.list_branch_refs_in(&request, &empty, next.as_ref(), 1, None))
            .is_err()
    );
    let (_, second, next) = node
        .runtime()
        .block_on(node.list_branch_refs_in(&request, &empty, next.as_ref(), 2, Some(head)))
        .unwrap();
    assert_eq!(
        second.iter().map(|(r, _)| r.clone()).collect::<Vec<_>>(),
        vec![reference("refs/heads/main"), reference("refs/heads/z")]
    );
    assert!(next.is_none());
    ...
    let (_, rows, _) = node
        .runtime()
        .block_on(node.list_branch_refs_in(&request, &hidden, None, 100, Some(head)))
        .unwrap();
    assert_eq!(rows.len(), 2);
    assert!(
        rows.iter()
            .all(|(name, _)| name != &reference("refs/heads/main"))
    );
    accepted(apply(&node, &[create("refs/heads/new", base)], "new"));
    assert!(
        node.runtime()
            .block_on(node.list_branch_refs_in(
                &request,
                &empty,
                Some(&reference("refs/heads/a")),
                1,
                Some(head)
            ))
            .is_err()
...
fn malformed_branch_requests_cannot_weaken_expected_old_or_atomic_rename() {
    ...
    let good = create("refs/heads/a", oid);
    assert!(branch_request(GitHashAlgorithm::Sha1, &[good.clone()]).is_ok());
    let mut unsafe_command = good.clone();
    unsafe_command.expected_old = ExpectedOld::Unspecified;
    assert!(branch_request(GitHashAlgorithm::Sha1, &[unsafe_command]).is_err());
    let mut force = good.clone();
    force.force = true;
    assert!(branch_request(GitHashAlgorithm::Sha1, &[force]).is_err());
    assert!(branch_request(GitHashAlgorithm::Sha1, &[create("refs/tags/a", oid)]).is_err());
    assert!(
        branch_request(
            GitHashAlgorithm::Sha1,
            &[good.clone(), create("refs/heads/b", oid)]
        )
        .is_err()
    );
    ...
    assert_eq!(
        branch_request(GitHashAlgorithm::Sha1, &rename).unwrap(),
        branch_request(
            GitHashAlgorithm::Sha1,
            &[rename[1].clone(), rename[0].clone()]
        )
        .unwrap()
    );
}
...
fn non_fast_forward_branch_update_is_refused_and_fast_forward_is_admitted() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        accepted(apply(
            &node,
            &[create("refs/heads/ff-test", base)],
            "create-base",
        ));
        ...
        assert!(matches!(
            err,
            NodeWorkspaceRefusal::BranchOperation(
                "non-fast-forward branch update is not permitted"
            )
        ));
