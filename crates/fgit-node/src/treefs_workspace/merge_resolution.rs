...
use super::{
    Cell, MergeMetadata, MergeObjectSource, NodeRequestContext, NodeWorkspaceRefusal, OneNode,
    ParseLimits, PreparationLimits, RefName, RefVisibility, RepositoryAuthorityHeadId,
    SelectedSource, VerifiedFabricPackSource, bundle_for_plan,
};
use fgit_forge::preparation::resolution::{
    ConflictResolution, ResolutionError, ResolutionInputs, ResolvedMerge, prepare_resolved_merge,
...
impl OneNode {
    ...
    pub async fn prepare_resolved_pull_request_bundle_in(
        ...
        let inputs = ResolutionInputs {
            base: merge_base,
            target: subject.target_tip,
            source: subject.source_tip,
        };
        ...
        let head = self
            ...
            .await?;
        self.prepare_resolved_merge_bundle_in(
            request,
            &subject.target_ref,
            &subject.source_ref,
            visibility,
    ...
    pub async fn prepare_resolved_merge_bundle_in(
            ...
            || !incoming.as_bytes().starts_with(b"refs/heads/")
        {
            return Err(NodeWorkspaceRefusal::InvalidWorkspaceCandidate(
                "resolution requires distinct branches",
            )
            .into());
        }
        ...
        let resolved = prepare_resolved_merge(
            &source,
            self.object_format,
            inputs,
            resolutions,
...
mod tests {
    ...
    use fgit_forge::preparation::{ConflictKind, MergePreparation};
    ...
    fn inputs(node: &OneNode, request: &NodeRequestContext) -> ResolutionInputs {
        ...
        let auto = node
            ...
            .unwrap();
        let MergePreparation::Conflicted { base, conflicts } = auto.outcome else {
            panic!("fixture must conflict");
        };
        assert_eq!(conflicts.len(), 1);
        assert_eq!(conflicts[0].path, b"text");
        assert_eq!(conflicts[0].kind, ConflictKind::Content);
        assert!(auto.bundle.is_none());
        ResolutionInputs {
            base,
            target: source.snapshot().refs[&target_ref],
            source: source.snapshot().refs[&source_ref],
        }
    }
    ...
    fn resolved_candidates_inspect_and_publish_through_the_existing_path_in_both_formats() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            ...
            assert!(
                ...
                "preparation must not stage candidates"
            );
            let merge = NativeMerge {
                source_ref: source_ref.clone(),
                source_tip: coordinates.source,
                base_tip: coordinates.base,
                target_ref: target_ref.clone(),
                target_tip_before: coordinates.target,
            ...
            assert_eq!(
                inspected.parents,
                vec![coordinates.target, coordinates.source]
            );
            ...
            assert_eq!(
                inspected.review.comparison.entries[0].after.unwrap().mode,
                0o100755
            );
            assert_eq!(
                node.runtime()
            ...
            assert!(matches!(
                ...
                Err(NodeResolutionRefusal::SnapshotMoved)
            ));
    ...
    fn unresolved_extra_hidden_and_stale_inputs_never_change_authority() {
        ...
        hidden
            .push_rule(source_ref.as_bytes(), &Default::default())
            .unwrap();
        assert!(
            node.runtime()
                .block_on(node.prepare_resolved_merge_bundle_in(
        ...
        let wrong = ResolutionInputs {
            base: coordinates.target,
            ..coordinates
        };
        assert!(
            node.runtime()
