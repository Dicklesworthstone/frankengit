...
fn exact_ancestors_are_selected_without_changing_authority_in_both_formats() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            basis(&node),
            current,
            "historical reads never replace the authority head"
        );
...
fn transition_budget_is_exact_and_unknown_tokens_do_not_become_current_reads() {
    ...
    assert!(matches!(
        node.runtime().block_on(select_bounded(
            &node.authority,
            request.authority(),
            &current,
            Some(genesis.id()),
        ...
        Err(SnapshotReadRefusal::Unavailable)
    ));
    assert_eq!(
        node.runtime()
            .block_on(select_bounded(
    ...
    let unknown = RepositoryAuthorityHeadId::from_digest(
        DigestAlgorithmId::try_new(1).unwrap(),
        CANONICAL_CODEC_VERSION,
        DigestBytes::try_new(&[0xf3; 32]).unwrap(),
    );
    assert!(matches!(
        node.runtime().block_on(select(
            &node.authority,
            request.authority(),
            &current,
            Some(unknown),
...
fn cancellation_before_or_during_ancestry_does_not_return_a_partial_basis() {
    ...
    for stop_at in [0, 2, 4] {
        ...
        let request = node.request_context();
        let result = node.runtime().block_on(select(
            &node.authority,
            request.authority(),
            &current,
            Some(genesis.id()),
...
fn invalid_transition_is_verified_instead_of_followed_as_an_ancestry_hint() {
    ...
    assert!(matches!(
        ...
        Err(SnapshotReadRefusal::Admission(_))
    ));
