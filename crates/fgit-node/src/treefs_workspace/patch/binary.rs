...
use fgit_forge::patch::{
    FilePatch, IndexExpectation, PatchError, PatchLimits, PatchedFile, UnifiedPatch,
};
...
fn identity(bytes: &[u8], format: GitHashAlgorithm) -> Result<Option<GitOid>, PatchError> {
    ...
    {
        return Err(invalid(
            "binary index must use the repository's full native hash width",
        ));
    }
    ...
    GitOid::from_hex(format, text)
        .map(Some)
        .map_err(|_| invalid("invalid binary index"))
}
fn identities(
    index: &IndexExpectation,
    ...
    pub(super) fn new(
        ...
        for file in patch.files() {
            if file.binary_hunks().is_some() {
                let index = file
                    .index()
                    .ok_or_else(|| invalid("binary index required"))?;
    ...
    pub(super) fn apply(
        ...
        file.apply_with_binary_decoder(source, limits, cancelled, |bytes, index, original| {
            ...
            let batch = self
                .batch
                .as_mut()
                .ok_or_else(|| invalid("binary budget was not reserved"))?;
            batch
                .apply(bytes, old, new, original, &mut || !cancelled())
                .map_err(refusal)
        })
    }
}
...
mod tests {
    ...
    fn native_width_and_absent_identity_are_not_prefix_comparisons() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            ...
            assert_eq!(
                identity("0".repeat(width).as_bytes(), format).unwrap(),
                None
            );
            assert!(
                identity("a".repeat(width).as_bytes(), format)
                    .unwrap()
                    .is_some()
            );
            for text in [
    ...
    fn native_decode_refusals_remain_read_errors_not_publication_outcomes() {
        ...
        assert!(matches!(
            refusal(BinaryPatchError::Limit("private data")),
            PatchError::Budget(_)
        ));
        for error in [
            BinaryPatchError::SourceMismatch,
            BinaryPatchError::TargetMismatch,
            BinaryPatchError::ReverseMismatch,
            BinaryPatchError::Invalid("private data"),
        ] {
