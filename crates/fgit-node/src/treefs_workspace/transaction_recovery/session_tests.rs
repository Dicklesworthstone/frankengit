...
use fgit_authority::{
    AsyncAuthorityStore, ExpectedOld, IdempotencyKey, ImmutableKey, OutcomeLookup, ProposedNew,
    RefCommand, SealAttempt, SemanticRequest, bind_idempotency_key_async, seal_request_async,
};
...
fn query(node: &OneNode, key: &[u8]) -> SessionRecovery {
    node.runtime
        .block_on(node.recover_receive_session_in(&node.request_context(), &session(key)))
        .unwrap()
}
...
fn descriptor(request: &SemanticRequest, order: &[u16]) -> Vec<u8> {
    ...
    encoder
        .write_bytes("receive-session.request", &encode_body(request).unwrap())
        .unwrap();
    encoder
        .write_sequence("receive-session.wire-order", order, |encoder, index| {
            encoder.write_scalar(*index);
            Ok(())
        })
        .unwrap();
    encoder.into_bytes()
...
fn bind_shape(node: &OneNode, context: &AdmissionContext) -> Vec<SealAttempt> {
    ...
    node.runtime
        .block_on(bind_idempotency_key_async(
            &node.authority,
            cx.authority(),
            &full,
            full.derive().unwrap().0,
        ))
        .unwrap();
    let mut children = Vec::new();
    for (index, name) in [b"refs/tags/z".as_slice(), b"refs/tags/a"]
        .into_iter()
        .enumerate()
    {
        let child = attempt(
            context,
            OneNode::receive_command_recovery_key(&context.idempotency_key, index).unwrap(),
...
fn push(node: &OneNode, key: &[u8]) -> Result<AdmissionResult, NodeSmartHttpRefusal> {
    ...
    let mut body = encode_packets(
        &[
            Packet::Data(
                format!(
                    "{oid} {oid} refs/tags/z-stale\0report-status object-format={}",
                    format.as_str()
                )
                .into_bytes(),
            ),
            Packet::Data(format!("{zero} {oid} refs/tags/a-good").into_bytes()),
            Packet::Data(format!("{zero} {oid} refs/tags/m-good").into_bytes()),
            Packet::Flush,
        ],
        &WireLimits::default(),
    )
...
fn complete_mixed_session_recovers_original_order_after_reopening_without_serving() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(matches!(
            original.commands[0].terminal.outcome,
            DecisionOutcome::Refused { .. }
        ));
        assert!(
            original.commands[1..].iter().all(|command| matches!(
        ...
        assert_eq!(
            generation(&node),
            before + 3,
            "descriptor staging publishes no extra decision"
        );
        node.shutdown().unwrap();
        let node = OneNode::open_existing(configuration).unwrap();
        let recovered = query(&node, b"original-session");
        let SessionRecovery::Recovered(receipt) = &recovered else {
            panic!("native receive must retain its descriptor")
        };
        assert!(receipt.all_terminal());
        assert_eq!(receipt.commands().len(), 3);
        for (index, name) in ["refs/tags/z-stale", "refs/tags/a-good", "refs/tags/m-good"]
            .into_iter()
            .enumerate()
        {
            ...
            let RequestRecovery::Recovered(known) = command.recovery() else {
                panic!("all commands must be sealed")
            };
            assert_eq!(known.tx_id(), original.commands[index].tx_id);
            assert_eq!(
                known.outcome(),
                OutcomeLookup::Decided(original.commands[index].terminal)
            );
        }
        assert_eq!(query(&node, b"original-session"), recovered);
        assert_eq!(generation(&node), before + 3);
        let foreign = LoopbackReceiveSession::authenticated(
            PrincipalId::from_bytes([0xb4; 16]),
            IdempotencyKey::new(b"original-session".to_vec()).unwrap(),
        );
        assert_eq!(
            node.runtime
                .block_on(node.recover_receive_session_in(&node.request_context(), &foreign))
                .unwrap(),
...
fn verified_unsealed_shape_and_undecided_child_do_not_become_empty_or_complete() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        node.runtime
            ...
            .unwrap();
        let before = generation(&node);
        let SessionRecovery::Recovered(receipt) = query(&node, b"interrupted-before-seal") else {
            panic!("verified shape must be retained")
        };
        ...
        assert!(
            receipt
                .commands()
                .iter()
                .all(|command| matches!(command.recovery(), RequestRecovery::SealNotObserved))
        );
        node.runtime
            .block_on(seal_request_async(
                &node.authority,
                cx.authority(),
                &children[0],
            ))
            .unwrap();
        let SessionRecovery::Recovered(sealed) = query(&node, b"interrupted-before-seal") else {
            panic!("shape remains recoverable")
        };
        assert!(!sealed.all_terminal());
        assert!(
            matches!(sealed.commands()[0].recovery(), RequestRecovery::Recovered(known) if known.outcome() == OutcomeLookup::Undecided)
        );
        assert!(matches!(
            sealed.commands()[1].recovery(),
...
fn validly_encoded_omission_reordering_and_format_substitution_fail_binding_verification() {
    for variant in 0..4 {
        ...
        let bytes = match variant {
            ...
            1 => descriptor(
                &request(GitHashAlgorithm::Sha1, &[b"refs/tags/z", b"refs/tags/a"]),
                &[0, 1],
            ),
            2 => descriptor(
                &request(GitHashAlgorithm::Sha256, &[b"refs/tags/z", b"refs/tags/a"]),
                &[1, 0],
            ),
        ...
        assert!(
            node.runtime
                .block_on(node.recover_receive_session_in(
                    ...
                    &session(b"do-not-trust-the-carrier")
                ))
                .is_err(),
            "variant {variant}"
        );
...
fn missing_descriptor_and_cancellation_do_not_create_metadata_or_terminal_knowledge() {
    ...
    assert!(
        node.runtime
            .block_on(node.recover_receive_session_in(&request, &session(b"never-submitted")))
            .is_err()
    );
    assert_eq!(generation(&node), before);
    node.shutdown().unwrap();
