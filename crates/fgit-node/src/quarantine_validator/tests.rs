    ...
    for synchronous in synchronous_refusals {
        ...
        match asynchronous {
            NodeReceiveTransportRefusal::Admission(admission) => match admission.as_ref() {
                ...
                other => {
                    panic!("the asynchronous transport must preserve ReceiveError, got {other:?}")
                }
            },
            NodeReceiveTransportRefusal::Unauthenticated => {
                panic!("a receive-core refusal must not be confused with missing authentication")
            }
            ...
            NodeReceiveTransportRefusal::ReceiveProcessingDeadlineExceeded { timeout, source } => {
                panic!(
...
fn object_bearing_pack_is_verified_staged_and_reported_as_its_exact_closure() {
    ...
    let mut handoff =
        ProductionReceiveQuarantineHandoff::new(handoff_validator, materialized.basis().clone());
...
fn graph_rooted_closure_stages_only_reachable_uploaded_objects() {
    ...
    .into_bytes();
    let tag_id = fgit_crypto::git_object_id(GitHashAlgorithm::Sha1, GitObjectKind::Tag, &tag_body);
    ...
    let source = SelectedObjectsSource {
        objects: BTreeMap::from([
            (
                blob_id,
                CanonicalPackObject::new(blob_id, ObjectType::Blob, blob_body, Vec::new(), 0, 0),
            ),
            (
                tree_id,
                CanonicalPackObject::new(tree_id, ObjectType::Tree, tree_body, Vec::new(), 0, 0),
            ),
            (
                commit_id,
            ...
            (
                junk_id,
                CanonicalPackObject::new(junk_id, ObjectType::Blob, junk_body, Vec::new(), 0, 0),
            ),
        ]),
    };
    let limits = PackLimits::default();
...
fn live_delete_only_receive_returns_the_canonical_empty_closure() {
    ...
    assert_eq!(
        ...
        ProductionQuarantineValidator::empty_closure().expect("empty closure has a canonical root")
    );
    node.shutdown().expect("node shuts down after test");
}
...
fn delta_chain_over_the_selected_budget_is_refused_before_any_fabric_placement() {
    ...
    let source = SelectedObjectsSource {
        objects: BTreeMap::from([
            (
                base_id,
                CanonicalPackObject::new(base_id, ObjectType::Blob, base_body, Vec::new(), 3, 1),
            ),
            (
                target_id,
                CanonicalPackObject::new(
