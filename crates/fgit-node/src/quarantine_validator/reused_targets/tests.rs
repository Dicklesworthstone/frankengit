...
use fgit_wire::receive::{
    ReceiveCommand, ReceiveContext, ReceiveLimits, ReceivePack, SignedPushProfile,
};
...
fn raw(format: GitHashAlgorithm, kind: ObjectType, bytes: Vec<u8>) -> Raw {
    Raw {
        id: git_object_id(format, kind, &bytes),
        kind,
        bytes,
    }
}
...
fn pack(format: GitHashAlgorithm, objects: &[Raw]) -> Vec<u8> {
    ...
    for object in objects {
        ...
        let mut byte = (kind << 4) | u8::try_from(size & 15).unwrap();
        size >>= 4;
        while size != 0 {
            out.push(byte | 128);
            byte = u8::try_from(size & 127).unwrap();
            size >>= 7;
        }
        out.push(byte);
        out.extend(stored(&object.bytes));
    }
    let checksum = match format {
        GitHashAlgorithm::Sha1 => fgit_crypto::sha1_digest(&out).to_vec(),
...
fn request(format: GitHashAlgorithm, roots: &[GitOid]) -> ReceiveRequest {
    ReceiveRequest {
        commands: roots
            .iter()
            .enumerate()
            .map(|(index, id)| ReceiveCommand {
                old: zero(format),
                new: *id,
                ref_name: format!("refs/tags/reused-{index}").into_bytes(),
            })
            .collect(),
...
fn validator<'a>(
    ...
    validator.visible_roots = roots.iter().copied().collect();
    validator
}
fn put(node: &OneNode, objects: &[Raw]) {
    for object in objects {
        assert_eq!(
            node.put_git_object(object.kind, object.bytes.clone())
                .unwrap()
                .identity(),
            object.id
        );
...
fn verified_empty_packs_reuse_visible_tips_without_fabric_placement() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let tip = commit(format, empty.id, &[], "existing");
        let objects = [empty.clone(), tip.clone()];
        put(&node, &objects);
        ...
        assert_eq!(
            admitted.object_closure_root,
            permitted_object_closure_root(&PermittedObjectClosure::new(admitted.objects.clone()))
                .unwrap()
        );
        let mut damaged = pack(format, &[]);
...
fn historical_ancestors_and_nested_tag_targets_are_proven_from_visible_roots() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let node = scratch.node(format);
        let empty = tree(format, &[]);
        let base = commit(format, empty.id, &[], "base");
        let tip = commit(format, empty.id, &[base.id], "tip");
        let inner = tag(format, &tip, "inner");
        let outer = tag(format, &inner, "outer");
        let objects = [
            empty,
            base.clone(),
            tip.clone(),
        ...
        for id in [base.id, tip.id, inner.id, outer.id] {
            assert_eq!(
                validate(&v, &[], &[id]).unwrap().objects,
                BTreeSet::from([id])
            );
        }
        assert_eq!(
            validate(&v, &[], &[base.id, inner.id]).unwrap().objects,
...
fn hidden_only_disconnected_and_merely_present_targets_cannot_be_resurrected() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let node = scratch.node(format);
        let empty = tree(format, &[]);
        let visible = commit(format, empty.id, &[], "visible");
        let hidden = commit(format, empty.id, &[], "hidden-only");
        let stray = commit(format, empty.id, &[], "unselected presence");
        let objects = [empty, visible.clone(), hidden.clone()];
        put(&node, &objects);
        put(&node, &[stray.clone()]);
        ...
        assert_eq!(
            validate(&no_visible, &[], &[visible.id]),
            Err(RefusalCode::ObjectClosureIncomplete)
        );
...
fn mixed_uploaded_and_reused_roots_keep_only_the_requested_witness() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let node = scratch.node(format);
        let empty = tree(format, &[]);
        let old = commit(format, empty.id, &[], "old");
        let originals = [empty.clone(), old.clone()];
        put(&node, &originals);
...
fn missing_selected_bytes_and_bad_reused_roots_fail_before_staging_any_upload() {
    ...
    assert_eq!(
        validate(&v, &[upload.clone()], &[upload.id, absent.id]),
...
fn duplicate_targets_and_uploaded_frontiers_share_one_inclusive_original_byte_budget() {
    ...
    let scratch = Scratch::new();
    let node = scratch.node(format);
    let blob = raw(format, ObjectType::Blob, vec![b'x'; 1024]);
    put(&node, &[blob.clone()]);
    ...
    let bounded = validator(
        ...
        &[blob.id],
        PackLimits {
            max_cached_bytes: 1023,
            max_total_expanded_bytes: 1023,
            ..PackLimits::default()
        },
    );
    assert_eq!(
...
fn visibility_walk_checks_required_kinds_and_cannot_follow_gitlinks() {
    ...
    let mut bytes = b"160000 external\0".to_vec();
    bytes.extend(foreign.id.as_bytes());
...
fn missing_pack_stays_distinct_and_cancellation_precedes_original_reads() {
    ...
    assert_eq!(
        v.validate(&input, Some(&quarantined), &receipt, &mut || false),
        Err(RefusalCode::CancellationInProgress)
    );
    let missing = QuarantineReceipt {
        pack_bytes: 0,
        ..receipt
    };
    assert_eq!(
        fgit_admission::validate_receive(&input, None, &missing, &v, &mut || true),
        Err(RefusalCode::ObjectClosureIncomplete)
    );
    node.shutdown().unwrap();
}
...
fn write_source(root: &Path, format: GitHashAlgorithm, objects: &[Raw], tip: GitOid) {
    ...
    .unwrap();
    for object in objects {
        let text = object.id.to_string();
        let path = root.join("objects").join(&text[..2]);
        std::fs::create_dir_all(&path).unwrap();
        let framed = [
            format!("{} {}\0", object.kind.label(), object.bytes.len()).as_bytes(),
            &object.bytes,
        ]
        .concat();
...
fn wire_proof(
    ...
    receiver
        .push_bytes(&pack(node.object_format, objects))
        .unwrap();
...
fn empty_wire_pack_creates_historical_branch_and_recovers_after_advance_and_reopen() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        write_source(
            &source,
            format,
            &[empty.clone(), base.clone(), tip.clone()],
            tip.id,
        );
        ...
        let session = LoopbackReceiveSession::authenticated(
            principal(),
            IdempotencyKey::new(b"copy-existing".to_vec()).unwrap(),
        );
        let applied = node
            .runtime()
        ...
        let later = LoopbackReceiveSession::authenticated(
            principal(),
            IdempotencyKey::new(b"advance-existing".to_vec()).unwrap(),
        );
        let result = node
            .runtime()
            .block_on(node.admit_basis_bound_loopback_receive_durable_in(
