...
use fgit_types::{
    CANONICAL_CODEC_VERSION, DecisionOutcome, DigestBytes, HeadGeneration, PrincipalId, RefName,
    RepositoryCommitId, RepositoryId, TenantId,
};
...
fn raw(format: GitHashAlgorithm, kind: ObjectType, body: Vec<u8>) -> Raw {
    Raw {
        id: git_object_id(format, kind, &body),
        kind,
        body,
    }
}
...
fn ref_copy(base: &Raw, suffix: &[u8]) -> Vec<u8> {
    ...
    let mut program = vec![
        base.body.len() as u8,
        (base.body.len() + suffix.len()) as u8,
        0x90,
        base.body.len() as u8,
    ];
    if !suffix.is_empty() {
        program.push(suffix.len() as u8);
        program.extend(suffix);
    }
    [
...
fn packed(format: GitHashAlgorithm, entries: &[Vec<u8>]) -> (QuarantinedPack, QuarantineReceipt) {
    ...
    let checksum = match format {
        GitHashAlgorithm::Sha1 => fgit_crypto::sha1_digest(&bytes).to_vec(),
...
fn fixture_validator<'a>(
    ...
    ProductionQuarantineValidator {
        node,
        selected_closure,
        visible_roots: roots.iter().copied().collect(),
        pack_limits: limits,
...
fn put(node: &OneNode, objects: &[Raw]) {
    for object in objects {
        assert_eq!(
            node.put_git_object(object.kind, object.body.clone())
                .unwrap()
                .identity(),
            object.id
        );
...
fn check(
    v: &ProductionQuarantineValidator<'_>,
    entries: &[Vec<u8>],
    target: GitOid,
) -> Result<ValidatedClosure, RefusalCode> {
    let format = v.node.object_format;
    let (pack, receipt) = packed(format, entries);
    v.validate(
        &request(format, zero(format), target, "refs/tags/result"),
        Some(&pack),
        &receipt,
        &mut || true,
    )
...
fn uploaded_wrappers_cannot_publish_hidden_only_graph_inputs() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let node = OneNode::init(scratch.config(format)).unwrap().0;
        let empty = tree(format, &[]);
        let public = commit(format, empty.id, &[], "public");
        ...
        for (entries, id) in [
            (vec![full(&wrapper_tree), full(&wrapper)], wrapper.id),
...
fn copying_a_hidden_delta_base_including_its_exact_oid_does_not_prove_upload_ownership() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let originals = [public.clone(), secret.clone()];
        put(&node, &originals);
        let v = fixture_validator(&node, &originals, &[public.id], PackLimits::default());
        for suffix in [b"!".as_slice(), b"".as_slice()] {
            let target = raw(
                format,
                ObjectType::Blob,
                [secret.body.as_slice(), suffix].concat(),
            );
            let (pack, _) = packed(format, &[ref_copy(&secret, suffix)]);
            let bases = v.external_bases(&pack, &mut || true).unwrap();
            let (_, ids) = v
                .verified_pack_objects(&pack, &bases, &mut || true)
                .unwrap();
            assert!(
                !independent_uploads(&pack, &ids, &mut || true)
                    .unwrap()
                    .contains(&secret.id)
            );
            assert_eq!(
                check(&v, &[ref_copy(&secret, suffix)], target.id),
                Err(RefusalCode::ObjectClosureIncomplete)
            );
            if target.id != secret.id {
                assert!(node.read_git_object(target.id).is_err());
            }
...
fn complete_uploaded_bodies_and_forward_deltas_remain_valid_without_visible_originals() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let result = raw(
            format,
            ObjectType::Blob,
            [base.body.as_slice(), b"!"].concat(),
        );
        for entries in [
            vec![full(&result)],
            vec![full(&base), ref_copy(&base, b"!")],
            vec![ref_copy(&base, b"!"), full(&base)],
        ] {
            assert!(
                check(&v, &entries, result.id)
                    .unwrap()
                    .objects
                    .contains(&result.id)
            );
...
fn one_visibility_walk_and_kind_cache_keep_the_original_byte_limit_inclusive() {
    ...
    let scratch = Scratch::new();
    let node = OneNode::init(scratch.config(format)).unwrap().0;
    let base = raw(format, ObjectType::Blob, b"shared original".to_vec());
    put(&node, &[base.clone()]);
    let result = raw(
        format,
        ObjectType::Blob,
    ...
    for (bytes, allowed) in [(base.body.len() - 1, false), (base.body.len(), true)] {
        ...
        if allowed {
            assert_eq!(
                answer.unwrap().objects,
                BTreeSet::from([result.id, new_tree.id])
            );
        } else {
...
fn irrelevant_visible_blob_contents_are_not_copied_to_prove_another_dependency() {
    ...
    let originals = [huge, needed.clone(), old_tree.clone(), tip.clone()];
    put(&node, &originals);
    ...
    let v = fixture_validator(
        ...
        &[tip.id],
        PackLimits {
            max_cached_bytes: exact,
            ..PackLimits::default()
        },
    );
    let upload = tree(format, &[("copy", needed.id)]);
    assert_eq!(
        check(&v, &[full(&upload)], upload.id).unwrap().objects,
        BTreeSet::from([upload.id])
    );
    node.shutdown().unwrap();
}
...
fn incomplete_visibility_proof_and_cancellation_never_grant_cached_partial_inputs() {
    ...
    let originals = [public.clone(), private.clone()];
    put(&node, &originals);
    let v = fixture_validator(&node, &originals, &[public.id], PackLimits::default());
    let bases = ExternalBases {
        bases: BTreeMap::new(),
        read_bytes: 0,
    };
    ...
    assert_eq!(
        frontier.authorize(
            &BTreeSet::from([public.id, private.id]),
            &mut budget,
            &mut || true
        ),
        Err(RefusalCode::ObjectClosureIncomplete)
    ...
    assert_eq!(
        frontier.kind(public.id, &mut || true),
        Err(RefusalCode::ObjectClosureIncomplete)
    );
    frontier
        .authorize(&BTreeSet::from([public.id]), &mut budget, &mut || true)
        .unwrap();
    assert_eq!(
        frontier.kind(public.id, &mut || true).unwrap(),
        ObjectType::Blob
    );
    assert_eq!(
        frontier.authorize(&BTreeSet::from([public.id]), &mut budget, &mut || false),
...
fn loose(root: &Path, object: &Raw) {
    ...
    let bytes = [
        format!("{} {}\0", object.kind.label(), object.body.len()).as_bytes(),
...
fn production_factory_cannot_republish_disconnected_history_through_an_uploaded_wrapper() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let private_tree = tree(format, &[("secret", secret.id)]);
        let private = commit(format, private_tree.id, &[], "private");
        for object in [&empty, &public, &secret, &private_tree, &private] {
            loose(&root, object);
        }
        ...
        let proof = validate_receive_at_basis(
            &delete,
            None,
            &receipt,
            before.basis(),
        ...
        let session = LoopbackReceiveSession::authenticated(
            principal(),
            IdempotencyKey::new(b"detach-private".to_vec()).unwrap(),
        );
        let deleted = node
            .runtime()
            ...
            .unwrap();
        assert!(matches!(
            deleted.commands[0].terminal.outcome,
        ...
        assert!(
            !before
                .snapshot()
                .refs
                .contains_key(&RefName::try_new(b"refs/heads/private").unwrap())
        );
        let v = node
            .production_quarantine_validator(
                &before,
        ...
        assert_eq!(
            check(&v, &[full(&new_tree), full(&candidate)], candidate.id),
            Err(RefusalCode::ObjectClosureIncomplete)
        );
        assert!(
            node.read_git_object(candidate.id).is_err()
                && node.read_git_object(new_tree.id).is_err()
        );
        assert_eq!(
            node.runtime()
                .block_on(node.materialize_admission_in(&cx))
        ...
        let session = LoopbackReceiveSession::authenticated(
            principal(),
            IdempotencyKey::new(b"supplied-content".to_vec()).unwrap(),
        );
        let applied = node
            .runtime()
            .block_on(node.admit_basis_bound_loopback_receive_durable_in(
        ...
        assert_eq!(
            node.runtime()
                ...
                .unwrap()
                .snapshot()
                .refs[&RefName::try_new(b"refs/heads/main").unwrap()],
            candidate.id
        );
        node.shutdown().unwrap();
    }
