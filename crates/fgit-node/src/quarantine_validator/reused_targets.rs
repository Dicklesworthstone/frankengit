...
impl<'a, 'node> OriginalFrontier<'a, 'node> {
    pub(super) fn new(
        ...
        Ok(Self {
            validator,
            bases,
            bytes: bases.read_bytes,
            kinds: BTreeMap::new(),
            authorized: BTreeSet::new(),
        })
    }

    pub(super) const fn read_bytes(&self) -> usize {
        self.bytes
    }
    ...
    pub(super) fn prerequisite_closure(
        &mut self,
        roots: &BTreeSet<GitOid>,
        ...
        while let Some(id) = pending.pop() {
            ...
            let (kind, edges) = self.with_object(id, deadline, |kind, bytes, deadline| {
                ...
                crate::loose_import::graph::references(
                    format, &parsed, bytes, &limits, edges_left, deadline,
                )
                .map(|edges| (kind, edges))
            })?;
    ...
    fn with_object<D: Deadline, R>(
        ...
        let result = if let Some(base) = self.bases.bases.get(&id) {
            ...
            let object = self
                .validator
                .load_selected_external_base(id, remaining, deadline)?
    ...
    pub(super) fn authorize(
        ...
        if wanted
            .iter()
            .all(|id| self.validator.visible_roots.contains(id))
        {
            for id in &wanted {
                self.read_kind(*id, deadline)?;
            }
        ...
        while let Some(id) = pending.pop() {
            ...
            if expected == Some(ObjectType::Blob) && !wanted.contains(&id) {
                continue;
            }
            let format = self.validator.node.object_format;
            let limits = self.validator.parse_limits.clone();
...
fn check_kind(actual: ObjectType, expected: Option<ObjectType>) -> Result<(), RefusalCode> {
    if expected.is_some_and(|kind| kind != actual) {
...
fn enqueue(
    ...
    pending: &mut Vec<GitOid>,
    requirements: &mut BTreeMap<GitOid, Option<ObjectType>>,
    kinds: &BTreeMap<GitOid, ObjectType>,
    deadline: &mut impl Deadline,
) -> Result<(), RefusalCode> {
    checkpoint(deadline)?;
    if let Some(actual) = kinds.get(&id) {
        check_kind(*actual, expected)?;
    }
...
pub(super) fn independent_uploads(
    ...
    for entry in pack.entries() {
        ...
        let base = match entry.delta_base.as_ref() {
            ...
            Some(ParsedDeltaBase::Ofs { base_offset, .. }) => *ids
                .get(base_offset)
                .ok_or(RefusalCode::PackFramingInvalid)?,
