...
use fgit_types::{
    CANONICAL_CODEC_VERSION, DigestBytes, RepositoryCommitId, RepositoryId, TenantId,
};
...
fn selected_closure(objects: BTreeSet<GitOid>) -> AuthoritySelectedClosure {
    ...
    AuthoritySelectedClosure {
        root: permitted_object_closure_root(&closure).unwrap(),
        closure,
...
fn raw(format: GitHashAlgorithm, kind: ObjectType, body: Vec<u8>) -> Raw {
    Raw {
        id: fgit_crypto::git_object_id(format, kind, &body),
        kind,
        body,
    }
}
...
fn header(kind: u8, mut size: usize) -> Vec<u8> {
    ...
    let mut byte = (kind << 4) | (size as u8 & 15);
    size >>= 4;
    while size != 0 {
        bytes.push(byte | 128);
        byte = size as u8 & 127;
...
fn delta(base: &Raw, target: &Raw) -> Vec<u8> {
    ...
    for chunk in target.body.chunks(127) {
        program.push(chunk.len() as u8);
        program.extend_from_slice(chunk);
    }
    [
        header(7, program.len()),
        base.id.as_bytes().to_vec(),
        zlib_stored(&program),
    ]
    .concat()
}
fn packed(format: GitHashAlgorithm, entries: &[Vec<u8>]) -> (QuarantinedPack, QuarantineReceipt) {
    ...
    let receipt = QuarantineReceipt {
        ...
        object_count: u32::try_from(entries.len()).unwrap(),
...
fn request(format: GitHashAlgorithm, roots: &[GitOid]) -> ReceiveRequest {
    ReceiveRequest {
        commands: roots
            .iter()
            .enumerate()
            .map(|(n, id)| ReceiveCommand {
                old: GitOid::from_hex(format, &"0".repeat(2 * format.digest_len())).unwrap(),
                new: *id,
...
fn selected_validator<'a>(
    node: &'a OneNode,
    format: GitHashAlgorithm,
    originals: &[GitOid],
    limits: PackLimits,
) -> ProductionQuarantineValidator<'a> {
    ProductionQuarantineValidator::new(
        node,
        selected_closure(originals.iter().copied().collect()),
        limits,
        ParseLimits {
...
fn check(
    validator: &ProductionQuarantineValidator<'_>,
    objects: &[Raw],
    roots: &[GitOid],
...
fn uploaded_commit_tree_and_parent_edges_cannot_name_other_native_kinds() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let parent = commit(format, empty.id, &[], "parent");
        ...
        for bad in [&wrong_tree, &wrong_parent, &tree_parent] {
            assert_eq!(
                check(
                    &validator,
                    &[blob.clone(), empty.clone(), bad.clone()],
                    &[bad.id]
                ),
            ...
            assert!(
                node.read_git_object(blob.id).is_err() && node.read_git_object(empty.id).is_err()
            );
        }
        ...
        assert_eq!(
            check(
                &validator,
                &[empty.clone(), parent.clone(), good.clone()],
                &[good.id]
            )
            .unwrap()
            .objects,
...
fn tree_modes_and_all_tag_target_types_require_the_actual_kind() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for (mode, wrong, good) in [
            ...
            offered.push(bad.clone());
            assert_eq!(
                check(&validator, &offered, &[bad.id]),
                Err(RefusalCode::EvidenceInvalid)
            );
            assert!(node.read_git_object(bad.id).is_err());
            ...
            *offered.last_mut().unwrap() = allowed.clone();
            assert!(
                check(&validator, &offered, &[allowed.id])
                    .unwrap()
                    .objects
                    .contains(&good.id)
            );
        ...
        for (declared, expected) in [
            ...
            for actual in &objects {
                ...
                let mut offered = objects.to_vec();
                if !offered.iter().any(|o| o.id == outer.id) {
                    offered.push(outer.clone());
                }
                let result = check(&validator, &offered, &[outer.id]);
                if actual.kind == expected {
                    assert!(result.unwrap().objects.contains(&actual.id));
                } else {
                    assert_eq!(result, Err(RefusalCode::EvidenceInvalid));
                }
            }
        }
        node.shutdown().unwrap();
...
fn selected_frontier_requires_real_typed_bytes_and_never_uses_unselected_presence() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let node = node(format, &scratch);
        let blob = node
            .put_git_object(ObjectType::Blob, b"present, wrong kind".to_vec())
            .unwrap()
            .identity();
        let empty = node
            .put_git_object(ObjectType::Tree, Vec::new())
            .unwrap()
            .identity();
        let absent = raw(
            format,
            ObjectType::Tree,
            tree(format, &[("100644", "missing", blob)]).body,
        )
        .id;
        let selected =
            selected_validator(&node, format, &[blob, empty, absent], PackLimits::default());
        for (id, expected) in [
            (blob, RefusalCode::EvidenceInvalid),
            (absent, RefusalCode::EvidenceMissing),
        ] {
        ...
        assert_eq!(
            check(&unselected, &[allowed.clone()], &[allowed.id]),
            Err(RefusalCode::ObjectClosureIncomplete)
        );
        assert!(node.read_git_object(allowed.id).is_err());
        assert_eq!(
            check(&selected, &[allowed.clone()], &[allowed.id])
                .unwrap()
                .objects,
            BTreeSet::from([allowed.id])
        );
        node.shutdown().unwrap();
    }
}
...
fn ambiguous_required_headers_and_zero_edges_refuse_without_selecting_a_convenient_view() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let validator = selected_validator(&node, format, &[], PackLimits::default());
        let empty = tree(format, &[]);
        let good = commit(format, empty.id, &[], "ordinary");
        ...
        for body in [
            ...
            assert_eq!(
                check(
                    &validator,
                    &[empty.clone(), good.clone(), bad.clone()],
                    &[bad.id]
                ),
                Err(RefusalCode::ObjectHeaderInvalid)
            );
            assert!(node.read_git_object(bad.id).is_err());
        }
...
fn repeated_frontier_kinds_and_noncanonical_gitlink_modes_preserve_valid_graphs() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let node = node(format, &scratch);
        let blob = node
            .put_git_object(ObjectType::Blob, vec![b'x'; 4096])
            .unwrap()
            .identity();
        ...
        let tree = tree(
            ...
            &[
                ...
                ("100755", "b", blob),
                ("120000", "c", blob),
                ("160000", "submodule", missing),
                ("0160000", "submodule2", missing),
            ],
        );
        ...
        let validator = selected_validator(
            ...
            PackLimits {
                ...
                max_total_expanded_bytes: 4096,
                ..PackLimits::default()
            },
        );
        assert_eq!(
            check(&validator, &[tree.clone(), tip.clone()], &[tip.id])
                .unwrap()
                .objects,
...
fn delta_inputs_and_native_frontier_share_one_budget_and_do_not_charge_reused_bases_twice() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for original in [&base, &other] {
            node.put_git_object(original.kind, original.body.clone())
                .unwrap();
        }
        ...
        let tree = tree(
            ...
            &[
                ("100644", "base", base.id),
                ("100644", "delta", result.id),
                ("100644", "other", other.id),
            ],
        );
        ...
        for object in [&result, &tree, &tip] {
            assert!(node.read_git_object(object.id).is_err());
        }
        limits.max_cached_bytes = 3072;
        limits.max_total_expanded_bytes = 3072;
...
fn forward_delta_native_edges_are_verified_without_losing_transport_only_dependencies() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let validator = selected_validator(&node, format, &[], PackLimits::default());
        let empty = tree(format, &[]);
        let blob = raw(format, ObjectType::Blob, b"not a tree".to_vec());
        let base = commit(format, empty.id, &[], "transport base");
        let bad = commit(format, blob.id, &[], "reconstructed wrong tree");
        let (pack, receipt) = packed(
            format,
            &[delta(&base, &bad), full(&empty), full(&blob), full(&base)],
        );
        assert_eq!(
            validator.validate(
        ...
        for id in [bad.id, empty.id, blob.id, base.id] {
            assert!(node.read_git_object(id).is_err());
        }
        let good = commit(format, empty.id, &[], "reconstructed good tree");
        let (pack, receipt) = packed(
            format,
            &[delta(&base, &good), full(&empty), full(&blob), full(&base)],
        );
        assert_eq!(
            validator
                .validate(
                    &request(format, &[good.id]),
                    Some(&pack),
                    &receipt,
                    &mut || true
                )
...
fn contradictory_kind_requirements_refuse_in_either_command_order() {
    ...
    for roots in [[good.id, bad.id], [bad.id, good.id]] {
        assert_eq!(
            check(
                &validator,
                &[blob.clone(), good.clone(), bad.clone()],
                &roots
            ),
            Err(RefusalCode::EvidenceInvalid)
        );
        for id in [blob.id, good.id, bad.id] {
            assert!(node.read_git_object(id).is_err());
        }
    }
    node.shutdown().unwrap();
...
fn typed_handoff_enters_real_admission_and_recovers_without_a_second_decision() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let before = node
            .runtime()
            .block_on(node.materialize_admission_in(&cx))
            .unwrap();
        ...
        let bad_validator = node
            ...
            .unwrap();
        let mut rejected =
            ProductionReceiveQuarantineHandoff::new(bad_validator, before.basis().clone());
        assert_eq!(
            rejected.handoff_with_deadline(
                &request(format, &[bad.id]),
                Some(&bad_pack),
                &bad_receipt,
                &mut || true
        ...
        let mut requested = request(format, &[tip.id]);
        requested.commands[0].ref_name = b"refs/heads/main".to_vec();
        let validator = node
            .production_quarantine_validator(
                &before,
                PackLimits::default(),
            ...
            .unwrap();
        let mut handoff =
            ProductionReceiveQuarantineHandoff::new(validator, before.basis().clone());
        handoff
        ...
        let session = LoopbackReceiveSession::authenticated(
            ...
            IdempotencyKey::new(b"typed-graph".to_vec()).unwrap(),
        );
        let result = node
            .runtime()
