...
fn validator<'a>(
    ...
    ProductionQuarantineValidator::new(
        node,
        selected_closure(selected.iter().copied().collect()),
        PackLimits {
...
fn tiny_thin_pack_cannot_materialize_more_external_bytes_than_its_envelope() {
    ...
    let a = node
        .put_git_object(ObjectType::Blob, vec![b'a'; 64])
        .unwrap()
        .identity();
    let b = node
        .put_git_object(ObjectType::Blob, vec![b'b'; 64])
        .unwrap()
        .identity();
    ...
    request
        .commands
        .extend(create_request(y).commands.into_iter().map(|mut command| {
            command.ref_name = b"refs/tags/second".to_vec();
            command
        }));
    ...
    assert_eq!(
        validator(&node, &[a, b], 127).validate(&request, Some(&pack), &receipt, &mut || true),
        Err(RefusalCode::ResourceBudgetExceeded)
    );
    assert!(
        node.read_git_object(x).is_err() && node.read_git_object(y).is_err(),
    ...
    assert_eq!(
        originals.read_bytes, 128,
        "the original-input bound is inclusive"
    );
    ...
    for expanded in [128, 129] {
        ...
        assert_eq!(
            selected.validate(&request, Some(&pack), &receipt, &mut || true),
            Err(RefusalCode::ResourceBudgetExceeded)
        );
        assert!(
            node.read_git_object(x).is_err() && node.read_git_object(y).is_err(),
    ...
    assert_eq!(
        selected.external_read_limit(),
        128,
        "the original-input cap is not widened"
    );
    let admitted = selected
        .validate(&request, Some(&pack), &receipt, &mut || true)
        .unwrap();
...
fn original_object_limits_apply_before_a_quarantine_copy() {
    ...
    assert!(matches!(
        bounded.external_bases(&pack, &mut || true),
        Err(RefusalCode::ResourceBudgetExceeded)
    ));
...
fn cancellation_observed_after_external_read_is_not_a_missing_base_verdict() {
    ...
    let node = test_node(scratch.path().to_path_buf());
    let missing =
        fgit_crypto::git_object_id(GitHashAlgorithm::Sha1, GitObjectKind::Blob, &[b'a'; 64]);
    let pack = parsed(&literal_delta_pack(&[missing], b"x"));
    let selected = validator(&node, &[missing], 128);
    let calls = Cell::new(0usize);
    let mut cancelled_at_read = || {
        calls.set(calls.get() + 1);
        calls.get() < 3
    };
    assert!(matches!(
        selected.external_bases(&pack, &mut cancelled_at_read),
        Err(RefusalCode::CancellationInProgress)
    ));
    assert!(matches!(
        selected.external_bases(&pack, &mut || true),
