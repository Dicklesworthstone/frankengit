...
fn read_records<T, K: Ord>(
    ...
    for (index, line) in text.lines().enumerate() {
        ...
        let record =
            parse(line).ok_or_else(|| format!("invalid webhook record at line {}", index + 1))?;
...
fn replace_file(path: &Path, bytes: &[u8]) -> Result<(), String> {
    ...
    directory
        .sync_all()
        .map_err(|error| format!("webhook directory sync unavailable: {error}"))?;
    for _ in 0..128 {
...
fn ensure_directory(root: &Path) -> Result<(), String> {
    ...
    std::fs::create_dir_all(root)
        .map_err(|error| format!("cannot create webhook directory: {error}"))?;
    #[cfg(unix)]
...
impl DeadLetterQueue {
    ...
    fn mutate<R>(
        ...
        let mut state = self
            .state
            .lock()
            .map_err(|_| "webhook dead-letter state is poisoned")?;
        if state.failed_open {
            return Err(state
                .last_error
                .clone()
                .unwrap_or_else(|| "webhook store open failed".into()));
        }
        let result: Result<R, String> = (|| {
            let _writer = self
                .persist_path
                .as_ref()
                .map(|path| lock_writer(path))
                .transpose()?;
    ...
    pub fn try_push(&self, entry: DeadLetterEntry) -> Result<(), String> {
        self.mutate(|entries| {
            ...
            if let Some(previous) = entries
                .iter()
                .find(|previous| previous.delivery_id == entry.delivery_id)
            {
                if previous.webhook_id != entry.webhook_id
                    || previous.payload_root != entry.payload_root
                    || previous.target_url != entry.target_url
                    || previous.event_name != entry.event_name
                {
                    return Err(
                        "dead-letter delivery key reused with different payload or destination"
                            .into(),
                    );
                }
            }
...
impl WebhookStore {
    pub fn open(root_dir: PathBuf) -> Result<Self, String> {
        ...
        let dead_letters =
            DeadLetterQueue::try_with_persist_path(root_dir.join("dead_letters.jsonl"))?;
    ...
    fn mutate<R>(
        ...
        let mut current = self
            .registrations
            .lock()
            .map_err(|_| "webhook registration state is poisoned")?;
    ...
    pub fn try_replay_dead_letter(
        &self,
        delivery_id: AsciiSlug,
...
mod tests {
    ...
    use fgit_forge::webhook::{
        SsrfPolicy, WebhookEventFilter, WebhookRetrySchedule, WebhookSecretRotation,
    };
    ...
    fn letter() -> DeadLetterEntry {
        DeadLetterEntry {
            delivery_id: AsciiSlug::from_static("durable-delivery"),
            webhook_id: WebhookId(1),
            target_url: "http://example.com/hook".into(),
            payload_root: Digest::new(
                DigestAlgorithmId::try_new(2).unwrap(),
                DigestBytes::try_new(&[0x17; 32]).unwrap(),
            ),
            event_name: "issue".into(),
            attempts: 5,
            terminal_reason: "unavailable".into(),
            failed_at_unix_secs: 7,
        }
    }
    ...
    fn reopen_preserves_registration_rotation_and_dead_letter_replay() {
        ...
        let updated = store
            .rotate_secret(
                WebhookId(1),
                WebhookSecret::new(vec![2; 32]).unwrap(),
                30,
                100,
            )
            .unwrap();
        ...
        assert_eq!(
            reopened
                .try_replay_dead_letter(letter().delivery_id)
                .unwrap(),
            Some(letter())
        );
        assert!(
    ...
    fn independent_stale_handles_preserve_each_others_registrations() {
        ...
        second.register(registration(2)).unwrap();
    ...
    fn independent_dead_letter_handles_do_not_lose_unseen_entries() {
        ...
        let mut other = letter();
        other.delivery_id = AsciiSlug::from_static("another-delivery");
        second.try_push(other).unwrap();
        assert_eq!(
            DeadLetterQueue::try_with_persist_path(path)
                .unwrap()
                .list()
                .len(),
            2
    ...
    fn failed_replay_retains_dead_letter_and_reports_error() {
        let dir = Directory::new();
        let path = dir.0.join("letters.jsonl");
    ...
    fn reused_delivery_key_cannot_change_payload_identity() {
        ...
        queue.try_push(letter()).unwrap();
        let mut replacement = letter();
        replacement.target_url.push_str("/different");
    ...
    fn unrepresentable_duration_refuses_without_losing_previous_configuration() {
        ...
        let mut invalid = registration(1);
        invalid.retry_schedule.initial_delay = Duration::from_nanos(1);
        assert!(store.register(invalid).is_err());
        assert_eq!(
            std::fs::read(dir.0.join("registrations.json")).unwrap(),
            before
        );
    ...
    fn abandoned_stage_is_not_selected_and_secrets_are_private() {
        ...
        store.register(registration(1)).unwrap();
        std::fs::write(
            dir.0.join(".fg-webhook-abandoned.tmp"),
            serialize_registration(&registration(2)),
        )
        .unwrap();
        let reopened = WebhookStore::open(dir.0.clone()).unwrap();
        assert_eq!(reopened.list(), vec![registration(1)]);
        let mode = std::fs::metadata(dir.0.join("registrations.json"))
            .unwrap()
            .permissions()
            .mode();
