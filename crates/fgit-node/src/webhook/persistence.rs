...
impl Reader<'_> {
    ...
    fn value(&mut self, depth: usize) -> Option<Value> {
        if depth > MAX_DEPTH || self.remaining_values == 0 {
            return None;
        }
        ...
        match self.rest.as_bytes().first().copied()? {
            ...
            b'0'..=b'9' => {
                ...
                if length > 1 && text.starts_with('0') {
                    return None;
                }
...
fn object(line: &str) -> Option<BTreeMap<String, Value>> {
    ...
    let Value::Object(value) = reader.value(0)? else {
        return None;
    };
    reader.space();
...
pub(super) fn serialize_registration(reg: &WebhookRegistration) -> String {
    ...
    format!(
        ...
        reg.id.0,
        quote(reg.url.raw()),
        super::hex_encode(reg.secrets.active().as_bytes()),
        expiring,
        filter,
        reg.active,
...
pub(super) fn parse_registration_line(line: &str) -> Option<WebhookRegistration> {
    ...
    let active_secret =
        WebhookSecret::new(hex_decode(fields.get("active_secret_hex")?.string()?)?).ok()?;
    let secrets = match fields.get("expiring")? {
        ...
        Value::Object(expiring) if expiring.len() == 2 => {
            let old =
                WebhookSecret::new(hex_decode(expiring.get("secret_hex")?.string()?)?).ok()?;
    ...
    let filter = match fields.get("filter")? {
        ...
        Value::Array(names) => WebhookEventFilter::Selected(
            names
                .iter()
                .map(|name| Some(name.string()?.to_owned()))
    ...
    let url = SsrfPolicy::PERMISSIVE_FOR_TESTS
        .validate_url(fields.get("url")?.string()?)
        .ok()?;
    Some(WebhookRegistration {
        id: WebhookId(fields.get("id")?.number()?),
        url,
        secrets,
        filter,
        active: fields.get("active")?.boolean()?,
        retry_schedule,
    })
}
...
pub(super) fn serialize_dead_letter(entry: &DeadLetterEntry) -> String {
    format!(
        ...
        entry.webhook_id.0,
        quote(&entry.target_url),
        quote(&entry.payload_root.to_string()),
        quote(&entry.event_name),
        entry.attempts,
        quote(&entry.terminal_reason),
...
pub(super) fn parse_dead_letter_line(line: &str) -> Option<DeadLetterEntry> {
    ...
    Some(DeadLetterEntry {
        delivery_id: AsciiSlug::try_new(
            "delivery_id",
            fields.get("delivery_id")?.string()?.as_bytes(),
        )
        .ok()?,
...
mod tests {
    ...
    fn registration() -> WebhookRegistration {
        WebhookRegistration {
            ...
            url: SsrfPolicy::STRICT
                .validate_url("https://example.com/hook?name=a%22b")
                .unwrap(),
    ...
    fn dead_letter() -> DeadLetterEntry {
        DeadLetterEntry {
            delivery_id: AsciiSlug::from_static("delivery-roundtrip"),
            webhook_id: WebhookId(7),
            target_url: "https://example.com/\"hook\"".into(),
            payload_root: Digest::new(
                DigestAlgorithmId::try_new(2).unwrap(),
                DigestBytes::try_new(&[0x73; 32]).unwrap(),
            ),
    ...
    fn selected_empty_and_wildcard_filters_roundtrip_without_widening() {
        for filter in [
            ...
            WebhookEventFilter::Selected(Vec::new()),
            WebhookEventFilter::Selected(vec!["issue".into(), "a\"b\\c\n雪".into()]),
        ] {
            let mut reg = registration();
            reg.filter = filter;
            assert_eq!(
                parse_registration_line(&serialize_registration(&reg)),
                Some(reg)
            );
        }
    }
    ...
    fn rotation_preserves_active_key_and_original_expiry_after_reopen() {
        ...
        assert_eq!(
            restored.secrets.sign_active(b"event"),
            active.sign(b"event")
        );
        ...
        assert!(
            restored
                .secrets
                .verify(b"event", &active.sign(b"event"), 131)
        );
        assert_eq!(
            parse_registration_line(&serialize_registration(&restored)),
            Some(reg)
        );
    }
    ...
    fn digest_domains_and_all_shell_lengths_roundtrip() {
        for algorithm in [1, 2, u16::MAX] {
            for length in [16, 20, 32, 64] {
                ...
                entry.payload_root = Digest::new(
                    DigestAlgorithmId::try_new(algorithm).unwrap(),
                    DigestBytes::try_new(&vec![0x19; length]).unwrap(),
                );
                assert_eq!(
                    parse_dead_letter_line(&serialize_dead_letter(&entry)),
                    Some(entry)
                );
            }
        }
    ...
    fn corrupt_or_absent_root_never_gets_a_dummy_identity() {
        ...
        for invalid in ["", "alg:0:abcd", "alg:2:abcd", "alg:2:éé", "alg:65536:abcd"] {
            assert!(
                parse_dead_letter_line(&encoded.replace(&entry.payload_root.to_string(), invalid))
                    .is_none()
            );
        }
        assert!(
            parse_dead_letter_line(&encoded.replace("\"payload_root\"", "\"other\"")).is_none()
        );
    }
    ...
    fn missing_malformed_or_duplicate_security_fields_refuse() {
        ...
        for candidate in [
            ...
            line.replace(
                "\"expiring\":null",
                "\"expiring\":{\"secret_hex\":\"abcd\",\"expires_at\":1}",
            ),
            format!("{line} trailing"),
        ] {
            assert!(
                parse_registration_line(&candidate).is_none(),
    ...
    fn hexadecimal_unicode_is_refused_without_slicing_panics() {
        for text in ["a雪", "雪a", "éé", "0g", "abc"] {
            assert!(hex_decode(text).is_none());
        }
