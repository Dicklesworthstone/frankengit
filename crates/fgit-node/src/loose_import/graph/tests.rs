...
impl Objects {
    ...
    fn tree(&mut self, format: GitHashAlgorithm, mode: &str, child: GitOid) -> GitOid {
        self.put(
            format,
            ObjectType::Tree,
            [format!("{mode} entry\0").as_bytes(), child.as_bytes()].concat(),
        )
    }
    fn commit(
        &mut self,
    ...
    fn load(&self, id: GitOid) -> Result<LooseObject, LooseGitImportRefusal> {
        self.0
            .get(&id)
            .cloned()
            .ok_or(LooseGitImportRefusal::ObjectMissing(id))
    }
}
...
fn write_loose(root: &Path, id: GitOid, object: &LooseObject) {
    ...
    let path = root.join("objects").join(&hex[..2]);
    fs::create_dir_all(&path).unwrap();
    let framed = [
        format!("{} {}\0", object.object_type.label(), object.body.len()).as_bytes(),
        &object.body,
    ]
    .concat();
...
fn write_pack(root: &Path, format: GitHashAlgorithm, objects: &Objects) {
    ...
    pack.extend(u32::try_from(objects.0.len()).unwrap().to_be_bytes());
    let mut entries = Vec::new();
    for (id, object) in &objects.0 {
        let offset = pack.len();
        let kind = match object.object_type {
            ObjectType::Commit => 1_u8,
            ObjectType::Tree => 2,
            ObjectType::Blob => 3,
            ObjectType::Tag => 4,
        };
    ...
    for (_, crc, _) in &entries {
        index.extend(crc.to_be_bytes());
    }
    for (_, _, offset) in &entries {
        index.extend(offset.to_be_bytes());
    }
    index.extend(checksum);
    let checksum = hash(format, &index);
    index.extend(checksum);
    let dir = root.join("objects/pack");
    fs::create_dir_all(&dir).unwrap();
    fs::write(dir.join("fixture.pack"), pack).unwrap();
    fs::write(dir.join("fixture.idx"), index).unwrap();
}
fn source(
    root: &Path,
    format: GitHashAlgorithm,
    ...
    if packed {
        write_pack(root, format, objects);
    } else {
        for (id, object) in &objects.0 {
            write_loose(root, *id, object);
        }
    }
}
...
fn assert_unstaged(node: &OneNode, objects: &Objects) {
    for id in objects.0.keys() {
        assert!(
            node.read_git_object(*id).is_err(),
            "unexpected partial staging of {id}"
        );
    }
}
...
fn absent_submodule_objects_import_from_loose_and_packed_sources_in_both_domains() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        for packed in [false, true] {
            ...
            let external =
                git_object_id(format, ObjectType::Commit, b"object in another repository");
            ...
            let unused = objects.put(
                format,
                ObjectType::Blob,
                b"unreachable source bytes".to_vec(),
            );
            ...
            source(
                ...
                format,
                &objects,
                &[("refs/heads/main", commit), ("refs/tags/release", nested)],
                packed,
            );
            ...
            assert_eq!(
                ...
                &BTreeSet::from([tree, commit, tag, nested])
            );
            assert_eq!(staged.object_count(), 4);
            assert_eq!(
                staged.refs().head_target(),
                Some(&RefName::try_new(b"refs/heads/main").unwrap())
            );
            assert!(node.read_git_object(external).is_err());
            assert!(node.read_git_object(unused).is_err());
            assert_eq!(
                node.runtime()
                    .block_on(node.materialize_admission_in(&request))
                    .unwrap()
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        for packed in [false, true] {
            for case in 0..6 {
                let scratch = Scratch::new();
                let mut objects = Objects::default();
                let blob = objects.put(
                    format,
                    ObjectType::Blob,
                    b"valid bytes, wrong relationship".to_vec(),
                );
                ...
                assert!(
                    matches!(
                        node.stage_loose_git_import(&path),
...
fn later_missing_dependencies_cannot_leave_an_earlier_valid_root_staged() {
    ...
    source(
        ...
        format,
        &objects,
        &[("refs/heads/main", commit), ("refs/notes/data", other)],
        true,
    );
    let node = scratch.node(format);
    assert!(
        matches!(node.stage_loose_git_import(&path), Err(LooseGitImportRefusal::ObjectMissing(id)) if id == missing)
    );
    assert_unstaged(&node, &objects);
    ...
    let id = objects.put(format, ObjectType::Blob, b"missing".to_vec());
...
fn contradictory_requirements_are_checked_even_for_previously_selected_roots() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for roots in [[blob, bad], [bad, blob]] {
            assert!(matches!(
                validate(roots, format, &limits(format), Limits::default(), |id| {
                    objects.load(id)
                }),
                Err(LooseGitImportRefusal::ObjectGraph {
                    code: RefusalCode::EvidenceInvalid,
        ...
        .unwrap();
        assert_eq!(
            proof.objects.keys().copied().collect::<BTreeSet<_>>(),
            BTreeSet::from([blob, tree, valid])
        );
    }
}
...
fn frontier_edges_and_retained_body_bytes_have_independent_inclusive_bounds() {
    ...
    assert_eq!(
        validate([tree], format, &limits(format), exact, |id| objects
            .load(id))
        .unwrap()
        .total_bytes,
        bytes
...
fn the_common_edge_reader_preserves_cancellation_and_requires_all_tag_target_types() {
    ...
    for kind in ["blob", "tree", "commit", "tag"] {
        let tag = objects.tag(format, blob, kind);
        let object = &objects.0[&tag];
        ...
        assert_eq!(edges[0].0, blob);
        assert_eq!(edges[0].1.label(), kind);
...
fn a_substituted_body_never_produces_an_import_proof() {
    ...
    assert!(
        matches!(validate([expected], format, &limits(format), Limits::default(), |_| objects.load(replacement)),
...
fn valid_submodule_import_publishes_and_reopens_through_the_original_durable_api() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let scratch = Scratch::new();
        let mut objects = Objects::default();
        let foreign = git_object_id(
            format,
            ObjectType::Commit,
            b"unavailable external repository",
        );
        let tree = objects.tree(format, "160000", foreign);
        let tip = objects.commit(format, tree, &[], "submodule import");
        let root = scratch.0.join("source");
        source(&root, format, &objects, &[("refs/heads/main", tip)], true);
