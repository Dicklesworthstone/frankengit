...
fn cancellation_is_sticky_and_prevents_even_the_first_read() {
    ...
    assert_eq!(
        probes.get(),
        1,
        "a stopped operation cannot revive its probe"
    );
}
...
fn eof_and_interrupted_os_calls_cannot_suppress_the_caller_stop() {
    ...
    assert_eq!(
        read_bytes(
            RetryReader { attempts: 0 },
            10,
            &ImportControl::new(&mut live)
        )
        .unwrap(),
        [0xff]
    );
...
fn cpu_sampling_does_not_charge_a_runtime_poll_for_every_decode_byte() {
    ...
    for _ in 0..CPU_PROBE_INTERVAL {
        assert!(Deadline::checkpoint(&mut cpu));
    }
