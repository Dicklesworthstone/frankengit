...
use fgit_git_object::{
    CancellationProbe, InflateLimits, LooseObject, ParseLimits, ZlibLooseObjectDecoder,
};
...
impl<'a> ImportControl<'a> {
    ...
    pub(super) fn checkpoint(&self) -> Result<(), LooseGitImportRefusal> {
        if let Some(code) = self.stopped.get() {
            return Err(interrupted(code));
        }
        let live = self
            .deadline
            .try_borrow_mut()
    ...
    pub(super) fn after<T, E>(
        &self,
        result: Result<T, E>,
        map: impl FnOnce(E) -> LooseGitImportRefusal,
    ...
    match checkpoint_pack_context(request.authority()) {
        ...
        PackContextCheckpoint::Stopped { budget_exhaustion } => {
            Err(LooseGitImportRefusal::Interrupted {
                code: if budget_exhaustion.is_some() {
...
pub(super) fn read_bytes(
    ...
    loop {
        ...
        bytes.try_reserve(count).map_err(|_| {
            ReadFailure::Interrupted(interrupted(RefusalCode::ResourceBudgetExceeded))
        })?;
...
pub(super) fn decode_loose(
    ...
    let mut decoder =
        ZlibLooseObjectDecoder::new(inflate_limits, parse_limits).map_err(decode_error)?;
...
impl CpuProbe<'_, '_> {
    fn poll(&mut self) -> bool {
        if self.control.stopped.get().is_some() {
            return false;
        }
        if self.remaining == 0 {
