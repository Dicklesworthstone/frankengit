...
use fgit_types::{
    DecisionOutcome, HeadGeneration, PrincipalId, RefusalCode, RepositoryId, TenantId,
};
use std::cell::Cell;
...
fn put(
    objects: &mut BTreeMap<GitOid, LooseObject>,
...
fn source(scratch: &Scratch, format: GitHashAlgorithm, packed: bool) -> Source {
    ...
    if packed {
        ...
        for (id, object) in &objects {
            ...
            pack.push(byte);
            pack.extend(stored(&object.body));
            entries.push((*id, crc32(&pack[offset..]), u32::try_from(offset).unwrap()));
        }
        let checksum = digest(format, &pack);
        ...
        for byte in 0_u16..256 {
            idx.extend(
                u32::try_from(
                    entries
                        .iter()
                        .filter(|(id, _, _)| u16::from(id.as_bytes()[0]) <= byte)
                        .count(),
                )
                .unwrap()
                .to_be_bytes(),
        ...
        for (id, _, _) in &entries {
            idx.extend(id.as_bytes());
        }
        for (_, crc, _) in &entries {
            idx.extend(crc.to_be_bytes());
        }
        for (_, _, offset) in &entries {
            idx.extend(offset.to_be_bytes());
        }
        idx.extend(checksum);
        let checksum = digest(format, &idx);
        idx.extend(checksum);
        let directory = root.join("objects/pack");
...
fn the_original_durable_api_uses_its_cancelled_request_before_source_io_and_sealing() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        for packed in [false, true] {
            ...
            let source = source(&scratch, format, packed);
            ...
            for path in [&source.root, &scratch.0.join("does-not-exist")] {
                assert!(
                    matches!(node.runtime().block_on(node.import_loose_git_directory_durable_in(
            ...
            let after = node
                .runtime()
                .block_on(node.materialize_admission_in(&fresh))
                .unwrap();
            ...
            let retry = reopened
                .runtime()
                .block_on(reopened.import_loose_git_directory_durable_in(
                    &request,
                    &source.root,
                    principal(),
                    b"controlled-import",
...
fn standalone_deadlines_cancel_before_reading_but_live_staging_remains_byte_identical() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let scratch = Scratch::new();
        let source = source(&scratch, format, false);
        let node = scratch.node(format);
        let stopped = node
            .stage_loose_git_import_with_deadline(&source.root, &mut || false)
            .unwrap_err();
        assert!(is_cancelled(&stopped));
        assert_absent(&node, &source);
        let staged = node
            .stage_loose_git_import_with_deadline(&source.root, &mut || true)
            .unwrap();
        ...
        for (id, object) in &source.objects {
            assert_eq!(
                node.read_git_object(*id).unwrap().payload(),
...
fn cancellation_during_staging_preserves_only_verified_noncanonical_objects() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let scratch = Scratch::new();
        let source = source(&scratch, format, true);
        let node = scratch.node(format);
        let first = *source.objects.keys().next().unwrap();
        let stopped = node
            .stage_loose_git_import_with_deadline(&source.root, &mut || {
                node.read_git_object(first).is_err()
            })
            .unwrap_err();
        assert!(is_cancelled(&stopped));
...
fn cancelled_pack_decoding_never_installs_a_partial_verified_cache_or_stages_objects() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let node = scratch.node(format);
        let mut live = || true;
        let setup = ImportControl::new(&mut live);
        let mut baseline =
            PackedObjectSources::open(&source.root, format, node.max_object_bytes, &setup).unwrap();
        ...
        let total = probes.get();
        assert!(total > 8);
        for stop_at in [1, 3, total / 2, total - 1] {
            let mut sources =
                PackedObjectSources::open(&source.root, format, node.max_object_bytes, &setup)
                    .unwrap();
            let probes = Cell::new(0);
            let mut deadline = || {
                probes.set(probes.get() + 1);
            ...
            assert!(
                sources
                    .sources
                    .iter()
                    .all(|source| source.verified_objects.is_none())
            );
            assert_absent(&node, &source);
        }
        node.shutdown().unwrap();
...
fn the_graph_loader_and_shared_edge_reader_cannot_restart_a_cancelled_walk() {
    ...
    let control = ImportControl::new(&mut deadline);
    let result = graph::validate_controlled(
        [tree],
        format,
        &parse_limits(format, 1024),
        graph::Limits::default(),
        |id| {
    ...
    let result = graph::validate_controlled(
        ...
        &parse_limits(format, 1024),
        graph::Limits::default(),
        |_| panic!("loader restarted after cancellation"),
        &control,
    );
    assert!(matches!(result, Err(ref error) if is_cancelled(error)));
    let mut live = || true;
    let control = ImportControl::new(&mut live);
    let valid = graph::validate_controlled(
        [tree],
        format,
        &parse_limits(format, 1024),
        graph::Limits::default(),
