...
fn markers(run: &str, client: &str) -> BTreeSet<GitOid> {
    ...
    match std::fs::read_to_string(&path) {
        Ok(text) => text
            .lines()
            .map(|line| {
                let format = match line.len() {
                    40 => GitHashAlgorithm::Sha1,
                    64 => GitHashAlgorithm::Sha256,
                    _ => panic!("invalid shallow file identity"),
                };
...
fn history(run: &str, client: &str) -> BTreeSet<String> {
    String::from_utf8(checked(command(run, "history", client, &[])))
        .unwrap()
        .lines()
        .map(str::to_owned)
        .collect()
...
fn append(node: &OneNode, parent: GitOid, foreign: GitOid, serial: usize) -> Revision {
    ...
    let blob = node
        .put_git_object(ObjectType::Blob, body)
        .unwrap()
        .identity();
    let tree = node
        .put_git_object(
            ...
            tree_bytes(&[
                ...
                (b"160000", b"submodule", foreign),
            ]),
        )
        .unwrap()
        .identity();
    let commit = node
        .put_git_object(ObjectType::Commit, commit_bytes(tree, &[parent]))
        .unwrap()
        .identity();
    ...
    let context = AdmissionContext {
        ...
        idempotency_key: IdempotencyKey::new(
            format!("shallow-oracle-append-{serial}").into_bytes(),
        )
        .unwrap(),
        object_format: node.object_format,
    };
...
fn pinned_git_shallow_clone_deepen_incremental_fetch_and_unshallow() {
    ...
    let output = Command::new(&oracle)
        .args(["create-run", "git-2.54.0", "shallow-node"])
        .output()
        .unwrap();
    assert!(
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let root_tree = git_object_id(
            format,
            GitObjectKind::Tree,
            &tree_bytes(&[(b"100644", b"common", root_blob)]),
        );
        let public_blob = git_object_id(format, GitObjectKind::Blob, b"current public content");
        let public_tree = git_object_id(
            format,
            GitObjectKind::Tree,
            &tree_bytes(&[
        ...
        let mut revisions = vec![
            (ancestor, root_tree, root_blob),
            (public, public_tree, public_blob),
        ];
        for serial in 0..2 {
            ...
            for (name, profile) in [
                ...
                let (returned, _) = live_client(
                    ...
                    command(
                        ...
                        "shallow-clone",
                        &client,
                        &[&endpoint, &repository, version, profile],
                    ),
                );
                node = returned;
                let mut expected = BTreeSet::from([tip.0.to_string()]);
                if name != "treeless" {
                    expected.insert(tip.1.to_string());
                }
                if name == "full" {
                    expected.insert(tip.2.to_string());
                }
                assert_eq!(
                    inventory(&run, &client),
                    expected,
                    "initial depth-one pack contains no ancestor content and obeys its filter"
                );
                ...
                let checkout = command(
                    ...
                    "checkout",
                    &client,
                    &[&endpoint, &repository, version, &tip.0.to_string()],
                );
                if name == "full" {
                    checked(checkout);
                } else {
                ...
                assert_eq!(
                    inventory(&run, &client),
                    BTreeSet::from([tip.0.to_string(), tip.1.to_string(), tip.2.to_string()]),
                    "checkout hydrates its own snapshot without crossing the shallow boundary"
                );
                let (returned, _) = live_client(
                    node,
                    &listener,
                ...
                assert_eq!(
                    history(&run, &client),
                    BTreeSet::from([tip.0.to_string(), old_parent.to_string()]),
                    "depth change on an already-common tip must supply its missing ancestor"
                );
                checked(command(&run, "fsck", &client, &[]));
                let context = node.request_context();
                let after_reads = node
                    .runtime()
                    .block_on(node.materialize_admission_in(&context))
                    .unwrap();
                assert_eq!(
                    before.basis(),
                    after_reads.basis(),
                    "clone, checkout and depth fetch never publish repository state"
                );
                let next = append(&node, tip.0, private, serial);
                serial += 1;
                revisions.push(next);
                let (returned, _) = live_client(
                    node,
                ...
                assert_eq!(
                    markers(&run, &client),
                    BTreeSet::from([old_parent]),
                    "ordinary incremental fetch retains the client's original boundary"
                );
                assert_eq!(
                    history(&run, &client),
                    BTreeSet::from([
                ...
                assert!(
                    !PathBuf::from(&run)
                        ...
                        .join(".git/shallow")
                        .exists(),
                    "unshallow removes the client's boundary file"
                );
                assert_eq!(
                    history(&run, &client),
                    revisions
                        ...
                        .collect(),
                    "unshallow supplies the entire native ancestry"
                );
                let present = inventory(&run, &client);
                assert!(
                    !present.contains(&private.to_string())
                        && !present.contains(&private_blob.to_string()),
                    "neither deleted-only history nor foreign gitlinks become public"
                );
                if name == "full" {
                    assert_eq!(
                        present,
                        revisions
                            ...
                            .collect(),
                        "full unshallow has the exact complete public object set"
                    );
                }
                checked(command(&run, "fsck", &client, &[]));
                eprintln!(
                    "PINNED_SHALLOW_CELL format={} protocol={} profile={} clone_checkout_deepen_incremental_unshallow=passed",
                    format.as_str(),
                    version,
                    name
                );
