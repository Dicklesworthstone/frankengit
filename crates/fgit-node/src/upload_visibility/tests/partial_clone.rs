...
fn minimum_tree_depth_is_shared_across_paths_and_selected_commit_roots() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let filtered = apply(
            &graph,
            &only_first,
            &request(vec![commit], vec![], Some(ObjectFilter::TreeDepth(2))),
        );
        assert_eq!(
            filtered.into_iter().collect::<BTreeSet<_>>(),
            BTreeSet::from([commit, root, deep, shared]),
        ...
        for wants in [vec![commit, other], vec![other, commit, other]] {
            let filtered = apply(
                &graph,
                &all,
                &request(wants, vec![], Some(ObjectFilter::TreeDepth(2))),
            );
            assert_eq!(
                filtered, all,
                "another selected commit seeds the shared tree at zero"
            );
        }
        let filtered = apply(
            &graph,
...
fn explicit_lazy_roots_override_filters_without_treating_a_have_commit_as_a_blob_have() {
    ...
    for filter in [
        ...
        assert_eq!(
            apply(&graph, &[], &request(vec![blob], vec![commit], filter)),
            vec![blob]
        );
    }
    assert_eq!(
        apply(&graph, &[], &request(vec![tree], vec![commit], None))
            .into_iter()
            .collect::<BTreeSet<_>>(),
        BTreeSet::from([tree, blob])
    );
    assert_eq!(
        apply(
            &graph,
            &[],
            &request(vec![tree], vec![commit], Some(ObjectFilter::TreeDepth(0)))
        ),
        vec![tree]
    );
    assert!(apply(&graph, &[], &request(vec![blob], vec![blob], None)).is_empty());
    assert!(apply(&graph, &[], &request(vec![commit], vec![commit], None)).is_empty());
    let missing = git_object_id(
    ...
    assert!(
        matches!(engine::apply_selection(&graph.objects, &mut ids, &request(vec![missing], vec![], None), &PackLimits::default(), &mut || true),
...
fn blob_limit_is_exclusive_including_empty_blobs_and_combines_by_intersection() {
    ...
    for (filter, expected) in [
        ...
        (
            ObjectFilter::BlobLimit(4),
            BTreeSet::from([commit, tree, empty, small]),
        ),
        (
            ObjectFilter::Combine(vec![ObjectFilter::BlobLimit(4), ObjectFilter::TreeDepth(1)]),
            BTreeSet::from([commit, tree]),
        ),
        (
            ObjectFilter::Combine(vec![ObjectFilter::BlobLimit(4), ObjectFilter::TreeDepth(2)]),
            BTreeSet::from([commit, tree, empty, small]),
        ),
    ] {
        assert_eq!(
            apply(&graph, &all, &request(vec![commit], vec![], Some(filter)))
                .into_iter()
                .collect::<BTreeSet<_>>(),
            expected
        );
...
fn every_partial_selection_stop_leaves_original_selection_untouched() {
    ...
    for stop in 1..=polls.get() {
        ...
        assert!(
            matches!(result, Err(NodePackMaterializationRefusal::Pack(error)) if matches!(*error, PackWriteError::Pack(PackError::DeadlineExceeded)))
        );
    ...
    for limits in [
        ...
        let mut ids = original.clone();
        assert!(matches!(
            engine::apply_selection(&graph.objects, &mut ids, &request, &limits, &mut || true),
...
fn unsupported_controls_fail_instead_of_silently_returning_an_unfiltered_pack() {
    ...
    for filter in [
        ObjectFilter::SparsePath(b"secret".to_vec()),
        ObjectFilter::SparseObject(blob),
        ObjectFilter::Combine(vec![
        ...
        assert!(matches!(
            engine::apply_selection(
                &graph.objects,
                &mut ids,
                &request(vec![blob], vec![], Some(filter)),
                &PackLimits::default(),
                &mut || true
            ),
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let metadata = visible
            ...
            .map(|id| {
                ...
                (
                    *id,
                    (object.envelope().object_kind(), object.payload().to_vec()),
                )
            })
            .collect::<BTreeMap<_, _>>();
        let tag_bodies =
            [tag, outer].map(|id| node.read_git_object(id).unwrap().payload().to_vec());
        ...
        for version in [0, 1, 2] {
            for filter in [
                ...
                Some("tree:1"),
                Some("tree:2"),
                Some("combine:tree%3A1+blob%3Anone"),
            ] {
                ...
                let (returned, result, response) =
                    exchange_filtered(node, version, &[public], None, filter, false);
                node = returned;
                assert!(
                    matches!(result, Ok(GitDaemonSessionOutcome::Pack(_))),
                    "{format:?} v{version} {filter:?}: {result:?}"
                );
                assert_eq!(
                    bodies(&response, version, format),
                    expected,
                ...
                let advertisement = if version == 2 {
                    ...
                    b"allow-reachable-sha1-in-want shallow deepen-relative deepen-since deepen-not filter".as_slice()
                };
                assert!(
                    response
                        .windows(advertisement.len())
                        .any(|bytes| bytes == advertisement)
                );
            }
            for filter in [None, Some("blob:none"), Some("tree:0")] {
                let (returned, result, response) =
                    exchange_filtered(node, version, &[public_blob], Some(public), filter, false);
                node = returned;
                assert!(
                    matches!(result, Ok(GitDaemonSessionOutcome::Pack(_))),
                    "lazy {format:?} v{version}: {result:?}"
                );
                assert_eq!(
                    bodies(&response, version, format),
            ...
            let (returned, result, response) =
                exchange_filtered(node, version, &[public], None, Some("blob:none"), true);
            node = returned;
            assert!(matches!(result, Ok(GitDaemonSessionOutcome::Pack(_))));
            let mut expected = metadata
                .values()
                .filter(|(kind, _)| *kind != fgit_object_fabric::ObjectKind::Blob)
                .map(|(_, body)| body.clone())
                .collect::<BTreeSet<_>>();
            ...
            for forbidden in [private, private_blob] {
                let (returned, result, response) =
                    exchange_filtered(node, version, &[forbidden], None, Some("blob:none"), false);
                node = returned;
                assert!(result.is_err());
                assert!(!response.windows(4).any(|bytes| bytes == b"PACK"));
            }
            let (returned, result, response) = exchange_filtered(
                node,
                version,
                &[public],
                None,
                Some("sparse:path=anything"),
                false,
            );
            node = returned;
            assert!(result.is_err());
        ...
        assert_eq!(
            before.basis(),
            after.basis(),
            "fetches never publish canonical state"
        );
        assert_eq!(
            after.selected_closure().closure().objects(),
            &historical_objects,
            "omission is not retention removal"
        );
        delete_named_ref(&node, public, b"refs/heads/public", b"partial-clone-revoke");
        assert!(node.read_git_object(public_blob).is_ok());
        for version in [0, 1, 2] {
            let (returned, result, response) =
                exchange_filtered(node, version, &[public_blob], None, None, false);
            node = returned;
            assert!(
                result.is_err(),
                "a previous partial-clone promise is not a permanent disclosure grant"
            );
            assert!(!response.windows(4).any(|bytes| bytes == b"PACK"));
            let (returned, result, response) =
                exchange_filtered(node, version, &[ancestor], None, Some("blob:none"), false);
            node = returned;
            assert!(matches!(result, Ok(GitDaemonSessionOutcome::Pack(_))));
            assert_eq!(
                bodies(&response, version, format).len(),
                2,
                "still-visible ancestor succeeds beside revoked lazy want"
            );
        }
        node.shutdown().unwrap();
        drop(scratch);
