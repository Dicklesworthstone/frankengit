...
fn pinned_git_relative_deepening_progresses_existing_full_and_partial_clones() {
    ...
    let output = Command::new(&oracle)
        .args(["create-run", "git-2.54.0", "relative-native"])
        .output()
        .unwrap();
    assert!(
            ...
            for (name, profile) in [
                ...
                for boundary in [middle.0, public] {
                    ...
                    assert_eq!(
                        history(&run, &client),
                        expected_history,
                        "each identical relative request must add another generation"
                    );
                ...
                assert_eq!(
                    before.basis(),
                    after.basis(),
                    "relative fetch never publishes repository state"
                );
                eprintln!(
                    "PINNED_RELATIVE_CELL format={} protocol={version} profile={name} repeated_deepen_checkout_completion=passed",
                    format.as_str()
                );
            }
        }
