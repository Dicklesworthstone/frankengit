...
pub(super) fn effective_depth(
    ...
    if !request.options.deepen_relative() {
        return Ok(request.deepen);
    }
    let Some(increment @ 1..=INFINITE_DEPTH) = request.deepen else {
        return Err(NodePackMaterializationRefusal::UnsupportedFetch(
            "relative deepening requires a positive bounded depth",
        ));
    };
    if request.deepen_since.is_some() || !request.deepen_not.is_empty() {
        return Err(NodePackMaterializationRefusal::UnsupportedFetch(
            "relative deepening cannot combine time/ref boundaries",
        ));
    }
    if increment == INFINITE_DEPTH {
        return Ok(Some(INFINITE_DEPTH));
    }
    ...
    let depth = increment
        .checked_add(offset)
        .filter(|depth| *depth <= INFINITE_DEPTH)
        .ok_or_else(budget)?;
    Ok(Some(depth))
}
