...
fn repeated_relative_deepening_crosses_successive_old_boundaries_without_resending_the_tip() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let first = graph.commit(&[root.0]);
        let second = graph.commit(&[first.0]);
        let tip = graph.commit(&[second.0]);
        for (old, next) in [(tip.0, second), (second.0, first), (first.0, root)] {
            let request = relative(tip.0, 1, vec![old], vec![tip.0]);
            assert_eq!(
                ids(&graph, &request),
                BTreeSet::from([next.0, next.1, next.2])
            );
            assert_eq!(
            ...
            let absolute = super::request(tip.0, Some(1), vec![old], vec![tip.0]);
        ...
        assert_eq!(
            update(&graph, &complete),
            ShallowUpdate {
                shallow: vec![],
                unshallow: vec![root.0]
            }
        );
    }
}
...
fn nearest_reachable_marker_defines_the_offset_not_input_order_or_a_foreign_history() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for old in [
            vec![root.0, second.0, unrelated.0, unknown],
            vec![unknown, unrelated.0, second.0, root.0],
        ] {
        ...
        assert_eq!(
            ids(&graph, &no_match),
            BTreeSet::from([tip.0, tip.1, tip.2])
        );
    }
}
...
fn relative_depth_does_not_turn_unknown_markers_into_permission_or_overflow_into_unshallow() {
    ...
    assert!(
        matches!(select(&graph.objects,&query,&PackLimits::default(),&mut || true),Err(NodePackMaterializationRefusal::RequestedWantOutsideClosure(id)) if id==unknown)
    );
    ...
    let mut orphan = relative(tip.0, 1, vec![tip.0], vec![]);
    orphan.deepen = None;
    assert!(matches!(
        select(&graph.objects, &orphan, &PackLimits::default(), &mut || {
            true
        }),
        Err(NodePackMaterializationRefusal::UnsupportedFetch(_))
    ));
}
...
fn relative_offset_and_selection_share_one_exact_work_and_cancellation_budget() {
    let mut graph = Graph::new(GitHashAlgorithm::Sha256);
    let root = graph.commit(&[]);
    let parent = graph.commit(&[root.0]);
    let tip = graph.commit(&[parent.0]);
    let query = relative(tip.0, 1, vec![parent.0], vec![tip.0]);
    let mut polls = 0;
    let expected = select(&graph.objects, &query, &PackLimits::default(), &mut || {
        polls += 1;
        true
    })
    .unwrap();
    let exact = PackLimits {
        max_delta_work: polls,
        ..PackLimits::default()
    };
    assert_eq!(
        select(&graph.objects, &query, &exact, &mut || true).unwrap(),
        expected
    );
    let short = PackLimits {
        max_delta_work: polls - 1,
        ..exact.clone()
    };
    assert!(matches!(
        select(&graph.objects, &query, &short, &mut || true),
        Err(NodePackMaterializationRefusal::DisclosureGraph(
            RefusalCode::ResourceBudgetExceeded
        ))
    ));
    for stop in 1..=polls {
        let mut seen = 0;
        assert!(
            matches!(select(&graph.objects,&query,&exact,&mut || {seen+=1;seen<stop}),Err(NodePackMaterializationRefusal::Pack(error)) if matches!(*error,PackWriteError::Pack(PackError::DeadlineExceeded)))
        );
    }
}
