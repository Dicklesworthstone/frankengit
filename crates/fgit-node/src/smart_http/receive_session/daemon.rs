...
use crate::{AdmissionReceivePackAdvertisement, DeadlineTcpStream, GitDaemonService,
    ...
    NodeReceiveTransportRefusal, NodeRequestContext, OneNode,
    ProductionReceiveQuarantineHandoff, read_git_daemon_request, read_receive_frame};
...
fn checkpoint(deadline: &GitDaemonSessionDeadline) -> Result<(), NodeSmartHttpRefusal> {
    deadline.check("guarded git-daemon ingress").map_err(NodeGitDaemonServeRefusal::from)?;
    Ok(())
}
...
fn is_receive(node: &OneNode, stream: &TcpStream, deadline: &GitDaemonSessionDeadline,
    limits: &WireLimits) -> Result<bool, NodeSmartHttpRefusal> {
    stream.set_nonblocking(true).map_err(|e| io_error("configure greeting peek", e))?;
    let result = (|| {
        ...
        loop {
            ...
            let read = match stream.peek(&mut bytes) {
                ...
                Ok(n) => n,
                Err(e) if matches!(e.kind(), io::ErrorKind::WouldBlock | io::ErrorKind::Interrupted) => 0,
                Err(e) => return Err(io_error("peek daemon greeting", e)),
            };
            if read >= 4 {
                let text = std::str::from_utf8(&bytes[..4]).map_err(|_| invalid("invalid greeting length"))?;
                let size = usize::from(u16::from_str_radix(text, 16).map_err(|_| invalid("invalid greeting length"))?);
                if !(4..=bytes.len()).contains(&size) { return Err(invalid("invalid greeting length")); }
                if read >= size {
                    let greeting = read_git_daemon_request(&mut Cursor::new(&bytes[..size]), limits)
    ...
    let restored = stream.set_nonblocking(false).map_err(|e| io_error("restore greeting socket", e));
    match result { Err(error) => Err(error), Ok(value) => restored.map(|()| value) }
}
...
impl OneNode {
    ...
    pub fn serve_guarded_git_daemon_stream(
        ...
        let ingress = GitDaemonSessionDeadline::new(self.git_daemon_session_timeout,
    ...
    fn serve_guarded_git_daemon_stream_in(
        ...
        if !is_receive(self, &stream, &ingress, &limits)? {
            return self.serve_git_daemon_stream_with_limits(stream, limits)
                .map(|_| None).map_err(NodeSmartHttpRefusal::from);
        }
        let mut output = stream.try_clone().map_err(|e| io_error("clone receive socket", e))?;
        ...
        let result = (|| {
            let principal = self.git_daemon_receive_principal
                .ok_or(NodeSmartHttpRefusal::UnauthenticatedReceive)?;
            shared_quota.unwrap_or(&self.push_quota).evaluate(&principal)?;
            ...
            context.limits.max_commands = context.limits.max_commands.min(AdmissionLimits::default().max_commands);
            let receive_limits = context.limits.clone();
            let advertisement_request = self.request_context();
            let mut live = || !ingress.expired();
            let selected = drive_request_while(self, &advertisement_request,
                self.materialize_admission_in(&advertisement_request), &mut live)
                .map_err(NodeAdmissionViewRefusal::from)?;
            let snapshot = selected.snapshot();
            let refs = AdmissionReceivePackAdvertisement::from_snapshot(snapshot,
                &snapshot.hidden_refs, self.object_format, &limits).map_err(NodeAdmissionViewRefusal::from)?;
            let advertised = advertise_receive_pack(refs.advertised_refs().to_vec(), &context)?;
            let bytes = encode_packets(&advertised, &limits)?;
            writer.write_all(&bytes).map_err(|e| io_error("write receive advertisement", e))?;
            writer.flush().map_err(|e| io_error("flush receive advertisement", e))?;
            drop(selected);
            ...
            let ready = 'commands: loop {
                ...
                prefix.try_reserve(raw.len()).map_err(|_| invalid("allocate receive prefix"))?;
                prefix.extend_from_slice(&raw);
                for event in machine.push_packet(packet)?.events {
                    if let ReceiveEvent::RequestReady(ready) = event { break 'commands ready; }
                }
            };
            let session = LoopbackReceiveSession::authenticated(principal,
                retry_key(greeting.repository_path().as_bytes(), &prefix)?);
            let mut input_bytes = prefix.len() as u64;
            drop(prefix);
            if ready.requires_pack() {
                let mut scanner = PackBoundaryScanner::new(self.object_format, receive_limits.pack.clone());
                ...
                loop {
                    ...
                    if count == 0 { return Err(invalid("truncated receive PACK")); }
                    let state = scanner.push(&chunk[..count], &mut || !ingress.expired())
                        .map_err(|e| NodeGitDaemonServeRefusal::from(GitDaemonTransportRefusal::ReceivePackFraming(e)))?;
                    if matches!(&state, ScanStatus::Finished { .. }) && !scanner.excess_bytes().is_empty() {
                        return Err(NodeSmartHttpRefusal::TrailingRequestBytes { count: scanner.excess_bytes().len() });
                    }
                    machine.push_bytes(&chunk[..count])?;
                    input_bytes = input_bytes.checked_add(count as u64).ok_or_else(|| invalid("receive size overflow"))?;
                    if matches!(&state, ScanStatus::Finished { .. }) { break; }
                }
            }
            ...
            let work = GitDaemonSessionDeadline::new(self.git_daemon_session_timeout,
                self.git_daemon_session_work_scaling);
            let processing = crate::GitDaemonReceiveProcessingDeadline::new(self.git_daemon_receive_processing_timeout);
            let request = NodeRequestContext { authority: self.receive_admission_authority_context(input_bytes, &work) };
            let mut live = || !work.expired() && !processing.expired();
            let selected = drive_request_while(self, &request, self.materialize_admission_in(&request), &mut live)
                .map_err(NodeAdmissionViewRefusal::from)?;
            let validator = self.production_quarantine_validator(&selected, receive_limits.pack.clone(),
                ParseLimits { tree_reference_bytes: self.object_format.digest_len(), ..ParseLimits::default() })
            ...
            let delivered: Result<(), NodeSmartHttpRefusal> = (|| {
                ...
                writer.write_all(&bytes).map_err(|e| io_error("write guarded receive report", e))?;
                writer.flush().map_err(|e| io_error("flush guarded receive report", e))
            })();
            match delivered {
                Ok(()) => Ok(Some(outcome)),
                Err(source) => Err(NodeSmartHttpRefusal::ReceiveResponse { outcome: Box::new(outcome), source: Box::new(source) }),
            }
        })();
...
mod tests {
    ...
    fn fatal_interruption_never_fabricates_per_ref_rejection() {
        ...
        assert!(bytes.windows(b"ERR receive outcome unknown".len()).any(|w| w == b"ERR receive outcome unknown"));
    ...
    fn raw_retry_selector_preserves_legacy_bytes_and_excludes_pack_representation() {
        ...
        preimage.extend_from_slice(route); preimage.push(0); preimage.extend_from_slice(commands);
        assert_eq!(retry_key(route, commands).unwrap().as_bytes(), fgit_crypto::sha256_digest(&preimage));
        assert_ne!(retry_key(route, commands).unwrap(), retry_key(b"/other.git", commands).unwrap());
