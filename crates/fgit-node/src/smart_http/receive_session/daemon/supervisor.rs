...
use super::{GitDaemonSessionDeadline, NodeSmartHttpRefusal, io_error, invalid};
...
impl Drop for Completion {
    fn drop(&mut self) {
        if self.success { self.completed.fetch_add(1, Ordering::Relaxed); }
        else { self.refused.fetch_add(1, Ordering::Relaxed); }
...
impl OneNode {
    ...
    pub fn serve_guarded_git_daemon_bounded(
        &self, listener: &TcpListener, limits: GitDaemonServerLimits,
        ...
        listener.set_nonblocking(true).map_err(|e| io_error("configure guarded daemon listener", e))?;
        while accepted < limits.max_sessions() {
            ...
            let (stream, _) = match listener.accept() {
                ...
                Err(e) => { failure = Some(io_error("accept guarded daemon connection", e)); break; }
            };
            accepted += 1;
            let finished = Arc::new(AtomicBool::new(false));
            let completion = Completion { finished: Arc::clone(&finished), completed: Arc::clone(&completed),
                refused: Arc::clone(&refused), success: false };
            ...
            let task = self.runtime.submit_blocking(move || {
                ...
                let mut node = match OneNode::open_existing(config) {
                    Ok(node) => node,
                    Err(error) => { eprintln!("guarded daemon child open failed: {error}"); return; }
                };
                let served: Result<(), NodeSmartHttpRefusal> = (|| {
                    let selected = node.runtime().block_on(node.authenticate_authority_head())
                        .map_err(|e| io_error("authenticate guarded daemon child", std::io::Error::other(e.to_string())))?;
                    node.bring_into_service(selected.receipt().generation())
                        .map_err(|e| io_error("bring guarded daemon child into service", std::io::Error::other(e.to_string())))?;
                    node.serve_guarded_git_daemon_stream_in(stream, deadline, Some(quota.as_ref()))?;
                    Ok(())
                })();
                ...
                if let Err(error) = served { eprintln!("guarded daemon connection failed: {error}"); }
                if let Err(error) = cleanup { eprintln!("guarded daemon child cleanup failed: {error}"); }
            });
            match task {
                Ok(task) => pending.push(Pending { finished, join: Box::new(move || { task.wait(); }) }),
                Err(error) => {
                    failure = Some(io_error("schedule guarded daemon connection", std::io::Error::other(error.to_string())));
                    break;
                }
            }
        }
        for child in pending { (child.join)(); }
        ...
        if let Some(error) = failure {
            if let Err(cleanup) = restored {
                return Err(io_error("guarded daemon failed with listener cleanup failure",
        ...
        if completed_sessions.checked_add(refused_sessions) != Some(accepted) {
            return Err(invalid("guarded daemon did not settle every accepted child"));
        }
        Ok(GitDaemonServerReceipt { accepted_sessions: accepted, completed_sessions, refused_sessions })
    }
}
