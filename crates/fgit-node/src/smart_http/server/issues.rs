...
use fgit_wire::smart_http::{
    BodyDecoder, BodyFraming, HttpError, HttpLimits, HttpVersion, head::Envelope,
};
...
pub(super) fn parse_decimal(text: &str) -> Result<u64, ApiError> {
    request::decimal(text)
}
pub(super) fn parse_page(query: Option<&str>, cursor: &str) -> Result<Page, ApiError> {
    request::page(query, cursor)
}
pub(super) fn parse_snapshot(
    text: &str,
) -> Result<fgit_types::RepositoryAuthorityHeadId, ApiError> {
...
impl ApiError {
    ...
    pub(super) fn bad(code: &'static str) -> Self {
        Self::new(Status::BadRequest, code)
    }
    pub(super) fn not_found() -> Self {
        Self::new(Status::NotFound, "not_found")
    }
    pub(super) fn too_large() -> Self {
        Self::new(Status::TooLarge, "resource_limit")
    }
    pub(super) fn media() -> Self {
        Self::new(Status::MediaType, "unsupported_media_type")
    }
    pub(super) fn method() -> Self {
        Self::new(Status::Method, "method_not_allowed")
    }
    pub(super) fn unavailable() -> Self {
        Self::new(Status::Unavailable, "repository_unavailable")
    }
    ...
    pub(super) fn from_status(status: Status, mutation: bool) -> Self {
        ...
        Self {
            status,
            code,
            outcome_unknown: mutation && status == Status::Unavailable,
        }
    }
    ...
    pub(super) fn send_named(
        ...
        let body = format!(
            "{{\"type\":{},\"schema_version\":1,\"code\":{},\"outcome_unknown\":{}}}",
            output::quote(family),
            output::quote(self.code),
...
pub(super) fn authenticate(
    ...
    let key = if request.is_mutation() {
        ...
        b"issue-read-no-publication".as_slice()
    };
    let key =
...
pub(super) fn execute(
    ...
    let command = if request.is_mutation() {
        Some(request.command(&read_form(reader, framing, limits)?)?)
    } else {
        None
    };
    ...
    match &request.operation {
        Operation::List(page) => {
            let result = drive_request_while(
                node,
                &context,
                node.read_issues_in(&context, page.after, page.limit, page.expected_head),
                &mut live,
            )?;
            Ok(Reply {
        ...
        Operation::Show { number, page } => {
            ...
            Ok(Reply {
                status: if result.issue.is_some() {
                    Status::Success
                } else {
                    Status::NotFound
                },
                body: output::history(node, *number, *page, &result, maximum)?,
                terminal: None,
            })
        }
        Operation::Search => {
            let (query, parameters) = search::parse(&read_form(reader, framing, limits)?)?;
            let result = fgit_forge::issue_search::search(
                &query,
                parameters,
                |after, limit, expected_head| {
                    let page = drive_request_while(
                        node,
                        &context,
                        node.read_issues_in(&context, after, limit, expected_head),
                        &mut live,
                    )
            ...
            Ok(Reply {
                status: Status::Success,
                body: output::search(node, parameters, &query, &result, maximum, &mut live)?,
                terminal: None,
            })
        }
        Operation::Mutate { .. } => {
            ...
            command
                ...
                .map_err(|_| ApiError::bad("invalid_issue_command"))?;
            let (tx, terminal) = drive_request_while(
                node,
                &context,
                node.admit_issue_durable_in(&context, session, &command, Default::default()),
                &mut live,
            )
...
pub(super) fn read_form(
    ...
    let limits = HttpLimits {
        ...
        max_body_wire_bytes: limits
            .max_body_wire_bytes
            .min((request::MAX_FORM_BYTES + 64 * 1024) as u64),
...
mod tests {
    ...
    fn error_replies_are_json_self_delimited_and_do_not_invent_terminal_outcomes() {
        let mut out = Vec::new();
        ApiError::unknown()
            .send(&mut out, HttpVersion::Http11)
            .unwrap();
