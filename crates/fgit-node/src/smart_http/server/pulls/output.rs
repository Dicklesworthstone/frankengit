...
pub(super) fn action(value: PullRequestAction) -> &'static str {
    match value { PullRequestAction::Open => "open", PullRequestAction::Update => "update", PullRequestAction::Close => "close" }
}
...
fn view(value: &PullRequestView) -> Result<String, ApiError> {
    if value.event.aggregate != AggregateId::PullRequest(value.number) { return Err(ApiError::unavailable()); }
    let (state, merge) = match &value.event.payload {
        ForgeEventPayload::PullRequestChangedNative(change) => {
            if value.data.as_ref() != Some(&change.data) { return Err(ApiError::unavailable()); }
            (if change.action == PullRequestAction::Close { "closed" } else { "open" }, "null".to_owned())
        }
    ...
    Ok(format!(concat!("{{\"number\":{},\"version\":{},\"state\":{},\"data\":{},",
        "\"opened_by\":{},\"last_metadata_actor\":{},\"merge_only\":{},\"merge\":{}}}"),
        value.number.get(), value.event.version.get(), quote(state),
        value.data.as_ref().map(data).transpose()?.unwrap_or_else(|| "null".to_owned()),
        value.opened_by.map_or_else(|| "null".to_owned(), |id| quote(&id.to_string())),
        value.last_metadata_actor.map_or_else(|| "null".to_owned(), |id| quote(&id.to_string())),
        value.data.is_none(), merge))
}
fn header(node: &OneNode, head: RepositoryAuthorityHeadId,
    expected: Option<RepositoryAuthorityHeadId>,
) -> Result<String, ApiError> {
    if expected.is_some_and(|expected| expected != head) { return Err(ApiError::snapshot_moved()); }
    let id = head.as_internal_object_id();
    let digest = id.digest().as_bytes().iter().map(|byte| format!("{byte:02x}")).collect::<String>();
    ...
    Ok(format!(concat!("\"schema_version\":1,\"tenant_id\":{},\"repository_id\":{},",
        ...
        quote(&head.to_string()), quote(&token)))
}
fn append(out: &mut String, part: &str, maximum: usize) -> Result<(), ApiError> {
    if out.len().checked_add(part.len()).is_none_or(|size| size > maximum) {
...
pub(super) fn list(node: &OneNode, page: Page, result: &PullRequestPage, maximum: usize) -> Result<String, ApiError> {
    validate_page(page, result)?;
    let mut out = String::new();
    append(&mut out, &format!("{{\"type\":\"pull_request_page\",{},\"after\":{},\"limit\":{},\"next_after\":{},\"pull_requests\":[",
        header(node, result.source_head, page.expected_head)?, page.after, page.limit,
        result.next_after.map_or_else(|| "null".to_owned(), |number| number.to_string())), maximum)?;
...
pub(super) fn show(node: &OneNode, number: PullRequestNumber,
    expected: Option<RepositoryAuthorityHeadId>, result: &PullRequestPage, maximum: usize,
) -> Result<(bool, String), ApiError> {
    validate_page(Page { after: number.get() - 1, limit: 1, expected_head: expected }, result)?;
    ...
    let row = result.pull_requests.first().filter(|row| row.number == number);
    let mut out = String::new();
    append(&mut out, &format!("{{\"type\":\"pull_request\",{},\"number\":{},\"found\":{},\"pull_request\":{} }}",
        header(node, result.source_head, expected)?, number.get(), row.is_some(),
        row.map(view).transpose()?.unwrap_or_else(|| "null".to_owned())), maximum)?;
    Ok((row.is_some(), out))
}

pub(super) fn mutation(node: &OneNode, principal: PrincipalId, command: &PullRequestCommand,
    ...
    let expected = match command.expected_version { ExpectedVersion::NewStream => 0,
        ExpectedVersion::Exactly(version) => version.get() };
    format!(concat!("{{\"type\":\"pull_request_publication\",\"schema_version\":1,\"tenant_id\":{},",
        ...
        quote(&node.tenant_id.to_string()), quote(&node.repository_id.to_string()),
        quote(&node.repository_incarnation_id().to_string()), quote(node.object_format.as_str()), quote(&principal.to_string()),
        command.number.get(), expected, quote(action(command.action)), quote(&tx.to_string()), quote(outcome),
        terminal.decision_sequence.get(), commit, refusal, code, code_point)
}
...
mod tests {
    ...
    fn snapshots_preserve_untrusted_text_without_inventing_lifecycle_state() {
        ...
        let command = PullRequestCommand { number: PullRequestNumber::FIRST,
            ...
            data: PullRequestData { source_ref: RefName::try_new(b"refs/heads/topic").unwrap(),
                ...
                title: "Untrusted <script>".into(), body: "é\n\"\\\u{202e}".into() } };
        let mut row = PullRequestView { number: command.number, event: command.proposed_event(actor, format).unwrap(),
            data: Some(command.data.clone()), opened_by: Some(actor), last_metadata_actor: Some(actor) };
