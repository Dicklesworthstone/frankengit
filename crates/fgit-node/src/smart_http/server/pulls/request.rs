...
use super::super::issues::{ApiError, Page, MAX_FORM_BYTES, parse_decimal, parse_form,
    parse_page, parse_snapshot};

#[derive(Debug)]
pub(super) enum Operation {
    List(Page),
    Show { number: PullRequestNumber, expected_head: Option<RepositoryAuthorityHeadId> },
    Mutate { number: PullRequestNumber, action: PullRequestAction },
}
...
impl<'a> Request<'a> {
    pub(in crate::smart_http::server) fn parse(head: &Envelope<'a>) -> Result<Option<Self>, ApiError> {
        ...
        if repository_route.len() < 2 || !repository_route.starts_with('/')
            || repository_route[1..].split('/').any(|part| part.is_empty()
                || matches!(part, "." | "..")
        ...
        let operation = match head.method {
            "GET" => {
                if !matches!(head.body, BodyFraming::Empty | BodyFraming::ContentLength(0))
                ...
                if suffix.is_empty() {
                    ...
                    for (key, value) in parse_form(query.unwrap_or("").as_bytes(), 1)? {
                        if key != "expected_head" { return Err(ApiError::bad("unknown_query_field")); }
            ...
            "POST" => {
                ...
                let (number_text, action) = suffix.strip_prefix('/').and_then(|suffix| suffix.split_once('/'))
                ...
                Operation::Mutate { number: number(number_text)?, action }
            }
    ...
    pub(super) fn command(&self, bytes: &[u8], format: GitHashAlgorithm) -> Result<PullRequestCommand, ApiError> {
        ...
        for (key, value) in parse_form(bytes, 8)? {
            if !matches!(key.as_str(), "expected_version" | "object_format" | "source_ref"
                | "target_ref" | "source_tip" | "target_tip" | "title" | "body")
            {
                return Err(ApiError::bad("unknown_or_inapplicable_field"));
            }
            if fields.insert(key, value).is_some() { return Err(ApiError::bad("duplicate_field")); }
        }
        let declared = take(&mut fields, "object_format")?;
        if declared != format.as_str() { return Err(ApiError::bad("object_format_mismatch")); }
        ...
        let data = PullRequestData { source_ref, target_ref, source_tip, target_tip,
            title: take(&mut fields, "title")?, body: take(&mut fields, "body")? };
        data.validate().map_err(|_| ApiError::bad("invalid_pull_request_content"))?;
        Ok(PullRequestCommand { number, expected_version, action, data })
    }
}
...
mod tests {
    ...
    fn fields(format: GitHashAlgorithm) -> String {
        format!("expected_version=0&object_format={}&source_ref=refs%2Fheads%2Ftopic&target_ref=refs%2Fheads%2Fmain&source_tip={}&target_tip={}&title=Review+%F0%9F%A6%80&body=%252f%0A%22",
            format.as_str(), "a".repeat(format.digest_len() * 2), "b".repeat(format.digest_len() * 2))
    }
    fn command(action: &str, body: &str, format: GitHashAlgorithm) -> Result<PullRequestCommand, ApiError> {
        let text = format!("POST /repo.git/api/v1/pulls/7/{action} HTTP/1.1\r\nHost: local\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: {}\r\n\r\n", body.len());
        let head = head::parse(text.as_bytes(), HttpLimits::default()).unwrap().unwrap();
        Request::parse(&head)?.unwrap().command(body.as_bytes(), format)
    }
    #[test]
    fn native_commands_keep_exact_versions_tips_and_decoded_text_in_both_formats() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            ...
            for action in ["update", "close"] {
                let next = command(action, &body.replace("expected_version=0", "expected_version=1"), format).unwrap();
                assert_eq!(next.data, opened.data);
                assert_eq!(next.expected_version, ExpectedVersion::Exactly(AggregateVersion::FIRST));
            }
        }
    }
    #[test]
    fn commands_cannot_inject_authority_omit_preconditions_or_change_hash_domains() {
        ...
        for body in [good.clone() + "&principal=admin", good.clone() + "&title=other",
            good.replace("expected_version=0", "expected_version=00"),
            good.replace("expected_version=0&", ""), good.replace("object_format=sha1", "object_format=sha256"),
            good.replace(&"a".repeat(40), &"0".repeat(40)), good.replace("body=%252f%0A%22", "body=%FF"),
            good.replace("body=%252f%0A%22", "body=%00"), good.replace("body=%252f%0A%22", "body=%"),
            good.replace("source_ref=refs%2Fheads%2Ftopic", "source_ref=refs%2Ftags%2Ftopic")]
        ...
        assert!(command("update", &good.replace("expected_version=0", "expected_version=18446744073709551615"), format).is_err());
    }
    #[test]
    fn read_envelopes_require_pinned_continuations_and_reject_inapplicable_parameters() {
        for suffix in ["?after=1", "?limit=101", "?limit=1&limit=2", "/0", "/01", "/1?after=2", "/1?principal=admin"] {
            let bytes = format!("GET /repo.git/api/v1/pulls{suffix} HTTP/1.1\r\nHost: local\r\n\r\n");
        ...
        for extra in ["Content-Length: 1\r\n", "Expect: 100-continue\r\n", "Git-Protocol: version=2\r\n"] {
            let bytes = format!("GET /repo.git/api/v1/pulls HTTP/1.1\r\nHost: local\r\n{extra}\r\n");
            let head = head::parse(bytes.as_bytes(), HttpLimits::default()).unwrap().unwrap();
