...
impl<'a> Request<'a> {
    pub(super) fn parse(head: &Envelope<'a>) -> Result<Option<Self>, ApiError> {
        ...
        let Some((repository_route, tail)) = path.split_once("/api/v1/pulls/") else { return Ok(None); };
        let Some(number) = tail.strip_suffix("/diff") else { return Ok(None); };
...
pub(super) fn authenticate(request: &Request<'_>, envelope: &Envelope<'_>,
    ...
    Ok(LoopbackReceiveSession::authenticated(grant.principal,
        IdempotencyKey::new(b"read-only-pull-request-diff".to_vec()).map_err(|_| ApiError::unavailable())?))
}
...
pub(super) fn execute(node: &OneNode, request: &Request<'_>, session: &LoopbackReceiveSession,
    ...
    review::execute_pull(node, request.number, session, framing, reader, limits, maximum_response)
}
...
mod tests {
    ...
    fn only_exact_pr_diff_routes_accept_query_bodies() {
        for (method, target, extra, accepted) in [
            ...
            ("POST", "/r.git/api/v1/pulls/1/diff", "Git-Protocol: version=2\r\n", false),
        ] {
