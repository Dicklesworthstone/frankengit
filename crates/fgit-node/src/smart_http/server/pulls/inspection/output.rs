...
use fgit_forge::review::{
    ChangeKind, ComparisonMode, EntryIdentity, ReviewContent, ReviewSpan, ReviewedEntry,
};
...
fn entry<C: FnMut() -> bool>(out: &mut Json<'_, C>, value: &ReviewedEntry) -> Result<(), ApiError> {
    if value.path.is_empty() || value.path.len() > 4096 {
        return Err(ApiError::unavailable());
    }
    out.put("{\"path_hex\":")?;
    out.hex(&value.path)?;
...
pub(super) fn build(
    ...
    out.put(&format!(
        concat!(
            "}},\"comparison\":{{\"mode\":\"direct\",\"before\":{},\"after\":{},",
