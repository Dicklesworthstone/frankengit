...
use fgit_forge::event::review::{CandidateBinding, CandidateReviewCommand, ReviewCommand, ReviewDecision, ReviewSubject};
use fgit_types::{GitHashAlgorithm, GitOid, PolicyEpoch, PrincipalId, RefName, RepositoryAuthorityHeadId};
...
#[derive(Clone, Copy, Debug)]
pub(crate) enum Operation { List(Page), Review(ReviewDecision), Merge }
#[derive(Clone, Copy, Debug)]
pub(crate) enum Encoding<'a> { Form, Multipart(&'a str) }
#[derive(Debug)]
...
impl<'a> Request<'a> {
    pub(crate) fn parse(head: &Envelope<'a>) -> Result<Option<Self>, ApiError> {
        ...
        if action != "merge" && action != "reviews" && !action.starts_with("reviews/") { return Ok(None); }
        ...
        let operation = match action {
            "reviews" => {
                if head.method != "GET" { return Err(ApiError::method()); }
            ...
            action => {
                ...
                if query.is_some() || head.body == BodyFraming::Empty { return Err(ApiError::bad("invalid_mutation_envelope")); }
                match action {
        ...
        let encoding = if matches!(operation, Operation::List(_)) {
            ...
            let encoding = if content_type.eq_ignore_ascii_case("application/x-www-form-urlencoded")
                || content_type.eq_ignore_ascii_case("application/x-www-form-urlencoded; charset=utf-8") {
                Encoding::Form
            } else {
                if matches!(operation, Operation::Review(ReviewDecision::Withdraw)) { return Err(ApiError::media()); }
                Encoding::Multipart(multipart::boundary(content_type).map_err(|_| ApiError::media())?)
            };
            let maximum = match encoding { Encoding::Form => multipart::MAX_COMMAND_BYTES,
        ...
        Ok(Some(Self { repository_route, number, operation, encoding }))
    }
    pub(crate) fn is_mutation(&self) -> bool { !matches!(self.operation, Operation::List(_)) }

    pub(super) fn command(&self, bytes: &[u8], format: GitHashAlgorithm, principal: PrincipalId) -> Result<Command, ApiError> {
        ...
        for (name, value) in parse_form(bytes, 41)? {
            if name == "required_reviewer" && matches!(self.operation, Operation::Merge) {
                ...
                let common = matches!(name.as_str(), "object_format" | "pull_request_version" | "policy_epoch"
                    | "source_ref" | "target_ref" | "source_tip" | "target_tip" | "merge_base" | "candidate_commit");
                let review = matches!(self.operation, Operation::Review(_)) && matches!(name.as_str(), "expected_version" | "reason");
                if !common && !review { return Err(ApiError::bad("unknown_or_inapplicable_field")); }
        ...
        match self.operation {
            Operation::Review(decision) => {
                ...
                let command = CandidateReviewCommand { candidate,
                    review: ReviewCommand { expected_version, subject, decision, reason: take(&mut fields, "reason")? } };
                command.proposed_event(principal, format).map_err(|_| ApiError::bad("invalid_review_command"))?;
                Ok(Command::Review(command))
            }
            Operation::Merge => {
                if reviewers.contains(&principal) { return Err(ApiError::bad("submitter_cannot_review_own_merge")); }
                let required = ReviewRequirements::new(subject.policy_epoch, reviewers)
                    .map_err(|_| ApiError::bad("invalid_required_reviewers"))?;
                Ok(Command::Merge { subject, candidate, required })
            }
...
pub(super) enum Command {
    ...
    Merge { subject: ReviewSubject, candidate: CandidateBinding, required: ReviewRequirements },
}
fn take(fields: &mut BTreeMap<String, String>, name: &'static str) -> Result<String, ApiError> {
    fields.remove(name).ok_or_else(|| ApiError::bad("required_field_missing"))
}
fn oid(text: &str, format: GitHashAlgorithm) -> Result<GitOid, ApiError> {
    if text.len() != format.digest_len() * 2 || !text.bytes().all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b)) {
        return Err(ApiError::bad("invalid_object_id"));
    }
    let oid = GitOid::from_hex(format, text).map_err(|_| ApiError::bad("invalid_object_id"))?;
    if oid.is_zero() { return Err(ApiError::bad("invalid_object_id")); }
    Ok(oid)
}
...
fn page(query: Option<&str>) -> Result<Page, ApiError> {
    ...
    Ok(Page { after, limit: limit as u16, expected_head })
}
...
mod tests {
    use super::*;
    fn fields() -> String {
        format!("object_format=sha1&pull_request_version=1&policy_epoch=1&source_ref=refs%2Fheads%2Ftopic&target_ref=refs%2Fheads%2Fmain&source_tip={}&target_tip={}&merge_base={}&candidate_commit={}",
            "a".repeat(40), "b".repeat(40), "c".repeat(40), "d".repeat(40))
    }
    fn request(operation: Operation) -> Request<'static> {
        Request { repository_route: "/repo.git", number: PullRequestNumber::FIRST, operation, encoding: Encoding::Form }
    }
    #[test]
    fn reviewer_identity_is_credential_owned_and_subject_is_not_refreshed() {
        ...
        let Command::Review(command) = request.command(body.as_bytes(), GitHashAlgorithm::Sha1, actor).unwrap() else { panic!("review"); };
        assert_eq!(command.review.reason, "exact\ncandidate");
        assert_eq!(command.review.subject.pull_request_version, AggregateVersion::FIRST);
        for extra in ["&reviewer=admin", "&force=true", "&expected_version=1", "&required_reviewer=admin"] {
            assert!(request.command((body.clone() + extra).as_bytes(), GitHashAlgorithm::Sha1, actor).is_err());
        }
        assert!(request.command(body.as_bytes(), GitHashAlgorithm::Sha256, actor).is_err());
    }
    #[test]
    fn merge_requires_a_nonempty_distinct_non_submitter_reviewer_set() {
        let actor = PrincipalId::from_bytes([3; 16]);
        let request = request(Operation::Merge);
        for extra in [String::new(), format!("&required_reviewer={actor}"),
            format!("&required_reviewer={0}&required_reviewer={0}", "44".repeat(16))] {
            assert!(request.command((fields() + &extra).as_bytes(), GitHashAlgorithm::Sha1, actor).is_err());
        }
        let body = fields() + &format!("&required_reviewer={}", "44".repeat(16));
        assert!(matches!(request.command(body.as_bytes(), GitHashAlgorithm::Sha1, actor).unwrap(), Command::Merge { .. }));
    }
    #[test]
    fn withdrawal_version_and_review_pagination_are_explicit() {
        let request = request(Operation::Review(ReviewDecision::Withdraw));
        let actor = PrincipalId::from_bytes([3; 16]);
        assert!(request.command((fields() + "&expected_version=0&reason=withdraw").as_bytes(), GitHashAlgorithm::Sha1, actor).is_err());
        assert!(request.command((fields() + "&expected_version=1&reason=withdraw").as_bytes(), GitHashAlgorithm::Sha1, actor).is_ok());
