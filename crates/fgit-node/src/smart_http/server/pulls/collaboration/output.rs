...
fn subject(value: &ReviewSubject) -> String {
    format!(
        concat!(
            "{{\"number\":{},\"pull_request_version\":{},\"policy_epoch\":{},",
            "{},{},\"source_tip\":{},\"target_tip\":{}}}"
        ),
        value.pull_request.get(),
        value.pull_request_version.get(),
...
fn freshness(value: ReviewFreshness) -> &'static str {
    match value {
        ...
        ReviewFreshness::SourceMoved => "source_moved",
        ReviewFreshness::TargetMoved => "target_moved",
...
pub(super) fn page(
    ...
    if let Some(result) = result {
        if result.pull_request != number
            ...
            || requested
                .expected_head
                .is_some_and(|head| head != result.source_head)
            || result.reviews.iter().any(|row| {
                requested
                    .after
                    .is_some_and(|after| row.event.reviewer <= after)
            })
            || result
                .reviews
                .windows(2)
                .any(|rows| rows[0].event.reviewer >= rows[1].event.reviewer)
    ...
    append(
        ...
        &format!(
            ...
            result.map_or_else(
                || "null".into(),
                |page| page.pull_request_version.get().to_string()
            ),
            result.map_or_else(|| "null".into(), |page| page.policy_epoch.get().to_string()),
            principal(requested.after),
            requested.limit,
            principal(result.and_then(|page| page.next_after))
        ),
        maximum,
    )?;
    if let Some(page) = result {
        for (index, row) in page.reviews.iter().enumerate() {
            ...
            if event.subject.pull_request != number
                || event.subject.source_tip.algorithm() != node.object_format
            {
                return Err(ApiError::unavailable());
            }
            let candidate = event.candidate.map_or_else(
                || "null".into(),
                |binding| {
                    format!(
                        "{{\"merge_base\":{},\"candidate_commit\":{}}}",
                        quote(&binding.merge_base.to_string()),
                        quote(&binding.commit.to_string())
                    )
                },
            );
            append(
...
pub(super) fn mutation(
    ...
    format!(
        concat!(
            "{{\"type\":{}, {},\"principal_id\":{},\"action\":{},\"review_expected_version\":{},",
