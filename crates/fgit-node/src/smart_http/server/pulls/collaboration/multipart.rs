...
fn part_head(head: &[u8]) -> Result<(&str, &str), Error> {
    ...
    for field in fields {
        ...
        if key.eq_ignore_ascii_case("name") && name.is_none() {
            name = Some(value);
        } else if key.eq_ignore_ascii_case("filename") && !filename {
            filename = true;
        } else {
            return Err(Error::Framing);
        }
...
fn prefix_table(marker: &[u8]) -> Vec<usize> {
    ...
    for index in 1..marker.len() {
        while matched > 0 && marker[index] != marker[matched] {
...
fn find_boundary(
    ...
    for (offset, &byte) in bytes[start..].iter().enumerate() {
        ...
        if matched == marker.len() {
            ...
            if matches!(
                bytes.get(after..after.saturating_add(2)),
                Some(b"\r\n" | b"--")
            ) {
...
pub(super) fn visit_parts<'a>(
    ...
    for _ in 0..maximum_parts {
        ...
        let head_end = tail[..tail.len().min(MAX_PART_HEAD + 4)]
            .windows(4)
            .position(|v| v == b"\r\n\r\n")
            .ok_or(Error::Framing)?;
...
pub(super) fn parse<'a>(
    ...
    visit_parts(bytes, boundary, MAX_UPLOAD_BYTES, 2, live, |part| {
        match part.name {
            "command" if command.is_none() && command_media(part.media) => {
                if part.content.len() > MAX_COMMAND_BYTES {
                    return Err(Error::Limit);
                }
            ...
            "bundle"
                if bundle.is_none()
                    && (part.media.eq_ignore_ascii_case("application/x-git-bundle")
                        || part.media.eq_ignore_ascii_case("application/octet-stream")) =>
            {
                if part.content.len() > MAX_BUNDLE_BYTES {
                    return Err(Error::Limit);
                }
    ...
    Ok(Upload {
        command: command.ok_or(Error::Framing)?,
        bundle,
    })
}
...
mod tests {
    ...
    fn upload(reverse: bool) -> Vec<u8> {
        ...
        let mut bytes = if reverse {
            [bundle, command].concat()
        } else {
            [command, bundle].concat()
        };
    ...
    fn binary_bundle_and_text_are_borrowed_exactly_in_either_part_order() {
        ...
        assert_eq!(
            boundary("multipart/form-data; boundary=\"example\"").unwrap(),
            "example"
        );
    }
    #[test]
    fn truncation_duplicates_epilogues_and_transfer_encoding_fail_closed() {
        ...
        for end in 0..bytes.len() - 2 {
            assert!(parse(&bytes[..end], "example", &mut || true).is_err());
        }
        let mut extra = bytes.clone();
        extra.extend_from_slice(b"HTTP/1.1");
    ...
    fn shared_framing_does_not_widen_the_existing_bundle_profile() {
        ...
        visit_parts(&bytes, "example", bytes.len(), 3, &mut || true, |part| {
            parts.push((part.name, part.content));
            Ok(())
        })
        .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            visit_parts(&bytes, "example", bytes.len(), 2, &mut || true, |_| Ok(())).unwrap_err(),
            Error::Limit
        );
        assert_eq!(
            visit_parts(&bytes, "example", bytes.len() - 1, 3, &mut || true, |_| Ok(
                ()
            ))
            .unwrap_err(),
            Error::Limit
