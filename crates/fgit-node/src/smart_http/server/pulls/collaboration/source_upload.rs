...
pub(in crate::smart_http::server) enum SourceUploadKind {
    Patch,
    Bundle,
}
...
pub(in crate::smart_http::server) fn source_upload<'a>(
    ...
    multipart::visit_parts(bytes, boundary, kind.maximum(), 2, live, |part| {
        if part.name == "command" && command.is_none() && multipart::command_media(part.media) {
            ...
            if part.content.len() > kind.payload_maximum() {
                return Err(multipart::Error::Limit);
            }
    ...
    let command = command
        ...
        .ok_or_else(|| ApiError::bad("source_command_required"))?;
    let payload = payload
        .filter(|bytes| !bytes.is_empty())
        .ok_or_else(|| ApiError::bad("source_payload_required"))?;
    Ok((command, payload))
}
...
mod tests {
    ...
    fn exact_patch_and_bundle_bytes_survive_both_part_orders() {
        for kind in [SourceUploadKind::Patch, SourceUploadKind::Bundle] {
            for reverse in [false, true] {
                ...
                let (command, payload) =
                    source_upload(&bytes, "source", kind, &mut || true).unwrap();
    ...
    fn source_uploads_require_one_nonempty_payload_of_the_selected_kind() {
        for bytes in [
            input("bundle", b"data", false),
            input("patch", b"", false),
            input("file_0", b"data", true),
        ] {
            assert!(
                source_upload(&bytes, "source", SourceUploadKind::Patch, &mut || true).is_err()
            );
        }
        ...
        assert!(
            source_upload(&duplicate, "source", SourceUploadKind::Patch, &mut || true).is_err()
        );
    }
    #[test]
    fn provisional_parts_cannot_escape_truncation_or_cancellation() {
        ...
        trailing.extend_from_slice(b"NEXT");
    ...
    fn payload_profiles_cannot_widen_native_or_http_envelopes() {
        ...
        assert_eq!(
            SourceUploadKind::Bundle.maximum(),
