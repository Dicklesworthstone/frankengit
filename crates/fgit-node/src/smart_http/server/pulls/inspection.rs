...
use crate::treefs_workspace::candidate_inspection::{BundleInspectionRefusal, PullRequestInspectionRefusal};
...
pub(super) fn authenticate(request: &Request<'_>, envelope: &Envelope<'_>,
    ...
    Ok(LoopbackReceiveSession::authenticated(grant.principal,
        IdempotencyKey::new(b"read-only-candidate-inspection".to_vec()).map_err(|_| ApiError::unavailable())?))
}
...
pub(super) fn execute(node: &OneNode, request: &Request<'_>, session: &LoopbackReceiveSession,
    ...
    let inspected = drive_request_while(node, &context,
        node.inspect_pull_request_bundle_in(&context, &subject, candidate, bundle, &visibility, &options),
        &mut live).map_err(inspection_error)?;
...
fn source_error(error: NodeWorkspaceRefusal) -> ApiError {
    match error {
        ...
        NodeWorkspaceRefusal::StaleWorkspaceBase => ApiError::new(Status::Conflict, "inspection_subject_moved"),
        NodeWorkspaceRefusal::ObjectFormatMismatch | NodeWorkspaceRefusal::InvalidWorkspaceCandidate(_) => ApiError::bad("invalid_inspection_request"),
...
fn inspection_error(error: PullRequestInspectionRefusal) -> ApiError {
    ...
    match error {
        ...
        BundleInspectionRefusal::BudgetExceeded | BundleInspectionRefusal::Source(MergeSourceError::BudgetExceeded)
        | BundleInspectionRefusal::Validation(ProjectionFailure::Unavailable(RefusalCode::ResourceBudgetExceeded)) => ApiError::too_large(),
        BundleInspectionRefusal::Source(MergeSourceError::Cancelled)
        | BundleInspectionRefusal::Validation(ProjectionFailure::Unavailable(RefusalCode::CancellationInProgress)) => ApiError::from_status(Status::Timeout, false),
        BundleInspectionRefusal::InvalidCandidate(_) | BundleInspectionRefusal::Pack(_)
        | BundleInspectionRefusal::Validation(ProjectionFailure::Refuse(_)) => ApiError::bad("invalid_candidate"),
        ...
        BundleInspectionRefusal::Review(error) => match *error {
            ...
            ReviewError::Budget(_) | ReviewError::Source(MergeSourceError::BudgetExceeded) => ApiError::too_large(),
            ReviewError::Source(MergeSourceError::Cancelled) => ApiError::from_status(Status::Timeout, false),
            _ => ApiError::unavailable(),
        },
        _ => ApiError::unavailable(),
    }
...
mod tests {
    ...
    fn invalid_candidates_and_unavailable_evidence_never_become_votes_or_empty_reports() {
        for cause in [BundleInspectionRefusal::BudgetExceeded, BundleInspectionRefusal::ParentMoved,
            BundleInspectionRefusal::InvalidCandidate("untrusted input"), BundleInspectionRefusal::Source(MergeSourceError::Cancelled)] {
