...
use fgit_forge::preparation::resolution::{ConflictResolution, ResolutionChoice, ResolutionError, validate_resolutions};
...
impl<'a> Request<'a> {
    pub(crate) fn parse(head: &Envelope<'a>) -> Result<Option<Self>, ApiError> {
        ...
        if !matches!(action, "prepare" | "resolve") && !action.starts_with("prepare/") && !action.starts_with("resolve/") {
            return Ok(None);
        }
        if !matches!(action, "prepare" | "resolve") || repository_route.len() < 2 || !repository_route.starts_with('/')
        ...
        let maximum = if boundary.is_some() { resolution_upload::MAX_UPLOAD_BYTES } else { MAX_FORM_BYTES };
        ...
        Ok(Some(Self { repository_route, number, resolution, boundary }))
    }
        ...
        -> Result<(ReviewSubject, MergeMetadata), ApiError>
    {
        ...
        if self.resolution { return Err(ApiError::bad("resolution_choices_required")); }
        let (fields, _) = self.fields(bytes)?;
        self.subject_and_metadata(fields, format)
    }

    pub(super) fn resolved_command(&self, bytes: &[u8], mut files: FileParts<'_>,
        format: GitHashAlgorithm,
    ) -> Result<ResolutionCommand, ApiError> {
        if !self.resolution { return Err(ApiError::bad("not_a_resolution_request")); }
        ...
        for descriptor in descriptors {
            ...
            let path = path_bytes(fields.next().ok_or_else(|| ApiError::bad("invalid_resolution"))?)?;
            let action = fields.next().ok_or_else(|| ApiError::bad("invalid_resolution"))?;
            retained_bytes = retained_bytes.checked_add(path.len()).ok_or_else(ApiError::too_large)?;
            let choice = match action {
                ...
                "file" => {
                    ...
                    let name = fields.next().ok_or_else(|| ApiError::bad("resolution_file_required"))?;
                    if !resolution_upload::file_name(name) { return Err(ApiError::bad("invalid_resolution_file")); }
                    let content = files.remove(name).ok_or_else(|| ApiError::bad("missing_or_reused_resolution_file"))?;
                    retained_bytes = retained_bytes.checked_add(content.len()).ok_or_else(ApiError::too_large)?;
                    if content.len() > resolution_upload::MAX_FILE_BYTES || retained_bytes > resolution_upload::MAX_CONTENT_BYTES {
                        return Err(ApiError::too_large());
                    }
            ...
            if retained_bytes > resolution_upload::MAX_CONTENT_BYTES { return Err(ApiError::too_large()); }
            choices.try_reserve(1).map_err(|_| ApiError::unavailable())?;
        ...
        validate_resolutions(&choices, PreparationLimits::default()).map_err(|error| match error {
            ResolutionError::Budget => ApiError::too_large(),
            _ => ApiError::bad("invalid_resolution_set"),
        })?;
        Ok(ResolutionCommand { subject, metadata, base, choices })
    }

    fn fields(&self, bytes: &[u8]) -> Result<(BTreeMap<String, String>, Vec<String>), ApiError> {
        let mut fields = BTreeMap::new();
        let mut resolutions = Vec::new();
        let maximum = if self.resolution { 12 + resolution_upload::MAX_FILES } else { 11 };
        for (name, value) in parse_form(bytes, maximum)? {
            if name == "resolution" && self.resolution {
                if resolutions.len() == resolution_upload::MAX_FILES { return Err(ApiError::too_large()); }
    ...
    fn subject_and_metadata(&self, mut fields: BTreeMap<String, String>, format: GitHashAlgorithm)
        -> Result<(ReviewSubject, MergeMetadata), ApiError>
    {
        ...
        if !fields.is_empty() { return Err(ApiError::bad("unknown_preparation_field")); }
        Ok((subject, metadata))
    }
}
...
fn path_bytes(text: &str) -> Result<Vec<u8>, ApiError> {
    if text.is_empty() || text.len() > 2 * PreparationLimits::default().max_path_bytes || text.len() % 2 != 0
        || !text.bytes().all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b))
    { return Err(ApiError::bad("invalid_resolution_path")); }
    let digit = |byte: u8| if byte <= b'9' { byte - b'0' } else { byte - b'a' + 10 };
    let mut bytes = Vec::new();
    bytes.try_reserve_exact(text.len() / 2).map_err(|_| ApiError::unavailable())?;
...
mod tests {
    ...
    fn request(resolution: bool) -> Request<'static> {
        Request { repository_route: "/r.git", number: PullRequestNumber::FIRST, resolution, boundary: None }
    }
    ...
    fn missing_fields_ambient_identity_and_header_injection_are_not_defaults() {
        ...
        for invalid in [valid.replace("&timestamp=1", ""), valid.replace("timestamp=1", "timestamp=0"),
    ...
    fn preparation_has_a_closed_body_bearing_route_not_a_mutation_fallback() {
        for (method, suffix, headers) in [("GET", "/1/prepare", ""),
    ...
    fn raw_paths_empty_files_and_binary_modes_survive_exactly() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            let input = form(format) + &format!("&merge_base={}&resolution=ff:file:100755:file_7&resolution=61:delete",
                "c".repeat(format.digest_len() * 2));
            let files = BTreeMap::from([("file_7", b"\0\xff\r\nno-normalization".as_slice())]);
            let parsed = request(true).resolved_command(input.as_bytes(), files, format).unwrap();
            assert_eq!(parsed.choices[0].path, b"a");
            assert_eq!(parsed.choices[1], ConflictResolution { path: vec![255], choice: ResolutionChoice::File {
                mode: 0o100755, bytes: b"\0\xff\r\nno-normalization".to_vec() } });
            let empty = request(true).resolved_command(input.as_bytes(), BTreeMap::from([("file_7", b"".as_slice())]), format).unwrap();
            assert!(matches!(&empty.choices[1].choice, ResolutionChoice::File { bytes, .. } if bytes.is_empty()));
    ...
    fn ambiguous_paths_missing_unused_files_and_non_regular_modes_refuse() {
        ...
        for suffix in ["&resolution=61:ours&resolution=61:theirs", "&resolution=61:ours&resolution=612f62:theirs",
            "&resolution=2e2e2f61:delete", "&resolution=2e6769742f61:delete", "&resolution=00:delete",
            "&resolution=FF:ours", "&resolution=61:ours:ignored", "&resolution=61:file:120000:file_0",
            "&resolution=61:file:100644:file_0", "&resolution=61:unknown"]
        { assert!(request(true).resolved_command((base.clone() + suffix).as_bytes(), BTreeMap::new(), GitHashAlgorithm::Sha1).is_err(), "{suffix}"); }
        let input = base.clone() + "&resolution=61:ours";
        assert!(request(true).resolved_command(input.as_bytes(), BTreeMap::from([("file_0", b"unused".as_slice())]), GitHashAlgorithm::Sha1).is_err());
        let input = base + "&resolution=61:file:100644:file_0&resolution=62:file:100644:file_0";
        assert!(request(true).resolved_command(input.as_bytes(), BTreeMap::from([("file_0", b"once".as_slice())]), GitHashAlgorithm::Sha1).is_err());
    }
}
