...
pub(super) fn inspection_upload<'a>(
    bytes: &'a [u8],
    boundary: &str,
...
pub(super) fn authenticate(
    ...
    if !profile.allow_pulls || !permitted {
    ...
    let key = if request.is_mutation() {
        ...
        b"review-read-no-publication".as_slice()
    };
...
pub(super) fn execute(
    ...
    if let Operation::List(page) = request.operation {
        ...
        return Ok(Reply {
            ...
            body: output::page(node, request.number, page, result.as_ref(), maximum)?,
            terminal: None,
        });
    }
    let upload = match request.encoding {
        Encoding::Form => multipart::Upload {
            command: &bytes,
            bundle: None,
        },
        Encoding::Multipart(boundary) => {
            multipart::parse(&bytes, boundary, &mut live).map_err(|error| match error {
                multipart::Error::Limit => ApiError::too_large(),
                multipart::Error::Framing => ApiError::bad("invalid_candidate_upload"),
                multipart::Error::Cancelled => ApiError::from_status(Status::Timeout, false),
            })?
    ...
    let (tx, terminal) = match &command {
        Command::Review(command) => drive_request_while(
            node,
            &context,
            node.admit_candidate_review_durable_in(
                &context,
                session,
                command,
                upload.bundle,
    ...
    if body.len() > maximum {
        eprintln!(
            "Review/merge HTTP reply exceeds limit after canonical outcome for transaction {tx}; recover the original key"
        );
...
pub(super) fn read_upload_bounded(
    reader: &mut impl Read,
    framing: BodyFraming,
    limits: HttpLimits,
    maximum_bytes: usize,
) -> Result<Vec<u8>, ApiError> {
    if maximum_bytes > multipart::MAX_UPLOAD_BYTES {
...
mod tests {
    ...
    fn inspection_cannot_use_the_terminal_review_retry_without_a_bundle() {
        ...
        assert_eq!(
            inspection_upload(command, "x", &mut || true)
                .unwrap_err()
                .code,
