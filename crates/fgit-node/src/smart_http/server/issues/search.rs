...
pub(super) fn parse(body: &[u8]) -> Result<(CompiledIssueQuery, SearchRequest), ApiError> {
    ...
    let opened_by = flags
        .get("opened_by")
        .map(|text| PrincipalId::from_hex(text).map_err(|_| ApiError::bad("invalid_opener")))
        .transpose()?;
    ...
    if case_sensitive && text.is_none() {
        return Err(ApiError::bad("case_sensitive_requires_query"));
    }
    labels.sort();
    ...
    let after = flags
        .get("after")
        .map(|value| request::decimal(value))
        .transpose()?
        .unwrap_or(0);
    let limit = flags
        .get("limit")
    ...
    if !(1..=u64::from(MAX_RESULTS)).contains(&limit)
        || !(1..=u64::from(MAX_SCAN)).contains(&max_scan)
    {
        return Err(ApiError::bad("invalid_search_limits"));
    }
    let expected_head = flags
        .get("expected_head")
        .map(|value| request::parse_head_token(value))
        .transpose()?;
    ...
    Ok((
        ...
        SearchRequest {
            ...
            max_scan: u16::try_from(max_scan)
...
pub(super) fn error(error: SearchError<ApiError>) -> ApiError {
    match error {
        ...
        SearchError::InvalidSourcePage
        | SearchError::InvalidSnapshot(_)
        | SearchError::Allocation => ApiError::unavailable(),
    }
}
