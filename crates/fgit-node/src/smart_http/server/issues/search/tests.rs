...
use super::super::{Status, request::{Operation, Request}};
...
fn search_post_is_read_only_and_rejects_get_query_and_wrong_media() {
    ...
    assert!(request.command(b"expected_version=0&title=forged&body=x").is_err());
    for request in [
        ...
        let head = head::parse(request.as_bytes(), HttpLimits::default()).unwrap().unwrap();
...
fn form_search_preserves_literal_bytes_and_conjoins_filters() {
    ...
    let body = format!("query=HTTP%2B%25%0A%C3%A9&state=closed&label=z&label=a&case_sensitive=true&opened_by={}&after=7&expected_head={token}&limit=2&max_scan=1", "03".repeat(16));
    ...
    assert_eq!(query.query().opened_by, Some(PrincipalId::from_bytes([3; 16])));
    assert_eq!(request.after, 7);
    assert_eq!(request.limit, 2);
    assert_eq!(request.max_scan, 1);
    assert_eq!(request.expected_head, Some(request::parse_head_token(&token).unwrap()));
...
fn invalid_or_authority_widening_fields_refuse_before_search() {
    for body in ["query=", "query=%00", "query=%FF", "query=%", "query=a&query=b", "label=a&label=a",
        "state=all&state=all", "case_sensitive=1", "case_sensitive=true", "opened_by=bad", "after=1",
        "limit=0", "limit=101", "max_scan=0", "max_scan=1001", "max_scan=01", "max_scan=18446744073709551616",
        "principal=admin", "storage_root=/tmp", "tenant=other", "repository=other", "expected_version=0",
        "idempotency_key=forged", "clear_labels=true", "after_version=1"]
    { assert!(parse(body.as_bytes()).is_err(), "{body}"); }
    assert!(parse(format!("query={}", "x".repeat(257)).as_bytes()).is_err());
    let labels = (0..33).map(|n| format!("label={n:02}")).collect::<Vec<_>>().join("&");
...
fn search_errors_do_not_invent_mutation_ambiguity_or_terminal_outcomes() {
    for (error, status) in [
        ...
        (SearchError::Source(ApiError::snapshot_moved()), Status::Conflict),
    ] {
