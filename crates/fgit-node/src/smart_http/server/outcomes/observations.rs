...
use fgit_authority::{
    AsyncAuthorityStore, ExpectedOld, IdempotencyKey, ImmutableRead, ProposedNew, RefCommand,
    SealAttempt, SemanticRequest, bind_idempotency_key_async, idempotency_binding_key, seal_key,
    seal_request_async,
};
...
fn attempt(node: &OneNode, key: &[u8]) -> SealAttempt {
    let old = GitOid::from_hex(
        node.object_format,
        &"1".repeat(node.object_format.digest_len() * 2),
    )
    .unwrap();
    SealAttempt {
...
fn query(node: &OneNode, attempt: &SealAttempt) -> String {
    ...
    let request = Request {
        repository_route: "/unused-after-authentication",
        selector: Selector::Transaction,
    };
    execute(node, &request, &session, MAX_REPLY_BYTES as u64)
        .unwrap()
        .body
}
...
fn incomplete_attempts_remain_nonterminal_and_lookup_does_not_advance_them() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(matches!(
            node.runtime()
                .block_on(node.authority.read_immutable(request.authority(), &binding))
                .unwrap(),
        ...
        assert!(
            !bound.contains(&tx.to_string()),
            "unverified binding targets are not disclosed"
        );
        assert_eq!(query(&node, &attempt), bound);
        assert!(matches!(
            node.runtime()
                .block_on(node.authority.read_immutable(request.authority(), &seal))
                .unwrap(),
            ImmutableRead::Absent
        ));
        ...
        assert_eq!(
            after, before,
            "reads cannot turn a staged seal into a decision"
        );
...
fn corrupt_seal_is_an_error_not_an_absent_or_undecided_observation() {
    let scratch = Scratch::new();
    let (node, _) = OneNode::init(NodeConfig::new(
        scratch.0.clone(),
        TenantId::from_bytes([0xa1; 16]),
        RepositoryId::from_bytes([0xa2; 16]),
    ))
    ...
    node.runtime()
        .block_on(
            node.authority
                .put_if_absent(request.authority(), &slot, b"not a canonical seal"),
        )
        .unwrap();
    let session = LoopbackReceiveSession::authenticated(
        attempt.authenticated_principal_id,
    ...
    error
        .send(&mut output, fgit_wire::smart_http::HttpVersion::Http11)
        .unwrap();
