...
pub(super) fn execute(
    node: &OneNode, session: &LoopbackReceiveSession, maximum_response: u64,
    ...
    let report = drive_request_while(node, &context,
        node.recover_receive_session_in(&context, session), &mut || !deadline.expired())
        .map_err(|_| ApiError::from_status(Status::Unavailable))?;
    let maximum = usize::try_from(maximum_response).unwrap_or(usize::MAX).min(MAX_SESSION_REPLY_BYTES);
    let body = render_session(node, principal, &report, maximum)?;
    let terminal_tx = match &report {
        SessionRecovery::NotObserved => None,
        SessionRecovery::Recovered(session) => session.commands().iter().find_map(|command| {
            match command.recovery() {
                RequestRecovery::Recovered(known) if command.recovery().terminal().is_some() => Some(known.tx_id()),
                _ => None,
            }
        }),
    };
...
fn render_session(
    node: &OneNode, principal: PrincipalId, report: &SessionRecovery, maximum: usize,
    ...
    let state = if session.is_none() { "session_not_observed" }
        else if all_terminal { "complete" } else { "partial" };
    let header = format!(concat!(
        ...
        quote(state), session.map_or_else(|| "null".into(), |session| quote(&session.identity().to_string())),
        session.map_or_else(|| "null".into(), |session| session.commands().len().to_string()),
        session.is_some(), if session.is_some() { all_terminal.to_string() } else { "null".into() },
        all_terminal);
...
fn append(out: &mut String, part: &str, maximum: usize) -> Result<(), ApiError> {
    let next = out.len().checked_add(part.len()).and_then(|length| length.checked_add(2))
        .ok_or_else(|| ApiError::from_status(Status::TooLarge))?;
    if next > maximum { return Err(ApiError::from_status(Status::TooLarge)); }
    out.try_reserve(part.len()).map_err(|_| ApiError::from_status(Status::Unavailable))?;
    out.push_str(part);
    Ok(())
}
