...
fn asset(route: &[u8], target: &str) -> Option<(&'static str, &'static str, AssetScope)> {
    match target.split('?').next()?.as_bytes().strip_prefix(route)? {
        ...
        b"/ui/issues.mjs" => Some((
            "text/javascript; charset=utf-8",
            ISSUES_CLIENT,
            AssetScope::Issues,
        )),
        b"/ui/issues-view.mjs" => Some((
            "text/javascript; charset=utf-8",
            ISSUES_VIEW,
            AssetScope::Issues,
        )),
...
fn checked_asset(
    ...
    let Some((media, body, scope)) = asset(route, request.target) else {
        return Ok(None);
    };
...
pub(super) fn serve(
    ...
    if replay::serve(profile, request, trailing, writer)? {
        return Ok(true);
    }
    if tags::serve(profile, request, trailing, writer)? {
        return Ok(true);
    }
    if export_verify::serve(profile, request, trailing, writer)? {
        return Ok(true);
    }
    if transfers::serve(profile, request, trailing, writer)? {
        return Ok(true);
    }
    if search::serve(profile, request, trailing, writer)? {
        return Ok(true);
    }
    if branches::serve(profile, request, trailing, writer)? {
        return Ok(true);
    }
    if history::serve(profile, request, trailing, writer)? {
        return Ok(true);
    }
    if initial::serve(profile, request, trailing, writer)? {
        return Ok(true);
    }
    if source_edit::serve(profile, request, trailing, writer)? {
        return Ok(true);
    }
    if pulls::serve(profile, request, trailing, writer)? {
        return Ok(true);
    }
...
mod tests {
    ...
    fn static_assets_are_exactly_repository_scoped() {
        ...
        for target in [
            "/repo.git/ui/../source.rs",
            "/repo.git/ui/%2e%2e",
            "/repo.git/ui/private",
            "/repo.git/api/v1/source/tree",
        ] {
    ...
    fn shell_never_relaxes_endpoint_framing_or_source_enablement() {
        ...
        for (method, suffix, extra, enabled, trailing, maximum, allowed) in [
            ...
            let bytes =
                format!("{method} /repo.git{suffix} HTTP/1.1\r\nHost: local\r\n{extra}\r\n");
            let request = head::parse(bytes.as_bytes(), HttpLimits::default())
                .unwrap()
                .unwrap();
            assert_eq!(
                checked_asset(b"/repo.git", enabled, false, maximum, &request, trailing).is_ok(),
                allowed
            );
        }
    }
        ...
        for source in [false, true] {
            for issues in [false, true] {
                for (suffix, allowed) in [
                    ("/ui/", source),
                    ("/ui/browser.mjs", source),
                    ("/ui/issues/", issues),
                    ("/ui/issues.mjs", issues),
                    ("/ui/issues-view.mjs", issues),
                    ("/ui/browser.css", source || issues),
                ] {
                    ...
                    let envelope = head::parse(bytes.as_bytes(), HttpLimits::default())
                        ...
                        .unwrap();
                    assert_eq!(
                        checked_asset(b"/repo.git", source, issues, u64::MAX, &envelope, false)
                            .is_ok(),
                        allowed
                    );
                }
    ...
    fn issue_shell_cannot_smuggle_bodies_queries_or_native_mutations() {
        ...
        for (method, suffix, header, trailing, maximum) in [
            ...
            let envelope = head::parse(bytes.as_bytes(), HttpLimits::default())
                ...
                .unwrap();
            assert!(
                checked_asset(b"/repo.git", false, true, maximum, &envelope, trailing).is_err()
            );
        }
