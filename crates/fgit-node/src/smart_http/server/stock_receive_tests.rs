...
fn ordinary_push_discovery_redirects_without_a_custom_header() {
    ...
    assert!(response.contains(&format!("Idempotency-Key: {KEY_PREFIX}{}\r\n", "42".repeat(32))));
...
fn discovery_and_rpc_reuse_one_key_without_any_process_local_session_state() {
    ...
    let rpc = request("POST", &scoped(&identifier, RPC), RPC_HEADERS, b"\xff\x00\xfe\x01");
    ...
    assert_eq!(retry_key(&normalized[..parsed.consumed]).unwrap(), Some(key.as_bytes()));
    let selected = authenticated_session(&parsed, &normalized[..parsed.consumed], &reopened).unwrap();
...
fn no_redirect_or_entropy_before_authentication_and_receive_permission() {
    ...
    for (input, expected) in [
        (String::from_utf8(valid.clone()).unwrap().replace(&format!("Authorization: Bearer {}\r\n", "a".repeat(64)), "").into_bytes(), Status::Unauthorized),
        (String::from_utf8(valid.clone()).unwrap().replace(&"a".repeat(64), &"b".repeat(64)).into_bytes(), Status::Unauthorized),
    ] {
        let mut output = Vec::new();
        assert_eq!(
            adapt_with_nonce(input, &profile, &mut HttpVersion::Http11, &mut output, || panic!("no entropy before authentication")),
            Err(expected)
        );
        assert!(output.is_empty());
    }
    ...
    let mut output = Vec::new();
    assert_eq!(
        adapt_with_nonce(valid, &readonly, &mut HttpVersion::Http11, &mut output, || panic!("no entropy before permission")),
...
fn scoped_url_never_authenticates_a_post_or_overrides_revocation() {
    ...
    assert_eq!(authenticated_session(&parsed, &normalized[..envelope.consumed], &profile), Err(Status::Forbidden));
    profile.allow_receive = true;
    profile.credentials = super::super::CredentialSource::Static {
        digest: sha256_digest(&[b'b'; 64]),
        principal: PrincipalId::from_bytes([3; 16]),
    };
    assert_eq!(authenticated_session(&parsed, &normalized[..envelope.consumed], &profile), Err(Status::Unauthorized));
}
...
fn explicit_keys_and_unrelated_services_are_byte_for_byte_unchanged() {
    let profile = profile();
    for input in [
        request("GET", &format!("/repo.git{DISCOVERY}"), "Idempotency-Key: explicit\r\n", b""),
        request("POST", "/repo.git/git-receive-pack", &format!("{RPC_HEADERS}Idempotency-Key: explicit\r\n"), b"0000"),
        request("GET", "/repo.git/info/refs?service=git-upload-pack", "", b""),
        request("GET", "/repo.git/api/v1/issues", "", b""),
        request("GET", "/repo.git-other/info/refs?service=git-receive-pack", "", b""),
    ] {
        let mut output = Vec::new();
        let expected = input.clone();
        assert_eq!(adapt_with_nonce(input, &profile, &mut HttpVersion::Http11, &mut output, || panic!("explicit or unrelated path must not allocate an attempt")).unwrap(), Some(expected));
        assert!(output.is_empty());
    }
}
...
fn conflicting_duplicate_and_hop_by_hop_keys_are_refused() {
    ...
    for extra in [
        ...
        assert!(run(request("POST", &target, &format!("{RPC_HEADERS}{extra}"), b"0000"), &profile, 1).is_err());
    }
    let input = request("POST", &target, &format!("{RPC_HEADERS}Idempotency-Key: {key}\r\n"), b"0000");
    let normalized = run(input, &profile, 1).unwrap().0.unwrap();
    let parsed = parse_head(&normalized, profile.http).unwrap().unwrap();
    assert_eq!(retry_key(&normalized[..parsed.consumed]).unwrap(), Some(key.as_bytes()));
}
...
fn alias_syntax_cannot_select_foreign_endpoints_or_ambiguous_nonces() {
    let profile = profile();
    for identifier in ["".to_owned(), "a".repeat(63), "a".repeat(65), "A".repeat(64), "g".repeat(64), "%61".repeat(32)] {
        assert!(run(request("GET", &scoped(&identifier, DISCOVERY), "", b""), &profile, 1).is_err());
    }
    for suffix in [
        "/info/refs?service=git-upload-pack", "/git-upload-pack", "/git-receive-pack?x=1",
        "/api/v1/source", "/api/v1/outcomes", "/../info/refs?service=git-receive-pack",
    ] {
        assert!(run(request("GET", &scoped(&"42".repeat(32), suffix), "", b""), &profile, 1).is_err());
    }
    assert!(run(request("GET", &scoped(&"42".repeat(32), RPC), "", b""), &profile, 1).is_err());
}
...
fn discovery_body_expectation_and_pipelined_bytes_do_not_redirect() {
    ...
    for (headers, body) in [
        ...
        let mut output = Vec::new();
        assert!(adapt_with_nonce(request("GET", &target, headers, body), &profile, &mut HttpVersion::Http11, &mut output, || panic!("invalid discovery must not allocate")).is_err());
        assert!(output.is_empty());
    }
    assert!(run(request("GET", &target, "Content-Length: 0\r\n", b""), &profile, 1).unwrap().0.is_none());
}
...
fn normalization_preserves_chunked_stream_and_protocol_headers() {
    ...
    let body = b"2\r\n\xff\x00\r\n2\r\n\xfe\x01\r\n0\r\n\r\n";
    let normalized = run(request("POST", &scoped(&"42".repeat(32), RPC), headers, body), &profile, 1).unwrap().0.unwrap();
    let parsed = parse_head(&normalized, profile.http).unwrap().unwrap();
    assert_eq!(parsed.body, BodyFraming::Chunked);
    assert_eq!(parsed.requested_version, fgit_wire::smart_http::ProtocolVersion::V1);
...
fn synthetic_header_is_charged_to_header_count_and_byte_limits() {
    ...
    let normalized_head = head::parse(&normalized, profile.http).unwrap().unwrap().consumed;
    ...
    assert_eq!(run(input.clone(), &count_limited, 1), Err(Status::HeaderTooLarge));
...
fn redirect_respects_http_version_and_target_limit() {
    let mut profile = profile();
    let input = String::from_utf8(request("GET", &format!("/repo.git{DISCOVERY}"), "", b"")).unwrap().replace("HTTP/1.1", "HTTP/1.0").into_bytes();
