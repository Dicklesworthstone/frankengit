...
fn asset(route: &[u8], target: &str) -> Option<(&'static str, &'static str)> {
    match target.split('?').next()?.as_bytes().strip_prefix(route)? {
        ...
        b"/ui/branches/branches.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("branches.mjs"),
        )),
        b"/ui/branches/branches-view.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("branches-view.mjs"),
        )),
        // Same bounded transport/outcome helpers, not an implicit PR grant.
        b"/ui/branches/pulls-core.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("pulls-core.mjs"),
        )),
        b"/ui/branches/pulls-actions.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("pulls-actions.mjs"),
        )),
        b"/ui/branches/pulls-candidate.mjs" => Some((
            "text/javascript; charset=utf-8",
...
mod tests {
    ...
    fn every_branch_asset_and_import_is_exactly_source_scoped() {
        for suffix in [
            ...
            assert!(matches!(
                checked_asset(b"/repo.git", false, u64::MAX, &request, false),
                Err(Status::NotFound)
            ));
            assert!(asset(b"/other.git", &target).is_none());
        }
        for target in [
            "/repo.git-more/ui/branches/",
            "/repo.git/ui/branches/../private",
            "/repo.git/ui/branches/%2e%2e",
            "/repo.git/api/v1/source/branches/delete",
            "/repo.git/ui/source/",
            "/repo.git/ui/pulls/",
        ] {
    ...
    fn static_routes_never_accept_mutation_framing_queries_or_oversized_responses() {
        for (method, query, headers, trailing, maximum) in [
            ...
            let bytes = format!(
                "{method} /repo.git/ui/branches/{query} HTTP/1.1\r\nHost: local\r\n{headers}\r\n"
            );
            let request = head::parse(bytes.as_bytes(), HttpLimits::default())
                .unwrap()
                .unwrap();
