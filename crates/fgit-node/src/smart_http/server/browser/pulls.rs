...
fn asset(route: &[u8], target: &str) -> Option<(&'static str, &'static str)> {
    ...
    match suffix {
        ...
        b"/ui/pulls-resolution-view.mjs" => {
            Some((script, include_str!("pulls-resolution-view.mjs")))
        }
...
mod tests {
    ...
    fn pr_enablement_and_framing_are_checked_before_static_success() {
        for (method, suffix, extra, enabled, trailing, maximum, allowed) in [
            ...
            let envelope = head::parse(text.as_bytes(), HttpLimits::default())
                ...
                .unwrap();
            assert_eq!(
                checked_asset(b"/r.git", enabled, maximum, &envelope, trailing).is_ok(),
                allowed
            );
        }
    }
    ...
    fn pr_shell_reuses_strict_security_and_has_no_inline_execution() {
        ...
        for script in [
            ...
            include_str!("pulls-resolution-view.mjs"),
        ] {
