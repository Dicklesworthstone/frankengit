...
fn asset(route: &[u8], target: &str) -> Option<(&'static str, &'static str)> {
    match target.split('?').next()?.as_bytes().strip_prefix(route)? {
        ...
        b"/ui/replay/replay.mjs" => {
            Some(("text/javascript; charset=utf-8", include_str!("replay.mjs")))
        }
        b"/ui/replay/replay-protocol.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("replay-protocol.mjs"),
        )),
        b"/ui/replay/replay-view.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("replay-view.mjs"),
        )),
        b"/ui/replay/pulls-core.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("pulls-core.mjs"),
        )),
        b"/ui/replay/pulls-candidate.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("pulls-candidate.mjs"),
        )),
        b"/ui/replay/pulls-actions.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("pulls-actions.mjs"),
        )),
        b"/ui/replay/source-edit-protocol.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("source-edit-protocol.mjs"),
        )),
        b"/ui/replay/source-edit-patch.mjs" => Some((
            "text/javascript; charset=utf-8",
...
mod tests {
    ...
    fn replay_and_shared_helpers_require_exact_source_routes_and_source_enablement() {
        ...
        for target in [
            "/repo.git-more/ui/replay/",
            "/repo.git/ui/replay/../private",
            "/repo.git/ui/replay/%2e%2e",
    ...
    fn static_replay_does_not_accept_request_bodies_queries_or_oversized_responses() {
        for (method, query, headers, trailing, maximum) in [
            ...
            let bytes = format!(
                "{method} /repo.git/ui/replay/{query} HTTP/1.1\r\nHost: local\r\n{headers}\r\n"
            );
            let request = head::parse(bytes.as_bytes(), HttpLimits::default())
                .unwrap()
                .unwrap();
