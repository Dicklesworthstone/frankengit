//! A source-only cherry-pick/revert shell. Static helpers grant no API
//! capability; native replay, inspection and admission retain their own grants.
use std::io::Write;
...
fn asset(route: &[u8], target: &str) -> Option<(&'static str, &'static str)> {
    match target.split('?').next()?.as_bytes().strip_prefix(route)? {
        b"/ui/replay/" => Some(("text/html; charset=utf-8", include_str!("replay.html"))),
        b"/ui/replay/replay.mjs" => Some(("text/javascript; charset=utf-8", include_str!("replay.mjs"))),
        b"/ui/replay/replay-protocol.mjs" => Some(("text/javascript; charset=utf-8", include_str!("replay-protocol.mjs"))),
        b"/ui/replay/replay-view.mjs" => Some(("text/javascript; charset=utf-8", include_str!("replay-view.mjs"))),
        b"/ui/replay/pulls-core.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-core.mjs"))),
        b"/ui/replay/pulls-candidate.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-candidate.mjs"))),
        b"/ui/replay/pulls-actions.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-actions.mjs"))),
        b"/ui/replay/source-edit-protocol.mjs" => Some(("text/javascript; charset=utf-8", include_str!("source-edit-protocol.mjs"))),
        b"/ui/replay/source-edit-patch.mjs" => Some(("text/javascript; charset=utf-8", include_str!("source-edit-patch.mjs"))),
        _ => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn replay_and_shared_helpers_require_exact_source_routes_and_source_enablement() {
        for suffix in ["", "replay.mjs", "replay-protocol.mjs", "replay-view.mjs", "pulls-core.mjs", "pulls-candidate.mjs", "pulls-actions.mjs", "source-edit-protocol.mjs", "source-edit-patch.mjs"] {
            let target = format!("/repo.git/ui/replay/{suffix}");
        ...
        for target in ["/repo.git-more/ui/replay/", "/repo.git/ui/replay/../private",
            "/repo.git/ui/replay/%2e%2e", "/repo.git/api/v1/source/apply", "/repo.git/ui/pulls/"] {
            assert!(asset(b"/repo.git", target).is_none());
        }
    }
    #[test]
    fn static_replay_does_not_accept_request_bodies_queries_or_oversized_responses() {
        for (method, query, headers, trailing, maximum) in [
            ...
            let bytes = format!("{method} /repo.git/ui/replay/{query} HTTP/1.1\r\nHost: local\r\n{headers}\r\n");
            let request = head::parse(bytes.as_bytes(), HttpLimits::default()).unwrap().unwrap();
            assert!(checked_asset(b"/repo.git", true, maximum, &request, trailing).is_err());
        }
    }
    #[test]
    fn replay_shell_and_scripts_keep_repository_text_inert_and_tokens_ephemeral() {
        assert!(!include_str!("replay.html").contains("<script>"));
        for script in [include_str!("replay.mjs"), include_str!("replay-view.mjs")] {
