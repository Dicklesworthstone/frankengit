...
fn asset(route: &[u8], target: &str) -> Option<(&'static str, &'static str)> {
    match target.split('?').next()?.as_bytes().strip_prefix(route)? {
        ...
        b"/ui/source/source-edit.mjs" => Some(("text/javascript; charset=utf-8", include_str!("source-edit.mjs"))),
        b"/ui/source/source-edit-patch.mjs" => Some(("text/javascript; charset=utf-8", include_str!("source-edit-patch.mjs"))),
        b"/ui/source/source-edit-protocol.mjs" => Some(("text/javascript; charset=utf-8", include_str!("source-edit-protocol.mjs"))),
        b"/ui/source/source-edit-view.mjs" => Some(("text/javascript; charset=utf-8", include_str!("source-edit-view.mjs"))),
        // Byte-identical helpers, not an implicit PR enablement or permission.
        b"/ui/source/pulls-core.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-core.mjs"))),
        b"/ui/source/pulls-candidate.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-candidate.mjs"))),
        b"/ui/source/pulls-actions.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-actions.mjs"))),
        _ => None,
    }
}
...
mod tests {
    ...
    fn authoring_and_shared_helpers_require_exact_source_routes_and_source_enablement() {
        for suffix in ["", "source-edit.mjs", "source-edit-patch.mjs", "source-edit-protocol.mjs",
            "source-edit-view.mjs", "pulls-core.mjs", "pulls-candidate.mjs", "pulls-actions.mjs"] {
        ...
        for target in ["/repo.git-more/ui/source/", "/repo.git/ui/source/../private",
            "/repo.git/ui/source/%2e%2e", "/repo.git/api/v1/source/apply", "/repo.git/ui/pulls/"] {
    ...
    fn static_authoring_does_not_accept_request_bodies_queries_or_oversized_responses() {
        for (method, query, headers, trailing, maximum) in [
            ...
            let bytes = format!("{method} /repo.git/ui/source/{query} HTTP/1.1\r\nHost: local\r\n{headers}\r\n");
            let request = head::parse(bytes.as_bytes(), HttpLimits::default()).unwrap().unwrap();
    ...
    fn authoring_shell_and_scripts_keep_repository_text_inert_and_tokens_ephemeral() {
        ...
        for script in [include_str!("source-edit.mjs"), include_str!("source-edit-view.mjs")] {
