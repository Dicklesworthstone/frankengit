...
fn asset(route: &[u8], target: &str) -> Option<(&'static str, &'static str)> {
    match target.split('?').next()?.as_bytes().strip_prefix(route)? {
        ...
        b"/ui/rebase/rebase.mjs" => {
            Some(("text/javascript; charset=utf-8", include_str!("rebase.mjs")))
        }
        b"/ui/rebase/rebase-data.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("rebase-data.mjs"),
        )),
        b"/ui/rebase/rebase-inspection.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("rebase-inspection.mjs"),
        )),
        b"/ui/rebase/rebase-view.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("rebase-view.mjs"),
        )),
        b"/ui/rebase/pulls-core.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("pulls-core.mjs"),
        )),
        b"/ui/rebase/pulls-candidate.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("pulls-candidate.mjs"),
        )),
        b"/ui/rebase/pulls-actions.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("pulls-actions.mjs"),
        )),
        b"/ui/rebase/source-edit-protocol.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("source-edit-protocol.mjs"),
        )),
        b"/ui/rebase/source-edit-patch.mjs" => Some((
            "text/javascript; charset=utf-8",
...
mod tests {
    ...
    fn every_asset_is_source_gated_and_exactly_repository_scoped() {
        for name in [
            ...
            let request = head::parse(bytes.as_bytes(), HttpLimits::default())
                ...
                .unwrap();
            assert!(
                checked_asset(b"/r.git", true, u64::MAX, &request, false)
                    .unwrap()
                    .is_some()
            );
        ...
        for target in [
            "/r.git-more/ui/rebase/",
            "/r.git/ui/rebase/../private",
            "/r.git/api/v1/source/rebase/apply",
        ] {
    ...
    fn shell_refuses_bodies_queries_protocol_options_and_response_overflow() {
        for (method, query, headers, trailing, maximum) in [
            ...
            let bytes = format!(
                "{method} /r.git/ui/rebase/{query} HTTP/1.1\r\nHost: local\r\n{headers}\r\n"
            );
            let request = head::parse(bytes.as_bytes(), HttpLimits::default())
                .unwrap()
                .unwrap();
