...
fn asset(route: &[u8], target: &str) -> Option<(&'static str, &'static str)> {
    match target.split('?').next()?.as_bytes().strip_prefix(route)? {
        ...
        b"/ui/transfers/transfers.mjs" => Some(("text/javascript; charset=utf-8", include_str!("transfers.mjs"))),
        b"/ui/transfers/transfers-protocol.mjs" => Some(("text/javascript; charset=utf-8", include_str!("transfers-protocol.mjs"))),
        b"/ui/transfers/transfers-view.mjs" => Some(("text/javascript; charset=utf-8", include_str!("transfers-view.mjs"))),
        b"/ui/transfers/transfers.css" => Some(("text/css; charset=utf-8", include_str!("transfers.css"))),
        // Shared bytes confer no PR/source-edit/initial/branch permission.
        b"/ui/transfers/pulls-core.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-core.mjs"))),
        b"/ui/transfers/pulls-candidate.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-candidate.mjs"))),
        b"/ui/transfers/pulls-actions.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-actions.mjs"))),
        _ => None,
    }
}
...
mod tests {
    ...
    fn transfer_assets_are_exactly_source_scoped_without_enabling_other_operations() {
        for suffix in ["", "transfers.mjs", "transfers-protocol.mjs", "transfers-view.mjs", "transfers.css",
            "pulls-core.mjs", "pulls-candidate.mjs", "pulls-actions.mjs"] {
            ...
            assert!(checked_asset(b"/repo.git", true, u64::MAX, &envelope, false).unwrap().is_some());
            assert!(checked_asset(b"/repo.git", false, u64::MAX, &envelope, false).is_err());
            assert!(asset(b"/other.git", &target).is_none());
        }
        for target in ["/repo.git-more/ui/transfers/", "/repo.git/ui/transfers/../private", "/repo.git/ui/transfers/%2e%2e",
            "/repo.git/ui/pulls/", "/repo.git/api/v1/source/bundle/import"] { assert!(asset(b"/repo.git", target).is_none()); }
    }
    #[test]
    fn static_transfer_assets_refuse_bodies_protocol_parameters_queries_and_byte_overruns() {
        for (method, query, headers, trailing, maximum) in [
            ("POST", "", "", false, u64::MAX), ("GET", "?token=private", "", false, u64::MAX),
            ...
            let bytes = format!("{method} /repo.git/ui/transfers/{query} HTTP/1.1\r\nHost: local\r\n{headers}\r\n");
            let envelope = head::parse(bytes.as_bytes(), HttpLimits::default()).unwrap().unwrap();
    ...
    fn transfer_ui_keeps_native_text_inert_and_reuses_the_strict_security_policy() {
        ...
        for script in [include_str!("transfers.mjs"), include_str!("transfers-protocol.mjs"), include_str!("transfers-view.mjs")] {
