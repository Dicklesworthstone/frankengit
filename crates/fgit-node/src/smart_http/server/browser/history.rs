...
fn asset(route: &[u8], target: &str) -> Option<(&'static str, &'static str)> {
    match target.split('?').next()?.as_bytes().strip_prefix(route)? {
        ...
        b"/ui/history/history.mjs" => Some(("text/javascript; charset=utf-8", include_str!("history.mjs"))),
        b"/ui/history/history-data.mjs" => Some(("text/javascript; charset=utf-8", include_str!("history-data.mjs"))),
        b"/ui/history/history-view.mjs" => Some(("text/javascript; charset=utf-8", include_str!("history-view.mjs"))),
        // Shared pure byte/transport helpers do not enable any PR operation.
        b"/ui/history/pulls-core.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-core.mjs"))),
        b"/ui/history/pulls-candidate.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-candidate.mjs"))),
        _ => None,
    }
}
...
mod tests {
    ...
    fn history_assets_require_exact_source_routes_and_source_enablement() {
        for suffix in ["", "history.mjs", "history-data.mjs", "history-view.mjs", "pulls-core.mjs", "pulls-candidate.mjs"] {
        ...
        for target in ["/repo.git-more/ui/history/", "/repo.git/ui/history/../private", "/repo.git/ui/history/%2e%2e",
            "/repo.git/api/v1/source/log", "/repo.git/ui/pulls/", "/repo.git/ui/history/unknown.mjs"] {
    ...
    fn history_assets_refuse_bodies_queries_protocol_options_and_response_overflow() {
        for (method, query, headers, trailing, maximum) in [
            ...
            let bytes = format!("{method} /repo.git/ui/history/{query} HTTP/1.1\r\nHost: local\r\n{headers}\r\n");
            let request = head::parse(bytes.as_bytes(), HttpLimits::default()).unwrap().unwrap();
    ...
    fn history_keeps_repository_text_inert_and_credentials_ephemeral() {
        ...
        for script in [include_str!("history.mjs"), include_str!("history-data.mjs"), include_str!("history-view.mjs")] {
