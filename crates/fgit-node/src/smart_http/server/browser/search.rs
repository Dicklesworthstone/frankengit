...
mod tests {
    ...
    fn search_assets_require_exact_source_routes_and_source_enablement() {
        ...
        for target in ["/repo.git-more/ui/search/", "/repo.git/ui/search/../private", "/repo.git/ui/search/%2e%2e",
            "/repo.git/api/v1/source/search", "/repo.git/ui/pulls/", "/repo.git/ui/search/unknown.mjs"] {
    ...
    fn search_assets_refuse_bodies_queries_protocol_options_and_response_overflow() {
        for (method, query, headers, trailing, maximum) in [
            ...
            let bytes = format!("{method} /repo.git/ui/search/{query} HTTP/1.1\r\nHost: local\r\n{headers}\r\n");
            let request = head::parse(bytes.as_bytes(), HttpLimits::default()).unwrap().unwrap();
    ...
    fn search_keeps_repository_text_inert_and_credentials_ephemeral() {
        ...
        assert!(include_str!("search.html").contains("id=\"submit-search\" type=\"submit\" disabled"));
    }
}
