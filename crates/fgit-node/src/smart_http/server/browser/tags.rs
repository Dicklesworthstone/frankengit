...
mod tests {
    ...
    fn shell_and_every_helper_require_exact_source_enabled_routes() {
        for name in [
            ...
            let request = head::parse(bytes.as_bytes(), HttpLimits::default())
                ...
                .unwrap();
            assert!(
                checked(b"/repo.git", true, u64::MAX, &request, false)
                    .unwrap()
                    .is_some()
            );
        ...
        for path in [
            "/repo.git-more/ui/tags/",
            "/repo.git/ui/tags/../private",
            "/repo.git/ui/tags/%2e%2e",
            "/repo.git/api/v1/source/tags/delete",
        ] {
    ...
    fn tag_static_shell_never_relaxes_http_framing_or_resource_limits() {
        for (method, suffix, extra, trailing, maximum) in [
            ...
            let bytes = format!(
                "{method} /repo.git/ui/tags/{suffix} HTTP/1.1\r\nHost: local\r\n{extra}\r\n"
            );
            let request = head::parse(bytes.as_bytes(), HttpLimits::default())
                .unwrap()
                .unwrap();
