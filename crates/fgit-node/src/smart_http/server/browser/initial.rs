//! A source-enabled static initial-history shell. These assets contain no repository
...
fn asset(route: &[u8], target: &str) -> Option<(&'static str, &'static str)> {
    match target.split('?').next()?.as_bytes().strip_prefix(route)? {
        b"/ui/initial/" => Some(("text/html; charset=utf-8", include_str!("initial.html"))),
        b"/ui/initial/initial.mjs" => Some(("text/javascript; charset=utf-8", include_str!("initial.mjs"))),
        b"/ui/initial/source-edit-patch.mjs" => Some(("text/javascript; charset=utf-8", include_str!("source-edit-patch.mjs"))),
        b"/ui/initial/source-edit-protocol.mjs" => Some(("text/javascript; charset=utf-8", include_str!("source-edit-protocol.mjs"))),
        b"/ui/initial/initial-view.mjs" => Some(("text/javascript; charset=utf-8", include_str!("initial-view.mjs"))),
        b"/ui/initial/initial-plan.mjs" => Some(("text/javascript; charset=utf-8", include_str!("initial-plan.mjs"))),
        // Byte-identical helpers, not an implicit PR enablement or permission.
        b"/ui/initial/pulls-core.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-core.mjs"))),
        b"/ui/initial/pulls-candidate.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-candidate.mjs"))),
        b"/ui/initial/pulls-actions.mjs" => Some(("text/javascript; charset=utf-8", include_str!("pulls-actions.mjs"))),
        _ => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn initial_and_shared_helpers_require_exact_source_routes_and_source_enablement() {
        for suffix in ["", "initial.mjs", "source-edit-patch.mjs", "source-edit-protocol.mjs",
            "initial-view.mjs", "initial-plan.mjs", "pulls-core.mjs", "pulls-candidate.mjs", "pulls-actions.mjs"] {
            let target = format!("/repo.git/ui/initial/{suffix}");
        ...
        for target in ["/repo.git-more/ui/initial/", "/repo.git/ui/initial/../private",
            "/repo.git/ui/initial/%2e%2e", "/repo.git/api/v1/source/apply", "/repo.git/ui/pulls/"] {
            assert!(asset(b"/repo.git", target).is_none());
        }
    }
    #[test]
    fn static_initial_does_not_accept_request_bodies_queries_or_oversized_responses() {
        for (method, query, headers, trailing, maximum) in [
            ...
            let bytes = format!("{method} /repo.git/ui/initial/{query} HTTP/1.1\r\nHost: local\r\n{headers}\r\n");
            let request = head::parse(bytes.as_bytes(), HttpLimits::default()).unwrap().unwrap();
            assert!(checked_asset(b"/repo.git", true, maximum, &request, trailing).is_err());
        }
    }
    #[test]
    fn initial_shell_and_scripts_keep_repository_text_inert_and_tokens_ephemeral() {
        assert!(!include_str!("initial.html").contains("<script>"));
        for script in [include_str!("initial.mjs"), include_str!("initial-view.mjs")] {
