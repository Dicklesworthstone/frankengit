...
fn asset(route: &[u8], target: &str) -> Option<(&'static str, &'static str)> {
    match target.split('?').next()?.as_bytes().strip_prefix(route)? {
        ...
        b"/ui/initial/initial.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("initial.mjs"),
        )),
        b"/ui/initial/source-edit-patch.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("source-edit-patch.mjs"),
        )),
        b"/ui/initial/source-edit-protocol.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("source-edit-protocol.mjs"),
        )),
        b"/ui/initial/initial-view.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("initial-view.mjs"),
        )),
        b"/ui/initial/initial-plan.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("initial-plan.mjs"),
        )),
        // Byte-identical helpers, not an implicit PR enablement or permission.
        b"/ui/initial/pulls-core.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("pulls-core.mjs"),
        )),
        b"/ui/initial/pulls-candidate.mjs" => Some((
            "text/javascript; charset=utf-8",
            include_str!("pulls-candidate.mjs"),
        )),
        b"/ui/initial/pulls-actions.mjs" => Some((
            "text/javascript; charset=utf-8",
...
mod tests {
    ...
    fn initial_and_shared_helpers_require_exact_source_routes_and_source_enablement() {
        ...
        for target in [
            "/repo.git-more/ui/initial/",
            "/repo.git/ui/initial/../private",
            "/repo.git/ui/initial/%2e%2e",
    ...
    fn static_initial_does_not_accept_request_bodies_queries_or_oversized_responses() {
        for (method, query, headers, trailing, maximum) in [
            ...
            let bytes = format!(
                "{method} /repo.git/ui/initial/{query} HTTP/1.1\r\nHost: local\r\n{headers}\r\n"
            );
            let request = head::parse(bytes.as_bytes(), HttpLimits::default())
                .unwrap()
                .unwrap();
