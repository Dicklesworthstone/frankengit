...
pub(super) fn execute(
    ...
    match command {
        Command::Inspect(query) => {
            ...
            if query
                .expected_object
                .is_some_and(|expected| expected != report.tip)
            {
...
fn read_error(error: NodeWorkspaceRefusal) -> ApiError {
    match error {
        NodeWorkspaceRefusal::Tag(TagRefusal::SnapshotMoved) => {
            ApiError::new(Status::Conflict, "source_snapshot_moved")
        }
        NodeWorkspaceRefusal::Tag(TagRefusal::Budget(_)) => ApiError::too_large(),
        NodeWorkspaceRefusal::Tag(
            TagRefusal::InvalidName | TagRefusal::InvalidLimits | TagRefusal::ObjectFormat,
...
fn publication_error(error: NodeWorkspaceRefusal) -> ApiError {
    match error {
        ...
        NodeWorkspaceRefusal::Tag(
            TagRefusal::InvalidName | TagRefusal::InvalidMetadata | TagRefusal::ObjectFormat,
...
mod tests {
    ...
    fn tag_read_failures_never_invent_absence_or_mutation_outcomes() {
        assert_eq!(
            read_error(NodeWorkspaceRefusal::Tag(TagRefusal::SnapshotMoved)).status,
            Status::Conflict
        );
        for reason in [
            TagRefusal::InvalidObject,
            TagRefusal::TargetKindMismatch,
            TagRefusal::Cycle,
        ] {
        ...
        assert_eq!(
            read_error(NodeWorkspaceRefusal::RefUnavailable).status,
            Status::NotFound
        );
    }
    #[test]
    fn publication_cancellation_preserves_ambiguity_but_syntax_does_not() {
        ...
        assert!(
            !publication_error(NodeWorkspaceRefusal::Tag(TagRefusal::InvalidName)).outcome_unknown
        );
