...
impl Command {
    ...
    pub(super) fn from_fields(
        ...
        if take(&mut fields, "profile")? != "path-v1" {
            return Err(ApiError::bad("unsupported_replay_profile"));
        }
        ...
        let mainline = fields
            .remove("mainline")
            .map(|text| {
                u16::try_from(parse_decimal(&text)?)
                    .ok()
                    .filter(|n| *n != 0)
        ...
        for (name, value) in [
            ...
            if let Some(text) = fields.remove(name) {
                *value = usize::try_from(parse_decimal(&text)?)
                    .map_err(|_| ApiError::bad("invalid_replay_limit"))?;
            }
        }
        limits
            .validate()
...
fn take(fields: &mut BTreeMap<String, String>, name: &str) -> Result<String, ApiError> {
    fields
        .remove(name)
        .ok_or_else(|| ApiError::bad("missing_replay_field"))
}
fn reference(
...
fn oid(text: &str, format: GitHashAlgorithm) -> Result<GitOid, ApiError> {
    if text.len() != 2 * format.digest_len() {
        return Err(ApiError::bad("invalid_native_oid"));
    }
...
mod tests {
    ...
    fn form(format: GitHashAlgorithm) -> String {
        ...
        format!(
            "profile=path-v1&object_format={}&target_ref=refs/heads/main&source_ref=refs/heads/topic&expected_target={oid}&expected_source={oid}&commit={oid}&author=User+%3Cu%40example.invalid%3E&timestamp=1&message=Replay",
            format.as_str()
        )
    }
    #[test]
    fn both_formats_explicit_direction_mainline_and_raw_bytes_remain_exact() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            let input = form(format)
                ...
                .replace("message=Replay", "message_hex=ff0d0a")
                + "&mainline=2&max_objects=7";
    ...
    fn selector_overrides_duplicate_fields_and_widened_limits_never_change_semantics() {
        ...
        for extra in [
            ...
            "&max_objects=10001",
            "&max_edges=0",
            "&max_output_bytes=33554433",
            "&message_hex=61",
            "&target_ref_hex=726566732f68656164732f61",
        ] {
            assert!(
                Command::parse(
                    (input.clone() + extra).as_bytes(),
        ...
        for changed in [
            input.replace("refs/heads/main", "refs/tags/main"),
            input.replace("timestamp=1", "timestamp=01"),
            input.replace("object_format=sha1", "object_format=sha256"),
            input.replace("message=Replay", "message_hex=00"),
            input.replace("profile=path-v1", "profile=automatic"),
        ] {
            assert!(
                Command::parse(
                    changed.as_bytes(),
