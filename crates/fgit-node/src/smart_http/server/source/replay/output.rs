...
use fgit_forge::preparation::replay::{
    ReplayCoordinates, ReplayDirection, ReplayPreparation, ReplayRequest,
};
...
fn valid_oid(format: GitHashAlgorithm, id: GitOid) -> bool {
    !id.is_zero() && id.algorithm() == format
}
fn coordinates(value: ReplayCoordinates, requested: ReplayRequest) -> Result<(), ApiError> {
    if value.request != requested
        || requested
            .mainline
            .is_some_and(|n| value.selected_mainline != Some(n))
        || !matches!(
            (value.selected_parent, value.selected_mainline),
            (None, None) | (Some(_), Some(1..=u16::MAX))
        )
        || value
            .selected_parent
            .is_some_and(|id| !valid_oid(requested.target.algorithm(), id))
    {
...
pub(super) fn entry(
    ...
    if !valid_oid(format, value.oid)
        || path.rsplit(|b| *b == b'/').next() != Some(value.name.as_slice())
        || !matches!(
            value.mode,
...
pub(super) fn conflict(
    ...
    Ok(format!(
        ...
        quote(&hex(&value.path)),
        quote(kind(value.kind)),
        entry(value.base.as_ref(), &value.path, format)?,
        entry(value.ours.as_ref(), &value.path, format)?,
        entry(value.theirs.as_ref(), &value.path, format)?
    ))
}
...
pub(super) fn build(
    ...
    let point = match outcome {
        ...
        ReplayPreparation::Conflicted { coordinates, .. }
        | ReplayPreparation::NoChange { coordinates } => *coordinates,
    };
    ...
    let direction = match command.inputs.direction {
    ...
    append(
        ...
        &format!(
            ...
            quote(&token),
            quote(direction),
            ref_fields("target_ref", &command.target),
            ref_fields("source_ref", &command.source),
            quote(&point.request.target.to_string()),
            quote(&point.request.source_tip.to_string()),
            quote(&point.request.selected_commit.to_string()),
            point
                .selected_parent
    ...
    if let Some((choices, paths)) = resolutions {
        super::resolution::append_receipts(
            &mut metadata,
            choices,
            paths,
            node.object_format,
    ...
    let status = match outcome {
        ReplayPreparation::Clean(plan) => {
            ...
            if !valid_oid(node.object_format, plan.commit)
                || !valid_oid(node.object_format, plan.tree)
                || plan.commit == command.inputs.target
                || plan.objects.is_empty()
                || plan.objects.len() > command.limits.max_objects
                || pack_objects == 0
                || pack_objects > command.limits.max_objects
                || borrowed_objects > pack_objects
            {
                return Err(ApiError::unavailable());
            }
            append(
                ...
                &format!(
                    ...
                    quote(&command.inputs.target.to_string()),
                    plan.objects.len(),
                    pack_objects,
                    borrowed_objects,
                    bytes.len(),
                    quote(&bytes.digest_hex())
                ),
            )?;
            Status::Success
        }
        ReplayPreparation::Conflicted { conflicts, .. } => {
            ...
            append(
                &mut metadata,
                "\"state\":\"conflicted\",\"candidate_commit\":null,\"root_tree\":null,\"bundle\":null,\"conflicts\":[",
            )?;
            for (index, value) in conflicts.iter().enumerate() {
                ...
                append(
                    &mut metadata,
                    &conflict(value, node.object_format, command.limits.max_path_bytes)?,
                )?;
            }
            append(&mut metadata, "]}")?;
            Status::Conflict
        ...
        ReplayPreparation::NoChange { .. } => {
            if bundle.is_some() || counts != (0, 0) {
                return Err(ApiError::unavailable());
            }
            append(
                &mut metadata,
                "\"state\":\"no_change\",\"candidate_commit\":null,\"root_tree\":null,\"bundle\":null,\"conflicts\":[]}",
            )?;
            Status::Success
        }
    };
...
mod tests {
    use super::*;
    fn id() -> GitOid {
        GitOid::from_hex(GitHashAlgorithm::Sha1, &"ab".repeat(20)).unwrap()
    }
    #[test]
    fn replay_response_cannot_change_direction_source_or_mainline() {
        ...
        let mut point = ReplayCoordinates {
            request,
            selected_parent: Some(id()),
    ...
    fn conflict_paths_and_side_identities_are_lossless_and_bound() {
        ...
        let mut value = MergeConflict {
            path: b"dir/\xff".to_vec(),
        ...
        assert!(
            conflict(&value, GitHashAlgorithm::Sha1, 4096)
                .unwrap()
                .contains("6469722fff")
        );
