...
fn parse_parts(
    ...
    for (name, value) in parse_form(form, 32 + PreparationLimits::default().max_conflicts)? {
        if name == "resolution" {
            if descriptors.len() == PreparationLimits::default().max_conflicts {
    ...
    if command.expected_head.is_none() {
        return Err(ApiError::bad("resolution_requires_snapshot"));
    }
    ...
    for descriptor in descriptors {
        ...
        let choice = match fields.next() {
            ...
            Some("file") => {
                ...
                if content.len() > command.limits.max_text_bytes
                    || retained > command.limits.max_output_bytes
                {
                    return Err(ApiError::too_large());
                }
        ...
        if retained > command.limits.max_output_bytes {
            return Err(ApiError::too_large());
        }
...
pub(super) fn append_receipts(
    ...
    if paths.is_empty()
        || paths.len() != choices.len()
        || paths.len() > limits.max_conflicts
        || paths
            .windows(2)
            .any(|rows| rows[0].conflict.path >= rows[1].conflict.path)
    {
    ...
    for (index, (choice, path)) in choices.iter().zip(paths).enumerate() {
        checkpoint(live)?;
        if choice.path != path.conflict.path
            || choice.choice.kind() != path.choice
        ...
        let expected = match &choice.choice {
            ...
            ResolutionChoice::File { mode, bytes } => {
                ...
                if result.mode != *mode
                    || result.oid != git_object_id(format, GitObjectKind::Blob, bytes)
                {
                    return Err(ApiError::unavailable());
                }
                Some(result)
        ...
        if path.result.as_ref() != expected {
            return Err(ApiError::unavailable());
        }
        let selected = match path.choice {
        ...
        append(
            out,
            &format!(
                "{}{{\"conflict\":{conflict},\"choice\":{},\"result\":{result}}}",
                if index == 0 { "" } else { "," },
                quote(selected)
            ),
        )?;
...
mod tests {
    ...
    fn form() -> String {
        let id = "ab".repeat(20);
        format!(
            "profile=path-v1&object_format=sha1&target_ref=refs/heads/main&source_ref=refs/heads/topic&expected_target={id}&expected_source={id}&commit={id}&author=U+%3Cu%40example.invalid%3E&timestamp=1&message=Resolve&expected_head=alg:1:{}",
            "cd".repeat(32)
        )
    }
    #[test]
    fn exact_binary_and_empty_files_are_consumed_once_without_normalization() {
        for bytes in [b"\0\xff\r\n".as_slice(), b"".as_slice()] {
            ...
            let (_, choices) = parse_parts(
                form.as_bytes(),
                BTreeMap::from([("file_0", bytes)]),
    ...
    fn empty_duplicate_overlapping_unpinned_and_unused_choices_cannot_fall_back() {
        for suffix in [
            ...
            "&resolution=61:ours&resolution=612f62:ours",
            "&resolution=2e2e2f61:ours",
            "&resolution=61:ours:extra",
            "&resolution=61:file:120000:file_0",
            "&resolution=61:file:100644:file_0&resolution=62:file:100644:file_0",
            "&resolution=61:ours&force=true",
        ] {
    ...
    fn cancelled_parsing_and_forged_resolution_receipts_refuse() {
        ...
        let side = fgit_forge::preparation::MergeEntry {
            name: b"a".to_vec(),
            mode: 0o100644,
            oid: id,
        };
        let mut receipt = ResolvedPath {
            conflict: fgit_forge::preparation::MergeConflict {
                path: b"a".to_vec(),
                kind: fgit_forge::preparation::ConflictKind::Content,
                base: None,
                ours: Some(side.clone()),
                theirs: None,
            },
            choice: ResolutionKind::Ours,
            result: Some(side),
        };
        append_receipts(
            &mut String::new(),
