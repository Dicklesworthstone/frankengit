...
use super::{
    ApiError, GitDaemonSessionDeadline, GitDaemonSessionWorkScaling, JsonReply,
    LoopbackReceiveSession, OneNode, Status, drive_request_while, output, read_error, read_form,
};
...
impl<'a> Request<'a> {
    ...
    fn command(&self, bytes: &[u8], format: GitHashAlgorithm) -> Result<Command, ApiError> {
        ...
        query
            .validate(format)
            .map_err(|_| ApiError::bad("invalid_historical_browse_query"))?;
...
fn take(fields: &mut BTreeMap<String, String>, name: &str) -> Result<String, ApiError> {
    fields
        .remove(name)
        .ok_or_else(|| ApiError::bad("historical_source_selection_required"))
}
fn positive(
...
fn oid(text: &str, format: GitHashAlgorithm) -> Result<GitOid, ApiError> {
    ...
    let id =
        GitOid::from_hex(format, text).map_err(|_| ApiError::bad("invalid_historical_commit"))?;
    if id.is_zero() {
        return Err(ApiError::bad("invalid_historical_commit"));
    }
    Ok(id)
...
fn prefix(command: &Command) -> String {
    format!(
        concat!(
            "{{\"type\":\"historical_source\",\"schema_version\":1,",
        ...
        quote(&command.expected_ref_tip.to_string()),
        quote(&command.commit.to_string())
    )
}
fn wrap(
    mut prefix: String,
