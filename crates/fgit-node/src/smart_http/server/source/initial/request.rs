...
pub(crate) enum Operation { Prepare, Apply }
...
impl<'a> Request<'a> {
    pub(crate) fn parse(envelope: &Envelope<'a>) -> Result<Option<Self>, ApiError> {
        let (path, query) = envelope.target.split_once('?')
        ...
        if envelope.method != "POST" { return Err(ApiError::method()); }
        if query.is_some() || envelope.git_protocol.is_some()
            || matches!(envelope.body, BodyFraming::Empty | BodyFraming::ContentLength(0))
        ...
        let request = Self { repository_route, boundary, operation };
    ...
    pub(super) fn command(&self, bytes: &[u8], format: GitHashAlgorithm) -> Result<Command, ApiError> {
        ...
        for (name, value) in parse_form(bytes, 9)? {
            ...
            let applicable = match self.operation {
                Operation::Prepare => matches!(name.as_str(), "author" | "committer" | "timestamp" | "message" | "expected_head"),
        ...
        let reference = RefName::try_new(name.as_bytes()).map_err(|_| ApiError::bad("invalid_ref"))?;
        match self.operation {
            Operation::Prepare => {
                ...
                let expected_head = fields.remove("expected_head").map(|value| parse_snapshot(&value)).transpose()?;
                Ok(Command::Prepare { reference, metadata, expected_head })
            }
            Operation::Apply => {
                ...
                let candidate = GitOid::from_hex(format, &text).map_err(|_| ApiError::bad("invalid_candidate_commit"))?;
                if candidate.is_zero() { return Err(ApiError::bad("invalid_candidate_commit")); }
                Ok(Command::Apply { reference, candidate })
            }
        }
    }
}
fn take(fields: &mut BTreeMap<String, String>, key: &str) -> Result<String, ApiError> {
    fields.remove(key).ok_or_else(|| ApiError::bad("missing_required_field"))
}

#[derive(Debug)]
pub(super) enum Command {
    Prepare { reference: RefName, metadata: MergeMetadata, expected_head: Option<RepositoryAuthorityHeadId> },
    Apply { reference: RefName, candidate: GitOid },
}
...
mod tests {
    ...
    fn command(operation: Operation, bytes: &[u8], format: GitHashAlgorithm) -> Result<Command, ApiError> {
        Request { repository_route: "/repo.git", boundary: "x", operation }.command(bytes, format)
    }
    #[test]
    fn preparation_has_explicit_metadata_and_no_parent() {
        let Command::Prepare { reference, metadata, expected_head } = command(Operation::Prepare, PREPARE, GitHashAlgorithm::Sha1).unwrap() else { panic!("prepare") };
        ...
        for extra in ["&expected_commit=0", "&principal=owner", "&force=true", "&expected_absent=true", "&candidate_commit=abc"] {
            let mut bytes = PREPARE.to_vec(); bytes.extend_from_slice(extra.as_bytes());
    ...
    fn application_is_creation_only_in_the_exact_native_domain() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            let body = format!("object_format={}&ref=refs/heads/new&expected_absent=true&candidate_commit={}", format.as_str(), "a".repeat(format.digest_len() * 2));
            assert!(command(Operation::Apply, body.as_bytes(), format).is_ok());
            for bad in [body.replace("=true", "=false"), body.replace("refs/heads/", "refs/tags/"),
                body.replace(&"a".repeat(format.digest_len() * 2), &"0".repeat(format.digest_len() * 2)),
                format!("{body}&expected_head=ignored"), format!("{body}&author=ignored")] {
    ...
    fn initial_routes_do_not_steal_existing_source_operations() {
        for (path, mutation) in [("initial/prepare", Some(false)), ("initial/apply", Some(true)), ("prepare", None), ("refs", None)] {
            let bytes = format!("POST /repo.git/api/v1/source/{path} HTTP/1.1\r\nHost: local\r\nContent-Type: multipart/form-data; boundary=x\r\nContent-Length: 1\r\n\r\n");
            let envelope = head::parse(bytes.as_bytes(), HttpLimits::default()).unwrap().unwrap();
            assert_eq!(Request::parse(&envelope).unwrap().map(|request| request.is_mutation()), mutation);
        }
    }
}
