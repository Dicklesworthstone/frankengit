...
fn contains(
    ...
    for start in (0..bytes.len()).step_by(64 * 1024) {
        check(live)?;
        let end = start
            .saturating_add(64 * 1024 + marker.len() - 1)
            .min(bytes.len());
        if bytes[start..end]
            .windows(marker.len())
...
impl Prepared {
    pub(in crate::smart_http::server::source) fn send(
        &self,
...
pub(super) fn prepared(
    ...
    check(live)?;
    if plan.object_format != node.object_format
        || plan.commit.is_zero()
        || plan.tree.is_zero()
        || [plan.commit, plan.tree]
            .iter()
            .any(|id| id.algorithm() != node.object_format)
        || plan.files.is_empty()
        || plan.files.len() > 1024
        || plan
            .files
    ...
    if bundle.bytes().is_empty() || bundle.bytes().len() > MAX_BUNDLE {
        return Err(ApiError::too_large());
    }
    let commit = plan
        .objects
        .iter()
        .find(|object| object.id == plan.commit && object.kind == GitObjectKind::Commit)
    ...
    append(
        ...
        &format!(
            concat!(
                "{{\"type\":\"initial_source_preparation\",{},{},",
    ...
    for (index, file) in plan.files.iter().enumerate() {
        ...
        if file.path.is_empty()
            ...
            || file.blob.is_zero()
            || file.blob.algorithm() != node.object_format
        {
            return Err(ApiError::unavailable());
        }
        append(
...
pub(super) fn publication(
    ...
    let body = format!(
        concat!(
            "{{\"type\":\"initial_source_publication\",{},\"principal_id\":{},{},",
    ...
    if body.len() > maximum {
        eprintln!(
            "Initial source receipt exceeded response limit after canonical transaction {}; recover the original key",
            command.tx_id
        );
        return Err(ApiError::unknown());
    }
    Ok(Reply {
        status,
        body,
        terminal: Some((command.tx_id, terminal)),
    })
}
