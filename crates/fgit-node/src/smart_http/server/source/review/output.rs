...
use super::super::super::{
    Status,
    issues::{ApiError, quote},
};
use super::Command;
use crate::OneNode;
use fgit_forge::review::{
    ChangeKind, ComparisonMode, EntryIdentity, ReviewContent, ReviewOptions, ReviewSelection,
    ReviewSpan, ReviewedEntry, SourceReview,
};
...
fn valid_oid(id: GitOid, format: GitHashAlgorithm) -> bool {
    !id.is_zero() && id.algorithm() == format
}
fn blob(entry: Option<EntryIdentity>) -> bool {
    entry.is_some_and(|e| matches!(e.mode, 0o100644 | 0o100755 | 0o120000))
}
...
fn validate_entries(
    ...
    if entries.len() > options.limits.max_changes
    ...
    for entry in entries {
        ...
        match &entry.content {
            ReviewContent::Identical => {
                if !blob(entry.before)
                    || !blob(entry.after)
                    || entry
                        .before
                        .zip(entry.after)
                        .is_none_or(|(a, b)| a.oid != b.oid)
                {
                    return Err(ApiError::unavailable());
                }
            }
            ...
            | ReviewContent::Text {
                ...
                if (!blob(entry.before) && *before_bytes != 0)
                    || (!blob(entry.after) && *after_bytes != 0)
                    || (!blob(entry.before) && !blob(entry.after))
                    || *before_bytes > options.limits.max_blob_bytes
                    || *after_bytes > options.limits.max_blob_bytes
                {
                    return Err(ApiError::unavailable());
                }
                if let ReviewContent::Text {
                    additions,
                    deletions,
                    hunks,
                    ..
...
fn entry_identity(out: &mut Output, value: Option<EntryIdentity>) -> Result<(), ApiError> {
    match value {
        ...
        Some(value) => out.append(&format!(
            "{{\"object_id\":{},\"mode\":{}}}",
            quote(&value.oid.to_string()),
            quote(&format!("{:06o}", value.mode))
        )),
    }
}
...
fn render_entry(
    ...
    out.append("{\"path_hex\":")?;
    out.hex(&entry.path, live)?;
    out.append(&format!(",\"kind\":{},\"before\":", quote(kind)))?;
    entry_identity(out, entry.before)?;
    out.append(",\"after\":")?;
    entry_identity(out, entry.after)?;
    out.append(",\"content\":{")?;
    match &entry.content {
        ...
        ReviewContent::Text {
            ...
            out.append(&format!(
                concat!(
                    "\"kind\":\"text\",\"algorithm\":{},\"additions\":{},\"deletions\":{},",
            ...
            for (index, hunk) in hunks.iter().enumerate() {
                ...
                out.append(",\"new\":")?;
                span(out, hunk.new)?;
                out.append(",\"before_hex\":")?;
                out.hex(&hunk.before, live)?;
                out.append(",\"after_hex\":")?;
                out.hex(&hunk.after, live)?;
                out.append("}")?;
            }
            out.append("]")?;
        }
...
pub(super) fn render(
    ...
    if command
        .expected_head
        .is_some_and(|expected| expected != report.source_head)
    {
    ...
    if command
        .expected_before
        .is_some_and(|id| id != comparison.requested_before)
        || command
            .expected_after
            .is_some_and(|id| id != comparison.requested_after)
    {
    ...
    match &command.selection {
        ReviewSelection::References { before, after } => {
            if &report.before_reference != before
        ...
        ReviewSelection::PullRequest {
            ...
            if report.pull_request.is_none_or(|(actual, version)| {
                actual != *number || expected_version.is_some_and(|expected| expected != version)
            }) {
                return Err(ApiError::unavailable());
            }
        }
    ...
    if comparison.mode != command.options.mode
        ...
        .iter()
        .any(|id| !valid_oid(*id, node.object_format))
    {
        return Err(ApiError::unavailable());
    }
    validate_entries(
    ...
    let pr = report.pull_request.map_or_else(
        || "null".into(),
        |(number, version)| {
            format!(
                "{{\"number\":{},\"version\":{}}}",
    ...
    out.append(",\"after_ref_hex\":")?;
    out.hex(report.after_reference.as_bytes(), live)?;
    out.append(&format!(
        concat!(
            ",\"requested_before\":{},\"requested_after\":{},\"compared_before\":{},",
    ...
    for (index, entry) in comparison.entries.iter().enumerate() {
        ...
        render_entry(&mut out, entry, live)?;
    }
    out.append("]}")?;
    checkpoint(live)?;
    Ok(out.body)
...
mod tests {
    ...
    fn directory_records_and_prefixes_do_not_become_file_payloads() {
        ...
        entry.content = ReviewContent::Binary {
            before_bytes: 0,
            after_bytes: 4,
        };
