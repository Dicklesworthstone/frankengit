...
use super::super::{
    Status,
    issues::{ApiError, Reply as JsonReply, admission_error},
};
...
pub(super) fn execute(
    ...
    match command {
        Command::Prepare {
            ...
            let candidate = drive_request_while(
                node,
                &context,
                node.prepare_trusted_patch_in(
                    &context,
                    &reference,
                    base,
                    *authenticated.principal_id().as_bytes(),
                    payload,
                    &metadata,
                    PatchLimits::default(),
                ),
            ...
            drop(bytes);
            output::prepared(node, &reference, base, candidate, maximum, &mut live)
                .map(Reply::patch)
        }
        Command::Candidate {
            reference,
            ...
            let inspected = drive_request_while(
                node,
                &context,
                node.inspect_workspace_bundle_in(
                    &context,
                    &reference,
                    base,
                    candidate,
            ...
            let body = output::inspection(
                ...
                &inspected.bundle_sha256,
                inspected.bundle_bytes,
                maximum,
                &mut live,
            )?;
        ...
        Command::Candidate {
            ...
            let result = drive_request_while(
                node,
                &context,
                node.apply_workspace_bundle_durable_in(
                    &context,
    ...
    match error {
        ...
        NodeWorkspaceRefusal::WorkspacePatch(error) => match error {
            ...
            PatchError::ContextMismatch { .. } => {
                ApiError::new(Status::Conflict, "patch_context_mismatch")
            }
            PatchError::SourcePresence
            | PatchError::SourceMode
            | PatchError::SourceRange { .. } => {
...
mod tests {
    ...
    fn failed_patch_construction_is_not_a_canonical_mutation_refusal() {
        for error in [
            NodeWorkspaceRefusal::StaleWorkspaceBase,
            NodeWorkspaceRefusal::WorkspacePatch(PatchError::ContextMismatch {
                hunk: 1,
                source_line: 2,
            }),
