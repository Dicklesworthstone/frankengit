...
use super::super::{
    Status,
    issues::{ApiError, admission_error},
    pulls::{read_source_upload, source_upload},
};
...
pub(super) fn execute(
    ...
    match command {
        Command::Prepare {
            ...
            let (head, plan, bundle) = drive_request_while(
                node,
                &context,
                node.prepare_trusted_initial_patch_in(
                    &context,
                    &reference,
                    payload,
                    &metadata,
            ...
            drop(bytes);
            output::prepared(node, &reference, head, plan, bundle, maximum, &mut live)
                .map(Reply::initial)
        }
        Command::Apply {
            reference,
            ...
            let result = drive_request_while(
                node,
                &context,
                node.apply_initial_patch_bundle_durable_in(
                    &context,
                    session,
                    &reference,
                    candidate,
...
mod tests {
    ...
    fn failed_construction_has_no_transaction_but_interrupted_publication_is_unknown() {
        ...
        assert!(
            !publication_error(NodeWorkspaceRefusal::InvalidWorkspaceCandidate(
                "initial commit has a parent"
            ))
            .outcome_unknown
        );
    }
