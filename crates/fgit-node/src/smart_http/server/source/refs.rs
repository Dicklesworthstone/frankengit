    ...
    match command {
        Command::List(page) => {
            ...
            let read = async {
                match page.namespace {
                    Namespace::All => node.list_refs_in(&context, &visibility,
                        page.after.as_ref(), page.limit, page.expected_head).await,
                    Namespace::Branches => node.list_branch_refs_in(&context, &visibility,
                        page.after.as_ref(), page.limit, page.expected_head).await,
                    Namespace::Tags => node.list_tag_refs_in(&context, &visibility,
                        page.after.as_ref(), page.limit, page.expected_head).await,
                }
            };
            let (head, rows, next) = drive_request_while(node, &context, read, &mut live)
        ...
        Command::Mutate(commands) => {
            ...
            let result = drive_request_while(node, &context,
                node.admit_branch_updates_durable_in(&context, session, &commands, Default::default()),
...
fn publication_error(error: NodeWorkspaceRefusal) -> ApiError {
    match error {
        ...
        NodeWorkspaceRefusal::BranchOperation(_) => ApiError::new(Status::Conflict, "branch_operation_refused"),
        NodeWorkspaceRefusal::CommitRequired => ApiError::new(Status::Conflict, "branch_target_not_commit"),
