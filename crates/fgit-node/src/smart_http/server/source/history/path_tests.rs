...
fn path_forms_accept_raw_bytes_and_keep_existing_snapshot_contracts() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let command =
            Command::parse((base.clone() + "&path_hex=6469722fff").as_bytes(), format).unwrap();
        assert_eq!(command.path.as_deref(), Some(b"dir/\xff".as_slice()));
        assert_eq!(command.options, LogOptions::default());
        assert!(
            Command::parse((base.clone() + "&path_hex=61&after=1").as_bytes(), format).is_err()
        );
        let pinned = format!(
            "{base}&path_hex=61&after=1&expected_head=alg:1:{}",
            "a".repeat(64)
        );
        assert_eq!(
            Command::parse(pinned.as_bytes(), format)
                .unwrap()
                .options
                .after,
            1
...
fn malformed_duplicate_oversized_and_inapplicable_path_fields_refuse() {
    ...
    for suffix in [
        ...
        "&path_hex=2f61",
        "&path_hex=612f",
        "&path_hex=61&path_hex=62",
        "&path_hex=61&max_tree_entries=0",
        "&path_hex=61&max_tree_entries=100001",
        "&path_hex=61&max_cached_bytes=33554433",
        "&max_tree_entries=12",
        ...
        assert!(
            Command::parse((base.clone() + suffix).as_bytes(), format).is_err(),
            "{suffix}"
        );
    }
...
fn path_response_identifies_exact_unsimplified_selection_and_charges_output() {
    ...
    assert!(
        out.body
            .contains("\"path_selection\":\"changed-against-any-parent-v1\"")
    );
    assert!(
        out.body
            .contains("\"total_commits_scope\":\"matching-path\"")
    );
...
fn outer_source_routing_keeps_path_history_in_the_read_only_profile() {
    ...
    let bytes = format!(
        "POST /repo.git/api/v1/source/log HTTP/1.1\r\nHost: local\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: {}\r\n\r\n",
        form.len()
    );
    ...
    assert!(
        Command::parse(form.as_bytes(), GitHashAlgorithm::Sha1)
            .unwrap()
            .path
            .is_some()
    );
