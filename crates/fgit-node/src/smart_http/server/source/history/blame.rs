...
use super::super::super::{
    Status,
    issues::{ApiError, Reply, parse_form, quote},
};
use super::{
    Output, Selection, checkpoint, commit, failure, identity, number, selection, take, valid_oid,
};
...
impl Command {
    fn parse(bytes: &[u8], format: GitHashAlgorithm) -> Result<Self, ApiError> {
        ...
        let end_line = if fields.contains_key("line_end") {
            Some(number(&mut fields, "line_end", 0)?)
        } else {
            None
        };
        ...
        options
            .validate()
            .map_err(|_| ApiError::bad("invalid_blame_range_path_or_limits"))?;
...
fn validate(
    ...
    if result.path != query.path
        ...
        || !valid_oid(format, result.tip)
        || !valid_oid(format, result.tree)
        || !valid_oid(format, result.blob)
        || result.graph_commits == 0
        || result.graph_commits > query.limits.max_commits
        || result.comparisons > query.limits.max_comparisons
        || result.algorithms.len() > query.limits.max_comparisons
    ...
    for origin in &result.origins {
        checkpoint(live)?;
        if !valid_oid(format, origin.id) || !origin_ids.insert(origin.id) {
            return Err(ApiError::unavailable());
        }
        metadata = metadata
            .checked_add(origin.body.len())
            .ok_or_else(ApiError::too_large)?;
        if metadata > query.limits.max_metadata_bytes {
    ...
    for (index, line) in result.lines.iter().enumerate() {
        checkpoint(live)?;
        if line.line != result.first_line + index
            ...
            || !valid_oid(format, line.origin_blob)
        ...
        let start = line
            .byte_start
            .checked_sub(result.content_byte_start)
...
fn render(
    ...
    out.append(&format!(
        concat!(
            ",\"profile\":\"exact-lines-all-parents-v1\",\"scope\":\"same_path\",",
    ...
    out.append(&format!(
        concat!(
            ",\"total_lines\":{},\"first_line\":{},\"end_line\":{},",
            "\"content_byte_start\":{},\"content_hex\":"
        ),
        result.total_lines, result.first_line, result.end_line, result.content_byte_start
    ))?;
    out.hex(&result.content, live)?;
    out.append(&format!(
        ",\"graph_commits\":{},\"comparisons\":{},\"max_diff_work\":{},\"algorithms\":[",
...
mod tests {
    ...
    fn blame_forms_preserve_byte_paths_and_bound_every_requested_dimension() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            let base = format!(
                "object_format={}&ref=refs/heads/main&path_hex=6469722fff",
                format.as_str()
            );
            let query = Command::parse(
                (base.clone() + "&line_start=1&line_end=3").as_bytes(),
                format,
            )
            .unwrap();
            assert_eq!(query.options.path, b"dir/\xff");
            assert_eq!(
                (query.options.first_line, query.options.end_line),
                (1, Some(3))
            );
            for extra in [
                "&path_hex=61",
                ...
                "&max_commits=0",
                "&max_edges=16385",
                "&max_tree_entries=100001",
                "&max_blob_bytes=1048577",
                "&max_lines=20001",
                "&max_cached_bytes=33554433",
                "&max_comparisons=129",
            ...
            for path in ["", "FF", "0", "00", "2e2e2f61", "2f61", "612f2f62"] {
                let input = format!(
                    "object_format={}&ref=refs/heads/main&path_hex={path}",
                    format.as_str()
                );
    ...
    fn fixture() -> (BlameOptions, BlameResult) {
        ...
        let result = BlameResult {
            ...
            first_line: 1,
            end_line: 2,
            content_byte_start: 5,
            content: b"two".to_vec(),
            graph_commits: 1,
            comparisons: 0,
            algorithms: vec![],
            origins: vec![HistoryCommit {
                id: tip,
                tree,
                parents: vec![],
                body,
    ...
    fn blame_route_is_read_only_and_shares_the_same_outer_authorization() {
        ...
        let envelope =
            fgit_wire::smart_http::head::parse(bytes, fgit_wire::smart_http::HttpLimits::default())
                .unwrap()
                .unwrap();
