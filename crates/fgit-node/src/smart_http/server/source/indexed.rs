...
use fgit_graph::lexical::{IndexError, IndexedLexicalReport, LexicalChannel, LexicalError,
    LexicalQuery, LexicalQueryLimits, LexicalReadLimits, MAX_WORK, PROFILE};
use fgit_treefs::TreePath;
use fgit_types::{CANONICAL_CODEC_VERSION, DigestBytes, GitHashAlgorithm, GitOid,
    HeadGeneration, InternalObjectId, RefName};
...
fn generation_id(text: &str) -> Result<GraphGenerationId, ApiError> {
    ...
    let raw = text.strip_prefix(&prefix).ok_or_else(|| ApiError::bad("invalid_index_generation"))?;
    let width = fgit_crypto::DigestAlgorithm::from_id(algorithm).ok_or_else(ApiError::unavailable)?.digest_len();
    let bytes = unhex(raw, width)?;
    if bytes.len() != width || bytes.iter().all(|byte| *byte == 0) { return Err(ApiError::bad("invalid_index_generation")); }
    let digest = DigestBytes::try_new(&bytes).map_err(|_| ApiError::bad("invalid_index_generation"))?;
...
fn command(bytes: &[u8], format: GitHashAlgorithm) -> Result<Command, ApiError> {
    ...
    for (name, value) in parse_form(bytes, 180)? {
        match name.as_str() {
            "term_hex" => {
                if terms.len() == 32 { return Err(ApiError::too_large()); }
                terms.push(unhex(&value, 128)?); continue;
            }
            "path_prefix_hex" => {
                if prefixes.len() == 128 { return Err(ApiError::too_large()); }
                prefixes.push(unhex(&value, 4096)?); continue;
            }
            "ref" | "object_format" | "expected_head" | "expected_commit" | "channel"
            | "index_token" | "index_number" | "minimum_index_token" | "minimum_index_number"
            | "after" | "limit" | "max_work" | "max_payload_bytes" => {}
    ...
    let expected_head = fields.remove("expected_head").map(|v| parse_snapshot(&v)).transpose()?;
    let expected_commit = fields.remove("expected_commit").map(|value| {
        if unhex(&value, format.digest_len())?.len() != format.digest_len() { return Err(ApiError::bad("invalid_expected_commit")); }
        let id = GitOid::from_hex(format, &value).map_err(|_| ApiError::bad("invalid_expected_commit"))?;
    ...
    let channel = match fields.remove("channel").as_deref().unwrap_or("content") {
        "content" => LexicalChannel::Content, "path" => LexicalChannel::Path,
        _ => return Err(ApiError::bad("unsupported_index_channel")),
    };
    // Apply the same path policy as native TreeFS, before index reads.
    for prefix in &prefixes { TreePath::parse_default(prefix).map_err(|_| ApiError::bad("invalid_search_scope"))?; }
    let query = LexicalQuery::new(channel, &terms, &prefixes).map_err(|_| ApiError::bad("invalid_index_query"))?;
    ...
    let after = fields.remove("after").map(|v| parse_decimal(&v)).transpose()?;
    if after == Some(0) || (after.is_some() && (generation.is_none() || expected_head.is_none() || expected_commit.is_none())) {
    ...
    let reads = LexicalReadLimits { max_payload_bytes: positive(&mut fields, "max_payload_bytes",
        defaults.max_payload_bytes as u64, defaults.max_payload_bytes as u64)? as usize, ..defaults };
...
pub(super) fn failure(error: NodeWorkspaceRefusal) -> ApiError {
    match error {
        ...
        NodeWorkspaceRefusal::SourceIndex(error) => match *error {
            IndexError::Uninitialized => ApiError::new(Status::Conflict, "source_index_uninitialized"),
            ...
            | IndexError::Generation(GenerationAuthorityError::ReadCancelled) => ApiError::from_status(Status::Timeout, false),
            IndexError::Generation(GenerationAuthorityError::CheckpointUnresolved) => ApiError::new(Status::Conflict, "index_checkpoint_unavailable"),
            IndexError::Generation(GenerationAuthorityError::InvalidReadLimits) => ApiError::bad("invalid_index_limits"),
            IndexError::Generation(GenerationAuthorityError::ReadBudgetExceeded(_)) => ApiError::too_large(),
...
pub(super) fn execute(node: &OneNode, _request: &Request<'_>, session: &LoopbackReceiveSession,
    ...
    let report = drive_request_while(node, &context, node.search_source_index_local_in(&context,
        &command.selection.reference, command.selection.expected_head, command.selection.expected_commit,
        command.generation.as_ref(), command.minimum.as_ref(), &command.query, command.after,
        command.limits, command.reads), &mut live).map_err(failure)?;
...
fn rows(command: &Command, report: &IndexedLexicalReport, maximum: usize,
    live: &mut impl FnMut() -> bool,
) -> Result<String, ApiError> {
    let hits = &report.results.hits;
    if report.query != command.query || hits.len() > command.limits.max_results
        || report.results.work_units > command.limits.max_work || hits.len() > report.indexed_documents
        || report.indexed_documents > 20_000 || report.indexed_source_bytes > 64 * 1024 * 1024
        || report.non_regular_entries > 50_000 || report.segments_read > command.reads.max_segments
        ...
        || (!report.results.complete && (hits.len() != command.limits.max_results
    ...
    for (index, hit) in hits.iter().enumerate() {
        ...
        if hit.document_id <= previous_id || previous_path.is_some_and(|p| p >= hit.path.as_slice())
            || hit.blob.is_zero() || hit.blob.algorithm() != report.source.namespace.object_format
            || hit.content_bytes as usize > 8 * 1024 * 1024 || hit.spans.len() != command.query.terms().len()
            || (!command.query.prefixes().is_empty() && !command.query.prefixes().iter().any(|prefix|
                hit.path == *prefix || (hit.path.starts_with(prefix) && hit.path.get(prefix.len()) == Some(&b'/'))))
        { return Err(ApiError::unavailable()); }
        previous_id = hit.document_id; previous_path = Some(&hit.path);
        append(&mut out, &format!("{}{{\"document_id\":{},\"path_hex\":{},\"blob\":{},\"content_bytes\":{},\"spans\":[",
            if index == 0 { "" } else { "," }, hit.document_id, quote(&hex(&hit.path)), quote(&hit.blob.to_string()), hit.content_bytes), maximum)?;
        for (i, span) in hit.spans.iter().enumerate() {
            let bound = match command.query.channel() { LexicalChannel::Content => hit.content_bytes as usize, LexicalChannel::Path => hit.path.len() };
            if span.query_index != i || usize::from(span.byte_length) != command.query.terms()[i].len()
                || (span.byte_offset as usize).checked_add(usize::from(span.byte_length)).is_none_or(|end| end > bound)
            { return Err(ApiError::unavailable()); }
            append(&mut out, &format!("{}{{\"query_index\":{},\"byte_offset\":{},\"byte_length\":{}}}",
                if i == 0 { "" } else { "," }, i, span.byte_offset, span.byte_length), maximum)?;
        }
...
fn render(node: &OneNode, command: &Command, report: &IndexedLexicalReport, maximum: usize,
    ...
    if source.namespace.tenant != node.tenant_id || source.namespace.repository != node.repository_id
        || source.namespace.incarnation != node.repository_incarnation_id() || source.namespace.object_format != node.object_format
        || source.reference != command.selection.reference
        || [source.commit, source.tree].iter().any(|id| id.is_zero() || id.algorithm() != node.object_format)
        || command.selection.expected_head.is_some_and(|head| head != source.source_head)
        || command.selection.expected_commit.is_some_and(|commit| commit != source.commit)
        || command.generation.as_ref().is_some_and(|value| value != &report.generation)
        || report.selected_generation_head.authority_generation < report.generation.authority_generation
        || command.minimum.as_ref().is_some_and(|floor| floor.authority_generation > report.selected_generation_head.authority_generation)
    { return Err(ApiError::unavailable()); }
    let hits = rows(command, report, maximum, live)?;
    let mut out = String::new();
    append(&mut out, &format!(concat!("{{\"type\":\"source_search_index\",\"schema_version\":1,\"profile\":{},",
        ...
        quote(&node.repository_incarnation_id().to_string()), quote(node.object_format.as_str()), ref_fields("ref", &source.reference),
        quote(&source.source_head.to_string()), quote(&token(source.source_head.as_internal_object_id())),
        quote(&source.source_rcr.to_string()), quote(&source.commit.to_string()), quote(&source.tree.to_string()),
        quote(&token(report.generation.generation_id.as_internal_object_id())), report.generation.authority_generation.get(),
        quote(&token(report.selected_generation_head.generation_id.as_internal_object_id())), report.selected_generation_head.authority_generation.get(),
        quote(match command.query.channel() { LexicalChannel::Content => "content", LexicalChannel::Path => "path" }),
        optional(command.after), command.limits.max_results, report.results.hits.len(), report.results.complete, optional(report.results.next_after),
        report.indexed_documents, report.indexed_source_bytes, report.non_regular_entries, report.segments_read,
        report.payload_bytes_read, report.generation_bytes_read, report.results.work_units), maximum)?;
    for (i, term) in command.query.terms().iter().enumerate() {
        append(&mut out, &format!("{}{}", if i == 0 { "" } else { "," }, quote(&hex(term))), maximum)?;
    }
    append(&mut out, "],\"path_prefix_hex\":[", maximum)?;
    for (i, prefix) in command.query.prefixes().iter().enumerate() {
        check(live)?;
        append(&mut out, &format!("{}{}", if i == 0 { "" } else { "," }, quote(&hex(prefix))), maximum)?;
    }
    append(&mut out, "],\"hits\":[", maximum)?; append(&mut out, &hits, maximum)?;
