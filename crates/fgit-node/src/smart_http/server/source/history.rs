...
use super::super::{Status, issues::{ApiError, MAX_FORM_BYTES, Reply, parse_decimal, parse_form, parse_snapshot, quote, read_form}};
...
impl Command {
    fn parse(bytes: &[u8], format: GitHashAlgorithm) -> Result<Self, ApiError> {
        ...
        options.validate().map_err(|_| ApiError::bad("invalid_history_limits"))?;
...
fn selection(fields: &mut BTreeMap<String, String>, format: GitHashAlgorithm) -> Result<Selection, ApiError> {
    if take(fields, "object_format")? != format.as_str() { return Err(ApiError::bad("object_format_mismatch")); }
    let reference = RefName::try_new(take(fields, "ref")?.as_bytes()).map_err(|_| ApiError::bad("invalid_ref"))?;
    let expected_head = fields.remove("expected_head").map(|text| parse_snapshot(&text)).transpose()?;
    let expected_commit = fields.remove("expected_commit").map(|text| {
        ...
        let oid = GitOid::from_hex(format, &text).map_err(|_| ApiError::bad("invalid_expected_commit"))?;
        if oid.is_zero() { return Err(ApiError::bad("invalid_expected_commit")); }
        Ok(oid)
    }).transpose()?;
...
fn failure(snapshot_moved: bool, unavailable: bool, unpinned: bool, cause: Option<&HistoryError>) -> ApiError {
    if snapshot_moved { return ApiError::new(Status::Conflict, "source_snapshot_moved"); }
    if unavailable { return ApiError::not_found(); }
    if unpinned { return ApiError::bad("history_continuation_requires_snapshot"); }
    match cause {
        Some(HistoryError::InvalidOptions | HistoryError::LineRange) => ApiError::bad("invalid_history_range_or_limits"),
        Some(HistoryError::PathUnavailable) => ApiError::not_found(),
        Some(HistoryError::BinaryContent) => ApiError::new(Status::Conflict, "binary_blame_unsupported"),
...
fn valid_oid(format: GitHashAlgorithm, oid: GitOid) -> bool { !oid.is_zero() && oid.algorithm() == format }
fn check_selection(request: &Selection, head: RepositoryAuthorityHeadId, tip: GitOid,
    format: GitHashAlgorithm,
) -> Result<(), ApiError> {
    if request.expected_head.is_some_and(|expected| expected != head) {
        return Err(ApiError::new(Status::Conflict, "source_snapshot_moved"));
    }
    if request.expected_commit.is_some_and(|expected| expected != tip) {
        return Err(ApiError::new(Status::Conflict, "source_commit_moved"));
    }
    if !valid_oid(format, tip) { return Err(ApiError::unavailable()); }
    Ok(())
}
...
impl Output {
    ...
    fn finish(self, live: &mut impl FnMut() -> bool) -> Result<String, ApiError> {
        checkpoint(live)?; Ok(self.body)
    }
}
fn identity(out: &mut Output, node: &OneNode, request: &Selection, head: RepositoryAuthorityHeadId,
    tip: GitOid, kind: &str, live: &mut impl FnMut() -> bool,
) -> Result<(), ApiError> {
    checkpoint(live)?;
    check_selection(request, head, tip, node.object_format)?;
    let internal = head.as_internal_object_id();
    let digest: String = internal.digest().as_bytes().iter().map(|b| format!("{b:02x}")).collect();
...
fn commit(out: &mut Output, row: &HistoryCommit, format: GitHashAlgorithm,
    live: &mut impl FnMut() -> bool,
) -> Result<(), ApiError> {
    checkpoint(live)?;
    if !valid_oid(format, row.id) || !valid_oid(format, row.tree) || row.body.len() > 64 * 1024
    ...
    out.append(&format!("{{\"object_id\":{},\"tree\":{},\"parents\":[", quote(&row.id.to_string()), quote(&row.tree.to_string())))?;
...
fn validate_page_bounds(page: &HistoryPage, options: LogOptions, format: GitHashAlgorithm) -> Result<(), ApiError> {
    ...
    for row in &page.commits {
        if !ids.insert(row.id) { return Err(ApiError::unavailable()); }
        bytes = bytes.checked_add(row.body.len()).ok_or_else(ApiError::unavailable)?;
        if bytes > options.limits.max_metadata_bytes { return Err(ApiError::too_large()); }
    }
    Ok(())
}
...
fn render_log(node: &OneNode, command: &Command, head: RepositoryAuthorityHeadId, page: &HistoryPage,
    ...
    out.append(&format!(concat!(",\"ordering\":\"child-before-parent-native-id-v1\",\"page_complete\":true,",
        ...
        page.next_after.map_or_else(|| "null".into(), |n| n.to_string())))?;
...
mod tests {
    ...
    fn form(format: GitHashAlgorithm) -> String { format!("object_format={}&ref=refs/heads/main", format.as_str()) }
    #[test]
    fn history_forms_are_closed_and_continuations_require_the_original_snapshot() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            ...
            assert!(Command::parse((base + "&expected_commit=" + &"0".repeat(format.digest_len() * 2)).as_bytes(), format).is_err());
        }
    }
    #[test]
    ...
    fn partial_or_duplicate_history_pages_and_forged_native_records_refuse() {
        ...
        let page = HistoryPage { tip: id, total_commits: 1, after: 0, next_after: None, commits: vec![row.clone()] };
        ...
        let mut forged = row.clone(); forged.body.push(b'x');
    ...
    fn internal_failures_are_not_empty_successes_or_transaction_outcomes() {
        for cause in [HistoryError::CyclicHistory, HistoryError::Budget("commits"), HistoryError::InvalidTree] {
            let failure = failure(false, false, false, Some(&cause));
            assert!(!failure.outcome_unknown);
            let mut bytes = Vec::new();
            failure.send_named(&mut bytes, fgit_wire::smart_http::HttpVersion::Http11, "source_error").unwrap();
