...
use fgit_forge::tags::{TagCommand, TagMetadata, TagReadLimits, validate_tag_name,
    MAX_TAG_MESSAGE_BYTES, MAX_TAG_NAME_BYTES};
...
use super::super::super::issues::{ApiError, MAX_FORM_BYTES, parse_decimal, parse_form, parse_snapshot};

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(super) enum Operation { Lightweight, Annotated, Delete, Inspect }
impl Operation {
    pub(super) const fn as_str(self) -> &'static str {
        match self { Self::Lightweight => "lightweight", Self::Annotated => "annotated",
            Self::Delete => "delete", Self::Inspect => "inspect" }
    }
}
#[derive(Debug)]
...
#[derive(Debug)]
pub(super) enum Command { Mutate(TagCommand), Inspect(Inspection) }
...
impl<'a> Request<'a> {
    pub(super) fn parse(head: &Envelope<'a>) -> Result<Option<Self>, ApiError> {
        ...
        let operation = match action {
            "lightweight" => Operation::Lightweight, "annotated" => Operation::Annotated,
            "delete" => Operation::Delete, "inspect" => Operation::Inspect,
    ...
    pub(super) fn is_mutation(&self) -> bool { self.operation != Operation::Inspect }
    ...
    pub(super) fn command(&self, bytes: &[u8], format: GitHashAlgorithm) -> Result<Command, ApiError> {
        ...
        for (name, value) in parse_form(bytes, 10)? {
            ...
            let applicable = match self.operation {
                ...
                Operation::Annotated => matches!(name.as_str(), "target" | "target_kind" | "tagger" | "timestamp" | "message_hex"),
                Operation::Delete => name == "expected_object",
                Operation::Inspect => matches!(name.as_str(), "expected_head" | "expected_object" | "max_tags" | "max_object_bytes" | "max_total_bytes"),
            };
        ...
        if self.operation == Operation::Inspect {
            ...
            let expected_object = fields.remove("expected_object").map(|text| oid(&text, format)).transpose()?;
            let defaults = TagReadLimits::default();
            let limits = TagReadLimits {
                max_tags: bound(&mut fields, "max_tags", defaults.max_tags)?,
                max_object_bytes: bound(&mut fields, "max_object_bytes", defaults.max_object_bytes)?,
                max_total_bytes: bound(&mut fields, "max_total_bytes", defaults.max_total_bytes)?,
            };
            limits.validate().map_err(|_| ApiError::bad("invalid_tag_limits"))?;
            return Ok(Command::Inspect(Inspection { reference: name, expected_head, expected_object, limits }));
        }
        let command = match self.operation {
            Operation::Lightweight => TagCommand::Lightweight { name, target: oid(&take(&mut fields, "target")?, format)? },
            Operation::Delete => TagCommand::Delete { name, expected: oid(&take(&mut fields, "expected_object")?, format)? },
            Operation::Annotated => TagCommand::Annotated {
                name, target: oid(&take(&mut fields, "target")?, format)?,
                target_kind: match take(&mut fields, "target_kind")?.as_str() {
                    "commit" => GitObjectKind::Commit, "tree" => GitObjectKind::Tree,
                    "blob" => GitObjectKind::Blob, "tag" => GitObjectKind::Tag,
                    _ => return Err(ApiError::bad("invalid_target_kind")),
                },
                metadata: TagMetadata { tagger: take(&mut fields, "tagger")?,
        ...
        command.prepare(format).map_err(|_| ApiError::bad("invalid_tag_command"))?;
        Ok(Command::Mutate(command))
    }
}
fn take(fields: &mut BTreeMap<String, String>, name: &str) -> Result<String, ApiError> {
    fields.remove(name).ok_or_else(|| ApiError::bad("missing_field"))
}
fn bound(fields: &mut BTreeMap<String, String>, name: &str, maximum: usize) -> Result<usize, ApiError> {
    let Some(text) = fields.remove(name) else { return Ok(maximum); };
    let value = usize::try_from(parse_decimal(&text)?).map_err(|_| ApiError::bad("invalid_tag_limits"))?;
    if value == 0 || value > maximum { return Err(ApiError::bad("invalid_tag_limits")); }
    Ok(value)
}
fn oid(text: &str, format: GitHashAlgorithm) -> Result<GitOid, ApiError> {
    if text.len() != format.digest_len() * 2 || !text.bytes().all(|byte|
    ...
    if id.is_zero() { return Err(ApiError::bad("zero_object_id")); }
    Ok(id)
}
fn unhex(text: &str, maximum: usize) -> Result<Vec<u8>, ApiError> {
    if text.len() > maximum * 2 { return Err(ApiError::too_large()); }
    if text.len() % 2 != 0 { return Err(ApiError::bad("invalid_hex")); }
    let digit = |byte| match byte { b'0'..=b'9' => Ok(byte - b'0'),
        b'a'..=b'f' => Ok(byte - b'a' + 10), _ => Err(ApiError::bad("invalid_hex")) };
    text.as_bytes().chunks_exact(2).map(|pair| Ok((digit(pair[0])? << 4) | digit(pair[1])?)).collect()
}
...
mod tests {
    ...
    fn parse(operation: Operation, body: &str) -> Result<Command, ApiError> {
        Request { repository_route: "/repo.git", operation }.command(body.as_bytes(), GitHashAlgorithm::Sha256)
    }
    fn basic() -> String { format!("object_format=sha256&ref=refs/tags/v1&target={}", "a".repeat(64)) }
    #[test]
    fn annotations_preserve_raw_names_messages_and_explicit_zero_timestamp() {
        ...
        let Command::Mutate(TagCommand::Annotated { name, metadata, .. }) = parse(Operation::Annotated, &body).unwrap() else { panic!("annotation") };
        assert_eq!(name.as_bytes(), b"refs/tags/v\xff");
        assert_eq!(metadata.timestamp, 0); assert_eq!(metadata.message, b"\xff\r\nx");
        assert!(parse(Operation::Annotated, &body.replace("message_hex=ff0d0a78", "message_hex=")).is_ok());
    }
    #[test]
    fn creation_deletion_and_inspection_have_disjoint_expectations() {
        assert!(parse(Operation::Lightweight, &basic()).is_ok());
        assert!(parse(Operation::Delete, &basic().replace("target=", "expected_object=")).is_ok());
        assert!(parse(Operation::Inspect, "object_format=sha256&ref=refs/tags/v1").is_ok());
        for suffix in ["&force=true", "&expected_object=absent", "&principal=owner", "&tagger=a", "&ref_hex=78"] {
    ...
    fn wrong_domains_duplicate_fields_and_non_tag_names_refuse() {
        for body in [basic().replace("sha256", "sha1"), basic().replace("refs/tags/v1", "refs/heads/main"),
            basic().replace(&"a".repeat(64), &"0".repeat(64)), basic().replace(&"a".repeat(64), &"A".repeat(64)),
            basic() + "&target=" + &"b".repeat(64)] {
            assert!(parse(Operation::Lightweight, &body).is_err());
        }
        assert!(unhex("aa0", 4).is_err()); assert!(unhex("AA", 4).is_err());
        assert!(unhex("aaaa", 1).is_err());
    }
    #[test]
    fn inspection_limits_can_only_narrow() {
        let base = "object_format=sha256&ref=refs/tags/v1";
        assert!(parse(Operation::Inspect, &(base.to_owned() + "&max_tags=1&max_total_bytes=16")).is_ok());
        for field in ["max_tags=65", "max_tags=0", "max_tags=01", "max_total_bytes=4194305", "max_object_bytes=1048577"] {
    ...
    fn invalid_envelopes_refuse_before_body_intake() {
        for (path, method, headers) in [
            ...
            ("/repo.git/api/v1/source/tags/annotated", "POST", "Git-Protocol: version=2\r\n"),
        ] {
            let bytes = format!("{method} {path} HTTP/1.1\r\nHost: local\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: 1\r\n{headers}\r\n");
            let head = fgit_wire::smart_http::head::parse(bytes.as_bytes(), Default::default()).unwrap().unwrap();
