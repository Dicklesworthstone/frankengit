...
struct Json<'a, C> { text: String, maximum: usize, live: &'a mut C }
impl<C: FnMut() -> bool> Json<'_, C> {
    fn reserve(&mut self, length: usize) -> Result<(), ApiError> {
        if !(self.live)() { return Err(ApiError::from_status(Status::Timeout, false)); }
        let end = self.text.len().checked_add(length).filter(|end| *end <= self.maximum)
    ...
    fn put(&mut self, value: &str) -> Result<(), ApiError> {
        self.reserve(value.len())?; self.text.push_str(value); Ok(())
    }
    fn hex(&mut self, bytes: &[u8]) -> Result<(), ApiError> {
        self.reserve(bytes.len().checked_mul(2).and_then(|count| count.checked_add(2))
            .ok_or_else(ApiError::too_large)?)?;
        self.text.push('"');
...
pub(super) fn inspection(node: &OneNode, query: &Inspection, report: &TagRead,
    ...
    if report.reference != query.reference
        ...
        || query.expected_object.is_some_and(|object| object != report.tip)
        || report.annotations.len() > query.limits.max_tags
        || report.peeled_kind == GitObjectKind::Tag
        || [report.tip, report.peeled].iter().any(|id| id.is_zero() || id.algorithm() != node.object_format)
    { return Err(ApiError::unavailable()); }
    let internal = report.head.as_internal_object_id();
    let digest: String = internal.digest().as_bytes().iter().map(|byte| format!("{byte:02x}")).collect();
    ...
    out.put(&format!(concat!("{{\"type\":\"source_tag\",{},\"source_head\":{},\"snapshot_token\":{},",
        ...
        metadata(node), quote(&report.head.to_string()), quote(&token)))?;
    ...
    for (index, annotation) in report.annotations.iter().enumerate() {
        out.reserve(0)?;
        total_bytes = total_bytes.checked_add(annotation.body.len()).filter(|total| *total <= query.limits.max_total_bytes)
            .ok_or_else(ApiError::unavailable)?;
        let following_kind = if index + 1 == report.annotations.len() { report.peeled_kind } else { GitObjectKind::Tag };
        if annotation.id != expected || annotation.target.is_zero()
            ...
            || git_object_id(node.object_format, GitObjectKind::Tag, &annotation.body) != annotation.id
        { return Err(ApiError::unavailable()); }
        expected = annotation.target;
        let signature = match annotation.signature { TagSignatureState::Absent => "absent",
            TagSignatureState::OpaqueUnverifiable => "opaque_unverifiable" };
        out.put(&format!(concat!("{}{{\"object_id\":{},\"target\":{},\"target_kind\":{},",
            "\"signature\":{},\"signature_verified\":false,\"body_bytes\":{},\"body_hex\":"),
            if index == 0 { "" } else { "," }, quote(&annotation.id.to_string()),
            quote(&annotation.target.to_string()), quote(annotation.target_kind.label()),
            quote(signature), annotation.body.len()))?;
        out.hex(&annotation.body)?; out.put("}")?;
    }
    if expected != report.peeled { return Err(ApiError::unavailable()); }
    out.put("]}")?;
    Ok(out.text)
}
...
fn terminal(result: &AdmissionResult) -> Result<(TxId, TerminalOutcome), ApiError> {
    ...
    if !result.session.atomic || command.tx_id != *tx { return Err(ApiError::unknown()); }
    Ok((*tx, command.terminal))
}

pub(super) fn publication(node: &OneNode, principal: PrincipalId, operation: Operation,
    command: &RefCommand, result: &AdmissionResult, maximum: usize,
) -> Result<Reply, ApiError> {
    let (tx, terminal) = terminal(result)?;
    let build = || -> Result<String, ApiError> {
        let old = match command.expected_old { ExpectedOld::Absent => "null".into(),
            ExpectedOld::Exactly(id) => quote(&id.to_string()), ExpectedOld::Unspecified => return Err(ApiError::unknown()) };
        let new = match command.proposed_new { ProposedNew::Update(id) => quote(&id.to_string()), ProposedNew::Delete => "null".into() };
        ...
        let mut live = || true;
        let mut out = Json { text: String::new(), maximum: maximum.min(64 * 1024), live: &mut live };
        ...
        out.hex(command.name.as_bytes())?; out.put("}")?;
        Ok(out.text)
    };
    ...
    Ok(Reply { status: match terminal.outcome { DecisionOutcome::Committed { .. } => Status::Success,
        DecisionOutcome::Refused { .. } => Status::Conflict }, body, terminal: Some((tx, terminal)) })
}
...
mod tests {
    ...
    fn hex_is_lossless_and_size_failure_precedes_growth() {
        let mut live = || true;
        let mut out = Json { text: String::new(), maximum: 32, live: &mut live };
        out.hex(b"\xff\r\n<script>").unwrap();
        assert_eq!(out.text, "\"ff0d0a3c7363726970743e\"");
        let prior = out.text.clone(); assert!(out.hex(b"too many bytes").is_err());
        assert_eq!(out.text, prior);
    }
    #[test]
    fn cancellation_can_interrupt_large_annotation_encoding() {
        let mut calls = 0; let mut live = || { calls += 1; calls < 3 };
