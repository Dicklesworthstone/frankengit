...
impl<'a> Request<'a> {
    pub fn parse(head: &Envelope<'a>) -> Result<Option<Self>, ApiError> {
        ...
        let operation = match action {
            "prepare" => Operation::Prepare, "inspect" => Operation::Inspect, "apply" => Operation::Apply,
            _ => return Ok(None),
        };
        ...
        let request = Self { repository_route, operation, boundary };
    ...
    pub fn command(&self, bytes: &[u8], format: GitHashAlgorithm) -> Result<Command, ApiError> {
        ...
        for (name, value) in parse_form(bytes, 7)? {
            ...
            let applicable = match self.operation {
                Operation::Prepare => matches!(name.as_str(), "author" | "committer" | "timestamp" | "message"),
                Operation::Inspect | Operation::Apply => name == "candidate_commit",
            };
            if !common && !applicable { return Err(ApiError::bad("unknown_source_change_field")); }
        ...
        match self.operation {
            Operation::Prepare => {
                ...
                Ok(Command::Prepare { reference, base, metadata })
            }
            Operation::Inspect | Operation::Apply => {
                let candidate = oid(&take(&mut fields, "candidate_commit")?, format)?;
                if candidate == base { return Err(ApiError::bad("candidate_equals_parent")); }
                Ok(Command::Candidate { reference, base, candidate })
            }
        }
    }
}
...
pub(super) enum Command {
    Prepare { reference: RefName, base: GitOid, metadata: MergeMetadata },
    Candidate { reference: RefName, base: GitOid, candidate: GitOid },
}
fn take(fields: &mut BTreeMap<String, String>, name: &str) -> Result<String, ApiError> {
    fields.remove(name).ok_or_else(|| ApiError::bad("missing_source_change_field"))
}
...
mod tests {
    ...
    fn request(operation: Operation) -> Request<'static> {
        Request { repository_route: "/repo.git", operation, boundary: "source" }
    }
    fn form(format: GitHashAlgorithm) -> String {
        format!("object_format={}&ref=refs/heads/main&expected_commit={}", format.as_str(), "a".repeat(format.digest_len() * 2))
    }
    #[test]
    fn explicit_metadata_and_candidate_hash_domains_are_preserved() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            let body = form(format) + "&author=Alice+%3Ca%40example.invalid%3E&committer=Bot+%3Cb%40example.invalid%3E&timestamp=1&message=Exact%0D%0A%252f";
            let Command::Prepare { base, metadata, .. } = request(Operation::Prepare).command(body.as_bytes(), format).unwrap() else { panic!("prepare"); };
            ...
            for operation in [Operation::Inspect, Operation::Apply] {
                assert!(matches!(request(operation).command(body.as_bytes(), format), Ok(Command::Candidate { .. })));
            }
        }
    }
    #[test]
    fn commands_cannot_infer_identity_force_or_missing_preconditions() {
        ...
        for body in [valid.clone() + "&force=true", valid.clone() + "&principal=admin",
            valid.clone() + "&expected_commit=" + &"a".repeat(40),
            valid.replace("refs/heads/main", "refs/tags/tag"), valid.replace(&"b".repeat(40), &"a".repeat(40)),
            valid.replace("object_format=sha1", "object_format=sha256"),
            valid.replace("expected_commit=", "ignored="), valid.replace(&"b".repeat(40), &"0".repeat(40))]
        { assert!(request(Operation::Apply).command(body.as_bytes(), GitHashAlgorithm::Sha1).is_err()); }
        assert!(request(Operation::Prepare).command(form(GitHashAlgorithm::Sha1).as_bytes(), GitHashAlgorithm::Sha1).is_err());
    }
    #[test]
