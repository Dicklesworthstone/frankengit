...
use fgit_forge::review::{ChangeKind, ComparisonMode, EntryIdentity, ReviewContent, ReviewSpan, SourceReview};
...
fn optional_oid(id: Option<GitOid>) -> String { id.map_or_else(|| "null".into(), |id| quote(&id.to_string())) }
...
fn append(out: &mut String, text: &str, maximum: usize) -> Result<(), ApiError> {
    if out.len().checked_add(text.len()).is_none_or(|n| n > maximum) { return Err(ApiError::too_large()); }
    out.try_reserve(text.len()).map_err(|_| ApiError::unavailable())?;
    out.push_str(text); Ok(())
}
fn append_hex(out: &mut String, bytes: &[u8], maximum: usize, live: &mut impl FnMut() -> bool) -> Result<(), ApiError> {
    let count = bytes.len().checked_mul(2).and_then(|n| n.checked_add(2)).ok_or_else(ApiError::too_large)?;
    if out.len().checked_add(count).is_none_or(|n| n > maximum) { return Err(ApiError::too_large()); }
    out.try_reserve(count).map_err(|_| ApiError::unavailable())?;
    out.push('"');
    const HEX: &[u8; 16] = b"0123456789abcdef";
    for chunk in bytes.chunks(16 * 1024) {
        checkpoint(live)?;
        for byte in chunk { out.push(char::from(HEX[usize::from(byte >> 4)])); out.push(char::from(HEX[usize::from(byte & 15)])); }
    }
    out.push('"'); Ok(())
}
...
pub(super) fn prepared(node: &OneNode, reference: &RefName, base: GitOid,
    candidate: WorkspacePatchCandidate, maximum: usize, live: &mut impl FnMut() -> bool,
) -> Result<PatchReply, ApiError> {
    checkpoint(live)?;
    if candidate.object_format != node.object_format || candidate.source_commit != base
        || candidate.candidate_commit.is_zero() || candidate.paths.is_empty() || candidate.paths.len() > 1024
    ...
    for (index, path) in candidate.paths.iter().enumerate() {
        checkpoint(live)?;
        if path.path.is_empty() || path.path.len() > 4096 { return Err(ApiError::unavailable()); }
        append(&mut metadata, &format!("{}{{\"path_hex\":{},\"old_blob\":{},\"new_blob\":{},\"new_mode\":{},\"hunks\":{}}}",
            if index == 0 { "" } else { "," }, quote(&hex(&path.path)), optional_oid(path.old_blob), optional_oid(path.new_blob),
            path.new_mode.map_or_else(|| "null".into(), |mode| mode.to_string()), path.hunks), MAX_METADATA)?;
    }
    ...
    for attempt in 0..16 {
        ...
        if !contains(metadata.as_bytes(), marker.as_bytes(), live)? && !contains(bytes, marker.as_bytes(), live)? {
    ...
    let suffix = format!("\r\n--{boundary}--\r\n");
    let length = prefix.len().checked_add(bytes.len()).and_then(|n| n.checked_add(suffix.len()))
        .filter(|n| *n <= maximum).ok_or_else(ApiError::too_large)?;
    checkpoint(live)?;
    Ok(PatchReply { candidate, content_type: format!("multipart/mixed; boundary={boundary}"), prefix, suffix, length })
}

fn entry(id: Option<EntryIdentity>) -> String {
    id.map_or_else(|| "null".into(), |id| format!("{{\"mode\":{},\"oid\":{}}}", id.mode, quote(&id.oid.to_string())))
}
fn span(value: ReviewSpan, bytes: &[u8], total: usize) -> Result<String, ApiError> {
    ...
    { return Err(ApiError::unavailable()); }
    Ok(format!("{{\"byte_start\":{},\"byte_end\":{},\"line_start\":{},\"line_count\":{}}}", value.byte_start, value.byte_end, value.line_start, value.line_count))
}
pub(super) fn inspection(node: &OneNode, reference: &RefName, base: GitOid, candidate: GitOid,
    review: &SourceReview, parents: &[GitOid], commit: &[u8], bundle_digest: &[u8; 32], bundle_bytes: usize,
    maximum: usize, live: &mut impl FnMut() -> bool,
) -> Result<String, ApiError> {
    checkpoint(live)?;
    let maximum = maximum.min(MAX_INSPECTION);
    let comparison = &review.comparison;
    if review.repository_id != node.repository_id || review.before_reference != *reference
        || review.after_reference != *reference || review.pull_request.is_some()
        || comparison.mode != ComparisonMode::Direct || comparison.requested_before != base
        || comparison.compared_before != base || comparison.requested_after != candidate || parents != [base]
        || commit.len() > 2 * 1024 * 1024 || comparison.entries.len() > 512
    ...
    append(&mut out, &format!(",\"comparison\":{{\"mode\":\"direct\",\"before_tree\":{},\"after_tree\":{},\"entry_count\":{},\"entries\":[",
        quote(&comparison.before_tree.to_string()), quote(&comparison.after_tree.to_string()), comparison.entries.len()), maximum)?;
    for (index, row) in comparison.entries.iter().enumerate() {
        checkpoint(live)?;
        if row.path.is_empty() || row.path.len() > 4096 { return Err(ApiError::unavailable()); }
        let kind = match row.kind { ChangeKind::Added => "added", ChangeKind::Deleted => "deleted", ChangeKind::Modified => "modified",
            ChangeKind::ModeChanged => "mode_changed", ChangeKind::TypeChanged => "type_changed" };
        append(&mut out, &format!("{}{{\"path_hex\":{},\"kind\":{},\"before\":{},\"after\":{},\"content\":",
            if index == 0 { "" } else { "," }, quote(&hex(&row.path)), quote(kind), entry(row.before), entry(row.after)), maximum)?;
        match &row.content {
            ReviewContent::Identical => append(&mut out, "{\"type\":\"identical\",\"content_read\":false}", maximum)?,
            ReviewContent::ObjectOnly => append(&mut out, "{\"type\":\"object_only\",\"content_read\":false}", maximum)?,
            ReviewContent::Binary { before_bytes, after_bytes } => append(&mut out,
                &format!("{{\"type\":\"binary\",\"before_bytes\":{before_bytes},\"after_bytes\":{after_bytes},\"body_included\":false}}"), maximum)?,
            ReviewContent::Text { algorithm, additions, deletions, before_bytes, after_bytes, hunks } => {
                ...
                append(&mut out, &format!("{{\"type\":\"text\",\"algorithm\":{},\"additions\":{additions},\"deletions\":{deletions},\"before_bytes\":{before_bytes},\"after_bytes\":{after_bytes},\"hunks\":[", quote(&format!("{algorithm:?}"))), maximum)?;
                let mut ends = (0, 0);
                for (index, hunk) in hunks.iter().enumerate() {
                    ...
                    append(&mut out, &format!("{}{{\"old\":{old},\"new\":{new},\"before_hex\":", if index == 0 { "" } else { "," }), maximum)?;
                    append_hex(&mut out, &hunk.before, maximum, live)?;
                    append(&mut out, ",\"after_hex\":", maximum)?; append_hex(&mut out, &hunk.after, maximum, live)?;
    ...
    append(&mut out, "]}}", maximum)?; checkpoint(live)?; Ok(out)
}

pub(super) fn publication(node: &OneNode, principal: PrincipalId, reference: &RefName,
    base: GitOid, candidate: GitOid, result: AdmissionResult, maximum: usize,
) -> Result<JsonReply, ApiError> {
    let [command] = result.commands.as_slice() else { return Err(ApiError::unknown()); };
    if !result.session.atomic || result.session.tx_ids.as_slice() != [command.tx_id] { return Err(ApiError::unknown()); }
    ...
    Ok(JsonReply { status, body, terminal: Some((command.tx_id, command.terminal)) })
}
...
mod tests {
    ...
    fn byte_growth_is_checked_before_encoding_and_cancellation_is_not_success() {
        ...
        assert!(append_hex(&mut out, b"xx", 6, &mut || true).is_err()); assert_eq!(out, "abc");
        let mut out = String::new(); append_hex(&mut out, b"\0\xff\r\n", 10, &mut || true).unwrap();
    ...
    fn boundary_scans_cover_chunk_edges_and_spans_reject_inconsistent_lengths() {
        let mut bytes = vec![b'x'; 65535]; bytes.extend_from_slice(b"--boundary");
        assert!(contains(&bytes, b"--boundary", &mut || true).unwrap());
        assert!(contains(&bytes, b"--boundary", &mut || false).is_err());
        assert!(span(ReviewSpan { byte_start: 0, byte_end: 3, line_start: 0, line_count: 1 }, b"x\r\n", 3).is_ok());
        assert!(span(ReviewSpan { byte_start: 0, byte_end: 4, line_start: 0, line_count: 1 }, b"x\r\n", 3).is_err());
    }
}
