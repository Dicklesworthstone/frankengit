...
use fgit_forge::source_search::regex::{MAX_REGEX_STEPS, RegexQuery, RegexQueryError, RegexSearchReport};
...
impl<'a> Request<'a> {
    pub(super) fn parse(head: &Envelope<'a>) -> Result<Option<Self>, ApiError> {
        ...
        let Some(route) = path.strip_suffix("/api/v1/source/search-regex") else { return Ok(None); };
...
#[derive(Debug)]
struct Command { selection: Selection, query: RegexQuery, limits: SearchLimits }
fn take(fields: &mut BTreeMap<String, String>, name: &str) -> Result<String, ApiError> {
    fields.remove(name).ok_or_else(|| ApiError::bad("missing_regex_field"))
}
    ...
    -> Result<u64, ApiError>
{
    ...
    if value == 0 || value > maximum { return Err(ApiError::bad("invalid_regex_limit")); }
    Ok(value)
}
...
fn command(bytes: &[u8], format: GitHashAlgorithm) -> Result<Command, ApiError> {
    ...
    for (name, value) in parse_form(bytes, 140)? {
        ...
        if !matches!(name.as_str(), "ref" | "object_format" | "expected_head" | "expected_commit"
            | "pattern_hex" | "case" | "max_matches" | "max_bytes" | "max_file_bytes" | "max_steps")
    ...
    let expected_commit = fields.remove("expected_commit").map(|value| {
        ...
        if raw.len() != format.digest_len() { return Err(ApiError::bad("invalid_expected_commit")); }
...
pub(super) fn execute(node: &OneNode, _request: &Request<'_>, session: &LoopbackReceiveSession,
    ...
    let (head, report) = drive_request_while(node, &context,
        node.search_source_regex_snapshot_local_in(&context, &command.selection.reference,
            command.selection.expected_head, command.selection.expected_commit, &command.query, command.limits),
        &mut live).map_err(read_error)?;
    let body = render(node, &command, head, &report,
        usize::try_from(maximum).unwrap_or(usize::MAX), &mut live)?;
...
fn append(out: &mut String, value: &str, maximum: usize) -> Result<(), ApiError> {
    if out.len().checked_add(value.len()).is_none_or(|n| n > maximum.min(super::output::MAX_REPLY_BYTES)) {
        return Err(ApiError::too_large());
    }
...
fn match_json(row: &SourceMatch, query: &RegexQuery, format: GitHashAlgorithm,
    maximum_file: usize,
) -> Result<String, ApiError> {
    let path = TreePath::parse_default(&row.path).map_err(|_| ApiError::unavailable())?;
    let end = row.byte_offset.checked_add(row.match_length).ok_or_else(ApiError::unavailable)?;
    let excerpt_end = row.excerpt_offset.checked_add(row.excerpt.len()).ok_or_else(ApiError::unavailable)?;
    let column = row.byte_column.checked_sub(1).ok_or_else(ApiError::unavailable)?;
    let line_start = row.byte_offset.checked_sub(column).ok_or_else(ApiError::unavailable)?;
    if row.line == 0 || end > maximum_file || excerpt_end > maximum_file
        || row.excerpt.len() > 416 || row.excerpt.contains(&b'\n')
        || row.excerpt_offset < line_start || row.excerpt_offset > row.byte_offset
        || row.byte_offset > excerpt_end || row.blob.is_zero() || row.blob.algorithm() != format
        || (!query.prefixes().is_empty() && !query.prefixes().iter().any(|p| path.starts_with(p)))
    { return Err(ApiError::unavailable()); }
    Ok(format!(concat!("{{\"path_hex\":{},\"blob\":{},\"byte_offset\":{},\"line\":{},",
        "\"byte_column\":{},\"match_length\":{},\"excerpt_offset\":{},\"excerpt_hex\":{},",
        "\"match_truncated_in_excerpt\":{}}}"),
        quote(&hex(&row.path)), quote(&row.blob.to_string()), row.byte_offset, row.line,
        row.byte_column, row.match_length, row.excerpt_offset, quote(&hex(&row.excerpt)), end > excerpt_end))
}
fn render(node: &OneNode, command: &Command, head: RepositoryAuthorityHeadId,
    report: &RegexSearchReport, maximum: usize, live: &mut impl FnMut() -> bool,
) -> Result<String, ApiError> {
    check(live)?;
    let source = &report.source;
    let limits = command.limits;
    let query = &command.query;
    if source.repository != node.repository_id || report.program_states != query.state_count()
        || report.steps > query.maximum_steps() || source.matches.len() > limits.max_matches
        ...
        || (source.completion == SearchCompletion::Complete
            && (source.files_read != source.files_selected || source.bytes_searched != source.bytes_read))
        || (source.completion == SearchCompletion::MatchLimit
            && (source.matches.len() != limits.max_matches || report.lines_searched <= source.matches.len()))
        || source.matches.windows(2).any(|p| (&p[0].path, p[0].line) >= (&p[1].path, p[1].line)
            || (p[0].path == p[1].path && p[0].byte_offset >= p[1].byte_offset))
        || [source.source_commit, source.source_tree].iter().any(|id| id.is_zero() || id.algorithm() != node.object_format)
        || command.selection.expected_head.is_some_and(|id| id != head)
        || command.selection.expected_commit.is_some_and(|id| id != source.source_commit)
    { return Err(ApiError::unavailable()); }
    ...
    append(&mut out, &format!(concat!("{{\"type\":\"source_search_regex\",\"schema_version\":1,",
        ...
        ref_fields("ref", &command.selection.reference), quote(&head.to_string()), quote(&token),
        quote(&source.source_rcr.to_string()), quote(&source.source_commit.to_string()), quote(&source.source_tree.to_string()),
        quote(&hex(query.pattern())), quote(match query.case() { SearchCase::Exact => "exact", SearchCase::AsciiInsensitive => "ascii-insensitive" }),
        query.maximum_steps(), report.steps, report.program_states, report.lines_searched,
        limits.max_matches, source.matches.len(),
        quote(match source.completion { SearchCompletion::Complete => "complete", SearchCompletion::MatchLimit => "match_limit" }),
        source.completion == SearchCompletion::Complete, source.files_selected, source.files_read,
        source.bytes_read, source.bytes_searched, source.non_regular_entries), maximum)?;
    for (index, path) in query.prefixes().iter().enumerate() {
        check(live)?;
        append(&mut out, &format!("{}{}", if index == 0 { "" } else { "," }, quote(&hex(path.as_bytes()))), maximum)?;
    }
    append(&mut out, "],\"matches\":[", maximum)?;
    for (index, row) in source.matches.iter().enumerate() {
        check(live)?;
        if index != 0 { append(&mut out, ",", maximum)?; }
        append(&mut out, &match_json(row, query, node.object_format, limits.max_file_bytes)?, maximum)?;
    }
...
mod tests {
    use super::*;
    fn form() -> String { "object_format=sha1&ref=refs/heads/main&pattern_hex=5e612b24".to_owned() }
    #[test]
    fn regex_fields_do_not_change_literal_grammar_or_grant_authority() {
        ...
        for suffix in ["&pattern_hex=61", "&needle_hex=61", "&principal=admin", "&case=unicode",
            "&max_steps=0", "&max_steps=67108865", "&max_matches=4097",
            "&expected_commit=0000000000000000000000000000000000000000", "&path_prefix_hex=2e2e2f736563726574"]
        { assert!(command((form() + suffix).as_bytes(), GitHashAlgorithm::Sha1).is_err()); }
        for pattern in ["28", "283f3d6129", "615c31", "615c7830", "615c707b4c7d", "61ff5A"] {
            let input = form().replace("5e612b24", pattern);
            assert!(command(input.as_bytes(), GitHashAlgorithm::Sha1).is_err());
        }
        assert!(command((form() + "&max_steps=1&path_prefix_hex=646972").as_bytes(), GitHashAlgorithm::Sha1).is_ok());
        assert!(command(form().as_bytes(), GitHashAlgorithm::Sha256).is_err());
    }
    #[test]
    fn strict_route_and_envelope_precede_regex_execution() {
        for (method, path, extra) in [("GET", "/r.git/api/v1/source/search-regex", ""),
            ("POST", "/r.git/api/v1/source/search-regex?q=a", ""),
            ("POST", "/../r.git/api/v1/source/search-regex", ""),
            ("POST", "/r.git/api/v1/source/search-regex", "Git-Protocol: version=2\r\n")]
        {
        ...
        let head = fgit_wire::smart_http::head::parse(bytes, HttpLimits::default()).unwrap().unwrap();
        assert_eq!(Request::parse(&head).unwrap().unwrap().repository_route, "/r.git");
    }
    #[test]
    fn long_and_zero_length_matches_preserve_spans_without_unbounded_excerpts() {
        ...
        let mut row = SourceMatch { path: b"file".to_vec(),
            blob: GitOid::from_hex(GitHashAlgorithm::Sha1, &"1".repeat(40)).unwrap(),
            byte_offset: 0, line: 1, byte_column: 1, match_length: 1000,
            excerpt: vec![b'a';416], excerpt_offset: 0 };
        ...
        row.match_length = 0; row.excerpt.clear();
        assert!(match_json(&row, &query, GitHashAlgorithm::Sha1, 2000).unwrap()
            .contains("\"match_truncated_in_excerpt\":false"));
        row.byte_column = 0;
        assert!(match_json(&row, &query, GitHashAlgorithm::Sha1, 2000).is_err());
        row.byte_column = 1; row.match_length = usize::MAX; row.byte_offset = 1;
        assert!(match_json(&row, &query, GitHashAlgorithm::Sha1, 2000).is_err());
    }
    #[test]
    fn output_budget_and_cancellation_do_not_emit_partial_success() {
        let mut out = String::from("abc");
        assert!(append(&mut out, "de", 4).is_err()); assert_eq!(out, "abc");
        append(&mut out, "d", 4).unwrap(); assert_eq!(out, "abcd");
