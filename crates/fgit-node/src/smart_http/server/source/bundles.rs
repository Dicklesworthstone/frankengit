...
use super::super::{
    Status,
    issues::{ApiError, MAX_FORM_BYTES, parse_form, parse_snapshot, read_form},
};
...
impl<'a> Request<'a> {
    pub(super) fn parse(head: &Envelope<'a>) -> Result<Option<Self>, ApiError> {
        let Some((repository_route, action)) = head.target.split_once("/api/v1/source/bundle/")
        else {
            return Ok(None);
        };
        ...
        let (boundary, maximum) = match operation {
            ...
            Operation::Import | Operation::Fetch => {
                ...
                let boundary =
                    source_upload_boundary(head.content_type.ok_or_else(ApiError::media)?)?;
                (Some(boundary), SourceUploadKind::Bundle.maximum())
            }
        };
        if head.body == BodyFraming::ContentLength(0) {
            return Err(ApiError::bad("empty_bundle_request"));
        }
    ...
    pub(super) const fn is_mutation(&self) -> bool {
        match self.operation {
            ...
            Operation::Import | Operation::Fetch => true,
        }
    }
}
...
pub(super) fn execute(
    ...
    match request.operation {
        Operation::Import | Operation::Fetch => intake::execute(
            node,
            request,
            session,
            framing,
        ...
        Operation::Export => {
            ...
            let (head, bundle) = drive_request_while(
                node,
                &context,
                node.export_full_git_bundle_in(&context, &RefVisibility::new(), expected),
                &mut live,
            )
...
mod tests {
    ...
    fn export_is_an_explicit_read_only_route_not_an_object_or_filesystem_selector() {
        ...
        for (method, path, extra) in [
            ...
            (
                "POST",
                "/r.git/api/v1/source/bundle/export",
    ...
    fn import_and_fetch_are_mutations_with_multipart_envelopes_only() {
        for action in ["import", "fetch"] {
            let bytes = format!(
                "POST /r.git/api/v1/source/bundle/{action} HTTP/1.1\r\nHost: local\r\nContent-Type: multipart/form-data; boundary=x\r\nContent-Length: 1\r\n\r\n"
            );
    ...
    fn export_failure_never_reports_a_repository_mutation() {
        for error in [
            NodeWorkspaceRefusal::FullBundle(Box::new(FullBundleError::Invalid(
    ...
    fn binary_response_preserves_bytes_and_writer_failure_stops_delivery() {
        let reply = Export {
            ...
            body: vec![0, 255, 13, 10],
        };
        for version in [HttpVersion::Http10, HttpVersion::Http11] {
            let mut out = Vec::new();
            reply.send(&mut out, version).unwrap();
