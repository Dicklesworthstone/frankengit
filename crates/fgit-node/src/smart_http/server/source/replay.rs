...
use super::super::{
    Status,
    issues::{ApiError, MAX_FORM_BYTES, read_form},
...
impl<'a> Request<'a> {
    pub(super) fn parse(head: &Envelope<'a>) -> Result<Option<Self>, ApiError> {
            ...
            || media.eq_ignore_ascii_case("application/x-www-form-urlencoded; charset=utf-8")
        {
            ...
            Some(resolution_upload_boundary(media)?)
        } else {
            return Err(ApiError::media());
        };
        let maximum = if boundary.is_some() {
            MAX_RESOLUTION_UPLOAD_BYTES
        } else {
            MAX_FORM_BYTES
        };
...
pub(super) fn execute(
    ...
    let bytes = if request.boundary.is_some() {
        read_resolution_upload(reader, framing, limits)?
    } else {
    ...
    let (command, choices) = if request.resolving {
        let (command, choices) = resolution::parse(
            &bytes,
            request.boundary,
            node.object_format,
            request.direction,
        ...
        (
            Command::parse(&bytes, node.object_format, request.direction)?,
            None,
        )
    };
    ...
    let prepared = if let Some(choices) = choices.as_deref() {
        drive_request_while(
            node,
            &context,
            node.prepare_resolved_replay_bundle_in(
                &context,
                &command.target,
                &command.source,
                command.inputs,
        ...
        .map(|resolved| (resolved.artifact, resolved.resolutions))
    } else {
        drive_request_while(
            node,
            &context,
            node.prepare_replay_bundle_in(
                &context,
                &command.target,
                &command.source,
                command.inputs,
    ...
    let (artifact, receipts) = prepared.map_err(|error| {
        if error.is_snapshot_moved() {
            ...
            ApiError::new(Status::Conflict, "replay_tip_moved")
        } else if error.is_unavailable() {
            ApiError::not_found()
        } else if error.is_invalid_input() {
            ApiError::bad("invalid_replay_inputs")
    ...
    let resolutions = choices
        .as_deref()
        .map(|choices| (choices, receipts.as_slice()));
    output::build(
        node,
        &command,
...
fn replay_error(error: &ReplayError) -> ApiError {
    match error {
        ReplayError::MainlineRequired { .. } => {
            ApiError::new(Status::Conflict, "mainline_required")
        }
...
mod tests {
    ...
    fn exact_routes_select_direction_without_granting_mutation() {
        for (action, direction) in [
            ...
            let envelope = head::parse(bytes.as_bytes(), HttpLimits::default())
                ...
                .unwrap();
            assert_eq!(
                Request::parse(&envelope).unwrap().unwrap().direction,
                direction
            );
        }
        for (method, action, extra) in [
            ...
            let bytes = format!(
                "{method} /repo.git/api/v1/source/{action} HTTP/1.1\r\nHost: local\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: 1\r\n{extra}\r\n"
            );
            let envelope = head::parse(bytes.as_bytes(), HttpLimits::default())
                .unwrap()
                .unwrap();
    ...
    fn history_authorization_refusal_never_discloses_ids_or_invents_no_change() {
        let failures = [
            ReplayError::CommitOutsideSourceHistory,
            ReplayError::MainlineRequired { parents: 2 },
            ReplayError::InvalidMainline {
                requested: 3,
                parents: 2,
            },
        ...
        for failure in failures {
            let error = replay_error(&failure);
    ...
    fn file_uploads_are_only_accepted_for_explicit_resolution_reads() {
        for action in ["cherry-pick", "revert"] {
            for media in [
                ...
                "multipart/form-data; boundary=replay",
            ] {
                let bytes = format!(
                    "POST /repo.git/api/v1/source/{action}/resolve HTTP/1.1\r\nHost: local\r\nContent-Type: {media}\r\nContent-Length: 1\r\n\r\n"
                );
                let envelope = head::parse(bytes.as_bytes(), HttpLimits::default())
                    .unwrap()
                    .unwrap();
                ...
                assert!(
                    !super::super::Request::parse(&envelope)
                        .unwrap()
                        .is_mutation()
                );
                if media.starts_with("multipart/") {
                    let automatic = bytes.replace("/resolve HTTP", "/prepare HTTP");
                    let envelope = head::parse(automatic.as_bytes(), HttpLimits::default())
                        .unwrap()
                        .unwrap();
                    assert!(Request::parse(&envelope).is_err());
                }
