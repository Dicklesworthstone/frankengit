...
use fgit_pack::full_bundle::{FullBundleError, FullBundleInput, FullBundleLimits, fetch::BundleRefMapping};
...
use super::super::super::{Status, issues::{ApiError, Reply as JsonReply, admission_error, parse_form, quote},
...
fn ref_hex(text: &str) -> Result<RefName, ApiError> {
    ...
    let bytes: Vec<_> = text.as_bytes().chunks_exact(2).map(|pair| (digit(pair[0]) << 4) | digit(pair[1])).collect();
    let name = RefName::try_new(&bytes).map_err(|_| ApiError::bad("invalid_bundle_ref"))?;
    if !name.as_bytes().starts_with(b"refs/") { return Err(ApiError::bad("full_bundle_ref_required")); }
    Ok(name)
}
fn mapping(text: &str, format: GitHashAlgorithm) -> Result<BundleRefMapping, ApiError> {
    let mut fields = text.split(':');
    let source = ref_hex(fields.next().ok_or_else(|| ApiError::bad("invalid_bundle_mapping"))?)?;
    let destination = ref_hex(fields.next().ok_or_else(|| ApiError::bad("invalid_bundle_mapping"))?)?;
    let old = fields.next().ok_or_else(|| ApiError::bad("explicit_bundle_lease_required"))?;
    if fields.next().is_some() { return Err(ApiError::bad("invalid_bundle_mapping")); }
    let expected_old = if old == "absent" { None } else {
        if old.len() != format.digest_len() * 2
            || !old.bytes().all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte))
        { return Err(ApiError::bad("invalid_bundle_old_tip")); }
        let id = GitOid::from_hex(format, old).map_err(|_| ApiError::bad("invalid_bundle_old_tip"))?;
        if id.is_zero() { return Err(ApiError::bad("use_explicit_absent_lease")); }
        Some(id)
    };
    Ok(BundleRefMapping { source, destination, expected_old })
}

fn command(bytes: &[u8], format: GitHashAlgorithm, operation: Operation, payload: &[u8],
    ...
    for (name, value) in parse_form(bytes, MAX_MAPPINGS + 2)? {
        ...
        match name.as_str() {
            ...
            "mapping" if operation == Operation::Fetch => {
                if mappings.len() == MAX_MAPPINGS { return Err(ApiError::too_large()); }
                let item = mapping(&value, format)?;
                if !destinations.insert(item.destination.clone()) { return Err(ApiError::bad("duplicate_bundle_destination")); }
                mappings.push(item);
            }
            _ => return Err(ApiError::bad("unknown_or_duplicate_bundle_field")),
        }
    }
    if object_format.as_deref() != Some(format.as_str()) { return Err(ApiError::bad("object_format_mismatch")); }
    if operation == Operation::Fetch && mappings.is_empty() { return Err(ApiError::bad("bundle_mapping_required")); }
    ...
    checkpoint(live)?;
    if actual != digest { return Err(ApiError::bad("bundle_artifact_mismatch")); }
...
pub(super) fn execute(node: &OneNode, request: &Request<'_>, session: &LoopbackReceiveSession,
    ...
    let mappings = command(form, node.object_format, request.operation, payload, &mut live)?;
    let limits = FullBundleLimits::default();
    let header = FullBundleInput::parse(payload, limits, &mut live).map_err(envelope_error)?;
    if header.format() != node.object_format { return Err(ApiError::bad("object_format_mismatch")); }
    let count = match request.operation {
        Operation::Import => header.references().len(),
        Operation::Fetch => header.select_updates(&mappings, limits, &mut live).map_err(envelope_error)?.len(),
        Operation::Export => return Err(ApiError::bad("invalid_bundle_operation")),
    };
    drop(header);
    let admission = AdmissionLimits::default();
    if count > admission.max_commands { return Err(ApiError::too_large()); }
    let result = match request.operation {
        Operation::Import => drive_request_while(node, &context,
            node.import_full_git_bundle_durable_in(&context, session, payload, admission), &mut live),
        Operation::Fetch => drive_request_while(node, &context,
            node.fetch_full_git_bundle_durable_in(&context, session, payload, &mappings, admission), &mut live),
        Operation::Export => return Err(ApiError::bad("invalid_bundle_operation")),
    }.map_err(publication_error)?;
    // A returned canonical terminal outcome wins over later cancellation.
    receipt(node, principal, request.operation, count, result, maximum_response).map(Reply::json)
}

fn envelope_error(error: FullBundleError) -> ApiError {
    match error {
        FullBundleError::Limit(_) => ApiError::too_large(),
        FullBundleError::Invalid(_) | FullBundleError::Unsupported(_) | FullBundleError::FormatMismatch =>
            ApiError::bad("invalid_or_unsupported_full_bundle"),
        FullBundleError::Pack(fgit_pack::PackError::DeadlineExceeded) => ApiError::from_status(Status::Timeout, false),
...
fn publication_error(error: NodeWorkspaceRefusal) -> ApiError {
    match error {
        ...
        NodeWorkspaceRefusal::FullBundle(error) => match *error {
            error @ (FullBundleError::Invalid(_) | FullBundleError::Unsupported(_)
                | FullBundleError::FormatMismatch | FullBundleError::Limit(_)) => envelope_error(error),
            _ => ApiError::unknown(),
        },
...
fn receipt(node: &OneNode, principal: PrincipalId, operation: Operation, count: usize,
    result: AdmissionResult, maximum: u64,
) -> Result<JsonReply, ApiError> {
    let first = result.commands.first().ok_or_else(ApiError::unknown)?;
    if !result.session.atomic || result.session.tx_ids.as_slice() != [first.tx_id]
        || result.commands.len() != count || result.commands.iter().any(|command| command != first)
    ...
    let operation = match operation {
        Operation::Import => "import", Operation::Fetch => "fetch",
        Operation::Export => return Err(ApiError::unknown()),
    };
    let body = format!(concat!("{{\"type\":\"source_bundle_publication\",\"schema_version\":1,",
        ...
        quote(operation), quote(&node.tenant_id.to_string()), quote(&node.repository_id.to_string()),
        quote(&node.repository_incarnation_id().to_string()), quote(node.object_format.as_str()),
        quote(&principal.to_string()), quote(&first.tx_id.to_string()), terminal.decision_sequence.get(),
        count, quote(outcome), decision);
    if body.len() as u64 > maximum.min(1024 * 1024) {
        eprintln!("Bundle receipt exceeded response limit after canonical transaction {}; recover the original key", first.tx_id);
        return Err(ApiError::unknown());
    }
    Ok(JsonReply { status, body, terminal: Some((first.tx_id, terminal)) })
}
...
mod tests {
    ...
    fn form(payload: &[u8], format: GitHashAlgorithm) -> String {
        format!("object_format={}&artifact_sha256={}", format.as_str(), hex(&sha256_digest(payload)))
    }
    ...
    fn imports_require_exact_artifact_bytes_and_never_accept_lease_or_actor_overrides() {
        ...
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            ...
            assert!(command(valid.as_bytes(), format, Operation::Import, payload, &mut || true).unwrap().is_empty());
            assert!(command(valid.as_bytes(), format, Operation::Import, b"changed", &mut || true).is_err());
            assert!(command(valid.as_bytes(), format, Operation::Import, payload, &mut || false).is_err());
            for extra in ["&force=true", "&expected_head=anything", "&principal=admin", "&artifact_sha256=00", "&mapping=x"] {
                assert!(command((valid.clone() + extra).as_bytes(), format, Operation::Import, payload, &mut || true).is_err());
            }
        }
        assert!(command(b"object_format=sha1", GitHashAlgorithm::Sha1, Operation::Import, payload, &mut || true).is_err());
    }
    #[test]
    fn fetch_maps_explicit_byte_refs_and_exact_native_leases_in_both_formats() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            ...
            let mappings = command(value.as_bytes(), format, Operation::Fetch, b"bundle", &mut || true).unwrap();
            assert_eq!(mappings.len(), 1);
            assert_eq!(mappings[0].expected_old.unwrap().to_string(), id);
            assert_eq!(mappings[0].source.as_bytes(), b"refs/heads/main");
            assert_eq!(mappings[0].destination.as_bytes(), b"refs/remotes/origin/main");
            assert!(mapping(&item("refs/heads/main", "refs/heads/new", "absent"), format).unwrap().expected_old.is_none());
            ...
            assert!(command(form(b"bundle", format).as_bytes(), format, Operation::Fetch, b"bundle", &mut || true).is_err());
            let duplicate = value + "&mapping=" + &map;
            assert!(command(duplicate.as_bytes(), format, Operation::Fetch, b"bundle", &mut || true).is_err());
        }
    }
    #[test]
    fn fetch_mapping_count_paths_and_namespaces_are_bounded() {
        let mut form = form(b"bundle", GitHashAlgorithm::Sha1);
        for i in 0..MAX_MAPPINGS { form += &format!("&mapping={}", item("refs/heads/main", &format!("refs/heads/d{i}"), "absent")); }
        assert_eq!(command(form.as_bytes(), GitHashAlgorithm::Sha1, Operation::Fetch, b"bundle", &mut || true).unwrap().len(), MAX_MAPPINGS);
        form += &format!("&mapping={}", item("refs/heads/main", "refs/heads/extra", "absent"));
        assert!(command(form.as_bytes(), GitHashAlgorithm::Sha1, Operation::Fetch, b"bundle", &mut || true).is_err());
