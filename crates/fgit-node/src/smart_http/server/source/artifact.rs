...
pub(super) fn append(out: &mut String, text: &str) -> Result<(), ApiError> {
    if out.len().checked_add(text.len()).is_none_or(|n| n > MAX_METADATA) {
...
pub(super) struct Bundle { bytes: Vec<u8>, digest: [u8; 32] }
impl Bundle {
    pub(super) fn new(bytes: Vec<u8>, live: &mut impl FnMut() -> bool) -> Result<Self, ApiError> {
        checkpoint(live)?;
        if bytes.is_empty() { return Err(ApiError::unavailable()); }
        if bytes.len() > MAX_BUNDLE { return Err(ApiError::too_large()); }
        let digest = sha256_digest(&bytes);
        checkpoint(live)?;
        Ok(Self { bytes, digest })
    }
    pub(super) fn len(&self) -> usize { self.bytes.len() }
    pub(super) fn digest_hex(&self) -> String { hex(&self.digest) }
}
...
impl PreparedReply {
    ...
    pub(super) fn build(metadata: String, bundle: Option<Bundle>, status: Status,
        ...
        let (content_type, prefix, suffix) = if let Some(bundle) = &bundle {
            ...
            let prefix = format!(concat!(
                ...
                boundary, metadata);
            (format!("multipart/mixed; boundary={boundary}"), prefix, format!("\r\n--{boundary}--\r\n"))
        } else {
            ("application/json; charset=utf-8".into(), metadata, String::new())
        };
        let length = prefix.len().checked_add(bundle.as_ref().map_or(0, Bundle::len))
            .and_then(|n| n.checked_add(suffix.len())).filter(|n| *n <= maximum)
            .ok_or_else(ApiError::too_large)?;
        checkpoint(live)?;
        Ok(Self { status, content_type, prefix, bundle, suffix, length })
    }
    ...
    pub(super) fn send(&self, writer: &mut impl Write, version: HttpVersion) -> io::Result<()> {
        ...
        write!(writer, "{version} {}\r\nContent-Type: {}\r\nContent-Length: {}\r\nCache-Control: no-store\r\nVary: Authorization\r\nX-Content-Type-Options: nosniff\r\nConnection: close\r\n\r\n",
            self.status.line(), self.content_type, self.length)?;
        writer.write_all(self.prefix.as_bytes())?;
        if let Some(bundle) = &self.bundle { writer.write_all(&bundle.bytes)?; }
...
fn contains(bytes: &[u8], pattern: &[u8], live: &mut impl FnMut() -> bool) -> Result<bool, ApiError> {
    ...
    while offset < bytes.len() {
        ...
        if bytes[offset..end].windows(pattern.len()).any(|part| part == pattern) { return Ok(true); }
...
mod tests {
    ...
    fn binary_artifact_is_exact_framed_and_never_attached_to_conflict() {
        ...
        let reply = PreparedReply::build("{\"state\":\"clean\"}".into(), Some(bundle), Status::Success,
            MAX_RESPONSE, &mut || true).unwrap();
        let mut wire = Vec::new(); reply.send(&mut wire, HttpVersion::Http11).unwrap();
        ...
        assert!(PreparedReply::build("{}".into(), Some(bundle), Status::Conflict, MAX_RESPONSE, &mut || true).is_err());
    }
    #[test]
    fn bounds_and_cancellation_refuse_before_a_reply_exists() {
        ...
        let mut calls = 0;
        assert!(PreparedReply::build("{}".into(), None, Status::Success, 2,
            &mut || { calls += 1; calls < 2 }).is_err());
    }
    #[test]
    fn delimiter_collision_across_chunks_is_not_missed_and_write_failures_escape() {
        let mut bytes = vec![b'x'; 64 * 1024 - 2]; bytes.extend_from_slice(b"--candidate");
        ...
        let reply = PreparedReply::build("{}".into(), None, Status::Conflict, 2, &mut || true).unwrap();
