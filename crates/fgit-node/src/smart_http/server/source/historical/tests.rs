...
fn form(format: GitHashAlgorithm) -> String {
    format!(
        "object_format={}&ref=refs%2Fheads%2Fmain&expected_head=alg:1:{}&expected_ref_tip={}&at_commit={}",
        format.as_str(),
        "1".repeat(64),
...
fn historical_selection_is_explicit_and_binds_both_native_formats() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let command = request(Operation::Tree)
            .command(input.as_bytes(), format)
            .unwrap();
        ...
        let command = request(Operation::Blob)
            .command(
                (input + "&path_hex=66696c65ff&offset=7&limit=2").as_bytes(),
                format,
            )
            .unwrap();
...
fn every_selection_field_is_required_even_on_the_first_page() {
    ...
    for omitted in [
        ...
        let input = valid
            .split('&')
            .filter(|part| part.split_once('=').unwrap().0 != omitted)
            .collect::<Vec<_>>()
            .join("&");
        assert!(
    ...
    for duplicate in [
        ...
        "&expected_ref_tip=bb",
        "&expected_head=alg:1:aa",
    ] {
        assert!(
            request(Operation::Tree)
                .command((valid.clone() + duplicate).as_bytes(), format)
                .is_err()
        );
    }
}
...
fn arbitrary_authority_fields_invalid_domains_and_unbounded_paths_are_rejected() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for extra in [
            ...
            "&limit=1001",
            "&path_hex=FF",
            "&path_hex=2e2e2f66696c65",
            "&path_hex=610062",
            "&after_hex=2f",
        ] {
            assert!(
                request(Operation::Tree)
                    .command((valid.clone() + extra).as_bytes(), format)
                    .is_err(),
                "{extra}"
            );
        }
        ...
        for replacement in [
            ...
            let input = valid.replace(
                &format!("at_commit={}", "b".repeat(width)),
                &format!("at_commit={replacement}"),
            );
            assert!(
                request(Operation::Tree)
                    .command(input.as_bytes(), format)
        ...
        assert!(
            request(Operation::Blob)
                .command(valid.as_bytes(), format)
                .is_err()
        );
        for extra in [
            "&path_hex=61&after_hex=61",
            "&path_hex=61&offset=-1",
            "&path_hex=61&limit=1048577",
        ] {
            assert!(
                request(Operation::Blob)
                    .command((valid.clone() + extra).as_bytes(), format)
        ...
        assert!(
            request(Operation::Tree)
                .command(
                    (valid + "&path_hex=" + &"61".repeat(4097)).as_bytes(),
                    format
                )
                .is_err()
        );
...
fn routes_are_post_only_bounded_forms_without_url_query_or_git_protocol() {
    for action in ["historical-tree", "historical-blob"] {
        for (method, route, extra, content_type, valid) in [
            (
                "POST",
                format!("/r.git/api/v1/source/{action}"),
                "",
            ...
            (
                "POST",
                format!("/r.git/api/v1/source/{action}?at_commit=aa"),
                "",
                "application/x-www-form-urlencoded",
                false,
            ),
    ...
    let bytes = format!(
        "POST /r.git/api/v1/source/historical-tree HTTP/1.1\r\nHost: local\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: {}\r\n\r\n",
        MAX_FORM_BYTES + 1
    );
    let envelope = head::parse(bytes.as_bytes(), HttpLimits::default())
        .unwrap()
...
fn historical_wrapper_identifies_both_commits_and_budget_covers_the_whole_reply() {
    let command = request(Operation::Tree)
        .command(
            form(GitHashAlgorithm::Sha256).as_bytes(),
