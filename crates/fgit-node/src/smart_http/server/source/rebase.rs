...
impl<'a> Request<'a> {
    ...
    pub(super) fn is_mutation(&self) -> bool { self.operation == Operation::Apply }
}
...
pub(super) fn execute(node: &OneNode, request: &Request<'_>, session: &LoopbackReceiveSession,
    ...
    if request.is_mutation() {
        ...
        let result = drive_request_while(node, &context,
            node.apply_rebase_bundle_durable_in(&context, authenticated.principal_id(),
                authenticated.client_idempotency_key().as_bytes(), &command.reference,
                command.expected_source, command.onto, command.candidate, bundle), &mut live)
...
fn source_error(error: &MergeSourceError) -> ApiError {
    match error { MergeSourceError::Cancelled => ApiError::from_status(Status::Timeout, false),
        MergeSourceError::BudgetExceeded => ApiError::too_large(), _ => ApiError::unavailable() }
}
fn preparation_error(error: &PreparationError) -> ApiError {
    match error {
        PreparationError::InvalidLimits | PreparationError::InvalidMetadata | PreparationError::ObjectFormat => ApiError::bad("invalid_rebase_inputs"),
...
fn rebase_error(error: &RebaseError) -> ApiError {
    match error {
        RebaseError::UpstreamOutsideLinearHistory => ApiError::new(Status::Conflict, "upstream_outside_linear_history"),
        RebaseError::MergeCommit { .. } => ApiError::new(Status::Conflict, "merge_commits_not_supported"),
        RebaseError::Preparation(error) => preparation_error(error),
        RebaseError::DuplicateResolutionCommit(_) | RebaseError::ResolutionOutsideSuffix(_) => ApiError::bad("invalid_rebase_resolution_subject"),
...
fn resolution_error(error: &ResolutionError) -> ApiError {
    match error {
        ...
        ResolutionError::NoConflicts | ResolutionError::NonConflictPath(_) => ApiError::new(Status::Conflict, "resolution_names_clean_step_or_path"),
...
fn publication_error(error: NodeWorkspaceRefusal) -> ApiError {
    match error {
        ...
        NodeWorkspaceRefusal::InvalidWorkspaceCandidate(_) => ApiError::bad("invalid_rebase_candidate"),
...
mod tests {
    ...
    fn prepare_and_apply_keep_distinct_media_and_mutation_semantics() {
        for (action, media, mutation) in [("prepare", "application/x-www-form-urlencoded", false),
            ...
            assert_eq!(super::super::Request::parse(&parsed).unwrap().is_mutation(), mutation);
            for invalid in [bytes.replace("POST ", "GET "), bytes.replace(" HTTP/1.1", "?force=true HTTP/1.1"),
    ...
    fn preparation_errors_are_not_terminal_transactions_or_partial_successes() {
        let errors = [RebaseError::UpstreamOutsideLinearHistory,
        ...
        for error in errors {
            let error = rebase_error(&error); assert!(!error.outcome_unknown);
            let mut response = Vec::new(); error.send_named(&mut response, fgit_wire::smart_http::HttpVersion::Http11, "source_error").unwrap();
            let text = String::from_utf8(response).unwrap();
            assert!(!text.contains("private backend detail") && !text.contains("\"state\":\"clean\""));
        }
