...
use super::super::super::{
    Status,
    issues::{ApiError, Reply, quote, ref_fields},
};
...
use fgit_forge::preparation::rebase::{
    EmptyCommitPolicy, RebasePreparation, RebaseStep, RebaseStepKind, RebaseStop,
};
use fgit_forge::preparation::{ConflictKind, MergeConflict, MergeEntry};
use fgit_types::{
    DecisionOutcome, GitHashAlgorithm, GitOid, PrincipalId, RepositoryAuthorityHeadId,
};
use std::collections::BTreeSet;

fn valid(format: GitHashAlgorithm, oid: GitOid) -> bool {
...
pub(super) fn build(
    ...
    let mut body = format!(
        concat!(
            "{{\"type\":\"rebase_preparation\",{},\"source_head\":{},\"snapshot_token\":{},",
        ...
        quote(&token),
        ref_fields("source_ref", &command.source),
        ref_fields("onto_ref", &command.onto_ref),
        quote(&request.source_tip.to_string()),
        quote(&request.upstream.to_string()),
        quote(&request.onto.to_string()),
        quote(empty),
        quote(&command.committer.identity),
        command.committer.timestamp
    );
    let (status, bundle) = match outcome {
        RebasePreparation::Clean(plan) => {
            if !valid(node.object_format, plan.commit)
                || !valid(node.object_format, plan.tree)
                || frontier != plan.commit
                || plan.objects.len() > command.limits.max_objects
                || steps.last().is_some_and(|step| {
                    step.original != request.source_tip || step.tree != plan.tree
                })
            ...
            if bundle.len() > command.limits.max_output_bytes {
        ...
        RebasePreparation::Stopped {
            ...
            if bundle.is_some()
                || counts != (0, 0)
                || !valid(node.object_format, *original)
                || *original == request.upstream
                || steps.iter().any(|step| step.original == *original)
            {
                return Err(ApiError::unavailable());
            }
            let state = match reason {
                RebaseStop::Conflicted(_) => "conflicted",
                RebaseStop::BecameEmpty => "became_empty",
            };
            append(
                &mut body,
                &format!(
                    concat!(
                        "\"state\":{},\"series_complete\":false,\"provisional_steps\":true,",
    ...
    append(
        &mut body,
        &format!("\"step_count\":{},\"steps\":[", steps.len()),
    )?;
...
fn validate_steps(command: &Prepare, steps: &[RebaseStep]) -> Result<GitOid, ApiError> {
    if steps.len() > command.limits.max_commits {
        return Err(ApiError::unavailable());
    }
    ...
    let mut tree = None;
    for step in steps {
        if [step.original, step.rewritten, step.tree]
            .iter()
            .any(|id| !valid(format, *id))
            || step.original == command.inputs.upstream
            || !seen.insert(step.original)
        {
            return Err(ApiError::unavailable());
        }
        if step.kind == RebaseStepKind::DroppedEmpty {
            if command.inputs.empty != EmptyCommitPolicy::Drop
                || step.rewritten != parent
                || tree.is_some_and(|previous| previous != step.tree)
            {
                return Err(ApiError::unavailable());
            }
        } else if step.rewritten == parent {
            return Err(ApiError::unavailable());
        }
...
pub(super) fn entry(
    value: Option<&MergeEntry>,
    format: GitHashAlgorithm,
) -> Result<String, ApiError> {
    value.map_or_else(
        || Ok("null".into()),
        |e| {
            if !valid(format, e.oid)
                || !matches!(e.mode, 0o040000 | 0o100644 | 0o100755 | 0o120000 | 0o160000)
            {
                return Err(ApiError::unavailable());
            }
            Ok(format!(
                "{{\"mode\":{},\"oid\":{}}}",
                e.mode,
                quote(&e.oid.to_string())
            ))
        },
    )
}
pub(super) fn append_conflict(
    out: &mut String,
    ...
    if path.is_empty()
        || path.len() > command.limits.max_path_bytes
        || path.contains(&0)
        || path
            .split(|b| *b == b'/')
    ...
    append(
        ...
        &format!(
            ...
            quote(&hex(path)),
            quote(kind),
            entry(conflict.base.as_ref(), format)?,
            entry(conflict.ours.as_ref(), format)?,
            entry(conflict.theirs.as_ref(), format)?
        ),
    )
}
...
pub(super) fn publication(
    ...
    let [row] = result.commands.as_slice() else {
        return Err(ApiError::unknown());
    };
    if !result.session.atomic || result.session.tx_ids.as_slice() != [row.tx_id] {
        return Err(ApiError::unknown());
    }
    ...
    let body = format!(
        concat!(
            "{{\"type\":\"rebase_publication\",{},\"principal_id\":{},{},",
        ...
        ref_fields("ref", &command.reference),
        quote(&command.expected_source.to_string()),
        quote(&command.onto.to_string()),
        quote(&command.candidate.to_string()),
        quote(&row.tx_id.to_string()),
        quote(outcome),
        row.terminal.decision_sequence.get(),
        record,
        code
    );
    if body.len() > maximum {
        eprintln!(
            "Rebase reply limit after canonical transaction {}; recover the original key",
            row.tx_id
        );
        return Err(ApiError::unknown());
    }
    Ok(Reply {
        status,
        body,
        terminal: Some((row.tx_id, row.terminal)),
    })
}
...
mod tests {
    ...
    fn command() -> Prepare {
        Prepare {
            source: RefName::try_new(b"refs/heads/topic").unwrap(),
            onto_ref: RefName::try_new(b"refs/heads/main").unwrap(),
            ...
            committer: RebaseCommitter {
                identity: "Bot <b@example.invalid>".into(),
                timestamp: 1,
            },
    ...
    fn conflict_disclosure_preserves_raw_paths_and_refuses_invalid_side_domains() {
        ...
        let mut conflict = MergeConflict {
            path: b"file\xff".to_vec(),
            kind: ConflictKind::ModifyDelete,
            base: None,
            ours: Some(MergeEntry {
                name: b"file\xff".to_vec(),
                mode: 0o100644,
                oid: id(4),
            }),
            theirs: None,
        ...
        append_conflict(&mut out, &conflict, &command).unwrap();
        assert!(out.contains("66696c65ff") && out.contains("\"theirs\":null"));
        conflict.ours.as_mut().unwrap().oid =
