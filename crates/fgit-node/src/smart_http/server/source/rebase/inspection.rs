...
use super::super::super::{Status, issues::{ApiError, Reply, parse_decimal, parse_form, parse_snapshot},
...
impl Command {
    fn parse(bytes: &[u8], format: GitHashAlgorithm) -> Result<Self, ApiError> {
        ...
        for (name, value) in parse_form(bytes, 24)? {
            if !matches!(name.as_str(), "object_format" | "profile" | "source_ref" | "source_ref_hex"
                | "onto_ref" | "onto_ref_hex" | "expected_source" | "expected_onto" | "candidate_commit"
                | "expected_head" | "context_lines" | "max_tree_entries" | "max_changes"
                | "max_text_files" | "max_blob_bytes" | "max_output_bytes" | "max_hunks" | "max_diff_work")
        ...
        for (name, field) in [("context_lines", &mut options.context_lines),
            ...
            ("max_diff_work", &mut options.limits.max_diff_work)]
        {
            if let Some(text) = fields.remove(name) {
                *field = usize::try_from(parse_decimal(&text)?).map_err(|_| ApiError::bad("invalid_inspection_limit"))?;
            }
        }
        options.validate().map_err(|_| ApiError::bad("invalid_inspection_options"))?;
        if !fields.is_empty() { return Err(ApiError::bad("unknown_rebase_inspection_field")); }
        Ok(Self { source, onto_ref, expected_source, onto, candidate, expected_head, options })
    }
}
fn take(fields: &mut BTreeMap<String, String>, name: &str) -> Result<String, ApiError> {
    fields.remove(name).ok_or_else(|| ApiError::bad("missing_rebase_inspection_field"))
}
...
pub(super) fn execute(node: &OneNode, boundary: &str, session: &LoopbackReceiveSession,
    ...
    let inspected = drive_request_while(node, &context,
        node.inspect_rebase_bundle_in(&context, &command.source, &command.onto_ref,
            command.expected_source, command.onto, command.candidate, bundle, &Default::default(),
            command.expected_head, &command.options), &mut live).map_err(failure)?;
    // No raw upload buffer is retained while expanding commit bodies and hunks.
    drop(bytes);
    let body = output::render(node, &command, inspected,
...
fn failure(error: BundleInspectionRefusal) -> ApiError {
    match error {
        ...
        BundleInspectionRefusal::SnapshotMoved => ApiError::new(Status::Conflict, "source_snapshot_moved"),
...
mod tests {
    ...
    fn form(format: GitHashAlgorithm) -> String {
        format!("object_format={}&profile=linear-v1&source_ref=refs/heads/topic&onto_ref=refs/heads/main&expected_source={}&expected_onto={}&candidate_commit={}",
    ...
    fn exact_pins_byte_refs_and_zero_commit_candidates_are_supported() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            let text = form(format).replace("source_ref=refs/heads/topic", "source_ref_hex=726566732f68656164732fff") + "&context_lines=0&max_changes=1";
            ...
            assert!(Command::parse(form(format).replace(&"c".repeat(format.digest_len()*2), &"b".repeat(format.digest_len()*2)).as_bytes(), format).is_ok());
        }
    }
    #[test]
    fn missing_pins_filters_replay_instructions_and_widened_limits_refuse() {
        let good = form(GitHashAlgorithm::Sha1);
        for extra in ["&path_prefix_hex=61", "&mode=merge-base", "&upstream=original", "&empty=drop",
            "&committer=admin", "&force=true", "&principal=admin", "&expected_source=other",
            "&source_ref_hex=61", "&max_changes=0", "&max_changes=513", "&context_lines=21"]
        { assert!(Command::parse((good.clone()+extra).as_bytes(), GitHashAlgorithm::Sha1).is_err(), "{extra}"); }
        for bad in [good.replace("expected_onto=", "unrecognized="),
    ...
    fn read_errors_never_claim_a_transaction_or_disclose_internal_details() {
        for error in [BundleInspectionRefusal::InvalidCandidate("private internals"),
            BundleInspectionRefusal::RefUnavailable, BundleInspectionRefusal::SnapshotMoved,
