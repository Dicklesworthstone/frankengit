...
impl Scratch {
    fn new() -> Self {
        Self(std::env::temp_dir().join(format!("fg-receive-session-fault-{}-{}",
...
fn node(root: &Scratch, format: GitHashAlgorithm) -> OneNode {
    let (mut node, _) = OneNode::init(NodeConfig::new(root.0.clone(),
        TenantId::from_bytes([0xe1; 16]), RepositoryId::from_bytes([0xe2; 16]))
...
fn native_receive(node: &OneNode) -> BasisBoundValidatedReceive {
    ...
    let validator = node.production_quarantine_validator(&selected,
        PackLimits::default(), ParseLimits::default()).unwrap();
    let mut handoff = ProductionReceiveQuarantineHandoff::new(validator, selected.basis().clone());
    let mut receive = ReceivePack::new(node.smart_http_receive_context(WireLimits::default()).unwrap()).unwrap();
    receive.push_bytes(&body).unwrap();
    receive.finish_with_handoff(&mut handoff, &mut || true).unwrap();
...
impl QuarantineValidator for DeleteOnly {
    fn validate(&self, request: &ReceiveRequest, pack: Option<&QuarantinedPack>,
        receipt: &QuarantineReceipt, _: &mut impl fgit_pack::Deadline)
        -> Result<ValidatedClosure, RefusalCode>
    {
        assert!(request.deletes_only() && receipt.delete_only && pack.is_none());
        Ok(ValidatedClosure {
            object_closure_root: permitted_object_closure_root(&PermittedObjectClosure::default()).unwrap(),
...
async fn publish_unrelated_refusal(
    ...
    let receipt = QuarantineReceipt { object_format: format, object_count: 0,
        pack_bytes: 0, delete_only: true };
    let validated = validate_receive_at_basis(&request, None, &receipt, basis, &DeleteOnly, &mut || true).unwrap();
    let projection = DurableAsyncAdmissionProjection::new(materializer, foreign.clone());
    let result = receive_session::admit(authority, cx, &foreign, &validated,
        AdmissionLimits::default(), &projection).await.unwrap();
    assert!(matches!(result.commands[0].terminal.outcome,
        DecisionOutcome::Refused { code: RefusalCode::ExpectedOldRefMismatch, .. }));
}

#[derive(Clone, Copy)]
enum Mode { InterruptSecondSnapshot, RaceSecondCommit }
struct Probe<'a> {
...
impl<'a> Probe<'a> {
    fn new(node: &'a OneNode, context: &AdmissionContext, mode: Mode) -> Self {
        Self { inner: node.durable_admission_projection(context).unwrap(),
            materializer: &node.admission_materializer, context: context.clone(),
            snapshots: AtomicUsize::new(0), commits: AtomicUsize::new(0), mode }
    }
}
impl AsyncAdmissionProjection<FsqliteAuthorityStore> for Probe<'_> {
    ...
    {
        async move {
            let prepared = self.inner.materialize_commit_async(authority, cx, basis, request, fold, closure).await?;
            ...
            if matches!(self.mode, Mode::RaceSecondCommit) && index == 1 {
                ...
                publish_unrelated_refusal(authority, cx, self.materializer, &self.context, basis).await;
            }
            Ok(prepared)
        }
    }
...
fn assert_only_first(node: &OneNode) {
    ...
    assert!(selected.snapshot().refs.contains_key(&RefName::try_new(b"refs/tags/first").unwrap()));
    assert!(!selected.snapshot().refs.contains_key(&RefName::try_new(b"refs/tags/second").unwrap()));
}
...
fn interrupted_second_command_preserves_first_outcome_and_stable_retry_mapping() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let error = node.runtime.block_on(receive_session::admit(&node.authority, request.authority(),
        ...
        assert!(matches!(error.admission_error(), AdmissionError::AsyncProjectionUnavailable(
        ...
        let retried = node.runtime.block_on(receive_session::admit(&node.authority,
            ...
            AdmissionLimits::default(), &projection)).unwrap();
        assert_eq!(&retried.session, error.session().unwrap());
        assert_eq!(retried.commands[0], error.completed_commands()[0]);
        assert!(matches!(retried.commands[1].terminal.outcome, DecisionOutcome::Committed { .. }));
