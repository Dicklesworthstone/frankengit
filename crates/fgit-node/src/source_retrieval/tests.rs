...
fn source() -> symbols::Source {
    ...
    let id = |domain, family| {
        internal_object_id(domain, schema(family), CodecVersion::new(1, 0), b"initial")
    };
    symbols::Source {
...
fn lexical_source(source: &symbols::Source) -> LexicalSource {
    LexicalSource {
        namespace: lexical::LexicalNamespace {
            tenant: source.tenant,
            repository: source.repository,
            incarnation: source.incarnation,
            object_format: source.format,
        },
        reference: source.reference.clone(),
        source_head: source.head,
        source_rcr: source.rcr,
        forge_position_root: source.forge,
...
fn generation(bytes: &[u8], number: u64) -> GenerationActivation {
    GenerationActivation {
        generation_id: GraphGenerationId::from_internal_object_id(internal_object_id(
...
fn query() -> InitialQuery {
    InitialQuery::new(&[b"Thing".to_vec()], &[b"src".to_vec()]).unwrap()
}
fn report() -> IndexedLexicalReport {
    let generation = generation(b"first", 1);
    IndexedLexicalReport {
        source: lexical_source(&source()),
        generation: generation.clone(),
        selected_generation_head: generation,
        query: query().content,
        results: lexical::LexicalReport {
            hits: vec![],
            complete: true,
            next_after: None,
            work_units: 0,
        ...
        generation_bytes_read: 0,
    }
}

#[test]
fn query_keeps_case_sensitive_symbols_and_one_component_bounded_scope() {
    let query = InitialQuery::new(
        &[b"THING".to_vec(), b"thing".to_vec()],
...
fn fixed_allowances_cover_failed_channels_without_renewing_or_exceeding_the_request() {
    ...
    for q in [
        ...
        for limits in [
            ...
            InitialLimits {
                max_results_per_channel: 0,
                ..Default::default()
            },
            InitialLimits {
                max_results_per_channel: 1025,
                ..Default::default()
            },
            InitialLimits {
...
fn same_source_cannot_hide_a_different_lexical_generation_or_checkpoint_position() {
    ...
    assert!(matches!(
        ...
        Err(RetrievalError::MixedGeneration)
    ));
    path = content.clone();
    path.generation.authority_generation = HeadGeneration::try_new(2).unwrap();
    assert!(matches!(
        check_lexical_join(&content, &path),
        Err(RetrievalError::MixedGeneration)
    ));
    path = content.clone();
    path.source.tree = git_object_id(GitHashAlgorithm::Sha1, GitObjectKind::Tree, b"other");
    assert!(matches!(
        check_lexical_join(&content, &path),
...
fn symbol_join_checks_all_source_and_namespace_coordinates_not_just_commit() {
    let original = source();
    let lexical = lexical_source(&original);
    ...
    for field in 0..9 {
        ...
        match field {
            ...
            6 => {
                changed.rcr = RepositoryCommitId::from_internal_object_id(internal_object_id(
                    IdentityDomain::RepositoryCommitRecord,
            ...
            _ => {
                changed.tree = git_object_id(GitHashAlgorithm::Sha1, GitObjectKind::Tree, b"other")
            }
        }
        assert!(matches!(
            check_symbol_join(&lexical, &changed),
            Err(RetrievalError::MixedSource)
        ));
    }
    let mut changed = original;
    changed.commit = git_object_id(GitHashAlgorithm::Sha1, GitObjectKind::Commit, b"other");
    assert!(matches!(
        check_symbol_join(&lexical, &changed),
        Err(RetrievalError::MixedSource)
    ));
...
fn optional_symbols_never_swallow_integrity_cancellation_or_retained_checkpoints() {
    assert_eq!(
        symbol_unavailable(SymbolError::Uninitialized, SymbolPolicy::Optional, false).unwrap(),
        SymbolUnavailable::Uninitialized
    );
    assert_eq!(
        symbol_unavailable(SymbolError::Stale, SymbolPolicy::Optional, false).unwrap(),
        SymbolUnavailable::Stale
    );
    for policy in [SymbolPolicy::Optional, SymbolPolicy::Required] {
        ...
        for error in [
            SymbolError::Index(symbols::Error::CommitmentMismatch),
...
fn optional_unavailability_is_incomplete_and_retained_bytes_are_shared() {
    ...
    let mut report = InitialReport {
        ...
        content,
        path,
        symbols: SymbolChannel::NotRequested,
        result_bytes: 0,
    };
    ...
    assert!(!report.complete());
    let mut total = 0;
    add_result(&mut total, 7, 10).unwrap();
    add_result(&mut total, 3, 10).unwrap();
    assert!(add_result(&mut total, 1, 10).is_err());
    assert_eq!(total, 10);
    total = usize::MAX;
    assert!(add_result(&mut total, 1, usize::MAX).is_err());
}
