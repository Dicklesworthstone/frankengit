// Shared host adapter included by the library and the legacy binary test target.
// Keeping the latter at its original repository:: test paths prevents existing
// focused commands from silently selecting zero tests after library promotion.
...
/// Run one explicitly authorized local source-recovery operation.
///
/// This is a top-level synchronous host adapter. It owns and drains its node
/// runtimes; do not call it from an existing request task. Arguments retain the
/// export/verify/restore grammar and independently trusted checksum requirement.
/// No helper executable or external Git engine is consulted.
pub fn run(args: &[String], output: &mut impl Write) -> Result<(), String> {
    repository::run(args, output)
}

/// Render the same escaped failure envelope for both CLI entrypoints.
#[must_use]
...
mod adapter_tests {
    ...
    #[test]
    fn shared_dispatch_exposes_all_three_operations_without_opening_a_repository() {
        let mut output = Vec::new();
        run(&["--help".into()], &mut output).unwrap();
        let text = String::from_utf8(output).unwrap();
        for operation in ["export", "verify", "restore"] {
            assert!(text.contains(&format!("usage: fg backup {operation}")));
            let mut output = Vec::new();
            run(&[operation.into(), "--help".into()], &mut output).unwrap();
            assert!(String::from_utf8(output).unwrap().contains(operation));
        }
        assert!(run(&[], &mut Vec::new()).is_err());
        assert!(run(&["unknown".into()], &mut Vec::new()).is_err());
    }

    #[test]
    fn output_failures_and_error_text_remain_explicit_at_the_shared_boundary() {
        struct Broken;
        ...
        assert!(run(&["--help".into()], &mut Broken).is_err());
