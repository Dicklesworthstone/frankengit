...
fn options(input: &Path, output: PathBuf, expected: [u8; 32]) -> Options {
    Options {
        ...
        output,
        expected,
        instance: StoreInstanceId::from_raw(991),
...
fn snapshot(root: &Path) -> ExportBundle {
    with_store(
        ...
        |runtime, store, cx| {
            runtime
                .block_on(store.export_portable(cx, Default::default()))
...
fn stop(options: &Options, stop: Stage) {
    ...
    let error = execute_with_checkpoints(options, |stage| {
        if stage == stop {
            reached = true;
            Err(format!("injected stop at {stage:?}"))
        } else {
            Ok(())
        }
    })
...
fn each_restore_boundary_resumes_in_both_domains_without_new_authority_tokens() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        .enumerate()
        {
            ...
            stop(&options, stage);
            assert_eq!(
                options.output.join("authority.fsqlite").exists(),
                matches!(
                    stage,
            ...
            assert_eq!(
                ...
                before,
                "retry cannot advance the ledger"
            );
        }
    }
}
...
fn a_partial_object_set_reuses_valid_bytes_and_installs_the_missing_selected_objects() {
    ...
    let (input, pin, _) = fixture(&scratch, format);
    let mut options = options(&input, scratch.0.join("partial"), pin);
    stop(&options, Stage::Authority);
    let quarantine = options.output.join(".restore-quarantine");
    with_node(node_config(&quarantine, format), |node| {
        node.put_git_object(GitObjectKind::Blob, PAYLOAD.to_vec())
            .map_err(|e| e.to_string())?;
        Ok(())
    })
    .unwrap();
    options.resume = true;
    assert!(execute(&options).unwrap().contains("\"objects\":3,"));
    with_node(node_config(&options.output, format), |node| {
        assert_eq!(
            node.read_git_object(git_object_id(format, GitObjectKind::Blob, PAYLOAD))
                .unwrap()
                .payload(),
            PAYLOAD
        );
...
fn a_published_destination_with_new_work_is_refused_without_rewinding_or_cleanup() {
    ...
    let mut options = options(&input, scratch.0.join("advanced"), pin);
    execute(&options).unwrap();
    with_node(node_config(&options.output, format), |node| {
        let request = node.request_context();
        let session = LoopbackReceiveSession::authenticated(
            PrincipalId::from_bytes([3; 16]),
            IdempotencyKey::new(b"new-work".to_vec()).unwrap(),
        );
        let command = RefCommand {
            name: RefName::try_new(b"refs/heads/new-work").unwrap(),
    ...
    assert!(error.contains("complete imported snapshot"), "{error}");
    assert_eq!(snapshot(&options.output), before);
    assert!(!options.output.join(".restore-quarantine").exists());
}
...
fn payload_file(root: &Path, matches: &mut Vec<PathBuf>) {
    for entry in fs::read_dir(root).unwrap() {
        ...
        if entry.file_type().unwrap().is_dir() {
            payload_file(&entry.path(), matches);
        } else if fs::read(entry.path()).unwrap().ends_with(PAYLOAD) {
...
fn corrupt_preexisting_object_is_not_overwritten_and_never_publishes_authority() {
    ...
    let format = GitHashAlgorithm::Sha1;
    let (input, pin, _) = fixture(&scratch, format);
    let mut options = options(&input, scratch.0.join("corrupt"), pin);
    stop(&options, Stage::Objects);
    let mut matches = Vec::new();
    payload_file(
        &options.output.join(".restore-quarantine/objects"),
        &mut matches,
    );
    ...
    *corrupted.last_mut().unwrap() ^= 1;
    ...
    assert!(!options.output.join("authority.fsqlite").exists());
