    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let receipt = execute_with_checkpoints(&request_options, |stage| {
            ...
            if stage == Stage::GraphCheckedAndNodeClosed {
                with_node(
                    node_config(&output.join(".verify-quarantine"), format),
                    |node| {
                        for id in ids {
                            assert!(
                                node.read_git_object(id).is_err(),
                                "preflight placed a Git payload"
                            );
                        }
                        Ok(())
                    },
        ...
        assert_eq!(
            observed,
            [Stage::AuthorityClosed, Stage::GraphCheckedAndNodeClosed]
        );
...
fn an_existing_scratch_directory_is_never_reused_or_removed() {
    ...
    assert_eq!(
        fs::read(output.join("keep")).unwrap(),
        b"not owned by verification"
    );
}

#[test]
fn a_validly_framed_archive_cannot_omit_an_authority_selected_object() {
    use super::super::super::archive::{Encoder, decode};
    let scratch = Scratch::new();
    ...
    let mut reduced =
        Encoder::new(decoded.identity, &authority, decoded.records.len() - 1).unwrap();
    for record in decoded.records.iter().skip(1) {
        let commitment =
            git_payload_commitment(record.kind, record.payload, CANONICAL_CODEC_VERSION);
        let mut original = [0_u8; 32];
        original.copy_from_slice(commitment.digest().as_bytes());
        reduced
            .object(record.oid, record.kind, record.payload, &original)
            .unwrap();
    }
    ...
    assert!(
        error.contains("inventory does not equal the authority-selected object set"),
        "{error}"
    );
    ...
    assert!(
        execute(&options(
            &input,
            &scratch.0.join("correct-verification"),
            pin
        ))
        .is_ok()
    );
...
fn interrupted_preflight_retains_private_state_and_never_enables_resume() {
    ...
    for stage in [Stage::AuthorityClosed, Stage::GraphCheckedAndNodeClosed] {
        ...
        let error = execute_with_checkpoints(&options, |observed| {
            if observed == stage {
                Err("injected preflight interruption".into())
            } else {
                Ok(())
            }
        })
...
fn verification_parser_is_bounded_and_has_no_restore_or_resume_authority() {
    ...
    for extra in [
        vec!["--resume"],
        vec!["--trusted-local"],
        vec!["--destination-instance", "3"],
        vec!["--expected-sha256", "00"],
        vec!["--timeout-secs", "0"],
    ] {
        let mut bad = base.clone();
