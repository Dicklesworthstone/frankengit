...
use crate::{AdmissionMaterializationRefusal, NodeRefusal, NodeRequestContext, OneNode,
...
use fgit_types::{Digest, GitOid, HeadGeneration, RepositoryAuthorityHeadId, RepositoryId,
...
impl Default for GraphAuditQuery {
    fn default() -> Self {
        Self { expected_head: None, expected_generation: None,
            limits: GraphLimits::default(), timeout: Duration::from_secs(300) }
    }
}
...
pub enum GraphAuditRefusal {
    ...
    Object { oid: GitOid, cause: Box<NodeRefusal> },
    NonGitObject(GitOid),
    ExpectedHead,
    ExpectedGeneration { expected: u64, observed: u64 },
    SnapshotChanged,
    Graph(GraphRefusal),
}
impl fmt::Display for GraphAuditRefusal {
    fn fmt(&self, out: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidTimeout => out.write_str("graph audit timeout must be in (0, 3600 seconds]"),
            Self::Cell(error) => write!(out, "graph audit read refused: {error}"),
            Self::Cancelled { exhaustion } => write!(out, "graph audit request stopped: {exhaustion:?}"),
            Self::Deadline => out.write_str("audit_deadline_exceeded"),
            Self::Materialization(error) => write!(out, "authority materialization failed: {error}"),
            ...
            Self::ExpectedGeneration { expected, observed } => write!(out,
...
impl SelectedGraphAudit {
    pub const fn repository_id(&self) -> RepositoryId { self.repository }
    pub const fn repository_incarnation_id(&self) -> RepositoryIncarnationId { self.incarnation }
    pub const fn head(&self) -> RepositoryAuthorityHeadId { self.head }
    pub const fn generation(&self) -> HeadGeneration { self.generation }
    pub const fn closure_root(&self) -> Digest { self.closure_root }
    pub const fn graph(&self) -> &GraphReport { &self.graph }
}

fn checkpoint(request: &NodeRequestContext, started: Instant, timeout: Duration) -> Result<(), GraphAuditRefusal> {
    if started.elapsed() >= timeout { return Err(GraphAuditRefusal::Deadline); }
    match checkpoint_pack_context(request.authority()) {
        ...
        PackContextCheckpoint::Stopped { budget_exhaustion } =>
            Err(GraphAuditRefusal::Cancelled { exhaustion: budget_exhaustion }),
    }
}

fn revalidate_head(before: &fgit_authority::AuthenticatedHead, current: &fgit_authority::AuthenticatedHead)
...
impl OneNode {
    ...
    pub async fn audit_selected_object_graph_local_in(
        &self, request: &NodeRequestContext, query: GraphAuditQuery,
        ...
        checkpoint(request, started, query.timeout)?;
        let selected = selected.map_err(|error| GraphAuditRefusal::Materialization(Box::new(error)))?;
        let head = selected.basis().id();
        let generation = selected.basis().generation();
        if let Some(expected) = query.expected_generation {
            if expected != generation {
                return Err(GraphAuditRefusal::ExpectedGeneration { expected: expected.get(), observed: generation.get() });
            }
        }
        ...
        for &oid in objects {
            ...
            checkpoint(request, started, query.timeout)?;
            let object = read.map_err(|cause| GraphAuditRefusal::Object { oid, cause: Box::new(cause) })?;
        ...
        Ok(SelectedGraphAudit {
            repository: self.repository_id(), incarnation: self.repository_incarnation_id(),
            head, generation, closure_root: selected.selected_closure().root(), graph,
        })
    }
}
