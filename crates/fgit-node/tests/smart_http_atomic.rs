...
impl Scratch {
    fn new() -> Self {
        Self(std::env::temp_dir().join(format!("fg-http-atomic-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed))))
    }
}
...
fn config(scratch: &Scratch, format: GitHashAlgorithm) -> NodeConfig {
    NodeConfig::new(scratch.0.clone(), TenantId::from_bytes([0xe1; 16]), RepositoryId::from_bytes([0xe2; 16])).with_object_format(format)
}
fn session(key: &[u8]) -> LoopbackReceiveSession {
    LoopbackReceiveSession::authenticated(PrincipalId::from_bytes([0xe3; 16]), IdempotencyKey::new(key.to_vec()).unwrap())
}
...
fn body(first: String, second: String, pack: &[u8]) -> Vec<u8> {
    let mut bytes = encode_packets(&[Packet::Data(first.into_bytes()), Packet::Data(second.into_bytes()), Packet::Flush], &WireLimits::default()).unwrap();
...
fn push(node: &OneNode, key: &[u8], body: &[u8]) -> (AdmissionResult, Vec<u8>) {
    ...
    let outcome = node.smart_http_receive_stream_in(&request, &session(key), &mut Cursor::new(body),
...
fn generation(node: &OneNode) -> u64 {
    node.runtime().block_on(node.materialize_admission()).unwrap().basis().generation().get()
}
fn assert_refs(node: &OneNode, expected: Option<GitOid>) {
    ...
    assert_eq!(materialized.snapshot().refs.len(), if expected.is_some() { 2 } else { 0 });
...
fn create(format: GitHashAlgorithm) -> (GitOid, Vec<u8>) {
    ...
    let request = body(format!("{zero} {oid} {A}\0report-status atomic object-format={}", format.as_str()),
        format!("{zero} {oid} {B}"), &pack);
    (oid, request)
}
...
fn atomic_create_retry_reopen_and_delete_preserve_one_decision_for_all_refs() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let route = std::str::from_utf8(node.git_daemon_repository_path().as_bytes()).unwrap();
        let discovery = format!("GET {route}/info/refs?service=git-receive-pack HTTP/1.1\r\nHost: local\r\n\r\n");
        let request = parse_head(discovery.as_bytes(), HttpLimits::default()).unwrap().unwrap();
        let advertisement = node.smart_http_receive_discovery_in(&request, &session(b"discovery"), WireLimits::default()).unwrap();
        assert!(advertisement.body().windows(b" atomic ".len()).any(|w| w == b" atomic "));

        let (oid, body) = create(format);
        let before = generation(&node);
        let (first, response) = push(&node, b"atomic-create", &body);
        assert!(matches!(first.commands[0].terminal.outcome, DecisionOutcome::Committed { .. }));
        assert_eq!(generation(&node), before + 1, "one head transition publishes both refs");
        ...
        assert_refs(&node, Some(oid));
        let zero = "0".repeat(oid.as_bytes().len() * 2);
        let deletion = self::body(format!("{oid} {zero} {A}\0report-status atomic delete-refs object-format={}", format.as_str()),
            format!("{oid} {zero} {B}"), &[]);
...
fn one_bad_expected_old_refuses_the_whole_atomic_delete_and_the_retry() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let zero = "0".repeat(oid.as_bytes().len() * 2);
        let deletion = body(format!("{oid} {zero} {A}\0report-status atomic delete-refs object-format={}", format.as_str()),
        ...
        for name in [A, B] {
            ...
            assert!(response.windows(record.len()).any(|w| w == record.as_bytes()));
            let success = format!("ok {name}\n");
            assert!(!response.windows(success.len()).any(|w| w == success.as_bytes()));
        }
