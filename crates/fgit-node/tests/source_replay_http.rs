...
fn native_cherry_pick_and_inverse_are_reproducible_read_only_and_target_complete() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            field(&metadata, "selected_parent"),
            data.source_tip.to_string()
        );
        assert!(numeric(&metadata, "borrowed_objects") > 0);
        assert!(
            bundle
                .windows(data.source_tip.to_string().len())
                .any(|part| part == data.source_tip.to_string().as_bytes())
        );
        assert_eq!(
            post_form(&server.client, "cherry-pick/prepare", 'a', &pick, true),
            first
        ); // 2
        let inverse = form(
            &data.target_ref,
        ...
        let (reverted, _) = extract_candidate(&post_form(
            &server.client,
            "revert/prepare",
            'a',
            &inverse,
        ...
        assert_ne!(
            field(&reverted, "candidate_commit"),
        ...
        assert_eq!(field(json(&unchanged), "state"), "no_change");
        assert!(json(&unchanged).contains("\"bundle\":null"));
        let root_mainline = pick.replace(
            &format!("commit={}", data.target_tip),
            &format!("commit={}", data.source_tip),
        ) + "&mainline=1";
        status(
            &post_form(
                &server.client,
        ...
        status(
            &post_form(&server.client, "cherry-pick/prepare", 'a', &outside, false),
            404,
        ); // 6
        status(
            &post_form(
        ...
        status(&absent, 200);
        assert!(json(&absent).contains("key_not_observed"));
...
fn replay_artifact_is_inspected_and_published_only_by_explicit_recoverable_write() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (metadata, bundle) = extract_candidate(&post_form(
            &server.client,
            "cherry-pick/prepare",
            'a',
            &pick,
        ...
        let command = format!(
            "object_format={}&ref={}&expected_commit={}&candidate_commit={candidate}",
            format.as_str(),
            encode(data.source_ref.as_bytes()),
            data.source_tip
        );
        let inspection = multipart(&server.client, "inspect", 'a', &command, &bundle, None); // 2
        status(&inspection, 200);
        assert!(json(&inspection).contains("source_inspection"));
        status(
            &multipart(
                &server.client,
                "apply",
        ...
        assert!(json(&published).contains("\"outcome\":\"committed\""));
        assert_eq!(
            multipart(
                &server.client,
                "apply",
        ...
        status(
            &post_form(&server.client, "cherry-pick/prepare", 'a', &pick, false),
            409,
        ); // 2
        let query = format!(
        ...
        status(&file, 200);
        assert_eq!(field(json(&file), "content_hex"), hex(b"left\n"));
...
fn conflicts_pins_scopes_and_rotation_never_publish_or_consume_unauthorized_bodies() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for action in ["cherry-pick/prepare", "revert/prepare"] {
            // 1, 2
        ...
        let stale = pick.replace(
            &format!("expected_target={}", data.target_tip),
            &format!("expected_target={}", data.source_tip),
        );
        status(
            &post_form(&server.client, "cherry-pick/prepare", 'a', &stale, false),
            409,
        ); // 3
        status(
            &post_form(
                ...
                "cherry-pick/prepare",
                'a',
                &(pick.clone() + "&expected_head=alg:1:" + &"ab".repeat(32)),
                false,
            ),
            409,
        ); // 4
        for (token, key, expected) in [
            ...
            let denied = binary_exchange(
                ...
                &request(
                    &server.client,
                    "POST",
                    "/api/v1/source/cherry-pick/prepare",
                    token,
                    &format!(
        ...
        status(
            &post_form(&server.client, "cherry-pick/prepare", '9', &pick, false),
            409,
        ); // 9
