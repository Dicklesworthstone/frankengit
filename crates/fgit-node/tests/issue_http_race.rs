...
fn competing_editors_and_their_retries_do_not_overwrite_the_winners_issue() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let server = thread::spawn(move || {
            let result = node.serve_git_and_issue_http_with_credentials_file_bounded(
                &listener,
                GitDaemonServerLimits::try_new(5, 2).unwrap(),
                &credentials,
                false,
        ...
        let opened = request(
            ...
            "open",
            "open",
            "expected_version=0&title=Before&body=immutable",
        );
        ...
        for title in ["Left", "Right"] {
            ...
            editors.push(thread::spawn(move || {
                ...
                (
                    title,
                    request(address, &route, &token, "edit", title, &body),
                )
            }));
        }
        barrier.wait();
        ...
        for (title, reply) in &replies {
            assert!(reply.1.contains(if reply.0 == 200 {
                "\"outcome\":\"committed\""
            } else {
                "\"outcome\":\"refused\""
            }));
            let retry = request(
                ...
                "edit",
                title,
                &format!("expected_version=1&title={title}"),
            );
            assert_eq!(
                &retry, reply,
                "each key retains its actual winning or refused terminal result"
            );
        }
        ...
        assert_eq!(
            ...
            before.get() + 3,
            "open, winning edit and losing refusal, never retry decisions"
        );
        let page = node
            .runtime()
