...
fn remote_multi_file_patch_inspects_and_publishes_exactly_once_after_restart() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        status(&original_file, 200);
        assert_eq!(text(&original_file.body, "content_hex"), hex(TEXT));
        let response = prepare(&server.client, base, &patch(), "Remote+exact+edit", false); // 2
        let artifact = extract(&response, format);
        assert_eq!(
            prepare(&server.client, base, &patch(), "Remote+exact+edit", true),
            response
        ); // 3
        ...
        assert_eq!(
            git_object_id(format, GitObjectKind::Commit, &commit),
            artifact.commit
        );
        ...
        assert!(
            node.read_git_object(artifact.commit).is_err(),
            "preparation and inspection must not stage the commit"
        );
        ...
        assert_eq!(
            prepare(&server.client, base, &patch(), "Remote+exact+edit", false),
            response
        ); // 1
...
fn competing_remote_edits_keep_expected_old_and_stable_terminal_retries() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let left = extract(
            &prepare(&server.client, base, &patch(), "Left+editor", false),
            format,
        );
        let right_patch = String::from_utf8(patch())
            .unwrap()
            .replace("Edited needle", "Other needle");
        let right = extract(
            &prepare(
                &server.client,
                base,
                right_patch.as_bytes(),
                "Right+editor",
                true,
            ),
            format,
        );
        assert_ne!(left.commit, right.commit);
        server.finish();
        let node = reopen(&config);
        assert_eq!(generation(&node), before);
        ...
        let (a, b) = std::thread::scope(|scope| {
            ...
            let a = scope.spawn(move || {
                first.wait();
                apply(client, base, left, "editor-left", false)
            });
            let b = scope.spawn(move || {
                second.wait();
                apply(client, base, right, "editor-right", true)
            });
        ...
        committed(winner_reply);
        assert_eq!(text(&loser.body, "outcome"), "refused");
        assert_eq!(apply(&server.client, base, &left, "editor-left", true), a); // 3
        assert_eq!(
            apply(&server.client, base, &right, "editor-right", false),
            b
        ); // 4
        ...
        status(&current, 200);
        assert_eq!(
            text(&current.body, "source_commit"),
            winner.commit.to_string()
        );
        let changed = if winner.commit == left.commit {
            &right
        } else {
            &left
        };
        ...
        status(&reuse, 409);
        assert!(reuse.body.contains("idempotency_key_reuse"));
        assert_eq!(server.finish().accepted_sessions(), 8);
        let node = reopen(&config);
        assert_eq!(
            generation(&node),
            before + 2,
            "commit and refusal only; retries and key conflicts publish nothing"
        );
        let selected = node
            .runtime()
            .block_on(node.materialize_admission())
...
fn withheld(client: &Endpoint, action: &str, token: char, headers: &str, length: usize) -> Reply {
    let reply = exchange(
        ...
        &request(
            ...
            token,
            &format!(
                "Content-Type: multipart/form-data; boundary=source-edit\r\nContent-Length: {length}\r\nExpect: 100-continue\r\n{headers}"
            ),
            &[],
        ),
        false,
...
fn source_reads_and_writes_keep_independent_gates_and_refuse_partial_inputs() {
    ...
    let wrong_context = String::from_utf8(patch())
        .unwrap()
        .replace("-Needle needle", "-Wrong context");
    let refused = prepare(
        &server.client,
        base,
        wrong_context.as_bytes(),
        "Bad+context",
        false,
    ); // 8
    assert_eq!(refused.status, 409);
    assert!(
        String::from_utf8(refused.body)
            .unwrap()
            .contains("patch_context_mismatch")
    );
    let invalid_path = String::from_utf8(patch())
        .unwrap()
        .replace("alpha.txt", "../outside-secret");
    assert_eq!(
        prepare(
            &server.client,
            base,
            invalid_path.as_bytes(),
            "Bad+path",
            false
        )
        .status,
    ...
    let mut truncated = change_bytes(
        &server.client,
        "prepare",
        'a',
        None,
        &metadata(base, "Truncated"),
        &patch(),
        false,
    );
    truncated.pop();
    status(&exchange(&server.client, &truncated, true), 400); // 10
    let mut incomplete_apply = change_bytes(
        &server.client,
        "apply",
        'b',
        Some("incomplete"),
        &candidate_form(base, artifact.commit),
        &artifact.bundle,
        true,
    );
    ...
    let last = corrupt.bundle.last_mut().unwrap();
    *last ^= 1;
    let inspected = inspect(&server.client, base, &corrupt); // 12
    status(&inspected, 503);
    assert!(!inspected.body.contains("candidate_commit_body_hex"));
    status(&inspect(&server.client, base, &artifact), 200); // 13, permitted twin
    assert_eq!(server.finish().accepted_sessions(), 13);
    let node = reopen(&config);
    assert_eq!(generation(&node), before);
    assert!(node.read_git_object(artifact.commit).is_err());
    node.shutdown().unwrap();
}
...
fn lost_source_receipt_recovers_after_restart_with_write_permission_revoked() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let artifact = extract(
            &prepare(&server.client, base, &patch(), "Lost+reply", false),
            format,
        );
        server.finish();
        let node = reopen(&config);
        assert_eq!(generation(&node), before);
        let server = Server::start(node, &path, 1, true, true);
        let bytes = change_bytes(
            &server.client,
            "apply",
            'b',
            Some("lost-source-edit"),
            &candidate_form(base, artifact.commit),
            &artifact.bundle,
            false,
        );
        ...
        replace(
            &path,
            &(header + &row('9', OWNER, "outcomes-read") + &row('d', FOREIGN, "outcomes-read")),
        );
        ...
        status(&foreign, 200);
        assert_eq!(text(&foreign.body, "state"), "key_not_observed");
        status(
            &apply(&server.client, base, &artifact, "lost-source-edit", false),
            401,
        ); // 4, old write token gone
        let missing = recover(&server.client, '9', "unobserved-edit"); // 5
        status(&missing, 200);
        assert_eq!(text(&missing.body, "state"), "key_not_observed");
        server.finish();
        let node = reopen(&config);
        assert_eq!(generation(&node), before + 1);
        let selected = node
