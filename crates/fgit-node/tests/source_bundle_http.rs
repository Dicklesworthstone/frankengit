...
fn binary_exchange(client: &Endpoint, bytes: &[u8]) -> BinaryReply {
    ...
    assert_eq!(
        reply.header("Content-Length").parse::<usize>().unwrap(),
        reply.body.len()
    );
    reply
}
...
fn complete_native_bundle_bytes_and_snapshot_survive_http_and_reopen_in_both_formats() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            first.header("X-Fgit-Artifact-Sha256"),
            hex(&sha256_digest(&first.body))
        );
        ...
        assert_eq!(reopened.body, first.body);
...
fn export_pins_the_whole_authority_snapshot_even_when_only_forge_state_changes() {
    ...
    let stale = export(
        ...
        'a',
        &format!(
            "object_format=sha1&expected_head={}",
            first.header("X-Fgit-Snapshot")
        ),
        "",
        false,
    );
    ...
    assert!(
        String::from_utf8(stale.body)
            .unwrap()
            .contains("source_snapshot_moved")
    );
    let current = export(&server.client, 'a', "object_format=sha1", "", false);
    assert_eq!(current.status, 200);
    assert_eq!(current.body, first.body);
    assert_ne!(
        current.header("X-Fgit-Snapshot"),
        first.header("X-Fgit-Snapshot")
    );
...
fn export_requires_source_enablement_and_read_scope_before_continue_or_disclosure() {
    ...
    for (token, extra, expected) in [
        ...
        assert!(!reply.body.starts_with(b"# v2 git bundle"));
    ...
    let foreign = Endpoint {
        ...
        route: format!("{}/other", server.client.route),
    };
    assert_eq!(
        export(&foreign, 'a', "object_format=sha1", "", false).status,
        404
    );
    assert_eq!(
        export(&server.client, 'a', "object_format=sha256", "", false).status,
        400
    );
    ...
    assert_eq!(
        export(&server.client, 'a', "object_format=sha1", "", false).status,
        403
    );
    assert_eq!(server.finish().accepted_sessions(), 1);
}
