...
impl Scratch {
    fn new() -> Self {
        let root = std::env::temp_dir().join(format!("fg-large-backup-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
...
fn config(path: &Path) -> NodeConfig {
    NodeConfig::new(path.to_path_buf(), TenantId::from_hex(TENANT).unwrap(), RepositoryId::from_hex(REPOSITORY).unwrap())
...
fn loose(root: &Path, kind: GitObjectKind, body: &[u8]) -> GitOid {
    ...
    let mut file = File::create(path).unwrap(); file.write_all(&[0x78, 0x01]).unwrap();
    ...
    while remaining > 0 {
        ...
        for &byte in &block[..count] { a = (a + u32::from(byte)) % 65521; b = (b + a) % 65521; }
        remaining -= count;
    }
    file.write_all(&((b << 16) | a).to_be_bytes()).unwrap(); oid
}
fn checksum(path: &Path) -> String {
    let mut file = File::open(path).unwrap(); let mut block = vec![0; 64 * 1024];
    let mut hash = Sha256Hasher::new();
    loop { let size = file.read(&mut block).unwrap(); if size == 0 { break; } hash.update(&block[..size]); }
...
fn larger_than_64_mib_backup_restores_from_disk_and_respects_smaller_explicit_budgets() {
    let scratch = Scratch::new(); let root = scratch.0.join("node"); let source = scratch.0.join("source.git");
    let mut blobs = Vec::new(); let mut tree = Vec::new();
    for ordinal in 0_u8..3 {
        let body = vec![b'a' + ordinal; PAYLOAD_BYTES];
        let oid = loose(&source, GitObjectKind::Blob, &body);
        tree.extend_from_slice(format!("100644 data{ordinal}\0").as_bytes()); tree.extend_from_slice(oid.as_bytes());
    ...
    fs::write(source.join("config"), b"[core]\nrepositoryformatversion = 1\nbare = true\n[extensions]\nobjectformat = sha256\n").unwrap();
    let (mut node, _) = OneNode::init(config(&root)).unwrap();
    node.bring_into_service(HeadGeneration::FIRST).unwrap();
    let request = node.request_context();
    let admission = node.runtime().block_on(node.import_loose_git_directory_durable_in(
        &request, &source, PrincipalId::from_bytes([3; 16]), b"large-backup-fixture")).unwrap();
    ...
    let error = refused(command(&["export", text(&root), text(&too_small), TENANT, REPOSITORY,
        "--trusted-local", "--object-format", "sha256", "--max-archive-bytes", "67108864", "--timeout-secs", "1800"]));
    assert!(error.contains("archive-byte limit"), "{error}"); assert!(!too_small.exists());
    let backup = scratch.0.join("large.fg");
    let receipt = success(command(&["export", text(&root), text(&backup), TENANT, REPOSITORY,
        "--trusted-local", "--object-format", "sha256", "--timeout-secs", "1800"]));
    assert!(fs::metadata(&backup).unwrap().len() > 64 * 1024 * 1024);
    let pin = checksum(&backup); assert!(receipt.contains(&pin));
    let refused_root = scratch.0.join("refused");
    let error = refused(command(&["restore", text(&backup), text(&refused_root), "--trusted-local",
        "--expected-sha256", &pin, "--destination-instance", "991", "--max-archive-bytes", "67108864"]));
    assert!(error.contains("archive-byte limit")); assert!(!refused_root.exists());
    ...
    let receipt = success(command(&["restore", text(&backup), text(&target), "--trusted-local",
        "--expected-sha256", &pin, "--destination-instance", "991", "--timeout-secs", "1800"]));
    ...
    assert_eq!(selected.snapshot().refs[&RefName::try_new(b"refs/heads/main").unwrap()], commit);
