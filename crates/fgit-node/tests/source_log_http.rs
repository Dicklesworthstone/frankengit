...
fn native_log_pages_pin_complete_history_and_survive_reopen_without_a_transaction() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (_, oracle) = node.runtime().block_on(node.read_commit_history_in(&node.request_context(),
        ...
        let first = post(&server.client, "log", 'a', &(query.clone() + "&limit=1"), false);
        status(&first, 200);
        ...
        assert!(first.body.contains("\"transaction_created\":false,\"published\":false"));
        ...
        assert_eq!(text(&tail.body, "object_id"), oracle.commits[1].id.to_string());
        assert_eq!(text(&tail.body, "body_hex"), hex(&oracle.commits[1].body));
        assert!(tail.body.contains("\"next_after\":null"));
        let end = post(&server.client, "log", 'a', &format!("{query}&after=2&expected_head={pin}"), false);
        status(&end, 200);
        assert!(end.body.contains("\"commits\":[]"));
        status(&post(&server.client, "log", 'a', &(query.clone() + "&after=1"), false), 400);
        status(&post(&server.client, "log", 'a', &format!("{query}&after=3&expected_head={pin}"), false), 400);
        let exhausted = post(&server.client, "log", 'a', &(query.clone() + "&max_commits=1"), false);
        status(&exhausted, 413);
        assert!(!exhausted.body.contains("\"commits\":[]"));
        status(&post(&server.client, "log", 'a', &format!("{query}&expected_commit={}", "e".repeat(format.digest_len() * 2)), false), 409);
        ...
        let sentinel = LoopbackReceiveSession::authenticated(OWNER, IdempotencyKey::new(b"read-only-source-query".to_vec()).unwrap());
        ...
        assert_eq!(post(&server.client, "log", 'a', &(query + "&limit=1"), true), first);
...
fn history_requires_read_scope_before_ingress_and_honors_rotation_and_disable() {
    ...
    let denied = exchange(&server.client, &request(&server.client, "/api/v1/source/log", 'b',
        "Content-Type: application/x-www-form-urlencoded\r\nContent-Length: 100\r\nExpect: 100-continue\r\n", &[]), true);
    status(&denied, 403);
    status(&post(&server.client, "log", 'c', &query, false), 403);
    status(&post(&server.client, "log", 'd', &query, false), 401);
    let key = exchange(&server.client, &request(&server.client, "/api/v1/source/log", 'a',
        &format!("Content-Type: application/x-www-form-urlencoded\r\nContent-Length: {}\r\nIdempotency-Key: forbidden-read-key\r\n", query.len()), query.as_bytes()), true);
    status(&key, 400);
...
fn even_an_unrelated_authority_write_invalidates_history_continuations() {
    ...
    status(&write, 200);
    let stale = post(&server.client, "log", 'a', &format!("{query}&after=1&expected_head={pin}"), false);
    status(&stale, 409);
    assert!(stale.body.contains("source_snapshot_moved"));
    assert!(!stale.body.contains("body_hex"));
    let current = post(&server.client, "log", 'a', &format!("{query}&limit=1&expected_commit={main}"), true);
    status(&current, 200);
    assert_ne!(token(&current), pin);
    assert_eq!(text(&current.body, "source_commit"), main.to_string());
    let tail = post(&server.client, "log", 'a', &format!("{query}&after=1&expected_head={}", token(&current)), false);
    status(&tail, 200);
