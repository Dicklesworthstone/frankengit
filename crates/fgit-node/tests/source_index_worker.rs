...
fn foreground_worker_builds_then_resumes_without_generation_churn() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let indexed = search(&node).unwrap();
        assert_eq!(indexed.results.hits.len(), 4);
        ...
        let node = reopen(&config);
        assert_eq!(search(&node).unwrap().generation, indexed.generation);
        assert_eq!(generation(&node), before);
        node.shutdown().unwrap();
    }
}
...
fn stopped_worker_never_builds_and_missing_resume_never_resets_state() {
    ...
    let state = progress(&root);
    assert!(
        !worker(&root, GitHashAlgorithm::Sha1, "resume")
            .status
            .success()
    );
    ...
    assert!(
        worker(&root, GitHashAlgorithm::Sha1, "init")
            .status
            .success()
    );
    let node = reopen(&config);
    assert!(
        matches!(search(&node), Err(NodeWorkspaceRefusal::SourceIndex(e)) if matches!(*e, IndexError::Uninitialized))
    );
    node.shutdown().unwrap();
...
fn stale_lock_blocks_a_second_worker_without_touching_checkpoint() {
    ...
    assert_eq!(
        fs::read(state.join("run.lock")).unwrap(),
        b"unresolved owner"
    );
}
