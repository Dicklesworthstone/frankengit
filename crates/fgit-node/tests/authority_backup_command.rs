...
use fgit_authority::{AuthorityLimits, CasOutcome, HeadGeneration, HeadInit, HeadKey, HeadRead,
    ImmutableKey, ImmutableRead, StoreInstanceId};
...
impl Scratch {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!("fg-portable-disk-{}-{id}", std::process::id()));
...
fn exported_authority_recovers_after_source_removal_and_reopens_with_new_tokens() {
    ...
    let mut source = runtime.block_on(FsqliteAuthorityStore::open(&cx, text(&source_path),
        StoreInstanceId::from_raw(51), AuthorityLimits::default())).unwrap();
    let original = runtime.block_on(async {
        source.put_if_absent(&cx, &body_key, b"persisted canonical payload\0\xff").await.unwrap();
        let HeadInit::Created(first) = source.initialize_head(&cx, &key, HeadGeneration::FIRST, b"first").await.unwrap()
            else { panic!("fresh store") };
        let CasOutcome::Committed(current) = source.compare_exchange_head(&cx, &key, first.token(),
            HeadGeneration::try_new(2).unwrap(), b"second").await.unwrap() else { panic!("source CAS") };
        current
    });
    ...
    let receipt = success(command(&["export", text(&source_path), text(&backup_path), "--trusted-local"]));
    assert!(receipt.contains("\"git_objects_included\":false"));
    assert!(receipt.contains("\"head_generation\":2"));
    let hash = receipt.split_once("\"sha256\":\"").unwrap().1.split('"').next().unwrap();
    ...
    let restored = success(command(&["restore", text(&backup_path), text(&destination), "--trusted-local",
        "--expected-sha256", hash, "--destination-instance", "52"]));
    assert!(restored.contains("\"source_tokens_preserved\":false"));
    assert!(restored.contains("\"routing_published\":false"));
    assert!(!source_path.exists());
    assert!(refusal(command(&["restore", text(&backup_path), text(&destination), "--trusted-local",
        "--expected-sha256", hash, "--destination-instance", "52"])).contains("new restore directory"));

    let runtime = RuntimeProfile::production(2).build().unwrap();
    let context = make_context(&runtime);
    let mut target = runtime.block_on(FsqliteAuthorityStore::open_portable_source(&context,
        text(&destination.join("authority.fsqlite")), AuthorityLimits::default())).unwrap();
    runtime.block_on(async {
        assert_eq!(target.instance_id(), StoreInstanceId::from_raw(52));
        let HeadRead::Present(restored) = target.read_head(&context, &key).await.unwrap() else { panic!("restored head") };
        ...
        assert!(target.authenticate_head_receipt(&context, &original).await.is_err());
        target.authenticate_head_receipt(&context, &restored).await.unwrap();
        assert_eq!(target.read_immutable(&context, &body_key).await.unwrap(),
            ImmutableRead::Present(b"persisted canonical payload\0\xff".to_vec()));
        let CasOutcome::Committed(next) = target.compare_exchange_head(&context, &key, restored.token(),
            HeadGeneration::try_new(3).unwrap(), b"after recovery").await.unwrap() else { panic!("restored CAS") };
    ...
    drop(target); drop(context);
    assert!(runtime.join_root(Duration::from_secs(5)));

    let mut bad = fs::read(&backup_path).unwrap();
    let end = bad.len() - 1; bad[end] ^= 1;
    fs::write(&backup_path, bad).unwrap();
    let rejected = scratch.0.join("checksum-rejected");
    assert!(refusal(command(&["restore", text(&backup_path), text(&rejected), "--trusted-local",
        "--expected-sha256", hash, "--destination-instance", "53"])).contains("checksum mismatch"));
