...
fn issue_list_walk_survives_insert_edit_close_and_listener_restart() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for number in [1, 3, 5] { // 1..3
            committed(&post(&server, &format!("/api/v1/issues/{number}/open"), 'b',
                &format!("open-{number}"), &format!("expected_version=0&title=Original-{number}&body="), false));
        }
        let first = get(&server, "/api/v1/issues?limit=1", 'a'); // 4
        status(&first, 200);
        assert!(first.body.contains("\"number\":1"));
        let pin = token(&first);
        committed(&post(&server, "/api/v1/issues/2/open", 'b', "later-insertion",
            "expected_version=0&title=Inserted-after-pin&body=", true)); // 5
        committed(&post(&server, "/api/v1/issues/3/edit", 'b', "later-edit",
            "expected_version=1&title=Edited-after-pin", false)); // 6
        ...
        committed(&post(&server, "/api/v1/issues/5/close", 'b', "later-close",
            "expected_version=1", false)); // 8
        ...
        assert!(fresh.body.contains("Inserted-after-pin") && fresh.body.contains("Edited-after-pin"));
        assert!(fresh.body.contains("\"state\":\"closed\""));
        let absent = get(&server, &format!("/api/v1/issues/2?expected_head={pin}"), 'a'); // 11
        status(&absent, 404);
        ...
        let unavailable = get(&server, &format!("/api/v1/issues?after=1&expected_head={unknown}"), 'a'); // 12
        ...
        assert_eq!(node.runtime().block_on(node.materialize_admission()).unwrap().basis().generation().get(),
            before.get() + 6, "only the six mutations publish");
        ...
        assert_eq!(node.runtime().block_on(node.materialize_admission()).unwrap().basis().generation().get(), before.get() + 6);
...
fn pinned_comment_history_is_stable_but_the_token_never_grants_access() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        committed(&post(&server, "/api/v1/issues/7/open", 'c', "open-seven",
            "expected_version=0&title=History&body=", false)); // 1
        committed(&post(&server, "/api/v1/issues/7/comment", 'c', "comment-one",
            "expected_version=1&body=First-comment", true)); // 2
        committed(&post(&server, "/api/v1/issues/7/comment", 'c', "comment-two",
            "expected_version=2&body=Second-comment", false)); // 3
        let first = get(&server, "/api/v1/issues/7?limit=1", 'a'); // 4
        status(&first, 200);
        let pin = token(&first);
        committed(&post(&server, "/api/v1/issues/7/comment", 'c', "later-comment",
            "expected_version=3&body=Not-in-the-pinned-history", false)); // 5
        committed(&post(&server, "/api/v1/issues/7/close", 'c', "later-close",
            "expected_version=4", true)); // 6
        ...
        let last = get(&server, &format!("/api/v1/issues/7?limit=1&after_version=2&expected_head={pin}"), 'a'); // 8
        status(&last, 200);
        assert_eq!(token(&last), pin);
        ...
        assert!(fresh.body.contains("\"state\":\"closed\""));
        replace(&path, &(header + &row('e', 0xa1, "issues-read") + &row('d', 0xd1, "read,receive")));
        ...
        let node = reopened(configuration);
        assert_eq!(node.runtime().block_on(node.materialize_admission()).unwrap().basis().generation().get(), before.get() + 5);
        node.shutdown().unwrap();
    }
}
