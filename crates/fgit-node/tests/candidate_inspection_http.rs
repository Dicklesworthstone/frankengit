...
fn open_local(node: &OneNode, candidate: &Candidate) {
    ...
    let session = LoopbackReceiveSession::authenticated(
        OWNER,
        IdempotencyKey::new(b"open-inspection".to_vec()).unwrap(),
    );
    let result = node
        .runtime()
...
fn inspected_result_is_not_the_source_diff_and_does_not_vote_or_stage_after_restart() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(
            original
                .body
                .contains(&format!("\"path_hex\":\"{}\"", hex(PATH)))
        );
        assert!(
            original
                .body
                .contains(&format!("\"before_hex\":\"{}\"", hex(b"left\n")))
        );
        assert!(
            original
                .body
        ...
        assert!(
            !original
                .body
                .contains(&format!("\"after_hex\":\"{}\"", hex(b"right\n"))),
        ...
        status(&votes, 200);
        assert!(votes.body.contains("\"reviews\":[]"));
        ...
        status(&missing, 404);
        assert!(!missing.body.contains("candidate_commit_body_hex"));
        assert_eq!(server.finish().accepted_sessions(), 5);
        let node = reopen(&config);
        assert_eq!(
            generation(&node),
            before + 1,
            "PR opening only; inspection has no canonical decision"
        );
        assert!(node.read_git_object(candidate.binding.commit).is_err());
        assert!(
            node.read_git_object(git_object_id(format, GitObjectKind::Blob, TEXT))
                .is_err()
        );
        let absent = LoopbackReceiveSession::authenticated(
            FOREIGN,
            IdempotencyKey::new(b"read-only-candidate-inspection".to_vec()).unwrap(),
        );
        assert!(matches!(
            node.runtime()
                .block_on(node.recover_transaction_in(&node.request_context(), &absent))
                .unwrap(),
        ...
        assert_eq!(
            json_text(&after_vote.body, "candidate_commit_body_hex"),
            json_text(&original.body, "candidate_commit_body_hex")
        );
        accepted(&send(
            &server.client,
            "merge",
        ...
        status(&stale, 409);
        assert!(stale.body.contains("inspection_subject_moved"));
        server.finish();
        let node = reopen(&config);
...
fn corrupt_mismatched_and_incomplete_uploads_never_disclose_a_partial_inspection() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let mut broken = candidate.bundle.clone();
        *broken.last_mut().unwrap() ^= 1;
        let corrupt = exchange(
            &server.client,
            &inspect_bytes(&server.client, 1, 'd', &command, Some(&broken), false),
            true,
        ); // 1
        status(&corrupt, 400);
        let changed = command.replace(
            &format!("candidate_commit={}", candidate.binding.commit),
            &format!("candidate_commit={}", "e".repeat(format.digest_len() * 2)),
        );
        status(
            &exchange(
                &server.client,
        ...
        let changed = command.replace(
            &format!("policy_epoch={}", candidate.epoch.get()),
            "policy_epoch=2",
        );
        status(
            &exchange(
        ...
        status(
            &exchange(
                &server.client,
                &inspect_bytes(&server.client, 1, 'd', &command, None, false),
                true,
            ),
            400,
        ); // 5
...
fn inspection_scopes_are_not_unioned_and_authentication_precedes_continue() {
    ...
    let (node, _) = resolved(&root, GitHashAlgorithm::Sha1, TEXT);
    let before = generation(&node);
    let path = root.0.join("credentials");
    credentials(&node, &path);
    let disabled = Server::start(node, &path, 1, false, false);
    status(&withheld(&disabled.client, 'd', "", 1024), 403);
    disabled.finish();
    let node = reopen(&config);
    let server = Server::start(node, &path, 8, true, false);
    for token in ['a', 'b', 'c', 'e', 'f'] {
        status(&withheld(&server.client, token, "", 1024), 403);
    } // 1..5
    ...
    status(&key, 400);
    assert!(key.body.contains("inspection_has_no_transaction_key"));
...
fn binary_candidates_are_visible_changes_not_successful_empty_text_diffs() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let reply = inspect(&server.client, &candidate, 'd', false);
        status(&reply, 200);
...
fn node_inspection_refuses_filters_hidden_refs_stale_subjects_cancellation_and_exhaustion() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        options.paths.push(PATH.to_vec());
        assert!(
            node.runtime()
                .block_on(node.inspect_pull_request_bundle_in(
                    &request,
                    &subject,
                    candidate.binding,
                    &candidate.bundle,
        ...
        hidden
            .push_rule(candidate.data.source_ref.as_bytes(), &Default::default())
            .unwrap();
        assert!(
            node.runtime()
                .block_on(node.inspect_pull_request_bundle_in(
        ...
        stale.pull_request_version = AggregateVersion::try_new(2).unwrap();
        assert!(
            node.runtime()
                .block_on(node.inspect_pull_request_bundle_in(
                    &request,
        ...
        let valid = node
            ...
            .unwrap();
        assert_eq!(
            valid.review.pull_request,
            Some((PullRequestNumber::FIRST, AggregateVersion::FIRST))
        );
