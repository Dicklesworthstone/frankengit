...
fn configuration(root: &Scratch, format: GitHashAlgorithm) -> NodeConfig {
    NodeConfig::new(root.0.join("node"), TENANT, REPOSITORY)
...
fn header(node: &OneNode) -> String {
    format!(
        "frankengit-http-credentials-v1 {TENANT} {REPOSITORY} {}\n",
...
fn exchange(server: &Server, token: char, key: &str, suffix: &str, headers: &str) -> (u16, String) {
    ...
    assert_eq!(
        ...
        1,
        "no interim or second final reply"
    );
...
fn assert_complete(reply: &(u16, String), original: &AdmissionResult) {
        ...
        .enumerate()
    {
        let position = reply
            .1
            .find(&format!(
                "\"command_index\":{index},\"ref_name\":\"{name}\""
            ))
            .unwrap();
        assert!(
            position > previous,
            "original wire order, not canonical ref-name order"
        );
        previous = position;
        assert!(
            reply
                .1
                .contains(&format!("\"tx_id\":\"{}\"", original.commands[index].tx_id))
        );
    }
...
fn seed_lost_push(node: &OneNode, format: GitHashAlgorithm) -> AdmissionResult {
    ...
    let NodeSmartHttpRefusal::ReceiveResponse { outcome, .. } = error else {
        panic!("native admission must complete before broken report")
    };
    assert!(matches!(
...
fn whole_push_recovery_survives_restart_rotation_and_lost_reports_without_write_permission() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let indexed = exchange(
            &server,
            'a',
            "original-private-session-key",
            "/receive/1",
            "Content-Length: 0\r\n",
        ); // 3
        ...
        assert!(
            indexed
                .1
                .contains(&format!("\"tx_id\":\"{}\"", known.commands[1].tx_id))
        );
        ...
        assert!(
            foreign
                .1
                .contains("\"command_count\":null,\"command_count_verified\":false")
        );
        ...
        assert_eq!(
            generation(&node),
            before,
            "recovery cannot publish or resubmit commands"
        );
        assert_eq!(
            node.runtime()
