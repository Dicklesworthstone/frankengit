...
use fgit_node::{
    OneNode,
    source_retrieval::{
        Checkpoints, InitialLimits, InitialQuery, InitialReport, RetrievalError, SymbolChannel,
        SymbolPolicy, SymbolUnavailable,
    },
};
...
fn query(policy: SymbolPolicy, prefixes: &[Vec<u8>]) -> InitialQuery {
    InitialQuery::new(&[b"Thing".to_vec()], prefixes)
        .unwrap()
        .with_symbols(b"Thing", SymbolMatchMode::Prefix, &[], policy)
        .unwrap()
}
...
fn initial(
    ...
    node.runtime().block_on(node.search_source_initial_local_in(
        &node.outbox_delivery_context(),
        &reference(),
        None,
        None,
...
fn channels_match_native_owners_share_source_and_keep_generation_vector_after_reopen() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let q = query(SymbolPolicy::Required, &[]);
        ...
        for (channel, expected) in [(q.content(), report.content()), (q.path(), report.path())] {
            let direct = node
                ...
                .block_on(node.search_source_index_local_in(
                    &node.outbox_delivery_context(),
                    &reference(),
                    Some(report.source().source_head),
                    Some(commit),
                    Some(&report.generations().lexical),
                    floors.lexical.as_ref(),
                    channel,
                    None,
                    Default::default(),
                    Default::default(),
        ...
        let direct = node
            ...
            .block_on(node.search_source_symbols_index_snapshot_local_in(
                ...
                Some(report.source().source_head),
                Some(commit),
                floors.symbols.as_ref(),
                q.symbols().unwrap().0,
                Default::default(),
                symbols::MAX_INDEX_BYTES,
            ))
            .unwrap();
        assert_eq!(found.matches, direct.matches);
        let scoped = initial(
            &node,
            &query(SymbolPolicy::Required, &[b"src".to_vec()]),
            &floors,
            Default::default(),
        )
        .unwrap();
        ...
        let SymbolChannel::Available(found) = scoped.symbols() else {
            panic!("required symbols absent")
        };
        assert_eq!(found.matches.len(), 1);
        assert_eq!(found.matches[0].location.path, b"src/Thing.rs");
...
fn authenticated_empty_symbol_inventory_is_available_not_missing() {
    ...
    .unwrap();
    let SymbolChannel::Available(found) = report.symbols() else {
        panic!("initialized empty corpus unavailable")
    };
    assert_eq!(found.indexed_files, 0);
    assert!(found.matches.is_empty());
    assert!(report.complete());
    assert!(report.generations().symbols.is_some());
    node.shutdown().unwrap();
}
...
fn stale_symbols_never_mix_old_source_with_refreshed_lexical_results() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        node.runtime()
            ...
            .unwrap();
        let no_symbol_floor = Checkpoints {
            symbols: None,
            ..floors.clone()
        };
        let q = query(SymbolPolicy::Optional, &[]);
        ...
        assert!(matches!(
            report.symbols(),
            SymbolChannel::Unavailable(SymbolUnavailable::Stale)
        ));
        ...
        assert!(
            node.runtime()
                .block_on(node.search_source_initial_local_in(
                    &node.outbox_delivery_context(),
                    &reference(),
                    Some(old.source().source_head),
                    Some(commit),
                    &floors,
                    &q,
                    Default::default()
        ...
        node.runtime()
            .block_on(node.refresh_source_symbol_index_local_in(
                ...
                None,
                Some(next),
                floors.symbols.as_ref().unwrap().generation_id,
                Default::default(),
            ))
            .unwrap();
        let current = initial(&node, &q, &floors, Default::default()).unwrap();
        let SymbolChannel::Available(found) = current.symbols() else {
            panic!("refreshed symbols absent")
        };
        assert_eq!(found.matches.len(), 3);
        assert!(current.complete());
        assert_eq!(found.source.commit, next);
        assert_eq!(generation(&node), canonical);
        node.shutdown().unwrap();
    }
}
...
fn shared_limits_truncation_and_cancellation_have_no_publication_side_effects() {
    let root = Scratch::new();
    let (node, _) = corpus(&root, GitHashAlgorithm::Sha256);
    let floors = build(&node, true);
    let canonical = generation(&node);
    let q = query(SymbolPolicy::Required, &[]);
    ...
    assert_eq!(bounded.content().results.hits.len(), 1);
    ...
    assert!(
        initial(
            ...
            &floors,
            InitialLimits {
                max_result_bytes: full.result_bytes(),
                ..limits
            }
        )
        .is_ok()
    ...
    assert!(matches!(
        initial(
            ...
            &floors,
            InitialLimits {
                max_result_bytes: full.result_bytes() - 1,
                ..limits
            }
        ),
    ...
    drop(
        node.search_source_initial_local_in(&request, &reference, None, None, &floors, &q, limits),
    );
    request.cancel();
    assert!(
        node.runtime()
            .block_on(node.search_source_initial_local_in(
                &request, &reference, None, None, &floors, &q, limits
            ))
            .is_err()
    );
    let again = initial(&node, &q, &floors, limits).unwrap();
    assert_eq!(again.generations(), full.generations());
    assert_eq!(generation(&node), canonical);
    node.shutdown().unwrap();
}
...
fn missing_lexical_index_never_becomes_a_source_scan_or_implicit_build() {
    ...
    for _ in 0..2 {
        ...
        assert!(
            matches!(result, Err(RetrievalError::Source(fgit_node::NodeWorkspaceRefusal::SourceIndex(error)))
