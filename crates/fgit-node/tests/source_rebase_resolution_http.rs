...
fn resolved(endpoint: &Endpoint, command: &str, files: &[(&str, &[u8])], reverse: bool, chunked: bool) -> BinaryReply {
    ...
    binary_exchange(endpoint, &request(endpoint, "POST", "/api/v1/source/rebase/resolve", 'a',
...
fn two_conflicts_reconstruct_after_restart_and_exact_binary_empty_files_publish_only_at_the_end() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let root = Scratch::new(); let (node, h) = fixture(&root, format, 1);
        let credentials = root.0.join("credentials"); configure(&node, &credentials);
        let before = generation(&node); let form = prepare_form(&h);
        let server = SourceServer::start(node, &credentials, 2);
        let first = post_form(&server.client, "rebase/prepare", 'a', &form, false); status(&first, 409);
        let pinned = form.clone()+"&expected_head="+&encode(field(json(&first), "snapshot_token").as_bytes());
        let partial_form = pinned.clone()+&format!("&resolution={}:61ff:theirs", h.first);
        let partial = post_form(&server.client, "rebase/resolve", 'a', &partial_form, true); status(&partial, 409);
        assert_eq!(field(json(&partial), "stopped_commit"), h.source.to_string());
        assert_eq!(numeric(json(&partial), "step_count"), 1);
        assert_eq!(numeric(json(&partial), "resolution_consumed_commits"), 1);
        assert!(json(&partial).contains("\"candidate_commit\":null") && json(&partial).contains("\"bundle\":null"));
        let provisional = GitOid::from_hex(format, field(json(&partial), "rewritten")).unwrap();
        server.finish();
        let node = reopen(&root.config(format));
        assert_eq!(generation(&node), before); assert_eq!(tip(&node, b"refs/heads/topic"), h.source);
        assert!(node.read_git_object(provisional).is_err(), "a resolved prefix must not be staged");
        let server = SourceServer::start(node, &credentials, 5);
        assert_eq!(partial, post_form(&server.client, "rebase/resolve", 'a', &partial_form, false));
        let full = pinned.clone()+&format!("&resolution={}:61ff:file:100755:file_0&resolution={}:62:file:100644:file_1", h.first, h.source);
        let data = b"\0\xff\r\nexact binary";
        ...
        assert!(metadata.contains("original-commit-path-v1") && metadata.contains("\"series_complete\":true"));
        assert_eq!(prepared, resolved(&server.client, &full, &files, true, true));
        ...
        status(&multipart(&server.client, "rebase/apply", 'a', &command, &bundle, Some("resolved-series")), 403);
        let applied = multipart(&server.client, "rebase/apply", 'b', &command, &bundle, Some("resolved-series"));
        status(&applied, 200); assert!(json(&applied).contains("\"outcome\":\"committed\""));
        server.finish();
        let node = reopen(&root.config(format));
        assert_eq!(generation(&node), before + 1); assert_eq!(tip(&node, b"refs/heads/topic"), candidate);
        ...
        drop(object);
        let server = SourceServer::start(node, &credentials, 2);
        assert_eq!(applied, multipart(&server.client, "rebase/apply", 'b', &command, &bundle, Some("resolved-series")));
...
fn resolution_refuses_bad_subjects_parts_and_grants_and_preserves_explicit_side_and_empty_choices() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        status(&post_form(&server.client, "rebase/resolve", 'a', &(form.clone()+&one), false), 400);
        status(&post_form(&server.client, "rebase/resolve", 'a', &(pinned.clone()+&one+"&force=true"), false), 400);
        status(&post_form(&server.client, "rebase/resolve", 'a', &(pinned.clone()+&format!("&resolution={}:61ff:theirs", h.onto)), false), 400);
        status(&post_form(&server.client, "rebase/resolve", 'a', &(pinned.clone()+&one+&one), false), 400);
        status(&post_form(&server.client, "rebase/resolve", 'a', &(pinned.clone()+&format!("&resolution={}:6f6e746f:theirs", h.first)), false), 409);
        status(&post_form(&server.client, "rebase/resolve", 'a', &(pinned.clone()+&format!("&resolution={}:61ff:file:100644:file_0", h.first)), false), 400);
        status(&resolved(&server.client, &(pinned.clone()+&one), &[("file_0", b"unused")], false, false), 400);
        let both = one.clone()+&format!("&resolution={}:62:theirs", h.source);
        status(&post_form(&server.client, "rebase/resolve", 'a', &(pinned.clone()+&both+"&max_conflicts=1"), false), 413);
        for (token, key, expected) in [('b', "", 403), ('a', "Idempotency-Key: no-resolution-transaction\r\n", 400)] {
            let refused = binary_exchange(&server.client, &request(&server.client, "POST", "/api/v1/source/rebase/resolve", token,
                &format!("Content-Type: application/x-www-form-urlencoded\r\nContent-Length: 1\r\nExpect: 100-continue\r\n{key}"), &[]), true);
            status(&refused, expected); assert!(!refused.head.contains("100 Continue"));
        }
        ...
        status(&empty_stop, 409); assert_eq!(field(json(&empty_stop), "state"), "became_empty");
        assert_eq!(numeric(json(&empty_stop), "resolution_consumed_commits"), 1);
        assert_eq!(numeric(json(&empty_stop), "step_count"), 0);
        let all_ours = ours.replace("empty=stop", "empty=drop")+&format!("&resolution={}:62:ours", h.source);
        ...
        for choice in ["base", "delete"] {
            let command = pinned.clone()+&format!("&resolution={}:61ff:{choice}&resolution={}:62:theirs", h.first, h.source);
        ...
        assert_eq!(tip(&node, b"refs/heads/topic"), h.source); node.shutdown().unwrap();
    }
}
