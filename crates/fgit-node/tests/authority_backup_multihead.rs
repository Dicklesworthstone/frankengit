...
use fgit_authority::{
    AuthorityLimits, AuthorityVersionToken, CasOutcome, HeadGeneration, HeadInit, HeadKey,
    HeadReadReceipt, ImmutableKey, StoreInstanceId,
};
...
fn fixture(scratch: &Scratch) -> (PathBuf, MultiHeadSnapshot) {
    ...
    let snapshot = store(&database, 41, |runtime, source, cx| {
        runtime.block_on(async {
            source
                .put_if_absent(
                    cx,
                    &ImmutableKey::new(b"shared\0\xff".to_vec()).unwrap(),
                    b"exact immutable bytes\0\xff",
                )
                .await
                .unwrap();
            let repo = HeadKey::new(b"repository/head".to_vec()).unwrap();
            let index = HeadKey::new(b"index/head".to_vec()).unwrap();
            let HeadInit::Created(repo_first) = source
                .initialize_head(cx, &repo, generation(5), b"repo-old")
                .await
                .unwrap()
            else {
                panic!("new repo head");
            };
            let HeadInit::Created(index_first) = source
                .initialize_head(cx, &index, generation(1), b"index-old")
                .await
                .unwrap()
            else {
                panic!("new index head");
            };
            assert!(matches!(
                source
...
fn all_heads_backup_restores_without_source_and_preserves_future_updates() {
    ...
    let (database, original) = fixture(&scratch);
    ...
    assert!(
        !v1.exists(),
        "the old format must not silently choose a head"
    );
    let archive = scratch.0.join("all.backup");
    let receipt = success(export(&database, &archive));
    assert!(receipt.contains("\"heads\":2,"));
    let bytes = fs::read(&archive).unwrap();
    let pin = checksum(&bytes);
    assert!(receipt.contains(&pin));
    assert_eq!(
        decode_multi_head_snapshot(&bytes, Default::default(), Default::default()).unwrap(),
        original
    );
    refused(export(&database, &archive));
    ...
    let advanced = store(&restored_db, 99, |runtime, restored, cx| {
        runtime.block_on(async {
            assert_eq!(restored.instance_id().raw(), 99);
            let receipts = restored
                .verify_multi_head_import(cx, &original, Default::default())
                .await
                .unwrap();
            ...
            for row in &original.issuance {
                ...
                assert!(
                    restored
                        .authenticate_head_receipt(cx, &source)
                        .await
                        .is_err()
                );
            }
            ...
            let result = restored
                ...
                .await
                .unwrap();
            assert_eq!(result.issuance.len(), 6);
            assert_eq!(result.heads.len(), 2);
            assert_eq!(result.bodies, original.bodies);
            result
        })
    });
    ...
    store(&restored_db, 99, |runtime, restored, cx| {
        assert_eq!(
            runtime
                .block_on(restored.export_multi_head_portable(cx, Default::default()))
                .unwrap(),
            advanced
        );
    });
...
fn corrupt_lineage_valid_checksums_and_wrong_format_cannot_create_destinations() {
    let scratch = Scratch::new();
    let (database, original) = fixture(&scratch);
    let archive = scratch.0.join("good.backup");
    success(export(&database, &archive));
    let bytes = fs::read(&archive).unwrap();
    let pin = checksum(&bytes);
    ...
    refused(restore(&archive, &wrong_pin, &"00".repeat(32)));
    assert!(!wrong_pin.exists());
    for change in 0..3 {
        ...
        match change {
            ...
            1 => {
                // A fully self-consistent OLD head is still not the slot's latest state.
                let old = &bad.issuance[0];
                let slot = bad
                    .heads
                    .iter_mut()
                    .find(|head| head.key == old.head_key)
                    .unwrap();
                slot.token = old.token.clone();
                slot.generation = old.generation;
                slot.body = old.body.clone();
            }
            _ => bad.issuance[0].token[0] ^= 1,
        }
        let malformed = fgit_codec::encode_body(&bad).unwrap();
        let input = scratch.0.join(format!("bad-{change}.backup"));
