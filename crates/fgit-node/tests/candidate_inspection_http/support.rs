...
pub fn resolved(root: &Scratch, format: GitHashAlgorithm, content: &[u8]) -> (OneNode, Candidate) {
    ...
    let metadata = MergeMetadata { author: "Inspector <inspect@example.invalid>".into(),
        committer: "Builder <build@example.invalid>".into(), timestamp: 1,
        message: b"Actual inspected candidate\n".to_vec() };
    let automatic = node.runtime().block_on(node.prepare_merge_bundle_in(&context, &data.target_ref,
        &data.source_ref, &RefVisibility::new(), &metadata, PreparationLimits::default())).unwrap();
    let MergePreparation::Conflicted { base, .. } = automatic.outcome else { panic!("native conflict fixture") };
    let inputs = ResolutionInputs { base, target: data.target_tip, source: data.source_tip };
    let choices = [ConflictResolution { path: PATH.to_vec(), choice: ResolutionChoice::File { mode: 0o100755, bytes: content.to_vec() } }];
    let built = node.runtime().block_on(node.prepare_resolved_merge_bundle_in(&context,
        &data.target_ref, &data.source_ref, &RefVisibility::new(), Some(automatic.source_head),
        inputs, &choices, &metadata, PreparationLimits::default())).unwrap();
    let epoch = node.runtime().block_on(node.materialize_admission()).unwrap().basis().body().policy_epoch;
    let candidate = Candidate { data, binding: CandidateBinding { merge_base: base, commit: built.resolved.plan.commit },
        epoch, bundle: built.bundle };
    assert!(node.read_git_object(candidate.binding.commit).is_err());
    (node, candidate)
}
pub fn subject(candidate: &Candidate) -> ReviewSubject {
    ReviewSubject { pull_request: PullRequestNumber::FIRST, pull_request_version: AggregateVersion::FIRST,
        source_ref: candidate.data.source_ref.clone(), target_ref: candidate.data.target_ref.clone(),
        source_tip: candidate.data.source_tip, target_tip: candidate.data.target_tip, policy_epoch: candidate.epoch }
}
pub fn credentials(node: &OneNode, path: &Path) -> String {
    let header = header(node);
    replace(path, &(header.clone() + &row('a', OWNER, "pulls-read,pulls-write")
...
pub fn multipart(command: &str, bundle: Option<&[u8]>, reverse: bool) -> Vec<u8> {
    ...
    text.extend_from_slice(command.as_bytes()); text.extend_from_slice(b"\r\n");
    ...
    if let Some(bundle) = bundle {
        ...
        binary.extend_from_slice(bundle); binary.extend_from_slice(b"\r\n");
    }
    let mut body = if reverse { [binary, text].concat() } else { [text, binary].concat() };
    body.extend_from_slice(b"--inspect-boundary--\r\n"); body
}
pub fn inspect_bytes(endpoint: &Endpoint, number: u64, token: char, command: &str,
    bundle: Option<&[u8]>, chunked: bool,
) -> Vec<u8> {
    ...
    request(endpoint, "POST", &format!("/api/v1/pulls/{number}/inspect"), token,
...
pub fn inspect(endpoint: &Endpoint, candidate: &Candidate, token: char, chunked: bool) -> Reply {
    exchange(endpoint, &inspect_bytes(endpoint, 1, token, &common(candidate), Some(&candidate.bundle), chunked), true)
}
...
pub fn withheld(endpoint: &Endpoint, token: char, extra: &str, length: usize) -> Reply {
    let wire = request(endpoint, "POST", "/api/v1/pulls/1/inspect", token,
