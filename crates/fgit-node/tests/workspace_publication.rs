...
impl Drop for Scratch {
    fn drop(&mut self) {
        if self.0.exists() {
            fs::remove_dir_all(&self.0).expect("owned test directory");
        }
    }
}
...
fn tree(changed: GitOid, untouched: GitOid) -> Vec<u8> {
    [
        b"100644 changed.txt\0".as_slice(),
        changed.as_bytes(),
        b"100644 untouched.txt\0",
        untouched.as_bytes(),
    ]
    .concat()
}
...
fn setup(root: &Path, format: GitHashAlgorithm) -> (OneNode, GitOid, GitOid) {
    ...
    let root_tree = loose(
        ...
        format,
        GitObjectKind::Tree,
        &tree(before, untouched),
    );
    let base = loose(
        &source,
        format,
        GitObjectKind::Commit,
        &commit(root_tree, &[], "base"),
    );
...
fn candidate_publishes_preserves_untouched_objects_and_replays_after_reopen() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            node.read_git_object(untouched).unwrap().payload(),
            b"preserve me\n"
        );
        ...
        assert_eq!(
            observed(&reopened),
            after,
            "replay must not publish another head"
        );
...
fn epoch_zero_candidate_refuses_before_publication_and_epoch_one_twin_commits() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for _ in 0..2 {
            assert!(matches!(
                apply(&node, b"epoch-zero", base, refused_tip, &refused_bundle),
            ...
            assert_eq!(
                after.basis().body(),
                before.basis().body(),
                "strict refusal cannot publish any root"
            );
        }
        ...
        assert_eq!(
            node.read_git_object(refused_tip).unwrap().payload(),
            epoch_zero.as_bytes()
        );
...
fn competing_candidate_is_refused_and_key_reuse_cannot_alias_the_winner() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            observed(&node),
            after_winner,
            "different semantics must not reuse the seal"
        );
        ...
        *corrupt.last_mut().unwrap() ^= 1;
        ...
        assert!(
            apply(
                ...
                base,
                blob,
                &envelope(format, base, blob, &blob_pack)
            )
            .is_err()
        );
        assert_eq!(observed(&node), original);
        assert!(matches!(
            apply(&node, b"valid", base, tip, &valid).unwrap().commands[0]
                .terminal
                .outcome,
            DecisionOutcome::Committed { .. }
        ));
...
fn merely_staged_prerequisites_are_not_authority_and_unserving_nodes_do_not_publish() {
    ...
    let staged = node
        .put_git_object(
            GitObjectKind::Commit,
            commit(fake_tree, &[], "not published"),
        )
        .unwrap()
        .identity();
...
fn terminal_workspace_decisions_recover_before_serving_without_accepting_new_intake() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(matches!(
            committed.commands[0].terminal.outcome,
            DecisionOutcome::Committed { .. }
        ));
        assert!(matches!(
            refused.commands[0].terminal.outcome,
        ...
        assert_eq!(
            apply(&node, b"committed", base, winner, &winner_bundle).unwrap(),
            committed
        );
        assert_eq!(
            apply(&node, b"refused", base, loser, &loser_bundle).unwrap(),
            refused
        );
        ...
        assert!(
            node.runtime()
                .block_on(node.apply_workspace_bundle_durable_in(
                    &request,
                    PrincipalId::from_bytes([0x84; 16]),
                    b"committed",
                    &reference(),
                    base,
                    winner,
                ...
                .is_err(),
            "another principal must not recover this principal's transaction"
        );
        assert_eq!(
            observed(&node),
            before,
            "recovery and rejected intake cannot move authority"
        );
        node.shutdown().unwrap();
    }
}
...
fn terminal_recovery_is_not_fresh_transport_validation_and_still_binds_the_header() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            apply(&node, b"once", base, tip, &transport).unwrap(),
            committed
        );
