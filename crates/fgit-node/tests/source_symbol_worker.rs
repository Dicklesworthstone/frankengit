...
fn worker(
    ...
    command
        ...
        .arg(root.0.join(state))
        .args([mode, "1", "0", "refs/heads/main"])
        .output()
        .unwrap()
}
...
fn selected(
    node: &OneNode,
) -> Result<data::Report, AccessError<NodeWorkspaceRefusal, GenerationAuthorityError>> {
    let query =
...
fn assert_no_lexical_index(node: &OneNode) {
    ...
    assert!(
        matches!(node.runtime().block_on(node.search_source_index_local_in(&node.request_context(),
...
fn add_source(node: &OneNode, base: GitOid) -> GitOid {
    ...
    let metadata = MergeMetadata {
        ...
        timestamp: 3,
        message: b"symbol maintenance edit\n".to_vec(),
    };
    let request = node.request_context();
    let reference = RefName::try_new(REF).unwrap();
    let candidate = node
        .runtime()
...
fn setup(root: &Scratch, node: &OneNode, format: GitHashAlgorithm) -> ProgressFile {
    ...
    let binding = format!(
        "31313131313131313131313131313131 32323232323232323232323232323232 {} {} symbols1",
...
fn symbol_worker_builds_refreshes_and_resumes_without_generation_churn() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let initial = selected(&node).unwrap();
        assert_eq!(initial.indexed_files, 0);
        let next = add_source(&node, commit);
        let canonical = generation(&node);
        assert!(matches!(selected(&node), Err(AccessError::Stale)));
        node.shutdown().unwrap();
        successful(worker(&root, format, "symbols", "resume", true));
        let updated = fs::read(state.join("checkpoint")).unwrap();
        assert_ne!(updated, first);
        let node = reopen(&config);
        let report = selected(&node).unwrap();
        assert_eq!(report.source.commit, next);
        assert_eq!(report.matches.len(), 1);
        assert_eq!(report.matches[0].name, b"ThingNew");
        assert_eq!(report.generation_number, 2);
        assert_no_lexical_index(&node);
...
fn profile_mismatches_fail_before_index_work_and_leave_both_checkpoints_unchanged() {
    ...
    assert_eq!(fs::read(lexical.join("checkpoint")).unwrap(), bytes);
    let node = reopen(&config);
    assert_eq!(selected(&node).unwrap().generation_number, 1);
    assert_eq!(generation(&node), canonical);
    node.shutdown().unwrap();
}
...
fn worker_recovers_a_lost_activation_reply_from_prepublication_symbol_progress() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let result = node
            .runtime()
            .block_on(node.reconcile_source_symbol_index_guarded_local_in(
        ...
        assert!(result.is_ok());
        drop(result); // Deliberately do not acknowledge the successful reply.
        let pending = file.state.rows[REF].pending.unwrap();
        node.shutdown().unwrap();
        file.release().unwrap(); // Explicitly establish quiescence, not stale-lock takeover.
        let output = successful(worker(&root, format, "symbols", "resume", true));
        assert!(output.contains("\"state\":\"recovered\""));
        assert!(!output.contains("observed_current"));
        ...
        successful(worker(&root, format, "symbols", "resume", true));
        assert_eq!(
            fs::read(root.0.join("symbols/checkpoint")).unwrap(),
            checkpoint
        );
    }
}
...
fn stop_and_stale_lock_do_not_initialize_or_reset_symbol_progress() {
    ...
    let path = directory(&root, "symbols");
    assert!(
        !worker(&root, format, "symbols", "resume", true)
            .status
            .success()
    );
    ...
    assert_eq!(
        fs::read(path.join("run.lock")).unwrap(),
        b"unresolved owner"
    );
}
