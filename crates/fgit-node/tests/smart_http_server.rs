...
use fgit_types::{GitHashAlgorithm, GitOid, HeadGeneration, PrincipalId, RefName, RepositoryId, TenantId};
...
impl Scratch {
    fn new() -> Self {
        Self(std::env::temp_dir().join(format!("fg-http-server-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed))))
    }
}
...
fn config(scratch: &Scratch, format: GitHashAlgorithm) -> NodeConfig {
    NodeConfig::new(scratch.0.clone(), TenantId::from_bytes([0xc1; 16]), RepositoryId::from_bytes([0xc2; 16]))
...
impl Server {
    fn start(node: OneNode, sessions: usize, receive: bool, in_flight: usize) -> Self {
        ...
        let worker = thread::spawn(move || {
            let result = node.serve_smart_http_bounded(&listener,
                GitDaemonServerLimits::try_new(sessions, in_flight).unwrap(), sha256_digest(TOKEN.as_bytes()),
                PrincipalId::from_bytes([0xc3; 16]), receive, Duration::from_secs(30));
...
fn exchange(server: &Server, request: &[u8], fragment: usize) -> Vec<u8> {
    ...
    for bytes in request.chunks(fragment) { stream.write_all(bytes).unwrap(); }
    ...
    (&mut stream).take(4 * 1024 * 1024).read_to_end(&mut response).unwrap();
    response
}
fn request(server: &Server, method: &str, suffix: &str, headers: &str, body: &[u8]) -> Vec<u8> {
    let mut bytes = format!("{method} {}{suffix} HTTP/1.1\r\nHost: loopback\r\n{headers}\r\n", server.route).into_bytes();
    bytes.extend_from_slice(body);
    bytes
}
fn auth() -> String { format!("Authorization: Bearer {TOKEN}\r\n") }
fn post_headers(service: &str, length: usize, extra: &str) -> String {
    format!("{}Content-Type: application/x-git-{service}-request\r\nContent-Length: {length}\r\n{extra}", auth())
}
...
fn packets(command: Vec<u8>, pack: &[u8]) -> Vec<u8> {
    let mut bytes = encode_packets(&[Packet::Data(command), Packet::Flush], &WireLimits::default()).unwrap();
...
fn stored_zlib(body: &[u8]) -> Vec<u8> {
    ...
    out.extend_from_slice(&length.to_le_bytes()); out.extend_from_slice(&(!length).to_le_bytes());
...
fn commit_pack(format: GitHashAlgorithm) -> (GitOid, Vec<u8>, BTreeSet<Vec<u8>>) {
    ...
    let mut tree = b"100644 README\0".to_vec(); tree.extend_from_slice(blob_id.as_bytes());
    ...
    for (kind, body) in [(1, &commit), (2, &tree), (3, &blob)] {
        pack.extend_from_slice(&object_header(kind, body.len())); pack.extend_from_slice(&stored_zlib(body));
    }
...
fn generation(config: &NodeConfig, expected: Option<GitOid>) -> HeadGeneration {
    ...
    assert_eq!(materialized.snapshot().refs.get(&reference).copied(), expected);
...
fn round_trip(format: GitHashAlgorithm) {
    ...
    let discovery = request(&server, "GET", "/info/refs?service=git-upload-pack", &format!("{}Git-Protocol: version=2\r\n", auth()), b"");
    let body = response_body(&exchange(&server, &discovery, 1));
    assert!(body.windows(b"fetch=shallow filter".len()).any(|b| b == b"fetch=shallow filter"));
    assert!(body.windows(format!("object-format={}", format.as_str()).len())
    ...
    let create = packets(format!("{zero} {commit} refs/heads/main\0report-status object-format={}", format.as_str()).into_bytes(), &pack);
    let encoded = chunked(&create);
    let headers = format!("{}Content-Type: application/x-git-receive-pack-request\r\nTransfer-Encoding: chunked\r\nIdempotency-Key: http-create-1\r\nExpect: 100-continue\r\n", auth());
    ...
    while !interim.ends_with(b"\r\n\r\n") {
        let mut byte = [0]; connection.read_exact(&mut byte).unwrap(); interim.push(byte[0]);
        assert!(interim.len() <= 1024);
    }
    assert_eq!(interim, b"HTTP/1.1 100 Continue\r\n\r\n");
    for fragment in encoded.chunks(3) { connection.write_all(fragment).unwrap(); }
    let mut response = Vec::new(); connection.read_to_end(&mut response).unwrap(); drop(connection);
    assert!(response_body(&response).windows(b"ok refs/heads/main".len()).any(|b| b == b"ok refs/heads/main"));
    ...
    let retry = request(&server, "POST", "/git-receive-pack", &post_headers("receive-pack", create.len(), "Idempotency-Key: http-create-1\r\n"), &create);
    assert!(response_body(&exchange(&server, &retry, 4096)).windows(b"ok refs/heads/main".len()).any(|b| b == b"ok refs/heads/main"));
    assert_eq!(generation(&config, Some(commit)), published);

    let legacy = request(&server, "GET", "/info/refs?service=git-upload-pack", &auth(), b"");
    let body = response_body(&exchange(&server, &legacy, 4096));
    assert!(body.starts_with(b"001e# service=git-upload-pack\n0000"));
    assert!(body.windows(commit.to_string().len()).any(|b| b == commit.to_string().as_bytes()));
    ...
    let fetch = request(&server, "POST", "/git-upload-pack", &post_headers("upload-pack", fetch.len(), "Git-Protocol: version=2\r\n"), &fetch);
    let body = response_body(&exchange(&server, &fetch, 2));
    let mut decoder = PktLineDecoder::new(WireLimits::default()).unwrap();
    let decoded = decoder.push(&body).unwrap(); decoder.finish().unwrap();
    let mut selected = Vec::new();
    for packet in decoded {
        if let Packet::Data(bytes) = packet {
            if bytes.first() == Some(&1) { selected.extend_from_slice(&bytes[1..]); }
        }
    }
    let received = fgit_pack::read_verified_pack(&selected, format, &fgit_pack::PackLimits::default(),
        &mut || true, &fgit_pack::NativeChecksumVerifier).expect("HTTP fetch returns a native checksum-verified pack");
    assert_eq!(received.entries().len(), 3);
    assert_eq!(received.entries().iter().map(|entry| entry.inflated.clone()).collect::<BTreeSet<_>>(), payloads);

    let delete = packets(format!("{commit} {zero} refs/heads/main\0report-status delete-refs object-format={}", format.as_str()).into_bytes(), b"");
    let delete = request(&server, "POST", "/git-receive-pack", &post_headers("receive-pack", delete.len(), "Idempotency-Key: http-delete-1\r\n"), &delete);
...
#[test]
fn sha1_push_fetch_delete_and_retry_cross_the_real_http_listener() { round_trip(GitHashAlgorithm::Sha1); }
#[test]
fn sha256_push_fetch_delete_and_retry_cross_the_real_http_listener() { round_trip(GitHashAlgorithm::Sha256); }

#[test]
fn authentication_and_framing_fail_before_continue_or_publication() {
    let scratch = Scratch::new(); let config = config(&scratch, GitHashAlgorithm::Sha1);
    let server = Server::start(initialized(config.clone()), 6, true, 2);
    let cases = [
        (request(&server, "GET", "/info/refs?service=git-upload-pack", "", b""), "401"),
        (request(&server, "GET", "/info/refs?service=git-upload-pack", "X-Forwarded-User: admin\r\nX-Forwarded-Proto: https\r\n", b""), "401"),
        (request(&server, "GET", "/info/refs?service=git-upload-pack", &format!("{}{}", auth(), auth()), b""), "400"),
        (request(&server, "POST", "/git-receive-pack", &post_headers("receive-pack", 4, "Expect: 100-continue\r\n"), b""), "400"),
        (request(&server, "POST", "/git-receive-pack", &post_headers("receive-pack", 128 * 1024 * 1024 + 1, "Idempotency-Key: oversized\r\nExpect: 100-continue\r\n"), b""), "413"),
        (request(&server, "POST", "/git-receive-pack", &post_headers("receive-pack", 4, "Transfer-Encoding: chunked\r\nIdempotency-Key: ambiguous\r\nExpect: 100-continue\r\n"), b""), "400"),
    ];
    ...
    let receipt = server.finish(); assert_eq!(receipt.refused_sessions(), 6);
...
fn readonly_service_accepts_fetch_discovery_but_not_receive_routes() {
    ...
    let fetch = request(&server, "GET", "/info/refs?service=git-upload-pack", &auth(), b"");
    response_body(&exchange(&server, &fetch, 4096));
    for request in [
        request(&server, "GET", "/info/refs?service=git-receive-pack", &auth(), b""),
        request(&server, "POST", "/git-receive-pack", &post_headers("receive-pack", 4, "Idempotency-Key: denied\r\n"), b"0000"),
    ] {
        assert!(exchange(&server, &request, 4096).starts_with(b"HTTP/1.1 403"));
    }
    let receipt = server.finish(); assert_eq!(receipt.completed_sessions(), 1); assert_eq!(receipt.refused_sessions(), 2);
    assert_eq!(generation(&config, None), HeadGeneration::FIRST);
}

#[test]
fn an_idle_service_exits_with_no_accepted_children() {
    let scratch = Scratch::new(); let node = initialized(config(&scratch, GitHashAlgorithm::Sha1));
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    let result = node.serve_smart_http_bounded(&listener, GitDaemonServerLimits::try_new(10, 2).unwrap(),
        sha256_digest(TOKEN.as_bytes()), PrincipalId::from_bytes([0xc3; 16]), false, Duration::from_millis(25));
    let cleanup = node.shutdown();
    let receipt = result.unwrap(); cleanup.unwrap();
    assert_eq!(receipt.accepted_sessions(), 0); assert_eq!(receipt.completed_sessions(), 0); assert_eq!(receipt.refused_sessions(), 0);
}
...
fn a_partial_header_expires_and_releases_the_only_worker_slot() {
    ...
    let config = config(&scratch, GitHashAlgorithm::Sha1)
        .with_git_daemon_session_timeout(GitDaemonSessionTimeout::try_new(Duration::from_millis(100)).unwrap());
    let server = Server::start(initialized(config), 2, false, 1);
    let mut stalled = client(server.address);
    stalled.write_all(b"GET ").unwrap();
    let second = request(&server, "GET", "/info/refs?service=git-upload-pack", "", b"");
    assert!(exchange(&server, &second, 4096).starts_with(b"HTTP/1.1 401"));
    let mut response = Vec::new(); stalled.read_to_end(&mut response).unwrap(); drop(stalled);
    assert!(response.starts_with(b"HTTP/1.1 408"));
    let receipt = server.finish(); assert_eq!(receipt.refused_sessions(), 2);
}
