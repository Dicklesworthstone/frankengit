...
use fgit_authority::{
    ExpectedOld, IdempotencyKey, ProposedNew, RefCommand, key_recovery::RequestRecovery,
};
...
fn pr_fixture(root: &Scratch, format: GitHashAlgorithm) -> (OneNode, PullRequestData) {
    ...
    let data = PullRequestData {
        ...
        target_ref: RefName::try_new(b"refs/heads/base").unwrap(),
        source_tip: main,
        target_tip: base,
        title: "Review original bytes".into(),
        body: "metadata is not source authorization".into(),
    };
...
fn grants(node: &OneNode, path: &Path) -> String {
    ...
    replace(
        ...
        &(header.clone()
            + &row('a', OWNER, "read")
            + &row('b', OWNER, "pulls-read")
            + &row('d', OWNER, "read,pulls-read")
            + &row('e', OWNER, "read,pulls-write")),
    );
    header
}
struct PullServer {
    ...
    fn start(node: OneNode, path: &Path, count: usize, source: bool) -> Self {
        ...
        let worker = thread::spawn(move || {
            ...
            let result = if source {
                node.serve_repository_http_with_source_bounded(
                    &listener,
                    limits,
                    &path,
                    false,
                ...
                node.serve_repository_http_with_pull_requests_bounded(
                    &listener,
                    limits,
                    &path,
                    false,
...
fn diff(client: &Endpoint, number: u64, token: char, form: &str, chunked: bool) -> Reply {
    ...
    exchange(
        ...
        &request(
            client,
            &format!("/api/v1/pulls/{number}/diff"),
            token,
...
fn no_read_transaction(node: &OneNode) {
    for key in [
        b"read-only-pull-request-diff".as_slice(),
        b"read-only-source-query".as_slice(),
    ] {
        assert!(matches!(
            node.runtime()
                .block_on(node.recover_transaction_in(&node.request_context(), &session(key)))
                .unwrap(),
...
fn recorded_pr_diff_survives_close_branch_advance_delete_and_restart_in_both_formats() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (node, data) = pr_fixture(&root, format);
        ...
        let original = diff(&server.client, 1, 'd', &query, false);
        status(&original, 200); // 1
        assert_eq!(text(&original.body, "mode"), "merge-base");
        assert!(
            original
                .body
                .contains("\"pull_request\":{\"number\":1,\"version\":1}")
        );
        assert_eq!(
            text(&original.body, "requested_before"),
            data.target_tip.to_string()
        );
        assert_eq!(
            text(&original.body, "requested_after"),
            data.source_tip.to_string()
        );
        assert_eq!(number(&original.body, "entry_count"), 8);
        assert!(
            original
                .body
                .contains(&format!("\"after_hex\":\"{}\"", hex(TEXT)))
        );
        let pinned = format!(
            "{query}&expected_version=1&expected_head={}",
            token(&original)
        );
        ...
        let direct = diff(
            ...
            1,
            'd',
            &(query.clone() + "&mode=direct"),
            false,
        ); // 3
        status(&direct, 200);
        assert_eq!(
            text(&direct.body, "compared_before"),
            data.target_tip.to_string()
        );
        server.finish();
        let node = reopen(&config);
        assert_eq!(generation(&node), before);
        no_read_transaction(&node);
        let close = PullRequestCommand {
            number: PullRequestNumber::FIRST,
        ...
        branch(
            &node,
            b"refs/heads/base",
            ExpectedOld::Exactly(data.target_tip),
            ProposedNew::Update(data.source_tip),
            b"pr-diff-advance",
        );
        branch(
            &node,
            b"refs/heads/topic",
            ExpectedOld::Exactly(data.source_tip),
            ProposedNew::Delete,
            b"pr-diff-delete",
        );
        assert_eq!(generation(&node), before + 3);
        let server = PullServer::start(node, &path, 8, true);
        let stale = diff(&server.client, 1, 'd', &pinned, false);
        status(&stale, 409); // 1
        assert!(stale.body.contains("source_snapshot_moved"));
        let stale_version = diff(
            &server.client,
            1,
            'd',
            &(query.clone() + "&expected_version=1"),
            false,
        ); // 2
        status(&stale_version, 409);
        assert!(stale_version.body.contains("pull_request_version_moved"));
        let current_query = query.clone() + "&expected_version=2";
        let current = diff(&server.client, 1, 'd', &current_query, true);
        status(&current, 200); // 3
        ...
        let live_refs = format!(
            "object_format={}&before_ref=refs/heads/base&after_ref=refs/heads/main",
            format.as_str()
        );
        let live_diff = post(&server.client, "diff", 'd', &live_refs, false);
        status(&live_diff, 200); // 4
        assert_eq!(
            number(&live_diff.body, "entry_count"),
            0,
            "live refs moved; PR tips must not float"
        );
        status(
            &post(
                &server.client,
                "diff",
                'd',
                &live_refs.replace("after_ref=refs/heads/main", "after_ref=refs/heads/topic"),
                false,
            ),
            404,
        ); // 5
        status(
            &diff(
                ...
                1,
                'd',
                &format!("{query}&expected_after={}", data.target_tip),
                false,
            ),
            409,
        ); // 6
        ...
        let selected = diff(
            ...
            1,
            'd',
            &(query + "&path_prefix_hex=616c7068612e747874"),
            true,
        ); // 8
        status(&selected, 200);
        assert_eq!(number(&selected.body, "entry_count"), 1);
        server.finish();
        let node = reopen(&config);
        assert_eq!(generation(&node), before + 3);
        no_read_transaction(&node);
...
fn both_grants_are_required_before_body_intake_and_reads_never_create_reviews() {
    ...
    let root = Scratch::new();
    let config = root.config(format);
    let (node, _) = pr_fixture(&root, format);
    let before = generation(&node);
    let path = root.0.join("credentials");
    ...
    status(
        &diff(
            ...
            1,
            'd',
            "object_format=sha1&before_ref=refs/heads/main",
            false,
        ),
        400,
    ); // 5
    let exhausted = diff(
        ...
        1,
        'd',
        "object_format=sha1&max_changes=1",
        true,
    ); // 6
    status(&exhausted, 413);
    assert!(!exhausted.body.contains("\"entries\":[]"));
    status(&diff(&server.client, 1, 'f', query, false), 401); // 7
    let successful = diff(&server.client, 1, 'd', query, false);
    status(&successful, 200); // 8
