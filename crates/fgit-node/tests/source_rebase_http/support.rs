...
mod wire;
pub use wire::{
    BinaryReply, Endpoint, OWNER, Scratch, SourceServer, binary_exchange, configure, encode,
    extract_candidate, field, generation, hex, json, multipart, numeric, post_form, reopen,
    replace, request, row, status,
};
...
pub fn fixture(root: &Scratch, format: GitHashAlgorithm, mode: u8) -> (OneNode, History) {
    ...
    let tree = |a: &[u8], b: &[u8], extra: bool| {
        ...
        for (name, content) in [
            (b"a\xff".as_slice(), a),
            (b"b".as_slice(), b),
            (b"onto".as_slice(), b"onto-only\n".as_slice()),
        ] {
            if name == b"onto" && !extra {
                continue;
            }
...
pub fn prepare_form(f: &History) -> String {
    format!(
        "profile=linear-v1&object_format={}&source_ref=refs%2Fheads%2Ftopic&onto_ref=refs%2Fheads%2Fmain&expected_source={}&upstream={}&expected_onto={}&empty=stop&committer=Rebaser+%3Crebaser%40example.invalid%3E&timestamp=9",
...
pub fn apply_form(f: &History, candidate: GitOid) -> String {
    format!(
        "profile=linear-v1&object_format={}&ref=refs%2Fheads%2Ftopic&expected_source={}&onto={}&candidate_commit={candidate}",
        f.source.algorithm().as_str(),
        f.source,
        f.onto
    )
