...
fn native_pr_lifecycle_keeps_retained_pages_retry_outcomes_and_git_refs_after_restart() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            post(client, 7, "open", 'b', "open-seven", &form(&data, 0), true),
            original
        ); // 2
        committed(&post(
            client,
        ...
        let next = get(
            client,
            &format!("/api/v1/pulls?after=7&limit=1&expected_head={snapshot}"),
            'a',
        ); // 10
        status(&next, 200);
        ...
        let old = get(
            client,
            &format!("/api/v1/pulls/7?expected_head={snapshot}"),
            'a',
        ); // 11
        status(&old, 200);
        ...
        assert!(
            !missing.body.contains("source_ref"),
            "a missing-number lookup must not disclose the next visible PR"
        );
        let closed = post(
            client,
            7,
        ...
        assert_eq!(
            generation(&node),
            before + 6,
            "five PR commits and one canonical refusal, no retry decisions"
        );
        let selected = node
            .runtime()
            .block_on(node.materialize_admission())
            .unwrap();
        assert_eq!(
            selected.snapshot().refs,
            refs,
            "metadata operations cannot change either branch"
        );
        assert_eq!(
            selected.snapshot().outbox.len(),
            outbox + 5,
            "each committed PR event has its delivery obligation"
        );
...
fn withheld(client: &Endpoint, token: char, key: bool, length: usize) -> Reply {
    ...
    let result = exchange(
        client,
        &request(client, "POST", "/api/v1/pulls/1/open", token, &headers, &[]),
        false,
    );
...
fn unrelated_scopes_forged_identity_and_incomplete_bodies_cannot_publish_pr_state() {
    ...
    status(
        &post(client, 1, "open", 'b', "bad-text", &invalid_utf8, false),
        400,
    ); // 10
    let duplicate = post(
        client,
    ...
    let wrong_route = format!(
        "GET /other.git/api/v1/pulls HTTP/1.1\r\nHost: local\r\nAuthorization: Bearer {}\r\n\r\n",
        "a".repeat(64)
    );
...
fn a_lost_pr_reply_is_recovered_after_restart_and_write_revocation_without_reexecution() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let session = LoopbackReceiveSession::authenticated(
            OWNER,
            IdempotencyKey::new(b"lost-pr-reply".to_vec()).unwrap(),
        );
        let known = node
            .runtime()
        ...
        let fgit_authority::key_recovery::RequestRecovery::Recovered(known) = known else {
            panic!("real PR seal must exist")
        };
        let OutcomeLookup::Decided(terminal) = known.outcome() else {
            panic!("real PR must be terminal")
        };
        assert!(matches!(
            terminal.outcome,
        ...
        replace(
            &path,
            &(header + &row('f', OWNER, "outcomes-read") + &row('e', FOREIGN, "outcomes-read")),
        );
        ...
        assert_eq!(
            generation(&node),
            before + 1,
            "lookups cannot repeat the original PR mutation"
        );
        let page = node
            .runtime()
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for title in ["Left", "Right"] {
            ...
            editors.push(thread::spawn(move || {
                ...
                (
                    title,
                    post(&client, 1, "update", 'b', title, &form(&change, 1), false),
                )
            }));
        }
        barrier.wait();
        let replies: Vec<_> = editors
            .into_iter()
        ...
        assert_eq!(
            generation(&node),
            before + 3,
            "open, winning edit, refused competitor; no retry decisions"
        );
        let page = node
            .runtime()
            .block_on(node.read_pull_requests_in(
