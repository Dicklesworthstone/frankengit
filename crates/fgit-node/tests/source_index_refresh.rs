...
use fgit_graph::lexical::{
    IndexedLexicalReport, LexicalChannel, LexicalQuery, LexicalReadLimits, LexicalRefreshStats,
    LexicalSource,
};
...
fn refresh(
    node: &OneNode,
    predecessor: &GenerationActivation,
    limits: SearchLimits,
...
fn edit(node: &OneNode, base: GitOid, patch: &[u8], key: &[u8]) -> GitOid {
    ...
    let metadata = MergeMetadata {
        ...
        timestamp: 2,
        message: b"index refresh fixture\n".to_vec(),
    };
    let candidate = node
        .runtime()
    ...
    assert!(
        terminal
            .commands
            .iter()
            .all(|r| matches!(r.terminal.outcome, DecisionOutcome::Committed { .. }))
    );
    candidate.candidate_commit
}
...
fn same_tree_reuses_every_blob_and_survives_reopen_without_repository_publication() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let old = query(&node, LexicalChannel::Content, b"needle");
        let (source, second, stats) = refresh(
            &node,
            &first,
            SearchLimits {
                max_matches: 1,
        ...
        assert_eq!(stats.prior_documents_not_reused, 0);
        ...
        assert_eq!(after.results, old.results);
        assert_eq!(after.generation, second);
        assert_eq!(after.non_regular_entries, 2);
        assert_eq!(generation(&node), before);
        node.shutdown().unwrap();
        let node = reopen(&config);
        assert_eq!(
            query(&node, LexicalChannel::Content, b"needle").results,
            after.results
        );
        let third = refresh(&node, &second, Default::default()).unwrap();
        assert_eq!(third.2.reused_documents, 5);
        assert_eq!(third.2.rebuilt_documents, 0);
        assert_eq!(generation(&node), before);
        node.shutdown().unwrap();
    }
}
...
fn actual_insert_delete_replace_rename_refresh_matches_full_rebuild_in_both_formats() {
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let first = build(&node, None);
        let commit = edit(&node, base, patch, b"refresh-source-edit");
        let before = generation(&node);
        ...
        assert!(
            query(&node, LexicalChannel::Path, b"empty")
                .results
                .hits
                .is_empty()
        );
        assert!(
            query(&node, LexicalChannel::Path, b"dir")
                .results
                .hits
                .is_empty()
        );
        assert_eq!(
            query(&node, LexicalChannel::Content, b"novel").results.hits[0].document_id,
            1
        );
        let full = build(&node, Some(second.generation_id));
        let rebuilt = query(&node, LexicalChannel::Content, b"needle");
        assert_eq!(rebuilt.generation, full);
        assert_eq!(rebuilt.source, source);
        assert_eq!(rebuilt.results, indexed.results);
        assert_eq!(rebuilt.indexed_documents, 5);
        assert_eq!(rebuilt.non_regular_entries, 2);
        assert_eq!(generation(&node), before);
        node.shutdown().unwrap();
    }
}
...
fn refresh_after_forge_only_write_restores_http_queries_without_reading_source_blobs() {
    ...
    let (source, second, stats) = refresh(&node, &first, Default::default()).unwrap();
    assert_eq!(source.commit, commit);
    assert_eq!(source.tree, original.source.tree);
    assert_ne!(source.source_head, original.source.source_head);
    assert_eq!(stats.rebuilt_source_bytes, 0);
    assert_eq!(stats.reused_documents, 5);
    assert_eq!(generation(&node), before);
    ...
    let response = post(
        ...
        "search-index",
        'a',
        &(common(GitHashAlgorithm::Sha1) + "&term_hex=6e6565646c65"),
        true,
    );
    status(&response, 200);
    assert_eq!(
        number(&response.body, "index_number"),
...
fn source_and_index_resource_limits_apply_to_reused_rows_without_partial_publication() {
    ...
    let first = build(&node, None);
    let report = query(&node, LexicalChannel::Content, b"needle");
    let before = generation(&node);
    for limits in [
        SearchLimits {
            max_files: 4,
        ...
        SearchLimits {
            max_total_bytes: report.indexed_source_bytes - 1,
            ..Default::default()
        },
    ] {
        assert!(refresh(&node, &first, limits).is_err());
        assert_eq!(
            query(&node, LexicalChannel::Content, b"needle").generation,
            first
        );
    }
    assert!(
        node.runtime()
            .block_on(node.refresh_source_index_local_in(
                &node.request_context(),
                &reference(),
                None,
                None,
    ...
    let exact = refresh(
        ...
        &first,
        SearchLimits {
            max_total_bytes: report.indexed_source_bytes,
            ..Default::default()
        },
    )
    .unwrap();
...
fn bad_source_pins_missing_refs_stale_predecessors_and_cancelled_requests_do_not_refresh() {
    ...
    let context = node.request_context();
    drop(node.refresh_source_index_local_in(
        &context,
        &reference(),
        None,
        None,
...
fn refreshed_generation_continuation_and_original_candidate_recovery_remain_exact() {
    ...
    let page = node
        ...
        .block_on(node.search_source_index_local_in(
            ...
            &q,
            None,
            fgit_graph::lexical::LexicalQueryLimits {
                max_results: 2,
                ..Default::default()
            },
            Default::default(),
    ...
    let continued = node
        ...
        .unwrap();
    assert_eq!(continued.generation, first);
    assert_eq!(continued.selected_generation_head, second);
    assert_eq!(
        continued
            .results
            .hits
    ...
    let observed = node
        ...
        .unwrap();
    assert!(
        matches!(observed, GenerationRecovery::Superseded { activation, .. } if activation == second)
    );
    node.shutdown().unwrap();
}
...
fn mode_only_changes_reuse_postings_but_rename_is_a_fresh_path() {
    ...
    edit(
        &node,
        commit,
        b"diff --git a/run b/renamed\nsimilarity index 100%\nrename from run\nrename to renamed\n",
        b"refresh-rename",
    );
    let (_, _, stats) = refresh(&node, &second, Default::default()).unwrap();
    assert_eq!(stats.reused_documents, 4);
    assert_eq!(stats.rebuilt_documents, 1);
    assert!(
        query(&node, LexicalChannel::Path, b"run")
            .results
            .hits
            .is_empty()
    );
    assert_eq!(
        query(&node, LexicalChannel::Path, b"renamed")
            .results
            .hits
            .len(),
        1
