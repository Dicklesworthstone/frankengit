...
use fgit_types::{
    DecisionOutcome, GitHashAlgorithm, GitOid, PrincipalId, RefName, RepositoryAuthorityHeadId,
...
impl Fixture {
    pub fn new(format: GitHashAlgorithm, workflow_text: &str) -> Self {
        ...
        let base = loose(
            ...
            &commit_bytes(tree(b"base\n", b"base\n"), &[], "base"),
        );
        let target = loose(
            &path,
            format,
            GitObjectKind::Commit,
            &commit_bytes(tree(b"target\n", b"base\n"), &[base], "target"),
        );
        let source = loose(
            &path,
            format,
    ...
    pub fn prepared(&self) -> (NativeMerge, Vec<u8>) {
        let prepared = self
            .node()
            .runtime()
            .block_on(self.node().prepare_merge_bundle_in(
                &self.node().request_context(),
                &target_ref(),
                &source_ref(),
                &Default::default(),
                &MergeMetadata {
                    author: "Test <t@example.invalid>".into(),
                    committer: "Test <t@example.invalid>".into(),
                    timestamp: 9,
                    message: b"merge candidate\n".to_vec(),
                },
                Default::default(),
            ))
            .unwrap();
...
pub fn target_ref() -> RefName {
    RefName::try_new(b"refs/heads/main").unwrap()
}
pub fn source_ref() -> RefName {
