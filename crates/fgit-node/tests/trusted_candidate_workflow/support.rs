...
impl Fixture {
    pub fn new(format: GitHashAlgorithm, workflow: &str) -> Self {
        let root = loop {
            let path = std::env::temp_dir().join(format!("fg-candidate-workflow-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
        ...
        let tree = loose(&source, format, GitObjectKind::Tree, &[b"100644 input.txt\0".as_slice(), input.as_bytes(),
            b"100644 workflow.yml\0", workflow.as_bytes()].concat());
        ...
        let result = node.runtime().block_on(node.import_loose_git_directory_durable_in(&node.request_context(),
            &source, PRINCIPAL, b"candidate-workflow-fixture")).unwrap();
    ...
    pub fn replace_file(&self, path: &str, before: &str, after: &str) -> WorkspacePatchCandidate {
        ...
        for line in before.lines() { patch.push('-'); patch.push_str(line); patch.push('\n'); }
        for line in after.lines() { patch.push('+'); patch.push_str(line); patch.push('\n'); }
        let metadata = MergeMetadata { author: "Fixture <fixture@example.invalid>".into(),
            committer: "Fixture <fixture@example.invalid>".into(), timestamp: 2, message: b"candidate workflow inputs\n".to_vec() };
        self.node().runtime().block_on(self.node().prepare_trusted_patch_in(&self.node().request_context(),
            &reference(), self.tip, [7; 16], patch.as_bytes(), &metadata, Default::default())).unwrap()
    }
}
...
pub fn reference() -> RefName { RefName::try_new(b"refs/heads/main").unwrap() }
pub fn inputs() -> Vec<Vec<u8>> { vec![b"workflow.yml".to_vec(), b"input.txt".to_vec()] }
fn loose(root: &Path, format: GitHashAlgorithm, kind: GitObjectKind, body: &[u8]) -> GitOid {
    let id = git_object_id(format, kind, body);
    let bytes = [format!("{} {}\0", kind.label(), body.len()).as_bytes(), body].concat();
    let n = u16::try_from(bytes.len()).unwrap(); let mut z = vec![0x78, 0x01, 0x01];
    z.extend(n.to_le_bytes()); z.extend((!n).to_le_bytes()); z.extend(&bytes);
    let (a, b) = bytes.iter().fold((1_u32, 0_u32), |(a, b), byte| { let a = (a + u32::from(*byte)) % 65521; (a, (b+a) % 65521) });
