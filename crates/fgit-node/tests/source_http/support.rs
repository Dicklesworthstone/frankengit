...
pub fn fixture(root: &Scratch, format: GitHashAlgorithm) -> (OneNode, GitOid) {
    ...
    let nested = tree(
        &[
            b"100644 nested.txt\0".as_slice(),
            blob(b"needle in nested\n").as_bytes(),
        ]
        .concat(),
    );
    ...
    for (mode, name, id) in [
        ...
        entries.extend_from_slice(mode.as_bytes());
        entries.push(b' ');
        entries.extend_from_slice(name);
        entries.push(0);
        entries.extend_from_slice(id.as_bytes());
    }
    ...
    fs::write(
        ...
        b"must never be read through a repository link",
    )
    .unwrap();
    let imported = node
        .runtime()
    ...
    replace(
        ...
        &(header.clone()
            ...
            + &row(
                'b',
                FOREIGN,
                "receive,issues-read,issues-write,outcomes-read,pulls-read,pulls-write,reviews-read,reviews-write,merges-write",
            )),
    );
    header
}
...
pub fn post(client: &Endpoint, operation: &str, token: char, form: &str, chunked: bool) -> Reply {
    ...
    exchange(
        ...
        &request(
            client,
            &format!("/api/v1/source/{operation}"),
            token,
...
pub fn common(format: GitHashAlgorithm) -> String {
    format!("object_format={}&ref=refs/heads/main", format.as_str())
}
pub fn status(reply: &Reply, expected: u16) {
...
pub fn token(reply: &Reply) -> String {
    text(&reply.body, "snapshot_token").to_owned()
}
