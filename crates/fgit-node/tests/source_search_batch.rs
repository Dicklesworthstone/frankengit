...
fn local(
    ...
    node.runtime()
        .block_on(node.search_source_batch_snapshot_local_in(
...
fn batch_native_matches_individual_queries_with_one_shared_read_budget_in_both_formats() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let exact = SearchLimits {
            max_total_bytes: batch.bytes_read,
            ..SearchLimits::default()
        };
        ...
        assert!(
            repeated
                .results
                .iter()
                .all(|result| result.matches == batch.results[0].matches)
        );
        assert!(
            node.runtime()
                .block_on(node.search_source_batch_snapshot_local_in(
                    ...
                    &repeats,
                    SearchLimits {
                        max_total_bytes: batch.bytes_read - 1,
                        ..exact
                    }
                ))
                .is_err()
        );
        let repeated_batch = node
            .runtime()
            .block_on(node.search_source_batch_snapshot_local_in(
                &node.request_context(),
                &reference(),
...
fn scoped<A: NativeFormat>(
    ...
    node.runtime().block_on(node.search_source_batch_in::<A>(
        &node.request_context(),
        &reference(),
        visibility,
        capability,
...
fn capability(repository: RepositoryId) -> TreeCapability {
    TreeCapability::new(
        WorkspaceId::from_bytes([0x95; 16]),
        repository,
...
fn batch_native_capabilities_and_prefixes_never_widen_disclosure() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(
            report.results[1..]
                .iter()
                .all(|result| result.matches.is_empty()
                    && result.completion == SearchCompletion::Complete)
        );
        assert!(
            run(
                &mut capability(RepositoryId::from_bytes([0x99; 16])),
                &RefVisibility::new()
            )
            .is_err()
        );
        let mut revoked = capability(repository);
        revoked.revoke();
        ...
        let narrowed = local(
            &node,
            &queries(&[b"needle", b"aba"], &[b"dir".to_vec()]),
            SearchLimits::default(),
        );
        ...
        let absent = local(
            &node,
            &queries(&[b"needle", b"absent"], &[b"di".to_vec()]),
            SearchLimits::default(),
        );
        assert_eq!(absent.files_selected, 0);
        assert_eq!(absent.bytes_read, 0);
        assert!(
            absent.results.iter().all(|result| result.matches.is_empty()
                && result.completion == SearchCompletion::Complete)
        );
        node.shutdown().unwrap();
    }
...
fn batch_native_lookahead_is_per_query_and_snapshot_checks_precede_answers() {
    ...
    assert!(
        matches!(result, Err(NodeWorkspaceRefusal::SourceBrowse(error)) if matches!(*error, SourceBrowseError::CommitMoved))
    );
...
fn batch_form(format: GitHashAlgorithm) -> String {
    format!(
        "{}&needle_hex={}&needle_hex={}&needle_hex={}&case=ascii-insensitive",
        common(format),
        hex(b"needle"),
        hex(b"aba"),
        hex(b"absent")
...
fn result_segment(body: &str, index: usize) -> &str {
    body.split_once(&format!("{{\"query_index\":{index},"))
        .unwrap()
        .1
        .split("{\"query_index\":")
        .next()
...
fn batch_http_uses_real_listener_lossless_results_shared_counters_and_reopen() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            number(result_segment(&first.body, 0), "returned_matches"),
            5
        );
        assert_eq!(
            number(result_segment(&first.body, 1), "returned_matches"),
            2
        );
        assert_eq!(
            number(result_segment(&first.body, 2), "returned_matches"),
            0
        );
        assert!(result_segment(&first.body, 0).contains(&hex(BINARY_PATH)));
        assert!(
            result_segment(&first.body, 1)
        ...
        status(&chunked, 200);
        assert_eq!(chunked, first);
        let single = post(
            &server.client,
            "search",
            'a',
            &format!(
                "{}&needle_hex={}&case=ascii-insensitive",
                common(format),
                hex(b"needle")
            ),
            false,
        ...
        status(&single, 200);
        assert_eq!(
            number(&single.body, "bytes_read"),
            number(&first.body, "bytes_read")
        );
        let limited = post(
            &server.client,
            "search-batch",
        ...
        status(&limited, 200);
        assert_eq!(
            text(result_segment(&limited.body, 0), "completion"),
            "match_limit"
        );
        assert_eq!(
            text(result_segment(&limited.body, 1), "completion"),
            "complete"
        );
        assert_eq!(
            text(result_segment(&limited.body, 2), "completion"),
            "complete"
        );
        let refused = post(
            &server.client,
            "search-batch",
            'a',
            &(form.clone() + "&max_bytes=1"),
            false,
        ); // 5
        status(&refused, 413);
        assert!(!refused.body.contains("\"results\""));
        ...
        assert_eq!(
            post(&server.client, "search-batch", 'a', &pinned, false),
            first
        );
...
fn batch_http_read_scope_and_no_transaction_boundary_are_enforced_before_disclosure() {
    ...
    for (token, code) in [('b', 403), ('c', 403), ('f', 401)] {
        ...
        assert!(
            !response.body.contains("\"source_commit\"") && !response.body.contains("\"results\"")
        );
    }
    let headers = format!(
...
fn batch_http_pinned_head_refuses_intervening_canonical_mutation() {
    ...
    let stale = post(
        ...
        "search-batch",
        'a',
        &format!("{form}&expected_head={}", token(&first)),
        false,
    ); // 3
    status(&stale, 409);
    ...
    status(&current, 200);
    assert_ne!(token(&current), token(&first));
