...
fn tree_and_file_pages_preserve_raw_bytes_modes_and_pins_after_reopen() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let next = post(
            ...
            'a',
            &format!(
                "{query}&limit=2&after_hex={}&expected_head={pin}",
                hex(BINARY_PATH)
            ),
            true,
        ); // 2
        status(&next, 200);
        assert_eq!(token(&next), pin);
        ...
        status(&binary, 200);
        assert_eq!(text(&binary.body, "content_hex"), hex(BINARY));
        assert!(binary.body.contains("\"next_offset\":null"));
        let slice = post(
            &server.client,
            "blob",
            'a',
            &(binary_query.clone() + "&limit=3"),
            false,
        ); // 4
        status(&slice, 200);
        assert_eq!(text(&slice.body, "content_hex"), hex(&BINARY[..3]));
        assert_eq!(number(&slice.body, "next_offset"), 3);
        let slice = post(
            &server.client,
            "blob",
            'a',
            &format!("{binary_query}&offset=3&limit=3&expected_head={pin}"),
            true,
        ); // 5
        status(&slice, 200);
        assert_eq!(text(&slice.body, "content_hex"), hex(&BINARY[3..6]));
        let eof = post(
            &server.client,
            "blob",
            'a',
            &format!(
                "{binary_query}&offset={}&limit=1&expected_head={pin}",
                BINARY.len()
            ),
            false,
        ); // 6
        status(&eof, 200);
        assert_eq!(number(&eof.body, "returned_bytes"), 0);
        ...
        status(
            &post(
                ...
                'a',
                &format!(
                    "{binary_query}&offset={}&expected_head={pin}",
                    BINARY.len() + 1
                ),
                false,
            ),
        ...
        let empty = post(
            ...
            "blob",
            'a',
            &(query.clone() + "&path_hex=656d707479"),
            false,
        ); // 8
        status(&empty, 200);
        assert_eq!(number(&empty.body, "total_bytes"), 0);
        let link = post(
            &server.client,
            "blob",
            'a',
            &(query.clone() + "&path_hex=6c696e6b"),
            false,
        ); // 9
        status(&link, 200);
        assert_eq!(text(&link.body, "content_hex"), hex(LINK));
        assert_eq!(text(&link.body, "kind"), "symlink");
        let traversal = post(
            &server.client,
            "blob",
            'a',
            &(query.clone() + "&path_hex=" + &hex(b"link/secret")),
            false,
        ); // 10
        status(&traversal, 409);
        assert!(traversal.body.contains("symlink_not_followed"));
        let nested = post(
            &server.client,
            "tree",
            'a',
            &(query.clone() + "&path_hex=646972"),
            false,
        ); // 11
        status(&nested, 200);
        assert!(nested.body.contains(&hex(b"nested.txt")));
        let executable = post(
            &server.client,
            "blob",
            'a',
            &(query.clone() + "&path_hex=72756e"),
            false,
        ); // 12
        status(&executable, 200);
        assert_eq!(text(&executable.body, "kind"), "executable");
        status(
            &post(
                &server.client,
                "blob",
                'a',
                &(query.clone() + "&path_hex=6d6f64756c65"),
                false,
            ),
            409,
        ); // 13
        status(
            &post(
                ...
                "blob",
                'a',
                &(query.clone() + "&path_hex=616273656e74"),
                false,
            ),
            404,
        ); // 14
        ...
        assert_eq!(
            post(&server.client, "tree", 'a', &(query + "&limit=2"), false),
            first
        );
        assert_eq!(
            post(&server.client, "blob", 'a', &binary_query, false),
            binary
        );
...
fn literal_search_reports_overlap_binary_content_component_prefixes_and_real_truncation() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        status(&complete, 200);
        assert_eq!(number(&complete.body, "returned_matches"), 4);
        ...
        let limited = post(
            ...
            "search",
            'a',
            &(needle.clone() + "&max_matches=1"),
            true,
        ); // 2
        status(&limited, 200);
        assert_eq!(number(&limited.body, "returned_matches"), 1);
        assert!(
            limited
                .body
                .contains("\"completion\":\"match_limit\",\"complete\":false")
        );
        let exact_bound = post(
            &server.client,
            "search",
            'a',
            &(needle.clone() + "&max_matches=4"),
            false,
        ); // 3
        status(&exact_bound, 200);
        assert_eq!(text(&exact_bound.body, "completion"), "complete");
        let folded = post(
            &server.client,
            "search",
            'a',
            &(needle.clone() + "&case=ascii-insensitive"),
            false,
        ); // 4
        status(&folded, 200);
        assert_eq!(number(&folded.body, "returned_matches"), 5);
        let overlap = post(
            &server.client,
            "search",
            'a',
            &format!("{query}&needle_hex=616261"),
            false,
        ); // 5
        status(&overlap, 200);
        assert_eq!(number(&overlap.body, "returned_matches"), 2);
        assert!(
            overlap
                .body
                .contains("\"byte_offset\":15,\"line\":2,\"byte_column\":1")
        );
        assert!(
            overlap
                .body
                .contains("\"byte_offset\":17,\"line\":2,\"byte_column\":3")
        );
        let binary = post(
            &server.client,
            "search",
            'a',
            &format!("{query}&needle_hex=00ff"),
            true,
        ); // 6
        status(&binary, 200);
        ...
        assert_eq!(
            text(&binary.body, "excerpt_hex"),
            hex(&BINARY[..BINARY.len() - 1])
        );
        ...
        let prefix = post(
            ...
            "search",
            'a',
            &(needle.clone() + "&path_prefix_hex=646972"),
            false,
        ); // 7
        status(&prefix, 200);
        assert_eq!(number(&prefix.body, "returned_matches"), 1);
        assert_eq!(text(&prefix.body, "path_hex"), hex(b"dir/nested.txt"));
        let not_component = post(
            &server.client,
            "search",
            'a',
            &(needle.clone() + "&path_prefix_hex=6469"),
            false,
        ); // 8
        status(&not_component, 200);
        assert_eq!(number(&not_component.body, "returned_matches"), 0);
        assert_eq!(text(&not_component.body, "completion"), "complete");
        let no_link_following = post(
            &server.client,
            "search",
            'a',
            &format!("{query}&needle_hex={}", hex(LINK)),
            false,
        ); // 9
        status(&no_link_following, 200);
        assert_eq!(number(&no_link_following.body, "returned_matches"), 0);
        let exhausted = post(
            &server.client,
            "search",
            'a',
        ...
        assert!(exhausted.body.contains("\"type\":\"source_error\""));
...
fn current_head_pins_refuse_intervening_writes_and_read_credentials_rotate_after_restart() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let old_search = post(
            ...
            "search",
            'a',
            &format!("{search}&expected_head={old}"),
            false,
        ); // 2
        status(&old_search, 200);
        assert_eq!(token(&old_search), old);
        ...
        status(
            &post(
                ...
                "blob",
                'a',
                &format!("{blob}&limit=2&expected_head={old}"),
                false,
            ),
            200,
        ); // 3
        ...
        for (operation, command) in [
            (
                "tree",
                format!("{query}&after_hex=616c7068612e747874&expected_head={old}"),
            ),
        ...
        status(&current_blob, 200);
        assert_eq!(text(&current_blob.body, "content_hex"), hex(BINARY));
        ...
        replace(
            &path,
            &(header.clone() + &row('9', OWNER, "read") + &row('c', OWNER, "outcomes-read")),
        );
        let server = Server::start(node, &path, 5, true, false);
        status(
            &post(&server.client, "search", 'a', &current_query, false),
            401,
        ); // 1
        assert_eq!(
            post(&server.client, "search", '9', &current_query, false),
            current
        ); // 2
        replace(&path, "malformed\n");
        let unavailable = post(&server.client, "search", '9', &current_query, false); // 3
        status(&unavailable, 503);
        assert!(unavailable.body.contains("\"outcome_unknown\":false"));
        replace(
            &path,
            &(header + &row('9', OWNER, "read") + &row('c', OWNER, "outcomes-read")),
        );
        assert_eq!(
            post(&server.client, "blob", '9', &current_blob_query, true),
            current_blob
        ); // 4
...
fn deployment_credentials_and_complete_envelopes_gate_every_source_read() {
    ...
    status(
        &withheld(
            &enabled.client,
            'a',
            "Idempotency-Key: not-a-mutation\r\n",
            100,
        ),
        400,
    ); // 2
    ...
    let forged = format!(
        "POST {}/api/v1/source/tree HTTP/1.1\r\nHost: local\r\nX-Forwarded-User: owner\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: 100\r\nExpect: 100-continue\r\n\r\n",
    ...
    status(
        &post(
            ...
            "tree",
            'a',
            &(query.clone() + "&ref=refs/heads/other"),
            false,
        ),
        400,
    ); // 5
    ...
    status(
        &post(
            ...
            "blob",
            'a',
            &(query.clone() + "&path_hex=" + &hex(b"../outside-secret")),
            false,
        ),
        400,
    ); // 7
    ...
    status(
        &post(
            ...
            "tree",
            'a',
            &(query.clone() + "&object_id=" + &"a".repeat(64)),
            false,
        ),
        400,
    ); // 10
...
fn native_search_binds_preconditions_and_refuses_cancelled_or_wrong_domain_reads() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (head, report) = node
            .runtime()
            .block_on(node.search_source_snapshot_local_in(
                &request,
                &reference,
                None,
                Some(main),
            ...
            .unwrap();
        assert_eq!(report.source_commit, main);
        ...
        let failure = node
            ...
            .unwrap_err();
        assert!(
            matches!(failure, NodeWorkspaceRefusal::SourceBrowse(error) if matches!(*error, SourceBrowseError::CommitMoved))
        );
