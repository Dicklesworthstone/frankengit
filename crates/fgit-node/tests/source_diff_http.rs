...
fn comparison_fixture(root: &Scratch, format: GitHashAlgorithm) -> (OneNode, GitOid, GitOid) {
    ...
    let session = LoopbackReceiveSession::authenticated(
        OWNER,
        IdempotencyKey::new(b"diff-base-branch".to_vec()).unwrap(),
    );
...
fn both_formats_return_exact_changed_entries_context_and_pins_after_restart() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (node, base, main) = comparison_fixture(&root, format);
        ...
        assert!(first.body.contains(&format!(
            "\"kind\":\"binary\",\"before_bytes\":0,\"after_bytes\":{}",
            BINARY.len()
        )));
        ...
        let pin = token(&first);
        let pinned =
            format!("{query}&expected_head={pin}&expected_before={base}&expected_after={main}");
        assert_eq!(post(&server.client, "diff", 'a', &pinned, true), first); // 2
        let merge_base = post(
            &server.client,
            "diff",
            'a',
            &(query.clone() + "&mode=merge-base"),
            true,
        ); // 3
        status(&merge_base, 200);
        assert_eq!(text(&merge_base.body, "compared_before"), base.to_string());
        let selected = post(
            &server.client,
            "diff",
            'a',
            &(query.clone() + "&path_prefix_hex=646972&context_lines=0"),
            false,
        ); // 4
        status(&selected, 200);
        assert_eq!(number(&selected.body, "entry_count"), 2);
        assert!(!selected.body.contains(&hex(TEXT)));
        assert!(!selected.body.contains(&hex(BINARY_PATH)));
        let empty = post(
            &server.client,
            "diff",
            'a',
            &(query.clone() + "&path_prefix_hex=6469"),
            false,
        ); // 5
        status(&empty, 200);
        assert_eq!(number(&empty.body, "entry_count"), 0);
        let reverse = post(
            &server.client,
            "diff",
            'a',
            &query.replace(
                "before_ref=refs/heads/base&after_ref=refs/heads/main",
                "before_ref=refs/heads/main&after_ref=refs/heads/base",
            ),
            false,
        ); // 6
        status(&reverse, 200);
        assert!(reverse.body.contains("\"kind\":\"deleted\""));
        assert!(
            reverse
                .body
                .contains(&format!("\"before_hex\":\"{}\"", hex(TEXT)))
        );
        let unchanged = post(
            &server.client,
            "diff",
            'a',
            &query.replace("before_ref=refs/heads/base", "before_ref=refs/heads/main"),
            false,
        ); // 7
        status(&unchanged, 200);
        assert_eq!(number(&unchanged.body, "entry_count"), 0);
        status(
            &post(
                &server.client,
                "diff",
                'a',
                &(query.clone() + "&expected_before=" + &main.to_string()),
                false,
            ),
            409,
        ); // 8
        ...
        assert_no_read_transaction(&node);
...
fn credentials_and_budgets_refuse_without_a_successful_partial_diff() {
    ...
    let root = Scratch::new();
    let config = root.config(format);
    let (node, _, _) = comparison_fixture(&root, format);
    let before = generation(&node);
    let path = root.0.join("credentials");
    let header = credentials(&node, &path);
    let server = Server::start(node, &path, 9, true, false);
    let query = query(format);
    for (token, extra, expected) in [
        ('b', "", 403),
        ('f', "", 401),
        ('a', "&max_changes=1", 413),
        ('a', "&max_blob_bytes=1", 413),
        ('a', "&max_output_bytes=1", 413),
        ('a', "&force=true", 400),
    ] {
        let reply = post(
            &server.client,
        ...
        assert!(!reply.body.contains("\"entries\":[]"));
    ...
    status(
        &post(
            ...
            "diff",
            'a',
            &query.replace("refs/heads/base", "refs/heads/absent"),
            false,
        ),
        404,
    ); // 8
...
fn an_intervening_metadata_write_invalidates_the_exact_comparison_snapshot() {
    ...
    let query = query(format);
    let original = post(&server.client, "diff", 'a', &query, false);
    status(&original, 200);
    ...
    let stale = post(
        ...
        "diff",
        'a',
        &format!("{query}&expected_head={}", token(&original)),
        true,
    );
    status(&stale, 409);
    assert!(stale.body.contains("source_snapshot_moved"));
    let fresh = post(&server.client, "diff", 'a', &query, false);
    status(&fresh, 200);
    assert_ne!(token(&fresh), token(&original));
    assert_eq!(
        number(&fresh.body, "entry_count"),
        number(&original.body, "entry_count")
    );
    server.finish();
    let node = reopen(&config);
