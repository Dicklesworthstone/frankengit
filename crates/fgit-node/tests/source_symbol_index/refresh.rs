...
fn edit_files(
    node: &OneNode,
    base: GitOid,
    changes: &[(&[u8], Option<&[u8]>, Option<&[u8]>)],
    key: &[u8],
) -> GitOid {
...
fn additions_and_copies_reuse_blobs_and_match_the_full_rebuild_after_reopen() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let next = add_files(
            &node,
            commit,
            &[(b"copy.rs", RAW), (b"new.rs", ADDED)],
            b"symbol-refresh-add-copy",
        );
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(
            report
                .matches
                .iter()
                .all(|m| m.location.path != RAW_PATH && m.location.path != b"src/a.rs")
        );
        assert!(
            report
                .matches
                .iter()
                .any(|m| m.location.path == b"src/renamed.rs")
        );
...
fn deleting_all_rust_files_publishes_authenticated_empty_not_old_matches() {
    ...
    edit_files(
        ...
        &[
            (b"src/a.rs", Some(A), None),
            (b"src/b.rs", Some(B), None),
            (RAW_PATH, Some(RAW), None),
        ],
        b"symbol-refresh-delete-all",
    );
    let expected = full_candidate(&node, &first);
    let (source, second, stats) = refresh(&node, &first, Default::default()).unwrap();
    assert_eq!(second.generation_id, expected);
...
fn whole_corpus_limits_cannot_be_bypassed_by_a_zero_blob_read_refresh() {
    ...
    let (_, _, stats) = refresh(
        &node,
        &first,
        SearchLimits {
            max_file_bytes: A.len(),
...
fn failed_or_cancelled_refresh_barriers_preserve_the_original_candidate_and_root() {
    ...
    let error = node
        ...
        .unwrap_err();
    assert!(
        matches!(error,AccessError::Publication { candidate,.. } if candidate == *expected.as_internal_object_id())
    );
    assert_eq!(
        search(
            &node,
