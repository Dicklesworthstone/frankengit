...
impl Scratch {
    fn new() -> Self {
        let root = std::env::temp_dir().join(format!("fg-repository-recovery-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
...
fn principal() -> PrincipalId { PrincipalId::from_bytes([3; 16]) }
fn branch(node: &OneNode, key: &[u8], name: &[u8], old: ExpectedOld, new: ProposedNew) {
    let session = LoopbackReceiveSession::authenticated(principal(), IdempotencyKey::new(key.to_vec()).unwrap());
    let command = RefCommand { name: RefName::try_new(name).unwrap(), expected_old: old, proposed_new: new };
    let request = node.request_context();
    let admission = node.runtime().block_on(node.admit_branch_updates_durable_in(&request, &session,
        &[command], Default::default())).unwrap();
    assert_eq!(admission.commands.len(), 1);
    assert!(matches!(admission.commands[0].terminal.outcome, DecisionOutcome::Committed { .. }));
}
fn loose(root: &Path, format: GitHashAlgorithm, kind: GitObjectKind, body: &[u8]) -> GitOid {
    let oid = git_object_id(format, kind, body);
    let text = oid.to_string(); let path = root.join("objects").join(&text[..2]).join(&text[2..]);
    ...
    zlib.extend_from_slice(&count.to_le_bytes()); zlib.extend_from_slice(&(!count).to_le_bytes()); zlib.extend_from_slice(&framed);
    let (mut a, mut b) = (1_u32, 0_u32);
    for byte in framed { a = (a + u32::from(byte)) % 65521; b = (b + a) % 65521; }
    zlib.extend_from_slice(&((b << 16) | a).to_be_bytes()); fs::write(path, zlib).unwrap(); oid
}
fn history(root: &Path, format: GitHashAlgorithm, payload: &[u8]) -> (GitOid, GitOid) {
    ...
    (loose(root, format, GitObjectKind::Commit, commit.as_bytes()), blob)
}
fn fixture(scratch: &Scratch, format: GitHashAlgorithm) -> (PathBuf, PathBuf, GitOid, GitOid, GitOid) {
    let source = scratch.0.join("source.git"); let root = scratch.0.join("node");
    let (main, _) = history(&source, format, b"current content\n");
    let (old, old_blob) = history(&source, format, b"historical deleted-branch content\0\xff\n");
    ...
    let imported = node.runtime().block_on(node.import_loose_git_directory_durable_in(
        &request, &source, principal(), b"recovery-import")).unwrap();
    ...
    branch(&node, b"remove-old", b"refs/heads/obsolete", ExpectedOld::Exactly(old), ProposedNew::Delete);
    let orphan = node.put_git_object(GitObjectKind::Blob, b"physical orphan, not canonical".to_vec()).unwrap().identity();
    node.shutdown().unwrap();
    (root, source, main, old_blob, orphan)
}
fn export(root: &Path, backup: &Path, format: GitHashAlgorithm) -> String {
    success(command(&["export", text(root), text(backup), TENANT, REPOSITORY, "--trusted-local", "--object-format", format.as_str()]))
}
fn restore(backup: &Path, output: &Path, expected: &str) -> Output {
    command(&["restore", text(backup), text(output), "--trusted-local", "--expected-sha256", expected, "--destination-instance", "991"])
}
#[test]
fn backup_alone_restores_both_domains_historical_objects_and_future_publication() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let original = fs::read(&backup).unwrap(); let digest = checksum(&original);
        assert!(exported.contains(&digest));
        refusal(command(&["export", text(&root), text(&backup), TENANT, REPOSITORY, "--trusted-local", "--object-format", format.as_str()]));
        assert_eq!(fs::read(&backup).unwrap(), original, "export never replaces an existing backup");
        // Delete ONLY the two isolated fixture directories: the restore has no source.
        fs::remove_dir_all(&root).unwrap(); fs::remove_dir_all(&source).unwrap();
        ...
        assert_eq!(node.read_git_object(old_blob).unwrap().payload(), b"historical deleted-branch content\0\xff\n");
        assert!(node.read_git_object(orphan).is_err(), "physical residue is not a recovery root");
        branch(&node, b"after-restore", b"refs/heads/recovered", ExpectedOld::Absent, ProposedNew::Update(main));
        node.shutdown().unwrap();
        ...
        assert_eq!(selected.snapshot().refs.len(), 2, "refused restore did not rewind new history");
...
fn checksum_valid_but_incomplete_inventory_cannot_publish_a_destination_head() {
    ...
    let backup = scratch.0.join("complete.fg"); export(&root, &backup, format);
    let bytes = fs::read(&backup).unwrap();
    let authority_length = usize::try_from(u64::from_be_bytes(bytes[57..65].try_into().unwrap())).unwrap();
    let count_at = 65 + authority_length;
    let count = u64::from_be_bytes(bytes[count_at..count_at + 8].try_into().unwrap()); assert_eq!(count, 6);
    let mut cursor = count_at + 8;
    for _ in 0..count - 1 {
        let length = usize::try_from(u64::from_be_bytes(bytes[cursor + 21..cursor + 29].try_into().unwrap())).unwrap();
    ...
    let corrupt = scratch.0.join("incomplete.fg"); fs::write(&corrupt, &incomplete).unwrap();
    ...
    assert!(target.join(".restore-quarantine").exists(), "retain rejected metadata for investigation");
    let wrong_pin_target = scratch.0.join("wrong-pin");
    refusal(restore(&backup, &wrong_pin_target, &"00".repeat(32)));
    assert!(!wrong_pin_target.exists(), "wrong trust pin must precede destination creation");
}
