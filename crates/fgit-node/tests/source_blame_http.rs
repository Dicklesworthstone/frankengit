...
fn blame_returns_exact_requested_lines_and_origin_records_in_both_formats_after_reopen() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let options = BlameOptions { path: b"alpha.txt".to_vec(), first_line: 0, end_line: None,
            limits: HistoryLimits::default() };
        let (_, direct) = node.runtime().block_on(node.blame_source_in(&node.request_context(),
            &RefName::try_new(b"refs/heads/main").unwrap(), &Default::default(), None, &options)).unwrap();
        ...
        assert!(full.body.contains("\"transaction_created\":false,\"published\":false"));
        ...
        let end = post(&server.client, "blame", 'a', &format!("{query}&line_start=2&line_end=2&expected_head={pin}"), false); // 3
        status(&end, 200);
        assert_eq!(text(&end.body, "content_hex"), "");
        assert!(end.body.contains("\"origins\":[],\"lines\":[]"));
        let binary = post(&server.client, "blame", 'a', &format!("{}&path_hex={}", common(format), hex(BINARY_PATH)), false); // 4
        status(&binary, 409);
        assert!(binary.body.contains("binary_blame_unsupported"));
        assert!(!binary.body.contains("content_hex"));
        let link = post(&server.client, "blame", 'a', &format!("{}&path_hex={}", common(format), hex(b"link/secret")), false); // 5
        status(&link, 404);
        assert!(!link.body.contains("outside-secret"));
        let invalid = post(&server.client, "blame", 'a', &format!("{}&path_hex={}", common(format), hex(b"../secret")), false); // 6
        status(&invalid, 400);
        for extra in ["&max_commits=1", "&max_lines=1"] { // 7, 8
            let exhausted = post(&server.client, "blame", 'a', &(query.clone() + extra), false);
        ...
        status(&post(&server.client, "blame", 'a', &query.replacen(format.as_str(), other_format, 1), false), 400); // 9
        status(&post(&server.client, "blame", 'a', &format!("{}&path_hex=616273656e74ff", common(format)), false), 404); // 10
        let moved = post(&server.client, "blame", 'a', &format!("{query}&expected_commit={}", "e".repeat(format.digest_len() * 2)), false); // 11
        status(&moved, 409);
        assert!(moved.body.contains("source_commit_moved"));
        let empty = post(&server.client, "blame", 'a', &format!("{}&path_hex=656d707479", common(format)), false); // 12
        status(&empty, 200);
        ...
        let sentinel = LoopbackReceiveSession::authenticated(OWNER,
            IdempotencyKey::new(b"read-only-source-query".to_vec()).unwrap());
        assert!(matches!(node.runtime().block_on(node.recover_transaction_in(&node.request_context(), &sentinel)).unwrap(), RequestRecovery::KeyNotObserved));
        let server = Server::start(node, &path, 2, true, false);
        assert_eq!(post(&server.client, "blame", 'a', &query, true), full);
        assert_eq!(post(&server.client, "blame", 'a', &range_query, false), range);
...
fn blame_is_read_scoped_before_ingress_and_disabled_without_source_service() {
    ...
    let denied = exchange(&server.client, &request(&server.client, "/api/v1/source/blame", 'b',
    ...
    let key = exchange(&server.client, &request(&server.client, "/api/v1/source/blame", 'a',
        &format!("Content-Type: application/x-www-form-urlencoded\r\nContent-Length: {}\r\nIdempotency-Key: blame-must-not-seal\r\n", query.len()), query.as_bytes()), true); // 5
    status(&key, 400);
...
fn blame_ranges_cannot_mix_snapshots_across_an_intervening_authority_write() {
    ...
    let query = format!("{}&path_hex={}", common(format), hex(b"alpha.txt"));
    let first = post(&server.client, "blame", 'a', &(query.clone() + "&line_end=1"), false); // 1
    status(&first, 200);
    let pin = token(&first);
    let body = b"expected_version=0&title=Intervening+write&body=";
    let write = exchange(&server.client, &request(&server.client, "/api/v1/issues/1/open", 'b',
        &format!("Content-Type: application/x-www-form-urlencoded\r\nContent-Length: {}\r\nIdempotency-Key: blame-pin-race\r\n", body.len()), body), true); // 2
    status(&write, 200);
    let stale = post(&server.client, "blame", 'a', &format!("{query}&line_start=1&expected_head={pin}"), false); // 3
    status(&stale, 409);
    assert!(stale.body.contains("source_snapshot_moved"));
    assert!(!stale.body.contains("content_hex"));
    let current = post(&server.client, "blame", 'a', &format!("{query}&expected_commit={main}"), true); // 4
    status(&current, 200);
    assert_ne!(token(&current), pin);
    assert_eq!(text(&current.body, "content_hex"), hex(TEXT));
    let range = post(&server.client, "blame", 'a', &format!("{query}&line_start=1&expected_head={}", token(&current)), false); // 5
    status(&range, 200);
    assert_eq!(text(&range.body, "content_hex"), hex(b"ababa\n"));
