...
use fgit_forge::source_browse::{
    SourceBrowseAction as Action, SourceBrowseContent as Content, SourceBrowseError as Error,
    SourceBrowseQuery as Query, SourceBrowseReport as Report, SourceEntryKind as Kind,
};
use fgit_node::{LoopbackReceiveSession, NodeConfig, NodeWorkspaceRefusal, OneNode};
use fgit_treefs::{SymlinkPolicy, TreeCapability, TreePath, WorkspaceId};
use fgit_types::{
    ByteCount, DecisionOutcome, GitHashAlgorithm as Format, GitOid, HeadGeneration, PrincipalId,
...
impl Fixture {
    ...
    fn browse(&self, query: &Query) -> Result<Report, NodeWorkspaceRefusal> {
        ...
        let request = node.request_context();
        node.runtime()
            .block_on(node.browse_source_local_in(&request, &reference(), query))
    }
    fn scoped(
        &self,
        query: &Query,
    ...
    ) -> Result<Report, NodeWorkspaceRefusal> {
        let node = self.node();
        let request = node.request_context();
        match self.format {
            Format::Sha1 => node.runtime().block_on(node.browse_source_in::<Sha1>(
                &request,
                &reference(),
                visibility,
                capability,
            ...
            Format::Sha256 => node.runtime().block_on(node.browse_source_in::<Sha256>(
                &request,
                &reference(),
                visibility,
                capability,
...
fn list(path: Option<&[u8]>, limit: u16) -> Query {
    Query {
        path: path.map(<[u8]>::to_vec),
        expected_head: None,
        expected_commit: None,
        action: Action::List { after: None, limit },
    }
}
fn read(path: &[u8], limit: u32) -> Query {
    Query {
        path: Some(path.to_vec()),
        expected_head: None,
        expected_commit: None,
        action: Action::Read { offset: 0, limit },
    }
}
fn cap(repo: RepositoryId) -> TreeCapability {
    TreeCapability::new(
...
fn both_formats_page_exact_raw_names_and_read_binary_without_mutating_authority() {
    for format in [Format::Sha1, Format::Sha256] {
        ...
        let before = f.head();
        let mut query = list(None, 2);
        let mut names = Vec::new();
        loop {
            let result = f.browse(&query).unwrap();
            assert_eq!(result.source_head, before);
            assert_eq!(result.source_commit, f.commit);
            assert_eq!(result.root_tree, f.tree);
            assert_eq!(result.object_id, f.tree);
            ...
            query.expected_head = Some(before);
            query.action = Action::List {
                after: Some(after),
                limit: 2,
            };
        }
        ...
        let nested = f.browse(&list(Some(b"dir"), 10)).unwrap();
        assert_eq!(nested.object_id, f.directory);
        let Content::Directory { entries, .. } = nested.content else {
            panic!()
        };
        assert_eq!(entries.len(), 2);
        let mut query = read(b"bin", 3);
        ...
        loop {
            ...
            assert_eq!(kind, Kind::Executable);
            assert_eq!(total_bytes, 9);
            body.extend(bytes);
            let Some(offset) = next_offset else { break };
            query.expected_head = Some(before);
            query.action = Action::Read { offset, limit: 3 };
        }
        ...
        node.bring_into_service(HeadGeneration::FIRST).unwrap();
        f.node = Some(node);
        assert_eq!(f.browse(&read(b"dir/alpha.txt", 100)).unwrap(), original);
...
fn capability_filtering_precedes_disclosure_and_denied_reads_preserve_budget() {
    for format in [Format::Sha1, Format::Sha256] {
        ...
        assert!(
            f.scoped(&read(b"private", 10), &mut allowed, &visible)
                .is_err()
        );
        assert_eq!(allowed.fetched_bytes(), spent);
        assert!(
            f.scoped(&read(b"dir/alpha.txt", 10), &mut allowed, &visible)
                .is_ok()
        );
        let mut other = cap(RepositoryId::from_bytes([0; 16]));
        assert!(matches!(
            f.scoped(&read(b"dir/alpha.txt", 10), &mut other, &visible),
        ...
        hidden
            ...
            .unwrap();
        assert!(matches!(
            f.scoped(&list(None, 100), &mut cap(REPO), &hidden),
            Err(NodeWorkspaceRefusal::RefUnavailable)
        ));
        let mut expired = cap(REPO).with_expiry(0);
        assert!(f.scoped(&list(None, 100), &mut expired, &visible).is_err());
        let mut revoked = cap(REPO);
        revoked.revoke();
        assert!(f.scoped(&list(None, 100), &mut revoked, &visible).is_err());
        let mut bounded = cap(REPO).with_fetch_budget(ByteCount::try_new("read", 1, 1).unwrap());
        assert!(
            f.scoped(&read(b"dir/alpha.txt", 1), &mut bounded, &visible)
                .is_err()
        );
        let mut one = cap(REPO).with_file_budget(1);
        assert!(
            f.scoped(&read(b"dir/alpha.txt", 1), &mut one, &visible)
                .is_err()
        );
    }
}
...
fn symlinks_are_bytes_gitlinks_are_opaque_and_empty_files_are_complete() {
    for format in [Format::Sha1, Format::Sha256] {
        let f = Fixture::new(format, false);
        let Content::Blob { kind, bytes, .. } = f.browse(&read(b"link", 100)).unwrap().content
        else {
            panic!()
        };
        assert_eq!(kind, Kind::Symlink);
        assert_eq!(bytes, b"/etc/passwd");
        assert!(f.browse(&read(b"link/anything", 100)).is_err());
        assert!(f.browse(&read(b"sub", 100)).is_err());
        assert!(f.browse(&read(b"dir", 100)).is_err());
        assert!(f.browse(&list(Some(b"bin"), 10)).is_err());
        let Content::Blob {
            bytes,
            total_bytes,
            next_offset,
            ..
        } = f.browse(&read(b"empty", 10)).unwrap().content
        else {
            panic!()
        };
        ...
        let mut capability = TreeCapability::new(
            ...
            REPO,
            vec![TreePath::parse_default(b"link").unwrap()],
            vec![],
        )
        .with_symlink_policy(SymlinkPolicy::Refuse);
        assert!(
            f.scoped(&read(b"link", 100), &mut capability, &RefVisibility::new())
                .is_err()
        );
        let empty = Fixture::new(format, true);
        assert!(matches!(empty.browse(&list(None,100)).unwrap().content,
...
fn stale_continuations_wrong_commit_ranges_and_cancelled_contexts_never_return_pages() {
    let f = Fixture::new(Format::Sha1, false);
    let head = f.head();
    let mut query = read(b"bin", 3);
    ...
    assert!(
        matches!(f.browse(&query).unwrap().content,Content::Blob{bytes,next_offset:None,..} if bytes.is_empty())
    );
    query.action = Action::Read {
        offset: 10,
        limit: 10,
    ...
    assert!(
        matches!(f.browse(&query),Err(NodeWorkspaceRefusal::SourceBrowse(e)) if matches!(*e,Error::RangeOutsideFile))
    );
    query.action = Action::Read {
        offset: u64::MAX,
        limit: 10,
    };
    assert!(f.browse(&query).is_err());
    query.expected_head = None;
    assert!(f.browse(&query).is_err());
    query = read(b"bin", 10);
    query.expected_commit = Some(f.tree);
    assert!(
        matches!(f.browse(&query),Err(NodeWorkspaceRefusal::SourceBrowse(e)) if matches!(*e,Error::CommitMoved))
    );
    let node = f.node();
    let request = node.request_context();
    request.cancel();
    assert!(
        node.runtime()
            .block_on(node.browse_source_local_in(&request, &reference(), &read(b"bin", 10)))
            .is_err()
    );
    ...
    query.expected_head = Some(head);
    query.action = Action::List {
        after: Some(b"bin".to_vec()),
        limit: 1,
    };
    assert!(
        matches!(f.browse(&query),Err(NodeWorkspaceRefusal::SourceBrowse(e)) if matches!(*e,Error::SnapshotMoved))
    );
