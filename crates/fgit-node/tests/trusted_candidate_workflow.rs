...
fn unpublished_input_runs_in_fresh_jobs_and_cannot_publish_or_replay_after_restart() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(
            !base_run.succeeded(),
            "base must fail or the candidate test is vacuous"
        );
        ...
        let run = f
            ...
            .block_on(f.node().run_trusted_candidate_workflow_in(
                &f.node().request_context(),
                &reference(),
                (f.tip, candidate.candidate_commit),
        ...
        assert_eq!(run.source_head, head);
        assert_eq!(run.source_commit, f.tip);
        assert_eq!(run.source_rcr, candidate.source_rcr);
        assert_eq!(run.executed_commit(), candidate.candidate_commit);
        assert_eq!(run.executed_tree(), candidate.root_tree);
        assert_eq!(run.workflow_blob, f.workflow);
        let binding = run.candidate.unwrap();
        assert_eq!(
            binding.bundle_sha256,
            sha256_digest(candidate.bundle_bytes())
        );
        ...
        for flag in [
            "\"input_kind\":\"unpublished_candidate\"",
            "\"admitted\":false",
            "\"published\":false",
        ...
        assert!(
            marker.contains(&candidate.candidate_commit.to_string())
                && marker.contains("unpublished_candidate")
        );
...
fn workflow_itself_comes_from_candidate_and_unsupported_candidate_graph_starts_nothing() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            run.workflow_blob,
            git_object_id(format, GitObjectKind::Blob, new_workflow.as_bytes())
        );
        ...
        assert!(
            !f.parent()
                .join(format!("workflow-{}", "04".repeat(16)))
                .exists()
        );
        assert_eq!(generation(f.node()), before);
        assert!(
...
fn actual_bundle_coordinates_scope_and_current_snapshot_precede_any_execution() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let f = Fixture::new(format, SIMPLE);
        let candidate = f.replace_file("input.txt", "original\n", "candidate\n");
        let mut corrupt = candidate.bundle_bytes().to_vec();
        ...
        let applied = f
            .node()
            .runtime()
            .block_on(f.node().apply_workspace_bundle_durable_in(
                &f.node().request_context(),
                PRINCIPAL,
...
fn candidate_timeout_retains_uncertain_workspace_without_publishing_or_starting_dependents() {
    let f = Fixture::new(GitHashAlgorithm::Sha1, SIMPLE);
    let before = generation(f.node());
