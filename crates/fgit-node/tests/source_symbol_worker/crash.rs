    ...
    fn start(root: &Scratch, format: GitHashAlgorithm, phase: &str) -> Self {
        let mut child = Self(
            Command::new(std::env::current_exe().unwrap())
                .args([
                    "--exact",
                    "crash::native_owner_child",
                    "--nocapture",
                    "--test-threads=1",
                ])
                .env(CHILD_ROOT, &root.0)
                .env("FGIT_SYMBOL_CRASH_TEST_FORMAT", format.as_str())
                .env("FGIT_SYMBOL_CRASH_TEST_PHASE", phase)
                .spawn()
                .unwrap(),
        );
        ...
        while !root.0.join("owner-ready").exists() {
            assert!(
                ...
                "native child exited before its durable boundary"
            );
            assert!(
                Instant::now() < deadline,
                "native child did not reach the selected boundary"
            );
...
fn checkpoint(root: &Path) -> Vec<u8> {
    fs::read(root.join("symbols/checkpoint")).unwrap()
}
...
fn native_owner_child() {
    ...
    let format = match std::env::var("FGIT_SYMBOL_CRASH_TEST_FORMAT")
        .unwrap()
        .as_str()
    {
    ...
    if phase != "preparing" {
        ...
        match phase.as_str() {
            "published" => {
                assert!(result.is_ok());
                drop(result);
            } // Lose the activation acknowledgement.
            "unpublished" => assert!(matches!(
                result,
                Err(AccessError::Source(NodeWorkspaceRefusal::WorkspaceCapacity))
            )),
            _ => unreachable!(),
        }
...
fn killed_effect_free_preparation_resumes_and_builds_without_operator_file_edits() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let commit = add_source(&node, base);
        let canonical = generation(&node);
        let expected = binding(&node, format);
...
fn killed_native_publications_recover_original_identity_and_unpublished_candidates_stay_pending() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        for phase in ["published", "unpublished"] {
            ...
            let pending = State::decode(&before, &expected).unwrap().rows[REF]
                .pending
                .unwrap();
