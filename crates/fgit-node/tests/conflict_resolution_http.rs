...
fn resolve_bytes(
    ...
    let (body, media) = if files.is_empty() {
        ...
        body.extend_from_slice(b"--resolve-http--\r\n");
    ...
    let (body, framing) = if chunked {
        ...
        let size = body.len();
        (body, format!("Content-Length: {size}\r\n"))
    };
    request(
        endpoint,
        "POST",
...
fn resolve(
    ...
    binary_exchange(
        endpoint,
        &resolve_bytes(endpoint, 'd', command, files, chunked),
        true,
    )
}
fn discovery(
    endpoint: &Endpoint,
    data: &fgit_forge::event::pull_request::PullRequestData,
) -> (String, String) {
    committed(&post(
        endpoint,
        1,
    ...
    let reviews = get(endpoint, "/api/v1/pulls/1/reviews", 'b');
    status(&reviews, 200);
...
fn binary_resolution_survives_restart_and_only_the_reviewed_candidate_can_publish() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            candidate.binding.merge_base,
            GitOid::from_hex(format, &base).unwrap()
        );
        ...
        assert_eq!(
            resolve(&server.client, &command, &[("file_0", CONTENT)], true),
            original
        );
        error(
            &resolve(&server.client, &base_command, &[], false),
            409,
            "unresolved_conflicts",
        );
        let extra =
            base_command + &format!("&resolution={PATH_HEX}:ours&resolution=636c65616e:delete");
        error(
            &resolve(&server.client, &extra, &[], false),
            409,
            "resolution_names_clean_path",
        );
        let wrong_base = command.replace(
            &format!("merge_base={base}"),
            &format!("merge_base={}", data.target_tip),
        );
        error(
            &resolve(&server.client, &wrong_base, &[("file_0", CONTENT)], false),
            409,
        ...
        let no_attempt = LoopbackReceiveSession::authenticated(
            FOREIGN,
            IdempotencyKey::new(b"read-only-candidate-preparation".to_vec()).unwrap(),
        );
        assert!(matches!(
            node.runtime()
                .block_on(node.recover_transaction_in(&node.request_context(), &no_attempt))
                .unwrap(),
            RequestRecovery::KeyNotObserved
        ));
        let server = Server::start(node, &path, 8, true, true);
        assert_eq!(
            resolve(&server.client, &command, &[("file_0", CONTENT)], false),
            original
        );
        accepted(&send(
            &server.client,
        ...
        let pr = get(&server.client, "/api/v1/pulls/1", 'a');
        status(&pr, 200);
        ...
        assert_eq!(
            generation(&node),
            before + 4,
            "PR, vote, refused alternative, coupled merge; resolution reads never publish"
        );
        let selected = node
            .runtime()
            .block_on(node.materialize_admission())
            .unwrap();
        assert_eq!(
            selected.snapshot().refs[&data.target_ref],
...
fn all_side_choices_and_explicit_delete_are_read_only_native_candidates() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for choice in ["base", "ours", "theirs", "delete"] {
            let reply = resolve(
                &server.client,
                &(base.clone() + &format!("&resolution={PATH_HEX}:{choice}")),
                &[],
                true,
            );
        ...
        for commit in candidates {
            assert!(node.read_git_object(commit).is_err());
        }
        node.shutdown().unwrap();
    }
}
...
fn grants_keys_framing_and_unused_binary_data_refuse_before_construction() {
    ...
    let withheld = |token, extra: &str, length| {
        binary_exchange(
            &server.client,
            &request(
                &server.client,
                ...
                token,
                &format!(
                    "Content-Type: multipart/form-data; boundary=resolve-http\r\nContent-Length: {length}\r\nExpect: 100-continue\r\n{extra}"
                ),
                &[],
            ),
            false,
    ...
    error(&reply, 400, "preparation_has_no_transaction_key");
    assert!(!reply.head.contains("100 Continue"));
    let reply = withheld('d', "", 80 * 1024 * 1024);
    assert_eq!(reply.status, 413);
    assert!(!reply.head.contains("100 Continue"));
    error(
        &resolve(&server.client, &command, &[("file_0", b"unused")], false),
        400,
        "unreferenced_resolution_file",
    );
    let mut truncated = resolve_bytes(&server.client, 'd', &command, &[("file_0", CONTENT)], true);
    truncated.truncate(truncated.len() - 3);
    assert_eq!(
        binary_exchange(&server.client, &truncated, true).status,
        400
    );
    let invalid = command.replace(&format!("{PATH_HEX}:ours"), "2e2e2f78:delete");
    error(
        &resolve(&server.client, &invalid, &[], false),
        400,
    ...
    replace(
        &path,
        &(header + &row('d', FOREIGN, "read") + &row('f', FOREIGN, "pulls-read")),
    );
    let server = Server::start(node, &path, 2, true, false);
    for token in ['d', 'f'] {
        let reply = binary_exchange(
            &server.client,
            &resolve_bytes(&server.client, token, &command, &[], false),
            true,
        );
        assert_eq!(reply.status, 403);
    }
...
fn exact_pr_resolution_rechecks_version_policy_visibility_cancellation_and_work_bounds() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let session = LoopbackReceiveSession::authenticated(
            OWNER,
            IdempotencyKey::new(b"native-resolution-pr".to_vec()).unwrap(),
        );
        ...
        let metadata = MergeMetadata {
            ...
            timestamp: 1,
            message: b"native resolved candidate\n".to_vec(),
        };
        let automatic = node
            .runtime()
        ...
        let choices = [ConflictResolution {
            path: b"file\xff.txt".to_vec(),
            choice: ResolutionChoice::Theirs,
        }];
        let mut wrong_version = subject.clone();
        wrong_version.pull_request_version = AggregateVersion::FIRST.next().unwrap();
        let mut wrong_epoch = subject.clone();
        wrong_epoch.policy_epoch = subject.policy_epoch.next().unwrap();
        for wrong in [&wrong_version, &wrong_epoch] {
            assert!(
                node.runtime()
                    .block_on(node.prepare_resolved_pull_request_bundle_in(
                        &request,
                        wrong,
                        base,
                        &RefVisibility::new(),
        ...
        hidden
            .push_rule(data.source_ref.as_bytes(), &Default::default())
            .unwrap();
        assert!(
            node.runtime()
                .block_on(node.prepare_resolved_pull_request_bundle_in(
...
fn edited_and_closed_prs_cannot_silently_refresh_an_old_resolution_subject() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let root = Scratch::new();
        let config = root.config(format);
        let (node, mut data) = non_clean_fixture(&root, format, true);
        let before = generation(&node);
        let path = root.0.join("credentials");
        configure(&node, &path);
        let server = Server::start(node, &path, 8, true, false);
        ...
        error(
            &resolve(&server.client, &original, &[], false),
            409,
            "preparation_subject_moved",
        );
        ...
        error(
            &resolve(&server.client, &refreshed, &[], true),
            409,
            "preparation_subject_moved",
        );
        assert_eq!(server.finish().accepted_sessions(), 8);
        ...
        assert_eq!(
            generation(&node),
            before + 3,
            "opening, edit and close only; stale reads do not publish decisions"
        );
        assert!(node.read_git_object(candidate.binding.commit).is_err());
        node.shutdown().unwrap();
    }
