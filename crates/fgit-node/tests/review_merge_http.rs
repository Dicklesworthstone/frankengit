...
fn withdrawal_blocks_merge_despite_retained_approval_then_renewal_publishes_once() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let original = send(&server.client, "reviews/approve", 'b', "approve", &review_form(&candidate, 0), Some(&candidate.bundle), false); // 2
        accepted(&original);
        assert!(original.body.contains("\"type\":\"candidate_review_publication\""));
        assert!(original.body.contains(&REVIEWER.to_string()));
        // Terminal retries need the same semantics, not another candidate upload.
        assert_eq!(send(&server.client, "reviews/approve", 'b', "approve", &review_form(&candidate, 0), None, false), original); // 3
        ...
        assert!(node.read_git_object(candidate.binding.commit).is_err(), "review does not import the candidate");
        assert_eq!(node.runtime().block_on(node.materialize_admission()).unwrap().snapshot().refs[&candidate.data.target_ref], candidate.data.target_tip);
        let server = Server::start(node, &path, 11, true, true);
        accepted(&send(&server.client, "reviews/withdraw", 'b', "withdraw", &review_form(&candidate, 1), None, false)); // 1
        ...
        let blocked = send(&server.client, "merge", 'c', "withdrawn-merge", &merge, Some(&candidate.bundle), false); // 3
        refused(&blocked);
        assert_eq!(send(&server.client, "merge", 'c', "withdrawn-merge", &merge, None, false), blocked); // 4
        accepted(&send(&server.client, "reviews/approve", 'b', "renew", &review_form(&candidate, 2), Some(&candidate.bundle), true)); // 5
        let published = send(&server.client, "merge", 'c', "accepted-merge", &merge, Some(&candidate.bundle), true); // 6
        accepted(&published);
        assert!(published.body.contains("\"type\":\"reviewed_merge_publication\""));
        assert_eq!(send(&server.client, "merge", 'c', "accepted-merge", &merge, None, false), published); // 7
        ...
        status(&current, 200);
        assert!(current.body.contains("\"freshness\":\"pull_request_closed\""));
        ...
        let node = reopen(&config);
        assert_eq!(generation(&node), before + 6, "PR, three review transitions, refused merge, committed merge only");
...
fn withheld(endpoint: &Endpoint, action: &str, token: char) -> Reply {
    let bytes = request(endpoint, "POST", &format!("/api/v1/pulls/1/{action}"), token,
...
fn deployment_scope_and_complete_binary_envelopes_precede_review_admission() {
    ...
    let mut malformed = mutation_bytes(&server.client, "reviews/approve", 'b', "bad-part", &form, Some(&candidate.bundle), false);
    let index = malformed.windows(b"name=\"command\"".len()).position(|v| v == b"name=\"command\"").unwrap();
    ...
    let mut truncated = mutation_bytes(&server.client, "reviews/approve", 'b', "truncated", &form, Some(&candidate.bundle), false);
    truncated.pop();
    status(&exchange(&server.client, &truncated, true), 400); // 8
    status(&send(&server.client, "reviews/approve", 'b', "identity-in-form", &(form.clone() + "&reviewer=admin"), Some(&candidate.bundle), false), 400); // 9
    let missing = send(&server.client, "reviews/approve", 'b', "missing-bundle", &form, None, false); // 10
    status(&missing, 503);
    assert!(missing.body.contains("\"outcome_unknown\":true"));
    accepted(&send(&server.client, "reviews/approve", 'e', "actual-review", &form, Some(&candidate.bundle), true)); // 11
    ...
    let node = reopen(&config);
    assert_eq!(generation(&node), before + 2, "only the PR and actually validated vote publish");
...
fn a_lost_merge_reply_is_recovered_after_restart_and_write_revocation_without_a_bundle() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        accepted(&send(&server.client, "reviews/approve", 'b', "vote-before-loss", &review_form(&candidate, 0), Some(&candidate.bundle), false));
        let bytes = mutation_bytes(&server.client, "merge", 'c', "lost-merge-reply", &merge_form(&candidate, &[REVIEWER]), Some(&candidate.bundle), false);
        ...
        server.finish();
        let node = reopen(&config);
        let session = LoopbackReceiveSession::authenticated(MERGER, IdempotencyKey::new(b"lost-merge-reply".to_vec()).unwrap());
        let known = node.runtime().block_on(node.recover_transaction_in(&node.request_context(), &session)).unwrap();
        let fgit_authority::key_recovery::RequestRecovery::Recovered(known) = known else { panic!("real merge seal"); };
        let OutcomeLookup::Decided(terminal) = known.outcome() else { panic!("real merge outcome"); };
        ...
        server.finish();
        let node = reopen(&config);
        assert_eq!(generation(&node), before + 3, "open, approve, merge; recovery publishes nothing");
        assert_eq!(node.runtime().block_on(node.materialize_admission()).unwrap().snapshot().refs[&candidate.data.target_ref], candidate.binding.commit);
        node.shutdown().unwrap();
    }
}
...
fn a_merge_grant_and_weaker_requested_reviewer_list_cannot_bypass_mandatory_protection() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let session = LoopbackReceiveSession::authenticated(OWNER, IdempotencyKey::new(b"local-open".to_vec()).unwrap());
        let opening = PullRequestCommand { number: PullRequestNumber::FIRST, expected_version: ExpectedVersion::NewStream,
            action: PullRequestAction::Open, data: candidate.data.clone() };
        let (_, result) = node.runtime().block_on(node.admit_pull_request_durable_in(&node.request_context(), &session, &opening, Default::default())).unwrap();
        assert!(matches!(result.outcome, DecisionOutcome::Committed { .. }));
        let policy = ProtectionCommand { expected_version: ExpectedVersion::NewStream, expected_epoch: candidate.epoch,
            protection: ReviewProtection { administrators: vec![OWNER], branches: vec![ProtectedBranch {
                name: candidate.data.target_ref.clone(), reviewers: vec![REVIEWER, SECOND],
            }] } };
        let session = LoopbackReceiveSession::authenticated(OWNER, IdempotencyKey::new(b"install-mandatory".to_vec()).unwrap());
        let (_, result) = node.runtime().block_on(node.admit_review_protection_durable_in(&node.request_context(), &session, &policy, Default::default())).unwrap();
        assert!(matches!(result.outcome, DecisionOutcome::Committed { .. }));
        candidate.epoch = node.runtime().block_on(node.read_review_protection_in(&node.request_context())).unwrap().policy_epoch;
        let before = generation(&node);
        let path = root.0.join("credentials");
        configure(&node, &path);
        let server = Server::start(node, &path, 6, true, false);
        accepted(&send(&server.client, "reviews/approve", 'b', "first-required", &review_form(&candidate, 0), Some(&candidate.bundle), false)); // 1
        let weaker = merge_form(&candidate, &[REVIEWER]);
        let blocked = send(&server.client, "merge", 'c', "weaker-requirements", &weaker, Some(&candidate.bundle), true); // 2
        refused(&blocked);
        accepted(&send(&server.client, "reviews/approve", 'e', "second-required", &review_form(&candidate, 0), Some(&candidate.bundle), true)); // 3
        assert_eq!(send(&server.client, "merge", 'c', "weaker-requirements", &weaker, None, false), blocked); // 4
        accepted(&send(&server.client, "merge", 'c', "all-mandates-satisfied", &weaker, Some(&candidate.bundle), false)); // 5
        let pr = get(&server.client, "/api/v1/pulls/1", 'a'); // 6
        status(&pr, 200);
        assert!(pr.body.contains("\"state\":\"merged\""));
        server.finish();
        let node = reopen(&config);
        assert_eq!(generation(&node), before + 4, "two votes, one canonical refusal, one merge");
        assert_eq!(node.runtime().block_on(node.materialize_admission()).unwrap().snapshot().refs[&candidate.data.target_ref], candidate.binding.commit);
        node.shutdown().unwrap();
    }
}
