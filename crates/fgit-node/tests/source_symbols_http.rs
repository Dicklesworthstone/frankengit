...
use fgit_forge::source_symbols::{
    MAX_SYMBOL_WORK, SymbolKind, SymbolMatchMode, SymbolQuery, SymbolReadError, SymbolSearchReport,
    SymbolSyntaxErrorKind,
};
...
fn add_files(node: &OneNode, base: GitOid, files: &[(&[u8], &[u8])], key: &[u8]) -> GitOid {
    ...
    for (path, body) in files {
        let old = quoted_path('a', path);
        let new = quoted_path('b', path);
...
fn symbols(root: &Scratch, format: GitHashAlgorithm) -> (OneNode, GitOid) {
    ...
    let commit = add_files(
        &node,
        base,
        &[(b"src/a.rs", A), (b"src/b.rs", B), (RAW_PATH, RAW)],
        b"symbol-fixture",
    );
    (node, commit)
}
fn form(format: GitHashAlgorithm) -> String {
    format!("{}&name_hex={}&match=prefix", common(format), hex(b"Thing"))
}
...
fn native_symbols_match_exact_source_spans_and_reopen_without_authority_mutation() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (head, report) = node
            ...
            .unwrap();
        assert_eq!(report.files_selected, 8);
        assert_eq!(report.files_read, 3);
        assert_eq!(report.unsupported_language_files, 5);
        assert_eq!(report.non_regular_entries, 2);
        assert_eq!(report.bytes_read, A.len() + B.len() + RAW.len());
        assert_eq!(report.bytes_searched, report.bytes_read);
        assert_eq!(report.declarations_examined, 6);
        assert_eq!(report.attributes_skipped, 1);
        assert_eq!(report.macro_bodies_skipped, 1);
        ...
        for row in &report.matches {
            let body = match row.location.path.as_slice() {
                RAW_PATH => RAW,
                b"src/a.rs" => A,
                b"src/b.rs" => B,
            ...
            assert_eq!(
                &body[row.location.byte_offset
                    ..row.location.byte_offset + row.location.match_length],
                row.name
            );
            assert_eq!(
                &body[row.location.excerpt_offset
                    ..row.location.excerpt_offset + row.location.excerpt.len()],
                row.location.excerpt
            );
        }
        let raw = local(
            &node,
        ...
        assert!(raw.matches[0].raw_identifier);
        assert_eq!(raw.matches[0].location.line, 2);
        ...
        let again = node
            ...
            .unwrap();
        assert_eq!(again, (head, report));
        assert_eq!(generation(&node), before);
        node.shutdown().unwrap();
    }
}
fn scoped<A: NativeFormat>(
    node: &OneNode,
    capability: &mut TreeCapability,
    visibility: &RefVisibility,
    q: &SymbolQuery,
) -> Result<SymbolSearchReport, SymbolReadError<NodeWorkspaceRefusal>> {
    node.runtime().block_on(node.search_source_symbols_in::<A>(
        &node.request_context(),
        &reference(),
        visibility,
        capability,
...
fn native_capability_visibility_kind_and_component_filters_never_widen_reads() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let q = query(b"Thing", SymbolMatchMode::Prefix, &[], &[]);
        let repo = RepositoryId::from_bytes([0x32; 16]);
        let run = |cap: &mut TreeCapability, v: &RefVisibility| match format {
            GitHashAlgorithm::Sha1 => scoped::<Sha1>(&node, cap, v, &q),
            GitHashAlgorithm::Sha256 => scoped::<Sha256>(&node, cap, v, &q),
        };
        let r = run(&mut cap(repo), &RefVisibility::new()).unwrap();
        assert_eq!(r.matches.len(), 4);
        assert_eq!(r.files_selected, 2);
        assert!(
            r.matches
                .iter()
                .all(|m| m.location.path.starts_with(b"src/"))
        );
        assert!(
            run(
                &mut cap(RepositoryId::from_bytes([0x99; 16])),
                &RefVisibility::new()
            )
            .is_err()
        );
        let mut revoked = cap(repo);
        revoked.revoke();
        ...
        hidden
            ...
            .unwrap();
        assert!(run(&mut cap(repo), &hidden).is_err());
        let r = local(
            &node,
            &query(
                b"Thing",
        ...
        assert_eq!(r.matches[0].name, b"Thing");
        assert!(
            local(
                &node,
                &query(b"Thing", SymbolMatchMode::Prefix, &[], &[b"sr".to_vec()]),
                Default::default()
            )
            .unwrap()
            .matches
...
fn native_limits_cancellation_and_invalid_pins_never_return_partial_success() {
    ...
    assert!(
        matches!(local(&node,&tiny,Default::default()),Err(SymbolReadError::Syntax{error,..}) if error.kind==SymbolSyntaxErrorKind::WorkLimit)
    );
    let request = node.request_context();
    request.cancel();
    assert!(
    ...
    let clean = node.request_context();
    drop(node.search_source_symbols_snapshot_local_in(
        &clean,
        &reference(),
        None,
        None,
    ...
    assert_eq!(generation(&node), before);
    assert!(local(&node, &q, Default::default()).is_ok());
    node.shutdown().unwrap();
}
#[test]
fn unsupported_source_is_distinct_from_an_empty_supported_search() {
    ...
    let q = query(b"Thing", SymbolMatchMode::Prefix, &[], &[]);
    let empty = local(&node, &q, Default::default()).unwrap();
    assert_eq!(empty.unsupported_language_files, 5);
    assert_eq!(empty.files_read, 0);
    assert!(empty.matches.is_empty());
    let commit = add_files(
        &node,
        base,
        &[
            (b"src/good.rs", b"fn Thing(){}\n"),
    ...
    assert!(
        matches!(local(&node,&q,Default::default()),Err(SymbolReadError::Syntax{error,..}) if error.kind==SymbolSyntaxErrorKind::UnterminatedComment)
    );
    let narrowed = local(
        &node,
        &query(b"Thing", SymbolMatchMode::Exact, &[], &[b"src".to_vec()]),
        Default::default(),
    )
    .unwrap();
    assert_eq!(narrowed.matches.len(), 1);
    node.shutdown().unwrap();
}
#[test]
fn authenticated_http_returns_same_snapshot_bytes_for_fixed_and_chunked_requests() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let first = post(&server.client, "search-symbols", 'a', &form, false);
        status(&first, 200);
        assert_eq!(text(&first.body, "type"), "source_search_symbols");
        assert_eq!(text(&first.body, "profile"), "rust-declaration-heads-v1");
        assert_eq!(number(&first.body, "returned_matches"), 5);
        assert_eq!(number(&first.body, "unsupported_language_files"), 5);
        assert!(first.body.contains(&hex(RAW_PATH)));
        assert!(first.body.contains("\"compiler_resolved\":false"));
        assert!(first.body.contains("\"transaction_created\":false"));
        let pinned = format!(
            "{form}&expected_head={}&expected_commit={commit}",
            token(&first)
        );
        assert_eq!(
            post(&server.client, "search-symbols", 'a', &pinned, true),
            first
        );
        let limited = post(
            &server.client,
            "search-symbols",
            'a',
            &(form.clone() + "&max_matches=2"),
            false,
        );
        status(&limited, 200);
        assert_eq!(text(&limited.body, "completion"), "match_limit");
        let exact = post(
            &server.client,
            "search-symbols",
            'a',
            &(form.clone() + "&max_matches=5"),
            false,
        );
        status(&exact, 200);
        assert_eq!(text(&exact.body, "completion"), "complete");
        status(
            &post(
                &server.client,
                "search-symbols",
...
fn http_authorization_revocation_source_switch_and_transaction_boundary_precede_disclosure() {
    ...
    for (token, code) in [('b', 403), ('c', 403), ('f', 401)] {
        let r = post(&server.client, "search-symbols", token, &form, false);
        status(&r, code);
        assert!(!r.body.contains("\"matches\""));
    }
    let r = exchange(
        &server.client,
        &request(
            &server.client,
            "/api/v1/source/search-symbols",
            'a',
            &format!(
    ...
    status(&r, 400);
    replace(&path, &(header + &row('b', FOREIGN, "receive")));
    status(
        &post(&server.client, "search-symbols", 'a', &form, false),
        401,
    );
    assert_eq!(server.finish().accepted_sessions(), 5);
    let node = reopen(&config);
...
fn http_snapshot_pins_refuse_intervening_forge_write_and_new_query_can_reselect() {
    ...
    assert!(!stale.body.contains("\"matches\""));
    let current = post(
        &server.client,
        "search-symbols",
        'a',
        &format!("{form}&expected_commit={commit}"),
        false,
    );
    status(&current, 200);
    assert_ne!(token(&first), token(&current));
    assert_eq!(number(&current.body, "returned_matches"), 5);
...
fn http_unsupported_source_refuses_without_disclosing_diagnostic_paths_or_falling_back() {
    ...
    let form = form(GitHashAlgorithm::Sha1);
    let error = post(&server.client, "search-symbols", 'a', &form, false);
    status(&error, 409);
    assert_eq!(text(&error.body, "code"), "symbol_source_unsupported");
    assert!(!error.body.contains("private") && !error.body.contains("\"matches\""));
    let good = post(
        &server.client,
        "search-symbols",
        'a',
        &(form.clone() + "&path_prefix_hex=737263"),
        false,
    );
    status(&good, 200);
    assert_eq!(number(&good.body, "returned_matches"), 1);
    status(
        &post(
            &server.client,
            "search-symbols",
