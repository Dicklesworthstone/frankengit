...
fn inspection_form(history: &History, candidate: GitOid) -> String {
    format!(
        "profile=linear-v1&object_format={}&source_ref_hex={}&onto_ref_hex={}&expected_source={}&expected_onto={}&candidate_commit={candidate}",
        candidate.algorithm().as_str(),
        hex(b"refs/heads/topic"),
...
fn full_series_inspection_survives_restart_then_explicit_apply_retains_its_own_lease() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (node, history) = fixture(&root, format, 0);
        ...
        let prepared = post_form(
            &server.client,
            "rebase/prepare",
            'a',
            &prepare_form(&history),
            false,
        );
        ...
        assert_eq!(
            json(&inspected).matches("\"type\":\"source_diff\"").count(),
            3
        );
        ...
        for flag in [
            ...
            "\"replay_equivalence_verified\":false",
            "\"all_rewritten_commits\":true",
            "\"all_changed_paths\":true",
        ] {
        ...
        assert!(json(&inspected).contains(&hex(
            b"author Original <original@example.invalid> 5 -0330\n"
        )));
        assert_eq!(
            field(json(&inspected), "sha256"),
        ...
        let applied = multipart(
            ...
            "rebase/apply",
            'b',
            &apply_form(&history, candidate),
            &bundle,
            Some("inspected-rebase"),
        );
        status(&applied, 200);
        ...
        assert_eq!(
            retried, applied,
            "inspection freshness must not change terminal retry semantics"
        );
        server.finish();
        let node = reopen(&root.config(format));
...
fn scope_keys_limits_and_corruption_fail_closed_and_rotation_does_not_change_content() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (node, history) = fixture(&root, format, 0);
        let credentials = root.0.join("credentials");
        let grants = configure(&node, &credentials);
        let before = generation(&node);
        ...
        status(&key, 400);
        assert!(json(&key).contains("source_read_has_no_transaction_key"));
        status(
            &multipart(
                &server.client,
                "rebase/inspect",
                'a',
                &(command.clone() + "&max_changes=1"),
                &bundle,
                None,
            ),
            413,
        );
        let stale = command.replace(
            &format!("expected_source={}", history.source),
            &format!("expected_source={}", history.first),
        );
        status(
            &multipart(&server.client, "rebase/inspect", 'a', &stale, &bundle, None),
            409,
        );
        let mut corrupt = bundle.clone();
        *corrupt.last_mut().unwrap() ^= 1;
        let corrupt = multipart(
            &server.client,
            "rebase/inspect",
            'a',
            &command,
        ...
        assert_ne!(corrupt.status, 200);
        assert!(!json(&corrupt).contains("\"complete\":true"));
        status(
            &multipart(
                &server.client,
                "rebase/inspect",
                'a',
                &(command.clone() + "&path_prefix_hex=61"),
                &bundle,
                None,
            ),
            400,
        ...
        replace(
            ...
            &(grants
                ...
                + &row('c', OWNER, "read,outcomes-read")),
        );
        status(
            &multipart(
                &server.client,
...
fn zero_commit_series_still_inspects_the_result_and_requires_complete_multipart_framing() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let root = Scratch::new();
        let (node, history) = fixture(&root, format, 2);
        let credentials = root.0.join("credentials");
        configure(&node, &credentials);
        let before = generation(&node);
        let server = SourceServer::start(node, &credentials, 4);
        let prepared = post_form(
            ...
            "rebase/prepare",
            'a',
            &prepare_form(&history).replace("empty=stop", "empty=drop"),
            false,
        );
        ...
        assert_eq!(
            chunked_inspect(&server.client, &command, &bundle, true),
            inspected
        );
        status(
            &chunked_inspect(&server.client, &command, &bundle, false),
            400,
        );
        server.finish();
        let node = reopen(&root.config(format));
