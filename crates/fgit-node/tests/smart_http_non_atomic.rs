...
use fgit_types::{
    DecisionOutcome, GitHashAlgorithm, GitOid, HeadGeneration, PrincipalId, RefName,
...
fn config(root: &Scratch, format: GitHashAlgorithm) -> NodeConfig {
    NodeConfig::new(root.0.clone(), TenantId::from_bytes([0xd1; 16]), RepositoryId::from_bytes([0xd2; 16]))
...
fn perform(
    ...
    for (index, (old, new, name)) in commands.iter().enumerate() {
        ...
        if index == 0 {
            command.push_str(&format!("\0report-status delete-refs object-format={}{}",
                new.algorithm().as_str(), if atomic { " atomic" } else { "" }));
        }
    ...
    let session = LoopbackReceiveSession::authenticated(
        PrincipalId::from_bytes([0xd3; 16]), IdempotencyKey::new(key.to_vec()).unwrap(),
    );
    if streaming {
        node.smart_http_receive_stream_in(&request, &session, &mut Cursor::new(body),
            HttpLimits::default(), AdmissionLimits::default(), &mut || true, writer)
    } else {
        node.smart_http_receive_rpc_in(&request, &session, &body,
            HttpLimits::default(), AdmissionLimits::default(), &mut || true, writer)
    }
}
fn committed(result: &AdmissionResult, index: usize) {
    assert!(matches!(result.commands[index].terminal.outcome, DecisionOutcome::Committed { .. }),
        "{:?}", result.commands[index]);
}
fn stale(result: &AdmissionResult, index: usize) {
    assert!(matches!(result.commands[index].terminal.outcome,
        DecisionOutcome::Refused { code: RefusalCode::ExpectedOldRefMismatch, .. }),
...
fn reports(response: &[u8]) -> Vec<Vec<u8>> {
    ...
    packets.into_iter().filter_map(|packet| match packet {
        Packet::Data(line) => Some(line), _ => None,
    }).collect()
}
...
fn refused_first_ref_allows_later_creates_and_deletes_with_reopenable_retries() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        for streaming in [false, true] {
            ...
            let first = perform(&node, b"mixed-create", &commands, &pack, false, streaming, &mut output).unwrap();
            assert!(!first.session.atomic);
            assert_eq!(first.session.tx_ids.len(), 3);
            assert_ne!(first.session.tx_ids[0], first.session.tx_ids[1]);
            stale(&first, 0); committed(&first, 1); committed(&first, 2);
            assert_eq!(state(&node).0.get(), before.get() + 3);
            assert_eq!(state(&node).1.len(), 2);
            assert_eq!(reports(&output), vec![b"unpack ok\n".to_vec(),
                b"ng refs/tags/stale stale info\n".to_vec(), b"ok refs/tags/good\n".to_vec(),
                b"ok refs/tags/also-good\n".to_vec()]);
            let published = state(&node);
            node.shutdown().unwrap();
            let node = start(configuration, true);
            let retry = perform(&node, b"mixed-create", &commands, &pack, false, streaming, &mut Vec::new()).unwrap();
            assert_eq!(retry, first);
            assert_eq!(state(&node), published);
            let deletes = [(oid, zero, "refs/tags/stale"), (oid, zero, "refs/tags/good"),
                (oid, zero, "refs/tags/also-good")];
            let removed = perform(&node, b"mixed-delete", &deletes, &[], false, streaming, &mut Vec::new()).unwrap();
            stale(&removed, 0); committed(&removed, 1); committed(&removed, 2);
            assert!(state(&node).1.is_empty());
            let deleted = state(&node);
            assert_eq!(perform(&node, b"mixed-delete", &deletes, &[], false, streaming, &mut Vec::new()).unwrap(), removed);
...
fn a_middle_refusal_does_not_discard_success_before_or_after_it() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (oid, zero, pack) = blob_pack(format);
        let commands = [(zero, oid, "refs/tags/first"), (oid, oid, "refs/tags/stale"),
            (zero, oid, "refs/tags/last")];
        let result = perform(&node, b"middle-refusal", &commands, &pack, false, true, &mut Vec::new()).unwrap();
        committed(&result, 0); stale(&result, 1); committed(&result, 2);
        let refs = state(&node).1;
        assert_eq!(refs.len(), 2);
        assert_eq!(refs.get(&RefName::try_new(b"refs/tags/first").unwrap()), Some(&oid));
        assert_eq!(refs.get(&RefName::try_new(b"refs/tags/last").unwrap()), Some(&oid));
        node.shutdown().unwrap();
    }
}
...
fn consecutive_refusals_keep_their_real_reason_instead_of_authority_receipt_stale() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let commands = [(oid, oid, "refs/tags/one"), (oid, oid, "refs/tags/two"),
            (oid, oid, "refs/tags/three")];
        let before = state(&node).0;
        let result = perform(&node, b"three-refusals", &commands, &pack, false, false, &mut Vec::new()).unwrap();
        for index in 0..3 { stale(&result, index); }
        assert_eq!(state(&node).0.get(), before.get() + 3);
        assert!(state(&node).1.is_empty());
        node.shutdown().unwrap();
    }
...
fn atomic_twin_still_refuses_every_ref_under_one_transaction() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let commands = [(oid, oid, "refs/tags/stale"), (zero, oid, "refs/tags/good")];
        let before = state(&node).0;
        let result = perform(&node, b"atomic-twin", &commands, &pack, true, true, &mut Vec::new()).unwrap();
        assert!(result.session.atomic);
        assert_eq!(result.session.tx_ids.len(), 1);
        stale(&result, 0); stale(&result, 1);
...
fn a_lost_mixed_report_retains_all_outcomes_and_retry_does_not_publish_again() {
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (oid, zero, pack) = blob_pack(format);
        let commands = [(oid, oid, "refs/tags/stale"), (zero, oid, "refs/tags/good")];
        let error = perform(&node, b"lost-mixed-report", &commands, &pack, false, true, &mut Broken).unwrap_err();
        let NodeSmartHttpRefusal::ReceiveResponse { outcome, .. } = error else { panic!("canonical outcomes must survive failed delivery") };
        stale(&outcome, 0); committed(&outcome, 1);
        let published = state(&node);
        let recovered = perform(&node, b"lost-mixed-report", &commands, &pack, false, false, &mut Vec::new()).unwrap();
...
fn the_complete_command_limit_is_checked_before_any_command_is_admitted() {
    ...
    let commands: Vec<_> = names.iter().map(|name| (zero, oid, name.as_str())).collect();
    let before = state(&node);
    let error = perform(&node, b"too-many-refs", &commands, &pack, false, true, &mut Vec::new()).unwrap_err();
    let NodeSmartHttpRefusal::ReceiveInterrupted(error) = error else { panic!("session planning preserves its typed failure") };
    assert!(error.session().is_none());
    assert!(error.completed_commands().is_empty());
    assert!(matches!(error.admission_error(), fgit_admission::AdmissionError::CommandLimitExceeded { limit: 64 }));
