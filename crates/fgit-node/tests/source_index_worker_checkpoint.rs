...
fn setup(root: &Scratch, node: &OneNode, format: GitHashAlgorithm) -> ProgressFile {
    ...
    let binding = format!("31313131313131313131313131313131 32323232323232323232323232323232 {} {}",
...
fn worker(root: &Scratch, format: GitHashAlgorithm) -> Output {
    Command::new(env!("CARGO_BIN_EXE_fg-index-maintain"))
        ...
        .arg(root.0.join("maintenance")).args(["resume", "1", "0", "refs/heads/main"])
        .output().unwrap()
}
fn digest(id: GraphGenerationId) -> [u8; 32] {
    id.as_internal_object_id().digest().as_bytes().try_into().unwrap()
}
fn selected(node: &OneNode) -> Result<fgit_graph::GenerationActivation, NodeWorkspaceRefusal> {
    ...
    node.runtime().block_on(node.search_source_index_local_in(&node.request_context(),
        &RefName::try_new(REF).unwrap(), None, None, None, None, &query, None,
        Default::default(), Default::default())).map(|r| r.generation)
}
...
fn actual_worker_recovers_confirmed_publication_using_only_prepublication_progress() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let result = node.runtime().block_on(node.reconcile_source_index_guarded_local_in(
            &node.request_context(), &RefName::try_new(REF).unwrap(), None, None,
            Default::default(), Default::default(), &mut |candidate| {
                file.arm(REF, digest(candidate)).unwrap(); Ok(())
            }));
        assert!(result.is_ok()); drop(result); // Lose activation reply before any progress acknowledgement.
        let expected = file.state.rows[REF].pending.unwrap();
        node.shutdown().unwrap(); file.release().unwrap(); // Establish quiescence before relinquishing ownership.
        ...
        let node = reopen(&config); assert_eq!(digest(selected(&node).unwrap().generation_id), expected);
        ...
        assert_eq!(fs::read(root.0.join("maintenance/checkpoint")).unwrap(), checkpoint);
    }
}
...
fn actual_worker_restarts_effect_free_preparation_and_builds_normally() {
    ...
    file.state.begin_preparation(REF).unwrap(); file.save().unwrap();
    node.shutdown().unwrap(); file.release().unwrap(); // No native build was called.
    ...
    let node = reopen(&config); assert!(selected(&node).is_ok()); node.shutdown().unwrap();
}

#[test]
fn actual_worker_preserves_a_recorded_but_unpublished_candidate_without_rebuilding() {
    let root = Scratch::new(); let config = root.config(GitHashAlgorithm::Sha256);
    let (node, _) = fixture(&root, GitHashAlgorithm::Sha256); let before = generation(&node);
    let mut file = setup(&root, &node, GitHashAlgorithm::Sha256);
    file.state.begin_preparation(REF).unwrap(); file.save().unwrap();
    let failed = node.runtime().block_on(node.reconcile_source_index_guarded_local_in(
        &node.request_context(), &RefName::try_new(REF).unwrap(), None, None,
        Default::default(), Default::default(), &mut |candidate| {
            file.arm(REF, digest(candidate)).unwrap(); Err(NodeWorkspaceRefusal::WorkspaceCapacity)
        }));
    ...
    for _ in 0..2 {
        ...
        assert_eq!(fs::read(root.0.join("maintenance/checkpoint")).unwrap(), pending);
    }
    let node = reopen(&config);
    assert!(matches!(selected(&node), Err(NodeWorkspaceRefusal::SourceIndex(e)) if matches!(*e, IndexError::Uninitialized)));
    assert_eq!(generation(&node), before); node.shutdown().unwrap();
}
...
fn legacy_unknown_running_attempt_remains_blocked_instead_of_becoming_safe_preparation() {
    ...
    node.shutdown().unwrap(); file.release().unwrap();
    let path = root.0.join("maintenance/checkpoint");
    let legacy = String::from_utf8(fs::read(&path).unwrap()).unwrap()
        .replacen("frankengit-index-worker-v2", "frankengit-index-worker-v1", 1);
