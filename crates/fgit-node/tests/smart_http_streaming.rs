...
use fgit_node::{
    GitDaemonServerLimits, GitDaemonSessionTimeout, LoopbackReceiveSession,
    NodeConfig, NodeReceiveTransportRefusal, NodeSmartHttpRefusal, OneNode,
};
use fgit_types::{DecisionOutcome, GitHashAlgorithm, GitOid, HeadGeneration, PrincipalId, RepositoryId, TenantId};
...
impl Scratch {
    fn new() -> Self {
        Self(std::env::temp_dir().join(format!("fg-http-stream-{}-{}",
...
fn config(scratch: &Scratch, format: GitHashAlgorithm) -> NodeConfig {
    NodeConfig::new(scratch.0.clone(), TenantId::from_bytes([0xd1; 16]), RepositoryId::from_bytes([0xd2; 16]))
...
fn node(scratch: &Scratch, format: GitHashAlgorithm, serving: bool) -> OneNode {
    ...
    if serving { node.bring_into_service(HeadGeneration::FIRST).unwrap(); }
    node
}
fn session(key: &[u8]) -> LoopbackReceiveSession {
    LoopbackReceiveSession::authenticated(PrincipalId::from_bytes([0xd3; 16]), IdempotencyKey::new(key.to_vec()).unwrap())
}
fn header(node: &OneNode, service: &str, length: usize, chunked: bool) -> Vec<u8> {
    ...
    let framing = if chunked { "Transfer-Encoding: chunked".to_owned() }
        else { format!("Content-Length: {length}") };
...
fn command(text: String, pack: &[u8]) -> Vec<u8> {
    let mut bytes = encode_packets(&[Packet::Data(text.into_bytes()), Packet::Flush], &WireLimits::default()).unwrap();
...
fn create(format: GitHashAlgorithm) -> (GitOid, Vec<u8>) {
    ...
    (oid, command(format!("{zero} {oid} {TAG}\0report-status object-format={}", format.as_str()), &pack))
}
...
impl Read for NoRead {
    fn read(&mut self, _: &mut [u8]) -> io::Result<usize> { panic!("request must finish/refuse before another transport read") }
}
...
fn assert_unstaged(node: &OneNode, oid: GitOid) {
    ...
    assert!(node.read_git_object(oid).is_err(), "incomplete HTTP cannot stage verified objects");
}
...
fn fragmented_push_retry_delete_and_reopen_work_in_both_formats_and_framings() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        for chunks in [false, true] {
            ...
            let wire = if chunks { chunked(&payload) } else { payload.clone() };
            let head = header(&node, "receive-pack", wire.len(), chunks);
            let request = parse_head(&head, HttpLimits::default()).unwrap().unwrap();
            let consumed = Cell::new(0);
            let mut fragmented = Fragmented { bytes: &wire, width: 1, consumed: &consumed, interrupt: true };
            let mut response = Vec::new();
            let first = node.smart_http_receive_stream_in(&request, &session(b"create"), &mut fragmented,
                HttpLimits::default(), AdmissionLimits::default(), &mut || true, &mut response).unwrap();
            assert_committed(&first);
            assert_eq!(consumed.get(), wire.len());
            assert!(response.ends_with(b"0000"));
            assert!(response.windows(b"ok refs/tags/streamed\n".len()).any(|w| w == b"ok refs/tags/streamed\n"));
            let generation = node.runtime().block_on(node.materialize_admission()).unwrap().basis().generation();
            ...
            let retry = node.smart_http_receive_stream_in(&request, &session(b"create"), &mut retry_body,
                HttpLimits::default(), AdmissionLimits::default(), &mut || true, &mut retry_response).unwrap();
            assert_committed(&retry);
            assert_eq!(first.commands[0].tx_id, retry.commands[0].tx_id);
            assert_eq!(response, retry_response);
            assert_eq!(generation, node.runtime().block_on(node.materialize_admission()).unwrap().basis().generation());
            node.shutdown().unwrap();

            let mut reopened = OneNode::open_existing(config(&scratch, format)).unwrap();
            reopened.bring_into_service(generation).unwrap();
            let before = reopened.runtime().block_on(reopened.materialize_admission()).unwrap();
            assert!(before.snapshot().refs.values().any(|id| *id == oid));
            reopened.read_git_object(oid).unwrap();
            let zero = "0".repeat(oid.as_bytes().len() * 2);
            let deletion = command(format!("{oid} {zero} {TAG}\0report-status delete-refs object-format={}", format.as_str()), &[]);
            let wire = if chunks { chunked(&deletion) } else { deletion };
            let head = header(&reopened, "receive-pack", wire.len(), chunks);
            let request = parse_head(&head, HttpLimits::default()).unwrap().unwrap();
            let result = reopened.smart_http_receive_stream_in(&request, &session(b"delete"), &mut Cursor::new(wire),
                HttpLimits::default(), AdmissionLimits::default(), &mut || true, &mut Vec::new()).unwrap();
            assert_committed(&result);
            assert!(reopened.runtime().block_on(reopened.materialize_admission()).unwrap().snapshot().refs.is_empty());
...
fn complete_pack_inside_unfinished_or_invalid_http_never_stages_objects() {
    for chunks in [false, true] {
        for suffix in [b"".as_slice(), b"NEXT".as_slice()] {
            ...
            let request = parse_head(&head, HttpLimits::default()).unwrap().unwrap();
            let mut response = Vec::new();
            let result = node.smart_http_receive_stream_in(&request, &session(b"invalid-http"), &mut Cursor::new(wire),
                HttpLimits::default(), AdmissionLimits::default(), &mut || true, &mut response);
...
fn authentication_cell_and_cancellation_gates_precede_stream_reads() {
    ...
    let call = |session: &LoopbackReceiveSession, live: bool| {
        node.smart_http_receive_stream_in(&request, session, &mut NoRead, HttpLimits::default(),
            AdmissionLimits::default(), &mut || live, &mut Vec::new())
    };
    assert!(matches!(call(&LoopbackReceiveSession::anonymous(), true), Err(NodeSmartHttpRefusal::UnauthenticatedReceive)));
    assert!(matches!(call(&session(b"bootstrapping"), true), Err(NodeSmartHttpRefusal::ReceiveTransport(error))
        if matches!(*error, NodeReceiveTransportRefusal::CellState(_))));
    assert!(matches!(call(&session(b"cancelled"), false), Err(NodeSmartHttpRefusal::Rpc(error))
...
fn malformed_git_is_refused_without_reading_the_remainder_of_the_declared_body() {
    ...
    for service in ["receive-pack", "upload-pack"] {
        ...
        let result = if service == "receive-pack" {
            node.smart_http_receive_stream_in(&request, &session(b"malformed"), &mut reader,
                HttpLimits::default(), AdmissionLimits::default(), &mut || true, &mut response).map(|_| ())
        } else {
            node.smart_http_upload_stream_in(&request, &mut reader, WireLimits::default(),
                HttpLimits::default(), 1024 * 1024, &mut || true, &mut response).map(|_| ())
        };
...
fn cancellation_during_fragmented_ingress_cannot_stage_or_publish() {
    ...
    let request = parse_head(&head, HttpLimits::default()).unwrap().unwrap();
    let consumed = Cell::new(0);
    let mut reader = Fragmented { bytes: &payload, width: 7, consumed: &consumed, interrupt: true };
    let mut response = Vec::new();
    let result = node.smart_http_receive_stream_in(&request, &session(b"cancel-in-body"), &mut reader,
        HttpLimits::default(), AdmissionLimits::default(), &mut || consumed.get() < 21, &mut response);
    assert!(matches!(result, Err(NodeSmartHttpRefusal::Rpc(error)) if matches!(*error, RpcError::Cancelled)));
...
fn fragmented_upload_matches_slice_response_without_waiting_for_eof() {
    let scratch = Scratch::new();
    let node = node(&scratch, GitHashAlgorithm::Sha256, true);
    let body = encode_packets(&[Packet::Data(b"command=ls-refs\n".to_vec()),
        Packet::Data(b"object-format=sha256\n".to_vec()), Packet::Delimiter, Packet::Flush], &WireLimits::default()).unwrap();
    let head = String::from_utf8(header(&node, "upload-pack", body.len(), false)).unwrap()
        .replace("Host: local\r\n", "Host: local\r\nGit-Protocol: version=2\r\n");
    let request = parse_head(head.as_bytes(), HttpLimits::default()).unwrap().unwrap();
    let mut expected = Vec::new();
    node.smart_http_upload_rpc_in(&request, &body, WireLimits::default(), HttpLimits::default(),
        1024 * 1024, &mut || true, &mut expected).unwrap();
    let consumed = Cell::new(0);
    let reader = Fragmented { bytes: &body, width: 1, consumed: &consumed, interrupt: true };
    let mut actual = Vec::new();
    let receipt = node.smart_http_upload_stream_in(&request, &mut reader.chain(NoRead), WireLimits::default(),
        HttpLimits::default(), 1024 * 1024, &mut || true, &mut actual).unwrap();
...
fn lost_streamed_reply_retains_the_actual_canonical_result() {
    ...
    let head = header(&node, "receive-pack", payload.len(), false);
    let request = parse_head(&head, HttpLimits::default()).unwrap().unwrap();
    let result = node.smart_http_receive_stream_in(&request, &session(b"lost-reply"), &mut Cursor::new(payload),
        HttpLimits::default(), AdmissionLimits::default(), &mut || true, &mut BrokenWriter);
    let Err(NodeSmartHttpRefusal::ReceiveResponse { outcome, .. }) = result else { panic!("lost reply must retain its canonical outcome") };
...
fn tcp_server_rejects_bad_git_before_client_finishes_uploading() {
    ...
    let worker = std::thread::spawn(move || {
        let result = node.serve_smart_http_bounded(&listener, GitDaemonServerLimits::try_new(2, 2).unwrap(),
            sha256_digest(&[b'a'; 64]), PrincipalId::from_bytes([0xd3; 16]), true, Duration::from_secs(30));
        node.shutdown().unwrap();
        result.unwrap()
    });
