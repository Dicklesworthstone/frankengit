...
impl Fixture {
    fn new(format: GitHashAlgorithm, workflow: &str) -> Self {
        ...
        let workflow = if workflow.starts_with("name:") {
            workflow.to_owned()
        } else {
        ...
        for (name, oid) in [
            (b"input.txt".as_slice(), input),
            (b"workflow.yml".as_slice(), workflow),
        ] {
...
fn one_source_snapshot_fresh_job_copies_and_persistent_report_in_both_formats() {
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(result.workspaces_closed);
        assert_eq!(result.source_commit, f.tip);
        assert_eq!(result.workflow_blob, f.workflow);
        assert_eq!(result.source_reference, reference().as_bytes());
        assert_eq!(
            result.read_prefixes,
            vec![b"input.txt".to_vec(), b"workflow.yml".to_vec()]
        );
        assert_eq!(
            result
                .execution
        ...
        assert_eq!(
            result.execution.jobs[0].steps[0].observation.stdout,
            b"first"
        );
        assert_eq!(
            result.execution.jobs[0].steps[1].observation.stdout,
            b"second"
        );
        assert_eq!(
            result.execution.jobs[1].steps[0].observation.stdout,
            b"dependent"
        );
        assert_eq!(
            result.execution.jobs[2].steps[0].observation.stdout,
            b"independent"
        );
        let json = result.to_json();
        assert_eq!(
            fs::read_to_string(result.run_directory.join("report.json")).unwrap(),
            json
        );
        assert!(
            json.contains("\"published\":false") && json.contains("\"authoritative_check\":false")
        );
        let mut names = fs::read_dir(&result.run_directory)
            .unwrap()
...
fn failed_dependency_skips_only_dependents_and_closes_all_started_jobs() {
    ...
    assert_eq!(result.execution.jobs[0].steps.len(), 1);
...
fn entire_graph_scope_and_source_pins_are_preflighted_before_host_creation() {
    ...
    for (path, prefixes, expected) in [
        (
            b"workflow.yml".as_slice(),
            vec![b"input.txt".to_vec()],
            None,
        ),
...
fn interrupted_attempt_is_never_adopted_or_overwritten() {
    ...
    assert_eq!(
        fs::read(directory.join("attempt.json")).unwrap(),
        b"interrupted responsibility"
    );
...
fn timeout_retains_workspace_and_does_not_start_other_jobs() {
    ...
    assert!(result.execution.jobs[1].steps.is_empty());
    assert!(
        fs::read_to_string(result.run_directory.join("report.json"))
            .unwrap()
            .contains("\"workspaces_closed\":false")
    );
    assert_eq!(generation(f.node()), before);
}
...
fn failure_and_always_diagnostics_execute_in_real_workspaces_and_persist_without_greenwashing() {
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            run.execution.jobs[0].steps[0].observation.stdout,
            b"primary"
        );
        assert_eq!(
            run.execution.jobs[0].steps[1].observation.stdout,
            b"same-job-diagnostic"
        );
        assert_eq!(
            run.execution.jobs[0].steps[2].observation.stdout,
            b"same-job-cleanup"
        );
        assert_eq!(
            run.execution.jobs[1].steps[0].observation.stdout,
            b"dependent-cleanup"
        );
        assert_eq!(
            run.execution.jobs[2].steps[0].observation.stdout,
            b"dependent-diagnostic"
        );
...
fn literal_multiline_script_executes_exact_decoded_bytes_and_persists_commitment() {
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            run.execution.jobs[0].steps[0].name.as_deref(),
            Some("multiline")
        );
        let saved = fs::read_to_string(run.run_directory.join("report.json")).unwrap();
        assert_eq!(saved, run.to_json());
