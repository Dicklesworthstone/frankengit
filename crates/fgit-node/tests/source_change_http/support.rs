...
mod base;
pub use base::{
    BINARY, BINARY_PATH, Endpoint, FOREIGN, LINK, OWNER, Reply, Scratch, TEXT, common, credentials,
    exchange, fixture, generation, hex, number, post, reopen, replace, request, row, status, text,
};
...
pub fn binary_exchange(client: &Endpoint, bytes: &[u8]) -> BinaryReply {
    ...
    assert_eq!(
        bytes.len() - split,
        length,
        "no truncated or appended response"
    );
...
pub fn change_bytes(
    ...
    let data = if action == "prepare" {
        part("patch", "text/x-diff", payload)
    } else {
        part("bundle", "application/x-git-bundle", payload)
    };
...
pub fn metadata(base: GitOid, message: &str) -> String {
    format!(
        "{}&expected_commit={base}&author=Editor+%3Ce%40example.invalid%3E&committer=Editor+%3Ce%40example.invalid%3E&timestamp=1&message={message}%0A",
        common(base.algorithm())
    )
}
pub fn candidate_form(base: GitOid, candidate: GitOid) -> String {
    format!(
        "{}&expected_commit={base}&candidate_commit={candidate}",
        common(base.algorithm())
    )
}
pub fn prepare(
...
pub fn blob(client: &Endpoint, format: GitHashAlgorithm, path: &[u8]) -> Reply {
    post(
        ...
        "blob",
        'a',
        &(common(format) + "&path_hex=" + &hex(path)),
        false,
    )
}
