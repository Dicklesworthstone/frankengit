...
fn remote_candidate_is_deterministic_unstaged_and_reviewable_after_restart() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            generation(&node),
            before + 1,
            "only PR creation is a publication"
        );
        assert!(
            node.read_git_object(candidate.binding.commit).is_err(),
            "downloaded candidate is not staged"
        );
        assert_eq!(
            node.runtime()
                .block_on(node.materialize_admission())
                .unwrap()
                .snapshot()
                .refs[&data.target_ref],
            data.target_tip
        );
        let no_transaction = LoopbackReceiveSession::authenticated(
            FOREIGN,
            IdempotencyKey::new(b"read-only-candidate-preparation".to_vec()).unwrap(),
        );
        assert!(matches!(
            node.runtime()
                .block_on(node.recover_transaction_in(&node.request_context(), &no_transaction))
                .unwrap(),
        ...
        assert!(
            String::from_utf8(stale.body)
                .unwrap()
                .contains("preparation_subject_moved")
        );
        server.finish();
        let node = reopen(&config);
        assert_eq!(
            generation(&node),
            before + 3,
            "PR, approval and coupled merge only"
        );
        let selected = node
            .runtime()
            .block_on(node.materialize_admission())
        ...
        assert!(
            metadata.contains("\"published\":false"),
            "preparation receipt never changes meaning after merge"
        );
...
fn both_read_grants_and_complete_envelopes_are_required_before_preparation() {
    ...
    for token in ['a', 'e', 'f', 'c'] {
        // 2..5; equal principals do not union different token grants.
    ...
    wrong = wrong.replace("pull_request_version=1", "pull_request_version=2");
    ...
    assert_eq!(
        prepare(&server.client, 999, 'd', &command, false).status,
        404
    ); // 11
...
fn native_conflicts_and_already_integrated_sources_return_no_candidate_or_decision() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        for conflict in [false, true] {
            let root = Scratch::new();
            let config = root.config(format);
            let (node, data) = non_clean_fixture(&root, format, conflict);
            let before = generation(&node);
            let path = root.0.join("credentials");
            configure(&node, &path);
            ...
            if conflict {
                ...
                assert!(
                    !metadata.contains('\u{fffd}'),
                    "raw paths cannot undergo lossy conversion"
                );
            } else {
                assert!(metadata.contains("\"state\":\"already_up_to_date\""));
            }
...
fn hidden_sources_changed_coordinates_cancellation_and_work_limits_never_stage_objects() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let session = LoopbackReceiveSession::authenticated(
            OWNER,
            IdempotencyKey::new(b"open-local-check".to_vec()).unwrap(),
        );
        ...
        let metadata = MergeMetadata {
            ...
            timestamp: 1,
            message: b"local check\n".to_vec(),
        };
        let prepared = node
            .runtime()
            .block_on(node.prepare_pull_request_bundle_in(
                &request,
                &subject,
                &RefVisibility::new(),
                &metadata,
