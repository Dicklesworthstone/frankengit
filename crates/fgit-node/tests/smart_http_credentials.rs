...
impl Scratch {
    fn new() -> Self {
        let root = std::env::temp_dir().join(format!("fg-http-principals-{}-{}",
...
fn configuration(root: &Scratch, format: GitHashAlgorithm) -> NodeConfig {
    NodeConfig::new(root.0.join("node"), TenantId::from_bytes([0x81; 16]), RepositoryId::from_bytes([0x82; 16]))
...
fn header(node: &OneNode) -> String {
    format!("frankengit-http-credentials-v1 {} {} {}\n",
        TenantId::from_bytes([0x81; 16]), RepositoryId::from_bytes([0x82; 16]), node.repository_incarnation_id())
}
fn row(token: char, principal: u8, scopes: &str) -> String {
    let hash: String = sha256_digest(token.to_string().repeat(64).as_bytes()).iter()
    ...
    fn start(node: OneNode, path: PathBuf, count: usize, allow_receive: bool) -> Self {
        ...
        let worker = thread::spawn(move || {
            let outcome = node.serve_smart_http_with_credentials_file_bounded(
                &listener, GitDaemonServerLimits::try_new(count, 2).unwrap(), &path,
                allow_receive, Duration::from_secs(30),
            );
...
fn exchange(server: &Server, method: &str, suffix: &str, token: char, extra: &str, body: &[u8]) -> Vec<u8> {
    let mut stream = TcpStream::connect(server.address).unwrap();
    stream.set_read_timeout(Some(Duration::from_secs(60))).unwrap();
    stream.set_write_timeout(Some(Duration::from_secs(60))).unwrap();
    let head = format!("{method} {}{suffix} HTTP/1.1\r\nHost: loopback\r\nAuthorization: Bearer {}\r\n{extra}\r\n",
        server.route, token.to_string().repeat(64));
...
fn status(response: &[u8], expected: u16) {
    assert!(response.starts_with(format!("HTTP/1.1 {expected} ").as_bytes()),
        "expected {expected}, got {}", String::from_utf8_lossy(response));
}
fn discovery(server: &Server, token: char, receive: bool) -> Vec<u8> {
    exchange(server, "GET", if receive { "/info/refs?service=git-receive-pack" }
        else { "/info/refs?service=git-upload-pack" }, token, "", &[])
}
fn push_body(format: GitHashAlgorithm, name: &str) -> Vec<u8> {
    ...
    let mut body = encode_packets(&[
        Packet::Data(format!("{zero} {oid} {name}\0report-status object-format={}", format.as_str()).into_bytes()),
        Packet::Flush,
...
fn push(server: &Server, token: char, body: &[u8]) -> Vec<u8> {
    exchange(server, "POST", "/git-receive-pack", token,
        &format!("Content-Type: application/x-git-receive-pack-request\r\nContent-Length: {}\r\nIdempotency-Key: shared-client-key\r\n", body.len()), body)
}
...
fn scoped_principals_rotate_and_revoke_without_restarting_or_republishing() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let original = header.clone() + &row('a', 1, "read") + &row('b', 2, "receive")
        ...
        assert!(committed.windows(27).any(|w| w == b"ok refs/tags/principal-two\n"));
        ...
        assert!(other.windows(29).any(|w| w == b"ok refs/tags/principal-three\n"));
        let rotated = header.clone() + &row('a', 1, "read") + &row('d', 2, "receive")
            + &row('c', 3, "read,receive");
        ...
        let foreign = rotated.replacen(&TenantId::from_bytes([0x81; 16]).to_string(), &"00".repeat(16), 1);
        ...
        assert_eq!(state.basis().generation().get(), before.get() + 2,
            "only the two distinct principals' original pushes publish");
        assert_eq!(state.snapshot().refs.len(), 2);
        for name in [b"refs/tags/principal-two".as_slice(), b"refs/tags/principal-three"] {
            assert!(state.snapshot().refs.contains_key(&RefName::try_new(name).unwrap()));
        }
        node.shutdown().unwrap();
    }
}
