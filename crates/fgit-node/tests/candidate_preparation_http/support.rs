...
pub fn binary_exchange(endpoint: &Endpoint, bytes: &[u8], half_close: bool) -> BinaryReply {
    ...
    assert!(
        ...
        "fixture response exceeded its envelope"
    );
    let split = response
        .windows(4)
    ...
    assert_eq!(
        body.len(),
        length,
        "no truncation or appended second response"
    );
...
pub fn preparation_form(data: &PullRequestData, epoch: PolicyEpoch) -> String {
    format!(
        concat!(
            "object_format={}&pull_request_version=1&policy_epoch={}&source_ref={}&target_ref={}",
        ...
        data.source_tip.algorithm().as_str(),
        epoch.get(),
...
pub fn prepare_bytes(
    ...
    request(
        endpoint,
        "POST",
        &format!("/api/v1/pulls/{number}/prepare"),
        token,
...
pub fn prepare(
    ...
    binary_exchange(
        endpoint,
        &prepare_bytes(endpoint, number, token, body, chunked),
        true,
    )
}
...
pub fn extract(reply: &BinaryReply, data: PullRequestData) -> (Candidate, String) {
    ...
    let bundle = body[header_end + 4..]
        .strip_suffix(format!("\r\n--{boundary}--\r\n").as_bytes())
        .unwrap()
        .to_vec();
    assert_eq!(numeric(&metadata, "bytes"), bundle.len() as u64);
    let checksum: String = sha256_digest(&bundle)
        .iter()
    ...
    let binding = CandidateBinding {
        merge_base: GitOid::from_hex(format, text(&metadata, "merge_base")).unwrap(),
    ...
    assert!(bundle.starts_with(match format {
...
pub fn non_clean_fixture(
    ...
    node.bring_into_service(selected.receipt().generation())
        .unwrap();
    let source = root.0.join("source-conflict");
    ...
    let tree = |content: &[u8]| {
        ...
        loose(
            ...
            GitObjectKind::Tree,
            "tree",
            &[b"100644 file\xff.txt\0".as_slice(), blob.as_bytes()].concat(),
        )
    };
    ...
    (
        ...
        PullRequestData {
            ...
            source_tip: theirs,
            target_tip: ours,
            title: "Non-clean candidate".into(),
            body: String::new(),
        },
    )
}
...
fn candidate_forms_preserve_reference_bytes() {
    ...
    for (raw, encoded) in [
        (
            b"refs/heads/a+b&c%d".as_slice(),
            "refs%2Fheads%2Fa%2Bb%26c%25d",
        ),
        (
            b"refs/heads/nonutf8\xff".as_slice(),
            "refs%2Fheads%2Fnonutf8%FF",
        ),
    ] {
        ...
        for body in [
            common(&candidate),
            preparation_form(&candidate.data, candidate.epoch),
        ] {
