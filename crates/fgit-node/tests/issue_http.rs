...
fn row(token: char, principal: u8, scope: &str) -> String {
    let digest: String = sha256_digest(token.to_string().repeat(64).as_bytes())
        .iter()
        .map(|b| format!("{b:02x}"))
        .collect();
...
fn grants(node: &OneNode, path: &Path) -> String {
    ...
    replace(
        path,
        &(header.clone()
            + &row('a', 0xa1, "issues-read")
            + &row('b', 0xb1, "issues-write")
            + &row('c', 0xc1, "issues-read,issues-write")
            + &row('d', 0xd1, "read,receive")),
    );
    header
}
struct Server {
    ...
    fn start(node: OneNode, path: PathBuf, requests: usize, issues: bool) -> Self {
        ...
        let worker = thread::spawn(move || {
            ...
            let served = if issues {
                node.serve_git_and_issue_http_with_credentials_file_bounded(
                    &listener,
                    limits,
                    &path,
                    false,
                ...
                node.serve_smart_http_with_credentials_file_bounded(
                    &listener,
                    limits,
                    &path,
                    false,
...
fn get(server: &Server, suffix: &str, token: char) -> Reply {
    exchange(
        server.address,
        &request(server, "GET", suffix, token, "", b""),
        false,
    )
}
fn post(server: &Server, suffix: &str, token: char, key: &str, body: &str, chunked: bool) -> Reply {
    ...
    exchange(
        server.address,
        &request(server, "POST", suffix, token, &headers, &wire),
        false,
    )
}
fn status(reply: &Reply, expected: u16) {
    assert_eq!(reply.status, expected, "{}", reply.raw);
}
...
fn full_issue_lifecycle_has_stable_retries_exact_pages_and_reopened_authority_state() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let open =
            "expected_version=0&title=Alpha&body=hello+%F0%9F%A6%80%0A%22quoted%22&label=z&label=a";
        let original = post(
            &server,
        ...
        committed(&post(
            &server,
            "/api/v1/issues/1/comment",
            'b',
            "comment-one",
            "expected_version=1&body=first+comment",
            false,
        )); // 4
        let retained_page = get(
            &server,
            &format!("/api/v1/issues/1?limit=1&expected_head={old}"),
            'a',
        ); // 5
        ...
        committed(&post(
            &server,
            "/api/v1/issues/1/edit",
            'b',
            "edit-one",
            "expected_version=2&title=Beta&body=&clear_labels=true",
            true,
        )); // 6
        committed(&post(
            &server,
            "/api/v1/issues/1/close",
            'b',
            "close-one",
            "expected_version=3",
            false,
        ...
        committed(&post(
            ...
            'c',
            "open-two",
            "expected_version=0&title=Second&body=",
            false,
        )); // 9
        ...
        let continuation = get(
            &server,
            &format!(
                "/api/v1/issues?limit=1&after=1&expected_head={}",
                token(&page)
            ),
            'a',
        ); // 11
        ...
        let second = get(
            &server,
            &format!("/api/v1/issues/1?limit=2&after_version=2&expected_head={history_token}"),
            'a',
        ); // 13
        status(&second, 200);
        ...
        let last = get(
            &server,
            &format!("/api/v1/issues/1?limit=2&after_version=4&expected_head={history_token}"),
            'a',
        ); // 14
        status(&last, 200);
        ...
        let stale = post(
            ...
            'b',
            "stale-edit",
            "expected_version=1&title=Overwrite",
            false,
        ); // 15
        status(&stale, 409);
        ...
        let conflict = post(
            ...
            'b',
            "open-one",
            "expected_version=0&title=Changed&body=",
            false,
        ); // 17
        status(&conflict, 409);
        ...
        assert_eq!(
            ...
            before.get() + 7,
            "six commits and one refusal, no retry publications"
        );
...
fn scope_denial_spoofed_identity_and_incomplete_http_never_publish_issue_state() {
    ...
    status(
        &get(&server, "/info/refs?service=git-upload-pack", 'c'),
        403,
    ); // 4: issue rights are not Git rights.
    ...
    status(
        &post(
            ...
            'c',
            "spoof",
            "expected_version=0&title=t&body=b&principal=admin",
            false,
        ),
        400,
    ); // 6
    ...
    let wrong = format!(
        "GET /other.git/api/v1/issues HTTP/1.1\r\nHost: local\r\n{}\r\n",
        auth('a')
    );
...
fn rotating_issue_credentials_preserves_the_principals_canonical_retry() {
    ...
    replace(
        &path,
        &(header + &row('a', 0xa1, "issues-read") + &row('f', 0xc1, "issues-write")),
    );
    status(
        &post(
            &server,
