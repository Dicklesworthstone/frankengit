...
fn reconcile(
    node: &OneNode,
    floor: Option<&GenerationActivation>,
...
fn add_file(node: &OneNode, base: GitOid, path: &str, body: &[u8], key: &[u8]) -> GitOid {
    ...
    let metadata = MergeMetadata {
        ...
        timestamp: 3,
        message: b"symbol reconciliation\n".to_vec(),
    };
        ...
        for _ in 0..2 {
            let observed = node
                ...
                .block_on(node.reconcile_source_symbol_index_guarded_local_in(
                    ...
                    Default::default(),
                    Default::default(),
                    &mut |_| panic!("current index cannot publish"),
                ))
                .unwrap();
...
fn checkpoint_failure_never_initializes_or_rolls_back_an_index() {
    ...
    let first = reconcile(&node, None).unwrap();
    let higher = GenerationActivation {
        generation_id: first.1.generation_id,
        authority_generation: HeadGeneration::try_new(2).unwrap(),
    };
    assert!(matches!(
        reconcile(&node, Some(&higher)),
        Err(AccessError::Generation(
            GenerationAuthorityError::CheckpointUnresolved
        ))
    ));
    let fresh = Scratch::new();
    let (fresh_node, _) = symbols(&fresh, GitHashAlgorithm::Sha1);
    assert!(matches!(
        reconcile(&fresh_node, Some(&first.1)),
        Err(AccessError::Generation(
            GenerationAuthorityError::CheckpointUnresolved
        ))
    ));
...
fn genesis_and_refresh_barriers_preserve_original_candidates_on_failure_and_cancel() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for pass in 0..2 {
            ...
            assert!(
                matches!(cancelled,Err(AccessError::Publication { candidate,.. })
...
fn malformed_source_pins_limits_and_unpolled_operations_never_publish() {
    ...
    drop(node.reconcile_source_symbol_index_local_in(
        &request,
        &reference,
        None,
        None,
    ...
    assert!(matches!(
        ...
        Err(AccessError::Index(data::Error::Source(_)))
    ));
    add_file(
        &node,
        commit,
    ...
    assert!(matches!(
        ...
        Err(AccessError::Index(data::Error::Cancelled))
    ));
    assert!(matches!(
        node.runtime()
            .block_on(node.recover_source_symbol_index_local_in(
...
fn forge_only_changes_are_reconciled_without_implicit_query_maintenance() {
    ...
    assert_ne!(second.0.head, first.0.head);
    assert_eq!(
        search(&node).unwrap().generation,
        *second.1.generation_id.as_internal_object_id()
    );
    assert_eq!(reconcile(&node, Some(&first.1)).unwrap(), second);
    assert_eq!(generation(&node), canonical);
    node.shutdown().unwrap();
