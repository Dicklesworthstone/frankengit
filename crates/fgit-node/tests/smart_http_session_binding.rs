...
fn key_reuse(error: NodeSmartHttpRefusal) {
    ...
    assert!(
        interrupted.completed_commands().is_empty(),
        "no child was admitted in this attempt"
    );
    assert!(
        matches!(interrupted.admission_error(), AdmissionError::Seal(error)
...
fn changed_shape_content_and_atomicity_cannot_repurpose_a_session_key_after_reopen() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(
            original.commands.iter().all(|command| matches!(
        ...
        for (offered, byte, atomic) in [
            ...
            key_reuse(
                perform(&node, format, &offered, byte, b"whole-session", atomic).unwrap_err(),
            );
            assert_eq!(
                state(&node),
                published,
                "key conflict cannot move the head or refs"
            );
        }
        ...
        .unwrap();
        assert!(matches!(
            fresh.commands[0].terminal.outcome,
            DecisionOutcome::Committed { .. }
        ));
...
fn an_atomic_key_cannot_be_reused_as_a_non_atomic_session() {
    ...
    assert_eq!(state(&node), published);
    assert_eq!(
        perform(&node, format, &names, b'x', b"atomic-session", true).unwrap(),
        original
    );
    node.shutdown().unwrap();
}
