...
fn ordinary() -> SymbolQuery {
    query(b"Thing", SymbolMatchMode::Prefix, &[], &[])
}
fn build(node: &OneNode, previous: Option<&GenerationActivation>) -> GenerationActivation {
    node.runtime()
        .block_on(node.build_source_symbol_index_local_in(
            ...
            None,
            None,
            previous.map(|p| p.generation_id),
            Default::default(),
        ))
        .unwrap()
        .1
...
fn add_files(node: &OneNode, base: GitOid, files: &[(&[u8], &[u8])], key: &[u8]) -> GitOid {
    ...
    for (path, body) in files {
        let old = quoted('a', path);
        let new = quoted('b', path);
...
fn native_persisted_symbols_match_live_scans_and_survive_reopen_in_both_formats() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(report.indexed_declarations, 6);
        assert_eq!(report.unsupported_language_files, 5);
        assert_eq!(report.non_regular_entries, 2);
        assert_eq!(report.indexed_source_bytes, A.len() + B.len() + RAW.len());
        assert_eq!(report.tables_read, 3);
        assert_eq!(
            report.generation,
            *activation.generation_id.as_internal_object_id()
        );
        let raw = search(
            &node,
            &query(
        ...
        .unwrap();
        assert_eq!(again.source, report.source);
...
fn shared_payload_source_file_and_result_bounds_have_permitted_twins() {
    ...
    assert_eq!(limited.matches.len(), 4);
    let exact = search(
        &node,
        &ordinary(),
        SearchLimits {
    ...
    .unwrap();
    assert!(report.complete);
    assert_eq!(report.tables_read, 1);
    assert_eq!(report.matches.len(), 1);
    let none = search(
        &node,
        &query(b"Missing", SymbolMatchMode::Exact, &[], &[]),
        Default::default(),
        data::MAX_INDEX_BYTES,
    )
    .unwrap();
    assert!(none.complete && none.matches.is_empty());
    node.shutdown().unwrap();
}
#[test]
fn write_ahead_barrier_precedes_effects_and_retains_candidate_after_cancellation() {
    ...
    assert!(matches!(
        error,
        AccessError::Source(NodeWorkspaceRefusal::RefUnavailable)
    ));
    ...
    let error = node
        ...
        .unwrap_err();
    assert!(
        matches!(error,AccessError::Publication{candidate:id,..}if id==*candidate.as_internal_object_id())
    );
...
fn same_source_rebuilds_keep_independent_checkpoints_and_recover_old_candidates() {
    ...
    let wrong = GenerationActivation {
        ...
        authority_generation: HeadGeneration::try_new(second.authority_generation.get() + 1)
            .unwrap(),
    };
    assert!(
        node.runtime()
    ...
    assert!(
        matches!(node.runtime().block_on(node.recover_source_symbol_index_local_in(&node.request_context(),&reference(),
...
fn invalid_refs_formats_cancelled_and_unpolled_operations_do_not_initialize_an_index() {
    ...
    let future = node.build_source_symbol_index_local_in(
        &request,
        &pinned_ref,
        None,
        None,
...
fn http_distinguishes_an_unbuilt_index_from_an_authenticated_empty_inventory() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        status(&missing, 409);
        assert!(missing.body.contains("symbol_index_uninitialized"));
        server.finish();
        let node = reopen(&config);
        let first = build(&node, None);
        ...
        for chunked in [false, true] {
            ...
            status(&reply, 200);
            assert!(reply.body.contains("\"complete\":true"));
            assert!(reply.body.contains("\"matches\":[]"));
            assert_eq!(number(&reply.body, "source_blobs_read"), 0);
            assert_eq!(number(&reply.body, "indexed_files"), 0);
        }
        server.finish();
        let node = reopen(&config);
        let result = search(
...
fn authenticated_indexed_http_preserves_native_results_scopes_budgets_and_revocation() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(first.body.contains("rust-declaration-tables-v1"));
        let pinned = format!(
            "{}&expected_head={}&expected_commit={}&minimum_index_token={}&minimum_index_number={}",
            form(format),
            token(&first),
            text(&first.body, "source_commit"),
            text(&first.body, "index_token"),
            number(&first.body, "index_number")
        );
        let next = post(&server.client, "search-symbols-index", 'a', &pinned, true);
        status(&next, 200);
        assert_eq!(next.body, first.body);
        let higher = format!(
            "{}&minimum_index_token={}&minimum_index_number={}",
            form(format),
            text(&first.body, "index_token"),
            number(&first.body, "index_number") + 1
        );
        let refused = post(&server.client, "search-symbols-index", 'a', &higher, false);
        status(&refused, 409);
        assert!(refused.body.contains("index_checkpoint_unavailable"));
        status(
            &post(
                &server.client,
                "search-symbols-index",
        ...
        let bytes = request(
            &server.client,
            "/api/v1/source/search-symbols-index",
            'a',
            &format!(
...
fn forge_only_write_invalidates_http_index_without_implicitly_rebuilding_it() {
    ...
    status(&stale, 409);
    assert!(stale.body.contains("symbol_index_stale"));
    server.finish();
    let node = reopen(&config);
    let before = generation(&node);
    ...
    assert_eq!(number(&current.body, "source_blobs_read"), 0);
    server.finish();
}
#[test]
fn operator_build_query_and_original_candidate_recovery_use_existing_nodes() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        node.shutdown().unwrap();
        let run = |tail: &[&str]| {
            std::process::Command::new(env!("CARGO_BIN_EXE_fg-symbol-index"))
        ...
        let built = run(&["build", "genesis"]);
        assert!(
            built.status.success(),
            "{}",
            String::from_utf8_lossy(&built.stderr)
        );
        let body = String::from_utf8(built.stdout).unwrap();
        let index = text(&body, "index_token");
        let query = run(&["query", "prefix", "Thing"]);
        assert!(
            query.status.success(),
            "{}",
            String::from_utf8_lossy(&query.stderr)
        );
        let body = String::from_utf8(query.stdout).unwrap();
        assert_eq!(number(&body, "source_blobs_read"), 0);
        assert!(body.contains(&hex(b"ThingWorker")));
        let recovery = run(&["recover", index]);
