...
fn commit(root: &Path, format: GitHashAlgorithm, tree: GitOid, parent: Option<GitOid>, label: &str) -> GitOid {
...
fn rename_fixture(root: &Scratch, format: GitHashAlgorithm, divergent: bool) -> (OneNode, PullRequestData) {
    ...
    let base_tree = tree(&source, format, &[("100644", b"keep", keep), ("100644", b"text", original)]);
    let base = commit(&source, format, base_tree, None, "base");
    let target_tree = if divergent {
        tree(&source, format, &[("100644", b"different", original), ("100644", b"keep", keep)])
    } else {
        tree(&source, format, &[("100644", b"keep", keep), ("100755", b"text", edited)])
    };
    let nested = tree(&source, format, &[("100644", DESTINATION, original)]);
    let source_tree = tree(&source, format, &[("100644", b"keep", keep), ("40000", b"nested", nested)]);
...
fn assert_published_tree(node: &OneNode, candidate: &Candidate) {
    ...
    let ParsedObject::Commit(parsed) = parse_object_body(
        GitObjectKind::Commit, commit.payload(), AcceptanceProfile::StrictCreate, &limits,
    ).unwrap() else { panic!("commit required"); };
    ...
    assert_eq!(parents, [candidate.data.target_tip, candidate.data.source_tip]);
    let tree = node.read_git_object(oid(parsed.tree_reference().unwrap())).unwrap();
    let ParsedObject::Tree(entries) = parse_object_body(
        GitObjectKind::Tree, tree.payload(), AcceptanceProfile::StrictCreate, &limits,
    ...
    let hex: String = entries[1].object_id.iter().map(|byte| format!("{byte:02x}")).collect();
    let nested = node.read_git_object(GitOid::from_hex(format, &hex).unwrap()).unwrap();
    let ParsedObject::Tree(entries) = parse_object_body(
        GitObjectKind::Tree, nested.payload(), AcceptanceProfile::StrictCreate, &limits,
    ).unwrap() else { panic!("nested tree required"); };
...
fn http_exact_rename_preserves_cross_directory_edits_modes_review_and_retry_after_restart() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(prepare(&server.client, 1, 'd', &(command.clone() + "&profile=path-merge-v1"), true), legacy); // 4
        ...
        assert_eq!(prepare(&server.client, 1, 'd', &exact_command, true), original); // 6
        assert_eq!(server.finish().accepted_sessions(), 6);
        let node = reopen(&config);
        assert_eq!(generation(&node), before + 1, "only PR creation may publish");
        assert!(node.read_git_object(candidate.binding.commit).is_err());
        let sentinel = LoopbackReceiveSession::authenticated(FOREIGN,
        ...
        assert!(matches!(node.runtime().block_on(node.recover_transaction_in(
            &node.request_context(), &sentinel,
        )).unwrap(), RequestRecovery::KeyNotObserved));
        let server = Server::start(node, &path, 6, true, true);
        assert_eq!(prepare(&server.client, 1, 'd', &exact_command, true), original); // 1
        accepted(&send(&server.client, "reviews/approve", 'b', "approve-rename",
            &review_form(&candidate, 0), Some(&candidate.bundle), true)); // 2
        let command = merge_form(&candidate, &[REVIEWER]);
        let published = send(&server.client, "merge", 'c', "merge-rename",
            &command, Some(&candidate.bundle), false); // 3
        accepted(&published);
        assert_eq!(send(&server.client, "merge", 'c', "merge-rename", &command, None, true), published); // 4
        let current = get(&server.client, "/api/v1/pulls/1", 'a'); // 5
        status(&current, 200);
        assert!(current.body.contains("\"state\":\"merged\""));
        error(&prepare(&server.client, 1, 'd', &exact_command, false), 409, "preparation_subject_moved"); // 6
        assert_eq!(server.finish().accepted_sessions(), 6);
        let node = reopen(&config);
        assert_eq!(generation(&node), before + 3, "PR, independent approval, coupled merge only");
        let view = node.runtime().block_on(node.materialize_admission()).unwrap();
        assert_eq!(view.snapshot().refs[&data.target_ref], candidate.binding.commit);
...
fn http_divergent_renames_return_typed_conflict_without_fallback_or_path_disclosure() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (candidate, _) = extract(&prepare(&server.client, 1, 'd', &command, false), data.clone()); // 3
...
fn http_profile_selection_cannot_bypass_credentials_subject_or_exact_body_rules() {
    ...
    let command = open_and_command(&server, &data); // 1, 2
    for value in ["", "ort", "exact-renames-v2", "EXACT-RENAMES-V1"] { // 3..6
        error(&prepare(&server.client, 1, 'd', &(command.clone() + "&profile=" + value), false),
            400, "unsupported_merge_profile");
    }
    // Short duplicate envelope isolates uniqueness from the field-count limit.
    error(&prepare(&server.client, 1, 'd', "profile=path-merge-v1&pr%6ffile=exact-renames-v1", true),
        400, "duplicate_field"); // 7
    for (token, code, expected) in [('a', "forbidden", 403), ('z', "unauthorized", 401)] { // 8, 9
        let headers = "Content-Type: application/x-www-form-urlencoded\r\nContent-Length: 1024\r\nExpect: 100-continue\r\n";
        let denied = binary_exchange(&server.client,
            &request(&server.client, "POST", "/api/v1/pulls/1/prepare", token, headers, &[]), false);
    ...
    error(&prepare(&server.client, 1, 'd', &stale, true), 409, "preparation_subject_moved"); // 10
    let truncated = request(&server.client, "POST", "/api/v1/pulls/1/prepare", 'd',
        &format!("Content-Type: application/x-www-form-urlencoded\r\nContent-Length: {}\r\n", exact.len() + 1), exact.as_bytes());
    let incomplete = binary_exchange(&server.client, &truncated, true); // 11
    assert_eq!(incomplete.status, 400);
    assert!(std::str::from_utf8(&incomplete.body).unwrap().contains("\"outcome_unknown\":false"));
    let resolved = command + &format!("&merge_base={}&resolution=74657874:ours&profile=exact-renames-v1", "c".repeat(40));
    error(&binary_exchange(&server.client, &request(&server.client, "POST", "/api/v1/pulls/1/resolve", 'd',
        &format!("Content-Type: application/x-www-form-urlencoded\r\nContent-Length: {}\r\n", resolved.len()), resolved.as_bytes()), true),
        400, "unknown_preparation_field"); // 12
