...
use fgit_forge::preparation::rebase::{
    EmptyCommitPolicy, RebaseCommitter, RebasePreparation, RebaseRequest,
};
...
fn branch(name: &[u8]) -> RefName {
    RefName::try_new(name).unwrap()
}
fn prepared(
    node: &OneNode,
    history: &support::History,
    empty: EmptyCommitPolicy,
) -> (GitOid, Vec<u8>) {
    ...
    let result = node
        ...
        .block_on(node.prepare_rebase_bundle_in(
            ...
            None,
            &RebaseCommitter {
                identity: "Reviewer <r@example.invalid>".into(),
                timestamp: 9,
            },
            Default::default(),
        ))
        .unwrap();
    let RebasePreparation::Clean(plan) = result.outcome else {
        panic!("clean fixture");
    };
...
fn actual_commits_and_every_diff_survive_restart_without_staging() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let inspected = node
            .runtime()
            .block_on(node.inspect_rebase_bundle_in(
                &request,
                &branch(b"refs/heads/topic"),
                &branch(b"refs/heads/main"),
                history.source,
        ...
        for commit in &inspected.commits {
            assert_eq!(
                git_object_id(format, GitObjectKind::Commit, &commit.body),
                commit.id
            );
            assert!(
                commit
                    .body
                    .windows(b"5 -0330".len())
                    .any(|p| p == b"5 -0330")
            );
...
fn complete_scope_exact_tips_and_corrupt_packs_fail_without_an_inspection() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for option in 0..3 {
            ...
            match option {
                0 => options.paths.push(b"onto".to_vec()),
                1 => options.mode = ComparisonMode::MergeBase,
...
fn pack(objects: &Objects, format: GitHashAlgorithm, onto: GitOid, candidate: GitOid) -> Vec<u8> {
    ...
    .plan_selected(
        objects,
        &objects.0.keys().copied().collect::<Vec<_>>(),
        &mut || true,
    )
    .unwrap();
    let (pack, _) = PackWriter::new(limits)
        .write(&planned, &mut || true)
        .unwrap();
...
fn source_tree(node: &OneNode, source: GitOid) -> (GitOid, Vec<u8>) {
    ...
    let tree = GitOid::from_hex(
        source.algorithm(),
        text.lines().next().unwrap().strip_prefix("tree ").unwrap(),
    )
    .unwrap();
...
fn transient_commit_changes_are_visible_even_when_final_source_tree_is_identical() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        transient.extend_from_slice(b"100644 zz-transient\0");
        transient.extend_from_slice(blob.as_bytes());
        let changed = insert(&mut objects, format, GitObjectKind::Tree, transient);
        let first = insert(
            &mut objects,
            format,
            GitObjectKind::Commit,
            commit(changed, history.onto, "introduce"),
        );
        let candidate = insert(
            &mut objects,
            format,
            GitObjectKind::Commit,
            commit(tree, first, "remove"),
        );
        ...
        let inspected = node
            ...
            .unwrap();
        assert!(
            inspected.comparisons[0].entries.is_empty(),
            "final source tree is exactly unchanged"
        );
        for comparison in &inspected.comparisons[1..] {
            assert!(
                comparison
                    .entries
                    .iter()
                    .any(|entry| entry.path == b"zz-transient")
            );
        }
        insert(
            &mut objects,
            format,
            GitObjectKind::Blob,
            b"unrelated hidden transport payload".to_vec(),
        );
        assert!(
            node.runtime()
                .block_on(node.inspect_rebase_bundle_in(
                    ...
                    history.onto,
                    candidate,
                    &pack(&objects, format, history.onto, candidate),
                    &Default::default(),
                    None,
                    &ReviewOptions::default()
                ))
...
fn old_source_objects_are_not_implicit_pack_prerequisites_and_empty_series_are_real() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let candidate = insert(
            ...
            format,
            GitObjectKind::Commit,
            commit(tree, history.onto, "missing source objects"),
        );
        assert!(
            node.runtime()
                .block_on(node.inspect_rebase_bundle_in(
                ...
                .is_err(),
            "the omitted tree is admitted but outside onto history"
        );
        node.shutdown().unwrap();
        let root = Scratch::new();
        let (node, history) = fixture(&root, format, 2);
        ...
        assert_eq!(result.bundle.pack_objects, 0);
        assert_eq!(result.comparisons.len(), 1);
        assert_eq!(result.comparisons[0].requested_before, history.source);
        node.shutdown().unwrap();
    }
}
