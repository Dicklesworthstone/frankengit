...
fn create(format: GitHashAlgorithm, name: &str, new: GitOid) -> String {
    format!(
        "object_format={}&ref={name}&new_commit={new}",
        format.as_str()
    )
}
fn update(format: GitHashAlgorithm, name: &str, old: GitOid, new: GitOid) -> String {
    format!(
        "object_format={}&ref={name}&expected_commit={old}&new_commit={new}",
        format.as_str()
    )
}
fn delete(format: GitHashAlgorithm, name: &str, old: GitOid) -> String {
    format!(
        "object_format={}&ref={name}&expected_commit={old}",
        format.as_str()
    )
}
...
fn wire(
    ...
    request(
        client,
        &format!("/api/v1/source/branches/{action}"),
        token,
        &format!(
...
fn list(client: &Endpoint, format: GitHashAlgorithm, extra: &str) -> Reply {
    post(
        ...
        "refs",
        'a',
        &format!("object_format={}{extra}", format.as_str()),
        false,
    )
}
...
fn contains_ref(reply: &Reply, name: &str, oid: GitOid) -> bool {
    reply.body.contains(&format!(
        "\"ref\":\"{name}\",\"ref_hex\":\"{}\",\"object_id\":\"{oid}\"",
        hex(name.as_bytes())
    ))
}
...
fn branch_lifecycle_and_original_transactions_survive_rename_delete_and_reopen() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        status(&initial, 200);
        assert!(contains_ref(&initial, "refs/heads/main", tip));
        ...
        status(&first, 200);
        assert_eq!(text(&first.body, "next_after"), "refs/heads/main");
        ...
        let stale = list(
            ...
            format,
            &format!(
                "&namespace=branches&after=refs/heads/main&expected_head={}",
                token(&first)
            ),
        ); // 5
        status(&stale, 409);
        ...
        accepted(&renamed);
        assert_eq!(renamed.body.matches("\"force\":false").count(), 2);
        assert_eq!(
            change(&server.client, "rename", "topic-rename", &renaming, true),
            renamed
        ); // 7
        let rows = list(&server.client, format, ""); // 8
        status(&rows, 200);
        assert!(contains_ref(&rows, "refs/heads/renamed", tip));
        ...
        accepted(&removed);
        assert_eq!(
            change(&server.client, "create", "topic-create", &creation, false),
            created
        ); // 10
        assert_eq!(
            change(&server.client, "rename", "topic-rename", &renaming, false),
            renamed
        ); // 11
        let conflict = change(
            &server.client,
            "create",
            "topic-create",
            &create(format, "refs/heads/different", tip),
            false,
        ); // 12
        status(&conflict, 409);
        assert!(conflict.body.contains("idempotency_key_reuse"));
        let tags = list(&server.client, format, "&namespace=tags"); // 13
        status(&tags, 200);
        assert!(tags.body.contains("\"refs\":[]"));
        let final_page = list(&server.client, format, ""); // 14
        status(&final_page, 200);
        assert!(contains_ref(&final_page, "refs/heads/main", tip));
        ...
        assert_eq!(
            ...
            original.selected_closure().closure()
        );
        let server = BranchServer::start(node, &path, 3);
        assert_eq!(
            change(&server.client, "delete", "topic-delete", &removal, false),
            removed
        );
        assert_eq!(
            change(&server.client, "update", "topic-update", &advance, true),
            advanced
        );
...
fn conflicting_and_competing_renames_never_publish_half_a_move() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        accepted(&change(
            ...
            "create",
            "source",
            &create(format, "refs/heads/source", parent),
            false,
        )); // 1
        accepted(&change(
            &server.client,
            "create",
            "occupied",
            &create(format, "refs/heads/occupied", tip),
            false,
        )); // 2
        ...
        status(&rows, 200);
        assert!(contains_ref(&rows, "refs/heads/source", parent));
        assert!(contains_ref(&rows, "refs/heads/occupied", tip));
        assert_eq!(
            change(&server.client, "rename", "collision", &collision, false),
            collided
        ); // 5
        refused(&change(
            &server.client,
            "rename",
            "stale-tip",
            &rename(format, "refs/heads/source", "refs/heads/unused", tip),
            false,
        )); // 6
        ...
        for destination in ["refs/heads/winner-a", "refs/heads/winner-b"] {
            ...
            workers.push(thread::spawn(move || {
                ...
                (
                    destination,
                    form.clone(),
                    change(&client, "rename", destination, &form, true),
                )
            }));
        ...
        status(&rows, 200);
        assert!(contains_ref(&rows, winner, parent));
        assert!(!rows.body.contains("\"ref\":\"refs/heads/source\""));
        for (destination, form, reply) in &results {
            assert_eq!(
                change(&server.client, "rename", destination, form, false),
                *reply
            ); // 10, 11
        }
        server.finish();
        let node = reopen(&config);
        assert_eq!(generation(&node), before + 6);
        let selected = node
            .runtime()
            .block_on(node.materialize_admission())
            .unwrap();
        ...
        assert_eq!(
            selected.snapshot().refs[&RefName::try_new(winner.as_bytes()).unwrap()],
            parent
        );
        assert_eq!(
            selected.snapshot().refs[&RefName::try_new(b"refs/heads/occupied").unwrap()],
            tip
        );
        node.shutdown().unwrap();
    }
...
fn client_lost_rename_receipts_recover_read_only_and_keys_remain_principal_scoped() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        accepted(&change(
            ...
            "create",
            "create",
            &create(format, "refs/heads/source", tip),
            false,
        ));
        ...
        replace(
            &path,
            &(header.clone() + &row('a', OWNER, "read") + &row('c', OWNER, "outcomes-read")),
        );
        ...
        status(&rows, 200);
        assert!(contains_ref(&rows, "refs/heads/recovered", tip));
        status(
            &exchange(
                &readonly.client,
                &wire(&readonly.client, "rename", 'b', "lost-rename", &form, false),
                true,
            ),
            401,
        ); // 3
        ...
        status(
            &exchange(
                &readonly.client,
                &wire(&readonly.client, "rename", '9', "lost-rename", &form, false),
                true,
            ),
            403,
        ); // 4
        ...
        let retried = exchange(
            &server.client,
            &wire(&server.client, "rename", '9', "lost-rename", &form, true),
            true,
        ); // 1
        accepted(&retried);
        assert_eq!(text(&retried.body, "tx_id"), text(&recovered.body, "tx_id"));
        assert_eq!(
            number(&retried.body, "decision_sequence"),
            number(&recovered.body, "decision_sequence")
        );
        let other = exchange(
            &server.client,
            &wire(
                ...
                'd',
                "lost-rename",
                &create(format, "refs/heads/foreign", tip),
                false,
            ),
            true,
        ); // 2
        accepted(&other);
        assert_ne!(text(&other.body, "tx_id"), text(&retried.body, "tx_id"));
...
fn scopes_default_branch_native_kind_and_complete_envelopes_precede_publication() {
    ...
    status(&list(&disabled.client, format, ""), 403);
    disabled.finish();
    let node = reopen(&config);
    let server = BranchServer::start(node, &path, 13);
    ...
    status(
        &post(&server.client, "refs", 'b', "object_format=sha256", false),
        403,
    ); // 2
    ...
    status(
        &change(
            ...
            "create",
            "tag",
            &create(format, "refs/tags/no", tip),
            false,
        ),
        400,
    ); // 5
    let reply = change(
        ...
        "delete",
        "default-delete",
        &delete(format, "refs/heads/main", tip),
        false,
    ); // 6
    status(&reply, 409);
    assert!(reply.body.contains("branch_operation_refused"));
    status(
        &change(
            &server.client,
            "rename",
            "default-rename",
            &rename(format, "refs/heads/main", "refs/heads/other", tip),
            false,
        ),
        409,
    ); // 7
    ...
    let reply = change(
        ...
        "create",
        "blob",
        &create(format, "refs/heads/blob", blob),
        false,
    ); // 8
    status(&reply, 409);
    assert!(reply.body.contains("branch_target_not_commit"));
    ...
    status(
        &post(&server.client, "tree", 'a', &common(format), false),
        200,
    ); // 12: existing source reader unaffected.
    let rows = list(&server.client, format, ""); // 13
    status(&rows, 200);
    assert!(contains_ref(&rows, "refs/heads/main", tip));
