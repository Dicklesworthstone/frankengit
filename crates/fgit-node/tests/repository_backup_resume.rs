...
fn fixture(scratch: &Scratch) -> (PathBuf, String) {
    ...
    let (node, _) = OneNode::init(config(&root)).unwrap();
    ...
    hash.update(&fs::read(&backup).unwrap());
...
fn command_resume_recovers_a_lost_success_response_without_republishing_the_head() {
    ...
    let (input, pin) = fixture(&scratch);
    let root = scratch.0.join("target");
    let absent = scratch.0.join("absent");
    refused(restore(&input, &absent, &pin, "991", true));
    assert!(!absent.exists());
    success(restore(&input, &root, &pin, "991", false));
    let original = head(&root);
    refused(restore(&input, &root, &pin, "991", false));
    let retry = success(restore(&input, &root, &pin, "991", true));
    assert!(retry.contains("\"resume_requested\":true"));
    assert!(retry.contains("\"already_published\":true"));
    ...
    let (node, _) = OneNode::init(config(&legacy)).unwrap();
    node.shutdown().unwrap();
    let before = head(&legacy);
    assert!(
        refused(restore(&input, &legacy, &pin, "991", true)).contains("missing restore intent")
    );
...
fn a_live_restore_lock_excludes_the_command_but_does_not_leave_a_stale_lock() {
    ...
    assert_eq!(head(&root), original);
    drop(lock);
    success(restore(&input, &root, &pin, "991", true));
    assert_eq!(head(&root), original);
}
