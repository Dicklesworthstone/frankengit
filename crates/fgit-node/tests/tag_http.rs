    ...
    fn start(node: OneNode, path: &Path, count: usize, source: bool, receive: bool) -> Self {
        ...
        let worker = thread::spawn(move || {
            ...
            let served = if source {
                ...
                node.serve_repository_http_with_credentials_file_bounded(
                    &listener,
                    limits,
                    &path,
                    receive,
...
fn annotation(
    ...
    format!(
        "{}&target={target}&target_kind={kind}&tagger=Release+Bot+%3Crelease%40example.invalid%3E&timestamp=0&message_hex={}",
        selection(format, reference),
        hex(message)
    )
}
...
fn wire(
    ...
    request(
        client,
        &format!("/api/v1/source/tags/{action}"),
        token,
        &headers,
        &body,
    )
...
fn send(
    ...
    exchange(
        client,
        &wire(client, action, token, key, form, chunked),
        true,
    )
}
fn inspect(client: &Endpoint, format: GitHashAlgorithm, reference: &[u8], extra: &str) -> Reply {
    send(
        ...
        'a',
        None,
        &(selection(format, reference) + extra),
        false,
    )
}
fn committed(reply: &Reply) {
    status(reply, 200);
    assert!(reply.body.contains("\"type\":\"tag_publication\""));
...
fn nested_tags_preserve_native_bytes_and_exact_delete_retries_do_not_resurrect() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        committed(&created);
        assert_eq!(text(&created.body, "new_object"), inner.to_string());
        let read = inspect(&server.client, format, INNER, ""); // 2
        status(&read, 200);
        assert_eq!(text(&read.body, "body_hex"), hex(&body));
        ...
        committed(&send(
            ...
            'b',
            Some("alias"),
            &format!("{}&target={inner}", selection(format, alias)),
            false,
        )); // 4
        ...
        status(&outer_read, 200);
        assert_eq!(number(&outer_read.body, "annotation_count"), 2);
        assert_eq!(text(&outer_read.body, "object_id"), outer.to_string());
        assert_eq!(text(&outer_read.body, "peeled_object"), main.to_string());
        assert!(outer_read.body.contains(&hex(&body)));
        assert!(outer_read.body.contains(&hex(&outer_body)));
        let alias_read = inspect(&server.client, format, alias, ""); // 7
        status(&alias_read, 200);
        assert_eq!(number(&alias_read.body, "annotation_count"), 1);
        status(
            &inspect(&server.client, format, outer_name, "&max_tags=1"),
            413,
        ); // 8
        ...
        assert_eq!(generation(&node), before + 5);
        let selected = node
            .runtime()
            .block_on(node.materialize_admission())
            .unwrap();
        assert!(
            !selected
                .snapshot()
                .refs
                .contains_key(&RefName::try_new(INNER).unwrap())
        );
        assert_eq!(
        ...
        status(
            &inspect(
                ...
                format,
                outer_name,
                &format!("&expected_head={}", token(&outer_read)),
            ),
            409,
        ); // 2
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for key in ["candidate-a", "candidate-b"] {
            ...
            workers.push(thread::spawn(move || {
                barrier.wait();
                let reply = send(&client, "annotated", 'b', Some(key), &form, true);
                (key, form, reply)
            }));
        ...
        status(&read, 200);
        assert_eq!(text(&read.body, "object_id"), winner_id);
        assert_eq!(
            text(&read.body, "body_hex"),
            hex(&tag_body(
                b"refs/tags/release",
                main,
                "commit",
        ...
        for (key, form, result) in &results {
            // 4, 5
            ...
            assert_eq!(
                send(&server.client, "annotated", 'b', Some(key), form, false),
                *result
            );
        }
        server.finish();
        ...
        let selected = node
            ...
            .unwrap();
        assert_eq!(
            selected.snapshot().refs[&RefName::try_new(b"refs/tags/release").unwrap()].to_string(),
            winner_id
        );
...
fn lost_release_receipt_recovers_after_restart_rotation_and_withdrawn_write_access() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let object = git_object_id(
            format,
            GitObjectKind::Tag,
            &tag_body(name, main, "commit", MESSAGE),
        );
        ...
        status(&outcome, 200);
        assert!(outcome.body.contains("\"state\":\"committed\""));
        ...
        committed(&retry);
        assert_eq!(text(&retry.body, "tx_id"), text(&outcome.body, "tx_id"));
        committed(&send(
            &server.client,
            "delete",
            '9',
            Some("remove"),
            &(selection(format, name) + &format!("&expected_object={object}")),
            false,
        )); // 2
        assert_eq!(
            send(
...
fn scopes_complete_bodies_and_actual_kinds_gate_creation_and_signatures_never_grant_trust() {
    ...
    status(
        &withheld(&disabled.client, "annotated", 'b', Some("disabled"), 10),
        403,
    );
    disabled.finish();
    let node = reopen(&config);
    ...
    status(
        &withheld(&server.client, "inspect", 'a', Some("not-a-mutation"), 10),
        400,
    ); // 4
    let form = annotation(
        format,
        b"refs/tags/signed-looking",
        main,
        "commit",
        b"release\n-----BEGIN SSH SIGNATURE-----\nnot-a-signature\n-----END SSH SIGNATURE-----\n",
    );
    status(
        &send(
            ...
            'b',
            Some("kind-mismatch"),
            &form.replace("target_kind=commit", "target_kind=blob"),
            false,
        ),
        503,
    ); // 5
    ...
    assert!(matches!(narrow.status, 413 | 503));
    assert!(!narrow.body.contains("\"annotations\":[]"));
    let read = inspect(&server.client, format, b"refs/tags/signed-looking", ""); // 12
    status(&read, 200);
    assert!(read.body.contains("\"signature\":\"opaque_unverifiable\""));
    assert!(!read.body.contains("\"signature_verified\":true"));
    assert!(
        read.body
            .contains("\"tagger_is_authenticated_principal\":false")
    );
    status(
        &inspect(
            &server.client,
            format,
            b"refs/tags/signed-looking",
            &format!("&expected_object={main}"),
        ),
        409,
    ); // 13
    status(
        &inspect(&server.client, format, b"refs/heads/main", ""),
        400,
    ); // 14
    server.finish();
    let node = reopen(&config);
