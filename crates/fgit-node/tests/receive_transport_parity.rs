...
fn raw(
    ...
    let selected = node
        ...
        .unwrap();
    node.runtime()
        .block_on(node.receive_pack_session_durable_in(
            &request,
            &session(key),
            &selected,
            wire_context(format),
...
fn recover(node: &OneNode, key: &[u8], expected: &AdmissionResult, names: &[&str]) {
    ...
    let recovered = node
        .runtime()
        .block_on(node.recover_receive_session_in(&request, &session(key)))
        .unwrap();
    let SessionRecovery::Recovered(recovered) = recovered else {
        panic!("raw admission must record its whole session")
    };
    assert!(recovered.all_terminal());
    assert_eq!(recovered.commands().len(), names.len());
    for (index, command) in recovered.commands().iter().enumerate() {
        ...
        assert_eq!(
            command.recovery().terminal(),
            Some(expected.commands[index].terminal)
        );
    }
}
...
fn http_session_bindings_cannot_be_extended_shortened_or_reordered_through_raw_admission() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let (oid, zero, pack) = pack(format);
        let a = (zero, oid, "refs/tags/a");
        let b = (zero, oid, "refs/tags/b");
        ...
        for changed in [vec![a], vec![a, b, (zero, oid, "refs/tags/c")], vec![b, a]] {
            let error = raw(
                ...
                format,
                b"closed-session",
                &body(format, &changed, &pack, false),
            )
            .unwrap_err();
            assert!(
                matches!(error, NodeSmartHttpRefusal::ReceiveInterrupted(ref error)
                if matches!(error.admission_error(), AdmissionError::Seal(_))),
                "{error:?}"
            );
            assert_eq!(state(&node), before);
        }
        assert_eq!(
            raw(&node, format, b"closed-session", &original).unwrap(),
            first
        );
...
fn atomic_raw_twin_keeps_one_decision_and_the_original_transaction_recovery_path() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let bytes = body(
            format,
            &[(oid, oid, "refs/tags/stale"), (zero, oid, "refs/tags/good")],
            &pack,
            true,
        );
        let before = state(&node).0;
        ...
        let recovered = node
            .runtime()
            .block_on(node.recover_transaction_in(&request, &session(b"atomic-twin")))
            .unwrap();
...
fn corrupt_truncated_and_over_limit_native_input_create_no_session_binding() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        *corrupt.last_mut().unwrap() ^= 1;
        ...
        let two = body(
            format,
            &[(zero, oid, "refs/tags/a"), (zero, oid, "refs/tags/b")],
            &pack,
            false,
        );
        let error = node
            ...
            .unwrap_err();
        assert!(
            matches!(error, NodeSmartHttpRefusal::ReceiveInterrupted(error)
...
fn authentication_cell_state_format_and_cancellation_guard_native_retention() {
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let selected = node
            ...
            .block_on(node.materialize_admission_in(&request))
            .unwrap();
        let call = |session: &LoopbackReceiveSession| {
            node.runtime()
                .block_on(node.receive_pack_session_durable_in(
                    &request,
                    session,
        ...
        assert!(matches!(
            call(&LoopbackReceiveSession::Anonymous),
            Err(NodeSmartHttpRefusal::UnauthenticatedReceive)
        ));
        assert!(
            matches!(call(&session(b"not-serving")), Err(NodeSmartHttpRefusal::ReceiveTransport(error))
        ...
        assert!(
            matches!(node.runtime().block_on(node.receive_pack_session_durable_in(
        ...
        assert!(matches!(
            node.runtime()
                .block_on(
                    node.recover_receive_session_in(
                        &node.request_context(),
                        &session(b"cancelled")
                    )
                )
