...
pub fn prepared(root: &Scratch, format: GitHashAlgorithm) -> (OneNode, Candidate) {
    ...
    let metadata = MergeMetadata {
        ...
        timestamp: 1,
        message: b"reviewed HTTP candidate\n".to_vec(),
    };
    ...
    let built = node
        ...
        .unwrap();
    let MergePreparation::Clean(plan) = built.outcome else {
        panic!("clean native fixture");
    };
    let epoch = node
        .runtime()
    ...
    let candidate = Candidate {
        ...
        epoch,
        bundle: built.bundle.unwrap(),
    };
    assert!(
        node.read_git_object(candidate.binding.commit).is_err(),
        "preparation must not import the candidate"
    );
    (node, candidate)
}
pub fn configure(node: &OneNode, path: &Path) -> String {
    ...
    replace(
        ...
        &(head.clone()
            ...
            + &row('e', SECOND, "reviews-write")
            + &row('f', FOREIGN, "reviews-read")),
    );
    head
}
pub fn open(endpoint: &Endpoint, candidate: &Candidate) {
    committed(&post(
        ...
        'a',
        "open-review-pr",
        &form(&candidate.data, 0),
        false,
    ));
}
pub fn common(candidate: &Candidate) -> String {
    format!(
        "object_format={}&pull_request_version=1&policy_epoch={}&source_ref={}&target_ref={}&source_tip={}&target_tip={}&merge_base={}&candidate_commit={}",
...
pub fn merge_form(candidate: &Candidate, reviewers: &[PrincipalId]) -> String {
    ...
    for reviewer in reviewers {
        body.push_str(&format!("&required_reviewer={reviewer}"));
    }
    body
}
pub fn mutation_bytes(
    endpoint: &Endpoint,
    action: &str,
    token: char,
    key: &str,
    ...
    let (body, content_type) = if let Some(bundle) = bundle {
        ...
        command_part.extend_from_slice(command.as_bytes());
        command_part.extend_from_slice(b"\r\n");
        let mut bundle_part = b"--review-boundary\r\nContent-Disposition: form-data; name=\"bundle\"; filename=\"candidate.bundle\"\r\nContent-Type: application/x-git-bundle\r\n\r\n".to_vec();
        bundle_part.extend_from_slice(bundle);
        bundle_part.extend_from_slice(b"\r\n");
        let mut body = if chunked {
            [bundle_part, command_part].concat()
        } else {
            [command_part, bundle_part].concat()
        };
...
pub fn send(
    ...
    exchange(
        endpoint,
        &mutation_bytes(endpoint, action, token, key, command, bundle, chunked),
        true,
    )
}
