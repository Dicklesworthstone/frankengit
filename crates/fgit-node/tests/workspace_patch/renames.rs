...
fn rename_prepares_one_exact_tree_publishes_once_and_survives_reopen() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            ...
            f.prepare(patch.as_bytes(), limits).unwrap().bundle_bytes()
        );
        assert_eq!(
            candidate
                .paths
        ...
        let tree_bytes = [
            ...
            removed.as_bytes(),
            b"100644 renamed.txt\0",
            f.old.as_bytes(),
        ]
        .concat();
        assert_eq!(
            candidate.root_tree,
            git_object_id(format, GitObjectKind::Tree, &tree_bytes)
        );
        ...
        assert_eq!(
            f.node().read_git_object(f.old).unwrap().payload(),
            b"before\n"
        );
...
fn edited_rename_builds_exact_nested_tree_and_verifies_both_native_indexes() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for (old, new) in [(f.kept, after), (f.old, f.kept)] {
            assert!(matches!(
                f.prepare(edited_rename(old, new).as_bytes(), PatchLimits::default()),
        ...
        assert_eq!(
            candidate.root_tree,
            git_object_id(format, GitObjectKind::Tree, &root)
        );
        assert_eq!(
            candidate
                .paths
        ...
        assert!(matches!(
            apply(&f, &candidate, b"edited-rename").commands[0]
                .terminal
                .outcome,
            DecisionOutcome::Committed { .. }
        ));
        ...
        assert_eq!(
            f.node().read_git_object(nested_id).unwrap().payload(),
            nested
        );
        assert_eq!(
            f.node().read_git_object(after).unwrap().payload(),
            b"after\n"
        );
        assert_eq!(
...
fn pure_binary_rename_preserves_verified_bytes_and_unedited_siblings() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let f = Fixture::new(format);
        let patch =
            rename("keep.txt", "binary.bin") + &format!("index {}..{} 100644\n", f.kept, f.kept);
        let candidate = f.prepare(patch.as_bytes(), PatchLimits::default()).unwrap();
        let removed = removed(&f);
        let root = [
            b"100644 binary.bin\0".as_slice(),
            f.kept.as_bytes(),
            b"100644 edit.txt\0",
            f.old.as_bytes(),
            b"100644 remove.txt\0",
        ...
        assert_eq!(candidate.paths[1].new_blob, None);
        assert!(matches!(
            apply(&f, &candidate, b"binary-rename").commands[0]
                .terminal
                .outcome,
            DecisionOutcome::Committed { .. }
        ));
...
fn both_paths_require_read_and_write_scope_before_any_fetch() {
    ...
    for (reads, writes) in [
        ...
        (
            reads()
                .into_iter()
                .filter(|p| p.as_bytes() != b"edit.txt")
                .collect(),
            writes(),
        ),
        (
            reads()
                .into_iter()
                .filter(|p| p.as_bytes() != b"renamed.txt")
                .collect(),
            writes(),
        ),
    ] {
        ...
        assert!(matches!(
            ...
            Err(NodeWorkspaceRefusal::Manifest(
                fgit_treefs::SparseRefusal::Capability(_)
            ))
        ));
...
fn occupied_or_missing_paths_and_false_identity_never_change_authority() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(matches!(
            f.prepare(
                rename("edit.txt", "keep.txt").as_bytes(),
                PatchLimits::default()
            ),
        ...
        assert!(matches!(
            f.prepare(
                rename("missing.txt", "new.txt").as_bytes(),
                PatchLimits::default()
            ),
            Err(NodeWorkspaceRefusal::WorkspacePatch(
                PatchError::SourcePresence
            ))
        ));
        assert!(
            f.prepare(
                rename("edit.txt", "keep.txt/child").as_bytes(),
                PatchLimits::default()
            )
            .is_err()
        );
        let false_identity = rename("edit.txt", "new.txt")
            + "--- a/edit.txt\n+++ b/new.txt\n@@ -1 +1 @@\n-before\n+after\n";
        assert!(
            f.prepare(false_identity.as_bytes(), PatchLimits::default())
                .is_err()
        );
        for unsupported in [
            "GIT binary patch\nliteral 1\nx\n",
            ...
            assert!(
                f.prepare(
                    (rename("edit.txt", "new.txt") + unsupported).as_bytes(),
                    PatchLimits::default()
                )
                .is_err()
            );
        ...
        assert!(
            f.prepare(
                rename("edit.txt", "new.txt").as_bytes(),
                PatchLimits::default()
            )
            .is_ok()
        );
...
fn later_failure_and_aggregate_budget_cannot_publish_half_a_rename() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(matches!(
            f.prepare(bad.as_bytes(), PatchLimits::default()),
            Err(NodeWorkspaceRefusal::WorkspacePatch(
...
fn rename_out_of_last_child_prunes_empty_ancestors_and_never_replaces_a_directory() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(matches!(
            apply(&f, &initial, b"rename-nested-seed").commands[0]
                .terminal
                .outcome,
            DecisionOutcome::Committed { .. }
        ));
        ...
        let before = f.observed();
        assert!(matches!(
            f.prepare(rename("edit.txt", "dir").as_bytes(), PatchLimits::default()),
            Err(NodeWorkspaceRefusal::InvalidWorkspaceCandidate(
                "rename destination already exists"
            ))
        ));
        ...
        let candidate = f
            .prepare(
                rename("dir/deep/file", "moved.txt").as_bytes(),
                PatchLimits::default(),
            )
            .unwrap();
        assert_eq!(f.observed(), before);
        ...
        assert!(matches!(
            apply(&f, &candidate, b"rename-prune").commands[0]
                .terminal
                .outcome,
            DecisionOutcome::Committed { .. }
        ));
