...
fn multi_commit_rebase_is_read_only_until_explicit_atomic_apply_and_recovers_after_restart() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            first, second,
            "framing cannot change the generated candidate"
        );
        status(
            &post_form(&server.client, "rebase/prepare", 'b', &form, false),
            403,
        );
        server.finish();
        ...
        for (id, parent, message) in [
            (intermediate, history.onto, "first original message"),
        ...
        status(&lookup, 200);
        assert!(json(&lookup).contains(&tx));
        let stale = post_form(&server.client, "rebase/prepare", 'a', &form, false);
        status(&stale, 409);
        assert!(json(&stale).contains("rebase_tip_moved"));
        server.finish();
        let node = reopen(&root.config(format));
        assert_eq!(generation(&node), before + 1);
...
fn stopped_series_resource_and_read_key_refusals_never_publish_or_claim_completion() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let root = Scratch::new();
        let (node, history) = fixture(&root, format, 1);
        let credentials = root.0.join("credentials");
        ...
        assert_eq!(
            field(json(&stop), "stopped_commit"),
            history.first.to_string()
        );
        assert!(
            json(&stop).contains("\"bundle\":null")
                && json(&stop).contains("\"candidate_commit\":null")
        );
        assert!(
            json(&stop).contains("\"series_complete\":false")
                && json(&stop).contains("\"path_hex\":\"61ff\"")
        );
        status(
            &post_form(
                &server.client,
                "rebase/prepare",
                'a',
                &(form.clone() + "&max_commits=1"),
                false,
            ),
            413,
        );
        let unrelated = form.replace(
            &format!("upstream={}", history.upstream),
            &format!("upstream={}", history.onto),
        );
        let refused = post_form(&server.client, "rebase/prepare", 'a', &unrelated, true);
        status(&refused, 409);
        assert!(json(&refused).contains("upstream_outside_linear_history"));
        let read_key = binary_exchange(
            &server.client,
            &request(
                &server.client,
                "POST",
                "/api/v1/source/rebase/prepare",
                'a',
                &format!(
        ...
        status(&read_key, 400);
        assert!(json(&read_key).contains("source_read_has_no_transaction_key"));
        replace(
            &credentials,
            &(grants
        ...
        status(
            &post_form(&server.client, "rebase/prepare", 'a', &form, false),
            401,
        );
        let again = post_form(&server.client, "rebase/prepare", 'c', &form, false);
        assert_eq!(
            stop, again,
            "credential rotation does not change the pinned comparison"
        );
        server.finish();
        let node = reopen(&root.config(format));
        assert_eq!(generation(&node), before);
...
fn empty_commit_policy_stop_drop_and_keep_has_an_executable_zero_commit_result() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        status(&stopped, 409);
        assert_eq!(field(json(&stopped), "state"), "became_empty");
        let dropped = post_form(
            &server.client,
            "rebase/prepare",
            'a',
            &form.replace("empty=stop", "empty=drop"),
            true,
        );
        ...
        let kept = post_form(
            ...
            "rebase/prepare",
            'a',
            &form.replace("empty=stop", "empty=keep"),
            false,
        );
        ...
        let applied = multipart(
            ...
            "rebase/apply",
            'b',
            &apply_form(&history, history.onto),
            &bundle,
            Some("drop-whole-suffix"),
        );
        status(&applied, 200);
        assert!(json(&applied).contains("\"outcome\":\"committed\""));
        server.finish();
        let node = reopen(&root.config(format));
        assert_eq!(generation(&node), before + 1);
        assert_eq!(tip(&node, b"refs/heads/topic"), history.onto);
        node.shutdown().unwrap();
    }
}
