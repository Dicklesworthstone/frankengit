...
///
/// The ref intent is identical for every `pull_request` value, which is what
/// makes the first drill's claim checkable: the two packages differ in the forge
/// event and in nothing the old ref-only seal could see.
fn package_with_event(pull_request: u64) -> MergeEffectPackage {
    MergeEffectPackage {
...
fn two_merges_differing_only_in_their_forge_event_no_longer_share_one_identity() {
    ...
    let attempt = attempt();
    let closure = closure();
    let left = package_with_event(41);
    let right = package_with_event(42);

    // The ref halves really are identical; the events really do differ.
    assert_eq!(
...
fn an_identical_merge_rebuilt_seals_to_the_same_identity() {
    ...
    let attempt = attempt();
    let closure = closure();
    let first = package_with_event(41);
    let second = package_with_event(41);
...
fn a_merge_asserting_a_different_workspace_epoch_seals_as_a_different_request() {
    let context = context();
    let package = package_with_event(41);
    let closure = closure();
    let ninth = attempt();
...
fn a_package_object_outside_the_validated_closure_is_refused_and_one_inside_is_not() {
    ...
    let attempt = attempt();
    let closure = closure();
    let mut stray = package_with_event(41);
