...
fn workspace_validation_hashes_complete_closure_in_both_domains() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            verified.object_closure_root,
            fgit_admission::permitted_object_closure_root(
                &fgit_admission::PermittedObjectClosure::new(expected)
            )
            .unwrap()
        );
    }
...
fn altered_bytes_kinds_and_parent_sets_do_not_become_workspace_candidates() {
    ...
    for offered in [
        parent,
        wrong,
        source.commit(format, tree, &[parent, wrong], "two parents"),
    ] {
        assert!(matches!(
...
fn merge_adapter_keeps_ordered_parents_and_common_ancestor_checks() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(
            validate_merge_objects(&source, &merge, MergeObjectLimits::default(), &mut || true)
                .is_ok()
        );
        assert!(
            validate_workspace_objects(
...
fn source_closure_does_not_authorize_unreachable_repository_objects() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let closure =
            validate_commit_closure(&source, parent, MergeObjectLimits::default(), &mut || true)
                .unwrap();
...
fn source_graph_rejects_ambiguous_commit_edges_without_reinterpreting_bytes() {
    ...
    assert!(
        validate_commit_closure(&source, valid, MergeObjectLimits::default(), &mut || true).is_ok()
    );
