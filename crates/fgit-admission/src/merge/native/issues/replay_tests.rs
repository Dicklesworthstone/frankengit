...
fn number(value: u64) -> IssueNumber {
    IssueNumber::try_new(value).unwrap()
}
fn event(issue: u64, version: u64, action: IssueAction) -> ForgeEvent {
    IssueCommand {
        number: number(issue),
        expected_version: if version == 1 {
    ...
    .proposed_event(PrincipalId::from_bytes([3; 16]))
    .unwrap()
}
...
fn history(issue: u64, count: u64) -> ForgeEventBatch {
    ...
    events.extend((2..=count).map(|version| {
        event(
            issue,
            version,
            IssueAction::Comment {
...
fn a_long_issue_history_crosses_4096_without_truncating_state_or_pages() {
    ...
    let result = run(read_history_at(
        &fixture.store,
        &(),
        &fixture.basis,
        number(1),
...
fn numeric_page_selection_happens_before_replay_and_never_uses_lexical_order() {
    let fixture = Fixture::new(vec![ForgeEventBatch {
        events: vec![opened(10), opened(2), opened(100), opened(1)],
    }]);
    ...
    let timelines = run(replay_selected(
        &fixture.store,
        &(),
    ...
    assert!(
        timelines[&number(2)].events.is_empty(),
        "list/write replay retains no comment timeline"
    );
}
...
fn issue_validation_reuses_the_long_history_without_retaining_it() {
    ...
    let next = event(
        ...
        IssueAction::Edit(IssueEdit {
            title: Some("after 4096".into()),
            ..IssueEdit::default()
        }),
    );
    assert_eq!(
        apply_event(Some(&prior.issue), &next).unwrap().title,
        "after 4096"
    );
...
fn unrelated_large_issue_history_does_not_spend_the_selected_page_memory_budget() {
    let mut batches = vec![
        ForgeEventBatch::of_one(opened(1)),
        ForgeEventBatch::of_one(opened(2)),
    ];
...
fn missing_selected_history_is_not_an_empty_or_partially_replayed_issue() {
    let mut fixture = Fixture::new(vec![
        ForgeEventBatch::of_one(opened(1)),
        ForgeEventBatch::of_one(event(1, 2, IssueAction::Close)),
    ]);
    ...
    let last = fixture
        ...
        .iter()
        .find(|entry| {
            let batch = run(storage::read_events(
                &fixture.store,
                &(),
                fixture.basis.body().repository_id,
                entry.payload_root(),
        ...
        .unwrap();
    fixture.outbox =
        CanonicalOutboxState::try_new(fixture.basis.body().repository_id, vec![last]).unwrap();
    assert!(
        run(replay_selected(
            &fixture.store,
            &(),
...
fn cancelled_invalid_limit_and_missing_issue_keep_distinct_results() {
    ...
    let missing = run(read_history_at(
        &fixture.store,
        &(),
        &fixture.basis,
        number(99),
...
fn canonical_frontiers_above_the_old_global_limit_still_select_bounded_pages() {
    ...
    let entries = (1..=4101)
        .map(|n| {
            fgit_codec::ForgePositionStateEntry::try_new(
                storage::aggregate_label(AggregateId::Issue(number(n))).unwrap(),
                0,
                1,
                root,
            )
        ...
        .collect();
    let forge =
        CanonicalForgePositionState::try_new(RepositoryId::from_bytes([1; 16]), entries).unwrap();
...
fn conflicting_selected_versions_and_missing_frontier_bodies_refuse() {
    ...
    let fork = ForgeEventBatch::of_one(event(
        1,
        2,
        IssueAction::Comment {
            body: "conflict".into(),
    ...
    let missing = CanonicalForgePositionState::try_new(
        repository,
        vec![
            fgit_codec::ForgePositionStateEntry::try_new(
                AsciiSlug::from_static("issue/1"),
                0,
                2,
                fgit_codec::harness::digest_of(200),
            )
            .unwrap(),
        ],
    )
