...
use fgit_types::{GitHashAlgorithm, GitOid, PrincipalId, RepositoryId};
...
fn data(pr: u64, version: u64, format: GitHashAlgorithm, body_bytes: usize) -> PullRequestData {
    PullRequestData {
        ...
        title: format!("PR {pr} version {version}"),
...
fn event(pr: u64, version: u64, format: GitHashAlgorithm, body_bytes: usize) -> ForgeEvent {
    ...
    .proposed_event(
        PrincipalId::from_bytes([if version == 1 { 7 } else { 8 }; 16]),
        format,
    )
    .unwrap()
}
fn history(
    ...
    let events: Vec<_> = (1..=count)
        .map(|version| event(pr, version, format, body_bytes))
        .collect();
    // Keep every frame inside the reference store's 1 MiB body envelope.
    events
        .chunks(8)
        .map(|events| ForgeEventBatch {
...
fn native_pr_histories_above_4096_preserve_latest_metadata_and_original_opener() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            ...
            Some(PrincipalId::from_bytes([8; 16]))
        );
...
fn shared_payload_fanout_is_replayed_once_not_once_per_destination() {
    let events = (1..=1000)
        .map(|version| event(1, version, GitHashAlgorithm::Sha1, 0))
        .collect();
    ...
    let entries = (0..70)
        .map(|index| {
            fixture.entry(
                payload,
                AsciiSlug::try_new("destination", format!("projection-{index}").as_bytes())
                    .unwrap(),
            )
        })
        .collect();
...
fn visibility_and_numeric_pagination_are_preserved_after_replay_changes() {
    let fixture = Fixture::new(vec![ForgeEventBatch {
        events: vec![
            event(10, 1, GitHashAlgorithm::Sha1, 0),
            event(2, 1, GitHashAlgorithm::Sha1, 0),
            event(1, 1, GitHashAlgorithm::Sha1, 0),
        ],
    }]);
    let page = run(read_page_at(
        &fixture.store,
        &(),
        &fixture.basis,
        0,
...
fn an_unselected_opening_object_cannot_supply_missing_canonical_provenance() {
    ...
    let payload = storage::root(&ForgeEventBatch::of_one(event(
        1,
        2,
