...
use super::{
    NativeMergeProjection, PreparationFailure, prepare_event, stage_prepared, storage, unavailable,
};
use crate::{
...
use fgit_authority::{
    AsyncAuthorityStore, AuthenticatedHead, OutcomeLookup, SealAttempt, TerminalOutcome,
};
...
{
    limits.validate()?;
    if !attempt.request.ref_commands().is_empty() {
        ...
        let prepared: Result<_, PreparationFailure> = async {
            ...
            if prepared.materialization.roots.ref_root != basis.body().ref_root
                ...
                || !prepared
                    .fold
                    .effects()
                    .is_some_and(|effects| effects.refs.is_empty())
            {
                return Err(
