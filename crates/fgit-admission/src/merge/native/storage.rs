...
use fgit_codec::canonical_state::{CanonicalForgePositionState, ForgePositionStateEntry, MAX_FORGE_POSITION_STATE_ENTRIES};
...
const EVENT_LIMITS: DecodeLimits = DecodeLimits { elements: MAX_EVENTS as u64, ..DecodeLimits::DEFAULT };
const POSITION_LIMITS: DecodeLimits = DecodeLimits { elements: MAX_FORGE_POSITION_STATE_ENTRIES as u64, ..DecodeLimits::DEFAULT };

pub(super) async fn aggregate_refusal<S: AsyncAuthorityStore + ?Sized>(
    store: &S, cx: &S::Context, positions: &CanonicalForgePositionState, intent: &NativeMergeIntent,
    ...
    let head = match previous {
        Some(entry) => AggregateHead::at(intent.event().aggregate,
    ...
    if let Some(entry) = previous {
        let batch = read_events(store, cx, positions.repository_id(), entry.event_batch_root()).await?;
        let event = batch.events.iter().rev().find(|event| event.aggregate == intent.event().aggregate)
            .ok_or_else(|| unavailable(RefusalCode::EvidenceInvalid))?;
        if event.version.get() != entry.successor_position() {
            return Err(unavailable(RefusalCode::EvidenceInvalid));
        }
        if matches!(event.payload, ForgeEventPayload::MergeCommitted { .. }
...
pub async fn load_forge_positions<S: AsyncAuthorityStore + ?Sized>(
    store: &S, cx: &S::Context, basis: &PublicationBasis,
    ...
    if let Some(frame) = read_frame(store, cx, repository, POSITION_NAMESPACE, selected_root).await? {
    ...
    while let Some(batch_id) = successor.decision_tail_id {
        if walked >= MAX_BOOTSTRAP_BATCHES { return Err(unavailable(RefusalCode::ResourceBudgetExceeded)); }
        walked += 1;
        ...
        verify_pair(&CryptoBodyIdentity, &PublicationBasis::new(predecessor_id, predecessor.clone()), &batch, &successor)
            .map_err(|_| unavailable(RefusalCode::EvidenceInvalid))?;
        if predecessor.forge_position_root != selected_root || successor.forge_position_root != selected_root {
        ...
        for record in batch.committed_rcrs.iter().rev() {
            ...
            if roots.len() >= MAX_EVENTS { return Err(unavailable(RefusalCode::ResourceBudgetExceeded)); }
            roots.push(record.forge_event_batch_root);
        }
        successor = predecessor;
    }
    if successor.predecessor_head_id.is_some() || successor.latest_committed_rcr_id.is_some()
        || successor.latest_decision_sequence.is_some() || successor.repository_id != repository
    { return Err(unavailable(RefusalCode::EvidenceInvalid)); }
    ...
    for event_root in roots.into_iter().rev() {
        ...
        events_seen = events_seen.checked_add(events.events.len()).filter(|count| *count <= MAX_EVENTS)
...
pub(super) fn advance_positions(
    previous: &CanonicalForgePositionState, batch: &ForgeEventBatch, event_root: Digest,
) -> Result<CanonicalForgePositionState, AdmissionError> {
    let mut entries: BTreeMap<_, _> = previous.entries().iter().map(|entry| (entry.stream(), *entry)).collect();
    ...
    for event in &batch.events {
        ...
        let initial = entries.get(&stream).map_or(0, ForgePositionStateEntry::successor_position);
        let range = ranges.entry(stream).or_insert((initial, 0));
        let next_count = range.1.checked_add(1)
            .ok_or_else(|| unavailable(RefusalCode::ResourceBudgetExceeded))?;
        let expected = range.0.checked_add(u64::from(next_count))
            .ok_or_else(|| unavailable(RefusalCode::ResourceBudgetExceeded))?;
        if event.version.get() != expected { return Err(unavailable(RefusalCode::EvidenceInvalid)); }
        range.1 = next_count;
    }
    for (stream, (before, count)) in ranges {
        entries.insert(stream, ForgePositionStateEntry::try_new(stream, before, count, event_root)
            .map_err(|_| unavailable(RefusalCode::EvidenceInvalid))?);
    }
...
pub(super) async fn verify_head_target<S: AsyncAuthorityStore + ?Sized>(
    store: &S, cx: &S::Context, repository: RepositoryId, ref_root: Digest, expected: Option<&RefName>,
) -> Result<(), AdmissionError> {
    let frame = read_frame(store, cx, repository, b"frankengit/admission/ref-state/v1/", ref_root)
        .await?.ok_or_else(|| unavailable(RefusalCode::EvidenceMissing))?;
    let state = decode_body::<crate::CanonicalRefState>(&frame, DecodeLimits::DEFAULT)
        .map_err(|_| unavailable(RefusalCode::EvidenceInvalid))?;
    if state.head_target() != expected { return Err(unavailable(RefusalCode::EvidenceInvalid)); }
    Ok(())
}

async fn read_events<S: AsyncAuthorityStore + ?Sized>(
    store: &S, cx: &S::Context, repository: RepositoryId, event_root: Digest,
) -> Result<ForgeEventBatch, AdmissionError> {
    let frame = read_frame(store, cx, repository, EVENT_NAMESPACE, event_root).await?
        .ok_or_else(|| unavailable(RefusalCode::EvidenceMissing))?;
    if let Ok(batch) = decode_body::<ForgeEventBatch>(&frame, EVENT_LIMITS) {
        if root(&batch)? != event_root { return Err(unavailable(RefusalCode::EvidenceInvalid)); }
        return Ok(batch);
    }
    ...
    let empty = decode_body::<crate::evidence::ForgeEventBatch>(&frame, EVENT_LIMITS)
        .map_err(|_| unavailable(RefusalCode::EvidenceInvalid))?;
    if root(&empty)? != event_root || !empty.is_empty().map_err(|_| unavailable(RefusalCode::EvidenceInvalid))? {
        return Err(unavailable(RefusalCode::EvidenceInvalid));
    }
    Ok(ForgeEventBatch { events: Vec::new() })
}

fn body_key(namespace: &[u8], repository: RepositoryId, digest: Digest) -> Result<ImmutableKey, AdmissionError> {
...
async fn read_frame<S: AsyncAuthorityStore + ?Sized>(
    ...
    match store.read_immutable(cx, &body_key(namespace, repository, digest)?).await? {
...
pub(super) async fn stage_body<S: AsyncAuthorityStore + ?Sized, B: CanonicalBody + Sync>(
    ...
    if frame.len() > MAX_FRAME_BYTES { return Err(unavailable(RefusalCode::ResourceBudgetExceeded)); }
    match store.put_if_absent(cx, &body_key(namespace, repository, digest)?, &frame).await? {
