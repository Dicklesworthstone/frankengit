...
use fgit_forge::event::review::{
    CandidateBinding, NativeReviewEvent, ReviewDecision, ReviewSubject,
};
...
impl ReviewRequirements {
    ...
    fn bytes(&self) -> Vec<u8> {
        ...
        for reviewer in &self.reviewers {
            bytes.extend_from_slice(reviewer.as_bytes());
        }
        bytes
    }
}
...
pub fn reviewed_seal_attempt(
    ...
    entries.push(ScopedEntry::new(
        ...
        AsciiSlug::from_static("merge.required-candidate-reviewers.v1"),
        required.bytes(),
    )?);
...
where
    ...
    P: NativeMergeProjection<S> + Sync + ?Sized,
{
    limits.validate()?;
    let attempt = reviewed_seal_attempt(context, intent, required)?;
    let guarded = GuardedProjection {
        inner: projection,
        context,
        required,
    };
...
pub(crate) async fn verify_at<S, C>(
    store: &S,
    cx: &S::Context,
    ...
    C: Fn() -> bool + Sync,
{
    ...
    let subject = ReviewSubject {
        ...
        policy_epoch: required.policy_epoch,
    };
    let candidate = CandidateBinding {
        merge_base: merge.base_tip,
        commit: merge.merge_commit,
    };
    let mut page = super::super::read_page_at(
        store,
        cx,
        basis,
        number.get() - 1,
        1,
        &|source, target| source == &subject.source_ref && target == &subject.target_ref,
        cancelled,
    )
    .await
    ...
    if change.action == fgit_forge::event::pull_request::PullRequestAction::Close
        || pr.event.version != version
        || !change.data.matches_merge(merge)
    {
        return Err(ProjectionFailure::Refuse(RefusalCode::EvidenceStale));
    }
    let opener = pr
    ...
    if required
        .reviewers
        .iter()
        .any(|reviewer| *reviewer == opener || *reviewer == submitter)
    {
        return Err(ProjectionFailure::Refuse(
            RefusalCode::ProtectedRefTransitionDenied,
        ));
...
mod tests {
    ...
    fn fixture() -> (
        ...
        let oid =
            |digit: &str| GitOid::from_hex(GitHashAlgorithm::Sha1, &digit.repeat(40)).unwrap();
        let reviewer = PrincipalId::from_bytes([9; 16]);
        let context = AdmissionContext {
            head_key: fgit_authority::HeadKey::new(b"head".to_vec()).unwrap(),
        ...
        let intent = NativeMergeIntent::new(
            ...
            binding.merge(&subject),
        )
        .unwrap();
        let review = NativeReviewEvent {
            reviewer,
            subject,
        ...
        (
            context,
            intent,
            ReviewRequirements::new(PolicyEpoch::FIRST, vec![reviewer]).unwrap(),
            review,
        )
    }
    #[test]
    fn gate_requirements_are_sealed_order_independent_and_cannot_be_dropped_on_retry() {
        ...
        assert_ne!(
            guarded.derive().unwrap().0,
            intent.seal_attempt(&context).unwrap().derive().unwrap().0
        );
        let other = PrincipalId::from_bytes([8; 16]);
        let a = ReviewRequirements::new(PolicyEpoch::FIRST, vec![other, required.reviewers[0]])
            .unwrap();
        let b = ReviewRequirements::new(PolicyEpoch::FIRST, vec![required.reviewers[0], other])
            .unwrap();
        assert_eq!(a, b);
        assert_eq!(
            reviewed_seal_attempt(&context, &intent, &a).unwrap(),
            reviewed_seal_attempt(&context, &intent, &b).unwrap()
        );
        assert_ne!(
            guarded.derive().unwrap().0,
            reviewed_seal_attempt(&context, &intent, &a)
                .unwrap()
                .derive()
                .unwrap()
                .0
        ...
        assert!(
            reviewed_seal_attempt(
                &context,
                &intent,
                &ReviewRequirements::new(PolicyEpoch::FIRST, vec![context.principal_id]).unwrap()
            )
            .is_err()
        );
    }
    ...
    fn only_exact_current_candidate_approvals_satisfy_the_gate() {
        ...
        for field in 0..7 {
            ...
            match field {
                ...
                4 => {
                    wrong.decision = ReviewDecision::Withdraw;
                    wrong.reason = "withdrawn".into();
                }
                5 => {
                    wrong.decision = ReviewDecision::RequestChanges;
                    wrong.reason = "blocked".into();
                }
                _ => wrong.reviewer = PrincipalId::from_bytes([8; 16]),
            }
            assert!(
                verify_vote(&wrong, valid.reviewer, &valid.subject, candidate).is_err(),
                "field {field}"
            );
        }
    }
