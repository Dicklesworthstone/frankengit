...
use super::super::{
    NativeMergeProjection, PreparationFailure, delivery, prepare_event, stage_prepared, storage,
    unavailable,
};
use crate::{
...
use fgit_authority::{
    AsyncAuthorityStore, AuthenticatedHead, OutcomeLookup, ScopedEntry, SealAttempt,
    SemanticRequest, TerminalOutcome,
};
...
use fgit_forge::event::review::{
    CandidateBinding, CandidateReviewCommand, NativeReviewEvent, ReviewCommand, ReviewDecision,
...
use fgit_types::{
    AsciiSlug, PolicyEpoch, PrincipalId, RefName, RefusalCode, RepositoryAuthorityHeadId,
};
...
fn seal_event(
    ...
    let request = SemanticRequest::build(
        ...
        vec![ScopedEntry::new(
            ...
            AsciiSlug::from_static("pull-request-review.event-batch-root"),
            root.bytes().as_bytes(),
        )?],
    )?;
    Ok((
...
where
    S: AsyncAuthorityStore + ?Sized,
    P: ReviewProjection<S> + ?Sized,
{
    let (event, attempt) = proposal(context, command)?;
    admit_event(
        store, cx, context, command, event, attempt, limits, projection,
    )
    .await
}
    ...
    P: ReviewProjection<S> + ?Sized,
{
    ...
    for _ in 0..limits.max_cas_replans {
        ...
        let preparation: Result<_, PreparationFailure> = async {
            ...
            {
                return Err(
                    ProjectionFailure::Unavailable(RefusalCode::AuthorityReceiptStale).into(),
                );
            }
            projection
                .merge_checkpoint(cx)
            ...
            validate_review_transition(previous.as_ref(), &event)
                .map_err(ProjectionFailure::Refuse)?;
            let pr =
                super::frontier_event(store, cx, &resolved.forge, subject.pull_request).await?;
            if command.decision != ReviewDecision::Withdraw {
                let freshness = review_freshness(
                    review,
                    pr.as_ref(),
                    basis.body().policy_epoch,
                    snapshot.refs.get(&subject.source_ref).copied(),
                    snapshot.refs.get(&subject.target_ref).copied(),
                );
                if freshness != ReviewFreshness::Current {
                    return Err(ProjectionFailure::Refuse(match freshness {
                        ...
                        ReviewFreshness::SourceMoved | ReviewFreshness::TargetMoved => {
            ...
            let closure = projection
                .validate_review_async(store, cx, &basis, &authenticated, command, review.candidate)
                .await?;
            let prepared = prepare_event(
                context, &event, &closure, tx_id, &attempt, &basis, &resolved,
            )
            .map_err(ProjectionFailure::Refuse)?;
            let pr_label =
                storage::aggregate_label(AggregateId::PullRequest(subject.pull_request))?;
            if prepared.refs != resolved.refs
                || prepared.materialization.roots.ref_root != basis.body().ref_root
            ...
            {
                return Err(
                    ProjectionFailure::Unavailable(RefusalCode::InternalInvariantBreach).into(),
                );
            }
            Ok(prepared)
        }
        ...
        let publication = plan.seal(
            &CryptoBodyIdentity,
            prepared.materialization.roots,
            &cumulative,
            receipt.token(),
        )?;
        projection
...
async fn review_frontier<S: AsyncAuthorityStore + ?Sized>(
    ...
    let Some(position) = positions.entry(storage::aggregate_label(aggregate)?) else {
        return Ok(None);
    };
    ...
    C: Fn() -> bool + Sync,
{
    ...
    for position in state.forge.entries() {
        ...
        let reviewer =
            PrincipalId::from_hex(text).map_err(|_| unavailable(RefusalCode::EvidenceInvalid))?;
        let aggregate = AggregateId::PullRequestReview {
            pull_request: number,
            reviewer,
        };
        if storage::aggregate_label(aggregate)? != label {
            return Err(unavailable(RefusalCode::EvidenceInvalid));
        }
        if after.is_none_or(|last| reviewer > last) {
    ...
    for reviewer in reviewers {
        ...
        if !visible(&review.subject.source_ref, &review.subject.target_ref) {
            continue;
        }
        ...
        bytes = bytes
            .checked_add(
                review.reason.len()
                    + review.subject.source_ref.as_bytes().len()
                    + review.subject.target_ref.as_bytes().len()
                    + 400,
            )
        ...
        let freshness = review_freshness(
            ...
            Some(&pr.event),
            basis.body().policy_epoch,
            refs.get(&review.subject.source_ref).copied(),
            refs.get(&review.subject.target_ref).copied(),
        );
    ...
    Ok(Some(ReviewPage {
        ...
        pull_request: number,
        pull_request_version: pr.event.version,
