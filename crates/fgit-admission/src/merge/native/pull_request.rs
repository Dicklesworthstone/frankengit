...
use fgit_forge::event::pull_request::{
    NativePullRequestEvent, PullRequestAction, PullRequestCommand, PullRequestData,
    validate_transition,
};
    ...
    P: PullRequestProjection<S> + ?Sized,
{
    ...
    super::metadata::admit_metadata_async(
        store,
        cx,
        context,
        event,
    ...
    P: PullRequestProjection<S> + ?Sized,
{
    fn precheck(&self, snapshot: &crate::AdmissionSnapshot) -> Result<(), ProjectionFailure> {
        if snapshot
            .hidden_refs
            .hides(self.0.data.source_ref.as_bytes())
            || snapshot
                .hidden_refs
                .hides(self.0.data.target_ref.as_bytes())
        {
    ...
    async fn validate<'a>(
        ...
        basis: &'a PublicationBasis,
        authenticated: &'a AuthenticatedHead,
        snapshot: &'a crate::AdmissionSnapshot,
...
async fn frontier_event<S: AsyncAuthorityStore + ?Sized>(
    ...
    let Some(entry) = positions.entry(label) else {
        return Ok(None);
    };
    let events = storage::read_events(
        store,
        cx,
    ...
    C: Fn() -> bool + Sync,
{
    ...
    for entry in state.forge.entries() {
        ...
        let number = text
            .parse::<u64>()
            .ok()
            .and_then(PullRequestNumber::try_new)
            .ok_or_else(|| unavailable(RefusalCode::EvidenceInvalid))?;
    ...
    for number in numbers {
        checkpoint(cancelled)?;
        let event = frontier_event(store, cx, &state.forge, number)
            .await?
            .ok_or_else(|| unavailable(RefusalCode::EvidenceInvalid))?;
        let permitted = match &event.payload {
            ForgeEventPayload::PullRequestChangedNative(change) => {
                visible(&change.data.source_ref, &change.data.target_ref)
            }
            ForgeEventPayload::MergeCommittedNative(merge) => {
                visible(&merge.source_ref, &merge.target_ref)
            }
            _ => false,
        };
        if !permitted {
        ...
        let (data, actor) = match &event.payload {
            ForgeEventPayload::PullRequestChangedNative(change) => {
                (Some(change.data.clone()), Some(change.actor))
            }
            _ => (None, None),
        };
        selected.push(PullRequestView {
            number,
            event,
            data,
            opened_by: None,
            last_metadata_actor: actor,
        });
    }
    ...
    let positions: BTreeMap<_, _> = selected
        .iter()
    ...
    if !selected.is_empty() {
        for obligation in state.outbox.entries() {
            ...
            for event in batch.events {
                ...
                let Some(version) = positions.get(&number) else {
                    continue;
                };
    ...
    let next_after = if more {
        selected.last().map(|view| view.number.get())
    } else {
        None
    };
    Ok(PullRequestPage {
...
mod tests {
    ...
    fn inputs() -> (AdmissionContext, PullRequestCommand) {
        ...
        let command = PullRequestCommand {
            ...
            expected_version: fgit_forge::ExpectedVersion::NewStream,
    ...
    fn metadata_only_seal_is_stable_and_binds_the_complete_event() {
        ...
        changed.data.body.replace_range(0..1, "y");
        assert_ne!(
    ...
    fn expected_version_and_action_are_not_rebuilt_from_current_state() {
        ...
        assert_ne!(original, update);
        assert_ne!(update, close);
