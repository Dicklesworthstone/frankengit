...
use fgit_codec::{
    CanonicalForgePositionState, CanonicalOutboxState, CryptoBodyIdentity,
...
use fgit_types::{
    DigestAlgorithmId, DigestBytes, GitHashAlgorithm, GitOid, HeadGeneration, PolicyEpoch,
    PrincipalId, RefName, RegistryEpoch, RepositoryAuthorityHeadId, RootLayoutVersion, TenantId,
};
use std::collections::{BTreeMap, BTreeSet};

fn epoch(value: u64) -> PolicyEpoch {
    PolicyEpoch::try_new(value).unwrap()
}
fn fixture(format: GitHashAlgorithm, before: u64, policy: bool) -> PreparedNativeMerge {
    ...
    let context = AdmissionContext {
        head_key: HeadKey::new(b"epoch-test".to_vec()).unwrap(),
        tenant_id: TenantId::from_bytes([1; 16]),
        repository_id: repository,
        principal_id: actor,
        idempotency_key: IdempotencyKey::new(b"epoch".to_vec()).unwrap(),
        object_format: format,
    };
    let refs = CanonicalRefState::new(BTreeMap::from([
        (name("refs/heads/main"), oid(10)),
        (name("refs/heads/topic"), oid(11)),
    ]));
    ...
    let head = RepositoryAuthorityHeadBody {
        ...
        forge_position_root: root(&resolved.forge).unwrap(),
        outbox_root: root(&resolved.outbox).unwrap(),
        outcome_index_root: digest(20),
        retention_root: digest(21),
        configuration_root: digest(22),
        policy_epoch: epoch(before),
        format_registry_epoch: RegistryEpoch::FIRST,
        last_checkpoint_id: None,
    };
    let id = RepositoryAuthorityHeadId::from_internal_object_id(
        body_id(&CryptoBodyIdentity, &head).unwrap(),
    )
    .unwrap();
    ...
    let (event, attempt, objects) = if policy {
        ...
        let intent = NativeMergeIntent::new(
            ...
            NativeMerge {
                source_ref: name("refs/heads/topic"),
                target_ref: name("refs/heads/main"),
                source_tip: oid(11),
                target_tip_before: oid(10),
                base_tip: oid(12),
                merge_commit: oid(13),
            },
        )
        ...
        (
            intent.event().clone(),
            intent.seal_attempt(&context).unwrap(),
            BTreeSet::from([oid(10), oid(11), oid(12), oid(13)]),
        )
    };
    let closure = ValidatedClosure {
        object_closure_root: crate::permitted_object_closure_root(&PermittedObjectClosure::new(
            objects.clone(),
        ))
        .unwrap(),
        objects,
...
fn protection_stages_old_authorization_with_exact_new_epoch_in_both_hash_domains() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        for before in [1, 17, u64::MAX - 1] {
            ...
            assert_eq!(
                root(prepared.evidence.policy_decision()).unwrap(),
                prepared.materialization.record.policy_decision_root
            );
        }
    }
}
...
fn policy_cannot_repeat_skip_roll_back_or_relabel_its_authorizing_epoch() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for after in [1, 6, 7, 9, u64::MAX] {
            ...
            bad.materialization.roots.policy_epoch = epoch(after);
            invalid_preparation(&bad);
        }
        let mut bad = good.clone();
        bad.materialization.record.policy_epoch = epoch(8);
        invalid_preparation(&bad);
        let mut bad = good;
        let ForgeEventPayload::ReviewProtectionChanged(change) = &mut bad.event.events[0].payload
        else {
            panic!()
        };
...
fn ordinary_native_merges_still_require_one_unchanged_epoch() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        for before in [1, 7, u64::MAX] {
            let good = fixture(format, before, false);
            validate_prepared(&good).unwrap();
            let mut bad = good.clone();
            bad.materialization.roots.policy_epoch = epoch(if before == 1 { 2 } else { 1 });
            invalid_preparation(&bad);
        }
    }
}
...
fn singleton_kind_actor_empty_closure_and_effect_bounds_are_not_optional() {
    let good = fixture(GitHashAlgorithm::Sha256, 1, true);
    validate_prepared(&good).unwrap();
    let mut bad = good.clone();
    bad.event.events.clear();
    invalid_preparation(&bad);
    let mut bad = good.clone();
    bad.event.events.push(bad.event.events[0].clone());
    invalid_preparation(&bad);
    let mut bad = good.clone();
    bad.event.events[0].aggregate = AggregateId::PullRequest(PullRequestNumber::FIRST);
    invalid_preparation(&bad);
    let mut bad = good.clone();
    let ForgeEventPayload::ReviewProtectionChanged(change) = &mut bad.event.events[0].payload
    else {
    ...
    change.actor = PrincipalId::from_bytes([8; 16]);
    invalid_preparation(&bad);
    let mut bad = good.clone();
    ...
    bad.materialization.record.object_closure_root =
        crate::permitted_object_closure_root(&bad.closure).unwrap();
    invalid_preparation(&bad);
    let mut bad = good;
    let fgit_reference::effect::FoldOutcome::Folded(effects) = &mut bad.fold.outcome else {
        panic!()
    };
    effects.refs.insert(
        RefName::try_new(b"refs/heads/main").unwrap(),
        fgit_reference::effect::RefEffect::Set(id),
    );
    invalid_preparation(&bad);
}
