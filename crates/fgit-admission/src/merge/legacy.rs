...
use super::prepare::{NativeMergeBasis, PreparedNativeMerge, prepare_native_merge};
...
fn check_event_describes_this_merge(sealed: &SealedMerge<'_>) -> Result<(), AdmissionError> {
    if let fgit_forge::ForgeEventPayload::MergeCommittedNative(event) = &sealed.package.event.payload
    {
        ...
        let coordinates = [
            (
                event.source_ref.as_bytes() == sealed.attempt.source_ref,
                "event source ref",
            ),
            (
                event.source_tip == sealed.attempt.source_tip,
                "event source tip",
            ),
            (event.base_tip == sealed.attempt.base_tip, "event base tip"),
            (
                event.target_ref.as_bytes() == sealed.attempt.target_ref,
                "event target ref",
            ),
            (
                event.target_tip_before == sealed.attempt.target_tip,
                "event target before",
            ),
            (
                event.merge_commit == sealed.package.ref_intent.new_tip,
                "event merge commit",
            ),
        ];
    ...
    Commitments: AsyncMergeMaterializer<S> + ?Sized,
{
    ...
    for replan in 0..limits.max_cas_replans {
        ...
        let terminal = match decide_from_snapshot(sealed, snapshot) {
            ...
            MergePlan::Commit(next_state) => {
                let materialization = match commitments
                    .materialize_merge_async(
                        store,
                        cx,
                        context,
                        sealed,
...
fn materialize(
    ...
    if matches!(
        ...
        let resolved = commitments
            .resolve_native_merge_basis(basis)
