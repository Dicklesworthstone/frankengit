...
use fgit_authority::{
    AsyncAuthorityStore, AuthorityFailure, AuthorityStore, ImmutableRead, OutcomeFailure,
    PutOutcome, body_key_for_id,
};
...
impl PolicyStoreLimits {
    fn decode(self) -> Result<DecodeLimits, PolicyStoreError> {
        ...
        if self.frame_bytes == 0
            || self.frame_bytes > ceiling.frame_bytes
            || self.byte_string_bytes == 0
            || self.byte_string_bytes > ceiling.byte_string_bytes
            || self.elements == 0
            || self.elements > ceiling.elements
            || self.depth == 0
        ...
        Ok(DecodeLimits {
            ...
            byte_string_bytes: self.byte_string_bytes,
...
pub enum PolicyStoreError {
    ...
    Authority {
        operation: PolicyStoreOperation,
        failure: Box<AuthorityFailure>,
    },
    Missing {
    ...
    fn fmt(&self, out: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::FrameTooLarge { bytes, limit } => {
                write!(out, "policy frame {bytes} exceeds {limit} bytes")
            }
            ...
            Self::Authority { operation, failure } => {
                write!(out, "policy {operation:?}: {failure}")
            }
            ...
            Self::IdentityMismatch {
                ...
                out,
                "policy mismatch: requested {requested}, received {observed}"
            ),
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PolicyStageDisposition {
    Created,
    IdenticalRetry,
}
...
fn codec(error: CodecRefusal) -> PolicyStoreError {
    PolicyStoreError::Codec(Box::new(error))
}
fn authority(operation: PolicyStoreOperation, failure: AuthorityFailure) -> PolicyStoreError {
    PolicyStoreError::Authority {
        operation,
...
fn identify<C>(
    bytes: &[u8],
    expected: Option<PolicySnapshotId>,
    limits: PolicyStoreLimits,
    checkpoint: &C,
    ...
    C: Fn() -> Result<(), RefusalCode> + ?Sized,
{
    ...
    if u64::try_from(bytes.len()).unwrap_or(u64::MAX) > limits.frame_bytes {
        return Err(PolicyStoreError::FrameTooLarge {
            bytes: bytes.len(),
    ...
    if let Some(requested) = expected
        && snapshot.id() != requested
    {
        return Err(PolicyStoreError::IdentityMismatch {
    ...
    if snapshot.encode().map_err(codec)?.as_slice() != bytes {
        return Err(PolicyStoreError::NonCanonical {
            id: Box::new(snapshot.id()),
        });
    }
    checkpoint().map_err(PolicyStoreError::Stopped)?;
    Ok(snapshot)
...
fn receipt(
    ...
    let disposition = match outcome {
        ...
        PutOutcome::Conflict => {
            return Err(PolicyStoreError::ConflictingSlot {
                id: Box::new(frame.id),
            });
        }
    };
    Ok(PolicyStageReceipt {
        id: frame.id,
        disposition,
        encoded_bytes: frame.bytes.len(),
    })
}
...
{
    checkpoint().map_err(PolicyStoreError::Stopped)?;
    let key = body_key_for_id(frame.id.as_internal_object_id())
        .map_err(|e| PolicyStoreError::Key(Box::new(e.into())))?;
    receipt(
        frame,
        store
            .put_if_absent(&key, &frame.bytes)
...
where
    ...
    C: Fn() -> Result<(), RefusalCode> + Sync + ?Sized,
{
    checkpoint().map_err(PolicyStoreError::Stopped)?;
    let key = body_key_for_id(frame.id.as_internal_object_id())
        .map_err(|e| PolicyStoreError::Key(Box::new(e.into())))?;
    receipt(
        frame,
        store
            .put_if_absent(cx, &key, &frame.bytes)
            .await
            .map_err(|e| authority(PolicyStoreOperation::Stage, e))?,
    )
}
    ...
    C: Fn() -> Result<(), RefusalCode> + ?Sized,
{
    limits.decode()?;
    checkpoint().map_err(PolicyStoreError::Stopped)?;
    let key = body_key_for_id(id.as_internal_object_id())
        .map_err(|e| PolicyStoreError::Key(Box::new(e.into())))?;
    let read = store
        .read_immutable(&key)
        .map_err(|e| authority(PolicyStoreOperation::Read, e))?;
    checkpoint().map_err(PolicyStoreError::Stopped)?;
    let ImmutableRead::Present(bytes) = read else {
        return Err(PolicyStoreError::Missing {
