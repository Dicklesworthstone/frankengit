...
fn store() -> MemoryAuthorityStore {
    MemoryAuthorityStore::new(StoreInstanceId::from_raw(912))
}
...
fn a_valid_wrong_policy_in_the_requested_slot_is_not_an_allow_verdict() {
    ...
    assert_eq!(
        bytes,
        allow.bytes(),
        "failure cannot overwrite an immutable slot"
    );
}
...
fn allow_and_deny_evaluations_name_the_actual_persisted_snapshot() {
    ...
    for allow in [false, true] {
        ...
        let repeat = evaluate_stored_policy(
            &store,
            frame.id(),
            &input,
            &codes,
...
fn missing_corrupt_and_oversized_policies_do_not_select_a_fallback() {
    ...
    let store = store();
    let key = body_key_for_id(frame.id().as_internal_object_id()).unwrap();
    store
...
fn cancellation_before_put_changes_nothing_and_after_read_returns_no_verdict() {
    ...
    assert!(matches!(
        stage_policy(&store, &frame, &stopped),
    ...
    assert!(matches!(
        read_policy(&store, frame.id(), limits, &stop_after_read),
        Err(PolicyStoreError::Stopped(
