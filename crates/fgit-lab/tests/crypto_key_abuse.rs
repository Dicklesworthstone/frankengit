//! Scheduled key-lifecycle conformance drills for `fgit-crypto`.
//!
//! `fgit-lab` is an L2 test consumer, so it may depend downward on the L1
//! crypto crate without creating an L1-to-L2 manifest edge. The schedules make
//! the rotation-window order explicit and replayable; they do not claim a
//! thread race.

use fgit_crypto::{
    CodecVersion, EnvelopeNonce, IdentityDomain, KeyEpoch, KeyHistory, KeyLifecycle,
    KeyLifecycleError, KeyPurpose, KeyScope, RECEIPT_SCHEMA, Recoverability, RootSecret, SecretKey,
    TenantEncryption, verify_internal_object_id,
};
