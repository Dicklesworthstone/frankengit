...
use fgit_authority::{AuthorityLimits, HeadGeneration, HeadKey, HeadReadReceipt, ImmutableKey,
    StoreInstanceId};
...
impl Default for PortableStoreLimits {
    fn default() -> Self {
        Self { max_bodies: 100_000, max_issuance: 100_000, max_field_bytes: 64 * 1024 * 1024 }
    }
}
impl PortableStoreLimits {
    fn validate(self) -> Result<(), PortableStoreError> {
        if self.max_bodies > MAX_EXPORT_BODIES || self.max_issuance > MAX_EXPORT_ISSUANCE
            || self.max_field_bytes == 0 || self.max_field_bytes > 1024 * 1024 * 1024
...
pub enum PortableStoreError {
    ...
    Cleanup { cause: Box<Self>, cleanup: EngineError },
}
impl std::fmt::Display for PortableStoreError {
    fn fmt(&self, out: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InvalidSourceToken => out.write_str("portable token disagrees with its source instance and sequence"),
            Self::InvalidLineage => out.write_str("portable head is not the latest monotonically issued single-head state"),
            ...
            Self::UnexpectedWriteCount(name) => write!(out, "portable import did not exclusively insert {name}"),
            Self::Cleanup { cause, cleanup } => write!(out, "{cause}; snapshot rollback also failed: {cleanup}"),
        }
    }
}
...
impl From<EngineError> for PortableStoreError {
    fn from(error: EngineError) -> Self { Self::Engine(error) }
}

fn live<Caps>(cx: &Cx<Caps>) -> Result<(), PortableStoreError>
where Caps: cap::SubsetOf<cap::All>, cap::None: cap::SubsetOf<Caps>,
{
    cx.checkpoint().map_err(|_| PortableStoreError::Engine(
        EngineError::Engine(crate::TransientClass::Cancelled)))
}
...
fn check_body(body: &[u8], limits: AuthorityLimits) -> Result<(), PortableStoreError> {
    if body.len() > limits.body_bytes { return Err(PortableStoreError::Limit("authority body bytes")); }
    Ok(())
}
...
fn validate_bundle(bundle: &ExportBundle, limits: PortableStoreLimits, store: AuthorityLimits,
    mut checkpoint: impl FnMut() -> Result<(), PortableStoreError>,
) -> Result<(), PortableStoreError> {
    limits.validate()?;
    checkpoint()?;
    if bundle.instance > i64::MAX as u64 { return Err(PortableStoreError::InvalidSourceToken); }
    ...
    for (ordinal, row) in bundle.issuance.iter().enumerate() {
        ...
        let head = bundle.head.as_ref().ok_or(PortableStoreError::InvalidLineage)?;
    ...
    if let Some(head) = &bundle.head {
        ...
        let tail = bundle.issuance.last().ok_or(PortableStoreError::InvalidLineage)?;
        if tail.token != head.token || tail.generation != head.generation || tail.body != head.body {
            return Err(PortableStoreError::InvalidLineage);
        }
    }
    bundle.validate().map_err(|error| PortableStoreError::Bundle(Box::new(error)))?;
    checkpoint()
}
...
impl FsqliteAuthorityStore {
    ...
    pub async fn export_portable<Caps>(&self, cx: &Cx<Caps>, limits: PortableStoreLimits)
    ...
    async fn export_portable_snapshot<Caps>(&self, cx: &Cx<Caps>, limits: PortableStoreLimits)
        -> Result<ExportBundle, PortableStoreError>
    where Caps: cap::SubsetOf<cap::All>, cap::None: cap::SubsetOf<Caps>,
    {
        ...
        let mut total = 0;
        for (name, maximum) in [("portable.body_sizes", limits.max_bodies.min(self.limits.immutable_slots)),
            ("portable.issuance_sizes", limits.max_issuance.min(self.limits.version_tokens)),
            ("portable.head_sizes", self.limits.head_slots.min(1))]
        {
            live(cx)?;
            let rows = self.query(cx, name, &[]).await?;
            let row = rows.first().ok_or(EngineError::RowMissing { statement: name })?;
            let count = read_unsigned(row, 0).map_err(EngineError::from)?;
            if name == "portable.head_sizes" && count > 1 { return Err(PortableStoreError::MultipleHeads); }
            if count > maximum as u64 { return Err(PortableStoreError::Limit(name)); }
            add_bytes(&mut total, read_unsigned(row, 1).map_err(EngineError::from)?, limits.max_field_bytes)?;
        ...
        let mut bundle = ExportBundle { schema_version: SCHEMA_VERSION, instance: self.instance.raw(),
            bodies: Vec::new(), head: None, issuance: Vec::new() };
        let rows = self.query(cx, "portable.bodies", &[]).await?;
        live(cx)?;
        bundle.bodies.try_reserve_exact(rows.len()).map_err(|_| PortableStoreError::Allocation)?;
        for row in rows {
            live(cx)?;
        ...
        live(cx)?;
        bundle.issuance.try_reserve_exact(rows.len()).map_err(|_| PortableStoreError::Allocation)?;
        for row in rows {
            live(cx)?;
        ...
        live(cx)?;
        if rows.len() > 1 { return Err(PortableStoreError::MultipleHeads); }
    ...
    pub async fn import_portable<Caps>(&self, cx: &Cx<Caps>, bundle: &ExportBundle,
        limits: PortableStoreLimits,
    ) -> Result<Option<HeadReadReceipt>, PortableStoreError>
    where Caps: cap::SubsetOf<cap::All>, cap::None: cap::SubsetOf<Caps>,
    {
        validate_bundle(bundle, limits, self.limits, || live(cx))?;
        if bundle.instance == self.instance.raw() { return Err(PortableStoreError::SameStoreInstance); }
        let mut lease = self.operation(cx).await?;
        self.begin(cx, &mut lease).await?;
    ...
    async fn import_portable_snapshot<Caps>(&self, cx: &Cx<Caps>, bundle: &ExportBundle)
        -> Result<Option<HeadReadReceipt>, PortableStoreError>
    where Caps: cap::SubsetOf<cap::All>, cap::None: cap::SubsetOf<Caps>,
    {
        for name in ["body.count", "head.count", "issuance.count"] {
            live(cx)?;
            if self.occupancy(cx, name).await? != 0 { return Err(PortableStoreError::DestinationNotEmpty); }
        }
        for item in &bundle.bodies {
            live(cx)?;
            let count = self.execute(cx, "body.put_if_absent", &[blob(&item.key), blob(&item.body)]).await?;
            if count != 1 { return Err(PortableStoreError::UnexpectedWriteCount("immutable body")); }
        }
        for row in &bundle.issuance {
            ...
            let key = HeadKey::new(row.head_key.clone()).map_err(|_| PortableStoreError::InvalidKey)?;
            let generation = HeadGeneration::try_new(row.generation).map_err(|_| PortableStoreError::InvalidLineage)?;
            self.record_issuance(cx, token, sequence, &key, generation, &row.body).await?;
        }
        live(cx)?;
        let Some(head) = &bundle.head else { return Ok(None); };
        let tail = bundle.issuance.last().ok_or(PortableStoreError::InvalidLineage)?;
        let token = mint_token(self.instance, IssuanceSequence::new(tail.sequence).map_err(EngineError::from)?);
        let generation = HeadGeneration::try_new(head.generation).map_err(|_| PortableStoreError::InvalidLineage)?;
        let key = HeadKey::new(head.key.clone()).map_err(|_| PortableStoreError::InvalidKey)?;
        let count = self.execute(cx, "head.create_if_absent", &[
            blob(&head.key), blob(&token.to_opaque_bytes()),
            unsigned(head.generation).map_err(EngineError::from)?, blob(&head.body),
        ]).await?;
        if count != 1 { return Err(PortableStoreError::UnexpectedWriteCount("head")); }
        live(cx)?;
        Ok(Some(HeadReadReceipt::new(key, token, generation, head.body.clone())))
    }

    async fn rollback_portable<Caps>(&self, cx: &Cx<Caps>, lease: &mut OperationLease,
        cause: PortableStoreError,
    ) -> PortableStoreError
    where Caps: cap::SubsetOf<cap::All>, cap::None: cap::SubsetOf<Caps>,
    {
        match self.connection.rollback_transaction(cx).await {
            Ok(()) => { lease.finalized(); cause }
            Err(error) => PortableStoreError::Cleanup { cause: Box::new(cause), cleanup: EngineError::from(&error) },
        }
    }
}
