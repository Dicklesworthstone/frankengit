...
use fgit_authority::{
    AuthorityLimits, CasOutcome, HeadInit, HeadRead, ImmutableKey, ImmutableRead,
};
...
fn sample() -> ExportBundle {
    ...
    let issuance: Vec<_> = (1..=2)
        .map(|sequence| ExportedIssuance {
            token: mint_token(
...
fn bytes(bundle: &ExportBundle) -> u64 {
    let body: usize = bundle
        .bodies
        .iter()
        .map(|row| row.key.len() + row.body.len())
        .sum();
    let issued: usize = bundle
        .issuance
        .iter()
        .map(|row| row.token.len() + row.head_key.len() + row.body.len())
        .sum();
    let head = bundle
        .head
        .as_ref()
        .map_or(0, |row| row.key.len() + row.token.len() + row.body.len());
...
fn source_tokens_must_match_both_instance_and_sequence() {
    ...
    assert!(matches!(
        valid(&bundle),
        Err(PortableStoreError::InvalidSourceToken)
    ));
    let mut bundle = sample();
...
fn old_valid_head_missing_history_and_cross_key_issuance_are_refused() {
    ...
    assert!(
        ...
        "the wire-level check alone accepts the old genuine head"
    );
    assert!(matches!(
        valid(&bundle),
        Err(PortableStoreError::InvalidLineage)
    ));
    let mut bundle = sample();
...
fn limits_apply_to_payload_history_and_destination_capacity() {
    ...
    assert!(matches!(
        check(
            PortableStoreLimits {
                max_field_bytes: bytes(&bundle) - 1,
                ..Default::default()
            },
            AuthorityLimits::default()
        ),
        Err(PortableStoreError::Limit("retained field bytes"))
    ));
    assert!(
        check(
            PortableStoreLimits {
...
fn malformed_keys_noncanonical_order_and_cancelled_work_do_not_validate() {
    ...
    let result = validate_bundle(
        ...
        || {
            calls += 1;
            if calls == 3 {
                Err(PortableStoreError::Limit("cancel probe"))
            } else {
                Ok(())
            }
        },
...
fn real_engine_export_import_remints_tokens_and_continues_cas() {
    ...
    runtime.block_on(async {
        for name in [b"z".as_slice(), b"a"] {
            source
                .put_if_absent(&cx, &ImmutableKey::new(name.to_vec()).unwrap(), name)
                .await
                .unwrap();
        }
        let HeadInit::Created(first) = source
            .initialize_head(&cx, &key, HeadGeneration::FIRST, b"old")
            .await
            .unwrap()
        else {
            panic!("fresh source")
        ...
        let receipt = target
            .import_portable(&cx, &decoded, Default::default())
            .await
            .unwrap()
            .unwrap();
        ...
        assert!(
            target
                .authenticate_head_receipt(&cx, &current)
                .await
                .is_err()
        );
        assert_eq!(
            target
                .read_immutable(&cx, &ImmutableKey::new(b"a".to_vec()).unwrap())
                .await
                .unwrap(),
        ...
        assert_eq!(
            target.read_head(&cx, &key).await.unwrap(),
            HeadRead::Present(next)
        );
        assert_eq!(
            source.read_head(&cx, &key).await.unwrap(),
            HeadRead::Present(current)
        );
    });
...
fn dropped_import_lease_cannot_leak_staged_bodies_head_or_tokens_into_an_export() {
    ...
    runtime.block_on(async {
        ...
        let staged = target
            .import_portable_snapshot(&cx, &bundle)
            .await
            .unwrap()
            .unwrap();
        ...
        assert!(
            target
                .authenticate_head_receipt(&cx, &staged)
                .await
                .is_err(),
            "a staged but uncommitted receipt is not authentic"
        );
        let committed = target
            .import_portable(&cx, &bundle, Default::default())
            .await
            .unwrap()
            .unwrap();
        target
            .authenticate_head_receipt(&cx, &committed)
            .await
            .unwrap();
        assert_eq!(
            target
    ...
    for occupied in 0..3 {
        ...
        let mut target = runtime
            .block_on(FsqliteAuthorityStore::open(
                &cx,
                ":memory:",
                StoreInstanceId::from_raw(60 + occupied),
                AuthorityLimits::default(),
            ))
            .unwrap();
        runtime.block_on(async {
            ...
            match occupied {
                0 => {
                    target
                        .put_if_absent(
                            &cx,
                            &ImmutableKey::new(b"preexisting/body".to_vec()).unwrap(),
                            b"keep",
                        )
                        .await
                        .unwrap();
                ...
                1 => {
                    target
                        .initialize_head(&cx, &key, HeadGeneration::FIRST, b"keep")
                        .await
                        .unwrap();
                }
                _ => {
                    ...
                    target
                        .record_issuance(
                            &cx,
                            mint_token(target.instance_id(), sequence),
                            sequence,
                            &key,
            ...
            assert!(matches!(
                target
                    .import_portable(&cx, &sample(), Default::default())
                    .await,
                Err(PortableStoreError::DestinationNotEmpty)
            ));
            for (table, count) in ["body.count", "head.count", "issuance.count"]
                .into_iter()
                .zip(before)
            {
            ...
            assert_eq!(
                target
                    .read_immutable(&cx, &ImmutableKey::new(b"body".to_vec()).unwrap())
                    .await
                    .unwrap(),
...
fn export_limits_and_multi_head_refusal_finalize_the_snapshot_without_changing_state() {
    ...
    runtime.block_on(async {
        ...
        for limits in [
            ...
            PortableStoreLimits {
                max_field_bytes: bytes(&original) - 1,
                ..Default::default()
            },
        ] {
            assert!(matches!(
                target.export_portable(&cx, limits).await,
        ...
        let HeadInit::Created(second) = target
            .initialize_head(&cx, &key, HeadGeneration::FIRST, b"other")
            .await
            .unwrap()
        else {
            panic!("new second slot")
        };
        assert!(matches!(
            target.export_portable(&cx, Default::default()).await,
            Err(PortableStoreError::MultipleHeads)
        ));
        assert!(!target.connection.in_transaction());
        assert_eq!(
            target.read_head(&cx, &key).await.unwrap(),
...
fn portable_source_open_does_not_initialize_an_unrelated_database() {
    ...
    runtime.block_on(async {
        let mut unrelated =
            fsqlite::AsyncConnection::open(&cx, database.to_string_lossy().to_string())
                .await
                .unwrap();
        unrelated
            .execute(
            ...
            .unwrap();
        unrelated
            .execute(&cx, "INSERT INTO unrelated VALUES (42)")
            .await
            .unwrap();
        unrelated.close(&cx).await.unwrap();
        assert!(
            FsqliteAuthorityStore::open_portable_source(
                &cx,
        ...
        let mut check = fsqlite::AsyncConnection::open(&cx, database.to_string_lossy().to_string())
            .await
            .unwrap();
        let names = check
            .query_with_params(
        ...
        assert_eq!(
            names.len(),
            1,
            "source opener must not add authority tables"
        );
        let values = check
            .query_with_params(&cx, "SELECT value FROM unrelated", &[])
            .await
            .unwrap();
