...
use super::{
    Cx, EngineError, ExportBundle, FsqliteAuthorityStore, HeadGeneration, HeadKey, HeadReadReceipt,
    IssuanceSequence, PortableStoreError, PortableStoreLimits, StoreInstanceId, cap, live,
    mint_token, validate_bundle,
};
...
impl FsqliteAuthorityStore {
    ...
    {
        self.reconcile_portable_import(cx, source, limits, true)
            .await
    }
    ...
    where
        ...
        cap::None: cap::SubsetOf<Caps>,
    {
        self.reconcile_portable_import(cx, source, limits, false)
            .await
    }
        ...
        cap::None: cap::SubsetOf<Caps>,
    {
        ...
        let inspected = async {
            ...
            live(cx)?;
            let empty = observed.bodies.is_empty()
                && observed.issuance.is_empty()
...
fn match_image(
    ...
    for (expected, actual) in source.issuance.iter().zip(&observed.issuance) {
        ...
        if actual.sequence != expected.sequence
            || actual.head_key != expected.head_key
            || actual.generation != expected.generation
            || actual.body != expected.body
            || actual.token.as_slice()
