...
use super::super::{
    ...
    HeadGeneration, HeadKey, HeadReadReceipt, IssuanceSequence, PortableStoreError,
    SCHEMA_VERSION, StoreInstanceId, add_bytes, blob, cap, live, mint_token,
    read_blob, read_unsigned, unsigned,
};

#[derive(Clone, Copy)]
enum ImportMode { Fresh, Resume, Verify }
...
impl FsqliteAuthorityStore {
    ...
    pub async fn export_multi_head_portable<Caps>(&self, cx: &Cx<Caps>, limits: MultiHeadLimits)
    ...
    async fn export_multi_head_snapshot<Caps>(&self, cx: &Cx<Caps>, limits: MultiHeadLimits)
        -> Result<MultiHeadSnapshot, PortableStoreError>
    where Caps: cap::SubsetOf<cap::All>, cap::None: cap::SubsetOf<Caps>,
    {
        let mut total = 0;
        for (statement, maximum) in [
            ("portable.body_sizes", limits.portable.max_bodies.min(self.limits.immutable_slots)),
            ("portable.issuance_sizes", limits.portable.max_issuance.min(self.limits.version_tokens)),
            ("portable.head_sizes", limits.max_heads.min(self.limits.head_slots)),
        ] {
            live(cx)?;
            ...
            add_bytes(&mut total, read_unsigned(row, 1).map_err(EngineError::from)?,
        ...
        let mut snapshot = MultiHeadSnapshot { schema_version: SCHEMA_VERSION,
            instance: self.instance.raw(), bodies: Vec::new(), heads: Vec::new(), issuance: Vec::new() };
        let rows = self.query(cx, "portable.bodies", &[]).await?;
        live(cx)?;
        snapshot.bodies.try_reserve_exact(rows.len()).map_err(|_| PortableStoreError::Allocation)?;
        for row in rows {
            live(cx)?;
        ...
        live(cx)?;
        snapshot.issuance.try_reserve_exact(rows.len()).map_err(|_| PortableStoreError::Allocation)?;
        for row in rows {
            live(cx)?;
        ...
        live(cx)?;
        snapshot.heads.try_reserve_exact(rows.len()).map_err(|_| PortableStoreError::Allocation)?;
        for row in rows {
            live(cx)?;
    ...
    pub async fn import_multi_head_portable<Caps>(&self, cx: &Cx<Caps>, source: &MultiHeadSnapshot,
        limits: MultiHeadLimits,
    ) -> Result<Vec<HeadReadReceipt>, PortableStoreError>
    where Caps: cap::SubsetOf<cap::All>, cap::None: cap::SubsetOf<Caps>,
    {
        self.reconcile_multi_head_import(cx, source, limits, ImportMode::Fresh).await
    }
    ...
    pub async fn resume_multi_head_import<Caps>(&self, cx: &Cx<Caps>, source: &MultiHeadSnapshot,
        limits: MultiHeadLimits,
    ) -> Result<Vec<HeadReadReceipt>, PortableStoreError>
    where Caps: cap::SubsetOf<cap::All>, cap::None: cap::SubsetOf<Caps>,
    {
        self.reconcile_multi_head_import(cx, source, limits, ImportMode::Resume).await
    }
    ...
    pub async fn verify_multi_head_import<Caps>(&self, cx: &Cx<Caps>, source: &MultiHeadSnapshot,
        limits: MultiHeadLimits,
    ) -> Result<Vec<HeadReadReceipt>, PortableStoreError>
    where Caps: cap::SubsetOf<cap::All>, cap::None: cap::SubsetOf<Caps>,
    {
        self.reconcile_multi_head_import(cx, source, limits, ImportMode::Verify).await
    }

    async fn reconcile_multi_head_import<Caps>(&self, cx: &Cx<Caps>, source: &MultiHeadSnapshot,
        limits: MultiHeadLimits, mode: ImportMode,
    ) -> Result<Vec<HeadReadReceipt>, PortableStoreError>
    where Caps: cap::SubsetOf<cap::All>, cap::None: cap::SubsetOf<Caps>,
    {
        source.validate_with(limits, self.limits, || live(cx))?;
        if source.instance == self.instance.raw() { return Err(PortableStoreError::SameStoreInstance); }
        let mut lease = self.operation(cx).await?;
        self.begin(cx, &mut lease).await?;
        let prepared = async {
            let mut empty = true;
            ...
            if empty && !matches!(mode, ImportMode::Verify) {
                return self.import_multi_head_snapshot(cx, source).await.map(|receipts| (receipts, true));
            }
    ...
    async fn import_multi_head_snapshot<Caps>(&self, cx: &Cx<Caps>, source: &MultiHeadSnapshot)
        -> Result<Vec<HeadReadReceipt>, PortableStoreError>
    where Caps: cap::SubsetOf<cap::All>, cap::None: cap::SubsetOf<Caps>,
    {
        let receipts = receipts_for(source, self.instance, || live(cx))?;
        for row in &source.bodies {
            live(cx)?;
            if self.execute(cx, "body.put_if_absent", &[blob(&row.key), blob(&row.body)]).await? != 1 {
        ...
        for row in &source.issuance {
            ...
            self.record_issuance(cx, mint_token(self.instance, sequence), sequence, &key, generation, &row.body).await?;
        }
        for receipt in &receipts {
            live(cx)?;
            let count = self.execute(cx, "head.create_if_absent", &[
                blob(receipt.key().as_bytes()), blob(&receipt.token().to_opaque_bytes()),
                unsigned(receipt.generation().get()).map_err(EngineError::from)?, blob(receipt.body()),
            ]).await?;
...
fn receipts_for(source: &MultiHeadSnapshot, instance: StoreInstanceId,
    mut checkpoint: impl FnMut() -> Result<(), PortableStoreError>,
) -> Result<Vec<HeadReadReceipt>, PortableStoreError> {
    let mut receipts = Vec::new();
    receipts.try_reserve_exact(source.heads.len()).map_err(|_| PortableStoreError::Allocation)?;
    for head in &source.heads {
        ...
        let sequence: [u8; 8] = head.token.get(8..).ok_or(PortableStoreError::InvalidSourceToken)?
            .try_into().map_err(|_| PortableStoreError::InvalidSourceToken)?;
        let sequence = IssuanceSequence::new(u64::from_be_bytes(sequence)).map_err(EngineError::from)?;
        receipts.push(HeadReadReceipt::new(
            ...
            HeadGeneration::try_new(head.generation).map_err(|_| PortableStoreError::InvalidLineage)?,
            head.body.clone(),
        ));
    }
    Ok(receipts)
...
fn match_image(source: &MultiHeadSnapshot, actual: &MultiHeadSnapshot, instance: StoreInstanceId,
    mut checkpoint: impl FnMut() -> Result<(), PortableStoreError>,
) -> Result<(), PortableStoreError> {
    let mismatch = || PortableStoreError::DestinationSnapshotMismatch;
    checkpoint()?;
    if actual.instance != instance.raw() || actual.schema_version != source.schema_version
        || actual.bodies.len() != source.bodies.len() || actual.heads.len() != source.heads.len()
        || actual.issuance.len() != source.issuance.len()
    { return Err(mismatch()); }
    for (expected, observed) in source.bodies.iter().zip(&actual.bodies) {
        checkpoint()?;
        if expected != observed { return Err(mismatch()); }
    }
    for (expected, observed) in source.issuance.iter().zip(&actual.issuance) {
        ...
        if expected.sequence != observed.sequence || expected.head_key != observed.head_key
            || expected.generation != observed.generation || expected.body != observed.body
            || observed.token.as_slice() != mint_token(instance, sequence).to_opaque_bytes().as_slice()
        { return Err(mismatch()); }
    }
    ...
    for (expected, observed) in source.heads.iter().zip(&actual.heads) {
        checkpoint()?;
        if expected.key != observed.key { return Err(mismatch()); }
    }
    checkpoint()
}
