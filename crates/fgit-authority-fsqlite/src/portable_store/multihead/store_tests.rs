...
use super::*;
use fgit_authority::{
    AuthorityLimits, AuthorityVersionToken, CasOutcome, HeadRead, ImmutableKey, ImmutableRead,
};
...
fn whole_store_round_trip_authenticates_each_head_and_continues_the_global_ledger() {
    ...
    runtime.block_on(async {
        let receipts = store
            .import_multi_head_portable(&cx, &source, Default::default())
            .await
            .unwrap();
        assert_eq!(receipts.len(), 2);
        for receipt in &receipts {
            assert_eq!(
                store.read_head(&cx, receipt.key()).await.unwrap(),
                HeadRead::Present(receipt.clone())
            );
            store.authenticate_head_receipt(&cx, receipt).await.unwrap();
        }
        for row in &source.issuance {
            store
                .authenticate_head_receipt(&cx, &issued(row, store.instance_id()))
                .await
                .unwrap();
            assert!(
                store
                    .authenticate_head_receipt(
                        &cx,
                        &issued(row, StoreInstanceId::from_raw(source.instance))
                    )
                    .await
                    .is_err()
            );
        ...
        let before = store
            .export_multi_head_portable(&cx, Default::default())
            .await
            .unwrap();
        assert_eq!(before.bodies, source.bodies);
        assert_eq!(before.heads.len(), 2);
        for (offset, old) in receipts.iter().enumerate() {
            let outcome = store
                .compare_exchange_head(
                    ...
                    old.key(),
                    old.token(),
                    old.generation().next().unwrap(),
                    b"after restore",
                )
                .await
                .unwrap();
            let CasOutcome::Committed(receipt) = outcome else {
                panic!("next CAS must commit");
            };
            assert_eq!(
                receipt.token(),
                mint_token(
                    store.instance_id(),
                    IssuanceSequence::new(5 + offset as u64).unwrap()
                )
            );
        }
        let after = store
            ...
            .await
            .unwrap();
        assert_eq!(after.issuance.len(), 6);
        assert_eq!(after.heads.len(), 2);
    });
...
fn exact_resumes_do_not_write_but_changed_or_extra_state_cannot_be_adopted() {
    ...
    runtime.block_on(async {
        let first = store
            .resume_multi_head_import(&cx, &source, Default::default())
            .await
            .unwrap();
        let original = store
            .export_multi_head_portable(&cx, Default::default())
        ...
        for _ in 0..3 {
            ...
            assert_eq!(
                store
                    .verify_multi_head_import(&cx, &source, Default::default())
                    .await
                    .unwrap(),
                first
            );
...
fn an_advanced_nonfirst_head_refuses_resume_without_rewinding_either_slot() {
    ...
    runtime.block_on(async {
        ...
        assert_eq!(
            store.read_head(&cx, receipts[0].key()).await.unwrap(),
            HeadRead::Present(receipts[0].clone())
        );
    });
    close(runtime, cx, store);
}
...
fn abandoned_import_transaction_does_not_publish_any_head_body_or_token() {
    ...
    runtime.block_on(async {
        ...
        for receipt in &staged {
            assert!(store.authenticate_head_receipt(&cx, receipt).await.is_err());
        }
        assert!(
            store
                .verify_multi_head_import(&cx, &source, Default::default())
        ...
        assert_eq!(
            store
                .read_immutable(&cx, &ImmutableKey::new(vec![0]).unwrap())
                .await
                .unwrap(),
...
fn canceled_or_over_limit_calls_leave_the_connection_and_snapshot_usable() {
    ...
    runtime.block_on(async {
        ...
        assert!(
            store
                .import_multi_head_portable(&cancelled, &source, Default::default())
                .await
                .is_err()
        );
        assert!(
            store
                .export_multi_head_portable(&cancelled, Default::default())
                .await
                .is_err()
        );
        assert!(
        ...
        let too_small = MultiHeadLimits {
            max_heads: 1,
            ..Default::default()
        };
        assert!(
            store
                .import_multi_head_portable(&cx, &source, too_small)
                .await
                .is_err()
        );
        store
        ...
        assert!(
            store
                .export_multi_head_portable(&cx, too_small)
                .await
                .is_err()
        );
        ...
        assert!(matches!(
            store.export_portable(&cx, Default::default()).await,
...
fn head_writes_cannot_interleave_with_the_multi_head_snapshot_transaction() {
    ...
    runtime.block_on(async {
        ...
        store.begin(&cx, &mut lease).await.unwrap();
        let snapshot = store
            .export_multi_head_snapshot(&cx, Default::default())
            .await
            .unwrap();
        let old = &receipts[1];
        let mut writer = Box::pin(store.compare_exchange_head(
            &cx,
            old.key(),
            old.token(),
            generation(3),
        ...
        poll_fn(|task| {
            assert!(writer.as_mut().poll(task).is_pending());
            Poll::Ready(())
        })
        .await;
        assert_eq!(
        ...
        store.connection.rollback_transaction(&cx).await.unwrap();
        lease.finalized();
        drop(lease);
        ...
        assert_eq!(after.heads[0], snapshot.heads[0]);
        assert_ne!(after.heads[1], snapshot.heads[1]);
    });
    close(runtime, cx, store);
}
...
fn an_occupied_ledger_without_any_head_is_not_an_empty_restore_target() {
    ...
    runtime.block_on(async {
        ...
        store
            .record_issuance(
                &cx,
                AuthorityVersionToken::from_opaque_bytes([9; 16]),
        ...
        assert!(matches!(
            store
                .import_multi_head_portable(&cx, &sample(), Default::default())
                .await,
            Err(PortableStoreError::DestinationNotEmpty)
        ));
        assert!(
            store
                .resume_multi_head_import(&cx, &sample(), Default::default())
                .await
                .is_err()
        );
