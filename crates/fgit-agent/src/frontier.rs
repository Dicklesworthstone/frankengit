//! Deterministic work-frontier construction for the Agent Control Plane.
//!
//! Given one authority-bound [`crate::AgentSituationReceipt`] and a bounded set
//! of task-projection rows, this module determines which rows are actionable for
//! the receipt's active Intent Run, retains a typed reason for every excluded
//! row, and orders only eligible rows by one closed deterministic policy.
//!
//! It does not read or mutate Beads, grant capabilities, reserve files, edit a
//! workspace, execute evidence, or publish repository state. Ranking is
//! advisory; the hard eligibility filter runs first and no score can make an
//! ineligible row actionable.
...
/// Eligibility inputs supplied by task and coordination projections.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl WorkFrontier {
    ...
    ///
    /// The hard eligibility precedence is stale projection, terminal phase,
    /// blockers, missing run, assignment, verifier independence, capability,
    /// then conflict state. Only rows that pass every hard precondition enter
    /// advisory ordering.
    ///
    /// # Errors
    ///
    /// Refuses an omitted or inconsistent task-projection component, more than
    /// [`MAX_WORK_ITEMS`] rows, the all-zero task identity, duplicate task
    /// identities, and unrepresentable commitment framing.
    pub fn build(
        situation: &AgentSituationReceipt,
        items: Vec<WorkItem>,
    ) -> Result<Self, FrontierRefusal> {
        Self::build_from_basis(FrontierBasis::from_situation(situation)?, items)
    }
    ...
    fn build_from_basis(
        ...
        for item in items {
            match exclusion_reason(&basis, item) {
                ...
                None => {
                    let action = item.phase.required_action().ok_or(
                        FrontierRefusal::InconsistentEligiblePhase {
...
pub enum FrontierRefusal {
    ...
    /// The bounded input ceiling was exceeded before sorting.
    TooManyItems {
        /// Rows supplied.
        observed: usize,
        /// Closed v1 ceiling.
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InconsistentTaskProjectionComponent => formatter.write_str(
                "task-projection component is neither consistently observed nor omitted",
            ),
            ...
            Self::ZeroTaskId => {
                formatter.write_str("work frontier refuses the all-zero task identity")
            }
            ...
            Self::Codec(refusal) => {
                write!(formatter, "work-frontier commitment refused: {refusal}")
            }
        }
    }
}
...
fn frontier_commitment(
    ...
    let item_count = candidates.len() + excluded.len();
    let mut encoder = Encoder::with_capacity(256 + item_count * 192);
...
mod tests {
    ...
    #[test]
    fn duplicate_zero_and_oversized_inputs_are_refused() {
        let run = RunId::new(7);
