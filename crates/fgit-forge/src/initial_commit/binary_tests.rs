...
fn decoder_opt_in_produces_the_same_complete_native_root_as_literal_bytes() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let input = compressed(format, "asset.bin");
        let mut calls = 0;
        let actual = prepare_initial_commit_with_binary_decoder(
            format,
            &input,
            &metadata(),
            PatchLimits::default(),
            ...
            |bytes, index, base| {
                calls += 1;
                assert_eq!(bytes, HUNKS.as_bytes());
                assert!(base.is_empty());
                assert!(index.old.iter().all(|b| *b == b'0'));
                Ok(BODY.to_vec())
            },
        )
        .unwrap();
        let expected = prepare_initial_commit(
            format,
            &literal(),
            &metadata(),
            PatchLimits::default(),
        ...
        assert_eq!(actual.patch_sha256, sha256_digest(&input));
        assert!(
...
fn mixed_literal_empty_and_compressed_files_only_decode_the_compressed_record() {
    ...
    let mut input = compressed(format, "asset.bin");
    input.extend_from_slice(b"diff --git a/empty b/empty\nnew file mode 100755\ndiff --git a/readme b/readme\nnew file mode 100644\n--- /dev/null\n+++ b/readme\n@@ -0,0 +1 @@\n+hello\n");
    let mut calls = 0;
    let plan = prepare_initial_commit_with_binary_decoder(
        format,
        &input,
        &metadata(),
        PatchLimits::default(),
    ...
    assert_eq!(plan.files[1].bytes, 0);
    assert_eq!(plan.files[1].mode, 0o100755);
    assert_eq!(
        plan.files[2].blob,
...
fn a_late_noncreation_refuses_before_any_decode() {
    ...
    let mut calls = 0;
    let result = prepare_initial_commit_with_binary_decoder(
    ...
    assert!(matches!(result, Err(InitialCommitError::CreationRequired)));
    assert_eq!(calls, 0);
}
...
fn binary_indexes_must_use_the_selected_full_native_hash_domain() {
    for (format, other) in [
        ...
        let input = compressed(other, "asset.bin");
        let mut calls = 0;
        let result = prepare_initial_commit_with_binary_decoder(
            format,
            &input,
        ...
        assert!(matches!(result, Err(InitialCommitError::IndexMismatch)));
        assert_eq!(calls, 0);
    }
}
...
fn decoder_results_still_require_exact_blob_identities_and_complete_closure_budget() {
    ...
    let wrong = prepare_initial_commit_with_binary_decoder(
        GitHashAlgorithm::Sha1,
        &input,
        &metadata(),
        PatchLimits::default(),
        &|| false,
        |_, _, _| Ok(b"different".to_vec()),
    );
    assert!(matches!(wrong, Err(InitialCommitError::IndexMismatch)));
    let bounded = prepare_initial_commit_with_binary_decoder(
        GitHashAlgorithm::Sha1,
        &input,
        &metadata(),
        PatchLimits {
            max_output_bytes: BODY.len(),
            ..PatchLimits::default()
        },
        &|| false,
...
fn decoder_error_after_an_earlier_file_never_returns_a_partial_plan() {
    ...
    input.extend(compressed(GitHashAlgorithm::Sha256, "b"));
    let mut calls = 0;
    let result = prepare_initial_commit_with_binary_decoder(
        ...
        |_, _, _| {
            ...
            if calls == 1 {
                ...
                Err(PatchError::Budget("test decoder"))
            }
        },
    );
    assert_eq!(calls, 2);
    assert!(matches!(
        result,
        Err(InitialCommitError::Patch(PatchError::Budget(_)))
    ));
}
