...
use fgit_diff::{
    CommitGraph, DiffAlgorithm, DiffError, DiffLimits, DiffOptions, Edit, MergeBaseError,
    MergeBaseLimits, MergeBaseResult, ParentSet, RenameProfile, TreeChange, TreeDiffLimits,
    TreeDiffOptions, TreeEntry, TreeMode, diff_trees, diff_with_cancellation, merge_bases_all,
};
...
impl ReviewOptions {
    pub fn validate(&self) -> Result<(), ReviewError> {
        ...
        for (value, ceiling) in [
            (limits.max_tree_entries, max.max_tree_entries),
            (limits.max_changes, max.max_changes),
            (limits.max_text_files, max.max_text_files),
            (limits.max_blob_bytes, max.max_blob_bytes),
            (limits.max_output_bytes, max.max_output_bytes),
            (limits.max_hunks, max.max_hunks),
            (limits.max_diff_work, max.max_diff_work),
        ] {
...
pub struct ReviewedEntry {
    pub path: Vec<u8>,
    pub before: Option<EntryIdentity>,
    pub after: Option<EntryIdentity>,
    pub kind: ChangeKind,
...
impl<S: MergeObjectSource> CommitGraph for Graph<'_, S> {
    ...
    fn parents_of(&self, id: &GitOid) -> Result<ParentSet<GitOid>, MergeSourceError> {
        self.0.checkpoint()?;
        if id.is_zero() || id.algorithm() != self.1 {
            return Err(MergeSourceError::InvalidObject(*id));
        }
        let commit = self.0.commit(*id)?;
        if commit.tree.is_zero()
            || commit.tree.algorithm() != self.1
            || commit
                .parents
                .iter()
                .any(|id| id.is_zero() || id.algorithm() != self.1)
        {
            return Err(MergeSourceError::InvalidObject(*id));
        }
        self.0.checkpoint()?;
...
pub fn compare_source<S: MergeObjectSource>(
    ...
    let compared_before = match options.mode {
        ...
        ComparisonMode::MergeBase => match merge_bases_all(
            &Graph(source, format),
            before,
            after,
            MergeBaseLimits {
...
impl<S: MergeObjectSource> Walker<'_, S> {
    fn tree(&mut self, id: Option<GitOid>) -> Result<Vec<TreeEntry<GitOid>>, ReviewError> {
        ...
        self.trees = self
            ...
            .filter(|n| *n <= self.options.limits.max_tree_entries)
    ...
    fn directory(
        ...
        if depth > 64 {
            return Err(ReviewError::Budget("tree depth"));
        }
        let old_entries = self.tree(old)?;
        let new_entries = self.tree(new)?;
        let differences = diff_trees(
            old_entries,
            new_entries,
            TreeDiffOptions {
                rename: RenameProfile::Disabled,
                limits: TreeDiffLimits {
                    max_entries_per_tree: self.options.limits.max_tree_entries,
        ...
        for change in differences.changes {
            let (old, new) = match change {
                ...
                TreeChange::Modified { before, after }
                | TreeChange::ModeChanged { before, after, .. } => (Some(before), Some(after)),
                TreeChange::Renamed { .. } => return Err(ReviewError::InvalidTree),
            };
            let name = old
                .as_ref()
                .or(new.as_ref())
                .ok_or(ReviewError::InvalidTree)?
                .path
                .clone();
        ...
        for (name, (old, new)) in pairs {
            ...
            let old_tree = old
                .as_ref()
                .filter(|entry| is_tree(entry.mode.0))
            ...
            if self.options.selected(&path) {
                self.entry(
                    path.clone(),
                    old.as_ref().map(identity),
                    new.as_ref().map(identity),
                )?;
            }
    ...
    fn blob(&self, entry: Option<EntryIdentity>) -> Result<Vec<u8>, ReviewError> {
        let Some(entry) = entry.filter(|entry| is_blob(entry.mode)) else {
            return Ok(Vec::new());
        };
        self.source.checkpoint()?;
        let body = self.source.blob(entry.oid)?;
        if body.len() > self.options.limits.max_blob_bytes {
            return Err(ReviewError::Budget("blob bytes"));
        }
        self.source.checkpoint()?;
        Ok(body)
    }
    fn entry(
        ...
        if self.entries.len() >= self.options.limits.max_changes {
            return Err(ReviewError::Budget("changed entries"));
        }
        ...
        let kind = match (before, after) {
            ...
            _ => return Err(ReviewError::InvalidTree),
        };
        let content = if before
            .zip(after)
            .is_some_and(|(a, b)| a.oid == b.oid && is_blob(a.mode) && is_blob(b.mode))
        {
            ReviewContent::Identical
        } else if !before.is_some_and(|e| is_blob(e.mode))
            && !after.is_some_and(|e| is_blob(e.mode))
        {
            ...
            if old.contains(&0) || new.contains(&0) {
                ReviewContent::Binary {
                    before_bytes: old.len(),
                    after_bytes: new.len(),
                }
            } else {
                self.text_files += 1;
                if self.text_files > self.options.limits.max_text_files {
                    return Err(ReviewError::Budget("text files"));
                }
                self.text(&old, &new)?
            }
        };
        self.entries.push(ReviewedEntry {
            path,
            before,
            after,
            kind,
    ...
    fn charge(&mut self, bytes: usize) -> Result<(), ReviewError> {
        self.output_bytes = self
            .output_bytes
            .checked_add(bytes)
            .filter(|n| *n <= self.options.limits.max_output_bytes)
    ...
    fn text(&mut self, old: &[u8], new: &[u8]) -> Result<ReviewContent, ReviewError> {
        ...
        let result = diff_with_cancellation(
            old,
            new,
            DiffOptions::myers_lines(DiffLimits {
                max_input_bytes: 2 * self.options.limits.max_blob_bytes,
                max_units: 100_000,
                max_work: self.options.limits.max_diff_work,
                max_trace_cells: 512_000,
            }),
            &|| match self.source.checkpoint() {
                Ok(()) => false,
                Err(error) => {
                    stopped.set(Some(error));
                    true
                }
            },
        );
        ...
        if let Some(error) = stopped.into_inner() {
            return Err(ReviewError::Source(error));
        }
        self.source.checkpoint()?;
        ...
        for edit in &result.edits {
            ...
            let range = (
                a.unit_start.saturating_sub(context),
                (a.unit_end + context).min(old_lines.len() - 1),
                b.unit_start.saturating_sub(context),
                (b.unit_end + context).min(new_lines.len() - 1),
            );
        ...
        for (a, b, c, d) in ranges {
            ...
            let before = old
                .get(old_span.byte_start..old_span.byte_end)
                .ok_or(ReviewError::Diff(DiffError::MalformedScript))?;
            let after = new
                .get(new_span.byte_start..new_span.byte_end)
                .ok_or(ReviewError::Diff(DiffError::MalformedScript))?;
...
fn span(offsets: &[usize], start: usize, end: usize) -> Result<ReviewSpan, ReviewError> {
    if start > end {
        return Err(ReviewError::Diff(DiffError::MalformedScript));
    }
    Ok(ReviewSpan {
        byte_start: *offsets
            .get(start)
