...
impl PreparationLimits {
    pub fn validate(self) -> Result<(), PreparationError> {
        ...
        for (value, ceiling) in [
            (self.max_commits, maximum.max_commits),
            (self.max_edges, maximum.max_edges),
            ...
            (self.max_conflicts, maximum.max_conflicts),
            (self.max_objects, maximum.max_objects),
    ...
    pub fn validate(&self) -> Result<(), PreparationError> {
        for identity in [&self.author, &self.committer] {
            ...
            if identity.len() > 1024
                ...
                || !address.ends_with('>')
                || address.len() <= 1
                || address[..address.len() - 1].contains(['<', '>'])
...
impl From<MergeSourceError> for PreparationError {
    fn from(value: MergeSourceError) -> Self {
        Self::Source(value)
    }
}
    ...
    fn parents_of(&self, id: &GitOid) -> Result<ParentSet<GitOid>, MergeSourceError> {
        ...
        if commit.tree.is_zero()
            ...
            || commit
                .parents
                .iter()
                .any(|parent| parent.is_zero() || parent.algorithm() != self.format)
        {
            return Err(MergeSourceError::InvalidObject(*id));
        }
        self.source.checkpoint()?;
...
pub fn prepare_merge_with_profile<S: MergeObjectSource>(
    ...
    if base == incoming {
        return Ok(MergePreparation::AlreadyUpToDate { target });
    }
    ...
    if !planner.conflicts.is_empty() {
        ...
        return Ok(MergePreparation::Conflicted {
            base,
...
impl<'a, S: MergeObjectSource> Planner<'a, S> {
    ...
    fn emit(&mut self, kind: GitObjectKind, body: Vec<u8>) -> Result<GitOid, PreparationError> {
        ...
        self.objects
            .insert(id, PlannedMergeObject { id, kind, body });
        Ok(id)
    }
    ...
    fn entries(
        &mut self,
        id: Option<GitOid>,
    ) -> Result<BTreeMap<Vec<u8>, MergeEntry>, PreparationError> {
        let Some(id) = id else {
        ...
        for entry in entries {
            ...
            if entry.name.is_empty()
                || entry.name.len() > self.limits.max_path_bytes
                || entry.name.contains(&0)
                || entry.name.contains(&b'/')
                || entry.name == b"."
                || entry.name == b".."
                || entry.name.eq_ignore_ascii_case(b".git")
                || !matches!(
                    entry.mode,
                    0o040000 | 0o100644 | 0o100755 | 0o120000 | 0o160000
                )
                || entry.oid.is_zero()
                || entry.oid.algorithm() != self.format
            {
                return Err(PreparationError::InvalidTree);
            }
    ...
    fn directory(
        ...
        let attributes = inherited_attributes
            || [&b, &o, &t]
                .iter()
        ...
        for entry in &result {
            ...
            body.try_reserve(new_len - body.len())
        ...
        if Some(id) == base || id == ours || id == theirs {
            return Ok(Some(id));
        }
    ...
    fn conflict(
        ...
        if let Some(bound) = self.resolutions.remove(path) {
            ...
            if bound.conflict.kind != kind
                || bound.conflict.base.as_ref() != base
                || bound.conflict.ours.as_ref() != ours
                || bound.conflict.theirs.as_ref() != theirs
            {
                return Err(PreparationError::InvalidTree);
            }
        ...
        if self.conflicts.len() == self.limits.max_conflicts {
            return Err(PreparationError::Budget("conflicts"));
        }
    ...
    fn entry(
        ...
        if o.mode == 0o040000 && t.mode == 0o040000 && base.is_none_or(|b| b.mode == 0o040000) {
            ...
            return Ok(merged.map(|oid| MergeEntry {
                name: o.name.clone(),
                mode: 0o040000,
                oid,
            }));
        ...
        let mode = if o.mode == t.mode || base.is_some_and(|b| b.mode == t.mode) {
            o.mode
        } else if base.is_some_and(|b| b.mode == o.mode) {
            t.mode
        } else {
            return self.conflict(path, ConflictKind::Mode, base, ours, theirs);
        };
        let oid = if o.oid == t.oid || base.is_some_and(|b| b.oid == t.oid) {
            o.oid
        } else if base.is_some_and(|b| b.oid == o.oid) {
            t.oid
        } else {
            if attributes {
                return self.conflict(
                    path,
                    ConflictKind::AttributesRequireDriver,
                    base,
                    ours,
                    theirs,
                );
            ...
            let merged = merge_content_with_cancellation(
                &b,
                &o_bytes,
                &t_bytes,
                options,
                &Cancellation(self.source),
            );
            self.source.checkpoint()?;
            match merged
                .map_err(|error| PreparationError::Content {
                    path: path.to_vec(),
                    error,
                })?
                .outcome
            {
                ContentMergeOutcome::Clean { bytes } => {
                    if bytes == o_bytes {
                        o.oid
                    } else if bytes == t_bytes {
                        t.oid
                    } else if bytes == b && base.is_some() {
                        base.ok_or(PreparationError::InvalidTree)?.oid
                    } else {
                        self.emit(GitObjectKind::Blob, bytes)?
                    }
                }
...
mod tests {
    ...
    impl Source {
        ...
        fn store_tree(&mut self, mut entries: Vec<MergeEntry>) -> GitOid {
            ...
            let mut body = Vec::new();
            for entry in &entries {
                body.extend(format!("{:o} ", entry.mode).as_bytes());
                body.extend(&entry.name);
                body.push(0);
                body.extend(entry.oid.as_bytes());
            }
            let id = git_object_id(self.format, GitObjectKind::Tree, &body);
    ...
    fn branch_pair(
        ...
        let bt = source.store_tree(b);
        let base = source.store_commit(bt, &[], "base");
        let ot = source.store_tree(o);
        let ours = source.store_commit(ot, &[base], "ours");
        let tt = source.store_tree(t);
        let theirs = source.store_commit(tt, &[base], "theirs");
        (ours, theirs)
    }
    ...
    fn recursive_text_mode_and_unchanged_subtrees_are_deterministic_in_both_formats() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            ...
            let bt = s.store_tree(vec![base_file]);
            let ot = s.store_tree(vec![our_file]);
            let tt = s.store_tree(vec![their_file]);
            let dir = |oid| MergeEntry {
                name: b"dir".to_vec(),
                mode: 0o040000,
                oid,
            };
            let keep = s.file(b"keep", b"do not copy me\n", 0o100644);
            let (ours, theirs) = branch_pair(
                &mut s,
                vec![dir(bt), keep.clone()],
                vec![dir(ot), keep.clone()],
                vec![dir(tt), keep.clone()],
            );
            let result = prepare_merge(
                &s,
                format,
                ours,
                theirs,
            ...
            let MergePreparation::Clean(plan) = result else {
                panic!("separate lines must merge");
            };
            assert!(plan.objects.iter().any(
                |o| o.kind == GitObjectKind::Blob && o.body == b"ONE\ntwo\nthree\nfour\nFIVE\n"
            ));
            assert!(!plan.objects.iter().any(|o| o.id == keep.oid));
            let commit = plan.objects.iter().find(|o| o.id == plan.commit).unwrap();
            assert!(String::from_utf8_lossy(&commit.body).starts_with(&format!(
                "tree {}\nparent {ours}\nparent {theirs}\n",
                plan.tree
            )));
            assert!(
                plan.objects
                    .iter()
                    .any(|o| o.kind == GitObjectKind::Tree && o.body.starts_with(b"100755 text\0"))
            );
    ...
    fn conflicts_never_return_partial_candidate_objects() {
        for (base, ours, theirs, kind) in [
            ...
            let b = s.file(b"file", base, 0o100644);
            let o = s.file(b"file", ours, 0o100644);
            let t = s.file(b"file", theirs, 0o100644);
            ...
            .unwrap();
            assert!(
                matches!(result, MergePreparation::Conflicted { conflicts, .. } if conflicts.len() == 1 && conflicts[0].kind == kind)
            );
        }
    }
    ...
    fn deletion_and_opaque_changes_do_not_get_silently_resolved() {
        ...
        let b = s.file(b"file", b"base", 0o120000);
        let t = s.file(b"file", b"new target", 0o120000);
        ...
        .unwrap();
        assert!(
            matches!(result, MergePreparation::Conflicted { conflicts, .. } if conflicts[0].kind == ConflictKind::ModifyDelete)
        );
    }
    ...
    fn attributes_require_an_explicit_driver_instead_of_default_text_resolution() {
        ...
        .unwrap();
        assert!(
            matches!(result, MergePreparation::Conflicted { conflicts, .. } if conflicts[0].kind == ConflictKind::AttributesRequireDriver)
        );
    }
    ...
    fn criss_cross_and_no_common_base_are_refused_without_picking_an_arbitrary_base() {
        let mut s = Source::new(GitHashAlgorithm::Sha1);
        let tree = s.store_tree(vec![]);
        let root = s.store_commit(tree, &[], "root");
        let a = s.store_commit(tree, &[root], "a");
        let b = s.store_commit(tree, &[root], "b");
        let left = s.store_commit(tree, &[a, b], "left");
        let right = s.store_commit(tree, &[b, a], "right");
        assert!(matches!(
            prepare_merge(
                &s,
                s.format,
            ...
            Err(PreparationError::MultipleMergeBases(_))
        ));
        ...
        assert!(matches!(
            ...
            Err(PreparationError::NoCommonAncestor)
        ));
        assert_eq!(
            prepare_merge(
                &s,
            ...
            .unwrap(),
            MergePreparation::AlreadyUpToDate { target: left }
        );
    }
    ...
    fn cancellation_limits_and_metadata_injection_refuse() {
        let mut s = Source::new(GitHashAlgorithm::Sha1);
        let tree = s.store_tree(vec![]);
        let base = s.store_commit(tree, &[], "base");
        let tip = s.store_commit(tree, &[base], "tip");
        let mut bad = metadata();
        bad.author = "T <t@x>\nparent forged".into();
        assert!(matches!(
            prepare_merge(&s, s.format, base, tip, &bad, PreparationLimits::default()),
            Err(PreparationError::InvalidMetadata)
        ));
        ...
        assert!(matches!(
            prepare_merge(&s, s.format, base, tip, &metadata(), limits),
            Err(PreparationError::Graph(
                MergeBaseError::CommitLimitExceeded { .. }
            ))
        ));
        s.live.set(false);
        assert!(matches!(
            ...
            Err(PreparationError::Source(MergeSourceError::Cancelled))
        ));
    }
    ...
    fn one_sided_deletion_and_disjoint_addition_merge_without_resurrecting_the_file() {
        ...
        let (ours, theirs) =
            branch_pair(&mut s, vec![deleted.clone()], vec![], vec![added, deleted]);
        let result = prepare_merge(
            &s,
            s.format,
            ours,
    ...
    fn fast_forward_capable_input_still_produces_an_explicit_two_parent_candidate() {
        ...
        assert!(
            String::from_utf8_lossy(&plan.objects[0].body).starts_with(&format!(
                "tree {new_tree}\nparent {target}\nparent {incoming}\n"
            ))
        );
    }
