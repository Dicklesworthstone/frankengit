...
use super::{
    BTreeMap, GitHashAlgorithm, GitObjectKind, GitOid, Graph, MergeBaseLimits, MergeBaseResult,
    MergeConflict, MergeEntry, MergeMetadata, MergeObjectSource, MergeSourceError, Planner,
    PreparationError, PreparationLimits, PreparedMerge, finish_merge, git_object_id,
    merge_bases_all,
};
...
impl ResolutionInputs {
    pub fn validate(self, format: GitHashAlgorithm) -> Result<(), ResolutionError> {
        if self.target == self.source
            || [self.base, self.target, self.source]
                .iter()
...
pub fn validate_resolutions(
    ...
    limits.validate()?;
    if resolutions.len() > limits.max_conflicts {
        return Err(ResolutionError::Budget);
    }
    ...
    for (index, resolution) in resolutions.iter().enumerate() {
        ...
        bytes = bytes
            ...
            .ok_or(ResolutionError::Budget)?;
        if let ResolutionChoice::File {
            mode,
            bytes: content,
        } = &resolution.choice
        {
    ...
    let mut paths: Vec<_> = resolutions
        .iter()
        .map(|resolution| resolution.path.as_slice())
        .collect();
    paths.sort_unstable();
    ...
    for pair in paths.windows(2) {
        if pair[0] == pair[1] {
            return Err(ResolutionError::DuplicatePath(pair[0].to_vec()));
        }
    }
    for path in paths {
...
pub fn prepare_resolved_merge<S: MergeObjectSource>(
    ...
    let base = match bases {
        ...
        MergeBaseResult::Bases(bases) => {
            return Err(PreparationError::MultipleMergeBases(bases).into());
        }
    };
    if base != inputs.base {
        return Err(ResolutionError::BaseMismatch {
            expected: inputs.base,
            actual: base,
        });
    }
    if base == inputs.source {
        return Err(ResolutionError::NoConflicts);
    }
...
pub(super) fn resolve_discovered_conflicts<S: MergeObjectSource>(
    ...
    let conflicts = std::mem::take(&mut planner.conflicts)
        .into_iter()
    ...
    for path in choices.keys() {
        if !conflicts.contains_key(*path) {
            return Err(ResolutionError::NonConflictPath(path.to_vec()));
        }
    }
    let unresolved: Vec<_> = conflicts
        .values()
        .filter(|c| !choices.contains_key(c.path.as_slice()))
        .cloned()
        .collect();
    if !unresolved.is_empty() {
        return Err(ResolutionError::Unresolved(unresolved));
    }
    ...
    for (path, conflict) in conflicts {
        ...
        let choice = choices
            .get(path.as_slice())
            .ok_or(ResolutionError::ReconstructionMismatch)?;
        let result = match choice {
            ...
            ResolutionChoice::File { mode, bytes } => {
                ...
                let reusable = [&conflict.base, &conflict.ours, &conflict.theirs]
                    .iter()
                ...
                if !reusable {
                    planner.emit(GitObjectKind::Blob, bytes.clone())?;
                }
                Some(MergeEntry {
                    name: path
                        .rsplit(|byte| *byte == b'/')
