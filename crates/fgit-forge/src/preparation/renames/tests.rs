...
use super::*;
use super::super::{
    CommitInput, GitHashAlgorithm, MergeMetadata, MergePreparation, MergeProfile,
    MergeSourceError, PreparationLimits, PreparedMerge, prepare_merge, prepare_merge_with_profile,
};
use std::cell::Cell;
...
impl Source {
    ...
    fn tree(&mut self, files: &[File<'_>]) -> GitOid {
        ...
        for &(path, body, mode) in files {
            if let Some(slash) = path.iter().position(|b| *b == b'/') {
                directories.entry(path[..slash].to_vec()).or_default().push((&path[slash + 1..], body, mode));
            } else {
                let oid = git_object_id(self.format, GitObjectKind::Blob, body);
                self.blobs.insert(oid, body.to_vec());
                leaves.push(MergeEntry { name: path.to_vec(), mode, oid });
            }
        }
    ...
    fn directory(&mut self, mut entries: Vec<MergeEntry>) -> GitOid {
        entries.sort_by_key(|entry| {
            ...
            if entry.mode == 0o040000 { name.push(b'/'); }
            name
        });
    ...
    fn commit(&mut self, tree: GitOid, parents: &[GitOid], label: &str) -> GitOid {
        ...
        self.commits.insert(oid, CommitInput { tree, parents: parents.to_vec() });
        oid
    }
...
impl MergeObjectSource for Source {
    fn checkpoint(&self) -> Result<(), MergeSourceError> {
        ...
        self.calls.set(n + 1);
        if n >= self.stop.get() { Err(MergeSourceError::Cancelled) } else { Ok(()) }
    }
...
fn metadata() -> MergeMetadata {
    MergeMetadata { author: "T <t@x>".into(), committer: "T <t@x>".into(), timestamp: 2, message: b"merged\n".to_vec() }
}
fn run(s: &Source, o: GitOid, t: GitOid) -> Result<MergePreparation, PreparationError> {
    prepare_merge_with_profile(s, s.format, o, t, &metadata(), PreparationLimits::default(), MergeProfile::ExactRenamesV1)
}
fn clean(result: Result<MergePreparation, PreparationError>) -> PreparedMerge {
    let MergePreparation::Clean(plan) = result.unwrap() else { panic!("expected clean candidate"); };
    plan
}
...
fn cross_directory_rename_transports_edits_and_mode_without_rewriting_parents() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        for swap in [false, true] {
            ...
            assert_eq!(run(&s, o, t).unwrap(), MergePreparation::Clean(plan.clone()));
            let commit = plan.objects.iter().find(|x| x.id == plan.commit).unwrap();
            assert!(commit.body.starts_with(format!("tree {expected}\nparent {o}\nparent {t}\n").as_bytes()));
            for object in &plan.objects {
                assert_eq!(object.id, git_object_id(format, object.kind, &object.body));
                assert_ne!(object.kind, GitObjectKind::Blob, "rename+edit borrows the original edited blob");
            }
            ...
            while let Some(id) = pending.pop() {
                ...
                for entry in &s.trees[&id] {
                    if entry.mode == 0o040000 { pending.push(entry.oid); }
                }
            }
            assert!(plan.objects.iter().all(|x| x.kind == GitObjectKind::Commit || reachable.contains(&x.id)));
            assert!(matches!(prepare_merge(&s, format, o, t, &metadata(), PreparationLimits::default()).unwrap(), MergePreparation::Conflicted { .. }));
        }
    }
}
...
fn rename_delete_and_divergent_rename_never_choose_a_survivor() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let mut s = Source::new(format);
        let (o, t) = s.pair(&[(b"old", b"base", 0o100644)], &[(b"new", b"base", 0o100644)], &[]);
        assert!(matches!(run(&s, o, t), Err(PreparationError::Rename(RenameRefusal::RenameDelete { from, to })) if from == b"old" && to == b"new"));
        let (o, t) = s.pair(&[(b"old", b"base", 0o100644)], &[(b"new", b"base", 0o100644)], &[(b"different", b"base", 0o100644)]);
        assert!(matches!(run(&s, o, t), Err(PreparationError::Rename(RenameRefusal::Divergent { from, target, source })) if from == b"old" && target == b"new" && source == b"different"));
    }
}
...
fn duplicate_deleted_or_added_identities_refuse_instead_of_using_path_order() {
    ...
    for (base, ours, theirs) in cases {
        ...
        assert!(matches!(run(&s, o, t), Err(PreparationError::Rename(RenameRefusal::AmbiguousIdentity { side: RenameSide::Target, .. }))));
    }
}
...
fn destinations_and_ancestor_file_collisions_do_not_overwrite_other_work() {
    for (destination, theirs) in [
        (b"new".as_slice(), vec![(b"old".as_slice(), b"edited".as_slice(), 0o100644), (b"new".as_slice(), b"other".as_slice(), 0o100644)]),
        (b"dir/file".as_slice(), vec![(b"old".as_slice(), b"edited".as_slice(), 0o100644), (b"dir".as_slice(), b"other".as_slice(), 0o100644)]),
    ] {
        let mut s = Source::new(GitHashAlgorithm::Sha1);
        let (o, t) = s.pair(&[(b"old", b"base", 0o100644)], &[(destination, b"base", 0o100644)], &theirs);
        assert!(matches!(run(&s, o, t), Err(PreparationError::Rename(RenameRefusal::DestinationOccupied { .. }))));
    }
    let mut s = Source::new(GitHashAlgorithm::Sha1);
    let (o, t) = s.pair(&[(b"a", b"A", 0o100644), (b"b", b"B", 0o100644)],
...
fn type_changes_and_attribute_context_changes_are_explicit_refusals() {
    let mut s = Source::new(GitHashAlgorithm::Sha1);
    let (o, t) = s.pair(&[(b"old", b"base", 0o100644)], &[(b"new", b"base", 0o100644)], &[(b"old", b"link", 0o120000)]);
    assert!(matches!(run(&s, o, t), Err(PreparationError::Rename(RenameRefusal::UnsupportedEntry { path })) if path == b"old"));
    for attr in [b".gitattributes".as_slice(), b"dir/.gitattributes".as_slice()] {
        let (o, t) = s.pair(&[(b"old", b"base", 0o100644)],
            &[(b"dir/new", b"base", 0o100644), (attr, b"* merge=custom", 0o100644)],
            &[(b"old", b"edited", 0o100644)]);
        assert!(matches!(run(&s, o, t), Err(PreparationError::Rename(RenameRefusal::AttributesRequireDriver { .. }))));
    }
}
...
fn every_cancellation_checkpoint_and_each_narrowed_resource_ceiling_refuses() {
    let mut s = Source::new(GitHashAlgorithm::Sha1);
    let (o, t) = s.pair(&[(b"old/a", b"base", 0o100644)], &[(b"new/a", b"base", 0o100644)], &[(b"old/a", b"edited", 0o100644)]);
    clean(run(&s, o, t));
    ...
    for limits in [
        PreparationLimits { max_tree_entries: 1, ..PreparationLimits::default() },
        PreparationLimits { max_path_bytes: 3, ..PreparationLimits::default() },
        PreparationLimits { max_objects: 1, ..PreparationLimits::default() },
        PreparationLimits { max_output_bytes: 16, ..PreparationLimits::default() },
    ] {
        assert!(matches!(prepare_merge_with_profile(&s, s.format, o, t, &metadata(), limits, MergeProfile::ExactRenamesV1), Err(PreparationError::Budget(_))));
    }
}
...
fn explicit_empty_directories_survive_beside_emptied_rename_parents() {
    ...
    let keep = MergeEntry { name: b"keep".to_vec(), mode: 0o040000, oid: empty };
    let mut tips = Vec::new();
    for (files, label) in [
        (vec![(b"old/a".as_slice(), b"base".as_slice(), 0o100644)], "base"),
...
fn rename_count_is_a_hard_ceiling_even_for_tiny_files() {
    ...
    let mut b = Flat::new();
    let mut o = Flat::new();
    let mut budget = PathBudget { used: 0, maximum: MAX_PATH_STORAGE_BYTES };
    for index in 0..=MAX_RENAMES {
        ...
        b.insert(name.clone(), MergeEntry { name, mode: 0o100644, oid });
        let name = format!("new-{index}").into_bytes();
        o.insert(name.clone(), MergeEntry { name, mode: 0o100644, oid });
        if index + 1 == MAX_RENAMES {
            assert_eq!(detect(&planner, &b, &o, RenameSide::Target, &mut budget).unwrap().len(), MAX_RENAMES);
        }
    }
    assert!(matches!(detect(&planner, &b, &o, RenameSide::Target, &mut budget), Err(PreparationError::Budget("renames"))));
}
