...
impl Source {
    ...
    fn fork(
        ...
        base: Vec<MergeEntry>,
        ours: Vec<MergeEntry>,
        theirs: Vec<MergeEntry>,
    ) -> ResolutionInputs {
        let tree = self.store_tree(base);
        let base = self.store_commit(tree, &[], "base");
        let tree = self.store_tree(ours);
        let target = self.store_commit(tree, &[base], "ours");
        let tree = self.store_tree(theirs);
        let source = self.store_commit(tree, &[base], "theirs");
        ResolutionInputs {
            base,
            target,
            source,
...
impl MergeObjectSource for Source {
    fn checkpoint(&self) -> Result<(), MergeSourceError> {
        let remaining = self
            ...
            .get()
            .checked_sub(1)
            .ok_or(MergeSourceError::Cancelled)?;
    ...
    fn commit(&self, oid: GitOid) -> Result<CommitInput, MergeSourceError> {
        self.commits
            .get(&oid)
            .cloned()
            .ok_or(MergeSourceError::Unavailable(oid))
    }
...
fn resolved_nested_content_preserves_clean_merges_modes_raw_bytes_and_siblings() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let bt = s.store_tree(vec![ab, b]);
        let ot = s.store_tree(vec![ao, o]);
        let tt = s.store_tree(vec![at, t]);
        let inputs = s.fork(
        ...
        assert_eq!(
            result.resolutions[0].result.as_ref().unwrap().oid,
            resolved_id
        );
        assert!(
            !result
        ...
        assert_eq!(
            result
                ...
                .len(),
            result.plan.objects.len()
        );
        for object in &result.plan.objects {
        ...
        let commit = result
            ...
            .objects
            .iter()
            .find(|o| o.id == result.plan.commit)
            .unwrap();
        assert!(
            commit.body.starts_with(
                format!(
                    "tree {expected_tree}\nparent {}\nparent {}\n",
        ...
        assert!(
            ...
            "explicit choices must not change automatic behavior"
        );
    }
}
...
fn missing_side_does_not_mean_delete_and_unresolved_or_extra_choices_fail() {
    ...
    assert!(
        matches!(run(&s, inputs, &[]), Err(ResolutionError::Unresolved(conflicts)) if conflicts.len() == 1)
    );
    assert!(matches!(
        run(&s, inputs, &[resolution(b"file", ResolutionChoice::Ours)]),
        Err(ResolutionError::MissingSide { .. })
    ));
    assert!(matches!(
        run(
            &s,
            inputs,
            &[resolution(b"unrelated", ResolutionChoice::Delete)]
        ),
    ...
    let stale = ResolutionInputs {
        base: inputs.target,
        ..inputs
    };
    assert!(matches!(
        run(&s, stale, &[resolution(b"file", ResolutionChoice::Theirs)]),
        Err(ResolutionError::BaseMismatch { .. })
    ));
}
...
fn choices_cannot_bypass_path_limits_missing_sources_or_cancellation() {
    ...
    for path in [
        ...
        assert!(
            validate_resolutions(
                &[resolution(path, ResolutionChoice::Delete)],
                PreparationLimits::default()
            )
            .is_err()
        );
    ...
    assert!(matches!(
        validate_resolutions(
            &[
                resolution(b"a", ResolutionChoice::Delete),
                resolution(b"a-", ResolutionChoice::Delete),
                resolution(b"a/b", ResolutionChoice::Delete)
            ],
            PreparationLimits::default()
        ),
        Err(ResolutionError::OverlappingPaths)
    ));
    let choices = [resolution(b"file", ResolutionChoice::Theirs)];
    assert!(matches!(
        run(&s, inputs, &[choices[0].clone(), choices[0].clone()]),
        Err(ResolutionError::DuplicatePath(_))
    ));
    s.remaining.set(0);
    assert!(matches!(
        run(&s, inputs, &choices),
        Err(ResolutionError::Preparation(PreparationError::Source(
...
fn discovery_and_reconstruction_share_one_work_budget() {
    ...
    assert!(matches!(
        prepare_resolved_merge(&s, s.format, inputs, &choices, &metadata(), limits),
        Err(ResolutionError::Preparation(PreparationError::Budget(
            "content merges"
        )))
    ));
...
fn attribute_conflicts_use_explicit_bytes_and_clean_changes_cannot_be_overridden() {
    ...
    assert!(matches!(
        run(
            ...
            &[
                ...
                resolution(b".gitattributes", ResolutionChoice::Delete)
            ]
        ),
        Err(ResolutionError::NonConflictPath(_))
    ));
    ...
    assert!(matches!(
        ...
        Err(ResolutionError::NoConflicts)
    ));
}
