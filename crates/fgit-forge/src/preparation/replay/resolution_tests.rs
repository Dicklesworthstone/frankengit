...
impl Source {
    fn new(format: GitHashAlgorithm) -> Self {
        Self {
            ...
            stop_after_trees: Cell::new(None),
        }
    }
...
impl MergeObjectSource for Source {
    fn checkpoint(&self) -> Result<(), MergeSourceError> {
        if self
            .stop_after_trees
            .get()
            .is_some_and(|limit| self.tree_reads.get() >= limit)
        {
...
fn pair(
    ...
    let bt = s.tree_id(b);
    let base = s.commit_id(bt, &[], "base");
    let ot = s.tree_id(o);
    let target = s.commit_id(ot, &[base], "target");
    let tt = s.tree_id(t);
    let selected = s.commit_id(tt, &[base], "selected");
    ReplayRequest {
        direction: ReplayDirection::CherryPick,
        target,
        source_tip: selected,
...
fn replay_resolution_keeps_clean_changes_and_exact_single_parent_bytes_in_both_formats() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let dir_b = s.tree_id(vec![b]);
        let dir_o = s.tree_id(vec![o]);
        let dir_t = s.tree_id(vec![t]);
        ...
        let input = pair(
            &mut s,
            vec![dir(dir_b), keep.clone(), gone.clone()],
            vec![dir(dir_o), keep.clone(), gone, ours.clone()],
            vec![dir(dir_t), keep.clone(), theirs.clone()],
        );
        let bytes = b"manual\0\xff\r\nno final newline";
        let file = s.file(b"file", bytes, 0o100755);
        let chosen = s.tree_id(vec![file]);
        ...
        let ReplayPreparation::Clean(plan) = result.outcome else {
        ...
        let body = [
            ...
            .as_bytes(),
            metadata().message.as_slice(),
        ]
        .concat();
        assert_eq!(
            plan.commit,
            git_object_id(format, GitObjectKind::Commit, &body)
        );
        assert_eq!(
            plan.objects
                .iter()
                .find(|o| o.id == plan.commit)
                .unwrap()
                .body,
            body
        );
        assert!(
            plan.objects
                .iter()
                .any(|o| o.kind == GitObjectKind::Blob && o.body == bytes)
        );
    }
}
...
fn ours_can_resolve_to_no_change_while_empty_file_and_deletion_remain_distinct() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert!(
            matches!(file.outcome, ReplayPreparation::Clean(ref plan) if plan.tree == empty_file_tree)
        );
        assert!(
            matches!(deleted.outcome, ReplayPreparation::Clean(ref plan) if plan.tree == empty_tree)
        );
...
fn revert_uses_the_selected_parent_as_theirs_and_requires_explicit_missing_side_deletion() {
    ...
    assert_eq!(
        result.resolutions[0].conflict.base.as_ref().unwrap().oid,
        s.trees[&applied_tree][0].oid
    );
...
fn all_and_only_conflicts_are_required_and_paths_are_validated_before_reads() {
    ...
    assert!(
        matches!(prepare_resolved_replay(&s, s.format, input, &[], &metadata(), PreparationLimits::default()),
        Err(ReplayError::Resolution(error)) if matches!(*error, ResolutionError::Unresolved(_)))
    );
    assert!(
        matches!(prepare_resolved_replay(&s, s.format, input, &[choice(b"clean", ResolutionChoice::Ours)],
        &metadata(), PreparationLimits::default()), Err(ReplayError::Resolution(error))
        if matches!(*error, ResolutionError::NonConflictPath(_)))
    );
    for paths in [
        vec![b"file".as_slice(), b"file"],
        vec![b"a", b"a-", b"a/b"],
        vec![b".git/config"],
        vec![b"../file"],
    ] {
        s.reads.borrow_mut().clear();
        let choices: Vec<_> = paths
            .iter()
            .map(|p| choice(p, ResolutionChoice::Delete))
            .collect();
        assert!(
            prepare_resolved_replay(
                &s,
    ...
    let clean = ReplayRequest {
        target: s.commits[&input.selected_commit].parents[0],
        ..input
    };
    assert!(
        matches!(prepare_resolved_replay(&s, s.format, clean, &[choice(b"file", ResolutionChoice::Theirs)],
...
fn discovery_and_reconstruction_share_the_tree_budget_and_cancellation() {
    ...
    assert!(matches!(
        prepare_replay(&s, s.format, input, &metadata(), limits).unwrap(),
        ReplayPreparation::Conflicted { .. }
    ));
    assert!(
        matches!(prepare_resolved_replay(&s, s.format, input, &[choice(b"file", ResolutionChoice::Theirs)], &metadata(), limits),
        Err(ReplayError::Resolution(error)) if matches!(*error, ResolutionError::Preparation(PreparationError::Budget("tree entries"))))
    );
    s.tree_reads.set(0);
    s.stop_after_trees.set(Some(4));
    assert!(
        matches!(prepare_resolved_replay(&s, s.format, input, &[choice(b"file", ResolutionChoice::Theirs)],
        &metadata(), PreparationLimits::default()), Err(ReplayError::Resolution(error))
...
fn missing_base_in_root_replay_is_not_an_implicit_delete_and_mainline_is_preserved() {
    ...
    let source_tree = s.tree_id(vec![theirs]);
    let source = s.commit_id(source_tree, &[], "source root");
    let target_tree = s.tree_id(vec![ours]);
    let target = s.commit_id(target_tree, &[], "target root");
    let input = ReplayRequest {
        direction: ReplayDirection::CherryPick,
        target,
        source_tip: source,
    ...
    assert!(
        matches!(prepare_resolved_replay(&s, s.format, input, &[choice(b"file", ResolutionChoice::Base)],
        &metadata(), PreparationLimits::default()), Err(ReplayError::Resolution(error))
        if matches!(*error, ResolutionError::MissingSide { side: ResolutionKind::Base, .. }))
    );
    let result = resolved(&s, input, &[choice(b"file", ResolutionChoice::Theirs)]);
    assert!(
        matches!(result.outcome, ReplayPreparation::Clean(ref p) if p.tree == source_tree && p.coordinates.selected_parent.is_none())
    );
    let merge = s.commit_id(source_tree, &[target, source], "explicit merge");
    let selected = ReplayRequest {
        source_tip: merge,
        selected_commit: merge,
        mainline: None,
        ..input
    };
    assert!(matches!(
        prepare_resolved_replay(
            ...
            s.format,
            selected,
            &[choice(b"file", ResolutionChoice::Ours)],
            &metadata(),
            PreparationLimits::default()
        ),
...
fn resolving_a_directory_conflict_reuses_the_selected_subtree_without_flattening_it() {
    ...
    let old = s.file(b"node", b"old", 0o100644);
    let ours = s.file(b"node", b"target change", 0o100644);
    let leaf = s.file(b"leaf", b"source subtree", 0o100755);
    let child = s.tree_id(vec![leaf]);
    let theirs = MergeEntry {
        name: b"node".to_vec(),
        mode: 0o040000,
        oid: child,
    };
    ...
    assert_eq!(
        result.resolutions[0].conflict.kind,
        ConflictKind::TypeChange
    );
    assert_eq!(result.resolutions[0].result, Some(theirs));
    assert!(
        matches!(result.outcome, ReplayPreparation::Clean(ref p) if p.tree == s.commits[&input.selected_commit].tree)
    );
}
...
fn explicit_choices_cannot_authorize_a_commit_outside_the_selected_source_history() {
    let (s, input) = simple(GitHashAlgorithm::Sha1);
    s.reads.borrow_mut().clear();
    let outside = ReplayRequest {
        selected_commit: input.target,
        ..input
    };
    assert!(matches!(
        prepare_resolved_replay(
