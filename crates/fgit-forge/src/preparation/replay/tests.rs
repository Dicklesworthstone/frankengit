...
impl Source {
    fn new(format: GitHashAlgorithm) -> Self {
        Self {
            ...
            trees: BTreeMap::new(),
            blobs: BTreeMap::new(),
            read_commits: RefCell::new(Vec::new()),
            live: Cell::new(true),
        }
    }
...
fn historical_pick_preserves_target_changes_without_replaying_later_source_commits() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let commit = plan
            .objects
            .iter()
            .find(|object| object.id == plan.commit)
            .unwrap();
        ...
        assert_eq!(
            plan.commit,
            git_object_id(format, GitObjectKind::Commit, &expected)
        );
        assert!(
            !plan
                .objects
                .iter()
                .any(|object| object.id == picked || object.id == later || object.id == keep.oid)
        );
        assert!(
            plan.objects
                .iter()
                .any(|object| object.id == expected_file.oid)
        );
...
fn revert_uses_inverse_changes_and_preserves_unrelated_target_additions() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let plan = clean(
            &s,
            request(target, target, selected, ReplayDirection::Revert),
        );
...
fn root_commits_use_the_real_empty_tree_in_both_directions() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            clean(&s, request(later, later, root, ReplayDirection::Revert)).tree,
            other_tree
        );
        ...
        assert!(
            reverted
                .objects
                .iter()
                .any(|o| o.id == empty && o.kind == GitObjectKind::Tree && o.body.is_empty())
        );
        let mut bad = request(root, root, root, ReplayDirection::Revert);
        bad.mainline = Some(1);
        assert!(matches!(
            prepare_replay(&s, format, bad, &metadata(), PreparationLimits::default()),
...
fn merge_mainline_is_explicit_and_uses_stored_parent_order() {
    ...
    let a = s.file(b"a", b"A", 0o100644);
    let b = s.file(b"b", b"B", 0o100644);
    let at = s.store_tree(vec![a.clone()]);
    let bt = s.store_tree(vec![b.clone()]);
    let both = s.store_tree(vec![a, b]);
    let first = s.store_commit(at, &[], "first");
    let second = s.store_commit(bt, &[], "second");
    ...
    for mainline in [0, 3, u16::MAX] {
        assert!(matches!(
            ...
            Err(ReplayError::InvalidMainline { .. })
        ));
    }
}
...
fn content_binary_and_modify_delete_conflicts_return_no_candidate() {
    for (base, ours, applied, kind) in [
        ...
        let file = s.file(b"conflict", base, 0o100644);
        let tree = s.store_tree(vec![file]);
        let root = s.store_commit(tree, &[], "base");
        let target_entries = ours
            .map(|bytes| s.file(b"conflict", bytes, 0o100644))
            .into_iter()
            .collect();
        let target_tree = s.store_tree(target_entries);
        let target = s.store_commit(target_tree, &[root], "ours");
        let theirs = s.file(b"conflict", applied, 0o100644);
        let their_tree = s.store_tree(vec![theirs]);
        let selected = s.store_commit(their_tree, &[root], "selected");
        let result = prepare_replay(
            &s,
            s.format,
            request(target, selected, selected, ReplayDirection::CherryPick),
            &metadata(),
            PreparationLimits::default(),
        )
        .unwrap();
        assert!(
            matches!(result, ReplayPreparation::Conflicted { conflicts, .. }
...
fn no_net_change_does_not_create_an_empty_commit_or_claim_history_equivalence() {
    let mut s = Source::new(GitHashAlgorithm::Sha1);
    let file = s.file(b"file", b"content", 0o100644);
    let tree = s.store_tree(vec![file]);
    let root = s.store_commit(tree, &[], "root");
    let empty = s.store_commit(tree, &[root], "empty change");
    for direction in [ReplayDirection::CherryPick, ReplayDirection::Revert] {
        assert!(matches!(
            prepare_replay(
                &s,
                s.format,
                request(root, empty, empty, direction),
                &metadata(),
                PreparationLimits::default()
            )
            .unwrap(),
    ...
    assert!(matches!(
        prepare_replay(
            &s,
            s.format,
            request(root, root, root, ReplayDirection::CherryPick),
            &metadata(),
            PreparationLimits::default()
        )
        .unwrap(),
...
fn unrelated_commit_is_not_read_before_selected_history_establishes_membership() {
    ...
    let visible = s.store_commit(tree, &[], "visible");
    let other = s.store_commit(tree, &[], "not reachable from source");
    assert!(matches!(
        prepare_replay(
            &s,
            s.format,
            request(other, visible, other, ReplayDirection::CherryPick),
            &metadata(),
            PreparationLimits::default()
        ),
...
fn graph_work_metadata_and_cancellation_are_bounded_before_a_candidate_exists() {
    ...
    let tree = s.store_tree(vec![file]);
    let root = s.store_commit(tree, &[], "root");
    let later = s.store_commit(tree, &[root], "later");
    ...
    bad.committer.push_str("\nparent injected");
    assert!(matches!(
        prepare_replay(&s, s.format, input, &bad, PreparationLimits::default()),
    ...
    assert!(matches!(
        ...
        Err(ReplayError::Preparation(PreparationError::Source(
    ...
    assert!(matches!(
        ...
        Err(ReplayError::Preparation(PreparationError::Source(
            MergeSourceError::InvalidObject(_)
        )))
    ));
}
