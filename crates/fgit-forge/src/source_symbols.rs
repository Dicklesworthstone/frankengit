...
pub use engine::{
    Error as SymbolSyntaxError, ErrorKind as SymbolSyntaxErrorKind, Kind as SymbolKind,
    MAX_WORK as MAX_SYMBOL_WORK,
};
...
impl SymbolQuery {
    pub fn new(
        ...
        Ok(Self {
            ...
            mode,
            kinds,
            scope: SourceQuery::new(b"\0", SearchCase::Exact, prefixes)?,
            maximum_work,
        })
    }
    #[must_use]
    ...
    #[must_use]
    pub const fn mode(&self) -> SymbolMatchMode {
        self.mode
    }
    #[must_use]
    ...
    fn accepts(&self, name: &[u8], kind: SymbolKind) -> bool {
        (self.kinds.is_empty() || self.kinds.contains(&kind))
            && match self.mode {
                SymbolMatchMode::Exact => name == self.name,
                SymbolMatchMode::Prefix => name.starts_with(&self.name),
            }
    }
}
...
impl<E> SymbolReadError<E> {
    pub fn map_source<F>(self, map: impl FnOnce(E) -> F) -> SymbolReadError<F> {
        match self {
            Self::Source(e) => SymbolReadError::Source(map(e)),
            Self::Syntax { path, error } => SymbolReadError::Syntax { path, error },
        }
    }
}
impl<E> From<E> for SymbolReadError<E> {
    fn from(e: E) -> Self {
        Self::Source(e)
    }
}
impl<E: std::fmt::Display> std::fmt::Display for SymbolReadError<E> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Source(e) => std::fmt::Display::fmt(e, f),
...
pub fn search_source_symbols<A: GitHashAlgorithm, S: ObjectSource<A>>(
    ...
    report.unsupported_language_files = discovery
        .files
        .keys()
        .filter(|p| !p.as_bytes().ends_with(b".rs"))
        .count();
    let mut files: Vec<_> = discovery.files.into_iter().collect();
    files.sort_by(|a, b| a.0.as_bytes().cmp(b.0.as_bytes()));
    let mut budget =
        engine::Budget::new(query.maximum_work).map_err(|_| SearchError::InvalidQuery)?;
    ...
    'files: for (path, blob) in files {
        ...
        if body.len() > limits.max_file_bytes {
            return Err(SearchError::Budget("symbol file bytes").into());
        }
        report.bytes_read = report
            .bytes_read
            .checked_add(body.len())
            .filter(|n| *n <= limits.max_total_bytes)
            .ok_or(SearchError::Budget("symbol total bytes"))?;
        report.files_read += 1;
        let blob = oid::<A>(&blob)?;
        let scanned = engine::extract(&body, &mut budget, cancelled).map_err(|error| {
            SymbolReadError::Syntax {
                path: path.as_bytes().to_vec(),
                error,
            }
        })?;
        report.bytes_searched += body.len();
        report.macro_bodies_skipped += scanned.macro_bodies_skipped;
        ...
        for declaration in scanned.declarations {
            ...
            if !query.accepts(name, declaration.kind) {
                continue;
            }
            ...
            let excerpt_end = body[end..upper]
                .iter()
                .position(|b| *b == b'\n')
                .map_or(upper, |n| end + n);
            result_bytes = result_bytes
                .checked_add(path.as_bytes().len() + name.len() + excerpt_end - excerpt_offset)
                .filter(|n| *n <= MAX_RESULT_BYTES)
                .ok_or(SearchError::Budget("symbol result bytes"))?;
            report.matches.push(SymbolMatch {
                name: name.to_vec(),
                kind: declaration.kind,
                raw_identifier: declaration.raw_identifier,
                location: SourceMatch {
                    path: path.as_bytes().to_vec(),
                    blob,
                    byte_offset: start,
                    line: declaration.line,
                    byte_column: declaration.byte_column,
                    match_length: declaration.byte_length,
                    excerpt_offset,
    ...
    checkpoint(cancelled)?;
    report.work_units = budget.work;
    report.declarations_examined = budget.declarations;
    Ok(report)
}
...
mod tests {
    ...
    fn query_profiles_are_closed_bounded_case_sensitive_and_byte_preserving() {
        ...
        .unwrap();
        assert_eq!(query.kinds(), &[SymbolKind::Struct]);
        assert!(query.accepts(b"ThingMore", SymbolKind::Struct));
        assert!(!query.accepts(b"thing", SymbolKind::Struct));
        assert!(!query.accepts(b"Thing", SymbolKind::Function));
        for name in [
            b"".as_slice(),
            b"r#thing",
            b"1thing",
            ...
            assert!(
                SymbolQuery::new(name, SymbolMatchMode::Exact, &[], &[], MAX_SYMBOL_WORK).is_err()
            );
        }
        for work in [0, MAX_SYMBOL_WORK + 1] {
            assert!(SymbolQuery::new(b"x", SymbolMatchMode::Exact, &[], &[], work).is_err());
        }
        assert!(
            SymbolQuery::new(
                b"x",
