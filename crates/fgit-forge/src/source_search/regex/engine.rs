...
impl std::fmt::Display for RegexError {
    fn fmt(&self, out: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(out, "byte regex refused at {}: {:?}", self.byte_offset, self.kind)
    }
}
...
impl Bytes {
    ...
    fn union(&mut self, other: Self) {
        for (held, offered) in self.0.iter_mut().zip(other.0) { *held |= offered; }
    }
    fn invert(&mut self) { for word in &mut self.0 { *word = !*word; } }
    ...
    fn single(byte: u8) -> Self { let mut set = Self::empty(); set.insert(byte); set }
}
...
struct Parser<'a> { bytes: &'a [u8], at: usize, insensitive: bool }
impl Parser<'_> {
    fn error(&self, kind: RegexErrorKind) -> RegexError {
        RegexError { byte_offset: self.at, kind }
    }
    ...
    fn expression(&mut self, depth: usize) -> Result<Expr, RegexError> {
        if depth > MAX_NESTING { return Err(self.error(RegexErrorKind::NestingLimit)); }
        let mut alternatives = vec![self.sequence(depth)?];
        while self.peek() == Some(b'|') {
            self.at += 1;
            alternatives.push(self.sequence(depth)?);
        }
        if alternatives.len() == 1 { Ok(alternatives.remove(0)) }
        else { Ok(Expr::Alternative(alternatives)) }
    }
    fn sequence(&mut self, depth: usize) -> Result<Expr, RegexError> {
        ...
        while self.peek().is_some_and(|byte| !matches!(byte, b')' | b'|')) {
            ...
            let repeated = match self.peek() {
                Some(b'*') => { self.at += 1; Expr::Repeat(Box::new(atom), 0, None) }
                Some(b'+') => { self.at += 1; Expr::Repeat(Box::new(atom), 1, None) }
                Some(b'?') => { self.at += 1; Expr::Repeat(Box::new(atom), 0, Some(1)) }
                Some(b'{') => {
                    self.at += 1;
                    let minimum = self.decimal()?;
                    let maximum = if self.peek() == Some(b',') {
                        self.at += 1;
                        if self.peek() == Some(b'}') { None } else { Some(self.decimal()?) }
                    } else { Some(minimum) };
            ...
            if self.peek().is_some_and(|byte| matches!(byte, b'*' | b'+' | b'?' | b'{')) {
                return Err(self.error(RegexErrorKind::InvalidRepetition));
            }
            items.push(repeated);
        }
    ...
    fn decimal(&mut self) -> Result<usize, RegexError> {
        ...
        while let Some(byte @ b'0'..=b'9') = self.peek() {
            ...
            if value > MAX_REPEAT { return Err(self.error(RegexErrorKind::InvalidRepetition)); }
        }
        if self.at == begin { Err(self.error(RegexErrorKind::InvalidRepetition)) } else { Ok(value) }
    }
    fn atom(&mut self, depth: usize) -> Result<Expr, RegexError> {
        let byte = self.take().ok_or_else(|| self.error(RegexErrorKind::UnexpectedToken))?;
        match byte {
            b'(' => {
                if self.peek() == Some(b'?') { return Err(self.error(RegexErrorKind::UnsupportedExtension)); }
                let nested = self.expression(depth + 1)?;
                if self.take() != Some(b')') { return Err(self.error(RegexErrorKind::UnclosedGroup)); }
                Ok(nested)
            }
            b'[' => self.class(),
            ...
            b'\\' => {
                ...
                if self.insensitive { bytes.ascii_fold(); }
                Ok(Expr::Byte(bytes))
            }
            b')' | b'*' | b'+' | b'?' | b'{' | b'}' | b']' | b'\n' => {
    ...
    fn escape(&mut self) -> Result<(Bytes, Option<u8>), RegexError> {
        let escaped = self.take().ok_or_else(|| self.error(RegexErrorKind::InvalidEscape))?;
        let byte = match escaped {
            ...
            b'd' | b'D' | b'w' | b'W' | b's' | b'S' => {
                ...
                for byte in 0..=u8::MAX {
                    ...
                    if yes { set.insert(byte); }
                }
                if escaped.is_ascii_uppercase() { set.invert(); }
                return Ok((set, None));
            }
            b'n' => b'\n', b'r' => b'\r', b't' => b'\t',
            b'0' => 0, b'a' => 7, b'b' => 8, b'f' => 12, b'v' => 11,
    ...
    fn class(&mut self) -> Result<Expr, RegexError> {
        ...
        if negated { self.at += 1; }
        let mut bytes = Bytes::empty();
        let mut count = 0usize;
        loop {
            if self.peek() == Some(b']') {
                self.at += 1;
                if count == 0 { return Err(self.error(RegexErrorKind::EmptyClass)); }
                break;
            }
            if self.peek().is_none() { return Err(self.error(RegexErrorKind::UnclosedClass)); }
            if self.bytes.get(self.at..self.at + 2).is_some_and(|v| v == b"&&" || v == b"--") {
            ...
            if self.peek() == Some(b'-') && self.bytes.get(self.at + 1) != Some(&b']') {
                ...
                if first > last { return Err(self.error(RegexErrorKind::InvalidRange)); }
                for byte in first..=last { bytes.insert(byte); }
            } else { bytes.union(item); }
            count += 1;
        }
        // Fold the positive set BEFORE complementing it: [^a] must exclude A.
        if self.insensitive { bytes.ascii_fold(); }
        if negated { bytes.invert(); }
        Ok(Expr::Byte(bytes))
    }
}
...
pub(super) struct Program { states: Vec<State>, start: usize }
impl Program {
    pub(super) fn compile(pattern: &[u8], insensitive: bool) -> Result<Self, RegexError> {
        if pattern.is_empty() || pattern.len() > MAX_PATTERN {
            return Err(RegexError { byte_offset: 0, kind: if pattern.is_empty() {
                RegexErrorKind::EmptyPattern
            } else { RegexErrorKind::PatternLimit } });
        }
        let mut parser = Parser { bytes: pattern, at: 0, insensitive };
    ...
    fn emit(&mut self, state: State) -> Result<usize, RegexError> {
        if self.states.len() == MAX_STATES {
            return Err(RegexError { byte_offset: 0, kind: RegexErrorKind::StateLimit });
        }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(super) enum ScanError { Cancelled, WorkLimit, Allocation }
pub(super) struct Budget { pub used: u64, pub maximum: u64 }
impl Budget {
    fn charge(&mut self, cancelled: &dyn Fn() -> bool) -> Result<(), ScanError> {
        if self.used % 1024 == 0 && cancelled() { return Err(ScanError::Cancelled); }
        if self.used == self.maximum { return Err(ScanError::WorkLimit); }
        self.used += 1;
        Ok(())
    }
}
#[derive(Clone, Copy)]
struct Thread { state: usize, start: usize }
...
fn reserved<T>(capacity: usize) -> Result<Vec<T>, ScanError> {
    ...
    values.try_reserve_exact(capacity).map_err(|_| ScanError::Allocation)?;
    Ok(values)
}
impl<'a> Runner<'a> {
    pub(super) fn new(program: &'a Program) -> Result<Self, ScanError> {
        ...
        seen.resize(n, 0);
        Ok(Self { program, seen, epoch: 0, seeds: reserved(n + 1)?,
            next: reserved(n + 1)?, active: reserved(n)?, stack: reserved(n * 2 + 2)? })
    }
    pub(super) fn find_line(&mut self, bytes: &[u8], budget: &mut Budget,
        cancelled: &dyn Fn() -> bool,
    ) -> Result<Option<(usize, usize)>, ScanError> {
        if cancelled() { return Err(ScanError::Cancelled); }
        self.seeds.clear();
        ...
        for position in 0..=bytes.len() {
            ...
            self.epoch += 1;
            if best.is_none() { self.seeds.push(Thread { state: self.program.start, start: position }); }
            self.active.clear();
            for &seed in &self.seeds {
                if best.is_some_and(|(start, _)| seed.start > start) { continue; }
                self.stack.clear(); self.stack.push(seed);
                while let Some(thread) = self.stack.pop() {
                    budget.charge(cancelled)?;
                    if self.seen[thread.state] == self.epoch { continue; }
                    self.seen[thread.state] = self.epoch;
                    let mut follow = |state| self.stack.push(Thread { state, start: thread.start });
                    match self.program.states[thread.state] {
                        State::Byte(_, _) => self.active.push(thread),
                        State::Split(left, right) => { follow(right); follow(left); }
                        ...
                        State::Boundary(positive, next) => {
                            ...
                            if (left != right) == positive { follow(next); }
                        }
                        State::Accept => {
                            if best.is_none_or(|(start, end)| thread.start < start
                                || (thread.start == start && position > end))
            ...
            if let Some(&byte) = bytes.get(position) {
                for thread in &self.active {
                    budget.charge(cancelled)?;
                    if best.is_some_and(|(start, _)| thread.start > start) { continue; }
                    if let State::Byte(set, next) = self.program.states[thread.state] {
                        if set.contains(byte) { self.next.push(Thread { state: next, start: thread.start }); }
                    }
                }
            }
...
mod tests {
    ...
    fn find(pattern: &[u8], bytes: &[u8], insensitive: bool) -> Option<(usize, usize)> {
        ...
        Runner::new(&program).unwrap().find_line(bytes,
            &mut Budget { used: 0, maximum: 10_000_000 }, &|| false).unwrap()
    }
    #[test]
    ...
    fn anchors_boundaries_binary_and_ascii_case_are_byte_exact() {
        assert_eq!(find(br"^\w+\s+\d{2,3}$", b"item_1 \t123", false), Some((0, 11)));
        ...
        assert_eq!(find(br"^\x00[\x80-\xff]+$", &[0, 128, 255], false), Some((0, 3)));
    ...
    fn malformed_or_unsupported_patterns_never_become_literals() {
        for pattern in [b"".as_slice(), b"(", b")", b"[", b"[]", b"[^]", b"[z-a]",
            br"\", br"\1", br"\p{L}", br"\x0", br"\xgg", b"(?=a)", b"(?:a)",
            b"(?i)a", b"a*?", b"a++", b"a{", b"a{65}", b"a{2,1}", b"a{,2}",
            b"*a", b"a\nb", b"[[:alpha:]]", b"[a&&b]", br"[\d-a]"]
        { assert!(Program::compile(pattern, false).is_err(), "{pattern:?}"); }
        assert_eq!(Program::compile(&vec![b'a'; MAX_PATTERN + 1], false).unwrap_err().kind,
            RegexErrorKind::PatternLimit);
        assert_eq!(Program::compile(b"((a{64}){64})", false).unwrap_err().kind,
    ...
    fn budget_has_an_exact_permitted_twin_and_cancellation_is_not_absence() {
        ...
        let mut budget = Budget { used: 0, maximum: 1_000_000 };
        assert_eq!(runner.find_line(&bytes, &mut budget, &|| false).unwrap(), None);
        let used = budget.used;
        assert!(used <= (bytes.len() as u64 + 1) * (program.states() as u64 * 4 + 2));
        assert_eq!(runner.find_line(&bytes, &mut Budget { used: 0, maximum: used }, &|| false).unwrap(), None);
        assert_eq!(runner.find_line(&bytes, &mut Budget { used: 0, maximum: used - 1 }, &|| false),
            Err(ScanError::WorkLimit));
        assert_eq!(runner.find_line(b"a", &mut Budget { used: 0, maximum: used }, &|| true),
            Err(ScanError::Cancelled));
        let polls = std::cell::Cell::new(0);
        let cancel = || { polls.set(polls.get() + 1); polls.get() > 2 };
        assert_eq!(runner.find_line(&bytes, &mut Budget { used: 0, maximum: 1_000_000 }, &cancel),
            Err(ScanError::Cancelled));
    }
    #[test]
    fn ambiguous_patterns_do_not_create_exponential_threads() {
        for size in [8, 64, 1024] {
            for pattern in [b"(a+)+b".as_slice(), b"(a|aa)*b", b"((a?)*)*b"] {
                ...
                let mut runner = Runner::new(&program).unwrap();
                let mut budget = Budget { used: 0, maximum: 1_000_000 };
                assert_eq!(runner.find_line(&vec![b'a'; size], &mut budget, &|| false).unwrap(), None);
    ...
    fn literal_programs_agree_with_independent_scalar_windows() {
        for size in 0..8 {
            for bits in 0..(1usize << size) {
                let bytes: Vec<_> = (0..size).map(|i| if bits & (1 << i) == 0 { b'a' } else { b'B' }).collect();
                for pattern in [b"a".as_slice(), b"B", b"aa", b"AbA", b"BBBB"] {
                    for insensitive in [false, true] {
                        let expected = bytes.windows(pattern.len()).position(|candidate| candidate.iter()
                            .zip(pattern).all(|(a, b)| if insensitive { a.eq_ignore_ascii_case(b) } else { a == b }))
