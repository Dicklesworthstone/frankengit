...
impl RegexQuery {
    pub fn new(pattern: &[u8], case: SearchCase, prefixes: &[Vec<u8>],
        maximum_steps: u64,
        ...
        let scope = SourceQuery::new(b"\0", case, prefixes)
            .map_err(|_| RegexQueryError::InvalidScope)?;
        Ok(Self { pattern: pattern.to_vec(), program, scope, maximum_steps })
    }
    #[must_use]
    pub fn pattern(&self) -> &[u8] { &self.pattern }
    #[must_use]
    pub const fn case(&self) -> SearchCase { self.scope.case() }
    #[must_use]
    pub fn prefixes(&self) -> &[TreePath] { self.scope.prefixes() }
    /// Shared node selection consumes only this query's normalized path scope.
    /// The sentinel needle has no role in regex matching.
    #[must_use]
    pub const fn source_scope(&self) -> &SourceQuery { &self.scope }
    #[must_use]
    pub const fn maximum_steps(&self) -> u64 { self.maximum_steps }
    #[must_use]
    pub fn state_count(&self) -> usize { self.program.states() }
}
...
pub fn search_source_regex<A: GitHashAlgorithm, S: ObjectSource<A>>(
    ...
    query: &RegexQuery, limits: SearchLimits, cancelled: &dyn Fn() -> bool,
    ...
    let mut report = RegexSearchReport {
        ...
        program_states: query.state_count(), steps: 0, lines_searched: 0,
    };
    let mut runner = engine::Runner::new(&query.program).map_err(scan_error)?;
    let mut budget = engine::Budget { used: 0, maximum: query.maximum_steps };
    ...
    'files: for (path, blob) in discovery.files {
        ...
        report.source.bytes_read = report.source.bytes_read.checked_add(body.len())
        ...
        for (line_index, record) in body.split_inclusive(|byte| *byte == b'\n').enumerate() {
            ...
            let matched = runner.find_line(line, &mut budget, cancelled).map_err(scan_error)?;
            ...
            if let Some((start, end)) = matched {
                ...
                let excerpt_end = line.len().min(end.saturating_add(80)).min(excerpt_start + 416);
                result_bytes = result_bytes.checked_add(path.as_bytes().len())
                    .and_then(|n| n.checked_add(excerpt_end - excerpt_start))
                    .filter(|n| *n <= MAX_RESULT_BYTES).ok_or(SearchError::Budget("regex result bytes"))?;
                report.source.matches.push(SourceMatch {
                    path: path.as_bytes().to_vec(), blob, byte_offset: line_start + start,
                    line: line_index + 1, byte_column: start + 1, match_length: end - start,
...
mod tests {
    ...
    fn query_limits_and_scope_cannot_disable_or_widen_the_profile() {
        for maximum in [0, MAX_REGEX_STEPS + 1, u64::MAX] {
            assert_eq!(RegexQuery::new(b"a", SearchCase::Exact, &[], maximum),
                Err(RegexQueryError::InvalidWorkLimit));
        }
        assert!(RegexQuery::new(b"a", SearchCase::Exact, &[], 1).is_ok());
        assert!(matches!(RegexQuery::new(b"(", SearchCase::Exact, &[], MAX_REGEX_STEPS),
            Err(RegexQueryError::Expression(_))));
        assert_eq!(RegexQuery::new(b"a", SearchCase::Exact, &[b"../secret".to_vec()], MAX_REGEX_STEPS),
            Err(RegexQueryError::InvalidScope));
        let query = RegexQuery::new(br"\bThing\b", SearchCase::Exact,
            &[b"src".to_vec(), b"src".to_vec()], MAX_REGEX_STEPS).unwrap();
        assert_eq!(query.prefixes().len(), 1);
        assert!(query.source_scope().includes(&TreePath::parse_default(b"src/a").unwrap()));
        assert!(!query.source_scope().includes(&TreePath::parse_default(b"src2/a").unwrap()));
        assert_eq!(query.pattern(), br"\bThing\b");
    }
    #[test]
    fn execution_errors_are_distinct_from_empty_success() {
        assert!(matches!(scan_error(engine::ScanError::Cancelled), SearchError::Cancelled));
        assert!(matches!(scan_error(engine::ScanError::WorkLimit), SearchError::Budget("regex VM work")));
        assert!(matches!(scan_error(engine::ScanError::Allocation), SearchError::Budget("regex VM allocation")));
    }
}
