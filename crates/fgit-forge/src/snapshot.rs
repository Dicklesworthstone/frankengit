            ...
            Self::Restricted {
                ...
                snapshot.pull_requests.retain(|num, pr| {
                    !revoked_prs.contains(num)
                        ...
                        && allowed_prs
                            .as_ref()
                            .is_none_or(|allowed| allowed.contains(num))
                });
                Ok(snapshot)
            }
        }
                    ...
                    if older_pr.state != newer_pr.state {
                        match &newer_pr.state {
                            ...
                            PullRequestState::MergedNative { merge } => {
                                pr_changes.insert(
                                    *num,
                                    PullRequestChange::MergedNative {
                                        merge: merge.clone(),
                                    },
                                );
                            }
...
impl fmt::Display for SnapshotRefusal {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ReplayBoundExceeded { limit, attempted } => write!(
                f,
                "replay bound exceeded: attempted {attempted} > limit {limit}"
            ),
            Self::AccessDenied { reason } => {
                write!(f, "snapshot access denied under current policy: {reason}")
            }
            Self::ConsistencyMismatch { detail } => {
                write!(f, "snapshot continuous consistency check failed: {detail}")
            }
...
impl From<ChronicleRefusal> for SnapshotRefusal {
    fn from(value: ChronicleRefusal) -> Self {
        Self::Chronicle(value)
    }
}
    ...
    if let AggregateId::PullRequest(num) = event.aggregate {
        match &event.payload {
            ...
            ForgeEventPayload::MergeCommittedNative(merge) => {
                if let Some(pr) = prs.get_mut(&num) {
                    pr.state = PullRequestState::MergedNative {
                        merge: merge.clone(),
                    };
                    pr.version = event.version;
                }
    ...
    if snapshot.effective_head_generation != live_head.generation {
        return Err(SnapshotRefusal::ConsistencyMismatch {
            detail: format!(
                ...
                snapshot.effective_head_generation, live_head.generation
            ),
        });
    }
            ...
            for b in batches {
                for record in &b.batch.committed_rcrs {
                    if record.repository_sequence == req_seq
                        && let Some(decision) =
                            b.batch.decisions.iter().find(|d| d.tx_id == record.tx_id)
                    {
