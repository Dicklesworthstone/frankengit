...
impl HistoryLimits {
    pub fn validate(self) -> Result<(), HistoryError> {
        ...
        for (value, max) in [
            ...
            (self.max_tree_entries, ceiling.max_tree_entries),
            (self.max_blob_bytes, ceiling.max_blob_bytes),
            (self.max_lines, ceiling.max_lines),
            (self.max_cached_bytes, ceiling.max_cached_bytes),
            (self.max_comparisons, ceiling.max_comparisons),
            (self.max_diff_work, ceiling.max_diff_work),
...
impl BlameOptions {
    pub fn validate(&self) -> Result<(), HistoryError> {
        ...
        if !valid_path(&self.path)
            || self.first_line > self.limits.max_lines
            || self
                .end_line
                .is_some_and(|end| end < self.first_line || end > self.limits.max_lines)
        {
            return Err(HistoryError::InvalidOptions);
        }
        Ok(())
...
fn check_oid(format: GitHashAlgorithm, id: GitOid) -> Result<(), HistoryError> {
    if id.is_zero() || id.algorithm() != format {
        Err(HistoryError::InvalidObject(id))
    } else {
        Ok(())
    }
}
fn charge(
    ...
    *total = total
        ...
        .ok_or(HistoryError::Budget(field))?;
    Ok(())
}
...
fn graph(
    ...
    for commit in commits.values() {
        ...
        for parent in commit.parents.iter().copied().collect::<BTreeSet<_>>() {
            *children
                .get_mut(&parent)
                .ok_or(HistoryError::InvalidObject(parent))? += 1;
        }
    }
    let mut ready: BTreeSet<_> = children
        .iter()
    ...
    while let Some(id) = ready.pop_first() {
            ...
            .collect::<BTreeSet<_>>()
        {
            let count = children
                .get_mut(&parent)
                .ok_or(HistoryError::InvalidObject(parent))?;
    ...
    if order.len() != commits.len() {
        return Err(HistoryError::CyclicHistory);
    }
    source.checkpoint()?;
...
fn record(
    ...
    if body.len() > 64 * 1024 {
        return Err(HistoryError::Budget("commit metadata"));
    }
    charge(bytes, body.len(), limit, "metadata bytes")?;
    if git_object_id(id.algorithm(), GitObjectKind::Commit, &body) != id {
        return Err(HistoryError::InvalidObject(id));
    }
    let commit = graph
        .commits
        .get(&id)
        .ok_or(HistoryError::InvalidObject(id))?;
    source.checkpoint()?;
    Ok(HistoryCommit {
        id,
        tree: commit.tree,
...
pub fn commit_history(
    ...
    if options.after > graph.order.len() {
    ...
    Ok(HistoryPage {
        tip,
        total_commits: graph.order.len(),
        after: options.after,
        next_after: (end < graph.order.len()).then_some(end),
        commits,
    })
}
...
struct Walker<'a, S> {
    ...
    path: &'a [u8],
    limits: HistoryLimits,
    paths: BTreeMap<GitOid, Option<GitOid>>,
    blobs: BTreeMap<GitOid, Lines>,
    entries: usize,
    cached: usize,
    comparisons: usize,
    algorithms: Vec<DiffAlgorithm>,
}
impl<S: HistorySource> Walker<'_, S> {
    fn file(&mut self, root: GitOid) -> Result<Option<GitOid>, HistoryError> {
        self.source.checkpoint()?;
        if let Some(found) = self.paths.get(&root) {
            return Ok(*found);
        }
    ...
    fn load(&mut self, id: GitOid) -> Result<(), HistoryError> {
        ...
        if body.len() > self.limits.max_blob_bytes {
            return Err(HistoryError::Budget("blob bytes"));
        }
        if git_object_id(self.format, GitObjectKind::Blob, &body) != id {
            return Err(HistoryError::InvalidObject(id));
        }
        if body.contains(&0) {
        ...
        let cost = body
            .len()
            .checked_add(count * std::mem::size_of::<Range<usize>>())
    ...
    fn correspondence(
        ...
        parent: GitOid,
        current: GitOid,
    ) -> Result<Vec<Option<usize>>, HistoryError> {
        self.load(parent)?;
        self.load(current)?;
        let old = &self.blobs[&parent];
        let new = &self.blobs[&current];
        if parent == current {
            return Ok((0..new.spans.len()).map(Some).collect());
        }
        charge(
        ...
        let comparison = diff(
            ...
            DiffOptions::myers_lines(DiffLimits {
                max_input_bytes: self.limits.max_blob_bytes * 2,
                max_units: self.limits.max_lines * 2,
                max_work: self.limits.max_diff_work,
                max_trace_cells: 512_000,
            }),
        )
        .map_err(HistoryError::Diff)?;
        self.source.checkpoint()?;
        if !self.algorithms.contains(&comparison.algorithm) {
            self.algorithms.push(comparison.algorithm);
        }
...
pub fn blame(
    ...
    if options.first_line > end_line || end_line > total_lines {
        return Err(HistoryError::LineRange);
    }
    let mut answers = vec![None; end_line - options.first_line];
    let mut pending = BTreeMap::from([(
        tip,
        (options.first_line..end_line)
            .map(|line| (line - options.first_line, line))
            .collect::<Vec<_>>(),
    )]);
    for current in &graph.order {
        source.checkpoint()?;
        let Some(mut active) = pending.remove(current) else {
            continue;
        };
        if active.is_empty() {
            continue;
        ...
        let current_blob = walker
            .file(commit.tree)?
            .ok_or(HistoryError::InconsistentAttribution)?;
        ...
        for parent in &commit.parents {
            ...
            let Some(parent_blob) = walker.file(graph.commits[parent].tree)? else {
                continue;
            };
        ...
        for (result, line) in active {
            ...
            if origin.body[span.clone()] != target.body[selected.clone()]
                || answers[result].is_some()
            {
                return Err(HistoryError::InconsistentAttribution);
            }
            answers[result] = Some(BlameLine {
                line: result + options.first_line,
                byte_start: selected.start,
                byte_end: selected.end,
                origin_commit: *current,
                origin_blob: current_blob,
                origin_line: line,
                origin_byte_start: span.start,
                origin_byte_end: span.end,
            });
        }
    }
    if !pending.is_empty() {
    ...
    let start = target
        .spans
        .get(options.first_line)
        .map_or(target.body.len(), |span| span.start);
    let end = target
        .spans
        .get(end_line)
        .map_or(target.body.len(), |span| span.start);
    ...
    Ok(BlameResult {
        ...
        content: target.body[start..end].to_vec(),
        lines,
        origins,
        graph_commits: graph.order.len(),
        comparisons: walker.comparisons,
        algorithms: walker.algorithms,
    })
}
