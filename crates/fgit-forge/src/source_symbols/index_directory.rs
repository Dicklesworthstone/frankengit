...
const KINDS: [engine::Kind; 8] = [engine::Kind::Function, engine::Kind::Struct,
    engine::Kind::Enum, engine::Kind::Trait, engine::Kind::Type, engine::Kind::Module,
    engine::Kind::Union, engine::Kind::Macro];
fn kind_tag(kind: engine::Kind) -> u8 {
    match kind {
        engine::Kind::Function => 0, engine::Kind::Struct => 1, engine::Kind::Enum => 2,
        engine::Kind::Trait => 3, engine::Kind::Type => 4, engine::Kind::Module => 5,
        engine::Kind::Union => 6, engine::Kind::Macro => 7,
    }
}
pub(super) fn summarize(table: &table::Table, cancelled: &dyn Fn() -> bool) -> Result<Names, Error> {
    let mut names = Names::new();
    for row in table.rows() {
        check(cancelled)?;
        *names.entry((row.name.clone(), kind_tag(row.kind))).or_insert(0) += 1;
    }
    check(cancelled)?;
    Ok(names)
}
...
struct Posting { document: usize, kind: u8, count: usize }
...
#[derive(Clone, Debug)]
pub struct NameDirectory { manifest: Digest, names: BTreeMap<Vec<u8>, Vec<Posting>> }
frame!(DirectoryFrame, "source-symbol-name-directory");

impl NameDirectory {
    pub(super) fn build(manifest: &Manifest, names: &[Names], cancelled: &dyn Fn() -> bool)
        -> Result<Self, Error>
    {
        if names.len() != manifest.documents().len() { return Err(Error::Invalid("directory inventory")); }
        let mut result = Self { manifest: manifest.encode(cancelled)?.root, names: BTreeMap::new() };
        for (document, summary) in names.iter().enumerate() {
            for ((name, kind), count) in summary {
                check(cancelled)?;
                result.names.entry(name.clone()).or_default().push(Posting { document, kind: *kind, count: *count });
            }
        }
    ...
    fn validate(&self, manifest: &Manifest, cancelled: &dyn Fn() -> bool) -> Result<(), Error> {
        check(cancelled)?;
        if self.manifest != manifest.encode(cancelled)?.root { return Err(Error::CommitmentMismatch); }
        let mut counts = vec![0usize; manifest.documents().len()];
        let mut postings = 0usize;
        for (name, rows) in &self.names {
            check(cancelled)?;
            if name.is_empty() || name.len() > engine::MAX_NAME_BYTES || name == b"_"
                || !(name[0].is_ascii_alphabetic() || name[0] == b'_')
                || !name.iter().all(|b| b.is_ascii_alphanumeric() || *b == b'_') || rows.is_empty()
            { return Err(Error::Invalid("directory name")); }
            let mut previous = None;
            for row in rows {
                check(cancelled)?;
                let key = (row.document, row.kind);
                if row.document >= counts.len() || usize::from(row.kind) >= KINDS.len()
                    || row.count == 0 || previous.is_some_and(|p| p >= key)
                { return Err(Error::Invalid("directory posting")); }
                add(&mut counts[row.document], row.count, manifest.documents()[row.document].declarations,
                    "directory declaration count")?;
                add(&mut postings, 1, engine::MAX_DECLARATIONS, "directory postings")?;
                previous = Some(key);
            }
        }
        if counts.iter().zip(manifest.documents()).any(|(n, d)| *n != d.declarations) {
    ...
    pub fn encode(&self, manifest: &Manifest, cancelled: &dyn Fn() -> bool) -> Result<Payload, Error> {
        ...
        for (name, rows) in &self.names {
            ...
            for row in rows {
                check(cancelled)?;
                out.write_scalar(row.document as u32); out.write_scalar(row.kind); out.write_scalar(row.count as u32);
                if out.len() > MAX_PAYLOAD { return Err(Error::Limit("directory bytes")); }
            }
        }
        let result = payload(&DirectoryFrame(out.into_bytes())).map_err(|error| match error {
            Error::Limit("payload bytes") => Error::Limit("directory bytes"), other => other,
        })?;
        let mut total = manifest.encode(cancelled)?.bytes.len();
        for doc in manifest.documents() { add(&mut total, doc.encoded_bytes, MAX_INDEX_BYTES, "index bytes")?; }
        add(&mut total, result.bytes.len(), MAX_INDEX_BYTES, "directory bytes")?;
        check(cancelled)?;
        Ok(result)
    }
    pub fn decode(raw: &[u8], expected: Digest, manifest: &Manifest, cancelled: &dyn Fn() -> bool)
        -> Result<Self, Error>
    {
        check(cancelled)?;
        let limits = || DecodeLimits { elements: 160_000, ..decode_limits() };
        let frame = decode_body::<DirectoryFrame>(raw, limits())?;
        if root(&frame)? != expected || encode_body(&frame)? != raw { return Err(Error::CommitmentMismatch); }
        ...
        for _ in 0..count {
            ...
            if names.last_key_value().is_some_and(|(last, _)| last >= &name) {
                return Err(Error::Invalid("directory name order"));
            }
            let n = input.read_scalar::<u32>("postings")? as usize;
            add(&mut total, n, engine::MAX_DECLARATIONS, "directory postings")?;
            if n > input.remaining() / 9 { return Err(Error::Invalid("directory posting count")); }
            let mut rows = Vec::with_capacity(n);
            for _ in 0..n {
                check(cancelled)?;
                rows.push(Posting { document: input.read_scalar::<u32>("document")? as usize,
                    kind: input.read_scalar::<u8>("kind")?, count: input.read_scalar::<u32>("declarations")? as usize });
            }
            names.insert(name, rows);
        }
        input.finish()?;
        let result = Self { manifest: bound, names };
        if result.encode(manifest, cancelled)?.bytes != raw { return Err(Error::CommitmentMismatch); }
        Ok(result)
    }
}
...
impl Query {
    ...
    pub fn directory_candidates(&mut self, manifest: &Manifest, raw: &[u8], expected: Digest,
        cancelled: &dyn Fn() -> bool,
    ) -> Result<Vec<usize>, Error> {
        self.work.charge(raw.len() as u64 + manifest.documents().len() as u64, cancelled)?;
        ...
        for (key, rows) in directory.names.range(name.to_vec()..) {
            ...
            for row in rows {
                self.work.charge(1 + self.query.kinds().len() as u64, cancelled)?;
                if (self.query.kinds().is_empty() || self.query.kinds().contains(&KINDS[usize::from(row.kind)]))
        ...
        check(cancelled)?;
        Ok(selected.into_iter().enumerate().filter_map(|(i, take)| take.then_some(i)).collect())
    }
}
