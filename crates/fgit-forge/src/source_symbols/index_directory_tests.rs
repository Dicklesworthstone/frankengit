...
fn source(format: Format) -> Source {
    let id = |domain, family| internal_object_id(domain,
        SchemaId::new(SchemaFamily::from_static(family), 1, 0), CodecVersion::new(1, 0), b"directory-test");
    Source {
...
fn corpus(format: Format, files: &[(&[u8], &[u8])]) -> (Manifest, Vec<Payload>, Vec<Names>) {
    let mut documents = Vec::new(); let mut payloads = Vec::new(); let mut summaries = Vec::new();
    for (path, bytes) in files {
        ...
        documents.push(Document { path: path.to_vec(), blob, root: payload.root,
        ...
        summaries.push(summarize(&table, &|| false).unwrap()); payloads.push(payload);
    }
    (Manifest { source: source(format), documents, unsupported: 0, non_regular: 0 }, payloads, summaries)
}
fn fixture(format: Format) -> (Manifest, Vec<Payload>, Vec<Names>) {
    corpus(format, &[(b"a.rs", b"fn Zebra() {}\nstruct Alpha;\nfn Alpha() {}\nmod M { fn Alpha() {} }\n"),
        (b"empty.rs", b""), (b"src/b.rs", b"fn Alpine() {}\nfn r#type() {}\n"),
        (b"src2/c.rs", b"enum Alpha {}\ntrait Beta {}\n"), (b"z\xff.rs", b"union Alpha { a: u8 }\n")])
}
fn query(name: &[u8], mode: SymbolMatchMode, kinds: &[engine::Kind], scopes: &[Vec<u8>]) -> SymbolQuery {
...
fn accelerated_exact_prefix_kind_and_path_queries_match_scalar_tables_in_both_formats() {
    for format in [Format::Sha1, Format::Sha256] {
        ...
        for name in [b"A".as_slice(), b"Al", b"Alpha", b"Alpine", b"Beta", b"Zebra", b"type", b"Missing", b"_"] {
                    ...
                    for scopes in [vec![], vec![b"src".to_vec()], vec![b"a.rs".to_vec()]] {
                        ...
                        for maximum in [1, 3, 100] {
                            ...
                            for (doc, raw) in manifest.documents().iter().zip(&payloads) {
                                if scalar.includes(doc) && scalar.observe(doc, &raw.bytes, &|| false).unwrap() { break; }
                            }
                            let mut fast = Query::new(&q, maximum).unwrap();
                            let candidates = fast.directory_candidates(&manifest, &payload.bytes, payload.root, &|| false).unwrap();
                            assert!(candidates.windows(2).all(|w| w[0] < w[1]));
                            for i in candidates {
                                if fast.observe(&manifest.documents()[i], &payloads[i].bytes, &|| false).unwrap() { break; }
                            }
        ...
        assert_eq!(Query::new(&q, 10).unwrap().directory_candidates(&manifest, &payload.bytes, payload.root, &|| false).unwrap(), vec![2]);
        assert_eq!(NameDirectory::decode(&payload.bytes, payload.root, &manifest, &|| false).unwrap()
            .encode(&manifest, &|| false).unwrap().bytes, payload.bytes);
    }
}
#[test]
fn directory_commitment_manifest_binding_truncations_and_cancellation_fail_closed() {
    ...
    let payload = NameDirectory::build(&manifest, &summaries, &|| false).unwrap().encode(&manifest, &|| false).unwrap();
    for cut in 0..payload.bytes.len() { assert!(NameDirectory::decode(&payload.bytes[..cut], payload.root, &manifest, &|| false).is_err()); }
    let mut corrupt = payload.bytes.clone(); *corrupt.last_mut().unwrap() ^= 1;
    assert!(NameDirectory::decode(&corrupt, payload.root, &manifest, &|| false).is_err());
    let mut moved = manifest.clone(); moved.source.commit = git_object_id(Format::Sha256, GitObjectKind::Commit, b"changed");
    assert!(NameDirectory::decode(&payload.bytes, payload.root, &moved, &|| false).is_err());
    let mut renamed = manifest.clone(); renamed.documents[0].path = b"aa.rs".to_vec();
    ...
    assert!(NameDirectory::decode(&payload.bytes, payload.root, &manifest, &|| {
        let n = calls.get() + 1; calls.set(n); n > 12
    }).is_err());
    assert!(calls.get() > 12);
}
#[test]
fn complete_document_counts_invalid_names_kinds_and_duplicate_postings_are_checked() {
    ...
    for alteration in 0..6 {
        ...
        match alteration {
            0 => { rows.pop(); },
            1 => { rows[0].document = usize::MAX; },
            2 => { rows[0].kind = 8; },
            3 => { rows[0].count = 0; },
            4 => { rows.insert(0, rows[0].clone()); },
            _ => { rows[0].count = usize::MAX; },
        }
        assert!(bad.encode(&manifest, &|| false).is_err());
    }
    let mut bad = directory.clone();
    let rows = bad.names.remove(b"Alpha".as_slice()).unwrap(); bad.names.insert(b"r#Alpha".to_vec(), rows);
    assert!(bad.encode(&manifest, &|| false).is_err());
    assert!(NameDirectory::build(&manifest, &summaries[..1], &|| false).is_err());
    let (empty, _, names) = corpus(Format::Sha1, &[]);
    let payload = NameDirectory::build(&empty, &names, &|| false).unwrap().encode(&empty, &|| false).unwrap();
    assert!(Query::new(&query(b"A", SymbolMatchMode::Prefix, &[], &[]), 10).unwrap()
        .directory_candidates(&empty, &payload.bytes, payload.root, &|| false).unwrap().is_empty());
}
#[test]
fn predecessor_verification_reuses_the_same_names_without_rescanning_source() {
    ...
    let reused: Vec<_> = manifest.documents().iter().map(|d| verified.names(&d.blob).unwrap().clone()).collect();
    assert_eq!(reused, summaries);
    let direct = NameDirectory::build(&manifest, &summaries, &|| false).unwrap().encode(&manifest, &|| false).unwrap();
    assert_eq!(NameDirectory::build(&manifest, &reused, &|| false).unwrap().encode(&manifest, &|| false).unwrap().bytes, direct.bytes);
}
#[test]
...
fn directory_size_fallback_does_not_reduce_the_legacy_corpus_or_swallow_cancellation() {
    let paths: Vec<_> = (0..20).map(|i| format!("file{i:02}.rs").into_bytes()).collect();
    let bodies: Vec<_> = (0..20).map(|file| (0..400).map(|i|
        format!("fn N{file:02}_{i:03}{}() {{}}\n", "x".repeat(116))).collect::<String>().into_bytes()).collect();
    let files: Vec<_> = paths.iter().zip(&bodies).map(|(p,b)| (p.as_slice(),b.as_slice())).collect();
    let (manifest, tables, names) = corpus(Format::Sha256, &files);
    let report = SourceSearchReport { repository: manifest.source.repository, source_rcr: manifest.source.rcr,
        source_commit: manifest.source.commit, source_tree: manifest.source.tree, matches: vec![],
        completion: SearchCompletion::Complete, files_selected: files.len(), files_read: files.len(),
        bytes_read: bodies.iter().map(Vec::len).sum(), bytes_searched: 0, non_regular_entries: 0 };
    let corpus = || Corpus { source: report.clone(), documents: manifest.documents.clone(), tables: tables.clone(),
        unsupported: 0, reused: 0, reuse_scope: None, names: names.clone() };
    let expected = manifest.encode(&|| false).unwrap();
    let (actual, _, directory) = corpus().finish_with_directory(manifest.source.clone(), &|| false).unwrap();
    assert!(directory.is_none());
    assert_eq!(actual.encode(&|| false).unwrap().bytes, expected.bytes);
    assert!(corpus().finish_with_directory(manifest.source.clone(), &|| true).is_err());
}
