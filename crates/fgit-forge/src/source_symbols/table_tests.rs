use super::*;
fn build(bytes: &[u8]) -> Table {
    Table::build(bytes, &mut Budget::new(engine::MAX_WORK).unwrap(), &|| {
        false
    })
    .unwrap()
}
fn find(table: &Table, name: &[u8], prefix: bool, kinds: &[Kind]) -> Vec<Vec<u8>> {
    table
        .lookup(
            ...
            prefix,
            kinds,
            &mut Work::new(engine::MAX_WORK).unwrap(),
            &|| false,
        )
        .unwrap()
        .into_iter()
        .map(|i| table.rows()[i].name.clone())
        .collect()
}
#[test]
fn persisted_rows_keep_raw_identifiers_offsets_excerpts_and_exclusions() {
    ...
    let decoded = Table::decode(&raw, &|| false).unwrap();
    assert_eq!(decoded, table);
    assert_eq!(table.rows().len(), 4);
    assert_eq!(table.macros, 1);
    assert_eq!(table.attributes, 1);
    assert_eq!(
        find(&decoded, b"Thing", true, &[]),
        vec![b"Thing".to_vec(), b"ThingMore".to_vec()]
    );
    assert!(find(&decoded, b"fake", false, &[]).is_empty());
    for row in decoded.rows() {
        assert_eq!(&source[row.offset..row.offset + row.name.len()], row.name);
        assert_eq!(
            &source[row.excerpt_offset..row.excerpt_offset + row.excerpt.len()],
            row.excerpt
        );
    }
...
fn prefix_dictionary_results_preserve_source_order_not_name_order() {
    ...
    assert_eq!(
        find(&table, b"Z", true, &[]),
        vec![b"Zoo".to_vec(), b"Zed".to_vec(), b"Zoo".to_vec()]
    );
    assert_eq!(
        find(&table, b"Zoo", false, &[Kind::Struct]),
        Vec::<Vec<u8>>::new()
    );
    assert_eq!(
        find(&table, b"Z", true, &[Kind::Struct]),
        vec![b"Zed".to_vec()]
    );
                ...
                let expected: Vec<_> = table
                    ...
                    .filter(|r| {
                        (if prefix {
                            ...
                            r.name == name
                        }) && (kinds.is_empty() || kinds.contains(&r.kind))
                    })
                    .map(|r| r.name.clone())
                    .collect();
...
fn every_truncation_and_suffix_is_rejected_without_a_partial_table() {
    let raw = build(b"struct One; fn Two() {}\n")
        ...
        .unwrap();
    for n in 0..raw.len() {
        assert!(Table::decode(&raw[..n], &|| false).is_err(), "cut {n}");
    }
...
fn empty_tables_and_literal_only_sources_are_initialized_empty_values() {
    for source in [
        ...
        assert_eq!(
            Table::decode(&table.encode(&|| false).unwrap(), &|| false).unwrap(),
            table
        );
...
fn utf8_comments_and_crlf_do_not_change_native_byte_coordinates() {
    let source = "// 🦀\r\nfn Café() {}".as_bytes();
    assert!(matches!(
        Table::build(source, &mut Budget::new(engine::MAX_WORK).unwrap(), &|| {
            false
        }),
        Err(Error::Syntax(_))
    ));
    ...
    assert_eq!(row.column, 4);
    assert_eq!(&source[row.offset..row.offset + 4], b"Good");
    assert_eq!(
        Table::decode(&table.encode(&|| false).unwrap(), &|| false).unwrap(),
        table
    );
...
fn query_work_is_shared_and_has_an_exact_success_boundary() {
    ...
    table
        .lookup(b"O", true, &[], &mut measured, &|| false)
        .unwrap();
    let exact = measured.used;
    assert!(
        table
            .lookup(b"O", true, &[], &mut Work::new(exact).unwrap(), &|| false)
            .is_ok()
    );
    assert!(
        table
            .lookup(b"O", true, &[], &mut Work::new(exact - 1).unwrap(), &|| {
                false
            })
            .is_err()
    );
    ...
    table
        .lookup(b"O", true, &[], &mut shared, &|| false)
        .unwrap();
    assert!(
        table
            .lookup(b"Absent", true, &[], &mut shared, &|| false)
            .is_err()
    );
    for limit in [0, engine::MAX_WORK + 1] {
        assert!(Work::new(limit).is_err());
    }
}
#[test]
fn table_byte_limits_and_invalid_source_never_silently_omit_declarations() {
    ...
    let source: String = (0..8000)
        .map(|i| format!("fn Name{i:05}{}() {{}}\n", "x".repeat(100)))
        .collect();
    assert!(matches!(
        Table::build(
            source.as_bytes(),
            &mut Budget::new(engine::MAX_WORK).unwrap(),
            &|| false
        ),
        Err(Error::Limit("table bytes"))
    ));
}
#[test]
fn repeated_builds_have_identical_canonical_bytes_and_search_work() {
    ...
    assert_eq!(
        first.encode(&|| false).unwrap(),
        second.encode(&|| false).unwrap()
    );
    let mut a = Work::new(engine::MAX_WORK).unwrap();
    let mut b = Work::new(engine::MAX_WORK).unwrap();
    assert_eq!(
        first.lookup(b"I", true, &[], &mut a, &|| false).unwrap(),
        second.lookup(b"I", true, &[], &mut b, &|| false).unwrap()
    );
