...
impl Source {
    ...
    fn tree_id(&mut self, format: GitHashAlgorithm, entries: &[(&[u8], u32, GitOid)]) -> GitOid {
        ...
        entries.sort_by_key(|entry| {
            ...
            key.push(if is_tree(entry.mode) { b'/' } else { 0 });
            key
        });
...
fn apply_hunks(old: &[u8], hunks: &[ReviewHunk]) -> Vec<u8> {
    ...
    for hunk in hunks {
        ...
        result.extend_from_slice(&hunk.after);
        cursor = hunk.old.byte_end;
    }
    result.extend_from_slice(&old[cursor..]);
    result
}
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        for (old, new) in [
            ...
            for context in [0, 1, 3, 20] {
                ...
                let (a, b) = source.pair(format, old, new);
                let options = ReviewOptions {
                    context_lines: context,
                    ..ReviewOptions::default()
                };
                let report = compare_source(&source, format, a, b, &options).unwrap();
                let ReviewContent::Text {
                    hunks,
                    before_bytes,
                    after_bytes,
                    ..
                } = &report.entries[0].content
                else {
                    panic!("text");
                };
                ...
                assert_eq!(
                    compare_source(&source, format, a, b, &options).unwrap(),
                    report
                );
            }
        }
...
fn merge_base_excludes_target_only_work_and_direct_comparison_does_not() {
    ...
    let root = source.tree_id(
        format,
        &[(b"a", 0o100644, original), (b"b", 0o100644, original)],
    );
    let base = source.commit_id(format, root, &[], "base");
    let root_a = source.tree_id(
        format,
        &[(b"a", 0o100644, change), (b"b", 0o100644, original)],
    );
    let root_b = source.tree_id(
        format,
        &[(b"a", 0o100644, original), (b"b", 0o100644, change)],
    );
    let a = source.commit_id(format, root_a, &[base], "ours");
    let b = source.commit_id(format, root_b, &[base], "theirs");
    let mut options = ReviewOptions::default();
    assert_eq!(
        compare_source(&source, format, a, b, &options)
            .unwrap()
            .entries
            .len(),
        2
    ...
    let report = compare_source(&source, format, a, b, &options).unwrap();
    assert_eq!(report.compared_before, base);
...
fn mode_binary_links_empty_files_and_empty_directories_remain_visible() {
    let f = GitHashAlgorithm::Sha1;
    let mut source = Source::default();
    let text = source.blob_id(f, b"same\n");
    let binary = source.blob_id(f, b"\0payload");
    let empty = source.blob_id(f, b"");
    let empty_tree = source.tree_id(f, &[]);
    let root = source.tree_id(f, &[(b"mode", 0o100644, text)]);
    let a = source.commit_id(f, root, &[], "a");
    let root = source.tree_id(
        f,
        &[
            (b"binary", 0o100644, binary),
            (b"empty", 0o100644, empty),
            (b"empty-dir", 0o040000, empty_tree),
            (b"link", 0o120000, text),
            (b"mode", 0o100755, text),
            (b"submodule", 0o160000, a),
        ],
    );
    ...
    assert!(matches!(
        report.entries[1].content,
        ReviewContent::Text { additions: 0, .. }
    ));
    assert!(matches!(
        report.entries[2].content,
        ReviewContent::ObjectOnly
    ));
    assert!(matches!(
        report.entries[4].content,
        ReviewContent::Identical
    ));
    assert_eq!(report.entries[4].kind, ChangeKind::ModeChanged);
    assert!(matches!(
        report.entries[5].content,
        ReviewContent::ObjectOnly
    ));
}
...
fn type_changes_coalesce_git_order_and_unchanged_subtrees_are_not_read() {
    let f = GitHashAlgorithm::Sha1;
    let mut source = Source::default();
    let blob = source.blob_id(f, b"file\n");
    let nested = source.tree_id(f, &[(b"inside", 0o100644, blob)]);
    let unchanged = source.tree_id(f, &[(b"unread", 0o100644, blob)]);
    let a_tree = source.tree_id(
        f,
        &[
            (b"a", 0o100644, blob),
            (b"a.b", 0o100644, blob),
            (b"stable", 0o040000, unchanged),
        ],
    );
    let b_tree = source.tree_id(
        f,
        &[
            (b"a", 0o040000, nested),
            (b"a.b", 0o100644, blob),
    ...
    let a = source.commit_id(f, a_tree, &[], "a");
    let b = source.commit_id(f, b_tree, &[a], "b");
    ...
    assert_eq!(
        report
            ...
            .collect::<Vec<_>>(),
        [b"a".as_slice(), b"a/inside"]
    );
    assert_eq!(report.entries[0].kind, ChangeKind::TypeChanged);
    let options = ReviewOptions {
        paths: vec![b"a/inside".to_vec()],
        ..ReviewOptions::default()
    };
    assert_eq!(
        compare_source(&source, f, a, b, &options)
            .unwrap()
            .entries
            .len(),
        1
...
fn failures_are_not_successful_empty_or_partial_reports() {
    let f = GitHashAlgorithm::Sha1;
    let mut source = Source::default();
    let (a, b) = source.pair(f, b"a\nb\n", b"A\nB\n");
    let mut options = ReviewOptions::default();
    options.limits.max_blob_bytes = 1;
    assert!(matches!(
        compare_source(&source, f, a, b, &options),
        Err(ReviewError::Budget("blob bytes"))
    ));
    options = ReviewOptions::default();
    options.limits.max_output_bytes = 1;
    assert!(matches!(
        compare_source(&source, f, a, b, &options),
        Err(ReviewError::Budget("output bytes"))
    ));
    options = ReviewOptions::default();
    options.limits.max_diff_work = 1;
    assert!(matches!(
        compare_source(&source, f, a, b, &options),
        Err(ReviewError::Diff(DiffError::WorkExceeded { .. }))
    ));
    ...
    options = ReviewOptions::default();
    source.cancel.set(true);
    assert!(matches!(
        compare_source(&source, f, a, b, &options),
        Err(ReviewError::Source(MergeSourceError::Cancelled))
    ));
    source.cancel.set(false);
    source.blobs.clear();
    assert!(matches!(
        compare_source(&source, f, a, b, &options),
        Err(ReviewError::Source(MergeSourceError::Unavailable(_)))
    ));
}
...
fn ambiguous_or_unrelated_histories_never_choose_an_arbitrary_base() {
    let f = GitHashAlgorithm::Sha1;
    let mut source = Source::default();
    let tree = source.tree_id(f, &[]);
    let root = source.commit_id(f, tree, &[], "root");
    let other = source.commit_id(f, tree, &[], "unrelated");
    let a = source.commit_id(f, tree, &[root], "a");
    let b = source.commit_id(f, tree, &[root], "b");
    let left = source.commit_id(f, tree, &[a, b], "left");
    let right = source.commit_id(f, tree, &[b, a], "right");
    let options = ReviewOptions {
        mode: ComparisonMode::MergeBase,
        ..ReviewOptions::default()
    };
    assert!(matches!(
        compare_source(&source, f, left, right, &options),
        Err(ReviewError::MultipleMergeBases(_))
    ));
    assert!(matches!(
        compare_source(&source, f, root, other, &options),
        Err(ReviewError::NoCommonAncestor)
    ));
    assert!(
        compare_source(&source, f, root, root, &options)
            .unwrap()
            .entries
            .is_empty()
    );
