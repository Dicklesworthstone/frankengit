...
use fgit_treefs::{
    BaseEntry, BaseError, BaseView, CapabilityRefusal, ObjectSource, ObjectSourceError,
    TreeCapability, TreePath,
};
...
impl SearchLimits {
    pub fn validate(self) -> Result<(), SearchError> {
        ...
        for (value, maximum) in [
            (self.max_matches, 4096),
            (self.max_entries, ceiling.max_entries),
...
pub fn search_source<A: GitHashAlgorithm, S: ObjectSource<A>>(
    ...
    for (path, blob) in discovery.files {
        ...
        let consumed = scan(&body, query, cancelled, |start, line, line_start| {
            ...
            report.matches.push(SourceMatch {
...
pub(super) fn discover<A: GitHashAlgorithm, S: ObjectSource<A>>(
    ...
    if depth > ctx.limits.max_depth {
        return Err(SearchError::Budget("directory depth"));
    }
    let children = ctx
        .base
        .list(ctx.source, ctx.capability, directory, ctx.now)
        .map_err(|e| SearchError::Base(Box::new(e)))?;
    for (name, entry) in children {
        checkpoint(ctx.cancelled)?;
        discovery.entries += 1;
        if discovery.entries > ctx.limits.max_entries {
            return Err(SearchError::Budget("tree entries"));
        }
        ...
        match entry {
            ...
            BaseEntry::File { oid, mode } if ctx.query.includes(&path) => {
                if mode != b"100644" && mode != b"100755" {
                    return Err(SearchError::Budget("unsupported file mode"));
                }
                // An ancestor can be disclosable without granting its content.
                // A requested descendant under a FILE is not a readable file.
                if !ctx.capability.admits_disclosure(&path) {
                    continue;
                }
                if discovery.files.len() == ctx.limits.max_files {
                    return Err(SearchError::Budget("files"));
                }
                if discovery.files.insert(path, oid).is_some() {
                    return Err(SearchError::Budget("duplicate path"));
                }
            }
            BaseEntry::Symlink { .. } | BaseEntry::Submodule { .. }
                if ctx.query.includes(&path) =>
            {
...
fn scan(
    ...
    for (i, raw) in bytes.iter().enumerate() {
        ...
        if byte == fold(query.needle[matched], query.case) {
            matched += 1;
        }
        if matched == query.needle.len() {
            if !found(i + 1 - matched, line, line_start) {
                return Ok(i + 1);
            }
            matched = query.failure[matched - 1];
        }
...
mod tests {
    ...
    fn overlaps_empty_files_and_final_lines_are_exact() {
        assert_eq!(
            hits(b"aaaa\naa", b"aa", SearchCase::Exact),
            vec![(0, 1, 0), (1, 1, 0), (2, 1, 0), (5, 2, 5)]
        );
    ...
    fn utf8_binary_and_crlf_keep_original_byte_coordinates() {
        assert_eq!(
            hits("éX\r\n\0xX".as_bytes(), b"x", SearchCase::AsciiInsensitive),
            vec![(2, 1, 0), (6, 2, 5), (7, 2, 5)]
        );
                        ...
                        let expected: Vec<_> = bytes
                            ...
                            .filter(|(_, window)| {
                                window
                                    .iter()
                                    .zip(needle)
                                    .all(|(a, b)| fold(*a, case) == fold(*b, case))
                            })
                            .map(|(i, _)| (i, 1, 0))
                            .collect();
    ...
    fn cancellation_and_lookahead_are_not_empty_success() {
        ...
        assert!(matches!(
            scan(b"aaaa", &query, &|| true, |_, _, _| true),
            Err(SearchError::Cancelled)
        ));
        ...
        assert!(matches!(
            scan(&vec![b'b'; 8192], &query, &cancel, |_, _, _| true),
            Err(SearchError::Cancelled)
        ));
        let mut count = 0;
        assert_eq!(
            scan(b"aaaa", &query, &|| false, |_, _, _| {
                count += 1;
                count < 3
            })
            .unwrap(),
