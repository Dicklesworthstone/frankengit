...
impl RepositoryProtectionPolicy {
    ...
    fn read(input: &mut Decoder<'_>) -> Result<Self, CodecRefusal> {
        ...
        check_count(input, "protection.branches", MAX_PROTECTED_BRANCHES)?;
        let mut count = 0;
...
fn check_count(input: &Decoder<'_>, field: &'static str, limit: usize) -> Result<(), CodecRefusal> {
...
fn read_principals(input: &mut Decoder<'_>, limit: usize) -> Result<Vec<PrincipalId>, CodecRefusal> {
    check_count(input, "protection.principals", limit)?;
    let mut count = 0;
