...
impl IssueAction {
        ...
        match self {
            ...
            Self::Comment { body } => {
                ...
                if body.trim().is_empty() {
                    return Err(invalid_native("issue.empty_comment"));
                }
                Ok(())
            }
            ...
            Self::Edit(edit) => {
                for (field, value) in [("issue.title", &edit.title), ("issue.body", &edit.body)] {
                    ...
                    if let Some(text) = value {
                        out.write_bytes(field, text.as_bytes())?;
                    }
                }
    ...
    fn read(input: &mut Decoder<'_>) -> Result<Self, CodecRefusal> {
        ...
        let action = match input.read_scalar::<u32>("issue.action")? {
            1 => Self::Open {
                title: text(input, "issue.title", MAX_TITLE_BYTES)?,
                body: text(input, "issue.body", MAX_BODY_BYTES)?,
                labels: read_labels(input)?,
            },
            2 => Self::Edit(IssueEdit {
                title: if input.read_bool("issue.has_title")? {
                    Some(text(input, "issue.title", MAX_TITLE_BYTES)?)
                } else {
                    None
                },
                body: if input.read_bool("issue.has_body")? {
                    Some(text(input, "issue.body", MAX_BODY_BYTES)?)
                } else {
                    None
                },
                labels: if input.read_bool("issue.has_labels")? {
                    Some(read_labels(input)?)
                } else {
                    None
                },
            }),
            ...
            4 => Self::Reopen,
            5 => Self::Comment {
                body: text(input, "issue.comment", MAX_BODY_BYTES)?,
            },
            observed => {
...
fn validate_body(body: &str) -> Result<(), CodecRefusal> {
    if body.len() > MAX_BODY_BYTES || body.contains('\0') {
        return Err(invalid_native("issue.body"));
    }
    Ok(())
}
...
fn write_labels(out: &mut Encoder, labels: &[String]) -> Result<(), CodecRefusal> {
    validate_labels(labels)?;
    out.write_sequence("issue.labels", labels, |out, label| {
        out.write_bytes("issue.label", label.as_bytes())
    })
}
fn read_labels(input: &mut Decoder<'_>) -> Result<Vec<String>, CodecRefusal> {
    ...
    let labels = input.read_sequence("issue.labels", |input| {
        count = count
            .checked_add(1)
            .ok_or_else(|| invalid_native("issue.labels"))?;
        if count > MAX_LABELS {
            return Err(invalid_native("issue.labels"));
        }
...
impl NativeIssueEvent {
    ...
    pub(super) fn read(input: &mut Decoder<'_>) -> Result<Self, CodecRefusal> {
        Ok(Self {
            actor: PrincipalId::from_bytes(input.read_opaque_id("issue.actor")?),
...
impl IssueCommand {
    pub fn proposed_event(&self, actor: PrincipalId) -> Result<ForgeEvent, RefusalCode> {
        ...
        if matches!(self.action, IssueAction::Open { .. })
        ...
        Ok(ForgeEvent {
            aggregate: AggregateId::Issue(self.number),
            version,
...
pub fn apply_event(
    ...
    event: &ForgeEvent,
) -> Result<IssueSnapshot, RefusalCode> {
    let (AggregateId::Issue(number), ForgeEventPayload::IssueChangedNative(change)) =
        (event.aggregate, &event.payload)
    else {
        return Err(RefusalCode::EvidenceInvalid);
    };
    change
    ...
    let Some(previous) = previous else {
        ...
        return Ok(IssueSnapshot {
            ...
            opened_by: change.actor,
            last_actor: change.actor,
            comments: 0,
        });
    };
...
impl CompiledIssueQuery {
    #[must_use]
    pub const fn query(&self) -> &IssueQuery {
        &self.query
    }
    ...
    pub fn matches(&self, issue: &IssueSnapshot) -> Result<bool, CodecRefusal> {
        ...
        if self.query.state.is_some_and(|state| state != issue.state)
            || self
                .query
                .opened_by
                .is_some_and(|actor| actor != issue.opened_by)
            || !self
                .query
                .labels
                .iter()
                .all(|label| issue.labels.binary_search(label).is_ok())
        {
            return Ok(false);
        }
    ...
    fn contains(&self, bytes: &[u8]) -> bool {
        ...
        let mut matched = 0;
        for &byte in bytes {
            let byte = if self.query.case_sensitive {
                byte
            } else {
                byte.to_ascii_lowercase()
            };
...
mod query_tests {
    ...
    fn snapshot() -> IssueSnapshot {
        IssueSnapshot {
            ...
            last_actor: PrincipalId::from_bytes([2; 16]),
            comments: 0,
        }
    }
    ...
    fn query_filters_are_conjoined_and_author_is_not_last_actor() {
        let query = IssueQuery {
            ...
            opened_by: Some(PrincipalId::from_bytes([1; 16])),
            labels: vec!["bug".into(), "urgent".into()],
            text: Some("http".into()),
        ...
        for changed in [
            ...
            IssueQuery {
                opened_by: Some(PrincipalId::from_bytes([2; 16])),
                ..query.clone()
            },
            IssueQuery {
    ...
    fn literal_matching_is_ascii_only_and_does_not_join_fields() {
        for (text, case_sensitive, expected) in [
            ...
            ("CAFé", false, true),
            ("CAFÉ", false, false),
            ("body needle", true, true),
            ("HTTPcafé", false, false),
            (".*", false, false),
            ("é\nbody", true, true),
        ] {
            let query = IssueQuery {
            ...
            assert_eq!(
                query.compile().unwrap().matches(&snapshot()).unwrap(),
                expected,
                "{text}"
            );
        }
    ...
    fn query_bounds_and_canonical_labels_have_permitted_twins() {
        ...
        assert!(
            IssueQuery {
                text: Some("a".repeat(MAX_QUERY_BYTES)),
                ..Default::default()
            }
            .compile()
            .is_ok()
        ...
        for labels in [
            ...
            vec!["a".into(), "a".into()],
            vec![" ".into()],
            vec!["a".repeat(MAX_LABEL_BYTES + 1)],
            vec!["a\nb".into()],
        ] {
            assert!(
                IssueQuery {
    ...
    fn linear_matcher_agrees_with_scalar_literal_oracle() {
        fn word(bits: u32, length: usize) -> Vec<u8> {
            (0..length)
                .map(|n| if bits & (1 << n) == 0 { b'A' } else { b'b' })
                .collect()
        }
        for case_sensitive in [false, true] {
            for needle_len in 1..=4 {
                for needle_bits in 0..(1 << needle_len) {
                    ...
                    let query = IssueQuery {
                        text: Some(String::from_utf8(needle.clone()).unwrap()),
                        case_sensitive,
                        ..Default::default()
                    }
                    .compile()
                    ...
                    for hay_len in 0..=7 {
                        for hay_bits in 0..(1 << hay_len) {
                            ...
                            let expected = hay.windows(needle.len()).any(|window| {
                                if case_sensitive {
