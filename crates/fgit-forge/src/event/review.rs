//! Immutable whole-tree PR review decisions. The reviewer comes from the
//! authenticated session, never from a command's text. A decision approves
//! only its exact source/target pair and PR version, not an arbitrary future
//! merge candidate. Admission owns publication and reviewer authorization.
...
/// Whole-tree, direct target-to-source comparison; no path filtering,
/// whitespace normalization, rename heuristics, attributes or external drivers.
...
pub struct NativeReviewEvent {
    ...
    /// Untrusted UTF-8 explanation. Withdrawal and change requests require a
    /// nonblank reason; retaining it never makes it an instruction or policy.
    pub reason: String,
}
impl NativeReviewEvent {
    ...
    pub(super) fn write(&self, out: &mut Encoder) -> Result<(), CodecRefusal> {
        self.validate()?;
        out.write_scalar(1_u32); // REVIEW_PROFILE, required and versioned.
    ...
    pub(super) fn read(input: &mut Decoder<'_>) -> Result<Self, CodecRefusal> {
        ...
        if profile != 1 { return Err(CodecRefusal::VariantUnknown { field: "review.profile", observed: profile, offset }); }
        ...
        let event = Self { reviewer, subject: ReviewSubject { pull_request, pull_request_version,
            source_ref, target_ref, source_tip, target_tip, policy_epoch }, decision, reason };
        event.validate()?;
        Ok(event)
    }
}
...
impl ReviewCommand {
    pub fn proposed_event(&self, reviewer: PrincipalId, format: GitHashAlgorithm) -> Result<ForgeEvent, RefusalCode> {
        ...
        let event = NativeReviewEvent { reviewer, subject: self.subject.clone(), decision: self.decision, reason: self.reason.clone() };
...
/// Compare-and-replace the current reviewer's vote. The immutable old event
/// remains in history. A withdrawal names the exact old subject, including its
/// policy epoch, and may therefore withdraw a stale decision after PR closure.
    ...
    match previous {
        ...
        Some(previous) => {
            ...
            if review.decision == ReviewDecision::Withdraw
                && (old.decision == ReviewDecision::Withdraw || old.subject != review.subject)
            { return Err(RefusalCode::EvidenceStale); }
            Ok(())
        }
    }
...
/// Derived applicability, never a permission or proof of reviewer independence.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
