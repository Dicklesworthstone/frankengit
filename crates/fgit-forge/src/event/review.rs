...
pub mod candidate;
pub use candidate::{CANDIDATE_REVIEW_PROFILE, CandidateBinding, CandidateReviewCommand};
...
impl NativeReviewEvent {
    pub fn validate(&self) -> Result<(), CodecRefusal> {
        ...
        if let Some(candidate) = self.candidate {
            candidate.validate(&self.subject)?;
        }
        if self.reason.len() > MAX_REVIEW_REASON_BYTES
    ...
    pub(super) fn write(&self, out: &mut Encoder) -> Result<(), CodecRefusal> {
        self.validate()?;
        out.write_scalar(if self.candidate.is_some() {
            2_u32
        } else {
            1_u32
        });
        ...
        out.write_scalar(match self.decision {
            ReviewDecision::Approve => 1_u32,
            ReviewDecision::RequestChanges => 2,
            ReviewDecision::Withdraw => 3,
        });
    ...
    pub(super) fn read(input: &mut Decoder<'_>) -> Result<Self, CodecRefusal> {
        ...
        let reviewer = PrincipalId::from_bytes(
            input
                .read_bytes("reviewer")?
                .try_into()
        ...
        let pull_request =
            PullRequestNumber::try_new(input.read_scalar::<u64>("review.pull_request")?)
                .ok_or_else(|| invalid_native("review.pull_request"))?;
        let pull_request_version =
            AggregateVersion::try_new(input.read_scalar::<u64>("review.pull_request_version")?)
        ...
        let target_ref =
            RefName::try_new(input.read_bytes("target_ref")?).map_err(CodecRefusal::from)?;
        let source_tip = input.read_git_oid()?;
        let target_tip = input.read_git_oid()?;
        let policy_epoch = PolicyEpoch::try_new(input.read_scalar::<u64>("review.policy_epoch")?)
            .map_err(CodecRefusal::from)?;
        let offset = input.offset();
        let decision = match input.read_scalar::<u32>("review.decision")? {
            1 => ReviewDecision::Approve,
            2 => ReviewDecision::RequestChanges,
            3 => ReviewDecision::Withdraw,
            observed => {
                return Err(CodecRefusal::VariantUnknown {
        ...
        if bytes.len() > MAX_REVIEW_REASON_BYTES {
            return Err(invalid_native("review.reason_limit"));
        }
        let reason = std::str::from_utf8(bytes)
            .map_err(|_| invalid_native("review.reason_utf8"))?
            .to_owned();
        let candidate = if profile == 2 {
            Some(CandidateBinding::read(input)?)
        } else {
            None
        };
        let event = Self {
...
impl ReviewCommand {
    pub fn proposed_event(
        ...
        if self.subject.source_tip.algorithm() != format
            || self.reason.len() > MAX_REVIEW_REASON_BYTES
            || (self.decision == ReviewDecision::Withdraw
                && self.expected_version == ExpectedVersion::NewStream)
        {
            return Err(RefusalCode::EvidenceInvalid);
        }
        let event = NativeReviewEvent {
            reviewer,
        ...
        Ok(ForgeEvent {
            ...
            version,
            payload: ForgeEventPayload::PullRequestReviewedNative(event),
        })
    }
}
...
pub fn validate_review_transition(
    ...
    let ForgeEventPayload::PullRequestReviewedNative(review) = &next.payload else {
        return Err(RefusalCode::EvidenceInvalid);
    };
    review
        .validate()
        .map_err(|_| RefusalCode::EvidenceInvalid)?;
    if review.aggregate() != next.aggregate {
        return Err(RefusalCode::EvidenceInvalid);
    }
    match previous {
        None if next.version == AggregateVersion::FIRST
            && review.decision != ReviewDecision::Withdraw =>
        {
            Ok(())
        }
        ...
        Some(previous) => {
            ...
            let ForgeEventPayload::PullRequestReviewedNative(old) = &previous.payload else {
                return Err(RefusalCode::EvidenceInvalid);
            };
            old.validate().map_err(|_| RefusalCode::EvidenceInvalid)?;
            if old.aggregate() != previous.aggregate {
                return Err(RefusalCode::EvidenceInvalid);
            }
            if review.decision == ReviewDecision::Withdraw
                && (old.decision == ReviewDecision::Withdraw
                    || old.subject != review.subject
                    || old.candidate != review.candidate)
            {
                return Err(RefusalCode::EvidenceStale);
            }
            Ok(())
...
pub fn review_freshness(
    ...
    source_tip: Option<GitOid>,
    target_tip: Option<GitOid>,
) -> ReviewFreshness {
    if review.decision == ReviewDecision::Withdraw {
        return ReviewFreshness::Withdrawn;
    }
    ...
    if current.version != subject.pull_request_version
    ...
    if policy_epoch != subject.policy_epoch {
        return ReviewFreshness::PolicyChanged;
    }
    if source_tip != Some(subject.source_tip) {
        return ReviewFreshness::SourceMoved;
    }
    if target_tip != Some(subject.target_tip) {
        return ReviewFreshness::TargetMoved;
    }
