...
impl PullRequestData {
    ...
    pub fn matches_merge(&self, merge: &NativeMerge) -> bool {
        self.source_ref == merge.source_ref
            && self.target_ref == merge.target_ref
            && self.source_tip == merge.source_tip
            && self.target_tip == merge.target_tip_before
    }
}
...
fn bounded_text(bytes: &[u8], maximum: usize) -> Result<String, CodecRefusal> {
    if bytes.len() > maximum {
        return Err(invalid_native("pull_request.text_limit"));
    }
...
impl PullRequestCommand {
    ...
    pub fn proposed_event(
        ...
        if self.data.source_tip.algorithm() != format {
        ...
        if (self.action == PullRequestAction::Open)
            != (self.expected_version == ExpectedVersion::NewStream)
        {
            return Err(RefusalCode::EvidenceInvalid);
        }
        Ok(ForgeEvent {
            aggregate: AggregateId::PullRequest(self.number),
            version,
...
pub fn validate_transition(
    ...
    change
        ...
        .validate()
        .map_err(|_| RefusalCode::EvidenceInvalid)?;
    if !matches!(next.aggregate, AggregateId::PullRequest(_)) {
        return Err(RefusalCode::EvidenceInvalid);
    }
    let Some(previous) = previous else {
        return if change.action == PullRequestAction::Open
            && next.version == AggregateVersion::FIRST
        {
            Ok(())
        } else {
    ...
    if old.data.source_ref != change.data.source_ref
        || old.data.target_ref != change.data.target_ref
    {
        return Err(RefusalCode::ProtectedRefTransitionDenied);
    }
...
mod tests {
    ...
    fn command(format: GitHashAlgorithm) -> PullRequestCommand {
        ...
        PullRequestCommand {
            ...
            data: PullRequestData {
                ...
                title: "Native PR".into(),
                body: "untrusted <script>\nUnicode: é".into(),
            },
        }
    }
    ...
    fn semantic_fields_change_event_bytes_and_actor_is_not_a_command_field() {
        ...
        tip.data.source_tip = original.data.target_tip;
        let mut number = original.clone();
        number.number = PullRequestNumber::try_new(2).unwrap();
    ...
    fn stale_versions_retargeting_and_terminal_resurrection_refuse() {
        ...
        command.data = match &updated.payload {
            ForgeEventPayload::PullRequestChangedNative(e) => e.data.clone(),
            _ => unreachable!(),
        };
    ...
    fn bounds_domains_and_command_shape_refuse_before_publication() {
        ...
        huge.data.body = "x".repeat(MAX_BODY_BYTES + 1);
        let mut blank = valid.clone();
        blank.data.title = " \t".into();
        let mut mixed = valid.clone();
        mixed.data.source_tip =
            GitOid::from_hex(GitHashAlgorithm::Sha256, &"a".repeat(64)).unwrap();
        let mut nul = valid.clone();
        nul.data.body.push('\0');
        let mut wrong = valid.clone();
        wrong.action = PullRequestAction::Close;
