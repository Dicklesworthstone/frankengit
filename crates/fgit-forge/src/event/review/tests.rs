...
fn decisions_roundtrip_both_hash_formats_and_keep_historical_pr_bytes() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for decision in [
            ...
            if let Some(prior) = &previous {
                command.expected_version = ExpectedVersion::Exactly(prior.version);
            }
    ...
    assert_eq!(
        canonical_body_bytes(&old).unwrap(),
        [
...
fn every_semantic_input_and_reviewer_changes_the_event_identity_input() {
    ...
    let bytes = encode_body(
        &original
            .proposed_event(actor(), GitHashAlgorithm::Sha1)
            .unwrap(),
    )
    .unwrap();
    for field in 0..10 {
    ...
    assert_ne!(
        encode_body(
            &original
                .proposed_event(PrincipalId::from_bytes([8; 16]), GitHashAlgorithm::Sha1)
                .unwrap()
        )
        .unwrap(),
        bytes
...
fn reviewer_streams_are_separate_bounded_and_non_spoofable() {
    ...
    if let ForgeEventPayload::PullRequestReviewedNative(review) = &mut forged.payload {
        review.reviewer = PrincipalId::from_bytes([8; 16]);
    }
    assert!(encode_body(&forged).is_err());
    forged = first.clone();
    forged.aggregate = AggregateId::PullRequest(command.subject.pull_request);
    assert!(encode_body(&forged).is_err());
    forged = first;
    forged.payload = ForgeEventPayload::PullRequestClosed { withdrawn: false };
    assert!(encode_body(&forged).is_err());
    let label = AggregateId::PullRequestReview {
        pull_request: PullRequestNumber::try_new(u64::MAX).unwrap(),
        reviewer: actor(),
    }
    .to_string();
...
fn withdrawals_require_the_exact_previous_vote_and_explicit_reason() {
    ...
    let original = command
        .proposed_event(actor(), GitHashAlgorithm::Sha256)
        .unwrap();
...
fn freshness_binds_pr_version_both_refs_and_policy_without_erasing_history() {
    let event = command(GitHashAlgorithm::Sha1)
        .proposed_event(actor(), GitHashAlgorithm::Sha1)
        .unwrap();
    ...
    assert_eq!(
        review_freshness(review, Some(&pr), epoch, source, target),
        ReviewFreshness::Current
    );
    assert_eq!(
        review_freshness(review, None, epoch, source, target),
        ReviewFreshness::PullRequestUnavailable
    );
    assert_eq!(
        review_freshness(
            review,
        ...
        ReviewFreshness::PolicyChanged
    );
    assert_eq!(
        review_freshness(review, Some(&pr), epoch, None, target),
        ReviewFreshness::SourceMoved
    );
    assert_eq!(
        review_freshness(review, Some(&pr), epoch, source, None),
        ReviewFreshness::TargetMoved
    );
    let mut changed = pr.clone();
    changed.version = changed.version.next().unwrap();
    assert_eq!(
        review_freshness(review, Some(&changed), epoch, source, target),
        ReviewFreshness::PullRequestChanged
    );
    changed = pr;
    if let ForgeEventPayload::PullRequestChangedNative(change) = &mut changed.payload {
        change.action = PullRequestAction::Close;
    }
    assert_eq!(
        review_freshness(review, Some(&changed), epoch, source, target),
        ReviewFreshness::PullRequestClosed
    );
    let mut withdrawn = review.clone();
    withdrawn.decision = ReviewDecision::Withdraw;
    assert_eq!(
        review_freshness(&withdrawn, Some(&changed), epoch, None, None),
...
fn malformed_text_domains_and_exhausted_versions_refuse() {
    ...
    for field in 0..6 {
        ...
        match field {
            ...
            1 => bad.reason.push('\0'),
            2 => {
                bad.decision = ReviewDecision::RequestChanges;
                bad.reason = " \t\n".into();
            }
            3 => {
        ...
        assert!(
            bad.proposed_event(actor(), GitHashAlgorithm::Sha1).is_err(),
            "invalid field {field}"
        );
    }
    assert!(
...
fn truncated_frames_and_unknown_review_profiles_never_decode_as_approvals() {
    let event = command(GitHashAlgorithm::Sha256)
        .proposed_event(actor(), GitHashAlgorithm::Sha256)
        .unwrap();
    let bytes = encode_body(&event).unwrap();
    for end in 0..bytes.len() {
        assert!(decode_body::<ForgeEvent>(&bytes[..end], DecodeLimits::DEFAULT).is_err());
    }
    ...
    bad[at + 48..at + 52].copy_from_slice(&99_u32.to_be_bytes());
    assert!(matches!(
        decode_body::<ForgeEvent>(&bad, DecodeLimits::DEFAULT),
        Err(CodecRefusal::VariantUnknown {
            field: "review.profile",
            ..
        })
    ));
    assert_eq!(
