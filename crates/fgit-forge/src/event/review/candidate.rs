...
impl CandidateBinding {
    ...
    pub fn merge(&self, subject: &ReviewSubject) -> super::super::NativeMerge {
        super::super::NativeMerge {
            ...
            target_tip_before: subject.target_tip,
            base_tip: self.merge_base,
    ...
    pub(super) fn read(input: &mut Decoder<'_>) -> Result<Self, CodecRefusal> {
        Ok(Self {
            merge_base: input.read_git_oid()?,
            commit: input.read_git_oid()?,
        })
    }
}
...
mod tests {
    ...
    fn command(format: GitHashAlgorithm) -> CandidateReviewCommand {
        let oid =
            |digit: &str| GitOid::from_hex(format, &digit.repeat(format.digest_len() * 2)).unwrap();
        CandidateReviewCommand {
            review: ReviewCommand {
                ...
                reason: "Reviewed actual merge bytes".into(),
            },
    ...
    fn both_profiles_roundtrip_and_candidates_are_identity_material() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            ...
            let old = encode_body(&source_only).unwrap();
            let exact = encode_body(&candidate).unwrap();
            assert_ne!(old, exact);
            for (event, bytes) in [(source_only, old), (candidate, exact.clone())] {
                ...
                assert_eq!(encode_body(&decoded).unwrap(), bytes);
            }
            for field in 0..2 {
                let mut changed = command.clone();
                let other = GitOid::from_hex(format, &"e".repeat(format.digest_len() * 2)).unwrap();
                if field == 0 {
                    changed.candidate.commit = other;
                } else {
                    changed.candidate.merge_base = other;
                }
                assert_ne!(
                    encode_body(&changed.proposed_event(actor, format).unwrap()).unwrap(),
                    exact
                );
            }
            for end in 0..exact.len() {
                assert!(decode_body::<ForgeEvent>(&exact[..end], DecodeLimits::DEFAULT).is_err());
            }
        }
    }
    #[test]
    fn source_only_or_different_candidate_cannot_withdraw_an_exact_vote() {
        ...
        let old = command.proposed_event(actor, format).unwrap();
        command.review.expected_version = ExpectedVersion::Exactly(old.version);
        command.review.decision = ReviewDecision::Withdraw;
        command.review.reason = "withdraw exact vote".into();
        let exact = command.proposed_event(actor, format).unwrap();
        validate_review_transition(Some(&old), &exact).unwrap();
        assert!(
            validate_review_transition(
                Some(&old),
                &command.review.proposed_event(actor, format).unwrap()
            )
            .is_err()
        );
        command.candidate.commit = GitOid::from_hex(format, &"e".repeat(64)).unwrap();
        assert!(
            validate_review_transition(Some(&old), &command.proposed_event(actor, format).unwrap())
                .is_err()
        );
    }
    #[test]
    fn candidate_domains_and_parent_aliases_are_rejected() {
        ...
        command.candidate.commit =
            GitOid::from_hex(GitHashAlgorithm::Sha256, &"d".repeat(64)).unwrap();
        assert!(
            command
                .proposed_event(PrincipalId::from_bytes([1; 16]), GitHashAlgorithm::Sha1)
                .is_err()
