...
use super::{HistoryError, HistoryLimits, HistoryPage, HistorySource, LogOptions,
    charge, check_oid, graph, record, valid_path};
...
impl PathLogOptions {
    pub fn validate(&self) -> Result<(), HistoryError> {
        self.log.validate()?;
        if !valid_path(&self.path) { return Err(HistoryError::InvalidOptions); }
        Ok(())
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct Entry { mode: u32, oid: GitOid }
...
impl<S: HistorySource> Paths<'_, S> {
    fn select(&mut self, root: GitOid) -> Result<Option<Entry>, HistoryError> {
        ...
        if let Some(entry) = self.roots.get(&root) { return Ok(*entry); }
        ...
        charge(&mut self.cached, std::mem::size_of::<(GitOid, Option<Entry>)>(),
            self.limits.max_cached_bytes, "path cache")?;
        let mut tree = root;
        ...
        while let Some(component) = components.next() {
            ...
            for entry in &entries {
                ...
                if entry.name.as_slice() == component { matching = Some(Entry { mode: entry.mode, oid: entry.oid }); }
            }
            let Some(entry) = matching else { break; };
            if components.peek().is_none() { selected = Some(entry); break; }
            if entry.mode != 0o040000 { break; }
            tree = entry.oid;
        }
        self.source.checkpoint()?;
...
pub fn path_history(source: &impl HistorySource, format: GitHashAlgorithm,
    tip: GitOid, options: &PathLogOptions,
) -> Result<HistoryPage, HistoryError> {
    options.validate()?;
    let log = options.log;
    let graph = graph(source, format, tip, log.limits)?;
    let mut paths = Paths { source, format, path: &options.path, limits: log.limits,
        roots: BTreeMap::new(), entries: 0, cached: 0 };
    ...
    for id in &graph.order {
        ...
        for parent in &commit.parents {
            source.checkpoint()?;
            if current != paths.select(graph.commits[parent].tree)? { changed = true; }
        }
        if changed { matches.push(*id); }
    }
    if log.after > matches.len() { return Err(HistoryError::LineRange); }
    let end = log.after.checked_add(log.limit).ok_or(HistoryError::InvalidOptions)?.min(matches.len());
    let mut commits = Vec::with_capacity(end - log.after);
    let mut metadata_bytes = 0;
    for id in &matches[log.after..end] {
        commits.push(record(source, &graph, *id, &mut metadata_bytes, log.limits.max_metadata_bytes)?);
    }
    source.checkpoint()?;
    Ok(HistoryPage { tip, total_commits: matches.len(), after: log.after,
        next_after: (end < matches.len()).then_some(end), commits })
}
