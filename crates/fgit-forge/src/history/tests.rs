...
impl Source {
    ...
    fn add_commit(&mut self, body: &[u8], parents: &[GitOid], label: &str) -> GitOid {
        ...
        let tree = self.add_tree(vec![MergeEntry {
            name: b"file".to_vec(),
            mode: 0o100644,
            oid: blob,
        }]);
...
fn both_formats_attribute_through_both_parents_and_verify_every_original_byte() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for line in &result.lines {
            ...
            assert_eq!(
                &result.content[line.byte_start..line.byte_end],
...
fn shifted_lines_ranges_empty_files_and_missing_final_newlines_remain_exact() {
    ...
    let mut range = options();
    range.first_line = 1;
    range.end_line = Some(3);
    let result = blame(&source, source.format, next, &range).unwrap();
    assert_eq!(result.content, b"same\nlast");
    assert_eq!(result.content_byte_start, 9);
    assert_eq!(
        result
            .lines
            .iter()
            .map(|line| (line.line, line.origin_line))
            .collect::<Vec<_>>(),
        vec![(1, 0), (2, 1)]
    );
    assert!(result.lines.iter().all(|line| line.origin_commit == base));
    range.end_line = Some(4);
    assert_eq!(
        blame(&source, source.format, next, &range),
...
fn missing_corrupt_and_binary_sources_never_become_attribution_boundaries() {
    ...
    assert!(
        matches!(blame(&source, source.format, merge, &options()), Err(HistoryError::Source(MergeSourceError::Unavailable(id))) if id == base)
    );
    let mut source = Source::new(GitHashAlgorithm::Sha256);
    ...
    assert_eq!(
        blame(&source, source.format, tip, &options()),
        Err(HistoryError::InvalidObject(blob))
    );
    let binary = source.add_commit(b"nul\0data", &[], "binary");
    assert_eq!(
        blame(&source, source.format, binary, &options()),
        Err(HistoryError::BinaryContent)
    );
    source.commits.get_mut(&tip).unwrap().1.push(b'!');
    assert_eq!(
        commit_history(&source, source.format, tip, LogOptions::default()),
        Err(HistoryError::InvalidObject(tip))
    );
}
...
fn every_bound_and_cancellation_stop_without_a_partial_success() {
    ...
    limited.limits.max_commits = 3;
    assert_eq!(
        blame(&source, source.format, tip, &limited),
        Err(HistoryError::Budget("commits"))
    );
    limited = options();
    limited.limits.max_lines = 2;
    assert_eq!(
        blame(&source, source.format, tip, &limited),
        Err(HistoryError::Budget("line count"))
    );
    limited = options();
    limited.limits.max_comparisons = 1;
    assert_eq!(
        blame(&source, source.format, tip, &limited),
        Err(HistoryError::Budget("line comparisons"))
    );
    limited = options();
    limited.limits.max_cached_bytes = 1;
    assert_eq!(
        blame(&source, source.format, tip, &limited),
        Err(HistoryError::Budget("blob cache"))
    );
...
fn tree_identity_reuse_skips_content_diffs_and_symlinks_are_not_followed() {
    ...
    assert_eq!(
        blame(&source, source.format, tip, &options()),
        Err(HistoryError::PathUnavailable)
    );
}
...
fn cyclic_input_is_explicitly_refused_and_raw_nested_names_are_preserved() {
    ...
    assert_eq!(
        commit_history(&source, source.format, tip, LogOptions::default()),
        Err(HistoryError::CyclicHistory)
    );
    source.commits.get_mut(&tip).unwrap().0.parents.clear();
    let blob = source.add_blob(b"raw\xff\r\n");
    let child = source.add_tree(vec![MergeEntry {
        name: vec![255],
        mode: 0o100755,
        oid: blob,
    }]);
    let root = source.add_tree(vec![MergeEntry {
        name: b"dir".to_vec(),
        mode: 0o040000,
        oid: child,
    }]);
    ...
    opts.path = b"dir/\xff".to_vec();
    let result = blame(&source, source.format, raw, &opts).unwrap();
    assert_eq!(result.path, opts.path);
    assert_eq!(result.content, b"raw\xff\r\n");
    for path in [
        b"../file".to_vec(),
        b"dir//file".to_vec(),
        b"/file".to_vec(),
        ...
        opts.path = path;
        assert_eq!(
            blame(&source, source.format, raw, &opts),
