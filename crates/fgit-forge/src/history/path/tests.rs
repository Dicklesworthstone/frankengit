...
impl Source {
    fn new(format: GitHashAlgorithm) -> Self {
        Self { format, commits: BTreeMap::new(), trees: BTreeMap::new(),
            tree_reads: Cell::new(0), checkpoints: Cell::new(0), stop_at: Cell::new(usize::MAX) }
    }
    fn entry(&self, name: &[u8], mode: u32, body: &[u8]) -> MergeEntry {
        MergeEntry { name: name.to_vec(), mode, oid: git_object_id(self.format, GitObjectKind::Blob, body) }
    }
    fn add_tree(&mut self, mut entries: Vec<MergeEntry>) -> GitOid {
        entries.sort_by_key(|entry| { let mut name = entry.name.clone();
            name.push(if entry.mode == 0o040000 { b'/' } else { 0 }); name });
        let mut body = Vec::new();
        for entry in &entries {
            body.extend_from_slice(format!("{:o} ", entry.mode).as_bytes());
            body.extend_from_slice(&entry.name); body.push(0); body.extend_from_slice(entry.oid.as_bytes());
        }
...
impl MergeObjectSource for Source {
    ...
    fn commit(&self, id: GitOid) -> Result<CommitInput, MergeSourceError> {
        self.commits.get(&id).map(|row| row.0.clone()).ok_or(MergeSourceError::Unavailable(id))
    }
...
impl HistorySource for Source {
    fn commit_body(&self, id: GitOid) -> Result<Vec<u8>, MergeSourceError> {
        self.commits.get(&id).map(|row| row.1.clone()).ok_or(MergeSourceError::Unavailable(id))
    }
}
fn options(path: &[u8]) -> PathLogOptions { PathLogOptions { path: path.to_vec(), log: LogOptions::default() } }
fn ids(page: &HistoryPage) -> Vec<GitOid> { page.commits.iter().map(|row| row.id).collect() }
...
fn both_formats_filter_before_paging_and_track_mode_deletion_and_recreation() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(ids(&read(&source, deletion, b"file")), vec![deletion, mode, root]);
        ...
        let mut query = options(b"file"); query.log.limit = 2;
        let first = path_history(&source, format, tip, &query).unwrap();
        assert_eq!(first.total_commits, 4); assert_eq!(first.next_after, Some(2));
        query.log.after = 2;
        let last = path_history(&source, format, tip, &query).unwrap();
        assert_eq!(ids(&last), vec![mode, root]); assert_eq!(last.next_after, None);
        query.log.after = 4;
        assert!(path_history(&source, format, tip, &query).unwrap().commits.is_empty());
        query.log.after = 5;
        assert_eq!(path_history(&source, format, tip, &query), Err(HistoryError::LineRange));
...
fn merges_compare_every_parent_and_keep_native_id_topological_ties() {
    ...
    assert_eq!(ids(&result), vec![merge, left.min(right), left.max(right), base]);
...
fn directories_raw_names_and_component_boundaries_are_exact() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let root_tree = source.add_tree(vec![MergeEntry { name: b"dir".to_vec(), mode: 0o040000, oid: leaf }]);
        let root = source.add_commit(root_tree, &[], "root");
        let leaf2 = source.add_tree(vec![source.entry(b"\xff", 0o100644, b"v2")]);
        let changed_tree = source.add_tree(vec![MergeEntry { name: b"dir".to_vec(), mode: 0o040000, oid: leaf2 }]);
        let changed = source.add_commit(changed_tree, &[root], "nested change");
        assert_eq!(ids(&read(&source, changed, b"dir")), vec![changed, root]);
        assert_eq!(ids(&read(&source, changed, b"dir/\xff")), vec![changed, root]);
...
fn symlinks_gitlinks_and_type_changes_are_opaque_and_never_traversed() {
    ...
    let foreign_commit = git_object_id(source.format, GitObjectKind::Commit, b"foreign repository commit");
    let module = source.add_tree(vec![MergeEntry { name: b"path".to_vec(), mode: 0o160000, oid: foreign_commit }]);
    let gitlink = source.add_commit(module, &[symlink], "submodule");
    assert_eq!(ids(&read(&source, gitlink, b"path")), vec![gitlink, symlink, root]);
...
fn a_never_present_path_is_a_complete_empty_result_but_missing_trees_are_not() {
    ...
    assert_eq!(page.tip, root); assert_eq!(page.total_commits, 0);
    assert_eq!(page.next_after, None); assert!(page.commits.is_empty());
    source.trees.remove(&tree);
    assert_eq!(path_history(&source, source.format, root, &options(b"absent")),
...
fn identical_root_trees_are_read_once_even_for_nonmatching_commits() {
    ...
    let root = source.add_commit(tree, &[], "root");
    let mut tip = root;
    for n in 0..30 { tip = source.add_commit(tree, &[tip], &format!("noop {n}")); }
...
fn limits_cover_the_entire_graph_and_all_tree_work_not_just_output_matches() {
    let mut source = Source::new(GitHashAlgorithm::Sha1);
    let tree = source.add_tree(vec![source.entry(b"a", 0o100644, b"a"), source.entry(b"b", 0o100644, b"b")]);
    let root = source.add_commit(tree, &[], "root");
    let middle = source.add_commit(tree, &[root], "middle");
    let tip = source.add_commit(tree, &[middle], "tip");
    let mut query = options(b"absent"); query.log.limits.max_commits = 2;
    assert_eq!(path_history(&source, source.format, tip, &query), Err(HistoryError::Budget("commits")));
    query = options(b"absent"); query.log.limits.max_edges = 1;
    assert_eq!(path_history(&source, source.format, tip, &query), Err(HistoryError::Budget("commit edges")));
    query = options(b"absent"); query.log.limits.max_tree_entries = 1;
    assert_eq!(path_history(&source, source.format, tip, &query), Err(HistoryError::Budget("tree entries")));
    query = options(b"absent"); query.log.limits.max_cached_bytes = 1;
    assert_eq!(path_history(&source, source.format, tip, &query), Err(HistoryError::Budget("path cache")));
    query = options(b"a"); query.log.limits.max_metadata_bytes = 1;
    assert_eq!(path_history(&source, source.format, tip, &query), Err(HistoryError::Budget("metadata bytes")));
}
...
fn every_checkpoint_cancels_without_returning_a_partial_page() {
    ...
    for stop in 1..=checkpoints {
        ...
        assert_eq!(path_history(&source, source.format, tip, &options(b"a")),
            Err(HistoryError::Source(MergeSourceError::Cancelled)), "checkpoint {stop}");
    }
    source.checkpoints.set(0); source.stop_at.set(usize::MAX);
...
fn malformed_paths_and_cross_format_tips_refuse_before_reading_objects() {
    ...
    for path in [vec![], b"/a".to_vec(), b"a/".to_vec(), b"a//b".to_vec(), b"a/../b".to_vec(),
        b"a/./b".to_vec(), b"a\0b".to_vec(), vec![b'a'; 4097], b"a/".repeat(64)] {
        assert_eq!(path_history(&source, source.format, tip, &options(&path)), Err(HistoryError::InvalidOptions));
    }
    assert_eq!(source.checkpoints.get(), 0);
    let other = git_object_id(GitHashAlgorithm::Sha256, GitObjectKind::Commit, b"tip");
    assert_eq!(path_history(&source, source.format, other, &options(b"a")), Err(HistoryError::InvalidObject(other)));
    assert_eq!(source.checkpoints.get(), 0);
}
...
fn malformed_tree_entries_missing_ancestors_and_corrupt_records_fail_closed() {
    ...
    assert_eq!(path_history(&source, source.format, root, &options(b"a")), Err(HistoryError::InvalidTree));
    source.trees.insert(tree, vec![MergeEntry { mode: 0o100600, ..entry.clone() }]);
    assert_eq!(path_history(&source, source.format, root, &options(b"a")), Err(HistoryError::InvalidTree));
    let other = git_object_id(GitHashAlgorithm::Sha256, GitObjectKind::Blob, b"a");
    source.trees.insert(tree, vec![MergeEntry { oid: other, ..entry.clone() }]);
    assert_eq!(path_history(&source, source.format, root, &options(b"a")), Err(HistoryError::InvalidObject(other)));
    source.trees.insert(tree, vec![entry]);
    let tip = source.add_commit(tree, &[root], "tip");
    source.commits.get_mut(&root).unwrap().1.push(b'x');
    assert_eq!(path_history(&source, source.format, tip, &options(b"a")), Err(HistoryError::InvalidObject(root)));
