...
use fgit_forge::review::{
    ComparisonMode, MAX_SERIES_COMPARISONS, ReviewError, ReviewOptions, compare_source,
    compare_source_series,
};
...
impl MergeObjectSource for Source {
    fn checkpoint(&self) -> Result<(), MergeSourceError> {
        ...
        if self.stop_at.get() == Some(next) {
            return Err(if self.resource_stop.get() {
                MergeSourceError::BudgetExceeded
            } else {
...
fn every_pair_matches_single_review_including_transient_changes() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let expected = pairs
            .iter()
            .map(|&(old, new)| compare_source(&source, format, old, new, &options).unwrap())
            .collect::<Vec<_>>();
        ...
        assert_eq!(
            ...
            1,
            "the transient change must not disappear with the final net diff"
        );
    }
}
...
fn every_cumulative_budget_is_shared_and_exact_boundaries_succeed() {
    ...
    assert_eq!(
        compare_source_series(&source, format, &pairs, &exact)
            .unwrap()
            .len(),
        2
    );
    for axis in 0..5 {
        ...
        *field -= 1;
        ...
        for &(old, new) in &pairs {
            compare_source(&source, format, old, new, &options).unwrap();
        }
        assert!(
            matches!(compare_source_series(&source, format, &pairs, &options), Err(ReviewError::Budget(reason)) if reason == expected)
        );
    }
    let mut one = exact;
    one.limits.max_changes = 1;
    assert_eq!(
        compare_source_series(&source, format, &[(a, b), (b, b)], &one)
            .unwrap()
            .len(),
        2
    );
...
fn every_observed_stop_discards_the_prefix_and_preserves_one_shot_cause() {
    ...
    for resource in [false, true] {
        ...
        for at in 1..=calls {
            ...
            assert!(
                matches!(result, Err(ReviewError::Source(ref error)) if *error ==
                if resource { MergeSourceError::BudgetExceeded } else { MergeSourceError::Cancelled }),
    ...
    assert_eq!(
        compare_source_series(&source, format, &pairs, &options).unwrap(),
        good
    );
}
...
fn shape_and_series_bounds_refuse_before_retrieval_and_late_source_failure_is_not_partial() {
    ...
    assert!(matches!(
        compare_source_series(&source, format, &pairs, &options),
        Err(ReviewError::Budget("series comparisons"))
    ));
    assert_eq!(source.calls.get(), 0);
    let mut invalid = options.clone();
    invalid.mode = ComparisonMode::MergeBase;
    assert!(matches!(
        compare_source_series(&source, format, &[(a, b)], &invalid),
        Err(ReviewError::InvalidOptions)
    ));
    assert_eq!(source.calls.get(), 0);
    source.commits.remove(&c);
    assert!(
        matches!(compare_source_series(&source, format, &[(a, b), (b, c)], &options), Err(ReviewError::Source(MergeSourceError::Unavailable(id))) if id == c)
    );
    assert!(
        compare_source_series(&source, format, &[], &options)
            .unwrap()
            .is_empty()
    );
