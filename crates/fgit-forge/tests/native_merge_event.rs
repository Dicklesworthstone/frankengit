...
use fgit_codec::{
    CanonicalBody, CryptoBodyIdentity, DecodeLimits, Encoder, decode_body, encode_body,
};
...
fn native(format: GitHashAlgorithm) -> NativeMerge {
    NativeMerge {
        ...
        base_tip: oid(format, "22"),
        target_ref: RefName::try_new(b"refs/heads/main").unwrap(),
        target_tip_before: oid(format, "33"),
...
fn native_merge_round_trips_both_formats_without_internal_digest_conversion() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(
            decode_body::<ForgeEventBatch>(&encode_body(&batch).unwrap(), DecodeLimits::DEFAULT)
                .unwrap(),
            batch
        );
    }
...
fn native_event_refuses_mixed_formats_zero_identities_and_non_transitions() {
    ...
    mixed.source_tip = oid(GitHashAlgorithm::Sha256, "11");
    let mut zero = original.clone();
    zero.base_tip = oid(GitHashAlgorithm::Sha1, "00");
    let mut unchanged = original.clone();
    unchanged.merge_commit = unchanged.target_tip_before;
    let mut same_branch = original.clone();
...
fn legacy_merge_payload_encoding_is_unchanged_and_native_projection_keeps_domains_distinct() {
    let legacy = ForgeEvent {
        ...
        payload: ForgeEventPayload::MergeCommitted {
            merge_commit: digest(4),
            target_ref: b"refs/heads/main".to_vec(),
            target_tip_before: digest(3),
            target_tip_after: digest(4),
        },
    };
    ...
    expected
        .write_bytes("target_ref", b"refs/heads/main")
        .unwrap();
    expected.write_digest(&digest(3)).unwrap();
    expected.write_digest(&digest(4)).unwrap();
    let mut actual = Encoder::new();
    legacy.write_payload(&mut actual).unwrap();
