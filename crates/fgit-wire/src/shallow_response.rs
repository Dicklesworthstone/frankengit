...
pub(super) fn response(
    ...
    for (values, removing) in [(&update.shallow, false), (&update.unshallow, true)] {
        ...
        for &id in values {
            ...
            if removing {
                if !old.contains(&id) || shallow.contains(&id) {
                    return Err(WireError::PackSourceRefused);
                }
            } else {
                shallow.insert(id);
    ...
    if legacy && !changes_boundary(request) {
        return Ok(Vec::new());
    }
    ...
    for (keyword, values) in [
        ("shallow", &update.shallow),
        ("unshallow", &update.unshallow),
    ] {
        for &id in values {
