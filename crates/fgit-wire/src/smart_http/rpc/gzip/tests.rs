...
fn stored_fixed_and_dynamic_members_survive_every_fragment_width() {
    ...
    assert_eq!(
        decode(&text, 1, HttpLimits::default()).unwrap(),
        b"hello gzip\n".repeat(8)
    );
}
...
fn corruption_truncation_and_suffixes_never_finish_and_poison_decoder() {
    for cut in 0..FIXED.len() {
        assert!(
            decode(&FIXED[..cut], 1, HttpLimits::default()).is_err(),
            "cut {cut}"
        );
    }
...
fn gateway_envelope_and_git_parser_agree_without_enabling_native_gzip() {
    ...
    for path in [
        ...
        assert!(matches!(
            head::parse(&bytes, HttpLimits::default()),
...
pub(super) const FIXED: &[u8] = &[
    ...
    0x48, 0xaf, 0xca, 0x2c, 0xe0, 0xca, 0xa0, 0x26, 0x13, 0x00, 0x33, 0x77, 0x18, 0x09, 0x58, 0x00,
    0x00, 0x00,
];
const DYNAMIC: &[u8] = &[
    0x1f, 0x8b, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x03, 0xed, 0xca, 0x31, 0x0d, 0x00, 0x30,
    0x08, 0x00, 0xb0, 0x1f, 0x15, 0xb3, 0x80, 0x24, 0x0c, 0x70, 0x2d, 0xc1, 0x3e, 0x16, 0xf6, 0x2e,
    0x69, 0xef, 0x4e, 0xf5, 0x3d, 0xf9, 0x28, 0xc6, 0xb6, 0x6d, 0xdb, 0xb6, 0x6d, 0xdb, 0xb6, 0x6d,
    0xdb, 0xb6, 0x6d, 0xdb, 0xb6, 0x6d, 0xdb, 0xb6, 0xbf, 0xd8, 0x0b, 0xe9, 0x1f, 0xb4, 0x37, 0x00,
    0x17, 0x00, 0x00,
];
