use super::tests::{FIXED, Repository, chunks, crc, decode, http_head, stored};
use super::*;
...
fn decorated(member: &[u8], flags: u8) -> Vec<u8> {
    ...
    if flags & 8 != 0 {
        header.extend_from_slice(b"untrusted-name\0");
    }
    if flags & 16 != 0 {
        header.extend_from_slice(b"opaque\xff-comment\0");
    }
    if flags & 2 != 0 {
...
fn every_optional_header_combination_survives_every_fragment_boundary() {
    for flags in 0..32 {
        ...
        for width in 1..=member.len() {
            assert_eq!(
                decode(&member, width, HttpLimits::default()).unwrap(),
                b"hello gzip\n".repeat(8)
            );
        }
    }
    ...
    assert_eq!(
        decode(&member, 1, HttpLimits::default()).unwrap(),
        b"hello gzip\n".repeat(8)
    );
}
...
fn invalid_magic_method_reserved_flags_and_header_checksums_are_terminal() {
    for (index, value) in [(0, 0), (1, 0), (2, 0), (3, 32), (3, 64), (3, 128)] {
        ...
        member[index] = value;
        assert!(matches!(
            decode(&member, 1, HttpLimits::default()),
    ...
    for index in 0..header_end {
        ...
        bad[index] ^= 1;
        assert!(
            decode(&bad, 1, HttpLimits::default()).is_err(),
            "header byte {index}"
        );
    }
    for cut in 0..member.len() {
        assert!(
            decode(&member[..cut], 1, HttpLimits::default()).is_err(),
            "cut {cut}"
        );
    }
}
...
fn metadata_budget_is_exact_and_never_counts_deflate_or_the_trailer() {
    ...
    let limits = HttpLimits {
        max_head_bytes: header_bytes,
        max_target_bytes: header_bytes,
        ..HttpLimits::default()
    };
    assert_eq!(
        decode(&member, 1, limits).unwrap(),
        b"hello gzip\n".repeat(8)
    );
    let tight = HttpLimits {
        max_head_bytes: header_bytes - 1,
        max_target_bytes: header_bytes - 1,
        ..limits
    };
    assert!(matches!(
        decode(&member, 1, tight),
        Err(RpcError::Http(HttpError::BodyTooLarge))
    ));
    for flag in [4, 8, 16] {
            ...
            for chunked in [false, true] {
                ...
                for width in [1, 3, 19, body.len()] {
                    ...
                    for fragment in body.chunks(width) {
                        assert_eq!(
                            rpc.push(fragment, &mut || true).unwrap().consumed,
                            fragment.len()
                        );
                    }
                    ...
                    if fetch {
                        assert_eq!(
                            reply.pack_request().unwrap().wants,
                            vec![repository.refs[0].oid]
                        );
                    } else {
...
fn verified_gzip_does_not_replace_http_termination_or_hide_a_second_command() {
    ...
    for suffix in [b"".as_slice(), b"0", command.as_slice()] {
        ...
        if suffix.is_empty() {
            ...
            assert!(matches!(
                rpc.finish(&mut || true),
                Err(RpcError::Http(HttpError::TruncatedBody))
            ));
        } else {
...
fn independently_compressed_large_fetch_negotiation_exceeds_gits_gzip_threshold() {
    ...
    assert_eq!(
        decode(LARGE_NEGOTIATION, 1, HttpLimits::default()).unwrap(),
        expected
    );
    for chunked in [false, true] {
        let head = http_head(
            ...
            "gzip",
            LARGE_NEGOTIATION.len(),
            chunked,
        );
        let request = parse_head(&head, HttpLimits::default()).unwrap().unwrap();
        let body = if chunked {
            chunks(LARGE_NEGOTIATION)
        } else {
            LARGE_NEGOTIATION.to_vec()
        };
        for width in [1, 17, body.len()] {
            let capabilities =
                Capabilities::parse_v1(b"multi_ack", &WireLimits::default()).unwrap();
            let mut rpc = UploadRpc::new(
                &request,
                ProtocolVersion::V0,
                capabilities,
...
const LARGE_NEGOTIATION: &[u8] = &[
    ,
];
