...
impl From<ReceiveError> for RpcError {
...
fn body_for(
    ...
    if request.operation != Operation::Rpc(service) {
        return Err(RpcError::WrongOperation);
    }
...
fn append_packets(
    ...
    for packet in packets {
        let count = match packet {
            Packet::Data(data) => data.len().checked_add(4),
            _ => Some(4),
        }
        ...
        let next = output
            ...
            .checked_add(count)
            .ok_or(RpcError::OutputLimit)?;
        if next > limits.max_outbound_bytes {
...
impl<'repo, R: UploadPackRepository> UploadRpc<'repo, R> {
    ...
    pub fn new(
        ...
        let machine = match selected_version {
            ProtocolVersion::V0 | ProtocolVersion::V1 => {
                ...
                UploadMachine::Legacy(
                    LegacyUploadPack::new(version, capabilities, wire_limits.clone())?
    ...
    fn push_inner<C: ReceiveCancellation>(
        ...
        if matches!(self.machine, UploadMachine::Failed) {
    ...
    fn accept_payload<C: ReceiveCancellation>(
        ...
        if self.command_complete {
            return Err(RpcError::MultipleCommands);
        }
        ...
        for fragment in payload.chunks(decoder_chunk_bytes) {
            ...
            for packet in packets {
                ...
                let transition = match &mut self.machine {
                    UploadMachine::Legacy(machine) => {
                        machine.push_packet(&packet, self.repository)?
                    }
    ...
    pub fn finish<C: ReceiveCancellation>(
        ...
        Ok(UploadReply {
            ...
            prefix: self.output,
            pack_request: self.pack_request,
        })
    }
}
...
pub fn upload_discovery(
    ...
    let packets = if version == ProtocolVersion::V2 {
        ...
        crate::validate_advertised_refs(
            repository.advertised_refs(),
            repository.object_format(),
            limits,
        )?;
        let mut refs = Vec::new();
        refs.try_reserve_exact(repository.advertised_refs().len())
            .map_err(|_| WireError::AllocationFailure)?;
        refs.extend_from_slice(repository.advertised_refs());
        let mut advertisement =
            V1Advertisement::new(refs, capabilities, repository.object_format(), limits)?;
...
pub fn receive_discovery(
    ...
    if version == ProtocolVersion::V1 {
        packets.push(Packet::Data(b"version 1\n".to_vec()));
    }
...
fn discovery_body(
    ...
    if prefix.len() > limits.max_outbound_bytes {
        return Err(RpcError::OutputLimit);
    }
