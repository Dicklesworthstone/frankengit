...
enum Phase { Prefix, Pack, Flush, Complete, Failed }
...
impl UploadReply {
    ...
    pub fn into_response<'source, Source: PackPayloadSource + ?Sized>(
        self, source: Option<&'source mut Source>, limits: WireLimits,
        maximum_response_bytes: u64,
        ...
        let maximum_chunk_bytes = limits.max_packet_bytes.checked_sub(5)
            .filter(|count| *count != 0).ok_or(WireError::InvalidLimit {
                field: "max_packet_bytes for HTTP pack response",
            })?;
        if self.prefix.len() > limits.max_outbound_bytes {
            return Err(RpcError::OutputLimit);
        }
        let minimum = (self.prefix.len() as u64).checked_add(if sideband { 4 } else { 0 })
            .ok_or(RpcError::OutputLimit)?;
        if minimum > maximum_response_bytes { return Err(RpcError::OutputLimit); }
        Ok(UploadResponse { prefix: self.prefix, prefix_offset: 0, source,
            phase: Phase::Prefix, limits, maximum_chunk_bytes, maximum_response_bytes,
            emitted_bytes: 0, sideband, wants_pack, saw_pack_bytes: false })
    }
}
...
impl<Source: PackPayloadSource + ?Sized> UploadResponse<'_, Source> {
    ...
    #[must_use]
    pub const fn emitted_bytes(&self) -> u64 { self.emitted_bytes }
    ...
    fn next_inner<C: ReceiveCancellation>(
        ...
        if self.phase == Phase::Failed { return Err(RpcError::FailedRequest); }
        if self.phase == Phase::Complete { return Ok(None); }
        if !cancellation.checkpoint() { return Err(RpcError::Cancelled); }
        if self.phase == Phase::Prefix {
            if self.prefix_offset < self.prefix.len() {
                let count = (self.prefix.len() - self.prefix_offset).min(self.limits.max_packet_bytes);
                self.charge(count, self.sideband)?;
                let end = self.prefix_offset + count;
                let mut output = Vec::new();
                output.try_reserve_exact(count).map_err(|_| WireError::AllocationFailure)?;
                output.extend_from_slice(&self.prefix[self.prefix_offset..end]);
                self.prefix_offset = end;
                if end == self.prefix.len() { self.prefix = Vec::new(); }
                return Ok(Some(output));
            }
            self.prefix = Vec::new();
            self.phase = if self.wants_pack { Phase::Pack } else { Phase::Complete };
        }
        if self.phase == Phase::Pack {
            ...
            if let Some(chunk) = next {
                if chunk.is_empty() { return Err(RpcError::EmptyPackChunk); }
                if chunk.len() > self.maximum_chunk_bytes {
                    return Err(WireError::PackChunkTooLarge {
                        observed: chunk.len(), limit: self.maximum_chunk_bytes,
                    }.into());
                }
                ...
                if self.sideband {
                    ...
                    payload.try_reserve_exact(chunk.len() + 1).map_err(|_| WireError::AllocationFailure)?;
                    payload.push(1); payload.extend(chunk);
            ...
            self.source = None;
            if !self.saw_pack_bytes { return Err(RpcError::MissingPack); }
            self.phase = if self.sideband { Phase::Flush } else { Phase::Complete };
        }
    ...
    fn charge(&mut self, count: usize, reserve_flush: bool) -> Result<(), RpcError> {
        let next = self.emitted_bytes.checked_add(count as u64).ok_or(RpcError::OutputLimit)?;
        let required = next.checked_add(if reserve_flush { 4 } else { 0 }).ok_or(RpcError::OutputLimit)?;
        if required > self.maximum_response_bytes { return Err(RpcError::OutputLimit); }
