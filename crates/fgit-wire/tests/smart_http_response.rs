...
use fgit_wire::smart_http::{
    BodyDecoder, BodyFraming, HttpLimits, HttpVersion, ProtocolVersion, ResponseEncoder, parse_head,
};
use fgit_wire::{
...
fn reply(
    ...
    let repo = Repository {
        format,
        refs: vec![AdvertisedRef::new(oid, b"refs/heads/main", &limits).unwrap()],
    };
    let (capabilities, packets) = if version == ProtocolVersion::V2 {
        ...
        let mut request = vec![
            ...
            Packet::Delimiter,
            data(format!("want {oid}\n")),
        ];
        if done {
            request.push(data(b"done\n"));
        } else {
            request.push(data(format!("have {}\n", "22".repeat(format.digest_len()))));
        }
        request.push(Packet::Flush);
        (caps, request)
    } else {
    ...
    for format in [GitObjectFormat::Sha1, GitObjectFormat::Sha256] {
        for version in [
            ...
            for sideband in [false, true] {
                ...
                let mut expected = reply.prefix().to_vec();
                ...
                while let Some(chunk) = stream.next_chunk(&mut || true).unwrap() {
                    assert!(chunk.len() <= WireLimits::default().max_packet_bytes);
                    output.extend(chunk);
                }
                assert_eq!(output, expected);
                assert_eq!(stream.emitted_bytes(), expected.len() as u64);
...
fn no_pack_source_is_polled_until_consumer_finishes_the_prefix() {
    ...
    let expected_prefix = reply.prefix().to_vec();
    let calls = Cell::new(0);
    let mut source = Source {
        bytes: pack(GitObjectFormat::Sha1),
        offset: 0,
        fragment: 7,
        calls: &calls,
    };
    let mut stream = reply
        .into_response(Some(&mut source), WireLimits::default(), 4096)
        .unwrap();
    assert_eq!(calls.get(), 0);
    assert_eq!(
        stream.next_chunk(&mut || true).unwrap().unwrap(),
        expected_prefix
    );
    assert_eq!(calls.get(), 0);
...
fn negotiation_only_stream_never_accepts_an_unrequested_pack() {
    ...
    assert!(matches!(
        reply(GitObjectFormat::Sha1, ProtocolVersion::V0, false, false).into_response(
            Some(&mut source),
            WireLimits::default(),
            4096
        ),
        Err(RpcError::UnexpectedPack)
    ));
    assert_eq!(calls.get(), 0);
...
fn missing_requested_pack_cannot_be_reported_as_a_complete_response() {
    assert!(matches!(
        reply(GitObjectFormat::Sha1, ProtocolVersion::V0, false, true).into_response(
            None::<&mut Source<'_>>,
            WireLimits::default(),
            4096
        ),
        Err(RpcError::MissingPack)
    ));
    let calls = Cell::new(0);
    ...
    let mut stream = reply(GitObjectFormat::Sha1, ProtocolVersion::V2, true, true)
        .into_response(Some(&mut empty), WireLimits::default(), 4096)
        .unwrap();
    stream.next_chunk(&mut || true).unwrap();
    assert!(matches!(
        stream.next_chunk(&mut || true),
        Err(RpcError::MissingPack)
    ));
    assert!(matches!(
        stream.next_chunk(&mut || true),
...
fn aggregate_response_budget_includes_every_sideband_header_and_final_flush() {
    for format in [GitObjectFormat::Sha1, GitObjectFormat::Sha256] {
        let reply = reply(format, ProtocolVersion::V2, true, true);
        let bytes = pack(format);
        ...
        let mut stream = reply
            .into_response(Some(&mut source), WireLimits::default(), (exact - 1) as u64)
            .unwrap();
...
fn source_errors_cannot_be_followed_by_a_git_success_trailer() {
    ...
    impl PackPayloadSource for Broken {
        fn next_chunk(&mut self, _: usize) -> Result<Option<Vec<u8>>, WireError> {
            Err(WireError::AllocationFailure)
        }
    }
    ...
    assert!(matches!(
        stream.next_chunk(&mut || true),
        Err(RpcError::Wire(WireError::AllocationFailure))
    ));
    assert!(matches!(
        stream.next_chunk(&mut || true),
        Err(RpcError::FailedRequest)
...
fn zero_progress_and_oversized_pack_producers_are_refused() {
    ...
    for oversize in [false, true] {
        ...
        let mut stream = reply(GitObjectFormat::Sha1, ProtocolVersion::V0, true, true)
            .into_response(Some(&mut source), WireLimits::default(), 1_000_000)
            .unwrap();
        stream.next_chunk(&mut || true).unwrap();
        ...
        if oversize {
            assert!(matches!(
                error,
                Err(RpcError::Wire(WireError::PackChunkTooLarge { .. }))
            ));
        } else {
            assert!(matches!(error, Err(RpcError::EmptyPackChunk)));
        }
        assert!(matches!(
            stream.next_chunk(&mut || true),
            Err(RpcError::FailedRequest)
...
fn cancellation_during_pack_production_discards_the_unreleased_chunk() {
    ...
    impl PackPayloadSource for Cancelling<'_> {
        fn next_chunk(&mut self, _: usize) -> Result<Option<Vec<u8>>, WireError> {
            self.0.set(true);
            Ok(Some(b"PACK".to_vec()))
        }
    }
    let cancelled = Cell::new(false);
    let mut source = Cancelling(&cancelled);
    let mut stream = reply(GitObjectFormat::Sha1, ProtocolVersion::V0, true, true)
        .into_response(Some(&mut source), WireLimits::default(), 4096)
        .unwrap();
    stream.next_chunk(&mut || !cancelled.get()).unwrap();
    let before = stream.emitted_bytes();
    assert!(matches!(
        stream.next_chunk(&mut || !cancelled.get()),
        Err(RpcError::Cancelled)
    ));
...
fn native_git_stream_round_trips_through_chunked_http_without_buffering_a_pack() {
    for format in [GitObjectFormat::Sha1, GitObjectFormat::Sha256] {
        let reply = reply(format, ProtocolVersion::V2, true, true);
        let prefix = reply.prefix().to_vec();
        let bytes = pack(format);
        let calls = Cell::new(0);
        let mut source = Source {
            bytes: bytes.clone(),
        ...
        for byte in encoder.finish().unwrap() {
