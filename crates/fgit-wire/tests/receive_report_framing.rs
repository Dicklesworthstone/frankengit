...
use fgit_wire::{
    Capabilities, GitObjectFormat, Packet, PktLineDecoder, SidebandBand, WireError, WireLimits,
    encode_packets, parse_sideband,
};
    ...
    for format in [GitObjectFormat::Sha1, GitObjectFormat::Sha256] {
        for mode in ["report-status", "report-status-v2"] {
            ...
            assert_eq!(
                ...
                Packet::Data(b"\x010000".to_vec())
            );
            assert_eq!(
                request, original,
                "framing must not mutate the semantic request"
            );
            assert_eq!(
                report_status(
                    &request,
...
fn non_sideband_output_stays_byte_exact_without_an_extra_outer_flush() {
    ...
    for mode in ["report-status", "report-status-v2"] {
        ...
        .expect("plain report");
        assert_eq!(
            encode_packets(&packets, &limits.wire).expect("encode"),
            SUCCESS
        );
...
fn inner_records_survive_both_sideband_and_network_fragmentation() {
    ...
    assert_eq!(
        packets.len(),
        5,
        "the command pkt-line needs two outer frames"
    );
...
fn outbound_budget_includes_inner_headers_inner_flush_and_outer_termination() {
    for packet_limit in [24, 65_520] {
        ...
        let required = encode_packets(&packets, &limits.wire)
            .expect("wire bytes")
            .len();
        ...
        assert_eq!(
            ...
            Err(ReceiveError::Wire(WireError::OutboundBytesExceeded {
                limit: required - 1
            }))
        );
    }
...
fn disabled_report_without_sideband_emits_nothing() {
    ...
    assert!(
        report_status(&request, UnpackStatus::Ok, &[], &ReceiveLimits::default())
...
fn rejected_unpack_is_framed_without_reinterpreting_the_supplied_outcome() {
    ...
    let packets = report_status(
        ...
        UnpackStatus::Rejected {
            message: b"invalid pack".to_vec(),
        },
        &[ReceiveCommandStatus::Rejected {
            message: b"unpacker error".to_vec(),
        }],
        &limits,
    )
    .expect("refusal report");
    assert_eq!(
        decode(&demultiplex(&packets, &limits.wire), 1),
        vec![
...
fn malformed_messages_and_status_count_still_fail_before_a_report_is_returned() {
    ...
    for capabilities in ["report-status", "report-status side-band-64k"] {
        ...
        assert_eq!(
            report_status(&request, UnpackStatus::Ok, &[], &limits),
            Err(ReceiveError::StatusCountMismatch {
                expected: 1,
                actual: 0
            })
        );
