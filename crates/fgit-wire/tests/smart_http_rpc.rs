...
use fgit_wire::receive::{
    QuarantineReceipt, ReceiveContext, ReceiveError, ReceiveLimits, ReceiveQuarantineHandoff,
...
impl Repository {
    fn new(format: GitObjectFormat) -> Self {
        ...
        Self {
            refs: vec![
                AdvertisedRef::new(oid, b"refs/heads/main", &WireLimits::default()).unwrap(),
            ],
            format,
        }
    }
    fn oid(&self) -> String {
        "11".repeat(self.format.digest_len())
    }
}
...
fn data(text: impl Into<Vec<u8>>) -> Packet {
    Packet::Data(text.into())
}
fn wire(packets: &[Packet]) -> Vec<u8> {
    encode_packets(packets, &WireLimits::default()).unwrap()
}
fn caps(bytes: &[u8]) -> Capabilities {
    if bytes.is_empty() {
        Capabilities::default()
    } else {
        Capabilities::parse_v1(bytes, &WireLimits::default()).unwrap()
    }
}
...
fn context(format: GitObjectFormat) -> ReceiveContext {
    let capabilities = format!(
        "delete-refs report-status object-format={}",
...
fn receive_body(format: GitObjectFormat, delete: bool) -> Vec<u8> {
    ...
    let algorithm = if format == GitObjectFormat::Sha1 {
        "sha1"
    } else {
        "sha256"
    };
    let mut body = wire(&[
        data(format!(
            "{old} {new} refs/tags/http\0report-status delete-refs object-format={algorithm}"
        )),
        Packet::Flush,
    ]);
    if !delete {
...
fn git_done_does_not_replace_http_chunk_termination() {
    ...
    for missing in 1..=5 {
        ...
        .unwrap();
        let progress = rpc
            .push(&encoded[..encoded.len() - missing], &mut || true)
            .unwrap();
...
fn upload_pipeline_suffix_is_not_consumed_as_git_data() {
    ...
    for is_chunked in [false, true] {
        let header = head(Service::UploadPack, body.len(), is_chunked);
        let request = parse_head(&header, HttpLimits::default()).unwrap().unwrap();
        let mut bytes = if is_chunked {
            chunked(&body, 3)
        } else {
            body.clone()
        };
        let boundary = bytes.len();
        bytes.extend_from_slice(b"GET /other/info/refs HTTP/1.1\r\n");
        let mut rpc = UploadRpc::new(
            &request,
            ProtocolVersion::V0,
            caps(b""),
        ...
        assert_eq!(progress.consumed, boundary);
        assert!(progress.body_complete);
...
fn trailing_git_commands_and_partial_frames_poison_entire_upload() {
    ...
    for suffix in [&b"0000"[..], &b"0"[..], &b"0009done\n"[..]] {
        let mut body = clone_request(&repo);
...
fn second_v2_command_in_one_http_body_is_refused() {
    ...
    .unwrap();
    assert!(matches!(
        rpc.push(&body, &mut || true),
        Err(RpcError::MultipleCommands)
    ));
    assert!(matches!(
        rpc.finish(&mut || true),
        Err(RpcError::FailedRequest)
    ...
    for format in [GitObjectFormat::Sha1, GitObjectFormat::Sha256] {
        for delete in [false, true] {
            ...
            for missing in 1..=5 {
                ...
                .unwrap();
                rpc.push(&encoded[..encoded.len() - missing], &mut || true)
                    .unwrap();
                let mut handoff = StructuralHandoff::default();
...
fn receive_checksum_failure_and_cancellation_leave_handoff_untouched() {
    for format in [GitObjectFormat::Sha1, GitObjectFormat::Sha256] {
        for corrupt in [false, true] {
            ...
            let mut handoff = StructuralHandoff::default();
            assert!(
                rpc.finish_with_handoff(&mut handoff, &mut || corrupt)
                    .is_err()
            );
            assert_eq!(handoff.calls, 0);
        }
...
fn failed_receive_cannot_be_completed_with_replacement_bytes() {
    ...
    let mut rpc = ReceiveRpc::new(
        &request,
        ProtocolVersion::V0,
        context(GitObjectFormat::Sha1),
        HttpLimits::default(),
    )
    .unwrap();
    assert!(rpc.push(b"!\r\n", &mut || true).is_err());
    assert!(matches!(
        rpc.push(&chunked(&body, 5), &mut || true),
        Err(RpcError::FailedRequest)
    ));
    let mut handoff = StructuralHandoff::default();
    assert!(matches!(
...
fn post_handoff_cancellation_does_not_fabricate_non_commit() {
    ...
    impl ReceiveQuarantineHandoff for Handoff<'_> {
        fn handoff(
            &mut self,
            _: &ReceiveRequest,
            _: Option<&fgit_pack::QuarantinedPack>,
            _: &QuarantineReceipt,
        ) -> Result<(), ReceiveError> {
            self.0.set(true);
            Ok(())
    ...
    let mut handoff = Handoff(&cancelled);
    let body = receive_body(GitObjectFormat::Sha1, true);
    let header = head(Service::ReceivePack, body.len(), false);
    let request = parse_head(&header, HttpLimits::default()).unwrap().unwrap();
    let mut rpc = ReceiveRpc::new(
    ...
    let receipt = rpc
        .finish_with_handoff(&mut handoff, &mut || !cancelled.get())
        .unwrap();
    assert!(cancelled.get());
    assert!(receipt.quarantine.delete_only);
}
...
fn discovery_has_exact_service_and_version_preludes_and_shared_limit() {
    ...
    for version in [
        ...
        let capabilities = if version == ProtocolVersion::V2 {
            v2_caps()
        } else {
            caps(b"multi_ack")
        };
        let body =
            upload_discovery(&repo, capabilities.clone(), version, &WireLimits::default()).unwrap();
        ...
        if version == ProtocolVersion::V1 {
            assert!(body.windows(14).any(|part| part == b"000eversion 1\n"));
        }
        assert!(body.ends_with(b"0000"));
        let tight = WireLimits {
            max_outbound_bytes: body.len() - 1,
            ..WireLimits::default()
        };
        assert!(upload_discovery(&repo, capabilities, version, &tight).is_err());
    }
    let receive = receive_discovery(
        repo.refs.clone(),
        &context(repo.format),
        ProtocolVersion::V0,
    )
        ...
        for common in [false, true] {
            for wait in [false, true] {
                for done in [false, true] {
                    ...
                    let mut packets = vec![
                        data(b"command=fetch\n"),
                        Packet::Delimiter,
                        data(format!("want {}\n", repo.oid())),
                        data(format!("have {have}\n")),
                    ];
                    if wait {
                        packets.push(data(b"wait-for-done\n"));
                    }
                    if done {
                        packets.push(data(b"done\n"));
                    }
                    packets.push(Packet::Flush);
                    let body = wire(&packets);
                    let header = head(Service::UploadPack, body.len(), false);
                    let request = parse_head(&header, HttpLimits::default()).unwrap().unwrap();
                    let capabilities = Capabilities::parse_v2_advertisement(
                        &[
                            data(b"version 2\n"),
...
fn one_packet_decoder_budget_is_independent_of_http_fragmentation() {
    ...
    let header = head(Service::ReceivePack, body.len(), false);
    let request = parse_head(&header, HttpLimits::default()).unwrap().unwrap();
    let mut context = context(repo.format);
    context.limits.wire.max_packets_per_push = 1;
    let mut rpc = ReceiveRpc::new(
        &request,
        ProtocolVersion::V0,
        context,
...
fn http_receive_preserves_a_real_native_writer_pack_and_its_target_identity() {
    ...
    use fgit_pack::{
        CanonicalObjectSource, CanonicalPackObject, ObjectId, PackLimits, PackPlanner,
    ...
    impl ReceiveQuarantineHandoff for Check {
        fn handoff(
            ...
            assert_eq!(
                entries[0].inflated,
                b"a real object transported by the native HTTP composition\n"
            );
            self.called = true;
            Ok(())
        }
    ...
    for format in [GitObjectFormat::Sha1, GitObjectFormat::Sha256] {
        ...
        let id = match format {
            GitObjectFormat::Sha1 => {
                ObjectId::from(native_object_oid::<Sha1>(ObjectType::Blob, &body))
            }
            GitObjectFormat::Sha256 => {
                ObjectId::from(native_object_oid::<Sha256>(ObjectType::Blob, &body))
            }
        };
        let source = Blob(CanonicalPackObject::new(
            id,
            ObjectType::Blob,
            body,
            Vec::new(),
        ...
        let plan = PackPlanner::new(format, PackWriteProfile::STORED_V1, PackLimits::default())
            ...
            .unwrap();
        let (pack, _) = PackWriter::new(PackLimits::default())
            .write(&plan, &mut || true)
            .unwrap();
        let algorithm = if format == GitObjectFormat::Sha1 {
            "sha1"
        ...
        let mut raw = wire(&[
            data(format!(
                "{} {id} refs/tags/http-blob\0report-status object-format={algorithm}",
        ...
        .unwrap();
        for fragment in chunked(&raw, 11).chunks(3) {
            rpc.push(fragment, &mut || true).unwrap();
        }
        let mut handoff = Check {
            target: id,
            called: false,
        };
        let completion = rpc.finish_with_handoff(&mut handoff, &mut || true).unwrap();
        assert!(handoff.called);
        assert_eq!(completion.quarantine.pack_bytes, pack.len());
    }
}
