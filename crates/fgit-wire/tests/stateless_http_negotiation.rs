...
use fgit_wire::{
    ...
    UploadPackRepository, UploadPackVersion, WireEvent, WireLimits, encode_packets,
};

struct Repository {
    refs: Vec<AdvertisedRef>,
    common: Vec<AnyGitOid>,
    format: GitObjectFormat,
}
impl Repository {
    fn new(format: GitObjectFormat) -> Self {
        let oid =
            |byte: &str| AnyGitOid::from_hex(format, &byte.repeat(format.digest_len())).unwrap();
        Self {
            refs: vec![
                AdvertisedRef::new(oid("11"), b"refs/heads/main", &WireLimits::default()).unwrap(),
            ],
            common: vec![oid("11"), oid("22")],
            format,
        }
    }
    fn text(&self, byte: &str) -> String {
        byte.repeat(self.format.digest_len())
    }
}
impl UploadPackRepository for Repository {
    ...
    fn contains_want(&self, oid: AnyGitOid) -> bool {
        self.refs.iter().any(|r| r.oid == oid)
    }
    fn is_common(&self, oid: AnyGitOid) -> bool {
        self.common.contains(&oid)
    }
}
...
fn machine(repo: &Repository, caps: &str) -> LegacyUploadPack {
    ...
    let mut machine = LegacyUploadPack::new(
        ...
        Capabilities::parse_v1(b"multi_ack multi_ack_detailed no-done", &limits).unwrap(),
        limits,
    )
    .unwrap()
    .with_stateless_http_rounds();
    let tail = if caps.is_empty() {
        String::new()
    } else {
        format!(" {caps}")
    };
    let first = machine
        .push_packet(&data(format!("want {}{tail}\n", repo.text("11"))), repo)
        .unwrap();
    assert!(first.output.is_empty());
    let flush = machine.push_packet(&Packet::Flush, repo).unwrap();
    assert!(
        flush.output.is_empty(),
        "HTTP want flush must not send a premature NAK"
    );
    machine
}
fn has_pack(events: &[WireEvent]) -> bool {
    events
        .iter()
        .any(|e| matches!(e, WireEvent::PackRequested(_)))
}
    ...
    for common in ["11", "22"] {
        ...
        if common == "11" {
            assert_eq!(
                flush.output,
                vec![
                    data(format!("ACK {} ready\n", repo.text("11"))),
                    data(b"NAK\n"),
                    data(format!("ACK {}\n", repo.text("11")))
                ]
            );
        } else {
            assert_eq!(flush.output, vec![data(b"NAK\n")]);
        }
    }
}
...
fn http_packet_fragmentation_does_not_change_negotiation_outputs() {
    ...
    let request = encode_packets(
        &[
            data(format!("want {} multi_ack_detailed\n", repo.text("11"))),
            Packet::Flush,
            data(format!("have {}\n", repo.text("22"))),
            data(b"done\n"),
        ],
    ...
    .unwrap();
    let expected = vec![
        data(format!("ACK {} common\n", repo.text("22"))),
        data(format!("ACK {}\n", repo.text("22"))),
    ];
    for width in [1, 2, 3, 4, 7, request.len()] {
        let mut machine =
            LegacyUploadPack::new(UploadPackVersion::V1, caps.clone(), WireLimits::default())
                .unwrap()
                .with_stateless_http_rounds();
        ...
        for chunk in request.chunks(width) {
            ...
            packs += transition
                .events
                .iter()
                .filter(|e| matches!(e, WireEvent::PackRequested(_)))
                .count();
        }
        machine.finish_stateless_http_round().unwrap();
