...
const DECODE: [u8; 256] = {
    ...
    let mut i = 0;
    while i < ALPHABET.len() { table[ALPHABET[i] as usize] = i as u8; i += 1; }
    table
};
...
impl Default for BinaryPatchLimits {
    fn default() -> Self {
        Self { max_input_bytes: 16 * 1024 * 1024, max_file_bytes: 8 * 1024 * 1024,
            max_expanded_bytes: 32 * 1024 * 1024, max_lines: 262_144,
            max_inflate_work: 64 * 1024 * 1024, max_delta_work: 64 * 1024 * 1024 }
    }
}
impl BinaryPatchLimits {
    pub fn validate(self) -> Result<(), BinaryPatchError> {
        let c = Self::default();
        if self.max_inflate_work < 2 || self.max_inflate_work > c.max_inflate_work
            || self.max_delta_work < 2 || self.max_delta_work > c.max_delta_work
        { return Err(BinaryPatchError::InvalidLimits); }
        for (value, maximum) in [(self.max_input_bytes, c.max_input_bytes),
            (self.max_file_bytes, c.max_file_bytes), (self.max_expanded_bytes, c.max_expanded_bytes),
            (self.max_lines, c.max_lines)] {
            if value == 0 || value > maximum { return Err(BinaryPatchError::InvalidLimits); }
        }
        Ok(())
    }
}
...
fn check(deadline: &mut impl Deadline) -> Result<(), BinaryPatchError> {
    if deadline.checkpoint() { Ok(()) } else { Err(BinaryPatchError::Cancelled) }
}
...
fn delta_error(error: PackError) -> BinaryPatchError {
    match error {
        ...
        PackError::InputLimit { .. } | PackError::ObjectSizeLimit { .. }
        | PackError::TotalExpandedLimit { .. } | PackError::DeltaResultSizeLimit { .. }
        | PackError::DeltaWorkLimit { .. } | PackError::AllocationFailed { .. } =>
            BinaryPatchError::Limit("delta"),
        _ => BinaryPatchError::Invalid("delta program"),
    }
}
struct Probe<'a, D>(&'a mut D);
impl<D: Deadline> CancellationProbe for Probe<'_, D> {
    fn is_cancelled(&mut self) -> bool { !self.0.checkpoint() }
}

struct Input<'a> { bytes: &'a [u8], at: usize, lines: usize, remaining: usize }
impl<'a> Input<'a> {
    fn line(&mut self, limits: BinaryPatchLimits, deadline: &mut impl Deadline)
        -> Result<&'a [u8], BinaryPatchError> {
        check(deadline)?;
        if self.lines == limits.max_lines { return Err(BinaryPatchError::Limit("lines")); }
        ...
        let end = rest.iter().take(67).position(|b| *b == b'\n')
            .ok_or(BinaryPatchError::Invalid("unterminated or oversized record"))?;
        self.at += end + 1; self.lines += 1;
        Ok(&rest[..end])
    }
...
fn decode_row(row: &[u8], output: &mut [u8; 52]) -> Result<usize, BinaryPatchError> {
    ...
    for (i, digits) in row[1..].chunks_exact(5).enumerate() {
        ...
        for digit in digits {
            ...
            if value == u8::MAX { return Err(BinaryPatchError::Invalid("base85 digit")); }
            word = word.checked_mul(85).and_then(|n| n.checked_add(u32::from(value)))
...
fn block(input: &mut Input<'_>, base: &[u8], limits: BinaryPatchLimits,
    deadline: &mut impl Deadline) -> Result<Vec<u8>, BinaryPatchError> {
    let header = input.line(limits, deadline)?;
    let (delta, digits) = if let Some(n) = header.strip_prefix(b"literal ") { (false, n) }
        else if let Some(n) = header.strip_prefix(b"delta ") { (true, n) }
        else { return Err(BinaryPatchError::Invalid("binary hunk header")); };
    if digits.is_empty() || (digits.len() > 1 && digits[0] == b'0') {
        return Err(BinaryPatchError::Invalid("binary hunk length"));
    }
    let mut declared = 0usize;
    for digit in digits {
        if !digit.is_ascii_digit() { return Err(BinaryPatchError::Invalid("binary hunk length")); }
        declared = declared.checked_mul(10).and_then(|n| n.checked_add(usize::from(*digit - b'0')))
            .filter(|n| *n <= limits.max_file_bytes).ok_or(BinaryPatchError::Limit("hunk bytes"))?;
    }
    input.charge(declared)?;
    let policy = InflateLimits { max_input_bytes: limits.max_input_bytes,
        max_pending_input_bytes: limits.max_input_bytes, max_output_bytes: declared.max(1),
        // Reserve half for a reverse member; a one-member patch cannot borrow it.
        max_work_units: limits.max_inflate_work / 2, ..InflateLimits::GIT_OBJECT };
    let mut inflater = Inflater::new(policy).map_err(inflate_error)?;
    let mut inflated = Vec::new();
    inflated.try_reserve_exact(declared).map_err(|_| BinaryPatchError::Limit("allocation"))?;
    let mut rows = 0;
    loop {
        ...
        inflater.push_with_control(&decoded[..count], &mut Probe(deadline)).map_err(inflate_error)?;
        ...
        inflated.extend_from_slice(&next); rows += 1;
    }
    if rows == 0 { return Err(BinaryPatchError::Invalid("empty encoded member")); }
    ...
    if inflated.len() != declared { return Err(BinaryPatchError::Invalid("inflated hunk length")); }
    check(deadline)?;
    if !delta { return Ok(inflated); }
    ...
    let pack = PackLimits { max_input_bytes: limits.max_input_bytes,
        max_object_bytes: limits.max_file_bytes,
        max_total_expanded_bytes: input.remaining.checked_add(base.len())
            .ok_or(BinaryPatchError::Limit("expanded bytes"))?,
        max_delta_work: limits.max_delta_work / 2, ..PackLimits::default() };
...
fn decode(bytes: &[u8], old: Option<ObjectId>, new: Option<ObjectId>,
    ...
    let format = old.or(new).filter(|id| !id.is_zero())
        .ok_or(BinaryPatchError::Invalid("object identities"))?.algorithm();
    if [old, new].into_iter().flatten().any(|id| id.is_zero() || id.algorithm() != format) {
        return Err(BinaryPatchError::Invalid("object identities"));
    }
    if old.map_or(!base.is_empty(), |id| git_object_id(format, GitObjectKind::Blob, base) != id) {
        return Err(BinaryPatchError::SourceMismatch);
    }
    check(deadline)?;
    let mut input = Input { bytes, at: 0, lines: 0, remaining: limits.max_expanded_bytes };
    ...
    if new.map_or(!result.is_empty(), |id| git_object_id(format, GitObjectKind::Blob, &result) != id) {
        return Err(BinaryPatchError::TargetMismatch);
    }
    check(deadline)?;
    if input.at != bytes.len() {
        let reverse = block(&mut input, &result, limits, deadline)?;
        if reverse != base { return Err(BinaryPatchError::ReverseMismatch); }
    }
