...
impl FullBundleInput<'_> {
    ...
    pub fn select_updates(
        &self, mappings: &[BundleRefMapping], limits: FullBundleLimits,
        ...
        for mapping in mappings {
            checkpoint(deadline)?;
            bytes = bytes.checked_add(mapping.source.as_bytes().len())
            ...
            if mapping.expected_old.is_some_and(|old| old.is_zero() || old.algorithm() != self.format()) {
                return Err(FullBundleError::FormatMismatch);
            }
            let index = self.references().binary_search_by(|reference| reference.name().cmp(&mapping.source))
                .map_err(|_| FullBundleError::Invalid("unadvertised fetch source"))?;
            let update = BundleRefUpdate { source: mapping.source.clone(), destination: mapping.destination.clone(),
                expected_old: mapping.expected_old, target: *self.references()[index].target() };
            if updates.insert(mapping.destination.clone(), update).is_some() {
