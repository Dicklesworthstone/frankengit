...
fn fixture(text: &str, format: ObjectFormat) -> (&[u8], Option<ObjectId>, Option<ObjectId>) {
    ...
    let parse = |s: &str| {
        if s.bytes().all(|b| b == b'0') {
            ...
            Some(ObjectId::from_hex(format, s).unwrap())
        }
    };
...
fn upstream_literal_creation_deletion_and_delta_goldens_match_both_native_formats() {
    for (format, fixtures) in [
        (
            ObjectFormat::Sha1,
            [
                include_str!("fixtures/literal-sha1.patch"),
                include_str!("fixtures/create-sha1.patch"),
                include_str!("fixtures/delete-sha1.patch"),
                include_str!("fixtures/delta-sha1.patch"),
            ],
        ),
        (
            ObjectFormat::Sha256,
            [
                include_str!("fixtures/literal-sha256.patch"),
                include_str!("fixtures/create-sha256.patch"),
                include_str!("fixtures/delete-sha256.patch"),
                include_str!("fixtures/delta-sha256.patch"),
            ],
        ),
    ] {
        ...
        for (text, base, expected) in [
            ...
            if let Some(old) = old {
                assert_eq!(id(format, base), old);
            }
            if let Some(new) = new {
                assert_eq!(id(format, expected), new);
            }
            let result = apply_binary_patch(
                body,
                old,
                new,
                base,
...
fn run(
    ...
    apply_binary_patch(
        body,
        Some(id(ObjectFormat::Sha1, base)),
        Some(id(ObjectFormat::Sha1, result)),
        base,
        limits,
        &mut || true,
    )
...
fn exact_source_target_and_hash_domain_are_not_optional() {
    ...
    assert_eq!(
        ...
        Err(BinaryPatchError::SourceMismatch)
    );
    assert_eq!(
        apply_binary_patch(
            body,
            old,
            Some(id(ObjectFormat::Sha1, b"wrong")),
            b"\0old\xff\n",
            BinaryPatchLimits::default(),
            &mut || true
        ),
        Err(BinaryPatchError::TargetMismatch)
    );
    assert!(
        apply_binary_patch(
            body,
            old,
            Some(id(ObjectFormat::Sha256, b"\0new\xfe\r\n")),
            b"\0old\xff\n",
            BinaryPatchLimits::default(),
            &mut || true
        )
    ...
    assert!(
        apply_binary_patch(
            body,
            Some(ObjectId::from_hex(ObjectFormat::Sha1, &"0".repeat(40)).unwrap()),
            new,
            &[],
            BinaryPatchLimits::default(),
            &mut || true
...
fn zlib_completion_checksum_and_declared_sizes_are_required() {
    ...
    *corrupt.last_mut().unwrap() ^= 1;
    ...
    for header in [
        ...
        "literal -3",
        "literal 8388609",
        "delta 9999999999999999999999999999999999999",
        "literal 3 trailing",
    ] {
        let patch = [
...
fn delta_instructions_use_native_size_and_copy_bounds() {
    ...
    let patch = [
        b"GIT binary patch\n".as_slice(),
        &member("delta", &valid),
        &member("literal", b"abc"),
    ]
    .concat();
    assert_eq!(
        run(&patch, b"abc", b"abc!", BinaryPatchLimits::default()).unwrap(),
        b"abc!"
    );
    for program in [
        vec![4, 4, 0x90, 3, 1, b'!'],
    ...
    let bad = [
        b"GIT binary patch\n".as_slice(),
        &member("delta", &oversized),
    ]
    .concat();
    assert!(matches!(
        run(&bad, b"abc", b"abc!", BinaryPatchLimits::default()),
...
fn cancellation_never_returns_the_provisional_forward_image() {
    let patch = literal(b"old", b"new");
    let old = Some(id(ObjectFormat::Sha1, b"old"));
    let new = Some(id(ObjectFormat::Sha1, b"new"));
    let mut polls = 0;
    assert!(
        apply_binary_patch(
            &patch,
