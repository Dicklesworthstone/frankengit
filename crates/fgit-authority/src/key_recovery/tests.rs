...
use fgit_types::{
    DecisionOutcome, DecisionSequence, GitHashAlgorithm, HeadGeneration, InternalObjectId,
    PolicyEpoch, RefusalRecordId, RegistryEpoch,
};
...
fn fixture() -> (MemoryAuthorityStore, HeadKey, RecoveryScope, SealAttempt) {
    ...
    let head = RepositoryAuthorityHeadBody {
        ...
        outcome_index_root: crate::outcome_index_root(&[]).unwrap(),
        retention_root: root,
        outbox_root: root,
    ...
    let request = SemanticRequest::build(
        crate::RECEIVE_ADMISSION_SCHEMA,
        GitHashAlgorithm::Sha1,
        true,
        Vec::new(),
        Vec::new(),
        vec![
            crate::ScopedEntry::new(
                fgit_types::AsciiSlug::from_static("test"),
                fgit_types::AsciiSlug::from_static("request"),
                b"payload",
            )
            .unwrap(),
        ],
    ...
    let attempt = SealAttempt {
        ...
        idempotency_key: IdempotencyKey::new(b"lost-response-secret-key".to_vec()).unwrap(),
        request,
    };
...
fn canonical_head_payload_exceeds_binding_limit_but_remains_recoverable() {
    ...
    let binding_sized = DecodeLimits {
    ...
    assert_eq!(
        AuthorityStore::read_head(&store, &head).unwrap(),
...
fn observations_distinguish_binding_from_seal_without_writing_or_replaying_work() {
    ...
    let RequestRecovery::Recovered(recovered) = recover() else {
        panic!("verified seal");
    };
    ...
    let different = RecoveryScope {
        principal_id: PrincipalId::from_bytes([4; 16]),
        ..scope
    };
    assert_eq!(
        recover_request(&store, &head, different, &attempt.idempotency_key, &|| Ok(
            ()
        ))
        .unwrap(),
        RequestRecovery::KeyNotObserved
    ...
    assert_eq!(
        recover_request(&store, &head, scope, &wrong_key, &|| Ok(())).unwrap(),
        RequestRecovery::KeyNotObserved
    );
...
fn every_scope_and_identity_mismatch_fails_before_outcome_disclosure() {
    ...
    assert!(
        checked_seal(
            ImmutableRead::Present(bytes.clone()),
            scope,
    ...
    for field in 0..6 {
        ...
        match field {
            ...
            _ => {
                ...
                other.idempotency_key = IdempotencyKey::new(b"other".to_vec()).unwrap();
        ...
        assert!(
            matches!(
                checked_seal(
                    ImmutableRead::Present(encode_body(&changed).unwrap()),
                    scope,
                    &attempt.idempotency_key,
                    tx
                ),
                Err(RecoveryFailure::Integrity { .. })
            ),
            "field {field}"
        );
    }
...
fn binding_and_seal_decoders_are_strict_bounded_and_domain_pinned() {
    ...
    encoder
        .write_internal_object_id(tx.as_internal_object_id())
        .unwrap();
    let bytes = encoder.into_bytes();
    assert_eq!(
        binding_identity(ImmutableRead::Present(bytes.clone())).unwrap(),
        Some(tx)
    );
    for end in 0..bytes.len() {
        assert!(binding_identity(ImmutableRead::Present(bytes[..end].to_vec())).is_err());
    }
    let mut trailing = bytes;
    trailing.push(0);
    assert!(binding_identity(ImmutableRead::Present(trailing)).is_err());
    let root = fgit_codec::harness::digest_of(1);
    let foreign = InternalObjectId::new(
        root.algorithm(),
        TransactionSealId::DOMAIN_TAG,
        CANONICAL_CODEC_VERSION,
        *root.bytes(),
    );
    let mut encoder = Encoder::new();
    encoder.write_internal_object_id(&foreign).unwrap();
    assert!(matches!(
        binding_identity(ImmutableRead::Present(encoder.into_bytes())),
        Err(RecoveryFailure::Integrity { .. })
    ));
    assert!(matches!(
        binding_identity(ImmutableRead::Present(vec![0; MAX_BINDING_BYTES + 1])),
        Err(RecoveryFailure::BoundExceeded { .. })
    ));
    let bytes = encode_body(&seal).unwrap();
    for end in 0..bytes.len() {
        assert!(
            checked_seal(
                ImmutableRead::Present(bytes[..end].to_vec()),
                scope,
                &attempt.idempotency_key,
                tx
            )
    ...
    assert!(
        checked_seal(
            ImmutableRead::Present(trailing),
            scope,
            &attempt.idempotency_key,
            tx
        )
    ...
    assert!(matches!(
        checked_seal(
            ImmutableRead::Present(vec![0; MAX_SEAL_BYTES + 1]),
            scope,
            &attempt.idempotency_key,
            tx
        ),
...
fn corrupt_binding_and_substituted_seals_do_not_become_absence_or_pending_success() {
    for corruption in 0..3 {
        ...
        let binding = idempotency_binding_key(
            scope.tenant_id,
            scope.repository_id,
            scope.principal_id,
            &attempt.idempotency_key,
        ...
        if corruption == 0 {
            ...
            if corruption == 1 {
                seal.authenticated_principal_id = PrincipalId::from_bytes([7; 16]);
            } else {
                seal.canonical_request_digest = fgit_codec::harness::digest_of(7);
            }
        ...
        assert!(
            recover_request(&store, &head, scope, &attempt.idempotency_key, &|| Ok(())).is_err()
        );
    }
}
...
fn head_scope_and_cancellation_are_checked_even_when_no_binding_exists() {
    let (store, head, scope, attempt) = fixture();
    let wrong = RecoveryScope {
        repository_id: RepositoryId::from_bytes([8; 16]),
        ..scope
    };
    assert!(matches!(
        recover_request(&store, &head, wrong, &attempt.idempotency_key, &|| Ok(())),
        Err(RecoveryFailure::Integrity {
            field: "head repository"
        })
    ));
    let absent = HeadKey::new(b"not-created".to_vec()).unwrap();
    assert!(matches!(
        recover_request(&store, &absent, scope, &attempt.idempotency_key, &|| Ok(())),
    ...
    for code in [
        ...
        assert!(
            matches!(recover_request(&store, &head, scope, &attempt.idempotency_key, &|| Err(code)),
...
fn cancellation_after_terminal_resolution_cannot_erase_a_known_decision() {
    let (_, _, scope, attempt) = fixture();
    let (tx, seal) = attempt.derive().unwrap();
    let (_, seal_id) = checked_seal(
        ImmutableRead::Present(encode_body(&seal).unwrap()),
        scope,
        &attempt.idempotency_key,
        tx,
    )
    ...
    .unwrap();
    let root = fgit_codec::harness::digest_of(1);
    let refused = crate::TerminalOutcome {
    ...
    assert!(matches!(
        finish(seal, seal_id, OutcomeLookup::Undecided, &cancelled),
        Err(RecoveryFailure::Interrupted(
            RefusalCode::CancellationInProgress
        ))
    ));
