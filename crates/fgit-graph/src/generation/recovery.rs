...
impl GenerationReadLimits {
    pub fn validate(self) -> Result<(), GenerationAuthorityError> {
        ...
        if self.max_generations == 0
            || self.max_generations > ceiling.max_generations
            || self.max_body_bytes == 0
            || self.max_body_bytes > ceiling.max_body_bytes
...
impl Budget {
    fn bytes(&mut self, count: usize) -> Result<(), GenerationAuthorityError> {
        if count > self.limits.max_body_bytes {
            return Err(GenerationAuthorityError::ReadBudgetExceeded(
                "generation body bytes",
            ));
        }
        self.bytes = self
            .bytes
            ...
            .ok_or(GenerationAuthorityError::ReadBudgetExceeded(
...
impl Walk {
    fn start(
        ...
        if encode_body(&body)? != receipt.body() {
        ...
        if body.graph_view_id() != view {
            return Err(GenerationAuthorityError::ViewMismatch {
                active: Box::new(body.graph_view_id()),
                proposed: Box::new(view),
            });
        }
        ...
        Ok(Some(Self {
            next: activation.generation_id,
            position: activation.authority_generation,
            selected: SelectedGeneration {
                activation,
                body,
                generations_read: 0,
    ...
    fn observe(&mut self, read: ImmutableRead) -> Result<bool, GenerationAuthorityError> {
        let ImmutableRead::Present(bytes) = read else {
            return Err(GenerationAuthorityError::MissingGeneration {
                generation_id: Box::new(self.next),
            });
        };
        ...
        if observed != self.next {
            return Err(GenerationAuthorityError::GenerationIdentityMismatch {
                expected: Box::new(self.next),
                observed: Box::new(observed),
            });
        }
        if encode_body(&body)? != bytes
            || body.graph_view_id() != self.selected.body.graph_view_id()
            || (self.budget.generations == 1 && body != self.selected.body)
            || ((self.position == HeadGeneration::FIRST)
                != body.predecessor_generation_id().is_none())
        {
            return Err(GenerationAuthorityError::HistoryInconsistent);
        }
        ...
        if self.target == Some(observed) {
            self.found = Some(GenerationActivation {
                ...
                authority_generation: self.position,
            });
    ...
    fn recovery(self) -> GenerationRecovery {
        match self.found {
            ...
            Some(activation) => GenerationRecovery::Superseded {
                activation,
                selected: self.selected,
            },
            None => GenerationRecovery::NotInSelectedHistory {
                selected: self.selected,
            },
        }
    }
...
impl<S: AuthorityStore> GenerationAuthority<'_, S> {
    fn inspect(
        ...
        let Some(mut walk) = Walk::start(&read, view, target, minimum, limits)? else {
            return Ok(None);
        };
        loop {
            check(live)?;
    ...
    pub fn read_active(
        ...
        self.inspect(view, None, minimum, limits, live)
...
impl<S: AsyncAuthorityStore> GenerationAuthority<'_, S> {
    ...
    pub async fn read_active_async(
        ...
        self.inspect_async(cx, view, None, minimum, limits, live)
            .await
            .map(|walk| walk.map(|walk| walk.selected))
    }
    ...
    pub async fn recover_activation_async(
        ...
        self.inspect_async(cx, view, Some(candidate), minimum, limits, live)
            .await
            .map(|walk| walk.map_or(GenerationRecovery::Uninitialized, Walk::recovery))
    }
}
