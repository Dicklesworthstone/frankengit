...
fn chain(store: &AsyncStore, count: usize) -> Vec<(GraphGenerationBody, GenerationActivation)> {
    ...
    for n in 0..count {
        ...
        body.source.source_rcr_id =
            RepositoryCommitId::from_internal_object_id(internal_object_id(
                IdentityDomain::RepositoryCommitRecord,
                SchemaId::new(SchemaFamily::from_static("repository-commit-record"), 1, 0),
                CodecVersion::new(1, 0),
                format!("source-{n}").as_bytes(),
            ))
            .unwrap();
...
fn continuation_returns_the_requested_manifest_not_the_new_active_index() {
    ...
    let current = ready(authority.read_active_async(
        &2,
        view(),
        Some(target),
        Default::default(),
    ...
    .unwrap();
    let pinned =
        ready(authority.read_at_async(&3, view(), target, None, Default::default(), &mut || true))
            .unwrap();
    ...
    assert_ne!(
        pinned.body().index_manifest_root(),
        current.body().index_manifest_root()
    );
    assert_ne!(pinned.body().source(), current.body().source());
    assert_eq!(
        pinned.body().vertices_root(),
        &generations[0].0.vertices_root
    );
    assert_eq!(pinned.body().edges_root(), &generations[0].0.edges_root);
    assert_eq!(
        pinned.body().evidence_root(),
        &generations[0].0.evidence_root
    );
...
fn every_original_position_has_exact_sync_async_body_and_counter_parity() {
    ...
    for (index, (body, activation)) in generations.iter().enumerate() {
        let asynchronous = ready(authority.read_at_async(
            &17,
            view(),
            activation,
            None,
        ...
        .unwrap();
        let synchronous = reference
            .read_at(view(), activation, None, Default::default(), &mut || true)
            .unwrap();
        ...
        assert_eq!(
            asynchronous.bytes_read(),
            head_bytes
                + generations[index..]
                    .iter()
                    .map(|(body, _)| encode_body(body).unwrap().len())
                    .sum::<usize>()
        );
    }
}
...
fn independent_checkpoints_are_verified_on_both_sides_of_the_query_generation() {
    ...
    for index in 0..4 {
        ...
        let result = ready(authority.read_at_async(
            &2,
            view(),
            target,
            Some(floor),
...
fn an_existing_identity_at_the_wrong_original_position_is_not_a_valid_pin() {
    ...
    for bad in [
        GenerationActivation {
            generation_id: generations[0].1.generation_id,
            authority_generation: generations[2].1.authority_generation,
        },
        GenerationActivation {
            generation_id: generations[2].1.generation_id,
            authority_generation: HeadGeneration::FIRST,
        },
        GenerationActivation {
            generation_id: generations[1].1.generation_id,
    ...
    assert!(matches!(
        ready(authority.read_at_async(
            &2,
            GraphViewId::try_new(b"another-view").unwrap(),
            &generations[0].1,
            None,
            Default::default(),
            &mut || true
...
fn a_staged_fork_or_uninitialized_head_never_becomes_a_query_generation() {
    ...
    store
        .inner
        .put_if_absent(&slot, &encode_body(&orphan).unwrap())
        .unwrap();
    let expected = GenerationActivation {
    ...
    assert!(matches!(
        store.inner.read_immutable(&slot).unwrap(),
...
fn missing_or_corrupt_backing_is_not_repaired_by_returning_a_different_generation() {
    for fault in ["missing-current", "missing-target", "wrong-target"] {
        ...
        store
            .inner
            .initialize_head(&key(), HeadGeneration::try_new(2).unwrap(), &next_bytes)
            .unwrap();
        if fault != "missing-current" {
            store
                .inner
                .put_if_absent(
                    &immutable_generation_key(next.generation_id().unwrap()).unwrap(),
                    &next_bytes,
                )
                .unwrap();
        }
        if fault != "missing-target" {
            let body = if fault == "wrong-target" {
                candidate(b"substituted", None)
            } else {
                first
            };
            store
                .inner
                .put_if_absent(
                    &immutable_generation_key(first_id).unwrap(),
                    &encode_body(&body).unwrap(),
                )
                .unwrap();
        }
        let target = GenerationActivation {
            generation_id: first_id,
            authority_generation: HeadGeneration::FIRST,
        };
        let result = ready(GenerationAuthority::new(&store, key()).read_at_async(
            &2,
            view(),
            &target,
            None,
        ...
        match (fault, result) {
            (
                "missing-current" | "missing-target",
                Err(GenerationAuthorityError::MissingGeneration { .. }),
            ) => {}
            (
                "wrong-target",
...
fn exact_work_bounds_include_the_independent_floor_and_have_permitted_twins() {
    ...
    let generations = chain(&store, 3);
    let authority = GenerationAuthority::new(&store, key());
    let sizes: Vec<_> = generations
        .iter()
        .map(|(body, _)| encode_body(body).unwrap().len())
        .collect();
    let exact = GenerationReadLimits {
        max_generations: 3,
        max_body_bytes: *sizes.iter().max().unwrap(),
        max_total_bytes: sizes[2] + sizes.iter().sum::<usize>(),
    };
    let expected = &generations[1].1;
    let minimum = Some(&generations[0].1);
    let result =
        ready(authority.read_at_async(&2, view(), expected, minimum, exact, &mut || true)).unwrap();
    ...
    for short in [
        ...
        GenerationReadLimits {
            max_body_bytes: exact.max_body_bytes - 1,
            ..exact
        },
    ] {
        assert!(matches!(
            ready(authority.read_at_async(&2, view(), expected, minimum, short, &mut || true)),
...
fn cancellation_before_or_after_a_read_exposes_no_provisional_manifest() {
    ...
    let allowed = ready(authority.read_at_async(
        &4,
        view(),
        &generations[0].1,
        None,
...
fn concurrent_activation_changes_the_observation_not_the_query_manifest() {
    ...
    let first = ready(authority.read_at_async(
        &19,
        view(),
        &generations[1].1,
        Some(&generations[0].1),
        Default::default(),
        &mut live,
    ))
    .unwrap();
    assert!(
        fired,
        "writer must actually advance the head during the walk"
    );
    ...
    assert!(
        store
            ...
            .unwrap()
            .iter()
            .all(|(op, cx)| ["head", "authenticate", "immutable"].contains(op) && *cx == 19)
    );
    let second = ready(authority.read_at_async(
        &20,
        view(),
        first.activation(),
        Some(first.selected_head()),
        Default::default(),
        &mut || true,
    ))
    .unwrap();
    ...
    assert_eq!(
        second.selected_head().generation_id,
        later.generation_id().unwrap()
    );
...
impl AsyncAuthorityStore for PendingRead<'_> {
    ...
    async fn compare_exchange_head(
        ...
        self.inner
            .compare_exchange_head(cx, key, expected, generation, body)
            .await
    }
...
fn pending_reads_resume_or_cancel_without_returning_a_staged_query_result() {
    for cancel in [false, true] {
        ...
        let future = authority.read_at_async(
            &52,
            view(),
            &generations[0].1,
            None,
        ...
        if cancel {
            ...
            assert_eq!(result.unwrap().body(), &generations[0].0);
        }
        assert!(
            store.calls.lock().unwrap().iter().all(|(op, cx)| [
                "head",
                "authenticate",
                "immutable"
            ]
...
fn an_unpolled_sendable_read_has_no_effect_and_an_owned_pin_stays_unchanged() {
    ...
    require_send(authority.read_at_async(
        &43,
        view(),
        &generations[0].1,
        None,
    ...
    assert!(store.calls.lock().unwrap().is_empty());
    let pinned = ready(authority.read_at_async(
        &44,
        view(),
        &generations[0].1,
        None,
    ...
    let again = GenerationAuthority::new(&store.inner, key())
        .read_at(
            view(),
            pinned.activation(),
            Some(pinned.selected_head()),
            Default::default(),
            &mut live,
        )
        .unwrap();
    ...
    assert_eq!(
        again.selected_head().generation_id,
        next.generation_id().unwrap()
    );
}
...
fn exact_only_queries_use_the_pinned_class_not_a_different_generations_class() {
    ...
    let old = ready(authority.read_at_async(
        &2,
        view(),
        &first,
        Some(&second),
    ...
    let latest = ready(authority.read_at_async(
        &2,
        view(),
        &second,
        Some(&first),
    ...
    .unwrap();
    assert_eq!(
        latest.body().require_exact(),
        Err(GraphAuthorityClassRefusal::ExactRequired {
            observed: GraphAuthorityClass::Statistical,
        })
    );
    assert_ne!(
        old.body().generation_id().unwrap(),
        latest.body().generation_id().unwrap()
    );
}
