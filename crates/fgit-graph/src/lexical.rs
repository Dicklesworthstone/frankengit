...
use fgit_types::{
    Digest, GitHashAlgorithm, GitOid, RepositoryId, RepositoryIncarnationId, TenantId,
};
...
impl LexicalSegment {
    ...
    pub fn build<'a>(
        ...
        check(live)?;
        if first_id == 0 {
            return Err(LexicalError::Invalid("zero document ID"));
        }
        let mut result = Self {
            namespace,
            documents: Vec::new(),
        ...
        for document in documents {
            ...
            if !path_valid(document.path)
                || result
                    .documents
                    .last()
                    .is_some_and(|last| last.path.as_slice() >= document.path)
            {
                return Err(LexicalError::Invalid("document path order"));
            }
            if document.content.len() > MAX_FILE_BYTES {
            ...
            if document.blob.algorithm() != namespace.object_format
                || document.blob.is_zero()
                || git_object_id(
                    namespace.object_format,
                    GitObjectKind::Blob,
                    document.content,
                ) != document.blob
            {
            ...
            check(live)?;
            let id = first_id
                .checked_add(result.documents.len() as u64)
                .ok_or(LexicalError::Limit("document IDs"))?;
            bounded_add(
                &mut encoded_bound,
            ...
            result.documents.push(IndexedDocument {
                id,
                path: document.path.to_vec(),
                blob: document.blob,
                content_bytes: document.content.len() as u32,
            });
            for (channel, bytes) in [
                (LexicalChannel::Content, document.content),
                (LexicalChannel::Path, document.path),
            ] {
                tokens(bytes, live, |token, offset| {
                    let term = Term {
                        channel,
                        bytes: token.iter().map(u8::to_ascii_lowercase).collect(),
                    };
    ...
    #[must_use]
    pub const fn namespace(&self) -> LexicalNamespace {
        self.namespace
    }
    #[must_use]
    ...
    pub fn posting_count(&self) -> usize {
        self.terms.values().map(|p| p.documents.len()).sum()
    }
    ...
    pub fn concatenate(
        ...
        let first = segments
            .first()
            .ok_or(LexicalError::Invalid("empty compaction"))?;
        let mut result = Self {
        ...
        for segment in segments {
            check(live)?;
            if segment.namespace != result.namespace {
                return Err(LexicalError::NamespaceMismatch);
            }
            if result.documents.len() + segment.documents.len() > MAX_DOCUMENTS {
                return Err(LexicalError::Limit("documents"));
            }
            ...
            for (term, columns) in &segment.terms {
                ...
                bounded_add(
                    &mut encoded_bound,
                    12 * columns.documents.len(),
                    MAX_SEGMENT_BYTES,
                    "segment bytes",
                )?;
                into.documents.extend_from_slice(&columns.documents);
                into.offsets.extend_from_slice(&columns.offsets);
            }
        }
        result.encode(live)?;
        check(live)?;
    ...
    pub fn encode(&self, live: &mut impl FnMut() -> bool) -> Result<Vec<u8>, LexicalError> {
        encoding::encode(self, live)
    }
    pub fn root(&self, live: &mut impl FnMut() -> bool) -> Result<Digest, LexicalError> {
        encoding::root(self, live)
    }
    pub fn decode(
        bytes: &[u8],
        ...
        encoding::decode(bytes, expected_root, namespace, live)
    }
    ...
    pub fn search(
        ...
        Ok(LexicalReport {
            next_after: if more {
                hits.last().map(|hit| hit.document_id)
            } else {
                None
            },
            complete: !more,
    ...
    fn search_into(
        ...
        for bytes in &query.terms {
            ...
            let Some(list) = self.terms.get(&Term {
        ...
        let pivot = lists
            .iter()
            .enumerate()
            .min_by_key(|(i, list)| (list.documents.len(), *i))
            .map(|(i, _)| i)
            .ok_or(LexicalError::Invalid("empty query"))?;
        let mut cursor = lower_bound(&lists[pivot].documents, after.unwrap_or(0), budget, live)?;
        while cursor < lists[pivot].documents.len() {
            let id = lists[pivot].documents[cursor];
            cursor += 1;
            if after.is_some_and(|value| id <= value) {
                continue;
            }
            ...
            for (query_index, list) in lists.iter().enumerate() {
                ...
                if list.documents.get(at) != Some(&id) {
                    break;
                }
                spans.push(LexicalSpan {
                    query_index,
                    byte_offset: list.offsets[at],
            ...
            if hits.len() == budget.limits.max_results {
                return Ok(true);
            }
            bounded_add(
                &mut budget.result_bytes,
                document.path.len() + spans.len() * 24 + 64,
                MAX_RESULT_BYTES,
                "result bytes",
            )?;
...
impl LexicalQuery {
    pub fn new(
        ...
        if prefixes.len() > 128
            || prefixes.iter().any(|p| !path_valid(p))
        ...
        let mut terms: Vec<Vec<u8>> = terms
            .iter()
            .map(|term| term.iter().map(u8::to_ascii_lowercase).collect())
            .collect();
        terms.sort();
        terms.dedup();
    ...
    #[must_use]
    pub const fn channel(&self) -> LexicalChannel {
        self.channel
    }
    #[must_use]
...
impl Default for LexicalQueryLimits {
    fn default() -> Self {
        Self {
            max_results: 100,
            max_work: MAX_WORK,
...
impl QueryBudget {
    ...
    fn charge(&mut self, amount: u64, live: &mut impl FnMut() -> bool) -> Result<(), LexicalError> {
        ...
        self.work = self
            ...
            .ok_or(LexicalError::Limit("query work"))?;
        Ok(())
    }
}
fn lower_bound(
