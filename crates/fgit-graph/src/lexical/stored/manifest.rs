...
fn payload<B: CanonicalBody>(kind: &'static str, body: B) -> Result<Payload, IndexError> {
    ...
    if bytes.len() > MAX_SEGMENT_BYTES {
        return Err(LexicalError::Limit("metadata bytes").into());
    }
    Ok(Payload {
        kind,
        root: payload_root(&body)?,
        bytes,
    })
}
...
impl SegmentRef {
    pub(super) fn new(
        ...
        let first = segment
            .documents()
            .first()
            .ok_or(LexicalError::Invalid("empty segment"))?;
        let last = segment
            .documents()
            .last()
            .ok_or(LexicalError::Invalid("empty segment"))?;
        Ok(Self {
            ...
            documents: segment.documents().len() as u32,
            source_bytes: segment
                .documents()
                .iter()
                .map(|d| u64::from(d.content_bytes))
                .sum(),
    ...
    fn write(&self, out: &mut Encoder) -> Result<(), IndexError> {
        ...
        out.write_scalar(self.bytes);
        out.write_scalar(self.first_id);
        out.write_scalar(self.last_id);
        out.write_bytes("first path", &self.first_path)?;
        out.write_bytes("last path", &self.last_path)?;
        out.write_scalar(self.documents);
        out.write_scalar(self.source_bytes);
        out.write_scalar(self.terms);
        out.write_scalar(self.postings);
        Ok(())
    }
    fn read(input: &mut Decoder<'_>) -> Result<Self, IndexError> {
        Ok(Self {
            root: input.read_digest()?,
            bytes: input.read_scalar("segment bytes")?,
            first_id: input.read_scalar("first ID")?,
            last_id: input.read_scalar("last ID")?,
            first_path: input.read_bytes("first path")?.to_vec(),
            last_path: input.read_bytes("last path")?.to_vec(),
            documents: input.read_scalar("document count")?,
            source_bytes: input.read_scalar("source bytes")?,
            terms: input.read_scalar("term count")?,
            postings: input.read_scalar("posting count")?,
        })
    }
}
...
impl Manifest {
    pub(super) fn new(
        ...
        if [source.commit, source.tree]
            .iter()
            .any(|id| id.is_zero() || id.algorithm() != source.namespace.object_format)
        {
            return Err(LexicalError::NativeIdentityMismatch.into());
        }
        if segments.len() > MAX_SEGMENTS || non_regular_entries > 50_000 {
            return Err(LexicalError::Limit("index entries").into());
        }
        let (mut documents, mut bytes, mut source_bytes) = (0usize, 0usize, 0usize);
        let mut previous: Option<&SegmentRef> = None;
        for entry in &segments {
            if entry.bytes == 0
                || entry.bytes as usize > MAX_SEGMENT_BYTES
                || entry.documents == 0
                || entry.documents as usize > MAX_DOCUMENTS
                || entry.first_id == 0
                || entry.first_id > entry.last_id
                || entry.last_id - entry.first_id < u64::from(entry.documents - 1)
                || !path_valid(&entry.first_path)
                || !path_valid(&entry.last_path)
                || entry.first_path > entry.last_path
                || entry.terms as usize > MAX_TERMS
                || entry.postings as usize > MAX_POSTINGS
                || entry.terms > entry.postings
                || entry.source_bytes > MAX_SOURCE_BYTES as u64
                || previous.is_some_and(|old| {
                    old.last_id >= entry.first_id || old.last_path >= entry.first_path
                })
            {
    ...
    pub(super) fn document_count(&self) -> usize {
        self.segments.iter().map(|s| s.documents as usize).sum()
    }
    pub(super) fn source_bytes(&self) -> usize {
        self.segments.iter().map(|s| s.source_bytes as usize).sum()
    }
    pub(super) fn payloads(
        ...
        check(live)?;
        let mut documents = Encoder::new();
        namespace_write(&mut documents, self.source.namespace);
        documents.write_scalar(self.segments.len() as u32);
        let mut postings = Encoder::new();
        namespace_write(&mut postings, self.source.namespace);
        ...
        for segment in &self.segments {
            ...
            documents.write_digest(&segment.root)?;
            documents.write_scalar(segment.documents);
            documents.write_scalar(segment.first_id);
            documents.write_scalar(segment.last_id);
            documents.write_bytes("first path", &segment.first_path)?;
            documents.write_bytes("last path", &segment.last_path)?;
            postings.write_digest(&segment.root)?;
            postings.write_scalar(segment.terms);
            postings.write_scalar(segment.postings);
            if documents.len() > MAX_SEGMENT_BYTES {
                return Err(LexicalError::Limit("document catalog bytes").into());
            }
        }
        let mut evidence = Encoder::new();
        source_write(&mut evidence, &self.source)?;
        evidence.write_digest(&profile_root()?)?;
        evidence.write_scalar(self.document_count() as u64);
        evidence.write_scalar(self.source_bytes() as u64);
        evidence.write_scalar(self.non_regular_entries as u64);
        evidence.write_scalar(self.segments.len() as u32);
        let mut manifest = Encoder::new();
        source_write(&mut manifest, &self.source)?;
        manifest.write_scalar(self.non_regular_entries as u32);
        manifest.write_scalar(self.segments.len() as u32);
        for segment in &self.segments {
            check(live)?;
            segment.write(&mut manifest)?;
            if manifest.len() > MAX_SEGMENT_BYTES {
                return Err(LexicalError::Limit("manifest bytes").into());
            }
        }
        let result = vec![
            payload("documents", DocumentsFrame(documents.into_bytes()))?,
            payload("postings", PostingsFrame(postings.into_bytes()))?,
            payload("evidence", EvidenceFrame(evidence.into_bytes()))?,
    ...
    pub(super) fn decode(
        ...
        if payload_root(&frame)? != expected || encode_body(&frame)? != raw {
        ...
        if source.namespace != namespace {
            return Err(LexicalError::NamespaceMismatch.into());
        }
        ...
        for _ in 0..n {
            check(live)?;
            segments.push(SegmentRef::read(&mut input)?);
        }
        input.finish()?;
...
fn source_write(out: &mut Encoder, source: &LexicalSource) -> Result<(), IndexError> {
    namespace_write(out, source.namespace);
    out.write_bytes("source ref", source.reference.as_bytes())?;
    ...
    out.write_raw(source.commit.as_bytes());
    out.write_raw(source.tree.as_bytes());
    Ok(())
}
fn source_read(input: &mut Decoder<'_>) -> Result<LexicalSource, IndexError> {
    ...
    let commit = oid_read(input, namespace.object_format)?;
    let tree = oid_read(input, namespace.object_format)?;
    Ok(LexicalSource {
        namespace,
        reference,
        source_head,
