...
impl LexicalReuse {
    fn empty(selection: &LexicalSelection) -> Self {
        Self {
            source: selection.source().clone(), activation: selection.activation().clone(),
            dictionary: Vec::new(), documents: BTreeMap::new(),
            payload_bytes: selection.payload_bytes, generation_bytes: selection.generation_bytes,
            retained_bytes: 0,
        }
    }

    #[must_use]
    pub const fn source(&self) -> &LexicalSource { &self.source }
    #[must_use]
    ...
    pub fn document_bytes(&self, path: &[u8], blob: GitOid) -> Option<usize> {
        self.documents.get(path).filter(|row| row.blob == blob).map(|row| row.content_bytes)
    }

    fn retain(&mut self, amount: usize) -> Result<(), LexicalError> {
        // Bound modeled metadata, not platform-dependent allocator/RSS bytes.
        bounded_add(&mut self.retained_bytes, amount, MAX_INDEX_BYTES * 2, "refresh metadata")
    }

    fn append(&mut self, segment: LexicalSegment, live: &mut impl FnMut() -> bool) -> Result<(), IndexError> {
        check(live)?;
        if segment.namespace != self.source.namespace { return Err(IndexError::SourceMismatch); }
        let mut rows: Vec<_> = segment.documents.iter().map(|d| SavedDocument {
            blob: d.blob, content_bytes: d.content_bytes as usize,
            postings: Vec::new(), copy_bytes: 64 + d.path.len(),
        }).collect();
        ...
        for (term, columns) in segment.terms {
            ...
            for (id, offset) in columns.documents.into_iter().zip(columns.offsets) {
                check(live)?;
                let at = segment.documents.binary_search_by_key(&id, |d| d.id)
                    .map_err(|_| LexicalError::Invalid("refresh posting document"))?;
                self.retain(12)?;
                rows[at].postings.push((index, offset));
                bounded_add(&mut rows[at].copy_bytes, 25 + term_bytes, MAX_INDEX_BYTES * 2, "refresh document work")?;
            }
        }
    ...
    fn reused(&self, row: RefreshDocument<'_>) -> Result<&SavedDocument, LexicalError> {
        self.documents.get(row.path).filter(|saved| saved.blob == row.blob)
            .ok_or(LexicalError::Invalid("refresh requires exact prior path and blob"))
    }

    fn singleton(&self, row: RefreshDocument<'_>, id: u64,
        live: &mut impl FnMut() -> bool,
    ) -> Result<LexicalSegment, LexicalError> {
        check(live)?;
        if let Some(content) = row.content {
            // Keep native verification and tokenization on the existing engine.
            return LexicalSegment::build(self.source.namespace, id,
                [SourceDocument { path: row.path, blob: row.blob, content }], live);
        }
        let old = self.reused(row)?;
        let mut terms = BTreeMap::new();
        for &(term, offset) in &old.postings {
            check(live)?;
            terms.insert(self.dictionary[term].clone(), Postings { documents: vec![id], offsets: vec![offset] });
        }
        Ok(LexicalSegment { namespace: self.source.namespace,
            documents: vec![IndexedDocument { id, path: row.path.to_vec(), blob: row.blob,
                content_bytes: old.content_bytes as u32 }], terms })
    }

    fn segment(&self, rows: &[RefreshDocument<'_>], first: usize, work: &mut usize,
        live: &mut impl FnMut() -> bool,
    ) -> Result<LexicalSegment, LexicalError> {
        let mut result = LexicalSegment { namespace: self.source.namespace,
            documents: Vec::new(), terms: BTreeMap::new() };
        ...
        for (offset, &row) in rows.iter().enumerate() {
            ...
            let document = single.documents.into_iter().next().ok_or(LexicalError::Invalid("empty refresh document"))?;
            ...
            for (term, columns) in single.terms {
                ...
                let target = match result.terms.entry(term) {
                    ...
                    Entry::Vacant(entry) => {
                        ...
                        bounded_add(&mut encoded_bound, 13 + entry.key().bytes.len(), MAX_SEGMENT_BYTES, "segment bytes")?;
                        entry.insert(Postings::default())
                    }
                };
                bounded_add(&mut encoded_bound, 12, MAX_SEGMENT_BYTES, "segment bytes")?;
                target.documents.extend(columns.documents); target.offsets.extend(columns.offsets);
            }
        }
        result.encode(live)?;
        check(live)?;
    ...
    pub fn prepare(&self, source: LexicalSource, rows: &[RefreshDocument<'_>], excluded: usize,
        live: &mut impl FnMut() -> bool,
        ...
        if rows.len() > MAX_INDEX_DOCUMENTS { return Err(LexicalError::Limit("indexed documents").into()); }
        let mut stats = LexicalRefreshStats { previous_payload_bytes_read: self.payload_bytes,
            previous_generation_bytes_read: self.generation_bytes, ..Default::default() };
        ...
        for &row in rows {
            ...
            let bytes = match row.content {
                Some(bytes) => {
                    ...
                    bounded_add(&mut stats.rebuilt_source_bytes, bytes.len(), MAX_SOURCE_BYTES, "source bytes")?;
                    bytes.len()
                }
                None => {
                    let bytes = self.reused(row)?.content_bytes;
                    stats.reused_documents += 1;
                    bounded_add(&mut stats.reused_source_bytes, bytes, MAX_SOURCE_BYTES, "source bytes")?;
                    bytes
                }
            };
            if bytes > MAX_FILE_BYTES { return Err(IndexError::Lexical(LexicalError::Limit("file bytes"))); }
        ...
        for (group, chunk) in rows.chunks(MAX_DOCUMENTS).enumerate() {
            ...
            while let Some((first, input)) = pending.pop() {
                check(live)?;
                attempts += 1;
                if attempts > 256 { return Err(IndexError::Lexical(LexicalError::Limit("segment build attempts"))); }
                match self.segment(input, first, &mut stats.build_work_bytes, live) {
                    Ok(segment) => {
                        if parts.len() == MAX_SEGMENTS { return Err(LexicalError::Limit("index segments").into()); }
                        bounded_add(&mut encoded_bytes, segment.encode(live)?.len(), MAX_INDEX_BYTES, "index bytes")?;
                        parts.push(segment);
                    }
                    Err(LexicalError::Limit("documents" | "dictionary terms" | "postings" | "segment bytes")) if input.len() > 1 => {
...
impl<S> LexicalIndexStore<'_, S> {
    fn start_refresh(&self, selection: &LexicalSelection, instance: StoreInstanceId,
        limits: LexicalReadLimits, live: &mut impl FnMut() -> bool,
    ) -> Result<LexicalReuse, IndexError> {
        limits.validate()?; check(live)?; self.check_source(selection.source())?;
        if selection.store_instance != instance { return Err(IndexError::SourceMismatch); }
        if selection.payload_bytes > limits.max_payload_bytes || selection.manifest.segments.len() > limits.max_segments {
...
fn observe(base: &mut LexicalReuse, reference: &SegmentRef, read: ImmutableRead,
    limits: LexicalReadLimits, live: &mut impl FnMut() -> bool,
) -> Result<(), IndexError> {
    check(live)?;
...
impl<S: AuthorityStore> LexicalIndexStore<'_, S> {
    ...
    pub fn load_refresh_base(&self, selection: &LexicalSelection, limits: LexicalReadLimits,
        live: &mut impl FnMut() -> bool,
    ) -> Result<LexicalReuse, IndexError> {
        ...
        for reference in &selection.manifest.segments {
            ...
            let read = self.store.read_immutable(&payload_key(self.namespace, "segment", reference.root)?)?;
...
impl<S: AsyncAuthorityStore> LexicalIndexStore<'_, S> {
    ...
    pub async fn load_refresh_base_async(&self, cx: &S::Context, selection: &LexicalSelection,
        limits: LexicalReadLimits, live: &mut (impl FnMut() -> bool + Send),
    ) -> Result<LexicalReuse, IndexError> {
        let mut base = self.start_refresh(selection, self.store.instance_id(), limits, live)?;
        for reference in &selection.manifest.segments {
            before_read(base.payload_bytes, limits, live)?;
            let read = self.store.read_immutable(cx, &payload_key(self.namespace, "segment", reference.root)?).await?;
            observe(&mut base, reference, read, limits, live)?;
        }
        check(live)?; Ok(base)
    }
