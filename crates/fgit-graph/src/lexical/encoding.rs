...
impl CanonicalBody for SegmentFrame {
    ...
    fn write_payload(&self, out: &mut Encoder) -> Result<(), CodecRefusal> {
        out.write_bytes("lexical.segment", &self.0)
    }
...
pub(super) const fn decode_limits() -> DecodeLimits {
    DecodeLimits {
        frame_bytes: MAX_SEGMENT_BYTES as u64,
        byte_string_bytes: MAX_SEGMENT_BYTES as u64,
        elements: MAX_POSTINGS as u64,
        depth: 8,
    }
}
pub(super) fn namespace_write(out: &mut Encoder, scope: LexicalNamespace) {
    out.write_opaque_id(scope.tenant.as_bytes());
    out.write_opaque_id(scope.repository.as_bytes());
    out.write_opaque_id(scope.incarnation.as_bytes());
    out.write_scalar(match scope.object_format {
        GitHashAlgorithm::Sha1 => 1u8,
        GitHashAlgorithm::Sha256 => 2u8,
    });
}
pub(super) fn namespace_read(input: &mut Decoder<'_>) -> Result<LexicalNamespace, LexicalError> {
    ...
    let incarnation =
        RepositoryIncarnationId::from_bytes(input.read_opaque_id("lexical.incarnation")?);
...
pub(super) fn oid_read(
    input: &mut Decoder<'_>,
    format: GitHashAlgorithm,
) -> Result<GitOid, LexicalError> {
    let oid = match format {
        GitHashAlgorithm::Sha1 => {
            let mut bytes = [0; 20];
            bytes.copy_from_slice(input.take("lexical.oid", 20)?);
            GitOidSha1::from_bytes(bytes).into()
        }
        GitHashAlgorithm::Sha256 => {
            let mut bytes = [0; 32];
            bytes.copy_from_slice(input.take("lexical.oid", 32)?);
...
pub(super) fn count(
    ...
    if n > maximum || n > input.remaining() / minimum_bytes {
        return Err(LexicalError::Limit(name));
    }
    Ok(n)
}
...
fn frame(
...
) -> Result<SegmentFrame, LexicalError> {
    check(live)?;
    let mut out = Encoder::new();
    namespace_write(&mut out, segment.namespace);
    ...
    for document in &segment.documents {
        ...
        bounded_add(
            &mut total,
            document.content_bytes as usize,
            MAX_SOURCE_BYTES,
            "source bytes",
        )?;
        out.write_scalar(document.id);
        out.write_bytes("lexical.path", &document.path)?;
        out.write_raw(document.blob.as_bytes());
        out.write_scalar(document.content_bytes);
        if out.len() > MAX_SEGMENT_BYTES {
            return Err(LexicalError::Limit("segment bytes"));
        }
    }
    out.write_scalar(segment.terms.len() as u32);
    for (term, columns) in &segment.terms {
        check(live)?;
        out.write_scalar(match term.channel {
            LexicalChannel::Content => 0u8,
            LexicalChannel::Path => 1u8,
        });
        out.write_bytes("lexical.term", &term.bytes)?;
        out.write_scalar(columns.documents.len() as u32);
...
pub(super) fn root(
    segment: &LexicalSegment,
    live: &mut impl FnMut() -> bool,
) -> Result<Digest, LexicalError> {
    let frame = frame(segment, live)?;
    if encode_body(&frame)?.len() > MAX_SEGMENT_BYTES {
        return Err(LexicalError::Limit("segment bytes"));
    }
    let root = payload_root(&frame)?;
    check(live)?;
    Ok(root)
}
pub(super) fn decode(
    ...
    if payload_root(&frame)? != expected || encode_body(&frame)? != bytes {
        return Err(LexicalError::CommitmentMismatch);
    }
    check(live)?;
    let mut input = Decoder::new(&frame.0, decode_limits());
    if namespace_read(&mut input)? != namespace {
        return Err(LexicalError::NamespaceMismatch);
    }
    let n = count(
        &mut input,
        MAX_DOCUMENTS,
        17 + namespace.object_format.digest_len(),
        "document count",
    )?;
    if n == 0 {
        return Err(LexicalError::Invalid("empty segment"));
    }
    ...
    for _ in 0..n {
        ...
        if id == 0
            ...
            || documents
                .last()
                .is_some_and(|d| d.id >= id || d.path.as_slice() >= path)
        {
            return Err(LexicalError::Invalid("document order"));
        }
        let blob = oid_read(&mut input, namespace.object_format)?;
        if blob.is_zero() {
            return Err(LexicalError::Invalid("zero blob"));
        }
        let content_bytes = input.read_scalar::<u32>("content length")?;
        if content_bytes as usize > MAX_FILE_BYTES {
            return Err(LexicalError::Limit("file bytes"));
        }
        bounded_add(
            &mut total_source,
            content_bytes as usize,
            MAX_SOURCE_BYTES,
            "source bytes",
        )?;
    ...
    for _ in 0..n {
        ...
        let channel = match input.read_scalar::<u8>("term channel")? {
            0 => LexicalChannel::Content,
            1 => LexicalChannel::Path,
            _ => return Err(LexicalError::Invalid("term channel")),
        };
        let bytes = input.read_bytes("term bytes")?;
        if bytes.is_empty()
            || bytes.len() > MAX_TERM_BYTES
            || bytes.iter().any(|b| !word(*b) || b.is_ascii_uppercase())
        {
        ...
        if terms
            .last_key_value()
            .is_some_and(|(last, _)| last >= &term)
        {
            return Err(LexicalError::Invalid("dictionary order"));
        }
        let n = count(
            &mut input,
            MAX_POSTINGS - total_postings,
            12,
            "posting count",
        )?;
        if n == 0 {
            return Err(LexicalError::Invalid("empty posting list"));
        }
        total_postings += n;
        let mut columns = Postings {
            documents: Vec::with_capacity(n),
        ...
        for id in &columns.documents {
            ...
            let at = documents
                ...
                .map_err(|_| LexicalError::Invalid("posting document"))?;
            let length = match channel {
                LexicalChannel::Content => documents[at].content_bytes as usize,
