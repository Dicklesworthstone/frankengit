...
use fgit_authority::{
    AsyncAuthorityStore, AuthorityFailure, AuthorityStore, HeadKey, ImmutableKey, ImmutableRead,
    KeyError, PutOutcome, StoreInstanceId,
};
use fgit_types::{
    RefName, RepositoryAuthorityHeadId, RepositoryCommitId, SchemaFamily, SchemaId, TypeRefusal,
};
...
impl From<LexicalError> for IndexError {
    fn from(e: LexicalError) -> Self {
        Self::Lexical(e)
    }
}
impl From<fgit_codec::CodecRefusal> for IndexError {
    fn from(e: fgit_codec::CodecRefusal) -> Self {
        Self::Lexical(e.into())
    }
}
impl From<GenerationAuthorityError> for IndexError {
    fn from(e: GenerationAuthorityError) -> Self {
        Self::Generation(e)
    }
}
impl From<AuthorityFailure> for IndexError {
    fn from(e: AuthorityFailure) -> Self {
        Self::Authority(e)
    }
}
impl From<KeyError> for IndexError {
    fn from(e: KeyError) -> Self {
        Self::Key(e)
    }
}
impl From<TypeRefusal> for IndexError {
...
impl std::fmt::Display for IndexError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "stored lexical index refused: {self:?}")
    }
}
...
impl PreparedLexicalIndex {
    pub fn new(
        ...
        for segment in segments {
            check(live)?;
            if segment.namespace() != source.namespace {
...
impl Default for LexicalReadLimits {
    fn default() -> Self {
        Self {
            ...
            max_payload_bytes: MAX_INDEX_BYTES,
...
impl LexicalReadLimits {
    fn validate(self) -> Result<(), IndexError> {
        ...
        if self.max_segments == 0
            ...
            || self.max_payload_bytes > MAX_INDEX_BYTES
        {
...
impl<'a, S> LexicalIndexStore<'a, S> {
    pub fn new(
        ...
        Ok(Self {
            ...
            namespace,
            reference,
            head_key: HeadKey::new(key)?,
            view: GraphViewId::try_new(VIEW)?,
        })
    }
    fn check_source(&self, source: &LexicalSource) -> Result<(), IndexError> {
        if source.namespace != self.namespace || source.reference != self.reference {
    ...
    fn generation(
    ...
    ) -> Result<GraphGenerationBody, IndexError> {
        ...
        Ok(GraphGenerationBody::new(
            ...
            GraphSourceStamp {
                source_rcr_id: source.source_rcr,
                source_forge_position_root: source.forge_position_root,
                builder_profile: BuilderProfileId::try_new(PROFILE.as_bytes())?,
                parser_model_root: manifest::profile_root()?,
            },
            root("documents")?,
            root("postings")?,
            root("manifest")?,
    ...
    fn check_intake(
        ...
        if prepared
            .metadata
            .iter()
            .chain(&prepared.segments)
            .any(|p| p.bytes.len() > maximum)
        {
    ...
    fn begin_selection(
        ...
        raw: &[u8],
        live: &mut impl FnMut() -> bool,
    ) -> Result<(Manifest, Vec<Payload>), IndexError> {
        if body.graph_view_id() != self.view
            || body.graph_schema_id() != manifest::schema()
        ...
        let metadata = manifest.payloads(live)?;
        for (kind, root) in [
            ("documents", *body.vertices_root()),
            ("postings", *body.edges_root()),
            ("manifest", *body.index_manifest_root()),
            ("evidence", *body.evidence_root()),
        ] {
            if !metadata.iter().any(|p| p.kind == kind && p.root == root) {
...
fn key_prefix(namespace: LexicalNamespace, kind: &str) -> Vec<u8> {
    ...
    out.extend_from_slice(namespace.tenant.as_bytes());
    out.extend_from_slice(namespace.repository.as_bytes());
    out.extend_from_slice(namespace.incarnation.as_bytes());
    out.push(match namespace.object_format {
...
fn payload_key(
    ...
    kind: &str,
    root: Digest,
) -> Result<ImmutableKey, IndexError> {
...
fn payload(
    ...
    let ImmutableRead::Present(body) = read else {
        return Err(IndexError::MissingPayload(root));
    };
    if body.len() > MAX_SEGMENT_BYTES {
        return Err(LexicalError::Limit("payload bytes").into());
    }
    bounded_add(
        bytes,
        body.len(),
...
impl<S: AuthorityStore> LexicalIndexStore<'_, S> {
    ...
    pub fn publish(
        ...
        check(live)?;
        self.check_intake(prepared, self.store.limits().body_bytes)?;
        ...
        for body in prepared.segments.iter().chain(&prepared.metadata) {
            ...
            accepted_put(
                self.store.put_if_absent(
                    &payload_key(self.namespace, body.kind, body.root)?,
                    &body.bytes,
                )?,
                body.root,
            )?;
    ...
    pub fn select(
        ...
        let (activation, selected_head, body, generation_bytes) = if let Some(expected) = expected {
            let selected =
                authority.read_at(self.view, expected, minimum, limits.generation, live)?;
            (
                selected.activation().clone(),
                selected.selected_head().clone(),
                selected.body().clone(),
                selected.bytes_read(),
            )
        } else {
            let selected = authority
                .read_active(self.view, minimum, limits.generation, live)?
                .ok_or(IndexError::Uninitialized)?;
            (
                selected.activation().clone(),
                selected.activation().clone(),
                selected.body().clone(),
    ...
    pub fn search(
        ...
        let mut scan = Scan::new(selection, read_limits, query_limits)?;
        self.check_source(selection.source())?;
        if selection.store_instance != self.store.instance_id() {
            return Err(IndexError::SourceMismatch);
        }
        for reference in &selection.manifest.segments {
            ...
            if scan.observe(reference, &bytes, self.namespace, query, after, live)? {
                break;
            }
        }
        check(live)?;
        Ok(scan.finish(selection, query))
    }
}
...
impl<S: AsyncAuthorityStore> LexicalIndexStore<'_, S> {
    ...
    pub async fn recover_async(
        ...
        Ok(GenerationAuthority::new(self.store, self.head_key.clone())
            .recover_activation_async(cx, self.view, candidate, minimum, limits, live)
            .await?)
    }
    pub async fn publish_async(
        ...
        check(live)?;
        Ok(GenerationAuthority::new(self.store, self.head_key.clone())
            .stage_and_activate_async(cx, &generation)
            .await?)
    }
    pub async fn select_async(
        ...
        let (activation, selected_head, body, generation_bytes) = if let Some(expected) = expected {
            let selected = authority
                .read_at_async(cx, self.view, expected, minimum, limits.generation, live)
                .await?;
            (
                selected.activation().clone(),
                selected.selected_head().clone(),
            ...
            let selected = authority
                .read_active_async(cx, self.view, minimum, limits.generation, live)
                .await?
                .ok_or(IndexError::Uninitialized)?;
            (
                selected.activation().clone(),
        ...
        let raw = payload(
            self.store
                .read_immutable(
                    cx,
                    &payload_key(self.namespace, "manifest", *body.index_manifest_root())?,
                )
                .await?,
            *body.index_manifest_root(),
            &mut bytes,
        ...
        for expected in metadata.iter().filter(|p| p.kind != "manifest") {
            ...
            let raw = payload(
                self.store
                    .read_immutable(
                        cx,
                        &payload_key(self.namespace, expected.kind, expected.root)?,
                    )
                    .await?,
                expected.root,
                &mut bytes,
...
impl Scan {
    fn new(
        ...
        Ok(Self {
            hits: Vec::new(),
            budget: QueryBudget::new(query)?,
            bytes: selection.payload_bytes,
            segments: 0,
            more: false,
        })
    }
    ...
    fn finish(self, selection: &LexicalSelection, query: &LexicalQuery) -> IndexedLexicalReport {
        IndexedLexicalReport {
            ...
            generation: selection.activation.clone(),
            selected_generation_head: selection.selected_head.clone(),
            query: query.clone(),
            results: LexicalReport {
                next_after: if self.more {
                    self.hits.last().map(|h| h.document_id)
                } else {
                    None
                },
                hits: self.hits,
                complete: !self.more,
                work_units: self.budget.work,
            },
            indexed_documents: selection.manifest.document_count(),
            indexed_source_bytes: selection.manifest.source_bytes(),
            non_regular_entries: selection.manifest.non_regular_entries,
            segments_read: self.segments,
            payload_bytes_read: self.bytes,
            generation_bytes_read: selection.generation_bytes,
        }
    }
}
