...
fn build(format: GitHashAlgorithm, first: u64, rows: &[(&[u8], &[u8])]) -> LexicalSegment {
    LexicalSegment::build(
        scope(format),
        first,
        rows.iter().map(|(path, content)| SourceDocument {
            path,
            content,
...
fn query(channel: LexicalChannel, terms: &[&[u8]], paths: &[&[u8]]) -> LexicalQuery {
    LexicalQuery::new(
        channel,
        &terms.iter().map(|x| x.to_vec()).collect::<Vec<_>>(),
        &paths.iter().map(|x| x.to_vec()).collect::<Vec<_>>(),
    )
    .unwrap()
}
fn find(segment: &LexicalSegment, terms: &[&[u8]]) -> LexicalReport {
    segment
        .search(
            &query(LexicalChannel::Content, terms, &[]),
            None,
            Default::default(),
            &mut || true,
        )
...
fn native_id_and_complete_token_semantics_for_both_formats() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        let s = build(
            ...
            71,
            &[
                (b"a.rs", b"Alpha alpha\r\nbeta"),
                (b"b.rs", b"alphabet beta"),
            ],
        );
        let answer = find(&s, &[b"BETA", b"ALPHA", b"alpha"]);
        assert!(answer.complete);
        assert_eq!(answer.hits.len(), 1);
        assert_eq!(answer.hits[0].document_id, 71);
        assert_eq!(answer.hits[0].path, b"a.rs");
        assert_eq!(
            answer.hits[0].spans,
            vec![
                LexicalSpan {
        ...
        let bytes = s.encode(&mut || true).unwrap();
        let root = s.root(&mut || true).unwrap();
        assert_eq!(
            LexicalSegment::decode(&bytes, root, scope(format), &mut || true).unwrap(),
            s
        );
        assert_eq!(
            s.documents()[0].blob,
            git_object_id(format, GitObjectKind::Blob, b"Alpha alpha\r\nbeta")
        );
    }
}
#[test]
fn binary_paths_empty_files_and_non_ascii_keep_original_byte_spans() {
    ...
    let paths = s
        .search(
            &query(LexicalChannel::Path, &[b"empty"], &[]),
            None,
            Default::default(),
            &mut || true,
        )
        .unwrap();
    assert_eq!(paths.hits[0].content_bytes, 0);
    assert_eq!(paths.hits[0].spans[0].byte_length, 5);
}
#[test]
fn path_filters_use_components_not_string_prefixes() {
    let s = build(
        ...
        1,
        &[(b"src/a", b"term"), (b"src2/a", b"term")],
    );
    let q = query(LexicalChannel::Content, &[b"term"], &[b"src"]);
    assert_eq!(
        s.search(&q, None, Default::default(), &mut || true)
            .unwrap()
            .hits
            .len(),
        1
    ...
    assert_eq!(
        s.search(
            &query(LexicalChannel::Path, &[b"src2"], &[]),
            None,
            Default::default(),
            &mut || true
        )
...
fn compaction_is_byte_identical_to_rebuild_without_renumbering() {
    let rows: Vec<(Vec<u8>, Vec<u8>)> = (0..25)
        .map(|i| {
            (
                format!("p/{i:03}.rs").into_bytes(),
                format!("COMMON symbol_{} repeated repeated", i % 7).into_bytes(),
            )
        })
        .collect();
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for size in [1, 2, 5, 8, 25] {
            let segments: Vec<_> = borrowed
                .chunks(size)
                .enumerate()
                .map(|(i, rows)| build(format, 1000 + (i * size) as u64, rows))
                .collect();
            let compacted = LexicalSegment::concatenate(&segments, &mut || true).unwrap();
            assert_eq!(compacted, full);
            assert_eq!(
                compacted.encode(&mut || true).unwrap(),
                full.encode(&mut || true).unwrap()
            );
            assert_eq!(
                compacted.root(&mut || true).unwrap(),
                full.root(&mut || true).unwrap()
            );
            assert_eq!(
                find(&compacted, &[b"COMMON", b"symbol_3"]).hits,
                find(&full, &[b"common", b"symbol_3"]).hits
            );
        }
    }
}
...
fn compaction_rejects_overlap_reordering_duplicate_paths_and_foreign_scope() {
    ...
    let mut foreign = b;
    foreign.namespace.incarnation = RepositoryIncarnationId::from_bytes([9; 16]);
    assert!(matches!(
        LexicalSegment::concatenate(&[a, foreign], &mut || true),
        Err(LexicalError::NamespaceMismatch)
    ));
}
#[test]
fn pagination_observes_an_extra_hit_before_reporting_a_limit() {
    ...
    let last = s
        .search(&q, first.next_after, limits, &mut || true)
        .unwrap();
    assert!(last.complete);
    assert_eq!(last.next_after, None);
    assert_eq!(last.hits[0].document_id, 9);
    let exact = s.search(&q, Some(7), limits, &mut || true).unwrap();
    assert!(exact.complete);
    assert_eq!(exact.hits.len(), 2);
    assert!(
        s.search(&q, Some(u64::MAX), limits, &mut || true)
            .unwrap()
            .hits
            .is_empty()
    );
...
fn rarest_posting_intersection_agrees_with_an_independent_scalar_corpus() {
    let rows: Vec<(Vec<u8>, Vec<u8>)> = (0..64u64)
        .map(|mask| {
            let text = (0..6)
                .filter(|bit| mask & (1 << bit) != 0)
                .map(|bit| format!("W{bit} w{bit} "))
                .collect::<String>();
    ...
    for wanted in 1..64u64 {
        let terms: Vec<Vec<u8>> = (0..6)
            .filter(|bit| wanted & (1 << bit) != 0)
            .map(|bit| format!("w{bit}").into_bytes())
            .collect();
        let q = LexicalQuery::new(LexicalChannel::Content, &terms, &[]).unwrap();
        let report = s
            .search(&q, None, Default::default(), &mut || true)
            .unwrap();
        let expected: Vec<u64> = (0..64u64)
            .filter(|mask| mask & wanted == wanted)
            .map(|mask| 101 + mask)
            .collect();
        assert_eq!(
            report
        ...
        for hit in report.hits {
            ...
            for span in hit.spans {
                assert_eq!(
                    source[span.byte_offset as usize
                        ..span.byte_offset as usize + span.byte_length as usize]
                        .to_ascii_lowercase(),
                    q.terms()[span.query_index]
                );
            }
        }
    }
...
fn decoder_rejects_substitution_truncation_suffix_and_cross_namespace() {
    ...
    for end in 0..bytes.len() {
        assert!(LexicalSegment::decode(&bytes[..end], root, s.namespace(), &mut || true).is_err());
    }
    ...
    assert!(matches!(
        LexicalSegment::decode(
            &other.encode(&mut || true).unwrap(),
            root,
            s.namespace(),
            &mut || true
        ),
        Err(LexicalError::CommitmentMismatch)
    ));
    let mut foreign = s.namespace();
    foreign.tenant = TenantId::from_bytes([9; 16]);
    assert!(matches!(
        LexicalSegment::decode(&bytes, root, foreign, &mut || true),
        Err(LexicalError::NamespaceMismatch)
    ));
}
#[test]
fn decoder_validates_structure_even_when_the_payload_digest_matches() {
    ...
    for variant in 0..5 {
        ...
        let bytes = malformed.encode(&mut || true).unwrap();
        let root = malformed.root(&mut || true).unwrap();
...
fn token_and_query_limits_have_permitted_boundary_twins() {
    let good = vec![b'a'; MAX_TERM_BYTES];
    let bad = vec![b'a'; MAX_TERM_BYTES + 1];
    assert_eq!(
        find(
            &build(GitHashAlgorithm::Sha1, 1, &[(b"a", &good)]),
            &[&good]
        )
        .hits
        .len(),
...
fn cancellation_and_work_exhaustion_are_not_complete_empty_answers() {
    ...
    assert!(matches!(
        s.search(&q, None, Default::default(), &mut || false),
        Err(LexicalError::Cancelled)
    ));
    assert!(matches!(
        s.search(
            &q,
        ...
        Err(LexicalError::Limit("query work"))
    ));
    let good = s
        .search(&q, None, Default::default(), &mut || true)
        .unwrap();
    ...
    assert!(matches!(
        LexicalSegment::build(
            ...
            [SourceDocument {
                path: b"a",
                content: &body,
                blob: git_object_id(GitHashAlgorithm::Sha1, GitObjectKind::Blob, &body)
            }],
            &mut || {
                checks += 1;
                checks < 5
    ...
    assert!(s.encode(&mut || false).is_err());
    assert!(LexicalSegment::concatenate(&[s], &mut || false).is_err());
}
