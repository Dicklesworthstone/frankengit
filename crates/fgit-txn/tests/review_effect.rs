...
use fgit_reference::intent::{
    ...
    Intent, OutboxDeliveryKey, OutboxIntent,
};
...
fn reviewer_streams_fold_without_moving_refs_or_advancing_the_pr() {
    ...
    let streams = [
        ForgeStreamId::new(label("review/17/a")),
        ForgeStreamId::new(label("review/17/b")),
    ];
    let events = streams.map(|stream| ForgeEventKind::PullRequestReviewed {
        review: ForgeEntityId::new(stream.label()),
        target: target.clone(),
    });
    ...
    let request = RequestBuilder::new(
        ...
        repository,
        principal,
        SchemaId::new(SchemaFamily::from_static("fgit/review-test"), 1, 0),
        IdempotencyKey::new(label("reviews")),
    )
    .statement(
    ...
    assert_eq!(
        effects.forge,
        BTreeMap::from([
            (streams[0], vec![events[0].clone()]),
            (streams[1], vec![events[1].clone()])
        ])
    );
    ...
    assert!(
        events
            .iter()
            .all(|event| event.required_ref_effect().is_none())
    );
    let encoded = canonical_fold_bytes(&request, &report).unwrap();
    assert_eq!(
        encoded,
        canonical_fold_bytes(&request, &IntentEvaluator.evaluate(basis, &request)).unwrap()
    );
    ...
    let changed = BTreeMap::from([
        (streams[0], ForgeStreamPosition::new(1)),
        (pr, ForgeStreamPosition::new(4)),
    ]);
    let stale = IntentEvaluator.evaluate(
        FoldBasis {
            forge_positions: &changed,
            ..basis
        },
        &request,
    );
    ...
    assert!(
        stale.effects().is_none(),
        "one stale reviewer precondition cannot publish the other effects"
    );
}
...
fn review_kind_survives_complete_trace_codec_and_replay_without_relabeling_metadata() {
    ...
    let request = RequestBuilder::new(
        ...
        mint.principal(),
        SchemaId::new(SchemaFamily::from_static("fgit/review-test"), 1, 0),
        IdempotencyKey::new(label("review-codec")),
    )
    .statement(
        MismatchPolicy::TxnAbort,
        vec![Intent::Forge(ForgeIntent {
            stream: ForgeStreamId::new(label("review/17/a")),
            expected_position: ForgeStreamPosition::GENESIS,
            event: ForgeEventKind::PullRequestReviewed {
                review: ForgeEntityId::new(label("review/17/a")),
    ...
    assert!(matches!(
        trace.steps[0].observed,
        fgit_reference::trace::ObservedOutcome::SealRejected(_)
    ));
