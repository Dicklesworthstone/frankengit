...
fn path(bytes: &[u8], at: usize, limit: usize) -> Result<Vec<u8>, PatchError> {
    ...
    let decoded = if bytes.first() == Some(&b'"') {
        ...
        if used != bytes.len() { return Err(syntax(at, "trailing rename path bytes")); }
        decoded
    } else {
...
fn consume<'a>(bytes: &'a [u8], prefix: &[u8], expected: &[u8], at: usize)
    -> Result<&'a [u8], PatchError>
{
    if bytes.first() == Some(&b'"') {
        ...
        bytes.strip_prefix(prefix).and_then(|rest| rest.strip_prefix(expected))
            .ok_or_else(|| syntax(at, "rename paths disagree"))
    }
}

pub(super) fn scan(records: &[&[u8]], start: usize, header: &[u8], limit: usize,
    cancelled: &dyn Fn() -> bool) -> Result<Option<Rename>, PatchError>
{
    let (mut from, mut to, mut similarity) = (None, None, None);
    for (at, record) in records.iter().enumerate().skip(start + 1) {
        checkpoint(cancelled)?;
        if record.starts_with(b"diff --git ") || record.starts_with(b"--- ")
            || record.starts_with(b"@@ ") { break; }
        ...
        if let Some(value) = text.strip_prefix(b"rename from ") {
            ...
            let digits = value.strip_suffix(b"%").ok_or_else(|| syntax(at, "invalid similarity index"))?;
            let score = number(digits, at)?;
            if score > 100 { return Err(syntax(at, "invalid similarity index")); }
            set(&mut similarity, score, at)?;
        }
    }
    if from.is_none() && to.is_none() && similarity.is_none() { return Ok(None); }
    let (Some(from), Some(to)) = (from, to) else {
        return Err(syntax(start, "incomplete rename metadata"));
    };
    if from == to { return Err(syntax(start, "rename paths are identical")); }
    ...
    let rest = rest.strip_prefix(b" ").ok_or_else(|| syntax(start, "missing second diff path"))?;
    if !consume(rest, b"b/", &to, start)?.is_empty() {
        return Err(syntax(start, "trailing diff path bytes"));
    }
    Ok(Some(Rename { from, to, similarity }))
}
