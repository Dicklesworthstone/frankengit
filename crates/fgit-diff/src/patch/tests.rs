...
fn apply(patch: &[u8], old: &[u8]) -> Result<Vec<u8>, PatchError> {
    ...
    Ok(parsed.files()[0]
        .apply(Some((0o100644, old)), parsed.limits(), &|| false)?
        .unwrap()
        .content)
}
...
fn creation_deletion_and_executable_modes_enforce_presence() {
    ...
    assert_eq!(
        file.apply(None, parsed.limits(), &|| false).unwrap(),
        Some(PatchedFile {
            mode: 0o100755,
            content: b"#!/bin/sh\nexit 0\n".to_vec()
        })
    ...
    assert_eq!(
        file.apply(Some((0o100644, b"")), parsed.limits(), &|| false),
    ...
    assert_eq!(
        file.apply(None, parsed.limits(), &|| false),
        Err(PatchError::SourcePresence)
    );
    assert_eq!(
        file.apply(Some((0o100755, b"old\n")), parsed.limits(), &|| false),
    ...
    assert_eq!(
        parsed.files()[0]
            .apply(Some((0o100644, b"binary\0bytes")), parsed.limits(), &|| {
                false
            })
            .unwrap(),
        Some(PatchedFile {
            mode: 0o100755,
            content: b"binary\0bytes".to_vec()
        })
    );
}
...
fn hunkless_empty_file_changes_do_not_drop_unmentioned_source_bytes() {
    let parsed =
        parse(b"diff --git a/file b/file\nnew file mode 100644\nindex 0000000..e69de29\n").unwrap();
    assert_eq!(
        parsed.files()[0]
            .apply(None, parsed.limits(), &|| false)
            .unwrap()
            .unwrap()
            .content,
        b""
    ...
    assert_eq!(
        file.apply(Some((0o100644, b"secret\n")), parsed.limits(), &|| false),
        Err(PatchError::NonemptyDeletion)
    );
}
...
fn raw_paths_are_decoded_once_and_never_normalized_or_followed() {
    ...
    let parsed = parse(
        b"diff --git a/file with spaces b/file with spaces\nold mode 100644\nnew mode 100755\n",
    )
    .unwrap();
    ...
    assert!(
        parse(b"diff --git \"a/file\\400\" \"b/file\\400\"\nold mode 100644\nnew mode 100755\n")
            .is_err()
    );
}
...
fn duplicate_paths_prefix_collisions_and_renames_are_not_sequential_side_effects() {
    let section =
        |path| format!("diff --git a/{path} b/{path}\nold mode 100644\nnew mode 100755\n");
    assert_eq!(
        parse((section("x") + &section("x")).as_bytes()),
        Err(PatchError::DuplicatePath)
    );
    assert_eq!(
        parse((section("x/y") + &section("x")).as_bytes()),
        Err(PatchError::OverlappingPaths)
    );
    // 'a-' sorts between 'a' and 'a/x'; checking adjacent names is insufficient.
    assert_eq!(
        parse((section("a") + &section("a-") + &section("a/x")).as_bytes()),
        Err(PatchError::OverlappingPaths)
    );
    assert!(matches!(
        parse(b"diff --git a/old b/new\nsimilarity index 100%\nrename from old\nrename to new\n"),
        Err(PatchError::Unsupported { .. })
    ));
    let input = section("z") + &section("a");
...
fn each_parser_resource_limit_has_a_permitted_boundary_twin() {
    ...
    assert_eq!(
        ...
        Err(PatchError::Budget("patch files"))
    );
    let two = text(b"@@ -1 +1 @@\n-a\n+A\n@@ -3 +3 @@\n-c\n+C\n");
    assert!(
        UnifiedPatch::parse(
    ...
    assert_eq!(
        ...
        Err(PatchError::Budget("patch hunks"))
    );
    assert!(
        UnifiedPatch::parse(
            &input,
...
fn application_bounds_source_output_and_cancellation() {
    ...
    assert_eq!(
        UnifiedPatch::parse(&input, patch.limits(), &|| true),
...
fn identity_expectations_bind_prefixes_and_absent_sides_without_guessing_hash_domains() {
    ...
    assert!(IndexExpectation::matches(
        b"abcdef0",
        Some(b"abcdef0123456789")
    ));
    assert!(!IndexExpectation::matches(
        b"abcdef0",
        Some(b"abcdef1123456789")
    ));
    assert!(!IndexExpectation::matches(
        b"abcdef01234567890",
        Some(b"abcdef0123456789")
...
fn exhaustive_small_sequences_replay_independently_constructed_whole_file_hunks() {
    fn lines(mut n: usize, length: usize, newline: bool) -> Vec<u8> {
        ...
        for index in 0..length {
            result.push(b'a' + (n % 3) as u8);
            n /= 3;
            if index + 1 < length || newline {
                result.push(b'\n');
            }
        }
        result
    ...
    for old_n in 0..4u32 {
        for new_n in 0..4u32 {
            ...
            for old_id in 0..3usize.pow(old_n) {
                for new_id in 0..3usize.pow(new_n) {
                    for old_lf in [false, true] {
                        for new_lf in [false, true] {
                            let old = lines(old_id, old_n as usize, old_lf);
                            let new = lines(new_id, new_n as usize, new_lf);
                            let mut hunk = format!(
                                "@@ -{},{} +{},{} @@\n",
                                usize::from(old_n != 0),
                                old_n,
                            ...
                            .into_bytes();
                            record(&mut hunk, b'-', &old);
                            record(&mut hunk, b'+', &new);
                            assert_eq!(apply(&text(&hunk), &old).unwrap(), new);
                            cases += 1;
                        }
                    }
                }
...
fn application_cannot_reuse_a_parsed_patch_to_bypass_narrower_limits() {
    ...
    assert!(matches!(
        parsed.files()[0].apply(Some((0o100644, b"tail\nextra\n")), limits, &|| false),
        Err(PatchError::Budget("result lines"))
    ));
    let bytes = text(b"@@ -1 +1 @@\n-a\n+A\n@@ -3 +3 @@\n-c\n+C\n");
    let parsed = parse(&bytes).unwrap();
    assert!(matches!(
        parsed.files()[0].apply(
            Some((0o100644, b"a\nb\nc\n")),
            PatchLimits {
                max_hunks: 1,
                ..parsed.limits()
