...
fn relocation_requires_explicit_opt_in_and_preserves_exact_bytes_and_modes() {
    ...
    assert!(matches!(
        UnifiedPatch::parse(input.as_bytes(), PatchLimits::default(), &|| false),
    ...
    for bytes in [
        ...
        for mode in [0o100644, 0o100755] {
            assert_eq!(
                file.apply(Some((mode, bytes)), patch.limits(), &|| false)
                    .unwrap(),
                Some(PatchedFile {
                    mode,
    ...
    assert_eq!(
        file.apply(None, patch.limits(), &|| false),
        Err(PatchError::SourcePresence)
    );
    assert_eq!(
        file.apply(Some((0o120000, b"target")), patch.limits(), &|| false),
...
fn rename_can_apply_exact_hunks_index_expectations_and_mode_changes() {
    ...
    assert_eq!(
        file.index().unwrap(),
        &IndexExpectation {
            old: b"1234567".to_vec(),
            new: b"abcdef0".to_vec()
        }
    );
    assert_eq!(
        file.apply(Some((0o100644, b"same\nold\n")), patch.limits(), &|| false)
            .unwrap(),
        Some(PatchedFile {
            mode: 0o100755,
            content: b"same\nnew\n".to_vec()
    ...
    assert_eq!(
        file.apply(Some((0o100755, b"same\nold\n")), patch.limits(), &|| false),
        Err(PatchError::SourceMode)
    );
}
...
fn mode_only_rename_and_no_newline_edit_are_supported() {
    ...
    assert_eq!(
        patch.files()[0]
            .apply(Some((0o100644, b"contents")), patch.limits(), &|| false)
            .unwrap(),
        Some(PatchedFile {
            mode: 0o100755,
    ...
    assert_eq!(
        patch.files()[0]
            .apply(Some((0o100644, b"old")), patch.limits(), &|| false)
            .unwrap()
            .unwrap()
            .content,
        b"new"
...
fn identical_claim_never_permits_changed_content() {
    ...
    let patch = parse(input.as_bytes()).unwrap();
    assert!(matches!(
        patch.files()[0].apply(Some((0o100644, b"old\n")), patch.limits(), &|| false),
        Err(PatchError::Syntax {
            reason: "identical rename changed file content",
            ..
        })
    ));
}
...
fn rename_cannot_disguise_copy_creation_deletion_or_unsupported_types() {
    for suffix in [
        ...
        assert!(
            parse((relocation("old", "new") + suffix).as_bytes()).is_err(),
            "{suffix}"
        );
    }
}
...
fn duplicate_and_overlapping_source_or_destination_paths_refuse() {
    ...
    assert_eq!(
        parse(relocation("dir", "dir/file").as_bytes()),
        Err(PatchError::OverlappingPaths)
    );
    let input =
        relocation("old", "new") + "diff --git a/old b/old\nold mode 100644\nnew mode 100755\n";
...
fn independent_renames_are_order_invariant_and_ordinary_edits_still_work() {
    let first = relocation("a", "b");
    let second = relocation("c", "d");
    let forward = first.clone() + &second;
    let reverse = second + &first;
    assert_eq!(
        parse(forward.as_bytes()).unwrap(),
        parse(reverse.as_bytes()).unwrap()
    );
    let input = b"diff --git a/file b/file\n--- a/file\n+++ b/file\n@@ -1 +1 @@\n-a\n+b\n";
    let patch = parse(input).unwrap();
    assert_eq!(
        patch,
        UnifiedPatch::parse(input, PatchLimits::default(), &|| false).unwrap()
    );
...
fn source_destination_and_parser_limits_have_boundary_twins() {
    ...
    assert_eq!(
        UnifiedPatch::parse_with_renames(
            input.as_bytes(),
            PatchLimits {
                max_path_bytes: 5,
                ..limits
    ...
    for limits in [
        ...
        assert!(matches!(
            UnifiedPatch::parse_with_renames(input.as_bytes(), limits, &|| false),
            Err(PatchError::Budget(_))
        ));
    }
    let two = input + &relocation("third", "fourth");
    assert_eq!(
        UnifiedPatch::parse_with_renames(two.as_bytes(), limits, &|| false),
...
fn cancellation_during_scan_and_apply_never_returns_partial_success() {
    ...
    UnifiedPatch::parse_with_renames(input.as_bytes(), PatchLimits::default(), &|| {
    ...
    let patch = parse(input.as_bytes()).unwrap();
    assert_eq!(
        patch.files()[0].apply(Some((0o100644, b"x")), patch.limits(), &|| true),
        Err(PatchError::Cancelled)
    );
}
