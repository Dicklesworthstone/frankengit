...
impl BinaryHunks<'_> {
    ...
    pub fn member_count(&self) -> usize { self.members }
    pub fn line_count(&self) -> usize { self.lines }
    /// Literal bytes or inflated delta PROGRAM bytes, not delta result sizes.
    pub fn declared_inflated_bytes(&self) -> usize { self.declared.iter().sum() }
    pub(super) fn check_limits(&self, limits: PatchLimits) -> Result<(), PatchError> {
        if self.bytes.len() > limits.max_patch_bytes || self.lines > limits.max_lines
            || self.members > limits.max_hunks
            || self.declared.iter().any(|size| *size > limits.max_file_bytes)
...
pub(super) fn check_index(index: Option<&IndexExpectation>, change: FileChange)
    -> Result<&IndexExpectation, PatchError>
{
    ...
    if !matches!(index.old.len(), 40 | 64) || index.old.len() != index.new.len() {
        return Err(syntax(0, "binary index identities must share a full hash width"));
    }
    let old_absent = index.old.iter().all(|b| *b == b'0');
    let new_absent = index.new.iter().all(|b| *b == b'0');
    if old_absent != (change == FileChange::Create)
        || new_absent != (change == FileChange::Delete) {
        return Err(PatchError::SourcePresence);
    }
    Ok(index)
}

struct Cursor<'a> { input: &'a [u8], at: usize, lines: usize, first_line: usize }
impl<'a> Cursor<'a> {
    fn line(&mut self, limits: PatchLimits, cancelled: &dyn Fn() -> bool)
        -> Result<&'a [u8], PatchError>
    {
        checkpoint(cancelled)?;
        if self.lines >= limits.max_lines { return Err(PatchError::Budget("binary lines")); }
        ...
        let end = rest.iter().take(67).position(|b| *b == b'\n')
            .ok_or_else(|| syntax(self.first_line + self.lines, "truncated or oversized binary record"))?;
...
pub(super) fn scan<'a>(input: &'a [u8], first_line: usize, limits: PatchLimits,
    cancelled: &dyn Fn() -> bool,
) -> Result<BinaryHunks<'a>, PatchError> {
    let mut cursor = Cursor { input, at: 0, lines: 0, first_line };
    ...
    let mut members = 0;
    loop {
        if members == 2 { return Err(syntax(first_line + cursor.lines, "trailing binary records")); }
        let at = first_line + cursor.lines;
        let header = cursor.line(limits, cancelled)?;
        let digits = header.strip_prefix(b"literal ").or_else(|| header.strip_prefix(b"delta "))
            .ok_or_else(|| syntax(at, "binary member header required"))?;
        if digits.len() > 1 && digits[0] == b'0' { return Err(syntax(at, "noncanonical binary size")); }
        let size = number(digits, at)?;
        if size > limits.max_file_bytes { return Err(PatchError::Budget("binary member bytes")); }
        ...
        loop {
            ...
            if row.len() != 1 + count.div_ceil(4) * 5 || row[1..].iter().any(|b|
                !b.is_ascii_alphanumeric() && !b"!#$%&()*+-;<=>?@^_`{|}~".contains(b)) {
                return Err(syntax(at, "binary row framing"));
            }
            rows += 1;
        }
        if rows == 0 { return Err(syntax(at, "empty encoded binary member")); }
        if cursor.file_end() { break; }
    }
    let hunks = BinaryHunks { bytes: &input[..cursor.at], lines: cursor.lines, members, declared };
