...
fn patch(width: usize, mode: &str, old: char, new: char, payload: &str) -> String {
    format!(
        ...
        old.to_string().repeat(width),
        new.to_string().repeat(width)
    )
}
...
fn defaults_refuse_and_opt_in_retains_exact_borrowed_payload() {
    for width in [40, 64] {
        ...
        assert!(
            UnifiedPatch::parse_with_renames(input.as_bytes(), PatchLimits::default(), &|| false)
                .is_err()
        );
        ...
        assert_eq!(
            body.bytes().as_ptr(),
            input.as_bytes()[input.find("GIT binary patch").unwrap()..].as_ptr()
        );
        assert_eq!(file.hunk_count(), 2);
        assert!(matches!(
            file.apply(Some((0o100644, b"old")), parsed.limits(), &|| false),
...
fn exact_create_modify_delete_and_empty_vs_absent() {
    for (mode, old, new, source, expected) in [
        ...
        let input = patch(40, mode, old, new, MEMBER);
        let parsed = parse(input.as_bytes()).unwrap();
        let mut called = false;
        let result = parsed.files()[0]
            .apply_with_binary_decoder(source, parsed.limits(), &|| false, |body, index, base| {
                called = true;
                assert!(body.starts_with(b"GIT binary patch\n"));
                assert_eq!(index.old.len(), 40);
                assert_eq!(base, source.map_or(b"".as_slice(), |(_, bytes)| bytes));
                Ok(vec![])
            })
            .unwrap();
        assert!(called);
...
fn decoder_failures_and_bad_results_never_become_unchanged_success() {
    let input = patch(40, "", 'a', 'b', MEMBER);
    let parsed = parse(input.as_bytes()).unwrap();
    let file = &parsed.files()[0];
    assert_eq!(
        file.apply_with_binary_decoder(
            Some((0o100644, b"old")),
            parsed.limits(),
            &|| false,
            |_, _, _| Err(PatchError::Cancelled)
        ),
    ...
    let parsed = parse(input.as_bytes()).unwrap();
    assert_eq!(
        parsed.files()[0].apply_with_binary_decoder(
            Some((0o100644, b"old")),
            parsed.limits(),
            &|| false,
            |_, _, _| Ok(vec![0])
        ),
        Err(PatchError::NonemptyDeletion)
    );
}
#[test]
fn source_modes_presence_and_narrowed_limits_precede_decoder() {
    ...
    for (source, limits) in [
        (None, parsed.limits()),
        (Some((0o100755, b"old".as_slice())), parsed.limits()),
        (
            Some((0o100644, b"old".as_slice())),
            PatchLimits {
                max_file_bytes: 2,
        ...
        assert!(
            parsed.files()[0]
                .apply_with_binary_decoder(source, limits, &|| false, |_, _, _| panic!(
                    "must not decode"
                ))
                .is_err()
        );
...
fn cancellation_after_decoder_cannot_return_a_candidate() {
    ...
    assert_eq!(
        UnifiedPatch::parse_with_binary(input.as_bytes(), parsed.limits(), &|| true),
        Err(PatchError::Cancelled)
    );
}
#[test]
fn truncated_extra_or_mixed_records_refuse() {
    for payload in [
        ...
        assert!(
            parse(patch(40, "", 'a', 'b', payload).as_bytes()).is_err(),
            "{payload}"
        );
    }
    assert!(parse(patch(40, "", 'a', 'b', &MEMBER.repeat(3)).as_bytes()).is_err());
    let input = patch(40, "", 'a', 'b', MEMBER).replace(
        "GIT binary patch\n",
        "--- a/file\n+++ b/file\nGIT binary patch\n",
    );
...
fn binary_and_literal_files_share_global_paths_and_budgets() {
    ...
    let input = binary.clone() + text;
    let parsed = parse(input.as_bytes()).unwrap();
    assert_eq!(parsed.files().len(), 2);
    assert_eq!(parsed.files()[0].path(), b"a");
    assert_eq!(
        parsed.files()[0]
            .apply_with_binary_decoder(
                Some((0o100644, b"old\n")),
                parsed.limits(),
                &|| false,
                |_, _, _| panic!("literal file")
            )
            .unwrap()
            .unwrap()
            .content,
    ...
    assert_eq!(
        parse((binary.clone() + &binary).as_bytes()),
        Err(PatchError::DuplicatePath)
    );
    assert_eq!(
        parse(
            (binary
                + &text
                    .replace("a/a b/a", "a/file/child b/file/child")
                    .replace("--- a/a", "--- a/file/child")
                    .replace("+++ b/a", "+++ b/file/child"))
                .as_bytes()
        ),
    ...
    assert!(
        UnifiedPatch::parse_with_binary(
            input.as_bytes(),
            PatchLimits {
                max_hunks: 1,
                ..Default::default()
    ...
    let input = patch(40, "", 'a', 'b', MEMBER)
        + &patch(40, "", 'a', 'b', MEMBER).replace("a/file b/file", "a/other b/other");
    assert!(
        UnifiedPatch::parse_with_binary(
            input.as_bytes(),
            PatchLimits {
...
fn raw_paths_are_exact_and_rename_is_not_implicitly_enabled() {
    let input =
        patch(40, "", 'a', 'b', MEMBER).replace("a/file b/file", "\"a/dir/\\377\" \"b/dir/\\377\"");
    assert_eq!(
        parse(input.as_bytes()).unwrap().files()[0].path(),
        b"dir/\xff"
    );
    let rename = patch(40, "rename from file\nrename to other\n", 'a', 'b', MEMBER)
        .replace("a/file b/file", "a/file b/other");
...
fn delta_size_is_a_program_bound_and_binary_output_is_not_line_split() {
    ...
    let parsed = parse(input.as_bytes()).unwrap();
    let file = &parsed.files()[0];
    assert_eq!(file.binary_hunks().unwrap().declared_inflated_bytes(), 3);
    assert_eq!(
        file.apply_with_binary_decoder(
            Some((0o100644, b"old")),
            parsed.limits(),
            &|| false,
            |_, _, _| Ok(vec![0, 255, 10, 0])
        )
        .unwrap()
        .unwrap()
        .content,
...
fn combined_profile_retains_rename_obligations_without_enabling_compressed_renames() {
    ...
    let parsed = UnifiedPatch::parse_with_binary_and_renames(
        input.as_bytes(),
        PatchLimits::default(),
        &|| false,
    )
    ...
    let binary_rename = patch(40, "rename from file\nrename to other\n", 'a', 'b', MEMBER)
        .replace("a/file b/file", "a/file b/other");
    assert!(
        UnifiedPatch::parse_with_binary_and_renames(
            binary_rename.as_bytes(),
            PatchLimits::default(),
            &|| false
        )
        .is_err()
