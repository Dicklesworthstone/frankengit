...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct UnifiedPatch<'a> {
    files: Vec<FilePatch<'a>>,
    limits: PatchLimits,
}
#[derive(Clone, Debug, Eq, PartialEq)]
...
fn number(bytes: &[u8], at: usize) -> Result<usize, PatchError> {
    ...
    bytes.iter().try_fold(0usize, |n, byte| {
        n.checked_mul(10)
...
const fn mode(bytes: &[u8], at: usize) -> Result<u32, PatchError> {
    match bytes {
        ...
        _ => Err(PatchError::Unsupported {
            line: at + 1,
            feature: "non-regular file mode",
        }),
    }
}
fn set<T>(slot: &mut Option<T>, value: T, at: usize) -> Result<(), PatchError> {
    if slot.is_some() {
        return Err(syntax(at, "duplicate metadata"));
    }
    *slot = Some(value);
    Ok(())
}
fn range(bytes: &[u8], at: usize, limit: usize) -> Result<(usize, usize), PatchError> {
    ...
    if parts.next().is_some() || (count != 0 && start == 0) {
        return Err(syntax(at, "invalid hunk range"));
    }
    let offset = if count == 0 { start } else { start - 1 };
    if offset.checked_add(count).is_none_or(|end| end > limit) {
        return Err(PatchError::Budget("hunk range"));
    }
    Ok((offset, count))
}
fn unquote(bytes: &[u8], at: usize) -> Result<(Vec<u8>, usize), PatchError> {
    if bytes.first() != Some(&b'"') {
        return Err(syntax(at, "expected quoted path"));
    }
    let mut out = Vec::new();
    let mut pos = 1;
    while let Some(&byte) = bytes.get(pos) {
        ...
        match byte {
            ...
            b'\\' => {
                let escaped = *bytes
                    .get(pos)
                    .ok_or_else(|| syntax(at, "truncated path escape"))?;
                pos += 1;
                let value = match escaped {
                    ...
                    b'0'..=b'3' => {
                        let digits = bytes
                            .get(pos..pos + 2)
                            .ok_or_else(|| syntax(at, "truncated octal path"))?;
                        if digits.iter().any(|byte| !(b'0'..=b'7').contains(byte)) {
                            return Err(syntax(at, "invalid octal path"));
                        }
                        pos += 2;
                        (escaped - b'0') * 64 + (digits[0] - b'0') * 8 + (digits[1] - b'0')
                    }
...
fn strip_path(bytes: &[u8], prefix: &[u8], limit: usize) -> Result<Vec<u8>, PatchError> {
    ...
    check_path(bytes, limit)?;
    Ok(bytes.to_vec())
}
fn diff_paths(bytes: &[u8], at: usize, limit: usize) -> Result<Vec<u8>, PatchError> {
    if bytes.len() > limit.saturating_mul(8).saturating_add(16) {
    ...
    let (left, right) = if bytes.first() == Some(&b'"') {
        ...
        let rest = bytes
            ...
            .ok_or_else(|| syntax(at, "missing second diff path"))?;
        let right = if rest.first() == Some(&b'"') {
            let (right, used) = unquote(rest, at)?;
            if used != rest.len() {
                return Err(syntax(at, "trailing diff path bytes"));
            }
            right
        } else {
            rest.to_vec()
        ...
        let mut boundaries = bytes
            .windows(3)
            .enumerate()
            .filter(|(_, part)| *part == b" b/")
            .map(|(i, _)| i);
        let split = boundaries
            .next()
            .ok_or_else(|| syntax(at, "ambiguous diff paths"))?;
        if boundaries.next().is_some() {
            return Err(syntax(at, "ambiguous diff paths"));
        }
...
fn file_path(
    ...
    if bytes.len() > limit.saturating_mul(4).saturating_add(16) {
        return Err(PatchError::Budget("path metadata"));
    }
    if bytes == b"/dev/null" {
        return Ok(None);
    ...
    let decoded = if bytes.first() == Some(&b'"') {
        ...
        if used != bytes.len() {
            return Err(syntax(at, "trailing file header bytes"));
        }
        decoded
    } else {
        ...
        if path.contains(&b'\t') {
            return Err(syntax(at, "unsupported file header suffix"));
        }
        path.to_vec()
    };
...
fn index(bytes: &[u8], at: usize) -> Result<(IndexExpectation, Option<u32>), PatchError> {
    ...
    if parts.next().is_some() {
        return Err(syntax(at, "trailing index metadata"));
    }
    let split = names
        .windows(2)
        .position(|part| part == b"..")
        .ok_or_else(|| syntax(at, "invalid index names"))?;
    let old = &names[..split];
    let new = &names[split + 2..];
    for name in [old, new] {
...
impl<'a> UnifiedPatch<'a> {
    ...
    fn parse_profile(
        ...
        if input.len() > limits.max_patch_bytes {
            return Err(PatchError::Budget("patch bytes"));
        }
        ...
        for line in input.split_inclusive(|byte| *byte == b'\n') {
            checkpoint(cancelled)?;
            if records.len() == limits.max_lines {
                return Err(PatchError::Budget("patch lines"));
            }
            if allow_binary {
                offsets.push(offset);
                offset += line.len();
        ...
        if records.is_empty() {
            return Err(syntax(0, "empty patch"));
        }
        ...
        while pos < records.len() {
            ...
            if files.len() == limits.max_files {
                return Err(PatchError::Budget("patch files"));
            }
            let first = metadata(records[pos], pos)?;
            let paths = first
                .strip_prefix(b"diff --git ")
                .ok_or_else(|| syntax(pos, "expected diff --git"))?;
            ...
            let mut file = FilePatch {
                path,
                renamed_from,
                identical_content: similarity == Some(100),
            ...
            while pos < records.len() && !records[pos].starts_with(b"diff --git ") {
                ...
                if allow_binary && text == b"GIT binary patch" {
                    ...
                    hunk_count = hunk_count
                        .checked_add(payload.member_count())
                        .filter(|count| *count <= limits.max_hunks)
                        .ok_or(PatchError::Budget("patch hunks"))?;
                    binary_expanded = binary_expanded
                        .checked_add(payload.declared_inflated_bytes())
                ...
                if text.starts_with(b"@@ ") {
                    if !headers {
                        return Err(syntax(pos, "hunk before file headers"));
                    }
                    if hunk_count == limits.max_hunks {
                        return Err(PatchError::Budget("patch hunks"));
                    }
                    hunk_count += 1;
                    let mut parts = text.splitn(5, |byte| *byte == b' ');
                    if parts.next() != Some(b"@@".as_slice()) {
                        return Err(syntax(pos, "invalid hunk header"));
                    }
                    let old = parts
                        .next()
                        .and_then(|part| part.strip_prefix(b"-"))
                        .ok_or_else(|| syntax(pos, "missing old range"))?;
                    let new = parts
                        .next()
                        .and_then(|part| part.strip_prefix(b"+"))
                        .ok_or_else(|| syntax(pos, "missing new range"))?;
                    if parts.next() != Some(b"@@".as_slice()) {
                        return Err(syntax(pos, "invalid hunk trailer"));
                    }
                    let (old, old_count) = range(old, pos, limits.max_lines)?;
                    let (new, new_count) = range(new, pos, limits.max_lines)?;
                    if old_count == 0 && new_count == 0 {
                        return Err(syntax(pos, "empty hunk"));
                    }
                    let mut hunk = Hunk {
                        old,
                        old_count,
                    ...
                    while removed < old_count || added < new_count {
                        ...
                        let line = *records
                            .get(pos)
                            .ok_or_else(|| syntax(pos, "truncated hunk"))?;
                        let tag = *line
                            .first()
                            .ok_or_else(|| syntax(pos, "empty hunk record"))?;
                        if !matches!(tag, b' ' | b'-' | b'+') {
                            return Err(syntax(pos, "invalid hunk record"));
                        }
                        metadata(line, pos)?;
                        removed += usize::from(tag != b'+');
                        added += usize::from(tag != b'-');
                        if removed > old_count || added > new_count {
                            return Err(syntax(pos, "hunk counts disagree"));
                        }
                        let mut line = Line {
                            kind: tag,
                            bytes: &line[1..],
                        ...
                        if records.get(pos).is_some_and(|line| line.starts_with(b"\\")) {
                            if metadata(records[pos], pos)? != b"\\ No newline at end of file" {
                                return Err(syntax(pos, "invalid missing-newline marker"));
                            }
                            line.bytes = line
                                .bytes
                                .strip_suffix(b"\n")
                            ...
                            if line.bytes.is_empty() {
                                return Err(syntax(pos, "zero-byte logical line"));
                            }
                ...
                if !file.hunks.is_empty() {
                    return Err(syntax(pos, "metadata after hunk"));
                }
                if let Some(value) = text.strip_prefix(b"--- ") {
                    if headers {
                        return Err(syntax(pos, "duplicate file headers"));
                    }
                    let old = file_path(value, b"a/", pos, limits.max_path_bytes)?;
                    pos += 1;
                    let next = *records
                        .get(pos)
                        .ok_or_else(|| syntax(pos, "missing new-file header"))?;
                    let value = metadata(next, pos)?
                        .strip_prefix(b"+++ ")
                        .ok_or_else(|| syntax(pos, "missing new-file header"))?;
                    let new = file_path(value, b"b/", pos, limits.max_path_bytes)?;
                    let change = match (&old, &new) {
                        (None, Some(_)) => FileChange::Create,
                        (Some(_), None) => FileChange::Delete,
                        (Some(_), Some(_)) => FileChange::Modify,
                        _ => return Err(syntax(pos, "two absent file sides")),
                    };
                    if old
                        .as_deref()
                        .is_some_and(|path| path != file.source_path())
                        || new.as_deref().is_some_and(|path| path != file.path())
                    {
                        return Err(syntax(pos, "file paths disagree"));
                    }
                    if kind.is_some_and(|kind| kind != change) {
                        return Err(syntax(pos, "file presence metadata disagrees"));
                    }
                    kind = Some(change);
                    headers = true;
                } else if headers {
                    ...
                    let (names, mode) = index(value, pos)?;
                    set(&mut file.index, names, pos)?;
                    index_mode = mode;
                } else {
                    return Err(PatchError::Unsupported {
                        line: pos + 1,
                        feature: "extended, binary, rename or copy record",
                    });
                }
                pos += 1;
            }
            ...
            if file.renamed_from.is_some() {
                if file.change != FileChange::Modify {
                    return Err(syntax(
                        ...
                        "rename cannot create or delete a file side",
                    ));
                }
                if file.hunks.is_empty() && similarity.is_some_and(|score| score != 100) {
                    return Err(syntax(
                        pos.saturating_sub(1),
                        "nonidentical rename needs exact hunks",
                    ));
                }
            }
            ...
            match file.change {
                FileChange::Create if file.old_mode.is_some() || file.new_mode.is_none() => {
                    return Err(syntax(
                        pos.saturating_sub(1),
                        "creation needs a new regular-file mode",
                    ));
                }
                FileChange::Delete if file.new_mode.is_some() || file.old_mode.is_none() => {
                    return Err(syntax(
                        pos.saturating_sub(1),
                        "deletion needs an old regular-file mode",
                    ));
                }
                FileChange::Modify if file.old_mode.is_some() != file.new_mode.is_some() => {
                    return Err(syntax(pos.saturating_sub(1), "incomplete mode change"));
                }
                _ => {}
            }
            ...
            if file.binary.is_none()
                && file.renamed_from.is_none()
                && file.hunks.is_empty()
                && file.change == FileChange::Modify
                && (file.old_mode.is_none() || file.old_mode == file.new_mode)
            {
                return Err(syntax(
                    pos.saturating_sub(1),
                    "file has no hunks or mode change",
                ));
            }
            files.push(file);
        }
        ...
        for path in &paths {
            ...
            for (i, byte) in path.iter().enumerate() {
                if *byte == b'/' && paths.contains(&path[..i]) {
...
impl FilePatch<'_> {
    ...
    pub fn apply_with_binary_decoder(
        ...
        if self.hunks.len() > limits.max_hunks {
            return Err(PatchError::Budget("patch hunks"));
        }
        if self
            .hunks
        ...
        if (self.change == FileChange::Create) != source.is_none() {
            return Err(PatchError::SourcePresence);
        }
        let (old_mode, body) = source.unwrap_or((0o100644, b""));
        if !matches!(old_mode, 0o100644 | 0o100755)
            || self.old_mode.is_some_and(|mode| mode != old_mode)
        {
            return Err(PatchError::SourceMode);
        }
        if body.len() > limits.max_file_bytes {
            return Err(PatchError::Budget("source file bytes"));
        }
        if let Some(binary) = &self.binary {
            ...
            if self.change == FileChange::Delete {
                if !output.is_empty() {
                    return Err(PatchError::NonemptyDeletion);
                }
                return Ok(None);
            }
            return Ok(Some(PatchedFile {
                mode: self.new_mode.unwrap_or(old_mode),
                content: output,
            }));
        }
        ...
        for line in body.split_inclusive(|byte| *byte == b'\n') {
            checkpoint(cancelled)?;
            if old_lines.len() == limits.max_lines {
                return Err(PatchError::Budget("source lines"));
            }
            old_lines.push(line);
        }
        ...
        for (index, hunk) in self.hunks.iter().enumerate() {
            checkpoint(cancelled)?;
            if hunk.old < old_cursor || hunk.old > old_lines.len() {
                return Err(PatchError::SourceRange { hunk: index });
            }
            ...
            if new_cursor != hunk.new {
                return Err(PatchError::ResultRange { hunk: index });
            }
            old_cursor = hunk.old;
            for line in &hunk.lines {
                checkpoint(cancelled)?;
                if line.kind != b'+' {
                    if old_lines.get(old_cursor).copied() != Some(line.bytes) {
                        return Err(PatchError::ContextMismatch {
                            hunk: index,
                            source_line: old_cursor,
                        });
                    }
                    old_cursor += 1;
                    if line.no_newline && old_cursor != old_lines.len() {
            ...
            if old_cursor != hunk.old + hunk.old_count || new_cursor != hunk.new + hunk.new_count {
                return Err(PatchError::ResultRange { hunk: index });
            }
        }
...
fn append_line(
    ...
    if *lines >= line_limit {
        return Err(PatchError::Budget("result lines"));
    }
    append(output, bytes, byte_limit)?;
    *lines += 1;
    Ok(())
}
fn append(output: &mut Vec<u8>, bytes: &[u8], limit: usize) -> Result<(), PatchError> {
    ...
    output
        ...
        .map_err(|_| PatchError::Budget("allocation"))?;
...
mod path_header_regressions {
    ...
    fn git_space_terminated_headers_apply_to_the_exact_filename() {
        for path in [
            ...
            assert!(matches!(
                file.apply(Some((0o100644, b"wrong\n")), patch.limits(), &|| false),
    ...
    fn space_paths_work_for_creation_deletion_and_mode_only_changes() {
        ...
        for (metadata, headers, hunk, source, expected, change) in [
            ...
            assert_eq!(
                file.apply(source, patch.limits(), &|| false).unwrap(),
                expected
            );
        }
    }
    ...
    fn file_header_suffixes_are_not_silently_discarded() {
        for bytes in [
            ...
            assert!(matches!(
                file_path(bytes, b"a/", 0, 4096),
                Err(PatchError::Syntax { .. })
            ));
        }
        assert_eq!(file_path(b"/dev/null", b"a/", 0, 4096).unwrap(), None);
        assert_eq!(
            file_path(b"/dev/null\t", b"a/", 0, 4096),
            Err(PatchError::InvalidPath)
        );
        assert_eq!(
            file_path(b"a/name \t", b"a/", 0, 4096).unwrap(),
            Some(b"name ".to_vec())
        );
    }
    ...
    fn unquoted_separator_resolution_does_not_allow_renames_or_traversal() {
        assert!(matches!(
            diff_paths(b"a/old b/new", 0, 4096),
            Err(PatchError::Unsupported {
                feature: "rename or copy",
                ..
            })
        ...
        for path in [
            ...
            assert_eq!(
                diff_paths(bytes.as_bytes(), 0, 4096),
                Err(PatchError::InvalidPath)
            );
        }
        assert_eq!(
            diff_paths(b"a/x\0 b/y b/x\0 b/y", 0, 4096),
            Err(PatchError::InvalidPath)
        );
    }
    ...
    fn file_headers_must_still_agree_with_the_complete_diff_path() {
        for (path, old, new) in [
            ...
            assert!(matches!(
                UnifiedPatch::parse(bytes.as_bytes(), PatchLimits::default(), &|| false),
                Err(PatchError::Syntax {
                    reason: "file paths disagree",
                    ..
                })
            ));
        }
