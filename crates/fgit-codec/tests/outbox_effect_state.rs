...
use fgit_codec::{
    CanonicalBody, CanonicalOutboxEffectState, CanonicalOutboxState, CodecRefusal, DecodeLimits,
    Decoder, Encoder, MAX_OUTBOX_EFFECT_TRANSITIONS, decode_body, encode_body,
};
...
fn committed_and_acknowledged_bodies_round_trip_with_exact_predecessor_identity() {
    ...
    let acknowledged = initial
        .transition(LifecycleEvent::Acknowledge, Some(digest(4)))
    ...
    assert_eq!(
        acknowledged.predecessor_root(),
        Some(initial.root().expect("initial root"))
    );
    assert_eq!(
        acknowledged.predecessor_state(),
    ...
    assert_ne!(
        acknowledged.root().expect("ack root"),
        initial.root().expect("initial root")
    );
...
fn deferred_escalated_and_terminal_paths_use_the_shared_lifecycle() {
        ...
        for (event, evidence) in [
            ...
            match shared {
                Ok(expected) => {
                    ...
                    assert!(
                        actual
                            .transition(LifecycleEvent::Acknowledge, Some(digest(11)))
                            .is_err()
                    );
                }
    ...
    let terminal = escalated
        .transition(LifecycleEvent::Acknowledge, Some(digest(12)))
        .expect("settle");
    assert_eq!(terminal.transition_ordinal(), MAX_OUTBOX_EFFECT_TRANSITIONS);
    assert!(
        initial
            .transition(LifecycleEvent::Acknowledge, None)
            .is_err()
    );
...
fn semantic_inputs_history_and_acknowledgement_evidence_change_identity() {
    ...
    for changed in variants {
        assert_ne!(
            initial.root().expect("root"),
            changed.root().expect("changed root")
        );
        roundtrip(&changed);
    }
    let direct = initial
        .transition(LifecycleEvent::Acknowledge, Some(digest(6)))
        .expect("ack");
    let changed_evidence = initial
        .transition(LifecycleEvent::Acknowledge, Some(digest(7)))
        .expect("ack");
    let deferred = initial
        .transition(LifecycleEvent::Defer, Some(digest(8)))
        .expect("defer");
    let via_deferred = deferred
        .transition(LifecycleEvent::Acknowledge, Some(digest(6)))
        .expect("ack");
    let changed_predecessor = initial
        .transition(LifecycleEvent::Defer, Some(digest(9)))
        .expect("defer")
        .transition(LifecycleEvent::Acknowledge, Some(digest(6)))
        .expect("ack");
    assert_ne!(
        direct.root().expect("root"),
        changed_evidence.root().expect("changed evidence root")
    );
    assert_ne!(
        direct.root().expect("root"),
        via_deferred.root().expect("changed history root")
    );
    assert_eq!(via_deferred.state(), changed_predecessor.state());
    assert_eq!(via_deferred.event(), changed_predecessor.event());
    assert_eq!(
        via_deferred.evidence_root(),
        changed_predecessor.evidence_root()
    );
    assert_ne!(
        via_deferred.predecessor_root(),
        changed_predecessor.predecessor_root()
    );
    assert_ne!(
        via_deferred.root().expect("root"),
        changed_predecessor
            .root()
            .expect("changed predecessor root")
    );
...
impl WireState {
    fn acknowledged() -> Self {
        Self {
            ordinal: 1,
            state: 4,
            predecessor_root: Some(committed().root().expect("root")),
            predecessor_state: Some(1),
            event: Some(2),
            evidence: Some(digest(4)),
        }
    }
    ...
    fn payload(&self) -> Vec<u8> {
        ...
        out.write_bytes("delivery_key", initial.delivery_key().as_bytes())
            .expect("slug");
        out.write_internal_object_id(initial.tx_id().as_internal_object_id())
            .expect("tx");
        ...
        out.write_option(self.predecessor_state.as_ref(), |out, state| {
            out.write_scalar(*state);
            Ok(())
        })
        .expect("state");
        out.write_option(self.event.as_ref(), |out, event| {
            out.write_scalar(*event);
            Ok(())
        })
        .expect("event");
        out.write_option(self.evidence.as_ref(), Encoder::write_digest)
            .expect("evidence");
        out.into_bytes()
    }
...
fn planted_illegal_and_incomplete_histories_are_refused_by_the_real_decoder() {
    ...
    for (index, mutate) in mutations.iter().enumerate() {
        ...
        assert!(
            wire.decode().is_err(),
            "planted invalid history {index} must refuse"
        );
        assert!(
            WireState::acknowledged().decode().is_ok(),
            "adjacent legal history remains accepted"
        );
    }
    ...
    payload.pop();
    assert!(matches!(
        CanonicalOutboxEffectState::read_payload(&mut Decoder::new(
            &payload,
