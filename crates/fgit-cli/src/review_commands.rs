...
use fgit_forge::event::review::{
    CANDIDATE_REVIEW_PROFILE, CandidateBinding, CandidateReviewCommand, MAX_REVIEW_REASON_BYTES,
    NativeReviewEvent, REVIEW_PROFILE, ReviewCommand, ReviewDecision, ReviewSubject,
};
use fgit_forge::{AggregateVersion, ExpectedVersion, PullRequestNumber};
use fgit_node::{LoopbackReceiveSession, NodeConfig, OneNode};
use fgit_types::{
    CANONICAL_CODEC_VERSION, DecisionOutcome, DigestAlgorithmId, DigestBytes, GitHashAlgorithm,
    HeadGeneration, PolicyEpoch, PrincipalId, RefName, RepositoryAuthorityHeadId, RepositoryId,
    TenantId, TxId,
};
...
pub(super) fn run(args: &[String], mode: Mode) -> Result<u8, String> {
    ...
    if mode == Mode::Reviews {
        return run_read(parse_read(args)?);
    }
    let options = parse_mutation(args, mode)?;
    let input = options
        .bundle
        .as_ref()
        .map(|path| read_bundle(path, 128 * 1024 * 1024))
        .transpose()?;
    let mut node = OneNode::open_existing(
        NodeConfig::new(
            options.scope.storage.clone(),
            options.scope.tenant,
            options.scope.repository,
        )
        .with_object_format(options.command.review.subject.source_tip.algorithm()),
    )
    ...
    let operation = (|| -> Result<_, String> {
        ...
        if mode == Mode::Review {
            ...
            node.runtime()
                .block_on(
                    node.apply_reviewed_merge_bundle_durable_in(
                        &request,
                        options.principal,
                        &options.key,
                        subject.pull_request,
                        ExpectedVersion::Exactly(subject.pull_request_version),
                        &options.command.candidate.merge(subject),
                        input
                            .as_deref()
                            .ok_or("reviewed merge requires bundle bytes")?,
    ...
    if let Err(error) =
        write_terminal_receipt(&mut std::io::stdout().lock(), &receipt, tx, &terminal)
    {
        return Err(format!(
            "{error}{}",
            cleanup
                .as_ref()
                .map_or_else(String::new, |e| format!("; node shutdown also failed: {e}"))
        ));
    }
...
fn flags(
    args: &[String],
    mode: Mode,
) -> Result<(Scope, BTreeMap<&str, &str>, Vec<PrincipalId>), String> {
    if args.len() < 4 {
        return Err(USAGE.into());
    }
    if args.len() > 128
        || args.iter().any(|arg| arg.len() > 32 * 1024)
    ...
    while at < args.len() {
        ...
        at += 1;
        let allowed = flag == "--trusted-local"
            || if mode == Mode::Reviews {
                matches!(
                    flag,
                ...
                matches!(
                    ...
                    && matches!(flag, "--review-version" | "--decision" | "--reason"))
        ...
        if flag == "--require-reviewer" {
            if reviewers.len() == 32 {
                return Err("at most 32 required reviewers".into());
            }
            let reviewer = PrincipalId::from_hex(value).map_err(|_| "invalid required reviewer")?;
            if !reviewers.insert(reviewer) {
                return Err("duplicate required reviewer".into());
            }
        } else if flags.insert(flag, value).is_some() {
            return Err(format!("duplicate {flag}"));
        }
    }
    if !flags.contains_key("--trusted-local") {
        return Err(
            "--trusted-local is required; these are authorized local-operator commands".into(),
        );
    }
...
fn parse_mutation(args: &[String], mode: Mode) -> Result<Mutation, String> {
    if mode == Mode::Reviews {
        return Err("read mode is not a mutation".into());
    }
    let (scope, flags, reviewers) = flags(args, mode)?;
    let principal =
        PrincipalId::from_hex(required(&flags, "--principal")?).map_err(|_| "invalid principal")?;
    ...
    let pr_version = AggregateVersion::try_new(decimal(required(&flags, "--expected-version")?)?)
        .ok_or("PR version must be positive")?;
    if mode == Mode::Apply {
        pr_version.next().map_err(|_| "PR version exhausted")?;
    }
    let subject = ReviewSubject {
        pull_request: scope.number,
        pull_request_version: pr_version,
        source_ref: RefName::try_new(required(&flags, "--source-ref")?.as_bytes())
            .map_err(|_| "invalid source ref")?,
        target_ref: RefName::try_new(required(&flags, "--target-ref")?.as_bytes())
            .map_err(|_| "invalid target ref")?,
        source_tip: parse_oid(required(&flags, "--source-tip")?)?,
        target_tip: parse_oid(required(&flags, "--target-tip")?)?,
        policy_epoch: PolicyEpoch::try_new(decimal(required(&flags, "--policy-epoch")?)?)
            .map_err(|_| "invalid policy epoch")?,
    };
    let candidate = CandidateBinding {
        merge_base: parse_oid(required(&flags, "--merge-base")?)?,
    ...
    let (expected_version, decision, reason) = if mode == Mode::Review {
        ...
        let version = if version == 0 {
            ...
            old.next().map_err(|_| "reviewer version exhausted")?;
            ExpectedVersion::Exactly(old)
        };
        let decision = match required(&flags, "--decision")? {
            ...
            _ => return Err("unknown review decision".into()),
        };
        let reason = flags.get("--reason").copied().unwrap_or("");
        if reason.len() > MAX_REVIEW_REASON_BYTES {
            return Err("review reason exceeds 16 KiB".into());
        }
        (version, decision, reason.to_owned())
    } else {
        if reviewers.is_empty() {
            return Err(
                "--require-reviewer is required; no ungated fallback exists in apply-reviewed"
                    .into(),
            );
        }
        if reviewers.contains(&principal) {
            return Err("submitter cannot be a required reviewer".into());
        }
        (
            ExpectedVersion::NewStream,
    ...
    command
        .proposed_event(principal, command.review.subject.source_tip.algorithm())
        .map_err(|_| "invalid review subject or decision")?;
    let bundle = flags
        .get("--bundle")
        .map(|path| {
            if path.is_empty() || path.len() > 4096 {
                Err("invalid bundle path".to_owned())
            } else {
    ...
    if mode == Mode::Review && decision == ReviewDecision::Withdraw {
        if bundle.is_some() {
            return Err("withdrawal needs the exact prior subject, not bundle bytes".into());
        }
    } else if bundle.is_none() {
        return Err(
            "--bundle is required for new approvals/change requests and reviewed publication"
                .into(),
        );
    }
    Ok(Mutation {
...
fn parse_read(args: &[String]) -> Result<ReadOptions, String> {
    ...
    if limit == 0 || limit > 100 {
        return Err("limit must be 1 through 100".into());
    }
    let after = flags
        .get("--after")
        .map(|s| PrincipalId::from_hex(s).map_err(|_| "invalid reviewer cursor".to_owned()))
        .transpose()?;
    let head = flags
        .get("--expected-head")
        .map(|s| parse_head(s))
    ...
    if after.is_some() && head.is_none() {
        return Err("review continuation requires --expected-head from the first page".into());
    }
    Ok(ReadOptions {
        scope,
        format,
...
fn run_read(options: ReadOptions) -> Result<u8, String> {
    ...
    let Some(page) = answer else {
        emit(
            ...
            &format!(
                "{{\"type\":\"pull_request_reviews\",{scope},\"found\":false,\"node_closed\":true}}"
            ),
        )?;
        return Ok(4);
    };
    if page.pull_request != options.scope.number
        || options.head.is_some_and(|head| head != page.source_head)
        || page.reviews.len() > usize::from(options.limit)
        || page
            .reviews
            .windows(2)
            .any(|p| p[0].event.reviewer >= p[1].event.reviewer)
        || page.reviews.iter().any(|v| {
            options.after.is_some_and(|a| v.event.reviewer <= a)
    ...
    let rows = page
        ...
        .map(|view| {
            format!(
                "{{{},\"review_version\":{},\"freshness\":{},\"reviewer_is_opener\":{}}}",
                review_fields(&view.event),
                view.version.get(),
                quote(&format!("{:?}", view.freshness)),
                view.reviewer_is_opener
    ...
    let text = format!(
        concat!(
            "{{\"type\":\"pull_request_reviews\",\"schema_version\":1,{},\"found\":true,",
        ...
        page.pull_request_version.get(),
        page.policy_epoch.get(),
        page.next_after
            .map_or_else(|| "null".into(), |id| quote(&id.to_string())),
        rows
    );
    emit(&mut std::io::stdout().lock(), &text)?;
    Ok(0)
}
fn scope_fields(scope: &Scope) -> String {
    format!(
        "\"tenant_id\":{},\"repository_id\":{},\"pull_request\":{}",
        quote(&scope.tenant.to_string()),
        quote(&scope.repository.to_string()),
        scope.number.get()
    )
}
fn subject_fields(subject: &ReviewSubject, candidate: Option<CandidateBinding>) -> String {
    format!(
        concat!(
            "\"pull_request_version\":{},\"source_reference_hex\":{},\"target_reference_hex\":{},",
            "\"source_tip\":{},\"target_tip\":{},\"policy_epoch\":{},\"candidate_commit\":{},\"merge_base\":{}"
        ),
        subject.pull_request_version.get(),
        quote(&hex(subject.source_ref.as_bytes())),
        quote(&hex(subject.target_ref.as_bytes())),
...
fn review_fields(review: &NativeReviewEvent) -> String {
    format!(
        "{},\"reviewer\":{},\"decision\":{},\"reason\":{},\"profile\":{}",
        subject_fields(&review.subject, review.candidate),
        quote(&review.reviewer.to_string()),
        quote(decision_name(review.decision)),
        quote(&review.reason),
        quote(if review.candidate.is_some() {
            CANDIDATE_REVIEW_PROFILE
        } else {
            REVIEW_PROFILE
        })
    )
...
fn render_terminal(
    ...
    let version = match options.command.review.expected_version {
        ExpectedVersion::NewStream => 0,
        ExpectedVersion::Exactly(v) => v.get(),
    };
    format!(
        concat!(
            "{{\"schema_version\":1,\"type\":{},\"profile\":{}, {}, {},\"principal\":{},",
        ...
        quote(if mode == Mode::Review {
            "candidate_review_decision"
        } else {
            "reviewed_merge_publication"
        }),
        quote(if mode == Mode::Review {
            CANDIDATE_REVIEW_PROFILE
        } else {
            "named-candidate-reviewers-v1"
        }),
        scope_fields(&options.scope),
        subject_fields(
            &options.command.review.subject,
            Some(options.command.candidate)
        ),
        quote(&options.principal.to_string()),
        if mode == Mode::Review {
            quote(decision_name(options.command.review.decision))
        } else {
            "null".into()
        },
        if mode == Mode::Review {
...
fn emit(output: &mut impl Write, text: &str) -> Result<(), String> {
    if text.len() > MAX_JSON {
        return Err("review JSON exceeds the bounded response profile".into());
    }
    writeln!(output, "{text}")
        .and_then(|()| output.flush())
        .map_err(|e| format!("review output incomplete: {e}"))
}
...
fn parse_head(text: &str) -> Result<RepositoryAuthorityHeadId, String> {
    ...
    let algorithm = DigestAlgorithmId::try_new(
        u16::try_from(decimal(algorithm)?).map_err(|_| "algorithm overflow")?,
    )
...
mod tests {
    ...
    fn args(width: usize, mode: Mode) -> Vec<String> {
        ...
        if mode == Mode::Review {
            ...
            args.extend(["--require-reviewer".into(), "44".repeat(16)]);
        }
        args
    }
    ...
    fn every_candidate_coordinate_is_explicit_and_both_hash_formats_are_supported() {
        for mode in [Mode::Review, Mode::Apply] {
            for width in [40, 64] {
                let good = args(width, mode);
                assert_eq!(
                    parse_mutation(&good, mode)
                        .unwrap()
                        .command
                        .candidate
                        .commit
                ...
                for flag in [
                    ...
                    let at = bad.iter().position(|a| a == flag).unwrap();
                    bad.drain(at..at + 2);
                    assert!(parse_mutation(&bad, mode).is_err(), "{flag}");
                    let mut bad = good.clone();
                    let at = bad.iter().position(|a| a == flag).unwrap();
                    bad.extend([flag.into(), bad[at + 1].clone()]);
                    assert!(parse_mutation(&bad, mode).is_err());
                }
                let mut bad = good.clone();
                change(
                    &mut bad,
                    "--source-tip",
                    &"a".repeat(if width == 40 { 64 } else { 40 }),
                );
                assert!(parse_mutation(&bad, mode).is_err());
                let mut bad = good.clone();
                bad.retain(|a| a != "--trusted-local");
                assert!(parse_mutation(&bad, mode).is_err());
            }
        }
    }
    ...
    fn withdrawal_and_named_reviewers_have_no_implicit_fallback() {
        let mut withdraw = args(40, Mode::Review);
        change(&mut withdraw, "--decision", "withdraw");
        change(&mut withdraw, "--review-version", "1");
        withdraw.extend(["--reason".into(), "retract exact candidate".into()]);
        assert!(parse_mutation(&withdraw, Mode::Review).is_err());
        let at = withdraw.iter().position(|a| a == "--bundle").unwrap();
        withdraw.drain(at..at + 2);
        assert!(parse_mutation(&withdraw, Mode::Review).is_ok());
        change(&mut withdraw, "--review-version", "0");
        assert!(parse_mutation(&withdraw, Mode::Review).is_err());
        let mut apply = args(40, Mode::Apply);
        apply.extend(["--require-reviewer".into(), "44".repeat(16)]);
        assert!(parse_mutation(&apply, Mode::Apply).is_err());
        let mut apply = args(40, Mode::Apply);
        change(&mut apply, "--require-reviewer", &"33".repeat(16));
        assert!(parse_mutation(&apply, Mode::Apply).is_err());
        let mut apply = args(40, Mode::Apply);
        apply.truncate(apply.len() - 2);
        assert!(parse_mutation(&apply, Mode::Apply).is_err());
    }
    #[test]
    fn continuations_require_the_original_head_and_tokens_roundtrip() {
        ...
        assert!(parse_read(&read).is_ok());
        read.extend(["--after".into(), "44".repeat(16)]);
        assert!(parse_read(&read).is_err());
        ...
        read.extend(["--expected-head".into(), head_token(head)]);
        assert_eq!(parse_read(&read).unwrap().head, Some(head));
        read.extend(["--limit".into(), "0".into()]);
        assert!(parse_read(&read).is_err());
    }
    #[test]
    fn terminal_receipts_keep_commits_and_cleanup_failures_without_leaking_retry_keys() {
        ...
        let digest = DigestBytes::try_new(&[7; 32]).unwrap();
        ...
        assert!(text.contains("\"git_refs_changed\":true"));
        ...
        let error = write_terminal_receipt(&mut Fail, &text, tx, &terminal).unwrap_err();
