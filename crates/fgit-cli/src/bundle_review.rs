...
use super::{
    ComparisonMode, HeadGeneration, NodeConfig, OneNode, Options, RefName, ReviewSelection, emit,
    hex, quote, render, text, unhex,
};
...
pub fn run(arguments: &[String], merging: bool) -> Result<(), String> {
    ...
    let operation = (|| {
        ...
        match &input.merge {
            ...
            None => node.runtime().block_on(node.inspect_workspace_bundle_in(
                &request,
                &input.target,
                input.base,
                input.candidate,
    ...
    let result = match (operation, cleanup) {
        ...
        (Ok(_), Some(error)) => {
            return Err(format!(
                "inspection node shutdown failed: {error}; no successful report returned"
            ));
        }
    ...
    let expected_parents = input.merge.as_ref().map_or_else(
        || vec![input.base],
    ...
    let parents = result
        ...
        .collect::<Vec<_>>()
        .join(",");
    let prerequisites = result
        .prerequisites
        .iter()
        .map(|id| quote(&id.to_string()))
        .collect::<Vec<_>>()
        .join(",");
    let source_reference = input.merge.as_ref().map_or_else(
        || "null".into(),
        |merge| quote(&hex(merge.source_ref.as_bytes())),
    );
    let merge_base = result
        .merge_base
        .map_or_else(|| "null".into(), |id| quote(&id.to_string()));
    let report = format!(
        concat!(
            "{{\"type\":\"candidate_review\",\"schema_version\":1,\"profile\":\"verified-bundle-v1\",",
        ...
        quote(if merging { "merge" } else { "workspace" }),
        quote(&hex(&result.bundle_sha256)),
        result.bundle_bytes,
        result.pack_bytes,
        result.pack_objects,
        result.expanded_bytes,
        result.closure_objects,
        result.transport_only_objects,
        quote(&input.candidate.to_string()),
        parents,
        merge_base,
        source_reference,
        prerequisites,
        quote(&hex(&result.candidate_commit_body)),
        text(&result.candidate_commit_body),
        review
    );
    if report.len() > super::MAX_JSON_BYTES {
        return Err("inspection JSON exceeds the output limit".into());
    }
...
fn parse(arguments: &[String], merging: bool) -> Result<Input, String> {
    ...
    while cursor < arguments.len() {
        ...
        cursor += 1;
        if matches!(flag, "--trusted-local" | "--refs-hex") {
            shared.push(flag.to_owned());
            continue;
        }
        let value = arguments
            .get(cursor)
        ...
        if let Some(key) = own {
            if values.insert(key, value.as_str()).is_some() {
                return Err(format!("duplicate inspection option {flag:?}"));
            }
        } else if matches!(
            flag,
            "--expected-head"
            ...
            return Err(format!(
                "unknown or inapplicable inspection option {flag:?}"
            ));
        }
    }
    let required = |name: &str| {
        values
            .get(name)
            .copied()
            .ok_or_else(|| format!("missing inspection expectation {name}"))
    };
    let base = parse_oid(required("base")?)?;
    let candidate = parse_oid(required("candidate")?)?;
    if candidate == base || candidate.algorithm() != base.algorithm() {
        return Err(
            "candidate and parent identities must be distinct and use the same format".into(),
        );
    }
    if !format_supplied {
        shared.extend(["--object-format".into(), base.algorithm().as_str().into()]);
    }
    let options = super::parse(&shared, false)?;
    if options.format != base.algorithm() {
        return Err("explicit object format disagrees with candidate expectations".into());
    }
    let ReviewSelection::References { before: target, .. } = &options.selection else {
        return Err("invalid target selection".into());
    };
    let target = target.clone();
    if !target.as_bytes().starts_with(b"refs/heads/") {
        return Err("candidate target must be a branch".into());
    }
    let merge = if merging {
        ...
        let merge = NativeMerge {
            ...
            source_tip: parse_oid(required("--expected-source")?)?,
            base_tip: parse_oid(required("--merge-base")?)?,
            target_ref: target.clone(),
            target_tip_before: base,
            merge_commit: candidate,
        };
        merge
            .validate()
            .map_err(|_| "invalid native merge expectations")?;
        if merge.source_tip == base {
            return Err("merge parents must be distinct".into());
        }
        Some(merge)
    } else {
        None
...
mod tests {
    ...
    fn exact_expectations_and_direct_comparison_are_required_in_both_domains() {
        for width in [40, 64] {
            for merging in [false, true] {
                let args = input(merging, width);
                let parsed = parse(&args, merging).unwrap();
                ...
                for flag in [
                    ...
                    assert!(parse(&missing, merging).is_err());
                }
            }
        }
    }
    ...
    fn duplicate_unknown_mutation_and_source_side_diff_options_refuse_before_io() {
        for merging in [false, true] {
            for extras in [
                vec!["--comparison", "merge-base"],
                vec!["--approve", "yes"],
                vec!["--principal", "x"],
                vec!["--expected-commit", "bad"],
                vec!["--max-changes", "0"],
                vec!["--object-format", "sha256"],
                vec!["--trusted-local"],
            ] {
                let mut args = input(merging, 40);
    ...
    fn raw_refs_pins_and_paths_reuse_the_existing_review_contract() {
        let mut args = input(true, 64);
        args[3] = hex(b"refs/heads/\xff");
        let at = args.iter().position(|arg| arg == "--source-ref").unwrap();
        args[at] = "--source-ref-hex".into();
        args[at + 1] = hex(b"refs/heads/\xfe");
        args.extend([
            "--refs-hex".into(),
            "--path-hex".into(),
            "ff".into(),
        ...
        assert_eq!(
            value.merge.unwrap().source_ref.as_bytes(),
            b"refs/heads/\xfe"
        );
