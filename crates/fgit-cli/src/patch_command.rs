...
pub(super) fn run(args: &[String]) -> Result<u8, String> {
    ...
    let mut node = OneNode::open_existing(
        NodeConfig::new(options.storage.clone(), options.tenant, options.repository)
            .with_object_format(options.base.algorithm()),
    )
    .map_err(|error| error.to_string())?;
    let result = (|| {
        ...
        match &options.operation {
            ...
            Operation::Apply {
                ...
                let [command] = admitted.commands.as_slice() else {
                    return Err(
                        "unexpected publication outcome count; do not infer non-commit".into(),
                    );
                };
                if !admitted.session.atomic || admitted.session.tx_ids != vec![command.tx_id] {
                    return Err(format!(
                        "unexpected publication mapping; {}",
    ...
    match result {
        Ok(Completed::Prepared(candidate)) => {
            ...
            let Operation::Prepare { output, .. } = &options.operation else {
                return Err("patch operation mismatch".into());
            };
        ...
        Err(error) => {
            ...
            match options.operation {
                Operation::Prepare { .. } => Err(format!(
                    "patch preparation failed: {error}{cleanup}; no bundle was published"
                )),
                Operation::Apply { .. } => Err(format!(
                    "no terminal patch outcome returned: {error}{cleanup}; this is not evidence of non-commit. Retry the identical saved bundle, principal, key and expected IDs, or use fg outcome; do not regenerate the patch or change the key"
                )),
            }
        }
    }
...
enum Completed {
    Prepared(WorkspacePatchCandidate),
...
fn prepared_receipt(
    ...
    if options.base != candidate.source_commit
        || options.base.algorithm() != candidate.object_format
        || candidate.candidate_commit.algorithm() != candidate.object_format
        || candidate.root_tree.algorithm() != candidate.object_format
        || candidate.candidate_commit == candidate.source_commit
        || candidate.bundle_bytes().is_empty()
    {
    ...
    Ok(format!(
        concat!(
            "{{\"type\":\"patch_preparation\",\"schema_version\":1,\"profile\":\"exact-v1\",",
        ...
        quote(candidate.object_format.as_str()),
        quote(&hex(options.reference.as_bytes())),
        quote(&candidate.source_commit.to_string()),
        ...
        quote(&hex(&fgit_crypto::sha256_digest(candidate.bundle_bytes()))),
        candidate.bundle_bytes().len(),
...
fn finish_applied(
    ...
    let Operation::Apply {
        ...
        return Err("patch receipt operation mismatch".into());
    };
    ...
    let receipt = format!(
        concat!(
            "{{\"type\":\"patch_publication\",\"schema_version\":1,\"outcome\":{},",
