...
pub fn parse_oid(text: &str) -> Result<GitOid, String> {
    ...
    if id.is_zero() {
        return Err("zero is not a commit ID".to_owned());
    }
    Ok(id)
}
...
pub fn read_bundle(path: &Path, limit: usize) -> Result<Vec<u8>, String> {
    ...
    if !metadata.is_file() || metadata.len() > limit as u64 {
        return Err(
            "candidate bundle must be a bounded regular file, not a symlink or device".to_owned(),
        );
    }
    ...
    loop {
        ...
        bytes
            .try_reserve(n)
            .map_err(|_| "candidate bundle allocation refused")?;
        bytes.extend_from_slice(&chunk[..n]);
    }
    if bytes.is_empty() {
        return Err("candidate bundle is empty".to_owned());
    }
    Ok(bytes)
}
...
mod tests {
    ...
    fn saved_pr_bidi_and_c1_escapes_do_not_change_metadata() {
        ...
        for scalar in [
            ...
            assert_eq!(
                quote(&format!("before{character}after")),
    ...
    fn bundle_symlinks_are_not_accepted_as_regular_artifacts() {
        let root =
            std::env::temp_dir().join(format!("fg-publication-symlink-{}", std::process::id()));
