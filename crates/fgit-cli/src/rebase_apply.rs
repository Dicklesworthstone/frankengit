...
use fgit_authority::{MAX_IDEMPOTENCY_KEY_BYTES, TerminalOutcome};
...
const USAGE: &str =
    "usage: fg rebase apply <storage-root> <tenant-id> <repository-id> <source-ref> <bundle>
...
pub fn run(args: &[String]) -> Result<u8, String> {
    ...
    let mut node = OneNode::open_existing(
        NodeConfig::new(options.storage.clone(), options.tenant, options.repository)
            .with_object_format(options.source.algorithm()),
    )
    ...
    let result = (|| {
        ...
        if !admitted.session.atomic
            || admitted.commands.len() != 1
            || admitted.session.tx_ids != vec![first.tx_id]
        {
            return Err(format!(
                "inconsistent rebase receipt; {}",
    ...
    match result {
        ...
        Err(error) => {
            ...
            Err(format!(
                "rebase application did not return a complete receipt: {error}{cleanup}; an error is not evidence of non-commit. Use fg outcome with the original principal/key or retry identical source/onto/candidate inputs; do not change the key"
            ))
        }
    }
}
...
fn finish(
    ...
    let receipt = format!(
        concat!(
            "{{\"type\":\"rebase_publication\",\"schema_version\":1,\"atomic\":true,",
        ...
        quote(options.source.algorithm().as_str()),
        quote(&hex(options.reference.as_bytes())),
        quote(&options.source.to_string()),
        quote(&options.onto.to_string()),
        quote(&options.candidate.to_string()),
        cleanup.is_none(),
