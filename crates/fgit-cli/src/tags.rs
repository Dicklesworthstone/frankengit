...
use fgit_forge::tags::{MAX_TAG_MESSAGE_BYTES, TagCommand, TagRead, TagSignatureState};
...
use options::{Key, Operation, Options, head_token, hex};
use std::{
    fs,
    io::{Read, Write},
    path::Path,
...
const USAGE: &str =
    "usage: fg tag <create|annotate|delete|list|show> <storage-root> <tenant-id> <repository-id>
...
fn message_bytes(path: &Path) -> Result<Vec<u8>, String> {
    ...
    if !metadata.is_file() || metadata.len() > MAX_TAG_MESSAGE_BYTES as u64 {
        return Err("message must be a bounded regular file, not a symlink or device".into());
    }
    let file = fs::File::open(path).map_err(|e| e.to_string())?;
    let opened = file.metadata().map_err(|e| e.to_string())?;
    if !opened.is_file() || opened.len() > MAX_TAG_MESSAGE_BYTES as u64 {
        return Err("message file changed type or exceeded its bound".into());
    }
    let mut bytes = Vec::new();
    file.take((MAX_TAG_MESSAGE_BYTES + 1) as u64)
        .read_to_end(&mut bytes)
        .map_err(|e| e.to_string())?;
    if bytes.len() > MAX_TAG_MESSAGE_BYTES || bytes.contains(&0) {
        return Err("message exceeds 65536 bytes or contains NUL".into());
    }
    Ok(bytes)
}
...
pub fn run(args: &[String]) -> Result<u8, String> {
    if args == ["--help"]
        || (args.len() == 2
            ...
            && matches!(
                args[0].as_str(),
                "create" | "annotate" | "delete" | "list" | "show"
            ))
    {
        write_read(&mut std::io::stdout().lock(), USAGE)?;
        return Ok(0);
    }
    ...
    let session = match &mut options.operation {
        Operation::Mutate {
            ...
            if let (TagCommand::Annotated { metadata, .. }, Some(path)) = (command, message_file) {
                metadata.message = message_bytes(path)?;
            }
            Some(LoopbackReceiveSession::authenticated(
                *principal,
                IdempotencyKey::new(read_key(key, &mut std::io::stdin().lock())?)
                    .map_err(|e| e.to_string())?,
            ))
        }
        _ => None,
    ...
    let operation = (|| {
        ...
        match &options.operation {
            Operation::Mutate { command, .. } => {
                let result = node
                    .runtime()
                    .block_on(node.admit_tag_durable_in(
                        &request,
                        session.as_ref().ok_or("tag authentication missing")?,
                        command,
                        Default::default(),
                    ))
                    .map_err(|e| e.to_string())?;
            ...
            Operation::List { after, limit, head } => {
                let (head, rows, next) = node
                    .runtime()
                    .block_on(node.list_tag_refs_in(
                        &request,
                        &Default::default(),
                        after.as_ref(),
                        *limit,
                    ...
                    .map_err(|e| e.to_string())?;
                Ok(Completed::Read(list_receipt(
                    &options,
                    head,
                    &rows,
                    next.as_ref(),
    ...
    match operation {
        Ok(Completed::Mutation(tx, terminal)) => finish_mutation(
        ...
        Ok(Completed::Read(receipt)) => {
            if let Some(error) = cleanup {
                return Err(format!(
                    "tag node shutdown failed: {error}; no complete read returned"
                ));
            }
            write_read(&mut std::io::stdout().lock(), &receipt)?;
            Ok(0)
        }
        Err(error) => {
            let cleanup =
            ...
            if session.is_some() {
                Err(format!(
                    "no terminal tag outcome returned: {error}{cleanup}; this is not evidence of non-commit. Retry identical inputs/key or use fg outcome; do not silently replace the key or expected tip"
                ))
            } else {
                Err(format!(
                    "tag read failed: {error}{cleanup}; no complete read returned"
                ))
            }
        }
    }
...
fn write_read(output: &mut impl Write, receipt: &str) -> Result<(), String> {
    writeln!(output, "{receipt}")
        ...
        .map_err(|e| format!("tag read output incomplete: {e}"))
}
...
fn list_receipt(
    ...
    let rows = rows
        ...
        .join(",");
    format!(
        "{{\"type\":\"tag_page\",\"schema_version\":1,{},\"source_head\":{},\"snapshot_token\":{},\"tags\":[{rows}],\"next_after_hex\":{},\"has_more\":{},\"node_closed\":true,\"repository_changed\":false}}",
        scope(options),
        quote(&head.to_string()),
        quote(&head_token(head)),
        next.map_or_else(|| "null".into(), |r| quote(&hex(r.as_bytes()))),
        next.is_some()
    )
}
fn show_receipt(options: &Options, read: &TagRead) -> String {
    ...
    format!(
        "{{\"type\":\"tag_object\",\"schema_version\":1,{},\"source_head\":{},\"snapshot_token\":{},\"reference_hex\":{},\"tip\":{},\"peeled\":{},\"peeled_kind\":{},\"annotations\":[{annotations}],\"signature_verification\":\"not_performed\",\"node_closed\":true,\"repository_changed\":false}}",
        scope(options),
        quote(&read.head.to_string()),
        quote(&head_token(read.head)),
        quote(&hex(read.reference.as_bytes())),
        quote(&read.tip.to_string()),
        quote(&read.peeled.to_string()),
        quote(read.peeled_kind.label())
    )
}
fn finish_mutation(
    ...
    let Operation::Mutate {
        ...
        return Err("tag receipt operation mismatch".into());
    };
    ...
    let receipt = format!(
        "{{\"type\":\"tag_publication\",\"schema_version\":1,{},\"action\":{},\"reference_hex\":{},\"principal_id\":{},\"atomic\":true,\"outcome\":{},\"command_committed\":{},\"tx_id\":{},\"decision_sequence\":{},\"repository_commit_id\":{rcr},\"refusal_code\":{code},\"refusal_record_id\":{refusal},\"node_closed\":{},\"cleanup_error\":{}}}",
        scope(options),
        quote(&options.action),
        quote(&hex(command.reference().as_bytes())),
    ...
    if let Err(error) = write_terminal_receipt(output, &receipt, tx, terminal) {
        return Err(cleanup.map_or(error.clone(), |c| {
            format!("{error}; node shutdown also failed: {c}")
        }));
    }
    if let Some(error) = cleanup {
        return Err(format!(
