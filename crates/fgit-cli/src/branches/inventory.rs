...
use super::{NodeConfig, OneNode, HeadGeneration, GitOid, RefName, RepositoryAuthorityHeadId, quote};
use super::options::{Options, Operation, parse_inventory, head_token, hex};
use std::io::Write;
...
pub(super) fn run(args: &[String]) -> Result<u8, String> {
    ...
    let Operation::List { after, limit, expected_head } = &options.operation
        else { return Err("reference inventory is read-only".into()); };
    let mut node = OneNode::open_existing(NodeConfig::new(options.storage.clone(), options.tenant, options.repository)
        .with_object_format(options.format)).map_err(|error| error.to_string())?;
    let operation = (|| {
        node.bring_into_service(HeadGeneration::FIRST).map_err(|error| error.to_string())?;
        let request = node.request_context();
        node.runtime().block_on(node.list_refs_in(&request, &Default::default(), after.as_ref(), *limit, *expected_head))
            .map_err(|error| error.to_string())
    })();
    let cleanup = node.shutdown().err().map(|error| error.to_string());
    match (operation, cleanup) {
        (Ok((head, rows, next)), None) => {
            write_page(&mut std::io::stdout().lock(), &receipt(&options, head, &rows, next.as_ref()))?;
            Ok(0)
        }
        (result, cleanup) => {
...
fn receipt(options: &Options, head: RepositoryAuthorityHeadId, rows: &[(RefName, GitOid)], next: Option<&RefName>) -> String {
    let references = rows.iter().map(|(name, tip)| format!("{{\"reference_hex\":{},\"tip\":{}}}",
        quote(&hex(name.as_bytes())), quote(&tip.to_string()))).collect::<Vec<_>>().join(",");
    format!(concat!("{{\"type\":\"reference_page\",\"schema_version\":1,\"tenant_id\":{},\"repository_id\":{},",
        ...
        quote(&head.to_string()), quote(&head_token(head)), references,
        next.map_or_else(|| "null".into(), |name| quote(&hex(name.as_bytes()))), next.is_some())
}
...
mod tests {
    ...
    fn args() -> Vec<String> { vec!["unopened".into(), "01".repeat(16), "02".repeat(16), "--trusted-local".into()] }
    fn head() -> RepositoryAuthorityHeadId {
        RepositoryAuthorityHeadId::from_digest(DigestAlgorithmId::try_new(2).unwrap(), CANONICAL_CODEC_VERSION, DigestBytes::try_new(&[5;32]).unwrap())
    }
    #[test]
    fn read_cursor_accepts_raw_remote_refs_but_never_mutation_flags() {
        let mut values = args();
        values.extend(["--after-hex".into(), hex(b"refs/remotes/origin/\xff"), "--expected-head".into(), head_token(head())]);
        let parsed = parse_inventory(&values).unwrap();
        let Operation::List { after, .. } = parsed.operation else { panic!() };
        assert_eq!(after.unwrap().as_bytes(), b"refs/remotes/origin/\xff");
        let mut branch = vec!["list".into()]; branch.extend(values.clone());
        assert!(super::super::options::parse(&branch).is_err());
        for flag in ["--principal", "--idempotency-key", "--target", "--force", "--key-stdin"] {
            let mut bad = args(); bad.extend([flag.into(), "forbidden".into()]);
            assert!(parse_inventory(&bad).is_err());
        }
        values.truncate(values.len()-2); assert!(parse_inventory(&values).is_err());
        let mut no_trust = args(); no_trust.pop(); assert!(parse_inventory(&no_trust).is_err());
    }
    #[test]
    fn lossless_receipt_and_flush_failure_remain_distinct_from_complete_pages() {
        ...
        let page = receipt(&options, head(), &[(reference.clone(), tip)], Some(&reference));
        assert!(page.contains("\"type\":\"reference_page\""));
        assert!(page.contains(&hex(reference.as_bytes()))); assert!(page.contains(&tip.to_string()));
        assert!(page.contains("\"has_more\":true")); assert!(page.contains(&head_token(head())));
        struct Broken;
        ...
        assert!(write_page(&mut Broken, &page).unwrap_err().contains("incomplete"));
        let mut bytes = Vec::new(); write_page(&mut bytes, &page).unwrap();
