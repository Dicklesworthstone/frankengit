use fgit_authority::{ExpectedOld, MAX_IDEMPOTENCY_KEY_BYTES, ProposedNew, RefCommand};
use fgit_types::hash::{DigestAlgorithmId, DigestBytes};
use fgit_types::{
    CANONICAL_CODEC_VERSION, GitHashAlgorithm, GitOid, PrincipalId, RefName,
...
pub(super) fn parse_inventory(args: &[String]) -> Result<Options, String> {
    if args.len() > 27
        ...
        || args.iter().map(String::len).sum::<usize>() > 32764
    {
        return Err("reference inventory arguments exceed the bounded profile".into());
    }
    let mut read = Vec::with_capacity(args.len() + 1);
    read.push("list".into());
    read.extend_from_slice(args);
...
fn parse_with_read_prefix(args: &[String], read_prefix: &[u8]) -> Result<Options, String> {
    ...
    if !matches!(action, "list" | "create" | "update" | "delete" | "rename") {
        return Err(super::USAGE.into());
    }
    if args[1].is_empty() || args[1].len() > 4096 {
        return Err("invalid branch storage path".into());
    }
    ...
    while index < args.len() {
        ...
        let permitted = if action == "list" {
            matches!(
                flag,
                "--after" | "--after-hex" | "--limit" | "--expected-head"
            )
        } else {
            matches!(
                flag,
                "--principal" | "--idempotency-key" | "--key-stdin" | "--ref" | "--ref-hex"
        ...
        if !common && !permitted {
            return Err(format!("inapplicable branch option {flag:?}"));
        }
        ...
        if flags.insert(flag, value).is_some() {
            return Err(format!("duplicate branch option {flag}"));
        }
    }
    if !flags.contains_key("--trusted-local") {
        return Err(
            "--trusted-local is required; branch names and keys are not credentials".into(),
        );
    }
    ...
    let operation = if action == "list" {
        let after = if flags.contains_key("--after") || flags.contains_key("--after-hex") {
            Some(reference_with_prefix(
                &flags,
                "--after",
                "--after-hex",
                read_prefix,
        ...
        let expected_head = flags
            .get("--expected-head")
            .map(|value| parse_head(value))
            .transpose()?;
        if after.is_some() && expected_head.is_none() {
            return Err("branch continuation requires --expected-head from the first page".into());
        }
        let limit = flags
            .get("--limit")
        ...
        if !(1..=100).contains(&limit) {
            return Err("branch limit must be 1..100".into());
        }
        Operation::List {
            after,
            limit: limit as u16,
        ...
        let key = match (
            ...
            (Some(key), false) if !key.is_empty() && key.len() <= MAX_IDEMPOTENCY_KEY_BYTES => {
                KeyInput::Bytes(key.as_bytes().to_vec())
            }
            (None, true) => KeyInput::Stdin,
            _ => return Err("supply exactly one bounded key or --key-stdin".into()),
        };
        let name = reference(&flags, "--ref", "--ref-hex")?;
        let oid = |flag| -> Result<GitOid, String> {
            let value = GitOid::from_hex(format, required(&flags, flag)?)
                .map_err(|_| "invalid native object ID")?;
            if value.is_zero() {
                return Err("branch tips must be nonzero".into());
            }
            Ok(value)
        };
        let commands = match action {
            "create" => vec![RefCommand {
                name,
                expected_old: ExpectedOld::Absent,
                proposed_new: ProposedNew::Update(oid("--target")?),
                force: false,
            }],
            "update" => vec![RefCommand {
                name,
                expected_old: ExpectedOld::Exactly(oid("--expected-tip")?),
                proposed_new: ProposedNew::Update(oid("--target")?),
                force: false,
            }],
            "delete" => vec![RefCommand {
                name,
                expected_old: ExpectedOld::Exactly(oid("--expected-tip")?),
            ...
            "rename" => {
                ...
                if destination == name {
                    return Err("rename requires distinct source and destination branches".into());
                }
                let tip = oid("--expected-tip")?;
                vec![
                    RefCommand {
                        name,
                        expected_old: ExpectedOld::Exactly(tip),
                        proposed_new: ProposedNew::Delete,
                        force: false,
                    },
                    RefCommand {
                        name: destination,
                        expected_old: ExpectedOld::Absent,
                        proposed_new: ProposedNew::Update(tip),
                        force: false,
                    },
                ]
            }
...
fn reference_with_prefix(
    flags: &BTreeMap<&str, &str>,
    plain: &str,
    encoded: &str,
    prefix: &[u8],
) -> Result<RefName, String> {
    ...
    if !bytes.starts_with(prefix) {
        return Err("a full reference in the selected namespace is required".into());
    }
...
mod tests {
    ...
    fn parse_exact_atomic_rename_and_non_utf8_reference() {
        let mut args = args("rename");
        args.extend([
            "--expected-tip".into(),
            "1".repeat(40),
            "--destination-hex".into(),
            hex(b"refs/heads/\xff"),
        ]);
        let options = parse(&args).unwrap();
        let Operation::Mutate { commands, .. } = options.operation else {
            panic!()
        };
        assert_eq!(commands.len(), 2);
        assert_eq!(commands[1].name.as_bytes(), b"refs/heads/\xff");
        assert!(matches!(commands[1].expected_old, ExpectedOld::Absent));
        assert!(matches!(commands[0].proposed_new, ProposedNew::Delete));
    }
    #[test]
    fn mutation_flags_are_action_specific_and_duplicates_refuse() {
        let mut args = args("create");
        args.extend(["--target".into(), "1".repeat(40)]);
        assert!(parse(&args).is_ok());
        let mut wrong = args.clone();
        wrong.extend(["--expected-tip".into(), "2".repeat(40)]);
        assert!(parse(&wrong).is_err());
        let mut duplicate = args.clone();
        duplicate.extend(["--ref-hex".into(), hex(b"refs/heads/other")]);
        assert!(parse(&duplicate).is_err());
        let mut both = args.clone();
        both.push("--key-stdin".into());
        assert!(parse(&both).is_err());
        let mut zero = args.clone();
        *zero.last_mut().unwrap() = "0".repeat(40);
    ...
    fn listing_requires_snapshot_pins_and_bounded_canonical_limits() {
        ...
        for value in ["0", "101", "01", "-1"] {
            let mut args = base.clone();
            args.extend(["--limit".into(), value.into()]);
            assert!(parse(&args).is_err());
        }
        let mut args = base;
        args.extend(["--after".into(), "refs/heads/a".into()]);
        assert!(parse(&args).is_err());
        args.extend([
            "--expected-head".into(),
