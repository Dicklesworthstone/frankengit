...
use fgit_types::{
    CANONICAL_CODEC_VERSION, GitHashAlgorithm, PrincipalId, RepositoryAuthorityHeadId,
...
pub(super) fn parse(arguments: &[String]) -> Result<Options, String> {
    ...
    if !mutate && !matches!(action, "list" | "show") {
        return Err(super::USAGE.to_owned());
    }
    ...
    let number = if action == "list" {
        ...
        Some(
            IssueNumber::try_new(decimal(
                arguments.get(4).ok_or("an issue number is required")?,
            )?)
    ...
    while cursor < arguments.len() {
        ...
        if flag == "--clear-labels" {
            if action != "edit" || clear_labels {
                return Err("--clear-labels is permitted once, only for edit".to_owned());
            }
            clear_labels = true;
            continue;
        }
        let allowed = flag == "--object-format"
            || if mutate {
                matches!(
                    ...
                    || (matches!(action, "open" | "edit" | "comment")
                        && matches!(flag, "--body" | "--body-file"))
            } else {
        ...
        if !allowed {
            return Err(format!("unknown or inapplicable issue option {flag:?}"));
        }
        let value = arguments
            .get(cursor)
        ...
        if flag == "--label" {
            if labels.len() == MAX_LABELS {
                return Err("at most 32 issue labels may be supplied".to_owned());
            }
    ...
    if !trusted {
        return Err(
            "--trusted-local is required for this repository-local operator interface".to_owned(),
        );
    }
    if clear_labels && !labels.is_empty() {
        return Err("--clear-labels and --label cannot be combined".to_owned());
    }
    labels.sort();
    if labels.windows(2).any(|pair| pair[0] == pair[1]) {
        return Err("duplicate issue label".to_owned());
    }
    ...
    let operation = if mutate {
        ...
        if (action == "open") != (version == 0) {
            return Err(
                "open requires version 0; all other changes require a positive exact version"
                    .to_owned(),
            );
        }
        ...
        let body_file = flags
            ...
            .map(|path| {
                if path.is_empty() || path.len() > 4096 {
                    Err("body-file must name a bounded nonempty path".to_owned())
                } else {
                    Ok(PathBuf::from(path))
                }
            })
            .transpose()?;
        if body_file.is_some() && flags.contains_key("--body") {
            return Err("supply only one of --body and --body-file".to_owned());
        }
        ...
        if matches!(action, "open" | "comment") && !supplied_body {
            return Err(
                "--body or --body-file is required; an empty opening body is explicit".to_owned(),
            );
        }
        ...
        let action = match action {
            ...
            "edit" => IssueAction::Edit(IssueEdit {
                title,
                body: if supplied_body {
                    Some(body.unwrap_or_default())
                } else {
                    None
                },
                labels: if clear_labels || !labels.is_empty() {
                    Some(labels)
                } else {
                    None
                },
        ...
        let command = IssueCommand {
            number: number.ok_or("missing issue number")?,
            expected_version,
            action,
        };
        ...
        if body_file.is_some() {
            *body_slot(&mut probe)? = "file-body-probe".to_owned();
        }
        probe
            .proposed_event(principal)
            .map_err(|_| "invalid issue title, body, labels, or action/version combination")?;
        ...
        if after != 0 && expected_head.is_none() {
            return Err(
                "continuation requires --expected-head from the first page's snapshot_token"
                    .to_owned(),
            );
        }
        Operation::Read(ReadOptions {
            number,
            after,
            limit: u16::try_from(limit).map_err(|_| "invalid page limit")?,
            expected_head,
        })
    };
    Ok(Options {
...
pub(super) fn parse_head(text: &str) -> Result<RepositoryAuthorityHeadId, String> {
    ...
    let algorithm = DigestAlgorithmId::try_new(
        u16::try_from(decimal(algorithm)?).map_err(|_| "head algorithm overflow")?,
    )
