...
pub(super) fn search(
    ...
    if read.number.is_some()
        ...
        || (page.stop == SearchStop::ScanLimit
            && (page.scanned != max_scan || page.issues.len() == usize::from(read.limit)))
    {
    ...
    for (index, row) in page.issues.iter().enumerate() {
        if !query
            .matches(row)
            .map_err(|_| "invalid issue search snapshot")?
        {
