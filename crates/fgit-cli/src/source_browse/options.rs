...
use fgit_types::{
    CANONICAL_CODEC_VERSION, GitHashAlgorithm, GitOid, RefName, RepositoryAuthorityHeadId,
...
pub(super) fn parse(args: &[String], file: bool) -> Result<Options, String> {
    ...
    while cursor < args.len() {
        ...
        let allowed = matches!(
            ...
            matches!(flag, "--offset" | "--max-bytes")
        } else {
            matches!(flag, "--after-hex" | "--limit")
        };
        if !allowed {
            return Err(format!("unknown source read option {flag}"));
        }
        let value = args
            .get(cursor)
    ...
    if !trusted {
        return Err("--trusted-local is required for local-owner source disclosure".into());
    }
    ...
    let expected_commit = flags
        .get("--expected-commit")
        .map(|value| GitOid::from_hex(format, value).map_err(|e| e.to_string()))
        .transpose()?;
    let action = if file {
        let offset = flags
            .get("--offset")
        ...
        let limit = flags
            ...
            .transpose()?
            .unwrap_or(65536);
        if limit == 0 || limit > u64::from(MAX_SOURCE_PAGE_BYTES) {
            return Err("byte limit must be 1..1048576".into());
        }
        SourceBrowseAction::Read {
            offset,
            limit: u32::try_from(limit).map_err(|_| "byte limit overflow")?,
        }
    } else {
        let limit = flags
            .get("--limit")
        ...
        if !(1..=1000).contains(&limit) {
            return Err("directory page limit must be 1..1000".into());
        }
        let after = flags
            .get("--after-hex")
        ...
        SourceBrowseAction::List {
            after,
            limit: u16::try_from(limit).map_err(|_| "page limit overflow")?,
        }
    };
...
fn raw_pair(
    ...
    match (flags.get(plain), flags.get(encoded)) {
        (None, None) => Ok(None),
        (Some(value), None) if !value.is_empty() && value.len() <= 4096 => {
            Ok(Some(value.as_bytes().to_vec()))
        }
        (None, Some(value)) => unhex(value, 4096).map(Some),
        _ => Err(format!(
            "supply at most one bounded nonempty {plain} or {encoded}"
        )),
    }
}
