...
use fgit_forge::event::protection::{
    BranchReviewRule, MAX_BRANCH_REVIEWERS, MAX_PROTECTED_BRANCHES, MAX_PROTECTION_ADMINISTRATORS,
    ProtectionCommand, RepositoryProtectionPolicy,
};
...
fn parse(args: &[String]) -> Result<Options, String> {
    if args.len() < 4 || !matches!(args[0].as_str(), "show" | "set") {
    ...
    if args[1].is_empty() || args[1].len() > 4096 {
        return Err("invalid bounded storage path".into());
    }
    ...
    while cursor < args.len() {
        ...
        if flag == "--clear-rules" {
            if !set || clear {
                return Err("--clear-rules is permitted once, only for set".into());
            }
            clear = true;
            continue;
        }
        ...
        match flag {
            "--administrator" => {
                if administrators.len() == MAX_PROTECTION_ADMINISTRATORS {
                    return Err("too many administrators".into());
                }
                administrators.push(principal(value)?);
            }
            "--rule" => {
                if branches.len() == MAX_PROTECTED_BRANCHES {
                    return Err("too many protected branches".into());
                }
                let target = RefName::try_new(value.as_bytes())
                    .map_err(|_| "invalid protected branch name")?;
                let names = args
                    .get(cursor)
                    .ok_or("each --rule requires a separate comma-delimited reviewer list")?;
                cursor += 1;
                let mut reviewers = Vec::new();
                for name in names.split(',') {
                    if reviewers.len() == MAX_BRANCH_REVIEWERS {
                        return Err("too many required reviewers".into());
                    }
    ...
    let change = if set {
        if clear == !branches.is_empty() {
            return Err("supply --rule entries OR explicit --clear-rules".into());
        }
        ...
        let expected_version = if version == 0 {
            ...
            ExpectedVersion::Exactly(AggregateVersion::try_new(version).ok_or("invalid version")?)
        };
        let expected_policy_epoch =
            PolicyEpoch::try_new(number(required("--expected-policy-epoch")?)?)
                .map_err(|_| "invalid policy epoch")?;
        administrators.sort();
        branches.sort_by(|a, b| a.target.cmp(&b.target));
        let command = ProtectionCommand {
            expected_version,
            expected_policy_epoch,
            policy: RepositoryProtectionPolicy {
                administrators,
                branches,
            },
        };
        command.proposed_event(principal).map_err(
            |_| "invalid, duplicate, empty, exhausted or out-of-scope protection policy",
        )?;
        Some(Change {
            principal,
            key,
...
fn version(command: &ProtectionCommand) -> u64 {
    match command.expected_version {
        ...
        ExpectedVersion::Exactly(n) => n.get(),
    }
}
...
fn policy_json(policy: &RepositoryProtectionPolicy) -> String {
    let branches = policy
        ...
        .map(|rule| {
            format!(
                "{{\"target\":{},\"target_hex\":{},\"reviewers\":{}}}",
                std::str::from_utf8(rule.target.as_bytes())
                    .map(quote)
                ...
                principals(&rule.reviewers)
            )
        })
        .collect::<Vec<_>>()
        .join(",");
    format!(
        "{{\"administrators\":{},\"branches\":[{branches}]}}",
        principals(&policy.administrators)
    )
}
fn read_json(options: &Options, view: &RepositoryProtectionView) -> Result<String, String> {
    let (configured, version, policy) = match &view.selected {
        ...
        Some(selected) => {
            if selected.source_head != view.source_head
                || selected
                    .event
                    .activated_epoch()
                    .map_err(|_| "invalid activated epoch")?
                    > view.policy_epoch
            {
                return Err(
                    "protection response is not bound to the selected head and epoch".into(),
                );
            }
            selected
                .event
                .validate()
                .map_err(|_| "invalid canonical policy response")?;
            (
                true,
                selected.version.get(),
                policy_json(&selected.event.policy),
            )
        }
    };
    Ok(format!(
        concat!(
            "{{\"type\":\"repository_protection\",\"schema_version\":1,\"tenant_id\":{},\"repository_id\":{},",
        ...
        quote(&view.source_head.to_string()),
        view.policy_epoch.get(),
        configured,
        version,
        policy
    ))
...
fn finish(
    ...
    let json = format!(
        concat!(
            "{{\"type\":\"repository_protection_publication\",\"schema_version\":1,\"outcome\":{},\"tx_id\":{},",
        ...
        change.command.expected_policy_epoch.get(),
        quote(&change.principal.to_string()),
        policy_json(&change.command.policy),
        cleanup.is_none(),
        cleanup.map_or_else(|| "null".into(), quote)
    );
    write_terminal_receipt(output, &json, tx, terminal)
        .map_err(|error| format!("{error}; cleanup: {cleanup:?}"))?;
...
fn emit(output: &mut impl Write, text: &str) -> Result<(), String> {
    writeln!(output, "{text}")
        ...
        .map_err(|error| format!("protection output failed: {error}"))
}
pub(super) fn run(args: &[String]) -> Result<u8, String> {
    if args == ["--help"]
        || (args.len() == 2 && matches!(args[0].as_str(), "set" | "show") && args[1] == "--help")
    {
    ...
    .map_err(|error| format!("cannot open protection node: {error}"))?;
    let result: Result<Completion, String> = (|| {
        let intake = node
        ...
        if let Some(change) = &options.change {
            ...
            let result = node
                .runtime()
                .block_on(node.admit_repository_protection_durable_in(
                    &request,
                    &session,
                    &change.command,
                    Default::default(),
            ...
            let view = node
                .runtime()
                .block_on(node.read_repository_protection_in(&request, None))
                .map_err(|error| error.to_string())?;
    ...
    match result {
        Ok(Completion::Mutation((tx, terminal))) => finish(
            &mut std::io::stdout().lock(),
            options.change.as_ref().ok_or("missing mutation")?,
            tx,
            &terminal,
            cleanup.as_deref(),
        ),
        Ok(Completion::Read(text)) => {
            if let Some(error) = cleanup {
                return Err(format!("protection read shutdown failed: {error}"));
            }
            emit(&mut std::io::stdout().lock(), &text)?;
            Ok(0)
        }
        Err(error) => Err(format!(
            "protection operation failed: {error}; shutdown: {cleanup:?}{}",
            if options.change.is_some() {
                "; this does not prove non-commit. Recover with fg outcome using the same principal and key; do not refresh the version or replace the key"
            } else {
                ""
            }
        )),
...
mod tests {
    ...
    fn policy_arguments_preserve_complete_replacement_and_sort_without_deduplicating() {
        let mut args = set();
        args.extend(["--administrator".into(), "88".repeat(16)]);
        let options = parse(&args).unwrap();
        let change = options.change.unwrap();
        ...
        let mut args = set();
        let index = args.iter().position(|s| s == "--rule").unwrap();
        args.truncate(index);
        args.push("--clear-rules".into());
        assert!(
            parse(&args)
                .unwrap()
                .change
    ...
    fn missing_trust_conflicting_rules_and_overflow_refuse_before_opening_storage() {
        for flag in [
            ...
            args.drain(index..index + if flag == "--trusted-local" { 1 } else { 2 });
        ...
        for value in ["01", "-1", "18446744073709551615", "18446744073709551616"] {
            let mut args = set();
            let at = args.iter().position(|s| s == "--expected-version").unwrap();
            args[at + 1] = value.into();
            assert!(parse(&args).is_err());
        }
        let mut args = set();
        let last = args.len() - 1;
        args[last] = format!("{},{}", "33".repeat(16), "33".repeat(16));
        assert!(parse(&args).is_err());
    }
    #[test]
    fn both_hash_formats_and_ref_names_with_equals_are_unambiguous() {
        for format in ["sha1", "sha256"] {
            let mut args = set();
            args.extend(["--object-format".into(), format.into()]);
            let at = args.iter().position(|s| s == "--rule").unwrap();
            args[at + 1] = "refs/heads/name=literal".into();
            let options = parse(&args).unwrap();
            assert_eq!(options.format.as_str(), format);
            assert_eq!(
                options.change.unwrap().command.policy.branches[0]
                    .target
                    .as_bytes(),
                b"refs/heads/name=literal"
            );
        }
    }
}
