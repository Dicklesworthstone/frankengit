...
use options::{KeyInput, Operation, Options, head_token, hex};
use std::io::{Read, Write};

const USAGE: &str =
    "usage: fg branch <create|update|delete|rename> <storage-root> <tenant-id> <repository-id>
...
pub fn run(args: &[String]) -> Result<u8, String> {
    if args == ["--help"]
        || (args.len() == 2
            ...
            && matches!(
                args[0].as_str(),
                "list" | "create" | "update" | "delete" | "rename"
            ))
    {
    ...
    let session = match &options.operation {
        Operation::Mutate { principal, key, .. } => {
            ...
            Some(LoopbackReceiveSession::authenticated(
                *principal,
                IdempotencyKey::new(bytes).map_err(|_| "invalid idempotency key")?,
            ))
        }
        Operation::List { .. } => None,
    };
    ...
    .map_err(|error| format!("cannot open branch node: {error}"))?;
    let operation = (|| {
        ...
        match &options.operation {
            Operation::Mutate { commands, .. } => {
                let session = session
                    .as_ref()
                    .ok_or("authenticated branch session missing")?;
                let result = node
                    .runtime()
                ...
                if !result.session.atomic
                    ...
                    || result.commands.len() != commands.len()
                    || result.commands.iter().any(|command| command != first)
                {
                    return Err(format!(
                        "inconsistent atomic branch receipt; {}",
            ...
            Operation::List {
                ...
                .map(|(head, rows, next)| Completed::Read(head, rows, next))
                .map_err(|error| error.to_string()),
        }
    })();
    ...
    match operation {
        ...
        Ok(Completed::Read(head, rows, next)) => {
            if let Some(error) = cleanup {
                return Err(format!("branch read node shutdown failed: {error}"));
            }
            emit(
                &mut std::io::stdout().lock(),
                &read_receipt(&options, head, &rows, next.as_ref()),
            )?;
            Ok(0)
        }
        Err(error) => {
...
fn emit(output: &mut impl Write, text: &str) -> Result<(), String> {
    writeln!(output, "{text}")
        ...
        .map_err(|error| format!("branch output incomplete: {error}"))
}
...
fn command_json(command: &RefCommand) -> String {
    ...
    format!(
        "{{\"reference_hex\":{},\"expected_tip\":{old},\"target\":{new}}}",
        quote(&hex(command.name.as_bytes()))
    )
}
fn finish_mutation(
    ...
    let Operation::Mutate {
        ...
        return Err("branch receipt operation mismatch".into());
    };
    ...
    let receipt = format!(
        concat!(
            "{{\"type\":\"branch_publication\",\"schema_version\":1,\"action\":{},\"atomic\":true,",
...
fn read_receipt(
    ...
    format!(
        concat!(
            "{{\"type\":\"branch_page\",\"schema_version\":1,\"tenant_id\":{},\"repository_id\":{},",
