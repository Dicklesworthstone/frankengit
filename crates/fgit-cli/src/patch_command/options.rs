...
pub(super) fn parse(args: &[String]) -> Result<Options, String> {
    ...
    {
        return Err("patch arguments exceed the bounded profile".into());
    }
    ...
    while at < args.len() {
        ...
        let allowed = matches!(name, "--expected-base")
            || if prepare {
                matches!(
                    name,
                    "--profile"
        ...
        if !switch && !allowed {
            return Err(format!("unknown patch option {name:?}"));
        }
        let value = if switch {
            ""
        } else {
    ...
    let bytes = if flags.contains_key("--ref-hex") {
        unhex(&args[4], 4096)?
    } else {
    ...
    let operation = if prepare {
        ...
        let workspace_id = unhex(required("--workspace-id")?, 16)?
            .try_into()
            .map_err(|_| "workspace ID must contain 16 bytes")?;
        ...
        let key = match (
            ...
            (Some(text), false)
                if !text.is_empty() && text.len() <= fgit_authority::MAX_IDEMPOTENCY_KEY_BYTES =>
            {
                Key::Bytes(text.as_bytes().to_vec())
            }
            (None, true) => Key::Stdin,
        ...
        if candidate == base || candidate.algorithm() != base.algorithm() {
            return Err(
                "base and reviewed commit must be distinct IDs in the same native hash domain"
                    .into(),
            );
        }
        Operation::Apply {
