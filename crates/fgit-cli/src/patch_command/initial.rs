...
pub(super) fn run(args: &[String]) -> Result<u8,String> {
    if args.len()==2 && args[1]=="--help" {
        return writeln!(std::io::stdout().lock(),"{USAGE}").map(|()|0).map_err(|e|e.to_string());
    }
    let mut options=options::parse(args)?;
    let (input,session)=match &mut options.operation {
        Operation::Prepare {input,output,metadata,message_file,limits} => {
            ...
            let patch=fgit_forge::patch::UnifiedPatch::parse(&input,*limits,&||false).map_err(|e|e.to_string())?;
            if patch.files().iter().any(|file|file.change()!=fgit_forge::patch::FileChange::Create) {
                return Err("initial patch may only create regular files".into());
            }
            require_absent(output)?;(input,None)
        }
        Operation::Apply {input,principal,key,..} => {
            let key=key_bytes(key,&mut std::io::stdin().lock())?;
            let session=LoopbackReceiveSession::authenticated(*principal,IdempotencyKey::new(key).map_err(|e|e.to_string())?);
    ...
    let result=(|| {
        ...
        match &options.operation {
            Operation::Prepare {metadata,limits,..} => {
                let (head,plan,bundle)=node.runtime().block_on(node.prepare_trusted_initial_patch_in(
                    &request,&options.reference,&input,metadata,*limits,None)).map_err(|e|e.to_string())?;
            ...
            Operation::Apply {candidate,..} => {
                let session=session.as_ref().ok_or("initial publication session missing")?;
                let result=node.runtime().block_on(node.apply_initial_patch_bundle_durable_in(
                    &request,session,&options.reference,*candidate,&input,Default::default())).map_err(|e|e.to_string())?;
                let [command]=result.commands.as_slice() else {return Err("initial publication returned no single terminal result; do not infer non-commit".into());};
                if !result.session.atomic || result.session.tx_ids!=vec![command.tx_id] {
                    return Err(format!("inconsistent initial publication mapping; {}",describe(command.tx_id,&command.terminal)));
                }
    ...
    match result {
        Ok(Completed::Prepared(head,plan,bytes,count)) => {
            if let Some(error)=cleanup {return Err(format!("initial preparation shutdown failed: {error}; no bundle file published"));}
            let Operation::Prepare {output,..}=&options.operation else {return Err("initial operation mismatch".into());};
        ...
        Ok(Completed::Applied(tx,terminal)) => finish_applied(&mut std::io::stdout().lock(),&options,tx,&terminal,cleanup.as_deref()),
        Err(error) => {
            let cleanup=cleanup.map_or_else(String::new,|e|format!("; node shutdown also failed: {e}"));
            match &options.operation {
                Operation::Prepare{..} => Err(format!("initial preparation failed: {error}{cleanup}; no bundle file published")),
                Operation::Apply{..} => Err(format!("initial publication returned no terminal outcome: {error}{cleanup}; this is not evidence of non-commit. Retry identical inputs/key or use fg outcome; do not change the scoped retry key")),
            }
        }
    }
}
enum Completed { Prepared(RepositoryAuthorityHeadId,InitialCommitPlan,Vec<u8>,u32), Applied(TxId,TerminalOutcome) }
fn key_bytes(key:&options::Key,input:&mut impl Read)->Result<Vec<u8>,String> {
    match key { options::Key::Bytes(bytes)=>Ok(bytes.clone()),options::Key::Stdin=>super::read_key(input) }
}
fn prepared_receipt(options:&Options,head:RepositoryAuthorityHeadId,plan:&InitialCommitPlan,bytes:&[u8],count:u32)->Result<String,String> {
    if plan.object_format!=options.format || plan.commit.algorithm()!=options.format || plan.commit.is_zero()
        || plan.tree.algorithm()!=options.format || bytes.is_empty() || count as usize!=plan.objects.len() {
        return Err("inconsistent initial candidate; no bundle was published".into());
    }
    let files=plan.files.iter().map(|f|format!("{{\"path_hex\":{},\"blob\":{},\"mode\":{},\"bytes\":{}}}",
        quote(&hex(&f.path)),quote(&f.blob.to_string()),quote(&format!("{:o}",f.mode)),f.bytes)).collect::<Vec<_>>().join(",");
    Ok(format!(concat!("{{\"type\":\"initial_commit_preparation\",\"schema_version\":1,\"profile\":\"exact-v1\",",
        ...
        "\"published_to_repository\":false,\"node_closed\":true}}"),quote(&options.tenant.to_string()),quote(&options.repository.to_string()),
        quote(options.format.as_str()),quote(&hex(options.reference.as_bytes())),quote(&head.to_string()),quote(&plan.commit.to_string()),quote(&plan.tree.to_string()),
        quote(&hex(&plan.patch_sha256)),quote(&hex(&fgit_crypto::sha256_digest(bytes))),bytes.len(),count,files))
}
fn finish_applied(output:&mut impl Write,options:&Options,tx:TxId,terminal:&TerminalOutcome,cleanup:Option<&str>)->Result<u8,String> {
    let Operation::Apply{principal,candidate,..}=&options.operation else {return Err("initial receipt operation mismatch".into());};
    ...
    let receipt=format!(concat!("{{\"type\":\"initial_commit_publication\",\"schema_version\":1,\"atomic\":true,\"expected_absent\":true,",
        ...
        "\"node_closed\":{},\"cleanup_error\":{}}}"),quote(outcome),exit==0,quote(&tx.to_string()),terminal.decision_sequence.get(),rcr,refusal,record,
        quote(&options.tenant.to_string()),quote(&options.repository.to_string()),quote(&principal.to_string()),quote(options.format.as_str()),
        quote(&hex(options.reference.as_bytes())),quote(&candidate.to_string()),cleanup.is_none(),cleanup.map_or_else(||"null".into(),quote));
