...
fn complete_explicit_commands_support_both_domains_and_byte_exact_keys() {
    for width in [40, 64] {
        ...
        assert_eq!(key, b"private-retry-key");
        assert_eq!(candidate.to_string(), "b".repeat(width));
    }
    assert_eq!(read_key(&mut &b"key\n"[..]).unwrap(), b"key\n");
    assert_eq!(read_key(&mut &vec![b'k'; 256][..]).unwrap().len(), 256);
    assert!(read_key(&mut &vec![b'k'; 257][..]).is_err());
...
fn missing_duplicate_and_cross_operation_fields_refuse_before_io() {
    for args in [prepare_args(40), apply_args(40)] {
        ...
        no_trust.retain(|s| s != "--trusted-local");
        assert!(options::parse(&no_trust).is_err());
        for (at, flag) in args
            .iter()
            .enumerate()
            .filter(|(_, s)| s.starts_with("--") && *s != "--trusted-local")
        {
            let mut missing = args.clone();
            missing.drain(at..at + 2);
            assert!(options::parse(&missing).is_err(), "{flag}");
            let mut duplicate = args.clone();
            duplicate.extend([flag.clone(), args[at + 1].clone()]);
            assert!(options::parse(&duplicate).is_err());
        }
        let mut force = args.clone();
        force.extend(["--force".into(), "true".into()]);
        assert!(options::parse(&force).is_err());
    }
    let mut args = prepare_args(40);
    args.extend(["--principal".into(), "33".repeat(16)]);
    assert!(options::parse(&args).is_err());
    let mut args = apply_args(40);
    args.extend(["--profile".into(), "exact-v1".into()]);
    assert!(options::parse(&args).is_err());
    let mut args = apply_args(40);
    args.push("--key-stdin".into());
    assert!(options::parse(&args).is_err());
    let at = args.iter().position(|s| s == "--idempotency-key").unwrap();
    args.drain(at..at + 2);
    assert!(matches!(
        options::parse(&args).unwrap().operation,
        Operation::Apply {
...
fn malformed_identity_profile_metadata_and_resource_envelopes_refuse() {
    for (flag, bad) in [
        ...
        assert!(options::parse(&args).is_err(), "{flag}");
    }
    for (flag, bad) in [
        ("--max-files", "0"),
        ("--max-files", "1025"),
        ("--max-hunks", "4097"),
        ("--max-input-bytes", "16777217"),
        ("--max-output-bytes", "33554433"),
    ] {
        let mut args = prepare_args(40);
        args.extend([flag.into(), bad.into()]);
        assert!(options::parse(&args).is_err());
    }
    for bad in [
        "0".repeat(40),
        ...
        let mut args = apply_args(40);
        replace(&mut args, "--expected-commit", &bad);
        assert!(options::parse(&args).is_err());
    }
    let mut args = prepare_args(40);
    args[4] = "refs/tags/main".into();
    assert!(options::parse(&args).is_err());
    let mut args = prepare_args(40);
    args[4] = hex(b"refs/heads/caf\xc3\xa9");
    args.push("--ref-hex".into());
    assert_eq!(
        options::parse(&args).unwrap().reference.as_bytes(),
        b"refs/heads/caf\xc3\xa9"
    );
    args[4] = "0g".into();
    assert!(options::parse(&args).is_err());
}
...
fn terminal_decisions_survive_broken_output_and_cleanup_without_key_disclosure() {
    ...
    assert_eq!(
        finish_applied(&mut bytes, &options, tx, &committed, None).unwrap(),
        0
    );
    let text = String::from_utf8(bytes).unwrap();
    assert!(text.contains("\"published_to_repository\":true"));
    assert!(!text.contains("private-retry-key"));
    assert!(text.contains(&rcr.to_string()));
    let mut bytes = Vec::new();
    assert!(finish_applied(&mut bytes, &options, tx, &committed, Some("close failed")).is_err());
    let text = String::from_utf8(bytes).unwrap();
    ...
    let text = String::from_utf8(bytes).unwrap();
    assert!(text.contains("\"published_to_repository\":false"));
