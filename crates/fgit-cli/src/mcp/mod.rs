...
impl WriteGrants {
    fn any(self) -> bool { self.issues || self.pulls }
}
...
mod tests {
    ...
    fn write_and_recovery_grants_require_identity_and_incarnation_but_imply_no_reads() {
        for flag in ["--allow-issue-writes", "--allow-pull-writes", "--allow-outcomes"] {
...
mod mutation_grant_tests {
    ...
    fn all_launch_grant_combinations_preserve_independent_write_ceilings() {
        let flags = ["--allow-issues", "--allow-pulls", "--allow-source", "--allow-issue-writes", "--allow-pull-writes", "--allow-outcomes"];
        for mask in 0_u8..64 {
            ...
            assert_eq!(options.outcomes, mask & 32 != 0);
            assert_eq!(options.writes.any(), mask & 24 != 0);
        }
    }
}
