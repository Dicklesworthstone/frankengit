...
impl WriteGrants {
    fn any(self) -> bool { self.issues || self.pulls || self.source || self.reviews }
}
...
fn parse_options(arguments: &[String]) -> Result<Options, String> {
    if arguments.len() < 3 || arguments.len() > 24 { return Err(USAGE.into()); }
...
mod tests {
    ...
    fn write_and_recovery_grants_require_identity_and_incarnation_but_imply_no_reads() {
        for flag in ["--allow-issue-writes", "--allow-pull-writes", "--allow-source-writes", "--allow-review-writes", "--allow-outcomes"] {
...
mod mutation_grant_tests {
    ...
    fn all_launch_grant_combinations_preserve_independent_write_ceilings() {
        let flags = ["--allow-issues", "--allow-pulls", "--allow-source", "--allow-issue-writes", "--allow-pull-writes", "--allow-source-writes", "--allow-outcomes", "--allow-review-writes"];
        for mask in 0_u16..256 {
            ...
            assert_eq!(options.writes.any(), mask & 184 != 0);
        }
    }
}
