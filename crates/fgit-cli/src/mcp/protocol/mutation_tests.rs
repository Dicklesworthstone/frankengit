...
impl ReadTools for Mutating {
    fn tools(&self) -> Vec<Tool> {
        vec![Tool {
            name: "write",
            description: "protocol mutation oracle",
    ...
    fn call(&mut self, _: &str, args: &Object) -> Result<Value, ToolError> {
        ...
        self.calls += 1;
        if self.fail {
            return Err(ToolError::uncertain("connection_lost"));
        }
...
fn terminal(refused: bool) -> Value {
    object([
        ("tx_id", text("test-transaction")),
        ("decision_sequence", text("7")),
        (
            "outcome",
            text(if refused { "refused" } else { "committed" }),
        ),
        ("terminal", Value::Bool(true)),
        ("outcome_unknown", Value::Bool(false)),
        ("command_committed", Value::Bool(!refused)),
...
fn annotations_follow_fixed_mutation_classification_and_notifications_never_write() {
    ...
    assert_eq!(hints["readOnlyHint"], Value::Bool(false));
    assert_eq!(hints["destructiveHint"], Value::Bool(true));
    assert_eq!(hints["idempotentHint"], Value::Bool(true));
    assert!(
        server
            .receive(
                &mut backend,
                br#"{"jsonrpc":"2.0","method":"tools/call","params":{"name":"write"}}"#
            )
            .is_none()
    );
    assert!(
        server
            .receive(
                &mut backend,
                br#"{"jsonrpc":"2.0","method":"notifications/cancelled","params":{"requestId":3}}"#
            )
            .is_none()
    );
    assert_eq!(backend.calls, 0);
    server.receive(&mut backend, CALL).unwrap();
...
fn canonical_refusal_and_unknown_outcome_have_different_tool_results() {
    ...
    assert_eq!(
        result["structuredContent"].object().unwrap()["outcome_unknown"],
        Value::Bool(false)
    );
    assert_eq!(
        result["structuredContent"].object().unwrap()["outcome"].text(),
        Some("refused")
    );
    let mut backend = Mutating {
        fail: true,
    ...
    assert_eq!(body["outcome_unknown"], Value::Bool(true));
    assert_eq!(body["terminal"], Value::Bool(false));
    assert!(!body.contains_key("outcome"));
}
#[test]
fn response_overflow_keeps_known_terminal_facts_or_preserves_uncertainty() {
    for refused in [false, true] {
        let mut value = terminal(refused);
        let Value::Object(fields) = &mut value else {
            unreachable!()
        };
        fields.insert("oversized".into(), text("x".repeat(MAX_TOOL_RESULT + 1)));
        let result = encode_tool_result(value, refused, true);
        let body = result.object().unwrap()["structuredContent"]
            .object()
            .unwrap();
        assert_eq!(result.object().unwrap()["isError"], Value::Bool(true));
        assert_eq!(body["tx_id"].text(), Some("test-transaction"));
        assert_eq!(body["decision_sequence"].text(), Some("7"));
        assert_eq!(body["outcome_unknown"], Value::Bool(false));
        assert_eq!(body["terminal"], Value::Bool(true));
        assert_eq!(body["code"].text(), Some("response_limit"));
        assert!(!body.contains_key("oversized"));
    ...
    assert_eq!(
        result.object().unwrap()["structuredContent"]
            .object()
            .unwrap()["outcome_unknown"],
        Value::Bool(true)
    );
}
#[test]
