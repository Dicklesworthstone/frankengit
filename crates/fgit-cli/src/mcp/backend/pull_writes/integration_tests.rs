...
use super::super::super::{
    WriteGrants,
    protocol::{ReadTools, Server},
};
use super::super::{NodeTools, Options};
use super::*;
...
use fgit_types::{
    DecisionOutcome, HeadGeneration, PrincipalId, RepositoryAuthorityHeadId, RepositoryId, TenantId,
};
use std::{
    fs,
    path::PathBuf,
...
impl Fixture {
    fn new(format: GitHashAlgorithm) -> Self {
        ...
        let mut options = Options {
            ...
            tenant: TenantId::from_bytes([0x81; 16]),
            repository: RepositoryId::from_bytes([0x82; 16]),
            format,
            incarnation: None,
            issues: false,
            pulls: false,
...
fn args(commit: GitOid, version: &str, key: &str, title: &str) -> Value {
    object([
        ...
        ("expected_source", text(commit.to_string())),
        ("expected_target", text(commit.to_string())),
        ("title", text(title)),
        ("body", text("literal é\r\nnot a policy")),
    ])
}
...
fn invoke(server: &mut Server, backend: &mut NodeTools, id: u64, name: &str, args: Value) -> Value {
    ...
    server
        .receive(backend, message.encode(16384).unwrap().as_bytes())
        .unwrap()
}
...
fn native_pr_lifecycle_is_versioned_read_independent_and_recoverable_after_reopen() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let first = invoke(
            ...
            2,
            OPEN,
            args(fixture.commit, "0", "open-pr", "Original"),
        );
        ...
        assert_eq!(head(&backend.node), held);
        for (index, name) in [
            "frankengit_pull_show",
        ...
        let updated = invoke(
            ...
            8,
            UPDATE,
            args(fixture.commit, "1", "update-pr", "Updated"),
        );
        ...
        let stale = invoke(
            ...
            9,
            UPDATE,
            args(fixture.commit, "1", "stale-pr", "Stale"),
        );
        assert_eq!(result(&stale)["outcome"].text(), Some("refused"));
        assert_eq!(
            stale.object().unwrap()["result"].object().unwrap()["isError"],
            Value::Bool(true)
        );
        ...
        let delete = RefCommand {
            name: reference("refs/heads/topic"),
            expected_old: ExpectedOld::Exactly(fixture.commit),
        ...
        let bad_close = invoke(
            &mut server,
            &mut backend,
            10,
            CLOSE,
            args(fixture.commit, "2", "bad-close", "Changed on close"),
        );
        ...
        let fresh = invoke(
            ...
            5,
            UPDATE,
            args(fixture.commit, "3", "new-stopped", "No"),
        );
...
fn pull_write_grants_do_not_imply_reads_issues_or_recovery_and_descriptors_are_mutating() {
    ...
    let list = server
        ...
        .unwrap();
    let Value::Array(tools) = &list.object().unwrap()["result"].object().unwrap()["tools"] else {
        panic!("tools")
    };
    for tool in tools {
    ...
    let Value::Object(fields) = &mut injected else {
        unreachable!()
    };
    fields.insert("principal".into(), text("admin"));
    let before = head(&backend.node);
    assert!(
        invoke(&mut server, &mut backend, 3, OPEN, injected)
            .object()
            .unwrap()
            .contains_key("error")
    );
    let Value::Object(fields) = args(fixture.commit, "0", "denied", "No") else {
        unreachable!()
    };
    assert!(
        super::super::issue_writes::call(&backend, "frankengit_issue_open", &fields)
            .unwrap_err()
            .invalid
    );
