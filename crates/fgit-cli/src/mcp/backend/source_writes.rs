...
pub(super) fn is_tool(name: &str) -> bool {
    matches!(name, CREATE | UPDATE | DELETE | RENAME | PUBLISH)
}
...
fn schema(name: &str) -> Value {
    ...
    if name == RENAME {
        pairs.push(("destination", "destination_hex"));
    }
    ...
    let pins: &[&str] = match name {
        CREATE => &["target"],
        UPDATE => &["expected_old", "target"],
        DELETE | RENAME => &["expected_old"],
        PUBLISH => &["expected_base", "expected_candidate"],
        _ => &[],
    };
    for pin in pins {
        required.push(*pin);
...
fn parse(name: &str, args: &Object, format: GitHashAlgorithm) -> Result<Input, ToolError> {
    ...
    let (old, new) = match name {
        CREATE => (
            ExpectedOld::Absent,
            ProposedNew::Update(oid(args, "target", format)?),
        ),
        UPDATE | PUBLISH => {
            let (a, b) = if name == PUBLISH {
                ("expected_base", "expected_candidate")
            } else {
                ("expected_old", "target")
            };
            let old = oid(args, a, format)?;
            let new = oid(args, b, format)?;
            if old == new {
                return Err(ToolError::invalid("source_transition_must_change_tip"));
            }
            (ExpectedOld::Exactly(old), ProposedNew::Update(new))
        }
        DELETE | RENAME => (
            ExpectedOld::Exactly(oid(args, "expected_old", format)?),
            ProposedNew::Delete,
        ),
        _ => return Err(ToolError::invalid("tool_not_granted")),
    };
    let mut commands = vec![RefCommand {
    ...
    if name == RENAME {
        ...
        if destination == reference {
            return Err(ToolError::invalid("rename_requires_distinct_branches"));
        }
        let ExpectedOld::Exactly(tip) = old else {
            return Err(ToolError::invalid("exact_tip_required"));
        };
...
pub(super) fn call(backend: &NodeTools, name: &str, args: &Object) -> Result<Value, ToolError> {
    ...
    let admitted = if let Some(bytes) = &input.bundle {
        ...
        let (ExpectedOld::Exactly(base), ProposedNew::Update(candidate)) =
            (command.expected_old, command.proposed_new)
        else {
            return Err(ToolError::invalid("invalid_publication_shape"));
        };
        backend
            .node
            .runtime()
            .block_on(backend.node.apply_workspace_bundle_durable_in(
                &request,
                principal,
        ...
        backend
            .node
            .runtime()
            .block_on(backend.node.admit_branch_updates_durable_in(
                &request,
                &session,
                &input.commands,
                Default::default(),
    ...
    let observations: Vec<_> = admitted
        .commands
        .iter()
        .map(|c| (c.tx_id, c.terminal))
        .collect();
    let (tx, terminal) = atomic_terminal(
        admitted.session.atomic,
        &admitted.session.tx_ids,
        &observations,
        input.commands.len(),
    )?;
            ...
            input
                ...
                .map(|c| {
                    object([
                        ...
                        (
                            "expected_old",
                            match c.expected_old {
                                ExpectedOld::Exactly(id) => text(id.to_string()),
                                _ => Value::Null,
                            },
                        ),
                        (
                            "proposed_new",
                            match c.proposed_new {
                                ProposedNew::Update(id) => text(id.to_string()),
                                ProposedNew::Delete => Value::Null,
                            },
                        ),
...
fn atomic_terminal(
    atomic: bool,
    ids: &[TxId],
    observations: &[(TxId, TerminalOutcome)],
    expected: usize,
    ...
    if !atomic
        || !(1..=2).contains(&expected)
        || observations.len() != expected
        || ids != [first.0]
        || observations.iter().any(|entry| *entry != first)
    {
        return Err(invalid());
    }
    Ok(first)
