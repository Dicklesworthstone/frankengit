...
pub(super) fn call(backend: &NodeTools, name: &str, args: &Object) -> Result<Value, ToolError> {
    ...
    let report = backend
        .node
        .runtime()
        .block_on(backend.node.review_source_in(
            &request,
            &query.selection,
            &Default::default(),
            query.expected_head,
    ...
    result
        ...
        .map_err(|_| ToolError::failed("review_response_limit"))?;
    Ok(result)
}
...
fn bounded(
    ...
    let value = args
        ...
        .map(|value| {
            value
                .unsigned()
                .ok_or(ToolError::invalid("invalid_review_limit"))
        })
        .transpose()?
        .unwrap_or(default as u64);
    let value = usize::try_from(value).map_err(|_| ToolError::invalid("invalid_review_limit"))?;
    if value < minimum || value > maximum {
        return Err(ToolError::invalid("invalid_review_limit"));
    }
    Ok(value)
}
...
) -> Result<Option<GitOid>, ToolError> {
    string(args, name)?
        .map(|value| {
            ...
            let oid = GitOid::from_hex(format, value)
...
fn parse(args: &Object, pr: bool, format: GitHashAlgorithm) -> Result<Query, ToolError> {
    ...
    allowed.extend(if pr {
        vec!["number", "expected_version"]
    } else {
        vec!["before_ref", "after_ref", "before_ref_hex", "after_ref_hex"]
    });
    ...
    let selection = if pr {
        ReviewSelection::PullRequest {
            ...
            expected_version: Some(
                AggregateVersion::try_new(decimal(args, "expected_version", 0)?)
    ...
    if let Some(value) = args.get("paths_hex") {
        let Value::Array(paths) = value else {
            return Err(ToolError::invalid("paths_must_be_array"));
        };
        if paths.len() > MAX_PATHS {
            return Err(ToolError::invalid("too_many_paths"));
        }
        let mut bytes = 0_usize;
        for path in paths {
            let path = unhex(
                path.text()
                    .ok_or(ToolError::invalid("path_must_be_hex_string"))?,
                4096,
            )?;
            bytes = bytes
    ...
    options.limits.max_output_bytes =
        bounded(args, "max_output_bytes", 64 * 1024, 1, MAX_OUTPUT_BYTES)?;
    ...
    options
        .validate()
        .map_err(|_| ToolError::invalid("invalid_review_options"))?;
    Ok(Query {
        selection,
        expected_head: head(args, 0)?,
        expected_before: native(args, "expected_before", format)?,
        expected_after: native(args, "expected_after", format)?,
        options,
    })
}
...
fn input_schema(pr: bool) -> Value {
    ...
    properties.insert(
        ...
        object([
            ...
            (
                "enum",
                Value::Array(vec![text("direct"), text("merge-base")]),
            ),
    ...
    for (name, default, min, max) in [
        ...
        ("max_changes", 64, 1, MAX_CHANGES as u64),
    ...
    if pr {
        ...
        for name in ["before_ref_hex", "after_ref_hex"] {
            properties.insert(name.into(), bytes.clone());
        }
    }
    let Value::Object(mut schema) = object([
        ("type", text("object")),
        ...
        (
            "required",
            Value::Array(if pr {
                vec![text("number"), text("expected_version")]
            } else {
                Vec::new()
            }),
        ),
    ...
    if !pr {
        ...
        schema.insert(
            ...
            Value::Array(
                [
                    ("before_ref", "before_ref_hex"),
                    ("after_ref", "after_ref_hex"),
                ]
                .into_iter()
                .map(|(a, b)| {
                    object([(
