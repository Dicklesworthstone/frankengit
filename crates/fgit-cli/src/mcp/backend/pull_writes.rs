...
use fgit_forge::{PullRequestNumber, event::pull_request::{PullRequestAction, PullRequestCommand, PullRequestData}};
...
const FIELDS: &[&str] = &[
    ...
    "target_reference", "target_reference_hex", "expected_source", "expected_target", "title", "body",
];

pub(super) fn is_tool(name: &str) -> bool { matches!(name, OPEN | UPDATE | CLOSE) }
...
fn schema() -> Value {
    ...
    let mut schema = common::input_schema(properties,
        &["number", "expected_version", "idempotency_key", "expected_source", "expected_target", "title", "body"]);
    ...
    if let Value::Object(fields) = &mut schema {
        ...
        ].into_iter().map(|(plain, hex)| object([("oneOf", Value::Array(vec![
...
pub(super) fn branch(args: &Object, plain: &str, encoded: &str) -> Result<RefName, ToolError> {
    ...
    if !bytes.starts_with(b"refs/heads/") { return Err(ToolError::invalid("full_branch_required")); }
    RefName::try_new(&bytes).map_err(|_| ToolError::invalid("invalid_branch"))
}
pub(super) fn oid(args: &Object, name: &str, format: GitHashAlgorithm) -> Result<GitOid, ToolError> {
    ...
    GitOid::from_hex(format, value).ok().filter(|oid| !oid.is_zero())
        .ok_or(ToolError::invalid("invalid_native_oid"))
}
pub(super) fn parse(name: &str, args: &Object, format: GitHashAlgorithm) -> Result<PullRequestCommand, ToolError> {
    ...
    let data = PullRequestData {
        ...
        source_tip: oid(args, "expected_source", format)?, target_tip: oid(args, "expected_target", format)?,
        title: common::required(args, "title")?.to_owned(),
        body: common::body(args, "body")?.ok_or(ToolError::invalid("body_required"))?,
    };
    data.validate().map_err(|_| ToolError::invalid("invalid_pull_request_data"))?;
    let action = match name {
        OPEN => PullRequestAction::Open, UPDATE => PullRequestAction::Update, CLOSE => PullRequestAction::Close,
...
pub(super) fn call(backend: &NodeTools, name: &str, args: &Object) -> Result<Value, ToolError> {
    if !backend.options.writes.pulls { return Err(ToolError::invalid("tool_not_granted")); }
    ...
    let (tx, terminal) = backend.node.runtime().block_on(backend.node.admit_pull_request_durable_in(
    ...
    result.insert("expected_version".into(), text(match command.expected_version {
        fgit_forge::ExpectedVersion::NewStream => 0,
        fgit_forge::ExpectedVersion::Exactly(version) => version.get(),
    }.to_string()));
    result.insert("action".into(), text(match command.action {
        PullRequestAction::Open => "open", PullRequestAction::Update => "update", PullRequestAction::Close => "close",
    }));
