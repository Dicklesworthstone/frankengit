...
use fgit_forge::{IssueNumber, event::issue::{IssueAction, IssueCommand, IssueEdit, MAX_LABELS}};
...
pub(super) fn is_tool(name: &str) -> bool {
    matches!(name, "frankengit_issue_open" | "frankengit_issue_edit" | "frankengit_issue_close"
        | "frankengit_issue_reopen" | "frankengit_issue_comment")
}
...
fn schema(name: &str) -> Value {
    ...
    if matches!(name, "frankengit_issue_open" | "frankengit_issue_edit") {
        ...
        properties.insert("labels".into(), object([("type", text("array")), ("maxItems", json::number(32)),
            ("uniqueItems", Value::Bool(true)), ("items", common::text_schema(64))]));
    }
    if matches!(name, "frankengit_issue_open" | "frankengit_issue_edit" | "frankengit_issue_comment") {
        properties.insert("body".into(), common::text_schema(common::MAX_TEXT_BYTES as u64));
    }
    if name == "frankengit_issue_open" { required.extend(["title", "body"]); }
    if name == "frankengit_issue_comment" { required.push("body"); }
    common::input_schema(properties, &required)
}
fn labels(args: &Object) -> Result<Option<Vec<String>>, ToolError> {
    let Some(value) = args.get("labels") else { return Ok(None); };
    let Value::Array(values) = value else { return Err(ToolError::invalid("labels_must_be_array")); };
    if values.len() > MAX_LABELS { return Err(ToolError::invalid("too_many_labels")); }
    let mut labels = values.iter().map(|value| value.text().filter(|value| value.len() <= 64)
        .map(str::to_owned).ok_or(ToolError::invalid("invalid_label")))
        .collect::<Result<Vec<_>, _>>()?;
    labels.sort();
    if labels.windows(2).any(|pair| pair[0] == pair[1]) { return Err(ToolError::invalid("duplicate_label")); }
    Ok(Some(labels))
}
pub(super) fn parse(name: &str, args: &Object) -> Result<IssueCommand, ToolError> {
    if !is_tool(name) { return Err(ToolError::invalid("tool_not_granted")); }
    let allowed: &[&str] = match name {
        "frankengit_issue_open" | "frankengit_issue_edit" => &["number", "expected_version", "idempotency_key", "title", "body", "labels"],
    ...
    let number = IssueNumber::try_new(common::number(args, "number")?).ok_or(ToolError::invalid("positive_issue_number_required"))?;
    ...
    let action = match name {
        ...
        "frankengit_issue_edit" => IssueAction::Edit(IssueEdit {
            title: string(args, "title")?.map(str::to_owned), body: common::body(args, "body")?, labels: labels(args)?,
        }),
    ...
    action.validate().map_err(|_| ToolError::invalid("invalid_issue_command"))?;
...
pub(super) fn call(backend: &NodeTools, name: &str, args: &Object) -> Result<Value, ToolError> {
    ...
    command.proposed_event(principal).map_err(|_| ToolError::invalid("invalid_issue_command"))?;
    ...
    let (tx, outcome) = backend.node.runtime().block_on(backend.node.admit_issue_durable_in(
        &context, &session, &command, Default::default(),
    ...
    result.insert("expected_version".into(), text(common::number(args, "expected_version")?.to_string()));
...
mod tests {
    ...
    fn base(version: &str) -> Object {
        let Value::Object(args) = object([("number", text("7")), ("expected_version", text(version)), ("idempotency_key", text("original"))]) else { unreachable!() }; args
    }
    #[test]
    fn edits_preserve_omissions_and_make_empty_replacements_explicit() {
        let mut args = base("1"); args.insert("title".into(), text("changed"));
        let IssueAction::Edit(edit) = parse("frankengit_issue_edit", &args).unwrap().action else { panic!("edit") };
        assert_eq!(edit.body, None); assert_eq!(edit.labels, None);
        args.insert("body".into(), text("")); args.insert("labels".into(), Value::Array(vec![]));
        let IssueAction::Edit(edit) = parse("frankengit_issue_edit", &args).unwrap().action else { panic!("edit") };
        assert_eq!(edit.body.as_deref(), Some("")); assert_eq!(edit.labels, Some(vec![]));
        assert!(parse("frankengit_issue_edit", &base("1")).is_err());
    }
    #[test]
    fn authority_fields_and_wrong_action_fields_refuse_before_admission() {
        for name in ["principal", "repository", "storage", "action", "command", "expected_head", "body"] {
            let mut args = base("1"); args.insert(name.into(), text("untrusted"));
        ...
        let mut args = base("1"); args.insert("body".into(), text(" \n"));
    ...
    fn label_order_normalizes_without_silently_discarding_duplicates() {
        let mut args = base("1"); args.insert("labels".into(), Value::Array(vec![text("z"), text("a")]));
        let IssueAction::Edit(edit) = parse("frankengit_issue_edit", &args).unwrap().action else { panic!("edit") };
