...
use fgit_forge::event::review::{
    CandidateBinding, CandidateReviewCommand, ReviewCommand, ReviewDecision,
    ReviewSubject, CANDIDATE_REVIEW_PROFILE,
};
...
pub(super) const CANDIDATE_FIELDS: &[&str] = &[
    "number", "expected_version", "idempotency_key", "source_reference", "source_reference_hex",
    "target_reference", "target_reference_hex", "expected_source", "expected_target",
    "merge_base", "candidate_commit", "policy_epoch", "bundle_hex_chunks",
];
...
pub(super) fn subject(args: &Object, format: GitHashAlgorithm) -> Result<(ReviewSubject, CandidateBinding), ToolError> {
    ...
    candidate.validate(&subject).map_err(|_| ToolError::invalid("invalid_candidate_binding"))?;
...
fn parse(args: &Object, format: GitHashAlgorithm) -> Result<(CandidateReviewCommand, Option<Vec<u8>>), ToolError> {
    ...
    let expected_version = if version == 0 { ExpectedVersion::NewStream } else {
        let version = AggregateVersion::try_new(version).ok_or(ToolError::invalid("invalid_review_version"))?;
        version.next().map_err(|_| ToolError::invalid("review_version_exhausted"))?;
    ...
    let bundle = if decision == ReviewDecision::Withdraw {
        if version == 0 || args.contains_key("bundle_hex_chunks") {
            return Err(ToolError::invalid("withdraw_requires_prior_review_without_bundle"));
        }
        None
    } else { Some(bundle(args)?) };
    let reason = common::body(args, "reason")?.unwrap_or_default();
    Ok((CandidateReviewCommand { candidate, review: ReviewCommand {
        expected_version, subject, decision, reason,
    } }, bundle))
}

pub(super) fn decision_name(decision: ReviewDecision) -> &'static str {
    match decision {
        ReviewDecision::Approve => "approve", ReviewDecision::RequestChanges => "request-changes",
...
pub(super) fn call(backend: &NodeTools, args: &Object) -> Result<Value, ToolError> {
    ...
    if !backend.options.writes.reviews { return Err(ToolError::invalid("tool_not_granted")); }
    ...
    command.proposed_event(principal, backend.options.format)
        .map_err(|_| ToolError::invalid("invalid_candidate_review"))?;
    let context = backend.node.request_context();
    let (tx, terminal) = backend.node.runtime().block_on(backend.node.admit_candidate_review_durable_in(
        &context, &session, &command, bundle.as_deref(), Default::default(),
    ...
    result.insert("decision".into(), text(decision_name(command.review.decision)));
    result.insert("expected_review_version".into(), text(match command.review.expected_version {
        ExpectedVersion::NewStream => 0, ExpectedVersion::Exactly(version) => version.get(),
    }.to_string()));
...
pub(super) fn candidate_schema(properties: Object, required: &[&str]) -> Value {
    ...
    if let Value::Object(fields) = &mut schema {
        fields.insert("allOf".into(), Value::Array([
            ("source_reference", "source_reference_hex"), ("target_reference", "target_reference_hex"),
        ].into_iter().map(|(plain, encoded)| object([("oneOf", Value::Array(vec![
...
fn schema() -> Value {
    ...
    properties.insert("decision".into(), object([("type", text("string")),
        ("enum", Value::Array(vec![text("approve"), text("request-changes"), text("withdraw")]))]));
    properties.insert("reason".into(), common::text_schema(common::MAX_TEXT_BYTES as u64));
    let mut schema = candidate_schema(properties, &[
        "number", "expected_version", "idempotency_key", "expected_source", "expected_target",
        "merge_base", "candidate_commit", "policy_epoch", "review_version", "decision",
    ]);
    if let Value::Object(fields) = &mut schema {
        fields.insert("oneOf".into(), Value::Array(vec![
            object([("properties", object([("decision", object([("enum", Value::Array(vec![text("approve"), text("request-changes")]))]))])),
                ("required", Value::Array(vec![text("bundle_hex_chunks")]))]),
            object([("properties", object([("decision", object([("const", text("withdraw"))]))])),
                ("not", object([("required", Value::Array(vec![text("bundle_hex_chunks")]))]))]),
        ]));
    }
    schema
}
...
mod tests {
    ...
    fn args(format: GitHashAlgorithm) -> Object {
        let Value::Object(args) = object([
            ("number", text("7")), ("expected_version", text("1")), ("idempotency_key", text("private-key")),
            ...
            ("policy_epoch", text("1")), ("review_version", text("0")),
            ("decision", text("approve")), ("bundle_hex_chunks", Value::Array(vec![text("abcd")])),
        ]) else { unreachable!() }; args
    }
    #[test]
    fn review_requires_all_coordinates_and_never_accepts_an_injected_principal() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            ...
            for field in good.keys() {
                let mut bad = good.clone(); bad.remove(field);
                assert!(parse(&bad, format).is_err(), "missing {field}");
            }
            for field in ["principal", "reviewer", "tenant_id", "repository_id", "bundle_path", "force", "required_reviewers"] {
                let mut bad = good.clone(); bad.insert(field.into(), text("not-authority"));
                assert!(parse(&bad, format).is_err(), "injected {field}");
            }
            let mut bad = good.clone(); bad.insert("source_reference_hex".into(), text(hex(b"refs/heads/topic")));
            assert!(parse(&bad, format).is_err());
            let mut raw = good.clone(); raw.remove("source_reference");
            raw.insert("source_reference_hex".into(), text(hex(b"refs/heads/topic-\xff")));
            assert!(parse(&raw, format).is_ok());
        }
    }
    #[test]
    fn withdrawals_require_a_prior_exact_review_and_never_revalidate_a_new_bundle() {
        let format = GitHashAlgorithm::Sha1; let mut input = args(format);
        input.insert("decision".into(), text("withdraw"));
        assert!(parse(&input, format).is_err());
        input.remove("bundle_hex_chunks"); assert!(parse(&input, format).is_err());
        input.insert("review_version".into(), text("1"));
        let (command, bytes) = parse(&input, format).unwrap();
        assert!(bytes.is_none()); assert_eq!(command.review.decision, ReviewDecision::Withdraw);
        input.insert("decision".into(), text("request-changes"));
        assert!(parse(&input, format).is_err());
    }
    #[test]
    fn integer_hash_and_text_bounds_fail_before_admission() {
        let format = GitHashAlgorithm::Sha256; let good = args(format);
        for field in ["number", "expected_version", "policy_epoch", "review_version"] {
            for value in [text("01"), text("-1"), text("18446744073709551616"), json::number(1)] {
                let mut bad = good.clone(); bad.insert(field.into(), value);
                assert!(parse(&bad, format).is_err());
            }
        }
        let mut bad = good.clone(); bad.insert("review_version".into(), text(u64::MAX.to_string()));
        assert!(parse(&bad, format).is_err());
        for field in ["expected_source", "expected_target", "merge_base", "candidate_commit"] {
            for value in ["00".repeat(32), "11".repeat(20), "AA".repeat(32)] {
                let mut bad = good.clone(); bad.insert(field.into(), text(value));
                assert!(parse(&bad, format).is_err());
            }
        }
        for reason in ["x\0y".into(), "é".repeat(common::MAX_TEXT_BYTES)] {
            let mut bad = good.clone(); bad.insert("reason".into(), text(reason));
            assert!(parse(&bad, format).is_err());
        }
    }
    #[test]
    fn complete_maximum_bundle_fits_unchanged_json_limits_and_bad_chunks_refuse() {
        ...
        let encoded = Value::Object(input.clone()).encode(json::MAX_INPUT).unwrap();
        ...
        for value in [Value::Null, text("abcd"), Value::Array(vec![]),
            Value::Array(vec![text("ab"); MAX_CHUNKS + 1]),
            Value::Array(vec![text("ab".repeat(CHUNK_BYTES + 1))]),
            Value::Array(vec![text("")]), Value::Array(vec![text("a")]),
            Value::Array(vec![text("AB")]), Value::Array(vec![json::number(1)])] {
            let mut bad = input.clone(); bad.insert("bundle_hex_chunks".into(), value);
            assert!(parse(&bad, format).is_err());
        }
    }
    #[test]
    fn discovery_describes_conditional_bundle_and_lossless_reference_requirements() {
        let descriptor = schema(); let fields = descriptor.object().unwrap();
        ...
        assert_eq!(properties["bundle_hex_chunks"].object().unwrap()["maxItems"].unsigned(), Some(3));
    }
}
