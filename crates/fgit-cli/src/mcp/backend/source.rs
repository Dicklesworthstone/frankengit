...
pub(super) fn tools() -> Vec<Tool> {
    vec![
        Tool {
            ...
            description: "List immediate children of a visible repository ref and path. Paths and names use exact hex bytes. Snapshot-pinned, read-only; no host filesystem access.",
            schema: input_schema(false),
        },
        Tool {
            name: "frankengit_source_blob",
            description: "Read a bounded exact file byte range at a visible ref. Symlink contents are data, never followed. Continue with next_offset and snapshot_token; no checkout or arbitrary object lookup.",
...
fn parse(
...
) -> Result<(RefName, SourceBrowseQuery), ToolError> {
    require_fields(
        args,
        if file {
            &[
    ...
    let path = string(args, "path_hex")?
        .map(|value| unhex(value, 4096))
        .transpose()?;
    ...
    let action = if file {
        SourceBrowseAction::Read {
            offset: decimal(args, "offset", 0)?,
            limit: bounded_number(args, "max_bytes", 16 * 1024, 64 * 1024)? as u32,
        }
    } else {
        SourceBrowseAction::List {
            after: string(args, "after_hex")?
                .map(|value| unhex(value, 4096))
                .transpose()?,
    ...
    query
        .validate(format)
        .map_err(|_| ToolError::invalid("invalid_source_query"))?;
    Ok((reference, query))
}
fn render(
    reference: &RefName,
    ...
    if query
        ...
        || [report.root_tree, report.object_id]
            .iter()
            .any(|id| id.is_zero() || id.algorithm() != report.source_commit.algorithm())
    {
        return Err(invalid());
    }
    let Value::Object(mut result) = object([
        ("reference_hex", text(hex(reference.as_bytes()))),
        ("source_commit", text(report.source_commit.to_string())),
        ("root_tree", text(report.root_tree.to_string())),
        ("source_rcr", text(report.source_rcr.to_string())),
        ("object_id", text(report.object_id.to_string())),
        (
            "path_hex",
            report.path.as_deref().map_or(Value::Null, |p| text(hex(p))),
        ),
    ]) else {
        unreachable!()
    };
    match (&query.action, &report.content) {
        (
            ...
            if entries.len() > usize::from(*limit)
                ...
                || entries.iter().any(|entry| {
                    entry.name.is_empty()
                        || after.as_ref().is_some_and(|n| entry.name <= *n)
                        || entry.oid.is_zero()
                        || entry.oid.algorithm() != report.source_commit.algorithm()
                })
                || next_after.as_ref().is_some_and(|next| {
                    entries.len() != usize::from(*limit)
                        || entries.last().map(|entry| &entry.name) != Some(next)
                })
            {
                return Err(invalid());
            }
            result.insert(
                ...
                Value::Array(
                    entries
                        ...
                        .map(|entry| {
                            object([
                                ("name_hex", text(hex(&entry.name))),
                                ("kind", text(entry.kind.as_str())),
            ...
            result.insert(
                "next_after_hex".into(),
                next_after.as_deref().map_or(Value::Null, |n| text(hex(n))),
            );
        ...
        (
            ...
            result.insert(
                "text_utf8".into(),
                std::str::from_utf8(bytes).ok().map_or(Value::Null, text),
            );
            ...
            result.insert(
                "next_offset".into(),
                next_offset.map_or(Value::Null, |n| text(n.to_string())),
            );
...
fn input_schema(file: bool) -> Value {
    ...
    properties.insert(
        ...
        object([
            ...
            (
                "description",
                text("Exact relative repository path bytes; omitted only for tree root."),
            ),
        ]),
    );
    properties.insert(
    ...
    properties.insert(
        ...
        object([("type", text("string")), ("maxLength", json::number(64))]),
    );
    if file {
        ...
        properties.insert(
            ...
            object([
                ...
                ("maximum", json::number(65536)),
                ("default", json::number(16384)),
            ]),
        );
    } else {
        properties.insert(
        ...
        properties.insert(
            ...
            object([
                ...
                (
                    "description",
                    text("Immediate child name in hex; requires expected_head."),
                ),
            ]),
        );
    }
    object([
        ...
        (
            "required",
            Value::Array(if file {
                vec![text("reference"), text("path_hex")]
            } else {
                vec![text("reference")]
            }),
        ),
