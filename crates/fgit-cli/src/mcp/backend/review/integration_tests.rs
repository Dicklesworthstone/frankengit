...
impl Scratch {
    ...
    fn options(&self, format: GitHashAlgorithm) -> Options {
        Options {
            storage: self.0.join("node"),
            tenant: TenantId::from_bytes([0xd1; 16]),
            repository: RepositoryId::from_bytes([0xd2; 16]),
            format,
            incarnation: None,
            issues: false,
            pulls: true,
...
fn invoke(
    ...
    server
        .receive(backend, request.encode(8192).unwrap().as_bytes())
        .unwrap()
}
...
fn tool_error(response: &Value) {
    assert_eq!(
        response.object().unwrap()["result"].object().unwrap()["isError"],
        Value::Bool(true)
    );
}
...
fn reopened_nodes_review_real_changes_and_recorded_pr_tips_without_publication() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let mut options = scratch.options(format);
        ...
        let branch = RefCommand {
            ...
            proposed_new: ProposedNew::Update(root.commit),
            force: false,
        };
        let published = node
            .runtime()
        ...
        let command = PullRequestCommand {
            ...
            data: PullRequestData {
                ...
                source_tip: tip,
                target_tip: root.commit,
                title: "Review source".into(),
                body: "not an approval".into(),
            },
        };
        let request = node.request_context();
        let (_, terminal) = node
        ...
        let compared = invoke(
            ...
            object([
                ...
                ("expected_before", text(root.commit.to_string())),
                ("expected_after", text(tip.to_string())),
            ]),
        );
        assert_eq!(
            result(&compared)["snapshot_token"].text(),
            Some(head_token(original_head).as_str())
        );
        assert_eq!(result(&compared)["entry_count"].text(), Some("1"));
        let Value::Array(entries) = &result(&compared)["entries"] else {
            panic!("entries")
        };
        let Value::Array(hunks) =
            &entries[0].object().unwrap()["content"].object().unwrap()["hunks"]
        else {
            panic!("hunks")
        };
        assert_eq!(
            hunks[0].object().unwrap()["before_hex"].text(),
            Some("6f6c640a")
        );
        assert_eq!(
            hunks[0].object().unwrap()["after_hex"].text(),
            Some("6e65770a")
        );
        let pr = invoke(
            &mut server,
            ...
            3,
            PULL_DIFF,
            object([("number", text("1")), ("expected_version", text("1"))]),
        );
        assert_eq!(result(&pr)["comparison"].text(), Some("merge-base"));
        assert_eq!(
            result(&pr)["compared_before"].text(),
            Some(root.commit.to_string().as_str())
        );
        assert_eq!(result(&pr)["merge_permission"], Value::Null);
        let stale_version = invoke(
            &mut server,
            &mut backend,
            4,
            PULL_DIFF,
            object([("number", text("1")), ("expected_version", text("2"))]),
        );
        ...
        assert_eq!(
            result(&recorded)["requested_after"].text(),
            Some(tip.to_string().as_str())
        );
        assert_ne!(tip, later);
        for (source, pulls) in [(false, true), (true, false), (false, false)] {
            backend.options.source = source;
            backend.options.pulls = pulls;
...
fn history_and_blame_page_reopened_native_nodes_with_exact_line_origins() {
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let Value::Array(rows) = &result(&first)["commits"] else {
            panic!("commits")
        };
        assert_eq!(
            rows[0].object().unwrap()["id"].text(),
            Some(tip.to_string().as_str())
        );
        let tail = invoke(
            &mut server,
        ...
        let Value::Array(rows) = &result(&tail)["commits"] else {
            panic!("commits")
        };
        assert_eq!(
            rows[0].object().unwrap()["id"].text(),
        ...
        let Value::Array(lines) = &result(&first_line)["lines"] else {
            panic!("lines")
        };
        assert_eq!(
            lines[0].object().unwrap()["origin_commit"].text(),
            Some(tip.to_string().as_str())
        );
        // Default 100-line page clamps at EOF rather than refusing a short file.
        let last_line = invoke(
            &mut server,
            &mut backend,
            5,
            BLAME,
            object([
                ...
                (
                    "expected_head",
                    result(&first_line)["snapshot_token"].clone(),
                ),
            ]),
        );
        assert_eq!(
            result(&last_line)["bytes_hex"].text(),
            Some("737461626c650a")
        );
        assert_eq!(result(&last_line)["complete"], Value::Bool(true));
        let Value::Array(lines) = &result(&last_line)["lines"] else {
            panic!("lines")
        };
        assert_eq!(
            lines[0].object().unwrap()["origin_commit"].text(),
        ...
        assert_eq!(
            result(&last_line)["human_authorship_proven"],
            Value::Bool(false)
        );
