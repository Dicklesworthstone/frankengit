...
use fgit_forge::{
    ExpectedVersion, IssueNumber,
    event::issue::{IssueAction, IssueCommand},
};
...
fn real_mcp_reads_reopened_sha1_and_sha256_nodes_without_publishing() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let before = {
            let request = node.request_context();
            node.runtime()
                .block_on(node.read_issues_in(&request, 0, 1, None))
                .unwrap()
                .source_head
        };
        node.shutdown().unwrap();
        let mut backend = NodeTools::open(Options {
            storage: scratch.0.join("node"),
            tenant: TenantId::from_bytes([0x91; 16]),
            repository: RepositoryId::from_bytes([0x92; 16]),
            format,
        ...
        assert_eq!(
            first["snapshot_token"].text(),
            Some(head_token(before).as_str())
        );
        let next = object([
            ("jsonrpc", text("2.0")),
            ("id", json::number(3)),
            ...
            (
                "params",
                object([
                    ("name", text("frankengit_issue_list")),
                    (
                        "arguments",
                        object([
                            ("after", text("7")),
                            ("limit", json::number(1)),
                            ("expected_head", first["snapshot_token"].clone()),
                        ]),
                    ),
                ]),
            ),
        ...
        let next = server
            .receive(&mut backend, next.encode(4096).unwrap().as_bytes())
            .unwrap();
        ...
        assert_eq!(
            result(&shown)["issue"].object().unwrap()["title"].text(),
            Some("Issue 7 é")
        );
        ...
        assert_eq!(
            before, after,
            "MCP reads and rejected authority fields must not publish"
        );
...
fn real_source_and_pr_tools_preserve_bytes_snapshots_and_independent_grants() {
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let accepted = node
            ...
            .unwrap();
        assert!(matches!(
            accepted.commands[0].terminal.outcome,
            DecisionOutcome::Committed { .. }
        ));
        ...
        let blob = invoke(
            ...
            object([
                ("reference", text("refs/heads/main")),
                ("path_hex", text("524541444d45")),
                ("max_bytes", json::number(4)),
            ]),
        );
        ...
        let rest = invoke(
            ...
            object([
                ...
                ("path_hex", text("524541444d45")),
                ("offset", text("4")),
                ("expected_head", result(&blob)["snapshot_token"].clone()),
            ]),
        );
        ...
        let after = backend
            .node
            .runtime()
            .block_on(backend.node.read_issues_in(&context, 0, 1, None))
            .unwrap()
            .source_head;
        ...
        let change = IssueCommand {
            number: IssueNumber::FIRST,
            expected_version: ExpectedVersion::NewStream,
            action: IssueAction::Open {
                title: "Move authority".into(),
                body: String::new(),
                labels: vec![],
            },
        };
        ...
        let stale = invoke(
            ...
            object([
                ("reference", text("refs/heads/main")),
                ("after_hex", result(&tree)["next_after_hex"].clone()),
                ("expected_head", result(&tree)["snapshot_token"].clone()),
            ]),
        );
        assert_eq!(
