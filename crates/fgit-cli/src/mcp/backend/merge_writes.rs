...
fn parse(args: &Object, format: GitHashAlgorithm, principal: PrincipalId) -> Result<Input, ToolError> {
    let mut allowed = candidate::CANDIDATE_FIELDS.to_vec(); allowed.push("required_reviewers");
    require_fields(args, &allowed)?;
    common::key(args)?;
    let (subject, candidate) = candidate::subject(args, format)?;
    subject.pull_request_version.next().map_err(|_| ToolError::invalid("pr_version_exhausted"))?;
    ...
    for reviewer in reviewers {
        let value = reviewer.text().ok_or(ToolError::invalid("invalid_reviewer_id"))?;
        if value.len() != 32 || !value.bytes().all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b)) {
            return Err(ToolError::invalid("invalid_reviewer_id"));
        }
        let reviewer = PrincipalId::from_hex(value).map_err(|_| ToolError::invalid("invalid_reviewer_id"))?;
        if reviewer == principal { return Err(ToolError::invalid("submitter_cannot_satisfy_review_gate")); }
        if !required.insert(reviewer) { return Err(ToolError::invalid("duplicate_required_reviewer")); }
    }
    Ok(Input { subject, candidate, bundle: candidate::bundle(args)?, reviewers: required.into_iter().collect() })
}

pub(super) fn call(backend: &NodeTools, args: &Object) -> Result<Value, ToolError> {
    if !backend.options.writes.merges { return Err(ToolError::invalid("tool_not_granted")); }
    ...
    let (tx, terminal) = backend.node.runtime().block_on(backend.node.apply_reviewed_merge_bundle_durable_in(
        &context, principal, common::required(args, "idempotency_key")?.as_bytes(),
        input.subject.pull_request, ExpectedVersion::Exactly(input.subject.pull_request_version),
        &input.candidate.merge(&input.subject), &input.bundle, input.subject.policy_epoch, &input.reviewers,
    ...
    result.insert("required_reviewers".into(), Value::Array(input.reviewers.iter().map(|id| text(id.to_string())).collect()));
    ...
    result.insert("repository_wide_branch_protection".into(), Value::Bool(false));
...
fn schema() -> Value {
    ...
    candidate::candidate_schema(properties, &[
        "number", "expected_version", "idempotency_key", "expected_source", "expected_target",
        "merge_base", "candidate_commit", "policy_epoch", "bundle_hex_chunks", "required_reviewers",
    ])
}
...
mod tests {
    ...
    fn args(format: GitHashAlgorithm) -> Object {
        let Value::Object(args) = object([
            ("number", text("7")), ("expected_version", text("1")), ("idempotency_key", text("merge-key")),
            ...
            ("candidate_commit", text("44".repeat(format.digest_len()))), ("policy_epoch", text("1")),
            ("bundle_hex_chunks", Value::Array(vec![text("abcd")])),
            ("required_reviewers", Value::Array(vec![text(principal(0x52).to_string()), text(principal(0x51).to_string())])),
        ]) else { unreachable!() }; args
    }
    #[test]
    fn every_merge_coordinate_and_an_explicit_nonempty_reviewer_set_are_required() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            let good = args(format); let parsed = parse(&good, format, principal(0x50)).unwrap();
            ...
            for field in good.keys() {
                ...
                assert!(parse(&bad, format, principal(0x50)).is_err(), "missing {field}");
            }
            for field in ["principal", "reviewer", "review_version", "decision", "reason", "force", "bundle_path"] {
                let mut bad = good.clone(); bad.insert(field.into(), text("no implicit authority"));
                assert!(parse(&bad, format, principal(0x50)).is_err(), "injected {field}");
            }
        }
    }
    #[test]
    fn missing_duplicate_self_malformed_and_over_limit_reviewers_fail_closed() {
        let format = GitHashAlgorithm::Sha1; let good = args(format);
        for reviewers in [Value::Null, text("all"), Value::Array(vec![]),
            Value::Array(vec![text(principal(0x50).to_string())]),
            Value::Array(vec![text(principal(0x51).to_string()); 2]),
            Value::Array(vec![text("AB".repeat(16))]), Value::Array(vec![json::number(1)]),
            Value::Array(vec![text("bad")]),
            Value::Array((1..=33).map(|n| text(principal(n).to_string())).collect())] {
            let mut bad = good.clone(); bad.insert("required_reviewers".into(), reviewers);
            assert!(parse(&bad, format, principal(0x50)).is_err());
        }
        let mut edge = good.clone();
        edge.insert("required_reviewers".into(), Value::Array((1..=32).map(|n| text(principal(n).to_string())).collect()));
        assert_eq!(parse(&edge, format, principal(0x50)).unwrap().reviewers.len(), MAX_REVIEWERS);
        let mut exhausted = good; exhausted.insert("expected_version".into(), text(u64::MAX.to_string()));
    ...
    fn reviewer_order_is_not_identity_but_changing_the_set_is_not_silently_discarded() {
        let format = GitHashAlgorithm::Sha256; let first = args(format); let mut second = first.clone();
        let Value::Array(reviewers) = second.get_mut("required_reviewers").unwrap() else { unreachable!() };
        reviewers.reverse();
        assert_eq!(parse(&first, format, principal(0x50)).unwrap().reviewers,
            parse(&second, format, principal(0x50)).unwrap().reviewers);
        second.insert("required_reviewers".into(), Value::Array(vec![text(principal(0x53).to_string())]));
        assert_ne!(parse(&first, format, principal(0x50)).unwrap().reviewers,
            parse(&second, format, principal(0x50)).unwrap().reviewers);
        ...
        let Value::Array(required) = &fields["required"] else { unreachable!() };
        assert!(required.contains(&text("required_reviewers")) && required.contains(&text("bundle_hex_chunks")));
    }
}
