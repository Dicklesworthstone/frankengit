...
use super::super::super::{
    WriteGrants,
    protocol::{self, ReadTools, Server},
};
use super::super::{NodeTools, Options, outcomes, pull_writes};
use super::*;
use fgit_authority::IdempotencyKey;
use fgit_forge::preparation::MergeMetadata;
use fgit_node::{NodeConfig, OneNode};
use fgit_types::{
    GitOid, HeadGeneration, PrincipalId, RefName, RepositoryAuthorityHeadId, RepositoryId, TenantId,
};
use std::{
...
impl Fixture {
    fn new(format: GitHashAlgorithm) -> Self {
        ...
        let mut options = Options {
            ...
            tenant: TenantId::from_bytes([0x61; 16]),
            repository: RepositoryId::from_bytes([0x62; 16]),
            format,
            incarnation: None,
            issues: false,
            pulls: true,
        ...
        let (_, plan, bundle) = node
            ...
            .unwrap();
        let session = fgit_node::LoopbackReceiveSession::authenticated(
            actor(),
            IdempotencyKey::new(b"seed".to_vec()).unwrap(),
        );
        let result = node
            .runtime()
...
fn prepare(
    ...
    assert_eq!(
        state(node).0,
        before,
        "preparing test inputs must not publish"
    );
    assert!(candidate.bundle_bytes().len() <= MAX_BUNDLE_BYTES);
    (
        candidate.candidate_commit,
        candidate.bundle_bytes().to_vec(),
    )
}
fn command(branch: &str, key: &str, fields: &[(&str, Value)]) -> Value {
    let Value::Object(mut args) =
        object([("reference", text(branch)), ("idempotency_key", text(key))])
    else {
        unreachable!()
    };
...
fn publishing(branch: &str, key: &str, base: GitOid, tip: GitOid, bytes: &[u8]) -> Value {
    command(
        ...
        key,
        &[
            ("expected_base", text(base.to_string())),
            ("expected_candidate", text(tip.to_string())),
            (
                "bundle_hex_chunks",
                Value::Array(
                    bytes
                        .chunks(CHUNK_BYTES)
                        .map(|chunk| text(hex(chunk)))
                        .collect(),
                ),
            ),
        ],
...
fn message(id: u64, name: &str, args: Value) -> String {
    object([
        ...
        (
            "params",
            object([("name", text(name)), ("arguments", args)]),
        ),
    ])
...
fn refused(value: &Value) {
    ...
    assert_ne!(
        result(value).get("outcome").and_then(Value::text),
        Some("committed")
    );
}
...
fn source_publish_pr_review_and_atomic_branch_lifecycle_compose_in_both_hash_domains() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let mut backend = NodeTools::open(f.options.clone()).unwrap();
        let mut server = start(&mut backend);
        let create = command(
            "refs/heads/topic",
            "topic",
            &[("target", text(f.commit.to_string()))],
        );
        let created = invoke(&mut server, &mut backend, 2, CREATE, create.clone());
        committed(&created);
        let held = state(&backend.node).0;
        let replay = invoke(&mut server, &mut backend, 3, CREATE, create);
        assert_eq!(result(&created), result(&replay));
        assert_eq!(state(&backend.node).0, held);
        let (tip, bundle) = prepare(&backend.node, "refs/heads/topic", f.commit, "after", 2);
        let (alternate, competing) =
            prepare(&backend.node, "refs/heads/topic", f.commit, "competing", 3);
        let published = invoke(
            &mut server,
            &mut backend,
            4,
            PUBLISH,
            publishing("refs/heads/topic", "publish", f.commit, tip, &bundle),
        );
        committed(&published);
        assert_eq!(state(&backend.node).1[&reference("refs/heads/topic")], tip);
        assert_eq!(
            result(&published)["current_refs_asserted"],
            Value::Bool(false)
        );
        ...
        let pr = object([
            ...
            ("idempotency_key", text("pr")),
            ("source_reference", text("refs/heads/topic")),
            ("target_reference", text("refs/heads/main")),
            ("expected_source", text(tip.to_string())),
            ("expected_target", text(f.commit.to_string())),
            ("title", text("Source written through MCP")),
            ("body", text("Ready for review, not approved")),
        ]);
        let opened = invoke(&mut server, &mut backend, 5, pull_writes::OPEN, pr);
        committed(&opened);
        let diff = invoke(
            &mut server,
            &mut backend,
            6,
            super::super::review::PULL_DIFF,
            object([("number", text("7")), ("expected_version", text("1"))]),
        );
        assert_eq!(result(&diff)["entry_count"].text(), Some("1"));
        assert_eq!(
            result(&diff)["requested_after"].text(),
        ...
        refused(&invoke(
            ...
            7,
            PUBLISH,
            publishing("refs/heads/topic", "stale", f.commit, alternate, &competing),
        ));
        ...
        *corrupt.last_mut().unwrap() ^= 1;
        refused(&invoke(
            ...
            8,
            PUBLISH,
            publishing("refs/heads/topic", "corrupt", f.commit, tip, &corrupt),
        ));
        assert_eq!(state(&backend.node).1, before);
        let occupied = command(
            ...
            "occupied",
            &[
                ("expected_old", text(tip.to_string())),
                ("destination", text("refs/heads/main")),
            ],
        );
        refused(&invoke(&mut server, &mut backend, 9, RENAME, occupied));
        assert_eq!(state(&backend.node).1, before);
        refused(&invoke(
            &mut server,
            &mut backend,
            ...
            command(
                "refs/heads/main",
                "delete-default",
                &[("expected_old", text(f.commit.to_string()))],
            ),
        ));
        assert_eq!(state(&backend.node).1, before);
        let rename = command(
            ...
            &[
                ("expected_old", text(tip.to_string())),
                ("destination", text("refs/heads/renamed")),
            ],
        );
        let renamed = invoke(&mut server, &mut backend, 11, RENAME, rename.clone());
        committed(&renamed);
        ...
        assert!(!held.1.contains_key(&reference("refs/heads/renamed")));
        ...
        let retried = invoke(&mut server, &mut stopped, 2, RENAME, rename);
        assert_eq!(result(&retried), result(&renamed));
        ...
        stopped
            ...
            .unwrap();
        assert_eq!(state(&stopped.node), held);
...
fn lost_published_reply_is_recoverable_and_no_following_request_runs() {
    ...
    let second = message(
        ...
        CREATE,
        command(
            "refs/heads/must-not-run",
            "unread-key",
            &[("target", text(tip.to_string()))],
        ),
    );
    ...
    let recovered = invoke(
        ...
        2,
        outcomes::NAME,
        object([("idempotency_key", text("lost-publish"))]),
    );
    ...
    let unread = invoke(
        ...
        3,
        outcomes::NAME,
        object([("idempotency_key", text("unread-key"))]),
    );
    assert_eq!(
        result(&unread)["observation"].text(),
        Some("key_not_observed")
    );
    let retried = invoke(&mut server, &mut backend, 4, PUBLISH, publish);
    committed(&retried);
    assert_eq!(result(&retried)["tx_id"], result(&recovered)["tx_id"]);
    assert_eq!(state(&backend.node), held);
    backend.close().unwrap();
}
...
fn source_write_grant_never_implies_other_tools_and_denial_precedes_body_intake() {
    ...
    for name in [CREATE, UPDATE, DELETE, RENAME, PUBLISH] {
        ...
        assert_eq!(
            call(&reader, name, &Object::new()).unwrap_err().code,
            "tool_not_granted"
        );
    }
    assert_eq!(state(&reader.node), held);
    reader.close().unwrap();
}
