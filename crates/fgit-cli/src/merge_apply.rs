...
use fgit_types::{
    DecisionOutcome, HeadGeneration, PrincipalId, RefName, RepositoryId, TenantId, TxId,
};
use std::path::PathBuf;

use super::publication_support::{
    describe, parse_oid, quote, read_bundle, set_once, write_terminal_receipt,
};
...
pub fn run(arguments: &[String]) -> Result<(), String> {
    ...
    let (tx_id, terminal) = match operation {
        ...
        Err(error) => {
            let cleanup = cleanup.as_ref().map_or_else(String::new, |error| {
                format!("; node shutdown also failed: {error}")
            });
            return Err(format!(
                "no terminal outcome returned: {error}{cleanup}; this is not evidence of non-commit. Retry the identical reviewed bundle, principal, key, source/target/base/candidate coordinates and PR version to reconcile; do not recompute a merge or change the key to recover this attempt"
            ));
        }
    };
    ...
    {
        return Err(cleanup.as_ref().map_or_else(
            || error.clone(),
...
fn parse(arguments: &[String]) -> Result<Options, String> {
    ...
    let target_ref =
        RefName::try_new(arguments[4].as_bytes()).map_err(|error| error.to_string())?;
    let (mut principal, mut key, mut source_ref, mut source_tip) = (None, None, None, None);
    let (mut target_tip, mut base_tip, mut candidate, mut number, mut version) =
    ...
    while cursor < arguments.len() {
        ...
        match flag {
            "--principal" => set_once(
                &mut principal,
                PrincipalId::from_hex(value).map_err(|error| error.to_string())?,
                flag,
            )?,
            "--idempotency-key" => {
            ...
            _ => {
                return Err(format!(
                    "unknown merge apply option {flag}; implicit or forced merges are not supported"
                ));
            }
        }
    }
    ...
    let pull_request =
        PullRequestNumber::try_new(number).ok_or("pull request number must be positive")?;
    let version_number = version
        .ok_or("--expected-version is required; 0 explicitly selects a new merge-receipt stream")?;
    let version = if version_number == 0 {
        ExpectedVersion::NewStream
    } else {
        let version =
            AggregateVersion::try_new(version_number).ok_or("invalid aggregate version")?;
        version
            .next()
    ...
    let merge = NativeMerge {
        ...
        merge_commit: candidate
            .ok_or("--expected-commit is required and must name the reviewed commit")?,
    };
    merge
        .validate()
        .map_err(|error| format!("invalid reviewed merge coordinates: {error:?}"))?;
    ...
    Ok(Options {
        ...
        key: key.ok_or("--idempotency-key is required")?,
        pull_request,
        version,
        version_number,
...
fn decimal(value: &str, field: &str) -> Result<u64, String> {
    ...
    {
        return Err(format!(
            "{field} must be a canonical nonnegative decimal integer"
        ));
    }
    value
        .parse()
        .map_err(|_| format!("{field} exceeds the integer limit"))
}
...
fn render_receipt(
    ...
    let fields = [
        ...
        format!(
            ...
            hex(&options.merge.source_ref)
        ),
        format!(
            "\"target_reference_hex\":\"{}\"",
            hex(&options.merge.target_ref)
        ),
        format!("\"expected_source\":\"{}\"", options.merge.source_tip),
        format!(
            "\"expected_target\":\"{}\"",
            options.merge.target_tip_before
        ),
...
mod tests {
    ...
    fn all_review_coordinates_are_required_in_both_hash_domains() {
        for width in [40, 64] {
            ...
            assert_eq!(
                options.merge.target_tip_before.to_string(),
                "b".repeat(width)
            );
