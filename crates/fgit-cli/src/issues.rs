...
pub(super) fn run(arguments: &[String]) -> Result<u8, String> {
    if arguments == ["--help"]
        || (arguments.len() == 2
            ...
            && matches!(
                arguments[0].as_str(),
                "open" | "edit" | "close" | "reopen" | "comment" | "list" | "show"
            ))
    {
    ...
    if let Operation::Mutate(mutation) = &mut options.operation {
        ...
        mutation
            .command
            .proposed_event(mutation.principal)
            .map_err(|_| "invalid issue command or file contents")?;
    }
    ...
    .map_err(|error| format!("cannot open issue node: {error}"))?;
    let result = (|| {
        let service = node
        ...
        match &options.operation {
            Operation::Mutate(mutation) => {
                ...
                node.runtime()
                    ...
                    .map(Completed::Mutation)
                    .map_err(|error| match service {
                        Ok(()) => error.to_string(),
            ...
            Operation::Read(read) => {
                ...
                let (receipt, exit) = if let Some(number) = read.number {
                    let page = node
                        .runtime()
                        .block_on(node.read_issue_history_in(
                            &request,
                            number,
                            read.after,
                            read.limit,
    ...
    match (&options.operation, result) {
        (Operation::Mutate(mutation), Ok(Completed::Mutation((tx, terminal)))) => finish_mutation(
        ...
        (Operation::Read(_), Ok(Completed::Read(receipt, exit))) => {
            if let Some(error) = cleanup {
                return Err(format!("issue read shutdown failed: {error}"));
            }
            write_read(&mut std::io::stdout().lock(), &receipt)?;
            Ok(exit)
        }
        (operation, Err(error)) => {
            let cleanup = cleanup.map_or_else(String::new, |value| {
                format!("; shutdown also failed: {value}")
            });
            if matches!(operation, Operation::Mutate(_)) {
                Err(format!(
                    "no terminal issue outcome returned: {error}{cleanup}; this is not evidence of non-commit. Recover using fg outcome with the same principal and key, or retry the identical command, version and content. Do not replace the key or refresh the expected version"
                ))
            } else {
                Err(format!(
                    "issue read failed: {error}{cleanup}; no complete result returned"
                ))
            }
        }
        _ => Err(format!(
            "issue operation/result mismatch; cleanup: {cleanup:?}"
        )),
    }
}
enum Completed {
...
fn finish_mutation(
    ...
    if let Err(error) = write_terminal_receipt(output, &receipt, tx, terminal) {
        return Err(format!(
            "{error}{}",
            cleanup.map_or_else(String::new, |value| format!(
                "; shutdown also failed: {value}"
            ))
        ));
    }
    if let Some(error) = cleanup {
...
fn write_read(output: &mut impl Write, report: &str) -> Result<(), String> {
    writeln!(output, "{report}")
        ...
        .map_err(|error| format!("issue report output failed: {error}"))
}
fn read_body_file(path: &Path) -> Result<String, String> {
    // Stable operator-controlled regular files; not a hostile-filesystem API.
    let before =
        fs::symlink_metadata(path).map_err(|error| format!("issue body metadata: {error}"))?;
    if !before.is_file() || before.len() > MAX_BODY_BYTES as u64 {
        return Err(
            "issue body must be a regular file of at most 65536 bytes, not a symlink or device"
                .to_owned(),
        );
    }
    ...
    let opened = file
        .metadata()
        .map_err(|error| format!("issue body metadata: {error}"))?;
    if !opened.is_file() || opened.len() > MAX_BODY_BYTES as u64 {
        return Err("issue body changed to a non-regular or oversized file".to_owned());
    }
    let mut bytes = Vec::new();
    bytes
        .try_reserve(MAX_BODY_BYTES + 1)
        .map_err(|_| "issue body allocation refused")?;
    file.take((MAX_BODY_BYTES + 1) as u64)
        .read_to_end(&mut bytes)
        .map_err(|error| format!("issue body read: {error}"))?;
    if bytes.len() > MAX_BODY_BYTES || bytes.contains(&0) {
        return Err("issue body exceeds 65536 bytes or contains NUL".to_owned());
    }
    String::from_utf8(bytes)
        .map_err(|_| "issue body must be UTF-8; no lossy conversion is performed".to_owned())
}
