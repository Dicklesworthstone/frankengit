...
use fgit_types::{
    CANONICAL_CODEC_VERSION, DigestAlgorithmId, DigestBytes, HeadGeneration, RefName,
...
pub(super) fn run(args: &[String], direction: ReplayDirection) -> Result<u8, String> {
    ...
    if let Some(path) = &options.message_file {
        options.metadata.message = read_bundle(path, 64 * 1024)?;
    }
    options
        .metadata
        .validate()
    ...
    let resolutions = options
        .resolutions
        .as_ref()
        .map(|choices| resolution::load(choices, options.limits))
        .transpose()?;
    ...
    let mut node = OneNode::open_existing(
        NodeConfig::new(options.storage.clone(), options.tenant, options.repository)
            .with_object_format(options.inputs.target.algorithm()),
    )
    .map_err(|error| error.to_string())?;
    let operation = (|| {
        ...
        match resolutions.as_deref() {
            Some(choices) => node
                .runtime()
                .block_on(node.prepare_resolved_replay_bundle_in(
                    &request,
                    &options.target,
                    &options.source,
                    options.inputs,
                ...
                .map(|result| (result.artifact, result.resolutions))
                .map_err(|error| error.to_string()),
            None => node
                .runtime()
                .block_on(node.prepare_replay_bundle_in(
                    &request,
                    &options.target,
                    &options.source,
                    options.inputs,
    ...
    let (artifact, resolved_paths) = match (operation, cleanup) {
        ...
        (Ok(_), Some(error)) => {
            return Err(format!(
                "node shutdown failed: {error}; no bundle was published"
            ));
        }
        (Err(error), Some(cleanup)) => {
            return Err(format!(
                "{error}; node shutdown also failed: {cleanup}; no bundle was published"
            ));
        }
    };
    ...
    let receipt = if let Some(choices) = resolutions.as_deref() {
        if matches!(artifact.outcome, ReplayPreparation::Conflicted { .. }) {
            return Err(
                "explicit resolution returned unresolved conflicts; no bundle was published".into(),
            );
        }
        resolution::decorate_receipt(
            receipt,
            choices,
            &resolved_paths,
            options.inputs.target.algorithm(),
        )?
    } else {
        if !resolved_paths.is_empty() {
            return Err("automatic replay returned unsolicited resolutions".into());
        }
        receipt
    };
...
fn parse(args: &[String], direction: ReplayDirection) -> Result<Options, String> {
    if args.len() < 6 || !matches!(args[0].as_str(), "prepare" | "resolve") {
        return Err(USAGE.into());
    }
    let resolving = args[0] == "resolve";
    let argument_limit = if resolving {
        48 + 4 * PreparationLimits::default().max_conflicts
    } else {
        48
    };
    ...
    while at < args.len() {
        ...
        if resolution::is_choice(flag) {
            if !resolving {
                return Err("conflict choices require the explicit resolve command".into());
            }
            if choices.len() >= PreparationLimits::default().max_conflicts {
                return Err("too many conflict choices".into());
            }
    ...
    let target_bytes = if flags.contains_key("--target-ref-hex") {
        unhex(&args[4], 4096)?
    } else {
    ...
    if target_bytes.len() > 4096
        || [target.as_bytes(), source.as_bytes()]
            .iter()
            .any(|name| !name.starts_with(b"refs/heads/"))
    {
    ...
    if [source_tip, selected_commit]
        .iter()
        .any(|id| id.algorithm() != expected_target.algorithm())
    {
    ...
    let mainline = flags
        .get("--mainline")
        .map(|text| {
            decimal(text).and_then(|n| {
                u16::try_from(n)
                    .ok()
                    .filter(|n| *n > 0)
                    .ok_or_else(|| "mainline must be a positive one-based u16".into())
            })
        })
        .transpose()?;
    let (message, message_file) = match (flags.get("--message"), flags.get("--message-file")) {
        ...
        (None, Some(path)) if !path.is_empty() && path.len() <= 4096 => {
            (Vec::new(), Some(PathBuf::from(*path)))
        }
    ...
    let resolutions = if resolving {
        if choices.is_empty() {
            return Err("resolve requires at least one explicit conflict choice".into());
        }
...
pub(super) fn parse_head(text: &str) -> Result<RepositoryAuthorityHeadId, String> {
    let (algorithm, bytes) = text
        ...
        .ok_or("expected algorithm-qualified head token")?;
...
fn render(
    ...
    outcome: &ReplayPreparation,
    bundle: Option<&[u8]>,
    pack_objects: usize,
    borrowed_objects: usize,
) -> Result<(String, u8), String> {
    if options.head.is_some_and(|expected| expected != head) {
        return Err("replay snapshot binding mismatch".into());
    }
    let (coordinates, result, code, details) = match outcome {
        ReplayPreparation::Clean(plan) => {
            let bytes = bundle
                .filter(|bytes| !bytes.is_empty())
                .ok_or("clean replay omitted its bundle")?;
            if pack_objects == 0 || borrowed_objects > pack_objects {
                return Err("replay pack accounting invalid".into());
            }
            (
                &plan.coordinates,
                "prepared",
        ...
        ReplayPreparation::NoChange { coordinates } => {
            if bundle.is_some() || pack_objects != 0 || borrowed_objects != 0 {
                return Err("no-change replay carried an artifact".into());
            }
            (
                coordinates,
                "no_change",
                0,
                "\"net_tree_change\":false".into(),
            )
        }
    };
    if coordinates.request != options.inputs {
        return Err("replay input binding mismatch".into());
    }
    let operation = match options.inputs.direction {
        ReplayDirection::CherryPick => "cherry-pick",
        ReplayDirection::Revert => "revert",
    };
    let receipt = format!(
        concat!(
            "{{\"type\":\"commit_replay_preparation\",\"schema_version\":1,\"profile\":\"path-v1\",",
        ...
        quote(options.inputs.target.algorithm().as_str()),
        quote(&head.to_string()),
        quote(&token(head)),
        quote(&hex(options.target.as_bytes())),
        quote(&hex(options.source.as_bytes())),
        quote(&options.inputs.target.to_string()),
        quote(&options.inputs.source_tip.to_string()),
        quote(&options.inputs.selected_commit.to_string()),
        coordinates
            .selected_parent
        ...
        quote(&options.metadata.author),
        quote(&options.metadata.committer),
        options.metadata.timestamp,
        quote(&hex(&options.metadata.message)),
        bundle.is_some(),
        if bundle.is_some() {
    ...
    if receipt.len() > 4 * 1024 * 1024 {
        return Err("replay receipt exceeds output bound".into());
    }
    Ok((receipt, code))
}
...
mod tests {
    ...
    fn exact_coordinates_metadata_and_profile_are_required_for_both_hash_formats() {
        for width in [40, 64] {
            ...
            let parsed = parse(&input, ReplayDirection::Revert).unwrap();
            assert_eq!(parsed.target, parsed.source);
            assert_eq!(parsed.metadata.message, b"raw\r\n");
            for flag in [
                "--profile",
                "--source-ref",
                "--expected-target",
                ...
                assert!(parse(&missing, ReplayDirection::CherryPick).is_err());
                let mut duplicate = input.clone();
                duplicate.extend([flag.into(), input[at + 1].clone()]);
                assert!(parse(&duplicate, ReplayDirection::CherryPick).is_err());
            }
            let mut untrusted = input;
            untrusted.retain(|s| s != "--trusted-local");
            assert!(parse(&untrusted, ReplayDirection::Revert).is_err());
        }
    }
    #[test]
    fn invalid_mainlines_mixed_domains_and_mutation_options_refuse() {
        for extra in [
            ...
            vec!["--force", "true"],
            vec!["--max-commits", "0"],
            vec!["--max-output-bytes", "33554433"],
            vec!["--source-ref-hex", "726566732f68656164732f6d61696e"],
            vec!["--message-file", "file"],
        ] {
            let mut bad = args(40);
            bad.extend(extra.into_iter().map(str::to_owned));
            assert!(parse(&bad, ReplayDirection::CherryPick).is_err());
        }
        let mut mixed = args(40);
        let at = mixed.iter().position(|s| s == "--commit").unwrap();
        mixed[at + 1] = "b".repeat(64);
        assert!(parse(&mixed, ReplayDirection::CherryPick).is_err());
        let mut valid = args(64);
        valid.extend(["--mainline".into(), "2".into()]);
        assert_eq!(
            parse(&valid, ReplayDirection::Revert)
                .unwrap()
                .inputs
                .mainline,
            Some(2)
    ...
    fn raw_reference_inputs_and_head_pins_are_lossless() {
        ...
        input.push("--target-ref-hex".into());
        let at = input.iter().position(|s| s == "--source-ref").unwrap();
        input[at] = "--source-ref-hex".into();
        input[at + 1] = hex(b"refs/heads/\xfe");
        let head = format!("alg:1:{}", "ab".repeat(32));
        input.extend(["--expected-head".into(), head.clone()]);
        let options = parse(&input, ReplayDirection::CherryPick).unwrap();
        assert_eq!(options.target.as_bytes(), b"refs/heads/\xff");
        assert_eq!(options.source.as_bytes(), b"refs/heads/\xfe");
    ...
    fn no_change_receipts_and_output_errors_do_not_claim_publication() {
        ...
        let result = ReplayPreparation::NoChange {
            coordinates: ReplayCoordinates {
                request: options.inputs,
                selected_parent: Some(options.inputs.target),
                selected_mainline: Some(1),
            },
        };
        let (text, status) = render(&options, head, &result, None, 0, 0).unwrap();
        assert_eq!(status, 0);
        assert!(text.contains("\"bundle_created\":false"));
        assert!(text.contains("\"objects_staged\":false"));
        assert!(render(&options, head, &result, Some(b"bad"), 1, 0).is_err());
        struct Fail;
        impl Write for Fail {
            ...
            fn flush(&mut self) -> std::io::Result<()> {
                Err(std::io::Error::other("broken flush"))
            }
        }
        assert!(
            write_receipt(&mut Fail, &text, true)
                .unwrap_err()
                .contains("bundle_published=true")
        );
        assert!(
            write_receipt(&mut Fail, &text, false)
                .unwrap_err()
                .contains("bundle_published=false")
        );
    }
}
