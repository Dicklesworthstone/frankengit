...
pub fn run(arguments: &[String]) -> Result<u8, String> {
    ...
    if let Operation::Mutate(mutation) = &mut options.operation {
        ...
        mutation
            .command
            .proposed_event(mutation.principal, options.format)
    ...
    .map_err(|error| format!("cannot open PR node: {error}"))?;
    let operation = (|| {
        ...
        match &options.operation {
            Operation::Mutate(mutation) => {
                let session = LoopbackReceiveSession::authenticated(
                    mutation.principal,
                    IdempotencyKey::new(mutation.key.clone())
            ...
            Operation::Read(read) => {
                ...
                let rows: Vec<_> = page
                    ...
                    .map(|view| output::Row {
                        ...
                        opened_by: view.opened_by,
                        last_metadata_actor: view.last_metadata_actor,
                    })
                    .collect();
    ...
    match (&options.operation, operation) {
        ...
        (Operation::Read(_), Ok(Completed::Read(receipt, exit))) => {
            if let Some(error) = cleanup {
                return Err(format!("PR read node shutdown failed: {error}"));
            }
            write_read_report(&mut std::io::stdout().lock(), &receipt)?;
            Ok(exit)
        }
        (operation, Err(error)) => {
            let cleanup = cleanup.as_deref().map_or_else(String::new, |cleanup| {
                format!("; node shutdown also failed: {cleanup}")
            });
            if matches!(operation, Operation::Mutate(_)) {
                Err(format!(
                    "no terminal PR outcome returned: {error}{cleanup}; this is not evidence of non-commit. Reconcile by retrying the identical command, principal, key, expected version, branch identities, tips, title and body bytes; do not replace its key or refresh its metadata"
                ))
            } else {
                Err(format!(
                    "PR read failed: {error}{cleanup}; no complete result was returned"
                ))
            }
        }
        _ => Err(format!(
            "internal PR operation/result mismatch{}",
            cleanup.as_deref().map_or_else(String::new, |error| format!(
                "; node shutdown also failed: {error}"
            ))
        )),
    }
}
...
fn read_body_file(path: &Path) -> Result<String, String> {
    ...
    let metadata =
        fs::symlink_metadata(path).map_err(|error| format!("PR body metadata: {error}"))?;
    if !metadata.is_file() || metadata.len() > MAX_BODY_BYTES as u64 {
        return Err(
            "PR body must be a regular file of at most 65536 bytes, not a symlink/device"
                .to_owned(),
        );
    }
    ...
    let opened = file
        .metadata()
        .map_err(|error| format!("PR body metadata: {error}"))?;
    ...
    bytes
        ...
        .map_err(|_| "PR body allocation refused")?;
