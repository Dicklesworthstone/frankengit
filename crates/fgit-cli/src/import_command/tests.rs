...
fn timeout_and_incarnation_work_in_either_order() {
    ...
    for extras in [
        vec!["--timeout-secs", "600", "--expected-incarnation", incarnation],
        vec!["--expected-incarnation", incarnation, "--timeout-secs", "600"],
    ] {
        let options = parse(&arguments(&extras)).unwrap();
        assert_eq!(options.timeout.unwrap().duration(), Duration::from_secs(600));
        assert_eq!(options.incarnation, Some(RepositoryIncarnationId::from_bytes([0x44; 16])));
    }
}

#[test]
fn invalid_and_duplicate_budgets_never_reach_node_open() {
    for token in ["", "0", "-1", "+1", "1.0", " 1", "1 ", "18446744073709551616"] {
        assert!(parse(&arguments(&["--timeout-secs", token])).is_err(), "{token:?}");
    }
    for extras in [
        vec!["--timeout-secs"],
        ...
        vec!["--expected-incarnation", "44444444444444444444444444444444", "--expected-incarnation", "55555555555555555555555555555555"],
    ] {
...
fn malformed_principal_key_and_path_are_rejected_before_effects() {
    for (index, bad) in [(0, ""), (1, "bad"), (2, "bad"), (3, "bad"), (4, ""), (5, "")] {
