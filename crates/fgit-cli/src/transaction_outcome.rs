...
pub fn run(args: &[String]) -> Result<u8, String> {
    ...
    let known = match &report {
        RequestRecovery::Recovered(request) => {
            report.terminal().map(|outcome| (request.tx_id(), outcome))
        }
        _ => None,
    };
    ...
    if let Some(error) = cleanup {
        let knowledge = known.map_or_else(
            || "no terminal outcome was established".into(),
            |(tx, terminal)| describe(tx, &terminal),
        );
...
fn selected_key(bytes: Vec<u8>, index: Option<usize>) -> Result<IdempotencyKey, String> {
    ...
    match index {
        None => Ok(original),
        Some(index) => OneNode::receive_command_recovery_key(&original, index)
...
fn emit(output: &mut impl Write, text: &str) -> Result<(), String> {
    writeln!(output, "{text}")
        ...
        .map_err(|error| {
            format!(
                "transaction outcome output incomplete: {error}; absence is not proof of non-commit"
            )
        })
}
fn read_key(input: &mut impl Read) -> Result<Vec<u8>, String> {
    ...
    if bytes.len() > MAX_IDEMPOTENCY_KEY_BYTES {
        return Err("original key exceeds 256 bytes".into());
    }
    Ok(bytes)
}
fn parse(args: &[String]) -> Result<Options, String> {
    ...
    while index < args.len() {
        ...
        if !seen.insert(group) {
            return Err(format!("duplicate {group}"));
        }
        ...
        match flag {
            ...
            "--receive-command-index" => {
                ...
                if selected >= 64 {
                    return Err("receive command index must be 0..63".into());
                }
    ...
    if !trusted {
        return Err("--trusted-local is required; a key is not a credential".into());
    }
    Ok(Options {
...
fn digest(value: Digest) -> String {
    format!(
        "{{\"algorithm\":{},\"hex\":{}}}",
        value.algorithm().code_point(),
        quote(&hex(value.bytes().as_bytes()))
    )
}
fn render(
    options: &Options,
        ...
        RequestRecovery::Recovered(request) => match request.outcome() {
            ...
            OutcomeLookup::Decided(terminal) => match terminal.outcome {
                DecisionOutcome::Committed { .. } => {
                    ("committed", 0, Some(request.as_ref()), Some(terminal))
                }
                DecisionOutcome::Refused { .. } => {
                    ("refused", 3, Some(request.as_ref()), Some(terminal))
                }
            },
        },
    };
    ...
    (
        format!(
            ...
            digest(key_digest),
            quote(if options.receive_command_index.is_some() {
                "receive_command"
            } else {
                "transaction"
            }),
...
mod tests {
    ...
    fn recovery_inputs_are_explicit_scoped_and_never_accept_mutation_arguments() {
        ...
        for option in [
            ...
            bad.extend([option.into(), "irrelevant".into()]);
        ...
        for extra in [
            vec!["--key-stdin"],
            vec!["--idempotency-key", "same"],
            vec!["--idempotency-key-hex", "00"],
            vec!["--principal", "bad"],
        ] {
            let mut bad = valid.clone();
        ...
        let mut bad = valid;
        *bad.last_mut().unwrap() = "x".repeat(257);
        assert!(parse(&bad).is_err());
    }
    #[test]
    fn stdin_and_hex_preserve_exact_bytes_including_empty_keys_and_newlines() {
        for bytes in [Vec::new(), b"key\0\n\xff".to_vec(), vec![b'x'; 256]] {
            ...
            assert_eq!(
                read_key(&mut std::io::Cursor::new(bytes.clone())).unwrap(),
                bytes
            );
        }
        assert!(read_key(&mut std::io::Cursor::new(vec![0; 257])).is_err());
        for text in ["0", "GG", "FF", "é"] {
            assert!(unhex_key(text).is_err());
        }
        let mut input = args();
        input.truncate(6);
        input.push("--key-stdin".into());
    ...
    fn receive_selector_is_bounded_and_uses_the_original_binary_key_exactly() {
        for index in [0, 1, 63] {
            ...
            assert_eq!(
                selected_key(original, Some(index)).unwrap(),
        ...
        assert_eq!(
            selected_key(b"atomic-key".to_vec(), None).unwrap(),
    ...
    fn nonterminal_reports_do_not_expose_raw_keys_or_assert_noncommit() {
        ...
        let key = IdempotencyKey::new(b"original-key".to_vec()).unwrap();
        for report in [
            ...
            assert!(failed.contains("\"node_closed\":false"));
    ...
    fn receive_receipts_identify_only_the_selected_observation_not_a_complete_session() {
        ...
        let selected =
            selected_key(b"original-key".to_vec(), options.receive_command_index).unwrap();
