...
pub(super) fn run(arguments: &[String]) -> Result<u8, String> {
    ...
    let operation = (|| {
        ...
        node.runtime()
            .block_on(node.search_source_local_in(
                &request,
                &options.reference,
                &options.query,
                options.limits,
    ...
    Ok(match report.completion {
        SearchCompletion::Complete => 0,
        SearchCompletion::MatchLimit => 3,
    })
}
...
fn parse(arguments: &[String]) -> Result<Options, String> {
    ...
    if arguments[0].is_empty() {
        return Err("storage root must be nonempty".to_owned());
    }
    ...
    let (mut needle, mut format, mut matches, mut bytes, mut file_bytes, mut files) =
    ...
    while cursor < arguments.len() {
        ...
        match flag {
            ...
            "--ignore-ascii-case" => {
                if insensitive {
                    return Err("duplicate --ignore-ascii-case".to_owned());
                }
                insensitive = true;
                continue;
            }
        ...
        match flag {
            ...
            "--path" | "--path-hex" => {
                if paths.len() == 128 {
                    return Err("at most 128 path prefixes are supported".to_owned());
                }
            ...
            _ => {
                return Err(format!(
                    "unknown source search option {flag}; regex and implicit decoding are unsupported"
                ));
            }
        }
    }
    ...
    (0..text.len())
        ...
        .map(|i| {
            u8::from_str_radix(&text[i..i + 2], 16)
                .map_err(|_| "invalid hexadecimal byte".to_owned())
        })
        .collect()
}
...
fn render(
    ...
    for (i, hit) in report.matches.iter().enumerate() {
        ...
        if out.len() > 48 * 1024 * 1024 {
            return Err(
                "source search response exceeds the output ceiling; narrow the query".to_owned(),
            );
        }
    }
    out.push_str("]}");
...
fn write_report(out: &mut impl Write, report: &str) -> Result<(), String> {
    writeln!(out, "{report}")
        .and_then(|()| out.flush())
        .map_err(|error| format!("source search report output failed: {error}"))
}
...
mod tests {
    ...
    fn byte_queries_formats_and_prefixes_are_explicit() {
        ...
        assert_eq!(o.format, GitHashAlgorithm::Sha256);
        assert_eq!(o.query.case(), SearchCase::AsciiInsensitive);
        assert_eq!(o.query.prefixes()[0].as_bytes(), b"src");
        let mut b = args();
        b[5] = "--literal-hex".into();
        b[6] = "00ff".into();
    ...
    fn missing_trust_duplicates_unknowns_and_invalid_limits_refuse_before_io() {
        ...
        for pair in [
            ...
            let mut a = args();
            a.extend(pair.map(str::to_owned));
            assert!(parse(&a).is_err(), "{pair:?}");
        }
        let mut a = args();
        a.extend(["--max-matches", "1", "--max-matches", "2"].map(str::to_owned));
        assert!(parse(&a).is_err());
    }
    #[test]
