...
use fgit_types::{
    ...
    RepositoryAuthorityHeadId, RepositoryId, RepositoryIncarnationId, TenantId,
};
...
pub(super) fn run(args: &[String]) -> Result<u8, String> {
    ...
    {
        let _ = options;
        Err("trusted workflow execution requires Linux; no alternative runner is selected".into())
    }
}
...
fn parse(args: &[String]) -> Result<Options, String> {
    ...
    while cursor < args.len() {
        ...
        if matches!(flag, "--input" | "--input-hex") {
            ...
            if bytes.contains(&b'/') || inputs.len() >= 1024 || !inputs.insert(bytes) {
                return Err(
                    "inputs must be distinct top-level file/directory names, at most 1024".into(),
                );
            }
    ...
    if !flags.contains_key("--trusted-local") {
        return Err("--trusted-local is mandatory: scripts have host-user privileges".into());
    }
    let value = |flag: &str| {
        flags
            .get(flag)
    ...
    if inputs.is_empty()
        || inputs.iter().map(Vec::len).sum::<usize>() > 64 * 1024
        || !inputs.iter().any(|prefix| {
            workflow == *prefix
                || workflow
    ...
    let run_id: [u8; 16] = unhex(value("--run-id")?, 16)?
        .try_into()
        .map_err(|_| "run ID must have 32 hex digits")?;
    if run_id == [0; 16] {
        return Err("run ID cannot be zero".into());
    }
    let parent = PathBuf::from(value("--run-parent")?);
    if !parent.is_absolute() {
        return Err("run parent must be an absolute private directory".into());
    }
    let head = flags
        .get("--expected-head")
    ...
    let commit = flags
        .get("--expected-commit")
        .map(|text| native_oid(text, format))
        .transpose()?;
    ...
    let candidate = if candidate_mode {
        ...
        if bundle.is_empty() || bundle.len() > 4096 {
            return Err("bundle must be a bounded local file path".into());
        }
        if candidate == base
            || !reference.as_bytes().starts_with(b"refs/heads/")
            || reference.as_bytes().len() > 4096
        {
            return Err(
                "candidate requires a bounded branch and must differ from the canonical base"
                    .into(),
            );
        }
        Some(Candidate {
...
fn native_oid(text: &str, format: GitHashAlgorithm) -> Result<GitOid, String> {
    if unhex(text, format.digest_len())?.len() != format.digest_len() {
        return Err("invalid expected commit width".into());
    }
    let oid = GitOid::from_hex(format, text).map_err(|_| "invalid expected commit")?;
    if oid.is_zero() {
        return Err("expected commit cannot be zero".into());
    }
    Ok(oid)
}
...
fn parse_head(text: &str) -> Result<RepositoryAuthorityHeadId, String> {
    ...
    if algorithm.is_empty()
        || !algorithm.bytes().all(|b| b.is_ascii_digit())
        || (algorithm.len() > 1 && algorithm.starts_with('0'))
    {
        return Err("invalid head algorithm".into());
    }
    let algorithm =
        DigestAlgorithmId::try_new(algorithm.parse().map_err(|_| "head algorithm overflow")?)
...
fn execute(options: Options, merge: Option<fgit_forge::event::NativeMerge>) -> Result<u8, String> {
    ...
    let bundle = options
        ...
        .as_ref()
        .map(|candidate| {
            super::publication_support::read_bundle(&candidate.bundle, 128 * 1024 * 1024)
        })
        .transpose()?;
    ...
    let operation = (|| -> Result<_, String> {
        if options
            .incarnation
            .is_some_and(|expected| expected != node.repository_incarnation_id())
        {
        ...
        let result = match (merge.as_ref(), &options.candidate, bundle.as_deref()) {
            ...
            (None, Some(candidate), Some(bytes)) => {
                node.runtime()
                    .block_on(node.run_trusted_candidate_workflow_in(
                        &request,
                        &options.reference,
            ...
            (None, None, None) => node.runtime().block_on(node.run_trusted_workflow_in(
                &request,
                &options.reference,
                &options.workflow,
                options.run_id,
    ...
    match operation {
        Ok((report, succeeded, directory)) => finish(
        ...
        .map_err(|e| {
            format!(
                "{e}; completed report remains at {}; do not replay the attempt",
                directory.join("report.json").display()
            )
        }),
        Err(error) => Err(format!(
            "{error}{}; inspect any existing attempt directory before retrying; an error is not proof that no command ran",
...
mod tests {
    ...
    fn explicit_trust_exact_selection_and_byte_paths_parse_before_io() {
        ...
        assert_eq!(options.inputs, [b"ci".to_vec(), b"src".to_vec()]);
        let mut raw = args();
        raw[6] = "--workflow-hex".into();
        raw[7] = "63692fff".into();
        raw[4] = "726566732f68656164732fff".into();
        raw.push("--ref-hex".into());
        raw.extend([
            "--expected-head".into(),
            format!("alg:1:{}", "ab".repeat(32)),
        ]);
        let options = parse(&raw).unwrap();
        assert_eq!(options.workflow, b"ci/\xff");
        assert_eq!(options.reference.as_bytes(), b"refs/heads/\xff");
        assert!(options.head.is_some());
    }
    #[test]
    fn ambiguous_paths_flags_and_replay_identifiers_refuse() {
        for extra in [
            ...
            vec!["--input", "../secret"],
            vec!["--input", "src/deep"],
            vec!["--workflow-hex", "6162"],
            vec!["--force"],
            vec!["--object-format", "sha512"],
            vec!["--expected-commit", "abc"],
        ] {
            let mut input = args();
        ...
        assert!(parse(&input).is_err());
        let mut input = args();
        input[13] = "00".repeat(16);
        assert!(parse(&input).is_err());
        let mut input = args();
        input[7] = "outside/run.yml".into();
        assert!(parse(&input).is_err());
        let mut input = args();
        input[15] = "relative".into();
        assert!(parse(&input).is_err());
    }
    #[test]
    fn known_execution_survives_cleanup_failure_and_broken_output_never_succeeds() {
        let mut out = Vec::new();
        assert_eq!(
            finish(&mut out, "{}", true, Some("close\nfailed")).unwrap(),
            2
        );
        ...
        let mut out = Vec::new();
        assert_eq!(finish(&mut out, "{}", false, None).unwrap(), 1);
        struct Broken;
    ...
    fn candidate_command_requires_independent_base_candidate_and_bundle_before_io() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            let mut input = args();
            input[0] = "run-candidate".into();
            let base = "a".repeat(format.digest_len() * 2);
            let candidate = "b".repeat(format.digest_len() * 2);
            input.extend([
                "--object-format".into(),
                format.as_str().into(),
                ...
                candidate.clone(),
                "--bundle".into(),
                "not-opened-at-parse.bundle".into(),
            ]);
            let options = parse(&input).unwrap();
            let selected = options.candidate.unwrap();
            assert_eq!(selected.base.to_string(), base);
            assert_eq!(selected.commit.to_string(), candidate);
            ...
            canonical[0] = "run".into();
            assert!(parse(&canonical).is_err());
            let mut untrusted = input.clone();
            untrusted.remove(5);
            assert!(parse(&untrusted).is_err());
            let mut same = input.clone();
            let at = same.iter().position(|s| s == "--candidate-commit").unwrap();
            same[at + 1] = base;
