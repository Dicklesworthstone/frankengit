...
pub(super) fn parse(args: &[String]) -> Result<Options, String> {
    ...
    if !matches!(action, "create" | "annotate" | "delete" | "list" | "show") || args[1].is_empty() {
        return Err(super::USAGE.into());
    }
        ...
        let allowed = common
            || match action {
                ...
                _ => {
                    matches!(
                        flag,
                        "--ref" | "--ref-hex" | "--principal" | "--idempotency-key" | "--key-stdin"
                    ) || (matches!(action, "create" | "annotate") && flag == "--target")
                        || (action == "delete" && flag == "--expected-tip")
                        || (action == "annotate"
                            && matches!(
                                flag,
                                "--target-kind" | "--tagger" | "--timestamp" | "--message-file"
                            ))
                }
            };
        if !allowed {
            return Err(format!("unsupported tag option {flag}"));
        }
    ...
    let operation = match action {
        "list" => {
            let after = if flags.contains_key("--after") || flags.contains_key("--after-hex") {
                Some(reference(&flags, "--after", "--after-hex")?)
            } else {
                None
            };
            if after.is_some() && head.is_none() {
                return Err("tag continuation requires --expected-head".into());
            }
            let limit = flags
                .get("--limit")
                .map(|v| decimal(v))
            ...
            if !(1..=100).contains(&limit) {
                return Err("tag page limit must be 1..100".into());
            }
            Operation::List {
                after,
                limit: limit as u16,
        ...
        "show" => Operation::Show {
            reference: reference(&flags, "--ref", "--ref-hex")?,
            head,
        },
        _ => {
            ...
            let key = match (
                ...
                (Some(value), false) if !value.is_empty() && value.len() <= 256 => {
                    Key::Bytes(value.as_bytes().to_vec())
                }
                (None, true) => Key::Stdin,
                _ => return Err("supply exactly one 1..256-byte key or --key-stdin".into()),
            };
            let target_flag = if action == "delete" {
                "--expected-tip"
            } else {
                "--target"
            };
            let target = GitOid::from_hex(format, required(&flags, target_flag)?)
                .map_err(|_| "invalid target object ID for selected format")?;
            if target.is_zero() {
                return Err("zero target is not an object ID".into());
            }
            let mut message_file = None;
            let command = match action {
                "create" => TagCommand::Lightweight { name, target },
                "delete" => TagCommand::Delete {
                    name,
                    expected: target,
                },
                _ => {
                    let target_kind = match required(&flags, "--target-kind")? {
                        ...
                        _ => return Err("target kind must be commit, tree, blob, or tag".into()),
                    };
                    let path = required(&flags, "--message-file")?;
                    if path.is_empty() {
                        return Err("message path must be nonempty".into());
                    }
                    ...
                    TagCommand::Annotated {
                        ...
                        metadata: TagMetadata {
                            tagger: required(&flags, "--tagger")?.into(),
                            timestamp: decimal(required(&flags, "--timestamp")?)?,
                            message: Vec::new(),
                        },
                    }
                }
...
fn reference(flags: &BTreeMap<&str, &str>, plain: &str, encoded: &str) -> Result<RefName, String> {
    ...
    validate_tag_name(&reference).map_err(|e| e.to_string())?;
    Ok(reference)
}
...
fn unhex(value: &str, maximum: usize) -> Result<Vec<u8>, String> {
    ...
    {
        return Err("invalid bounded lowercase hexadecimal bytes".into());
    }
    value
        .as_bytes()
        .chunks_exact(2)
...
fn decimal(value: &str) -> Result<u64, String> {
    if value.is_empty() || !value.bytes().all(|b| b.is_ascii_digit()) {
        return Err("unsigned decimal integer required".into());
    }
