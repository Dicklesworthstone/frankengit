...
use fgit_types::{
    DecisionOutcome, GitOid, HeadGeneration, PrincipalId, RefName, RepositoryId, TenantId, TxId,
};
...
pub fn run(arguments: &[String]) -> Result<(), String> {
    ...
    let operation = (|| {
        ...
        match result.commands.as_slice() {
            ...
            _ => Err(
                "publication returned an unexpected outcome mapping; do not infer non-commit"
                    .to_owned(),
            ),
        }
    })();
    ...
    let (tx_id, terminal) = match operation {
        ...
        Err(error) => {
            ...
            return Err(format!(
                "no terminal outcome returned: {error}{cleanup}; this is not evidence of non-commit. Reconcile by retrying the identical bundle, expectations, principal and idempotency key; do not rerun the tool or change the key to recover this attempt"
            ));
        }
    };
...
fn parse(arguments: &[String]) -> Result<Options, String> {
    ...
    while cursor < arguments.len() {
        ...
        match flag {
            ...
            _ => {
                return Err(format!(
                    "unknown workspace apply option {flag}; no force option is supported"
                ));
            }
        }
    }
    ...
    let candidate =
        candidate.ok_or("--expected-commit is required and must name the reviewed commit")?;
...
fn render_receipt(
    ...
    format!(
        "{{\"type\":\"workspace_publication\",\"outcome\":\"{status}\",\"published_to_repository\":{published},\"tx_id\":{},\"decision_sequence\":{},\"repository_commit_id\":{rcr},\"refusal_code\":{code},\"refusal_record_id\":{refusal},\"expected_base\":\"{}\",\"candidate_commit\":\"{}\",\"reference_hex\":\"{reference}\",\"node_closed\":{},\"cleanup_error\":{cleanup_text}}}",
...
mod tests {
    ...
    fn known_decisions_survive_cleanup_and_receipt_output_failures() {
        ...
        let error =
            write_terminal_receipt(&mut BrokenOutput, &receipt, tx, &committed).unwrap_err();
