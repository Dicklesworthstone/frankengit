...
const USAGE: &str = "usage: fg workflow run-merge-candidate <storage-root> <tenant-id> <repository-id> <target-ref>
...
pub(super) fn run(args: &[String]) -> Result<u8, String> {
    ...
    { let _ = (options, merge); Err("trusted merge workflow execution requires Linux; no alternative runner is selected".into()) }
}

fn parse(args: &[String]) -> Result<(Options, NativeMerge), String> {
    if args.len() < 5 || args[0] != "run-merge-candidate" { return Err(USAGE.into()); }
    if args.len() > 2108 || args.iter().any(|s| s.len() > 8192)
        || args.iter().map(String::len).sum::<usize>() > 128 * 1024
    { return Err("merge workflow arguments exceed the bounded profile".into()); }
    let mut common = args[..5].to_vec(); common[0] = "run-candidate".into();
    let mut fields = BTreeMap::new(); let mut cursor = 5;
    while cursor < args.len() {
        let flag = args[cursor].as_str(); cursor += 1;
        if matches!(flag, "--trusted-local" | "--ref-hex") {
            common.push(flag.to_owned()); continue;
        }
        ...
        cursor += 1;
        if matches!(flag, "--source-ref" | "--source-ref-hex" | "--expected-source" | "--merge-base") {
            if fields.insert(flag, value.as_str()).is_some() { return Err(format!("duplicate {flag}")); }
        } else {
            common.push(flag.to_owned()); common.push(value.clone());
        }
    }
    ...
    if source_ref == options.reference { return Err("merge requires distinct source and target branches".into()); }
    let source_tip = native_oid(fields.get("--expected-source").ok_or("missing --expected-source")?, options.format)?;
    let base_tip = native_oid(fields.get("--merge-base").ok_or("missing --merge-base")?, options.format)?;
    let candidate = options.candidate.as_ref().ok_or("missing candidate inputs")?;
    let merge = NativeMerge { target_ref: options.reference.clone(), target_tip_before: candidate.base,
        source_ref, source_tip, base_tip, merge_commit: candidate.commit };
    merge.validate().map_err(|_| "invalid native merge coordinates")?;
    Ok((options, merge))
}
...
mod tests {
    ...
    fn args(format: GitHashAlgorithm) -> Vec<String> {
        vec!["run-merge-candidate".into(), "not-opened".into(), "11".repeat(16), "22".repeat(16), "refs/heads/main".into(),
            "--trusted-local".into(), "--workflow".into(), "ci/run.yml".into(), "--input".into(), "ci".into(),
            "--run-id".into(), "33".repeat(16), "--run-parent".into(), std::env::temp_dir().to_string_lossy().into_owned(),
            "--bundle".into(), "not-opened.bundle".into(), "--object-format".into(), format.as_str().into(),
            "--expected-commit".into(), "a".repeat(format.digest_len()*2), "--candidate-commit".into(), "b".repeat(format.digest_len()*2),
            "--source-ref".into(), "refs/heads/topic".into(), "--expected-source".into(), "c".repeat(format.digest_len()*2),
            "--merge-base".into(), "d".repeat(format.digest_len()*2)]
    }
    #[test]
    fn exact_merge_coordinates_and_byte_refs_parse_without_io_in_both_formats() {
        for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
            let input = args(format); let (options, merge) = parse(&input).unwrap();
            assert_eq!(merge.target_tip_before, options.candidate.as_ref().unwrap().base);
            assert_eq!(merge.merge_commit, options.candidate.as_ref().unwrap().commit);
            assert_ne!(merge.source_tip, merge.target_tip_before);
            let mut bytes = input.clone(); let at = bytes.iter().position(|s| s == "--source-ref").unwrap();
            bytes[at] = "--source-ref-hex".into(); bytes[at+1] = "726566732f68656164732fff".into();
            assert_eq!(parse(&bytes).unwrap().1.source_ref.as_bytes(), b"refs/heads/\xff");
        }
    }
    #[test]
    fn no_implicit_parents_trust_or_force_and_no_merge_flags_in_other_modes() {
        let good = args(GitHashAlgorithm::Sha1);
        for flag in ["--source-ref", "--expected-source", "--merge-base", "--expected-commit", "--candidate-commit", "--bundle"] {
            let mut missing = good.clone(); let at = missing.iter().position(|s| s == flag).unwrap();
            missing.drain(at..at+2); assert!(parse(&missing).is_err());
        }
        for extras in [vec!["--expected-source", "abcd"], vec!["--force", "true"],
            vec!["--source-ref-hex", "726566732f68656164732fff"], vec!["--principal", "admin"]] {
            let mut input = good.clone(); input.extend(extras.into_iter().map(str::to_owned)); assert!(parse(&input).is_err());
        }
        let mut untrusted = good.clone(); untrusted.remove(5); assert!(parse(&untrusted).is_err());
        let mut same = good.clone(); let at = same.iter().position(|s| s == "--source-ref").unwrap();
        same[at+1] = "refs/heads/main".into(); assert!(parse(&same).is_err());
        for mode in ["run", "run-candidate"] {
            let mut input = good.clone(); input[0] = mode.into(); assert!(super::super::parse(&input).is_err());
        }
    }
    #[test]
    fn merge_flag_spelling_in_an_argument_value_does_not_change_selection() {
        let mut input = args(GitHashAlgorithm::Sha1);
        let at = input.iter().position(|s| s == "--bundle").unwrap(); input[at+1] = "--source-ref".into();
        let (options, merge) = parse(&input).unwrap();
        assert_eq!(options.candidate.unwrap().bundle, PathBuf::from("--source-ref"));
        assert_eq!(merge.source_ref.as_bytes(), b"refs/heads/topic");
        let at = input.iter().position(|s| s == "--expected-source").unwrap();
        input[at+1] = "a".repeat(64); assert!(parse(&input).is_err());
    }
}
