...
pub(super) fn publish(path: &Path, bytes: &[u8], live: &dyn Fn() -> bool) -> Result<(), String> {
    if bytes.is_empty() || bytes.len() > MAX_BYTES {
        return Err("recovered artifact exceeds the export envelope".into());
    }
    if !live() {
        return Err("recovery export cancelled before staging".into());
    }
    ...
    let metadata =
        fs::symlink_metadata(parent).map_err(|e| format!("inspect recovery output parent: {e}"))?;
    ...
    let prepared = (|| -> Result<(), String> {
        if !live() {
            return Err("recovery export cancelled".into());
        }
        file.write_all(bytes)
            .and_then(|()| file.sync_all())
            .map_err(|e| format!("write/sync recovery stage: {e}"))?;
        file.seek(SeekFrom::Start(0)).map_err(|e| e.to_string())?;
        // Compare in fixed chunks; do not allocate a second evidence-sized copy.
        let mut buffer = [0; 16 * 1024];
        for expected in bytes.chunks(buffer.len()) {
            if !live() {
                return Err("recovery export cancelled during readback".into());
            }
            file.read_exact(&mut buffer[..expected.len()])
                .map_err(|e| format!("read recovery stage: {e}"))?;
            if &buffer[..expected.len()] != expected {
                return Err("recovery stage readback mismatch".into());
            }
        }
        let mut trailing = [0];
        if file.read(&mut trailing).map_err(|e| e.to_string())? != 0 {
            return Err("recovery stage contains trailing bytes".into());
        }
        ...
        if !live() {
            return Err("recovery export cancelled before publication".into());
        }
        // hard_link is no-replace even if a competing destination appeared.
        fs::hard_link(&stage, path)
            .map_err(|e| format!("publish recovery output without replacement: {e}"))?;
        Ok(())
    })();
    if let Err(error) = prepared {
        return Err(format!(
            "{error}; private staging file retained at {}",
            stage.display()
        ));
    }
    let published_error = |e| {
        format!(
            "recovery output is published at {}, but finalization failed: {e}; inspect output and staging at {}",
            path.display(),
            stage.display()
        )
    };
