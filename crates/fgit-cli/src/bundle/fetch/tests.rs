...
fn exact_maps_preserve_raw_names_and_resolve_format_independently_of_flag_order() {
    ...
    assert_eq!(
        options.mappings[0].destination.as_bytes(),
        b"refs/heads/local"
    );
...
fn malformed_ambiguous_or_implicit_updates_refuse_before_node_open() {
    ...
    for bad in [
        vec!["--map", "refs/heads/main", "refs/meta/hidden", "absent"],
        vec!["--map", "refs/heads/*", "refs/heads/a", "absent"],
        vec!["--map", "refs/heads/a", "refs/heads/b"],
        vec!["--map-hex", "0g", "aa", "absent"],
        vec!["--map-hex", "6", "aa", "absent"],
        vec!["--map", "refs/heads/a", "refs/heads/b", "latest"],
    ] {
        assert!(parse(&args(&bad)).is_err());
    }
    let zero = "0".repeat(40);
    let mixed = "1".repeat(64);
    for old in [&zero, &mixed] {
        assert!(parse(&args(&["--map", "refs/heads/a", "refs/heads/b", old])).is_err());
    }
    let mut duplicate = args(&valid);
    duplicate.extend(valid.map(String::from));
    assert!(parse(&duplicate).is_err());
    for flag in ["--force", "--prune", "--key-stdin", "--trusted-local"] {
        let mut options = args(&valid);
        options.push(flag.into());
        assert!(parse(&options).is_err());
    }
    let mut untrusted = args(&valid);
    untrusted.remove(4);
    assert!(parse(&untrusted).is_err());
    let mut no_principal = args(&valid);
    no_principal.drain(5..7);
    assert!(parse(&no_principal).is_err());
    let mut no_key = args(&valid);
    no_key.remove(7);
    assert!(parse(&no_key).is_err());
    let mut two_keys = args(&valid);
    two_keys.extend(["--idempotency-key".into(), "private".into()]);
    assert!(parse(&two_keys).is_err());
    let mut huge = args(&valid);
    huge[0] = "x".repeat(65537);
...
fn fetch_receipts_preserve_terminal_outcomes_and_never_include_key_bytes() {
    ...
    inputs[7] = "--idempotency-key".into();
    inputs.insert(8, "secret-private-exact-key".into());
    ...
    for refused in [false, true] {
        ...
        assert_eq!(
            finish(&mut bytes, &options, tx, &terminal, None).unwrap(),
            if refused { 3 } else { 0 }
        );
        let text = String::from_utf8(bytes).unwrap();
        assert!(text.contains("\"type\":\"git_bundle_fetch\""));
        assert!(text.contains("\"reference_count\":1"));
        assert!(text.contains(&format!("\"command_committed\":{}", !refused)));
        assert!(!text.contains("secret-private-exact-key"));
        assert!(text.contains(&hex(b"refs/remotes/origin/topic")));
        assert!(text.contains("\"expected_old\":null"));
        let mut bytes = Vec::new();
        assert!(
            finish(&mut bytes, &options, tx, &terminal, Some("close failed"))
                .unwrap_err()
