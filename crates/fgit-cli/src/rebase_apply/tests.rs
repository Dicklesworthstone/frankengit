...
fn terminal(refused: bool) -> (TxId, TerminalOutcome) {
    ...
    let outcome = if refused {
        DecisionOutcome::Refused {
            code: fgit_types::RefusalCode::ExpectedOldRefMismatch,
...
fn refs_preserve_bytes_and_key_intake_is_exact_and_bounded() {
    ...
    args.push("--source-ref-hex".into());
    assert_eq!(
        options::parse(&args).unwrap().reference.as_bytes(),
        b"refs/heads/\xff"
    );
    args[3] = hex(b"refs/tags/not-a-branch");
    ...
    let mut args = arguments(40);
    drop(args.splice(9..11, ["--key-stdin".into()]));
    assert!(matches!(
        options::parse(&args).unwrap().key,
        KeyInput::Stdin
    ));
}
...
fn receipts_preserve_committed_and_refused_outcomes_after_cleanup_failure() {
    for refused in [false, true] {
        ...
        assert_eq!(
            finish(&mut output, &options, tx, &terminal, None).unwrap(),
            if refused { 3 } else { 0 }
        );
        ...
        let mut output = Vec::new();
        let error =
            finish(&mut output, &options, tx, &terminal, Some("close\nfailed")).unwrap_err();
