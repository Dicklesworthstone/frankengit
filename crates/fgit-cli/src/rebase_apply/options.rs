...
pub(super) fn parse(args: &[String]) -> Result<Options, String> {
    ...
    while at < args.len() {
        ...
        let switch = matches!(
            flag,
            "--trusted-local" | "--allow-rewrite" | "--source-ref-hex" | "--key-stdin"
        );
        if !switch
            && !matches!(
                flag,
    ...
    if !flags.contains_key("--allow-rewrite") {
        return Err("--allow-rewrite is required to publish a rebase".into());
    }
    ...
    let bytes = if flags.contains_key("--source-ref-hex") {
        unhex(&args[3], 4096)?
    } else if args[3].len() <= 4096 {
        args[3].as_bytes().to_vec()
    } else {
        return Err("source reference exceeds its byte limit".into());
    };
    if !bytes.starts_with(b"refs/heads/") {
        return Err("source must be a full refs/heads/ reference".into());
    }
    ...
    let key = match (
        ...
        (Some(text), false) if !text.is_empty() && text.len() <= MAX_IDEMPOTENCY_KEY_BYTES => {
            KeyInput::Bytes(text.as_bytes().to_vec())
        }
        (None, true) => KeyInput::Stdin,
        _ => {
            return Err(
                "supply exactly one bounded nonempty --idempotency-key or --key-stdin".into(),
            );
        }
    };
    Ok(Options {
