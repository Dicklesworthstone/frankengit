...
use fgit_types::{
    CANONICAL_CODEC_VERSION, DecisionSequence, GitHashAlgorithm, RefusalCode, RefusalRecordId,
    RepositoryAuthorityHeadId, RepositoryCommitId,
};
...
fn row<'a>(event: &'a ForgeEvent, mutation: &'a Mutation) -> Row<'a> {
    ...
    Row {
        number,
        event,
        data: Some(&mutation.command.data),
        opened_by: Some(mutation.principal),
        last_metadata_actor: Some(mutation.principal),
    }
}
...
fn complete_mutations_preserve_bytes_versions_and_both_hash_domains() {
    for (width, format) in [(40, GitHashAlgorithm::Sha1), (64, GitHashAlgorithm::Sha256)] {
        ...
        for action in ["update", "close"] {
            let mut input = args(width);
            input[0] = action.into();
            replace(&mut input, "--expected-version", "17");
            let parsed = parse(&input).unwrap();
            assert_eq!(
                mutation(&parsed).command.expected_version,
...
fn every_semantic_mutation_field_and_local_trust_are_required() {
    ...
    for action in ["open", "update", "close", "list", "show"] {
        let mut missing = if matches!(action, "list" | "show") {
            read_args(action)
        } else {
            let mut value = args(40);
            value[0] = action.into();
            value
        };
        missing.retain(|value| value != "--trusted-local");
...
fn unknown_duplicates_irrelevant_options_and_conflicting_encodings_refuse() {
    ...
    for at in (6..original.len()).step_by(2) {
        ...
        duplicate.extend_from_slice(&original[at..at + 2]);
    ...
    for extra in [
        ...
        vec!["--force", "true"],
        vec!["--limit", "2"],
        vec!["--body-file", "ignored"],
        vec!["--source-ref-hex", "61"],
    ] {
    ...
    for action in ["list", "show"] {
        let mut input = read_args(action);
        input.extend(["--principal".into(), "33".repeat(16)]);
        assert!(parse(&input).is_err());
    }
    let mut show = read_args("show");
    show.extend(["--limit", "1"].map(str::to_owned));
    assert!(parse(&show).is_err());
    let mut binary = args(40);
    let at = binary.iter().position(|arg| arg == "--source-ref").unwrap();
    binary[at] = "--source-ref-hex".into();
    binary[at + 1] = options::hex(b"refs/heads/topic");
    assert_eq!(
        mutation(&parse(&binary).unwrap())
            .command
            .data
            .source_ref
            .as_bytes(),
...
fn numeric_shape_metadata_limits_and_object_formats_fail_before_io() {
    ...
    let mut exhausted = args(40);
    exhausted[0] = "update".into();
    replace(&mut exhausted, "--expected-version", &u64::MAX.to_string());
    assert!(parse(&exhausted).is_err());
    let mut mixed = args(40);
    replace(&mut mixed, "--expected-target", &"b".repeat(64));
    assert!(parse(&mixed).is_err());
    let mut mismatched = args(64);
    mismatched.extend(["--object-format", "sha1"].map(str::to_owned));
    assert!(parse(&mismatched).is_err());
    let mut body = args(40);
    replace(&mut body, "--body", &"x".repeat(MAX_BODY_BYTES));
    assert!(parse(&body).is_ok());
    replace(&mut body, "--body", &"x".repeat(MAX_BODY_BYTES + 1));
    assert!(parse(&body).is_err());
    let mut title = args(40);
    replace(&mut title, "--title", &"é".repeat(129));
    assert!(parse(&title).is_err());
    let mut nonexistent = args(40);
    let at = nonexistent.iter().position(|arg| arg == "--body").unwrap();
    nonexistent[at] = "--body-file".into();
    nonexistent[at + 1] = "/does-not-exist/body".into();
    assert!(
        parse(&nonexistent).is_ok(),
        "syntax parsing must not perform file I/O"
    );
}
...
fn head_tokens_roundtrip_and_list_continuations_cannot_mix_snapshots() {
    for seed in 1..4 {
        assert_eq!(parse_head(&head_token(head(seed))).unwrap(), head(seed));
    }
    for invalid in [
        "",
        "abcd",
    ...
    let mut input = read_args("list");
    input.extend(["--after", "2"].map(str::to_owned));
    assert!(parse(&input).is_err());
    input.extend([
        "--expected-head".into(),
        head_token(head(1)),
    ...
    let parsed = parse(&input).unwrap();
    let read = read_options(&parsed);
    assert_eq!(read.after(), 2);
    assert_eq!(read.limit(), 2);
    assert_eq!(read.expected_head, Some(head(1)));
...
fn exact_show_does_not_disclose_a_different_next_number() {
    let original = parse(&args(40)).unwrap();
    let value = mutation(&original);
    let mut event = value
        .command
        .proposed_event(value.principal, original.format)
        .unwrap();
    event.aggregate = AggregateId::PullRequest(PullRequestNumber::try_new(8).unwrap());
    let options = parse(&read_args("show")).unwrap();
    let (text, exit) = read_receipt(
        &options,
    ...
    .unwrap();
    assert_eq!(exit, 4);
    assert!(text.contains("\"found\":false,\"pull_request\":null"));
    assert!(!text.contains("A reviewed change"));
    assert!(!text.contains("next_after"));
...
fn page_order_cursor_and_row_binding_are_checked_before_disclosure() {
    ...
    assert!(
        read_receipt(
            ...
            head(1),
            None,
            &[row(&event, value), row(&event, value)]
        )
        .is_err()
    );
    assert!(read_receipt(&options, read, head(1), Some(999), &[row(&event, value)]).is_err());
    let mut invalid = row(&event, value);
    invalid.number = PullRequestNumber::FIRST;
...
fn merge_only_receipts_do_not_invent_an_opener_or_metadata() {
    ...
    let event = ForgeEvent {
        aggregate: AggregateId::PullRequest(value.command.number),
        ...
        payload: ForgeEventPayload::MergeCommittedNative(NativeMerge {
            ...
            base_tip: data.target_tip,
            target_ref: data.target_ref.clone(),
            target_tip_before: data.target_tip,
            merge_commit: crate::publication_support::parse_oid(&"c".repeat(40)).unwrap(),
        }),
    };
    let row = Row {
        number: value.command.number,
        event: &event,
        data: None,
        opened_by: None,
        last_metadata_actor: None,
    };
    let options = parse(&read_args("list")).unwrap();
...
fn receipt_text_is_control_safe_and_private_key_is_not_echoed() {
    ...
    assert!(text.contains("\"command_committed\":true"));
    assert!(text.contains("\"refs_changed\":false"));
}
...
fn write_flush_and_cleanup_failures_keep_the_known_terminal_decision() {
    ...
    for committed in [true, false] {
        ...
        for fail_write in [true, false] {
            ...
            .unwrap_err();
            assert!(error.contains(&tx.to_string()));
            assert!(error.contains("cleanup-marker"));
...
fn body_files_allow_empty_data_but_refuse_bad_encoding_size_and_symlinks() {
    ...
    for body in [b"".as_slice(), b"<script>\n\r\t", "é\n".as_bytes()] {
        ...
        assert_eq!(read_body_file(&file).unwrap().as_bytes(), body);
    }
    for bad in [vec![0], vec![0xff], vec![b'x'; MAX_BODY_BYTES + 1]] {
        fs::write(&file, bad).unwrap();
        assert!(read_body_file(&file).is_err());
    }
