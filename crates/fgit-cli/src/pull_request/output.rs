...
fn data_json(data: &PullRequestData) -> String {
    format!(
        "{{\"source_reference_hex\":{},\"target_reference_hex\":{},\"source_tip\":{},\"target_tip\":{},\"title\":{},\"body\":{}}}",
        quote(&hex(data.source_ref.as_bytes())),
        quote(&hex(data.target_ref.as_bytes())),
        quote(&data.source_tip.to_string()),
        quote(&data.target_tip.to_string()),
        quote(&data.title),
        quote(&data.body)
    )
}
fn merge_json(merge: &NativeMerge) -> String {
    format!(
        "{{\"source_reference_hex\":{},\"target_reference_hex\":{},\"source_tip\":{},\"target_tip_before\":{},\"base_tip\":{},\"commit\":{}}}",
        quote(&hex(merge.source_ref.as_bytes())),
        quote(&hex(merge.target_ref.as_bytes())),
...
pub(super) fn mutation_receipt(
    ...
    let version = match mutation.command.expected_version {
        ExpectedVersion::NewStream => 0,
        ExpectedVersion::Exactly(value) => value.get(),
    };
    format!(
        concat!(
            "{{\"type\":\"pull_request_publication\",\"schema_version\":1,\"action\":{},",
        ...
        quote(action_name(mutation.command.action)),
        quote(outcome),
        committed,
        quote(&tx.to_string()),
        ...
        version,
        data_json(&mutation.command.data),
        cleanup.is_none(),
...
pub(super) fn read_receipt(
    ...
    let header = format!(
        concat!(
            "\"schema_version\":1,\"scope\":\"native_prs_and_merge_receipts\",",
        ...
        quote(&options.repository.to_string()),
        quote(options.format.as_str()),
        quote(&source_head.to_string()),
        quote(&head_token(source_head))
    );
    match read.selection {
        Selection::Show(number) => {
            ...
            Ok((
                format!(
                    "{{\"type\":\"pull_request\",{header},\"requested_number\":{},\"found\":{},\"pull_request\":{body}}}",
                    number.get(),
                    row.is_some()
                ),
        ...
        Selection::List { after, limit } => {
            let mut out = format!(
                "{{\"type\":\"pull_request_page\",{header},\"after\":{after},\"limit\":{limit},\"count\":{},\"has_more\":{},\"next_after\":{},\"pull_requests\":[",
                rows.len(),
                next_after.is_some(),
                next_after.map_or_else(|| "null".to_owned(), |value| value.to_string())
            );
            for (index, view) in rows.iter().enumerate() {
                ...
                if out.len().saturating_add(row.len()).saturating_add(3) > MAX_REPLY_BYTES {
                    return Err(
                        "PR page exceeds the output ceiling; request a smaller page".to_owned()
                    );
                }
                if index != 0 {
                    out.push(',');
...
fn render_view(view: &Row<'_>) -> Result<String, String> {
    ...
    if let Some(data) = view.data {
        data.validate().map_err(|_| "PR row metadata is invalid")?;
    }
    let (state, action, merge) = match &view.event.payload {
        ...
        ForgeEventPayload::MergeCommittedNative(merge) => {
            merge
                .validate()
                .map_err(|_| "PR merge coordinates are invalid")?;
        ...
        _ => {
            return Err(
                "unexpected non-native PR event; no lossy legacy conversion is supported"
                    .to_owned(),
            );
        }
    };
    Ok(format!(
        concat!(
            "{{\"number\":{},\"version\":{},\"kind\":{},\"state\":{},\"last_action\":{},",
            "\"data\":{},\"opened_by\":{},\"last_metadata_actor\":{},\"merge\":{}}}"
        ),
        view.number.get(),
        view.event.version.get(),
        quote(if view.data.is_some() {
            "pull_request"
        } else {
            "merge_receipt"
        }),
        quote(state),
        quote(action),
        view.data.map_or_else(|| "null".to_owned(), data_json),
        view.opened_by
            .map_or_else(|| "null".to_owned(), |actor| quote(&actor.to_string())),
        view.last_metadata_actor
            .map_or_else(|| "null".to_owned(), |actor| quote(&actor.to_string())),
        merge
    ))
}
