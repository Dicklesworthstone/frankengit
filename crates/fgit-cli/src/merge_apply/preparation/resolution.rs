...
use super::{
    HeadGeneration, NodeConfig, OneNode, Options, PreparationLimits, hex, parse,
    publish_new_bundle, quote, render_conflict, render_entry, require_absent, set_once,
};
use crate::publication_support::parse_oid;
use fgit_forge::preparation::resolution::{
    ConflictResolution, ResolutionChoice, ResolutionInputs, ResolvedMerge, validate_resolutions,
};
...
pub(super) fn run(arguments: &[String]) -> Result<(), String> {
    ...
    let mut node = OneNode::open_existing(
        NodeConfig::new(options.storage.clone(), options.tenant, options.repository)
            .with_object_format(input.inputs.target.algorithm()),
    )
    ...
    let artifact = match (operation, cleanup) {
        ...
        (Ok(_), Some(error)) => {
            return Err(format!(
                "node shutdown failed: {error}; no resolved bundle was published"
            ));
        }
        (Err(error), Some(cleanup)) => {
            return Err(format!(
                "{error}; node shutdown also failed: {cleanup}; no resolved bundle was published"
            ));
        }
    };
    ...
    emit(
        ...
        &report,
        Some((&options.output, artifact.resolved.plan.commit)),
    )
}
...
fn emit(
    ...
    report: &str,
    artifact: Option<(&Path, fgit_types::GitOid)>,
) -> Result<(), String> {
...
fn parse_input(arguments: &[String]) -> Result<Input, String> {
    if arguments.len() < 6 || arguments[0] != "resolve" {
        return Err(USAGE.into());
    }
    if arguments.len() > 600
        || arguments.iter().any(|arg| arg.len() > 64 * 1024)
        || arguments.iter().map(String::len).sum::<usize>() > 512 * 1024
    {
        return Err("resolution arguments exceed their bounded profile".into());
    }
    let mut normal = arguments[..6].to_vec();
    normal[0] = "prepare".into();
    ...
    while cursor < arguments.len() {
        ...
        match flag {
            ...
            "--source-ref" | "--profile" | "--author" | "--committer" | "--timestamp"
            | "--message" => {
            ...
            | "--delete" | "--delete-hex" | "--file" | "--file-hex" => {
                ...
                let path = if flag.ends_with("-hex") {
                    unhex_path(value)?
                } else {
                ...
                let choice = match flag.trim_end_matches("-hex") {
                    ...
                    "--file" => {
                        let mode = match arguments.get(cursor).map(String::as_str) {
                            ...
                            _ => {
                                return Err(
                                    "--file requires an explicit 100644 or 100755 mode".into()
                                );
                            }
                        };
                        let file = arguments
            ...
            _ => {
                return Err(format!(
                    "unknown merge resolve option {flag}; blanket defaults and arbitrary edits are unsupported"
                ));
            }
        }
    }
    let inputs = ResolutionInputs {
        base: base.ok_or("--merge-base is required")?,
        target: target.ok_or("--expected-target is required")?,
        source: source.ok_or("--expected-source is required")?,
    };
    inputs
        .validate(inputs.target.algorithm())
        .map_err(|error| error.to_string())?;
    ...
    if resolutions.is_empty() {
        return Err("at least one explicit conflict resolution is required".into());
    }
    validate_resolutions(&resolutions, PreparationLimits::default())
        .map_err(|error| error.to_string())?;
...
fn load_files(input: &mut Input) -> Result<(), String> {
    ...
    for (index, path) in &input.files {
        let remaining = limits
            .max_output_bytes
            .checked_sub(bytes)
            .ok_or("resolution input budget exhausted")?;
        let content = read_resolution_file(path, limits.max_text_bytes.min(remaining))?;
        bytes = bytes
            .checked_add(content.len())
            .ok_or("resolution input length overflow")?;
...
fn read_resolution_file(path: &Path, limit: usize) -> Result<Vec<u8>, String> {
    ...
    if !metadata.is_file() || metadata.len() > limit as u64 {
        return Err(
            "resolution input must be a bounded regular file, not a symlink or device".into(),
        );
    }
    ...
    content
        .try_reserve(usize::try_from(opened.len()).map_err(|_| "resolution length overflow")?)
...
fn render(
    ...
    if plan.base != input.inputs.base
        || plan.target != input.inputs.target
        || plan.source != input.inputs.source
        || plan.commit.is_zero()
        || plan.commit.algorithm() != input.inputs.target.algorithm()
        || resolved.resolutions.len() != input.resolutions.len()
        || !resolved
            .resolutions
            .windows(2)
            .all(|pair| pair[0].conflict.path < pair[1].conflict.path)
    {
    ...
    for result in &resolved.resolutions {
        if !input
            .resolutions
            .iter()
            .any(|r| r.path == result.conflict.path && r.choice.kind() == result.choice)
        {
            return Err("resolution response does not match submitted choices".into());
        }
        rows.push(format!(
            "{{\"conflict\":{},\"choice\":{},\"result\":{}}}",
            render_conflict(&result.conflict),
            quote(&format!("{:?}", result.choice)),
            render_entry(result.result.as_ref())
        ));
    }
    let out = format!(
        concat!(
            "{{\"type\":\"merge_resolution\",\"schema_version\":1,\"profile\":\"path-resolved-v1\",",
        ...
        quote(&input.options.repository.to_string()),
        quote(&head.to_string()),
        quote(plan.commit.algorithm().as_str()),
        quote(&hex(input.options.target.as_bytes())),
        quote(&hex(input.options.incoming.as_bytes())),
        quote(&plan.target.to_string()),
        quote(&plan.source.to_string()),
        quote(&plan.base.to_string()),
        quote(&plan.commit.to_string()),
        quote(&plan.tree.to_string()),
        plan.objects.len(),
        bundle_bytes,
        quote(&hex(bundle_sha256)),
        quote(&input.options.output.to_string_lossy()),
        rows.len(),
        rows.join(",")
    );
    if out.len() > 4 * 1024 * 1024 {
        return Err("resolution receipt exceeds its byte limit".into());
    }
    Ok(out)
}
...
mod tests {
    ...
    fn pins_profile_trust_and_choices_are_required_without_repository_access() {
        let valid = args();
        assert!(parse_input(&valid).is_ok());
        for flag in [
            "--expected-target",
            "--expected-source",
            "--merge-base",
            ...
            assert!(parse_input(&invalid).is_err(), "{flag}");
        }
        let mut invalid = valid.clone();
        invalid.retain(|arg| arg != "--trusted-local");
        assert!(parse_input(&invalid).is_err());
        for extra in [
            vec!["--ours", "file"],
            vec!["--theirs", "../escape"],
            vec!["--file", "other", "120000", "link"],
            vec!["--object-format", "sha256"],
            vec!["--expected-target", &"44".repeat(20)],
            vec!["--all-ours", "true"],
        ] {
            let mut invalid = valid.clone();
        ...
        for flag in ["--expected-target", "--expected-source", "--merge-base"] {
            let at = sha256.iter().position(|arg| arg == flag).unwrap();
            let first = sha256[at + 1][..2].to_owned();
            sha256[at + 1] = first.repeat(32);
        }
        assert_eq!(
            parse_input(&sha256).unwrap().inputs.target.algorithm(),
    ...
    fn raw_paths_and_file_modes_are_not_lossy_or_implicit() {
        ...
        assert_eq!(parsed.resolutions[0].path, vec![255]);
        assert!(matches!(
            parsed.resolutions[0].choice,
            ResolutionChoice::File { mode: 0o100755, .. }
        ));
        assert_eq!(parsed.files, vec![(0, PathBuf::from("local.bytes"))]);
        input.extend(["--ours-hex".into(), "FF".into()]);
    ...
    fn manual_file_intake_accepts_empty_and_binary_but_refuses_symlinks_and_bounds() {
        ...
        let file = root.join("bytes");
        std::fs::write(&file, b"").unwrap();
        assert_eq!(read_resolution_file(&file, 0).unwrap(), b"");
        std::fs::write(&file, b"\xff\0\r\n").unwrap();
        assert_eq!(read_resolution_file(&file, 4).unwrap(), b"\xff\0\r\n");
        assert!(read_resolution_file(&file, 3).is_err());
        assert!(read_resolution_file(&root, 10).is_err());
        #[cfg(unix)]
        {
    ...
    fn receipt_write_and_flush_failures_preserve_known_artifact_state() {
        ...
        for broken in [true, false] {
            let error = emit(
                &mut Broken(broken),
                "{}",
                Some((Path::new("out.bundle"), commit)),
            )
            .unwrap_err();
            assert!(error.contains(&commit.to_string()));
            assert!(error.contains("exists at"));
        }
    }
}
