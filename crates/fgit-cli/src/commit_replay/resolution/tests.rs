...
fn resolve_is_explicit_and_duplicate_or_overlapping_raw_paths_refuse_before_io() {
    ...
    let mut wrong = args("prepare");
    wrong.extend(strings(&["--ours", "file"]));
    assert!(parse(&wrong).is_err());
    let mut valid = args("resolve");
    valid.extend(strings(&["--ours-hex", "7372632fff"]));
    let parsed = parse(&valid).unwrap();
    assert_eq!(parsed.resolutions.as_ref().unwrap()[0].path, b"src/\xff");
    for extra in [
        vec!["--ours", "file", "--theirs-hex", "66696c65"],
        vec!["--ours", "a", "--ours", "a-", "--theirs", "a/b"],
        vec!["--ours", "../escape"],
        vec!["--delete", ".git/config"],
        vec!["--file", "file", "120000", "missing"],
    ] {
        let mut bad = args("resolve");
        bad.extend(strings(&extra));
        assert!(parse(&bad).is_err(), "{extra:?}");
    }
    let mut valid = args("resolve");
    valid.extend(strings(&[
        "--file",
        "file",
        "100755",
    ...
    for option in ["--force", "--continue", "--abort", "--approve"] {
        ...
        bad.extend(strings(&[option, "true"]));
...
fn exact_empty_binary_inputs_are_distinct_from_deletion_and_share_one_byte_budget() {
    ...
    fs::write(&raw, b"\0\xff\r\nend").unwrap();
    let choices = vec![
        LocalResolution {
            path: b"empty".to_vec(),
            choice: LocalChoice::File {
                mode: 0o100644,
                source: empty,
            },
        },
        ...
        LocalResolution {
            path: b"gone".to_vec(),
            choice: LocalChoice::Side(ResolutionChoice::Delete),
        },
    ];
    let loaded = load(&choices, PreparationLimits::default()).unwrap();
    assert!(
        matches!(&loaded[0].choice, ResolutionChoice::File { mode: 0o100644, bytes } if bytes.is_empty())
    );
    assert!(
        matches!(&loaded[1].choice, ResolutionChoice::File { mode: 0o100755, bytes } if bytes == b"\0\xff\r\nend")
    );
    assert_eq!(loaded[2].choice, ResolutionChoice::Delete);
    let limit = PreparationLimits {
        max_output_bytes: 18,
        ..PreparationLimits::default()
    };
    // Paths consume 12 bytes and exact file content consumes seven more.
    assert!(load(&choices, limit).is_err());
    let enough = PreparationLimits {
        max_output_bytes: 19,
        ..PreparationLimits::default()
    };
...
fn resolution_reader_refuses_nonregular_files_and_oversized_content() {
    let scratch = Scratch::new();
    let file = scratch.0.join("content");
    fs::write(&file, b"12345").unwrap();
    assert!(read_file(&file, 4).is_err());
    assert_eq!(read_file(&file, 5).unwrap(), b"12345");
...
fn receipt_fixture() -> (ConflictResolution, ResolvedPath) {
    ...
    let entry = |bytes: &[u8]| MergeEntry {
        name: b"file".to_vec(),
        mode: 0o100644,
        oid: git_object_id(format, GitObjectKind::Blob, bytes),
    };
    let requested = ConflictResolution {
        path: b"file".to_vec(),
        choice: ResolutionChoice::File {
            mode: 0o100755,
            bytes: b"manual\0\xff".to_vec(),
        },
    };
    let result = MergeEntry {
        mode: 0o100755,
        ..entry(b"manual\0\xff")
    };
    let actual = ResolvedPath {
        conflict: MergeConflict {
            path: b"file".to_vec(),
            kind: ConflictKind::Content,
            base: Some(entry(b"base")),
            ours: Some(entry(b"ours")),
            theirs: Some(entry(b"theirs")),
        },
...
fn receipt_checks_exact_file_identity_mode_count_and_choice_before_publication() {
    ...
    assert!(rendered.contains("\"choice\":\"file\""));
    assert!(rendered.contains("\"approval_granted\":false"));
    for mutation in 0..6 {
        ...
        match mutation {
            ...
            2 => {
                changed.result.as_mut().unwrap().oid =
                    git_object_id(GitHashAlgorithm::Sha1, GitObjectKind::Blob, b"wrong")
            }
...
fn receipt_never_relabels_a_missing_side_as_an_explicit_deletion() {
    let (_, mut row) = receipt_fixture();
    row.conflict.theirs = None;
    row.result = None;
    row.choice = ResolutionKind::Theirs;
    let base = "{\"type\":\"commit_replay_preparation\"}";
    let mut requested = ConflictResolution {
        path: b"file".to_vec(),
    ...
    requested.choice = ResolutionChoice::Delete;
    row.choice = ResolutionKind::Delete;
