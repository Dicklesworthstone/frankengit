...
use fgit_forge::preparation::resolution::{
    ConflictResolution, ResolutionChoice, ResolutionKind, ResolvedPath, validate_resolutions,
};
...
pub(super) fn parse_choice(
    ...
    let raw = args
        .get(*at)
        .ok_or_else(|| format!("missing repository path for {flag}"))?;
    *at += 1;
    let path = if flag.ends_with("-hex") {
        unhex(raw, 4096)?
    } else {
        if raw.len() > 4096 {
            return Err("resolution path exceeds 4096 bytes".into());
        }
        raw.as_bytes().to_vec()
    };
...
pub(super) fn validate_inputs(
    choices: &[LocalResolution],
    limits: PreparationLimits,
) -> Result<(), String> {
    if choices.len() > limits.max_conflicts {
        return Err("too many conflict choices".into());
    }
    let shapes: Vec<_> = choices
        .iter()
        .map(|item| ConflictResolution {
            path: item.path.clone(),
            choice: match &item.choice {
                LocalChoice::Side(choice) => choice.clone(),
                LocalChoice::File { mode, .. } => ResolutionChoice::File {
                    mode: *mode,
                    bytes: Vec::new(),
                },
            },
...
pub(super) fn load(
    ...
    let mut consumed = choices
        .iter()
        .try_fold(0_usize, |sum, choice| sum.checked_add(choice.path.len()))
        .ok_or("resolution path byte overflow")?;
    let mut out = Vec::new();
    out.try_reserve(choices.len())
        ...
        let choice = match &item.choice {
            ...
            LocalChoice::File { mode, source } => {
                let remaining = limits
                    ...
                    .ok_or("resolution input byte budget exhausted")?
                    .min(limits.max_text_bytes);
                let bytes = read_file(source, remaining)?;
                consumed = consumed
                    .checked_add(bytes.len())
                    .ok_or("resolution input byte overflow")?;
                ResolutionChoice::File { mode: *mode, bytes }
            }
        };
        out.push(ConflictResolution {
            path: item.path.clone(),
            choice,
        });
    }
...
fn read_file(path: &Path, limit: usize) -> Result<Vec<u8>, String> {
    ...
    let metadata =
        fs::symlink_metadata(path).map_err(|e| format!("resolution input metadata: {e}"))?;
    ...
    if !opened.is_file() || opened.len() > limit as u64 {
        return Err("resolution input changed type or exceeded limit".into());
    }
    let mut out = Vec::new();
    ...
    loop {
        ...
        if count > limit.saturating_sub(out.len()) {
            return Err("resolution input grew beyond its byte budget".into());
        }
        out.try_reserve(count)
            .map_err(|_| "resolution input allocation refused")?;
...
pub(super) fn decorate_receipt(
    ...
    let choices: BTreeMap<_, _> = requested
        .iter()
        .map(|r| (r.path.as_slice(), &r.choice))
        .collect();
    if choices.len() != requested.len() {
        return Err("duplicate requested resolution in receipt".into());
    }
    let mut rows = Vec::new();
    for row in actual {
        let choice = choices
            .get(row.conflict.path.as_slice())
            .ok_or("unsolicited resolution path")?;
        let name = row
            .conflict
            .path
            .rsplit(|b| *b == b'/')
        ...
        if choice.kind() != row.choice {
            return Err("resolution choice mismatch".into());
        }
        let expected = match choice {
            ...
            ResolutionChoice::Theirs => {
                Some(row.conflict.theirs.clone().ok_or("missing applied side")?)
            }
        ...
        if row.result != expected
            ...
            .any(|entry| {
                entry.name.as_slice() != name
                    || entry.oid.is_zero()
                    || entry.oid.algorithm() != format
            })
        {
            return Err(
                "resolution result differs from the selected side or exact file bytes".into(),
            );
        }
        rows.push(format!(
            "{{\"conflict\":{},\"choice\":{},\"result\":{}}}",
            render_conflict(&row.conflict),
            quote(kind_name(row.choice)),
            entry_json(row.result.as_ref())
        ));
    }
    ...
    if output.len() > 4 * 1024 * 1024 {
        return Err("resolution receipt exceeds output bound".into());
    }
    Ok(output)
}
