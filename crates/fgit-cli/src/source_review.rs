...
use fgit_forge::review::{
    ComparisonMode, EntryIdentity, ReviewContent, ReviewOptions, ReviewSelection, ReviewSpan,
    ReviewedEntry, SourceReview,
};
use fgit_forge::{AggregateVersion, PullRequestNumber};
use fgit_node::{NodeConfig, OneNode};
use fgit_types::{
    CANONICAL_CODEC_VERSION, DigestAlgorithmId, DigestBytes, GitHashAlgorithm, HeadGeneration,
...
pub fn run(arguments: &[String], pr: bool) -> Result<(), String> {
    ...
    let result = (|| {
        ...
        node.runtime()
            .block_on(node.review_source_in(
                &request,
                &options.selection,
                &Default::default(),
    ...
    let review = match (result, cleanup) {
        ...
        (Ok(_), Some(error)) => {
            return Err(format!(
                "review node shutdown failed: {error}; no successful report returned"
            ));
        }
...
fn parse(arguments: &[String], pr: bool) -> Result<Options, String> {
    ...
    if arguments.len() < positional {
        return Err(USAGE.into());
    }
    if arguments.len() > 160
        || arguments.iter().any(|arg| arg.len() > 8192)
    ...
    while at < arguments.len() {
        ...
        match flag {
            "--comparison" => {
                review.mode = match value.as_str() {
            ...
            "--path" | "--path-hex" => {
                if review.paths.len() >= 64 {
                    return Err("at most 64 path prefixes".into());
                }
                review.paths.push(if flag == "--path" {
    ...
    if !trusted {
        return Err("--trusted-local is required for this local/ref-authorized review".into());
    }
    review.paths.sort();
    review.paths.dedup();
    ...
    let selection = if pr {
        ReviewSelection::PullRequest {
            number: PullRequestNumber::try_new(decimal(&arguments[3])?)
                .ok_or("PR number must be positive")?,
            expected_version: version,
        }
    } else {
        let reference = |value: &str| {
            let bytes = if refs_hex {
                unhex(value, 1024)?
            } else {
                value.as_bytes().to_vec()
            };
        ...
        ReviewSelection::References {
            before: reference(&arguments[3])?,
            after: reference(&arguments[4])?,
        }
    };
    Ok(Options {
...
fn text(value: &[u8]) -> String {
    std::str::from_utf8(value).map_or_else(|_| "null".into(), quote)
}
    ...
    let content = match &entry.content {
        ...
        ReviewContent::Text {
            ...
            let hunks = hunks.iter().map(|hunk| format!(concat!("{{\"old\":{},\"new\":{},",
                ...
                .collect::<Vec<_>>().join(",");
            format!(
                "{{\"kind\":\"text\",\"algorithm\":{},\"additions\":{additions},\"deletions\":{deletions},\"before_bytes\":{before_bytes},\"after_bytes\":{after_bytes},\"hunks\":[{hunks}]}}",
                quote(&format!("{algorithm:?}"))
            )
        }
    };
    format!(
        "{{\"path_hex\":{},\"path_text\":{},\"change\":{},\"before\":{},\"after\":{},\"content\":{content}}}",
        quote(&hex(&entry.path)),
        text(&entry.path),
        quote(&format!("{:?}", entry.kind)),
        entry_identity(entry.before),
        entry_identity(entry.after)
    )
}
fn render(options: &Options, review: &SourceReview) -> Result<String, String> {
    let comparison = &review.comparison;
    if review.repository_id != options.repository
        || comparison.mode != options.review.mode
        || options
            .expected_head
            .is_some_and(|head| head != review.source_head)
    ...
    let selection_matches = match &options.selection {
        ReviewSelection::References { before, after } => {
            review.pull_request.is_none()
                && before == &review.before_reference
                && after == &review.after_reference
        }
        ReviewSelection::PullRequest {
            number,
            expected_version,
        } => review.pull_request.is_some_and(|(actual, version)| {
            actual == *number && expected_version.is_none_or(|wanted| wanted == version)
        }),
    };
    ...
    let paths = options
        ...
        .paths
        .iter()
        .map(|path| quote(&hex(path)))
        .collect::<Vec<_>>()
        .join(",");
    let pr = review.pull_request.map_or_else(
        || "null".into(),
        |(number, version)| {
            format!(
                "{{\"number\":{},\"version\":{}}}",
    ...
    let mode = match comparison.mode {
        ComparisonMode::Direct => "direct",
        ComparisonMode::MergeBase => "merge-base",
    };
    let mut out = format!(
        concat!(
            "{{\"type\":\"source_review\",\"schema_version\":1,\"profile\":\"path-myers-v1\",",
        ...
        quote(&head_token(review.source_head)),
        quote(&hex(review.before_reference.as_bytes())),
        quote(&hex(review.after_reference.as_bytes())),
        pr,
        quote(mode),
        quote(&comparison.requested_before.to_string()),
        quote(&comparison.requested_after.to_string()),
        quote(&comparison.compared_before.to_string()),
        quote(&comparison.before_tree.to_string()),
        quote(&comparison.after_tree.to_string()),
        options.review.context_lines,
        paths,
...
mod tests {
    ...
    fn args(pr: bool) -> Vec<String> {
        ...
        if !pr {
            args.push("refs/heads/topic".into());
        }
    ...
    fn modes_pins_domains_and_duplicate_flags_are_explicit() {
        assert_eq!(
            parse(&args(false), false).unwrap().review.mode,
            ComparisonMode::Direct
        );
        assert_eq!(
            parse(&args(true), true).unwrap().review.mode,
            ComparisonMode::MergeBase
        );
        for pr in [false, true] {
            let mut input = args(pr);
            input.extend([
                "--expected-head".into(),
            ...
            let options = parse(&input, pr).unwrap();
            assert_eq!(options.review.context_lines, 0);
            assert_eq!(options.format, GitHashAlgorithm::Sha256);
            assert_eq!(
                parse_head(&head_token(options.expected_head.unwrap())).unwrap(),
                options.expected_head.unwrap()
            );
            input.extend(["--context-lines".into(), "0".into()]);
            assert!(parse(&input, pr).is_err());
        }
    }
    #[test]
    fn invalid_inputs_refuse_without_opening_a_repository() {
        for extra in [
            vec!["--expected-version", "1"],
            vec!["--max-changes", "0"],
            vec!["--max-blob-bytes", "1048577"],
            vec!["--context-lines", "21"],
            vec!["--path", "../x"],
            vec!["--path-hex", "xx"],
            vec!["--driver", "shell"],
            vec!["--comparison", "automatic"],
            vec!["--trusted-local"],
        ] {
            let mut input = args(false);
            input.extend(extra.iter().map(|s| (*s).to_owned()));
        ...
        input = args(true);
        input.extend(["--expected-version".into(), "0".into()]);
        assert!(parse(&input, true).is_err());
    }
    #[test]
    fn raw_paths_and_reference_names_are_lossless() {
        ...
        input[4] = hex(b"refs/heads/topic");
        input.extend([
            "--refs-hex".into(),
            "--path-hex".into(),
            "ff".into(),
        ...
        let ReviewSelection::References { before, .. } = options.selection else {
            panic!("refs");
        };
