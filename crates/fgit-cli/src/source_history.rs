...
use fgit_forge::history::{
    BlameOptions, BlameResult, HistoryCommit, HistoryLimits, HistoryPage, LogOptions,
};
use fgit_node::{NodeConfig, OneNode};
use fgit_types::{
    CANONICAL_CODEC_VERSION, DigestAlgorithmId, DigestBytes, GitHashAlgorithm, GitOid,
    ...
    let operation = (|| -> Result<_, String> {
        ...
        match &options.query {
            Query::Log(query) => node
                ...
                .map(|(head, result)| (head, Answer::Log(result)))
                .map_err(|e| e.to_string()),
            Query::Blame(query) => node
                .runtime()
                ...
                .map(|(head, result)| (head, Answer::Blame(result)))
                .map_err(|e| e.to_string()),
        }
    })();
...
fn emit(output: &mut impl Write, text: &str) -> Result<(), String> {
    writeln!(output, "{text}")
        ...
        .map_err(|e| format!("history output incomplete: {e}"))
}
fn parse(args: &[String], is_blame: bool) -> Result<Options, String> {
    ...
    while at < args.len() {
        ...
        let allowed = switch
            || matches!(
                flag,
                "--object-format" | "--expected-head" | "--max-commits" | "--max-edges"
            )
            || if is_blame {
                matches!(
                    flag,
                ...
                matches!(flag, "--after" | "--limit")
            };
        if !allowed {
            return Err(format!("unknown or inapplicable history option {flag:?}"));
        }
        let value = if switch {
            ""
        } else {
    ...
    if !flags.contains_key("--trusted-local") {
        return Err("--trusted-local is required for local/ref-authorized history".into());
    }
    ...
    let head = flags
        ...
        .transpose()?;
    let number = |flag: &str, fallback: usize| -> Result<usize, String> {
        flags.get(flag).map_or(Ok(fallback), |text| size(text))
    };
    ...
    let query = if is_blame {
        ...
        let query = BlameOptions {
            path,
            first_line: number("--line-start", 0)?,
            end_line: flags.get("--line-end").map(|text| size(text)).transpose()?,
            limits,
        };
        query.validate().map_err(|e| e.to_string())?;
        Query::Blame(query)
    } else {
        let query = LogOptions {
            after: number("--after", 0)?,
            limit: number("--limit", 50)?,
            limits,
        };
        query.validate().map_err(|e| e.to_string())?;
        if query.after != 0 && head.is_none() {
            return Err(
                "log continuation requires the original --expected-head snapshot_token".into(),
            );
        }
        Query::Log(query)
    };
...
fn size(text: &str) -> Result<usize, String> {
    usize::try_from(decimal(text)?).map_err(|_| "integer exceeds target width".into())
}
...
fn text(bytes: &[u8]) -> String {
    std::str::from_utf8(bytes).map_or_else(|_| "null".into(), quote)
}
...
fn parse_head(text: &str) -> Result<RepositoryAuthorityHeadId, String> {
    let (algorithm, digest) = text
        ...
        .ok_or("expected algorithm-qualified authority head")?;
...
fn valid_id(id: GitOid, options: &Options) -> bool {
    !id.is_zero() && id.algorithm() == options.format
}
fn header(
    options: &Options,
    ...
    if options.head.is_some_and(|expected| expected != head) || !valid_id(tip, options) {
        return Err("history source binding mismatch".into());
    }
    Ok(format!(
        concat!(
            "\"schema_version\":1,\"complete\":true,\"node_closed\":true,",
...
fn append(out: &mut String, row: &str) -> Result<(), String> {
    if out.len().saturating_add(row.len()).saturating_add(2) > MAX_JSON {
        return Err("history JSON limit exceeded".into());
    }
    out.push_str(row);
    Ok(())
}
fn records(options: &Options, commits: &[HistoryCommit]) -> Result<String, String> {
    ...
    for (i, commit) in commits.iter().enumerate() {
        bytes = bytes
            .checked_add(commit.body.len())
            .ok_or("metadata length overflow")?;
        if commit.body.len() > 64 * 1024
            || bytes > HistoryLimits::default().max_metadata_bytes
            || !valid_id(commit.id, options)
            || !valid_id(commit.tree, options)
        ...
        append(
            &mut out,
            &format!(
                "{{\"commit\":{},\"tree\":{},\"parents\":[{parents}],\"body_hex\":{},\"body_text\":{}}}",
                quote(&commit.id.to_string()),
                quote(&commit.tree.to_string()),
                quote(&hex(&commit.body)),
                text(&commit.body)
            ),
        )?;
    }
...
fn render_log(
    ...
    let Query::Log(query) = &options.query else {
        return Err("history operation mismatch".into());
    };
    let end = page
        .after
    ...
    if page.after != query.after
        || page.total_commits > query.limits.max_commits
        || end > page.total_commits
        || page.commits.len()
            != query
                .limit
                .min(page.total_commits.saturating_sub(page.after))
        || page.next_after != (end < page.total_commits).then_some(end)
        || (page.after == 0
            && page
                .commits
                .first()
                .is_none_or(|commit| commit.id != page.tip))
    {
    ...
    let mut out = format!(
        "{{\"type\":\"commit_history\",\"profile\":\"topo-oid-v1\",\"scope\":\"reachable_commit_dag\",{header},\"total_commits\":{},\"after\":{},\"limit\":{},\"next_after\":{},\"commits\":",
        page.total_commits,
        page.after,
        query.limit,
        page.next_after
...
fn render_blame(
    ...
    let Query::Blame(query) = &options.query else {
        return Err("blame operation mismatch".into());
    };
    if result.path != query.path
        || result.first_line != query.first_line
        || result.end_line != query.end_line.unwrap_or(result.total_lines)
        || result.end_line < result.first_line
        || result.end_line > result.total_lines
        ...
        || !valid_id(result.tree, options)
        || !valid_id(result.blob, options)
        || result.graph_commits > query.limits.max_commits
        || result.comparisons > query.limits.max_comparisons
    {
    ...
    let mut out = format!(
        concat!(
            "{{\"type\":\"source_blame\",\"profile\":\"exact-lines-all-parents-v1\",",
        ...
        header,
        quote(&result.tree.to_string()),
        quote(&result.blob.to_string()),
        quote(&hex(&result.path)),
        result.total_lines,
        result.first_line,
        result.end_line,
        result.content_byte_start,
        quote(&hex(&result.content)),
        text(&result.content),
        result.graph_commits,
        result.comparisons,
        query.limits.max_diff_work,
        algorithms
    );
    append(&mut out, &origins)?;
    append(&mut out, ",\"lines\":[")?;
    let mut cursor = result.content_byte_start;
    for (i, line) in result.lines.iter().enumerate() {
        if line.line != result.first_line + i
            || line.byte_start != cursor
            || line.byte_end <= line.byte_start
            || line.origin_byte_end <= line.origin_byte_start
            || !valid_id(line.origin_blob, options)
        ...
        append(
            ...
            &format!(
                ...
                line.byte_start,
                line.byte_end,
                quote(&line.origin_commit.to_string()),
                quote(&line.origin_blob.to_string()),
                line.origin_line,
                line.origin_byte_start,
                line.origin_byte_end
            ),
        )?;
    }
    if cursor.checked_sub(result.content_byte_start) != Some(result.content.len()) {
        return Err("blame content span mismatch".into());
    }
    out.push_str("]}");
    Ok(out)
}
...
mod tests {
    ...
    fn modes_raw_names_ranges_and_pinned_continuations_parse_without_repository_io() {
        let input = args();
        assert!(matches!(parse(&input, false).unwrap().query, Query::Log(_)));
        let mut input = args();
        ...
        assert_eq!(
            parse_head(&head_token(options.head.unwrap())).unwrap(),
            options.head.unwrap()
        );
        let mut input = args();
        input[3] = hex(b"refs/heads/\xff");
        input.extend([
            "--ref-hex".into(),
            "--path-hex".into(),
            "6469722fff".into(),
        ...
        assert_eq!(query.path, b"dir/\xff");
    ...
    fn malformed_or_inapplicable_options_refuse_instead_of_changing_the_query() {
        ...
        for extra in [
            vec!["--path", "../file"],
            vec!["--path", "file", "--path-hex", "66"],
            vec!["--path", "file", "--line-start", "5", "--line-end", "4"],
            vec!["--path", "file", "--limit", "2"],
        ] {
            let mut input = args();
            input.extend(extra.into_iter().map(str::to_owned));
        ...
        no_trust.pop();
        assert!(parse(&no_trust, false).is_err());
    ...
    fn rendering_preserves_raw_metadata_and_rejects_inconsistent_pages_and_line_origins() {
        ...
        let record = HistoryCommit {
            id: tip,
            tree,
            parents: Vec::new(),
            body: b"claim\x1b[2J".to_vec(),
        };
        ...
        assert!(json.contains("body_hex"));
        page.next_after = Some(1);
        assert!(render_log(&options, head, &page).is_err());
        let mut input = args();
        input.extend(["--path".into(), "file".into()]);
        let options = parse(&input, true).unwrap();
        let mut result = BlameResult {
            ...
            content: b"a\n".to_vec(),
            lines: vec![fgit_forge::history::BlameLine {
                line: 0,
                byte_start: 0,
                byte_end: 2,
                origin_commit: tip,
