...
fn command(f: &Fixture, candidate: GitOid, run: &str, trusted: bool) -> Command {
    ...
    c.args(["workflow", "run-candidate"]).arg(f.root.join("node"))
        .arg(TENANT.to_string()).arg(REPOSITORY.to_string()).arg("refs/heads/main")
        .args(["--workflow", "workflow.yml", "--run-id"]).arg(run.repeat(16))
        .arg("--run-parent").arg(f.parent()).args(["--object-format", f.tip.algorithm().as_str()])
        .arg("--expected-commit").arg(f.tip.to_string()).arg("--candidate-commit").arg(candidate.to_string())
        .arg("--bundle").arg(f.root.join("candidate.bundle"));
    for path in inputs() { c.arg("--input").arg(String::from_utf8(path).unwrap()); }
    if trusted { c.arg("--trusted-local"); }
    c
}
...
fn cli_executes_candidate_persists_exact_binding_and_refuses_replay_in_both_formats() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let digest: String = sha256_digest(candidate.bundle_bytes()).iter().map(|b| format!("{b:02x}")).collect();
        f.node.take().unwrap().shutdown().unwrap();
        let out = command(&f, candidate.candidate_commit, "51", true).output().unwrap(); exit(&out, 0);
        ...
        assert!(text.contains(&format!("\"executed_commit\":\"{}\"", candidate.candidate_commit)));
        assert!(text.contains(&format!("\"workflow_blob\":\"{}\"", f.workflow)));
        assert!(text.contains(&format!("\"bundle_sha256\":\"{digest}\"")));
        assert!(text.contains("\"admitted\":false") && text.contains("\"authoritative_check\":false"));
        let report_path = f.parent().join(format!("workflow-{}", "51".repeat(16))).join("report.json");
        ...
        let retry = command(&f, candidate.candidate_commit, "51", true).output().unwrap(); exit(&retry, 2);
        assert!(retry.stdout.is_empty()); assert_eq!(fs::read_to_string(&report_path).unwrap(), saved);
        assert_unchanged(&mut f, candidate.candidate_commit, before);
    }
}
...
fn cli_trust_and_corrupt_inputs_refuse_before_execution_and_non_green_is_not_an_io_error() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        fs::write(f.root.join("candidate.bundle"), candidate.bundle_bytes()).unwrap();
        f.node.take().unwrap().shutdown().unwrap();
        let denied = command(&f, candidate.candidate_commit, "52", false).output().unwrap(); exit(&denied, 2);
        assert_eq!(fs::read_dir(f.parent()).unwrap().count(), 0);
        let mut bad = candidate.bundle_bytes().to_vec(); let end = bad.len() - 1; bad[end] ^= 1;
        fs::write(f.root.join("candidate.bundle"), &bad).unwrap();
        let refused = command(&f, candidate.candidate_commit, "52", true).output().unwrap(); exit(&refused, 2);
        assert!(refused.stdout.is_empty()); assert_eq!(fs::read_dir(f.parent()).unwrap().count(), 0);
        fs::write(f.root.join("candidate.bundle"), candidate.bundle_bytes()).unwrap();
        let failed = command(&f, candidate.candidate_commit, "52", true).output().unwrap(); exit(&failed, 1);
        ...
        let report = f.parent().join(format!("workflow-{}", "52".repeat(16))).join("report.json");
