...
fn edge_budget_exhaustion_has_no_partial_or_objects_only_success() {
    ...
    assert!(
        !error.contains("graph_object_malformed"),
        "a work budget is not corruption: {error}"
    );
    assert_eq!(success(audit(&root, "sha1", &[])), receipt);
    refused(audit(
        &root,
        "sha1",
        &["--objects-only", "--max-edges", "4"],
    ));
    let absent = scratch.0.join("never-opened");
    for extra in [
        vec!["--max-edges", "0"],
        vec!["--max-edges", "8000001"],
        vec!["--objects-only", "--objects-only"],
        vec!["--objects-only", "--max-edges", "1"],
    ] {
...
fn both_hash_formats_import_tag_chains_symlinks_and_external_gitlinks() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for (mode, name, oid) in [
            (b"100644".as_slice(), b"file".as_slice(), file),
        ...
        assert!(
            refused(audit(&root, format.as_str(), &["--max-edges", "5"])).contains("max-edges")
        );
