...
impl Fixture {
    fn new(format: GitHashAlgorithm, fail: bool) -> Self {
        let root = loop {
            let path = std::env::temp_dir().join(format!("fg-workflow-cli-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
        ...
        let run = if fail { "printf workflow-failed; exit 7" } else { "printf workflow-ok" };
        let workflow = format!("name: cli\non: push\njobs:\n  check:\n    runs-on: fgit-trusted-local\n    steps:\n      - run: {run}\n");
        ...
        let result = node.runtime().block_on(node.import_loose_git_directory_durable_in(
            &node.request_context(), &source, PrincipalId::from_bytes([0x53; 16]), b"cli-workflow-source")).unwrap();
        assert!(result.commands.iter().all(|c| matches!(c.terminal.outcome, DecisionOutcome::Committed { .. })));
        let generation = node.runtime().block_on(node.authenticate_authority_head()).unwrap().receipt().generation().get();
        node.shutdown().unwrap();
        Self { root, config, tip, format, generation }
    }
    fn command(&self, trusted: bool) -> Command {
        let mut command = Command::new(env!("CARGO_BIN_EXE_fg"));
        command.args(["workflow", "run"]).arg(self.root.join("node"))
            .arg(TENANT.to_string()).arg(REPOSITORY.to_string()).arg("refs/heads/main")
            .args(["--workflow", "workflow.yml", "--input", "workflow.yml", "--run-id"])
            .arg("61".repeat(16)).arg("--run-parent").arg(self.root.join("runs"))
    ...
    fn assert_unchanged(&self) {
        ...
        assert_eq!(node.runtime().block_on(node.authenticate_authority_head()).unwrap().receipt().generation().get(), self.generation);
        node.shutdown().unwrap();
    }
    fn report(&self) -> PathBuf { self.root.join("runs").join(format!("workflow-{}", "61".repeat(16))).join("report.json") }
}
...
fn loose(root: &Path, format: GitHashAlgorithm, kind: GitObjectKind, name: &str, body: &[u8]) -> GitOid {
    ...
    encoded.extend(len.to_le_bytes()); encoded.extend((!len).to_le_bytes()); encoded.extend(&raw);
    ...
    let hex = id.to_string(); fs::create_dir_all(root.join("objects").join(&hex[..2])).unwrap();
    fs::write(root.join("objects").join(&hex[..2]).join(&hex[2..]), encoded).unwrap();
    id
}
...
fn binary_executes_exact_source_persists_receipt_and_refuses_replay_in_both_formats() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let output = fixture.command(true).output().unwrap(); exit(&output, 0);
        ...
        let retry = fixture.command(true).output().unwrap(); exit(&retry, 2);
...
fn binary_requires_trust_and_reports_failed_commands_without_publication() {
    ...
    let refused = fixture.command(false).output().unwrap(); exit(&refused, 2);
    assert_eq!(fs::read_dir(fixture.root.join("runs")).unwrap().count(), 0);
    let output = fixture.command(true).output().unwrap(); exit(&output, 1);
    let response = String::from_utf8(output.stdout).unwrap();
    assert!(response.contains("\"node_closed\":true"));
    assert!(response.contains("\"succeeded\":false"));
    assert!(response.contains("\"exit_code\":7"));
    assert!(response.contains("\"published\":false"));
    assert!(fixture.report().is_file()); fixture.assert_unchanged();
}
