...
use fgit_types::{
    GitHashAlgorithm, GitOidSha1, MismatchPolicy, RegistryEpoch, SchemaFamily, SchemaId,
};
...
fn fixture(may_publish: bool) -> (RepositoryState, TransactionRequest, RefName) {
    ...
    state.head.body.roots.refs.insert(
        source.clone(),
    ...
    let request = RequestBuilder::new(
        ...
        principal,
        schema,
        IdempotencyKey::new(label("check")),
    )
    .statement(
        ...
        vec![Intent::Forge(ForgeIntent {
            ...
            event: ForgeEventKind::WorkflowCheckObserved {
                check: ForgeEntityId::new(label("check/one")),
                source: source.clone(),
            },
        })],
    )
...
fn workflow_observation_names_a_source_without_ref_mutation_authority() {
    ...
    let Intent::Forge(forge) = intent else {
        panic!("forge intent")
    };
    assert_eq!(forge.event.required_ref_effect(), None);
    let PreparedVerdict::Commit(effects) = evaluate(&state, &request).verdict else {
        panic!("metadata permission")
    };
...
fn refined_workflow_witness_is_not_reused_after_its_source_branch_moves() {
    ...
    assert_eq!(
        witness.refs.get(&source),
        Some(&state.head.body.roots.refs.get(&source).copied())
    );
    ...
    changed
        .refs
        .insert(source, GitOid::Sha1(GitOidSha1::from_bytes([2; 20])));
    assert!(!witness.is_reusable_against(
        &changed,
        state.head.body.generation,
        state.head.body.configuration.epoch
