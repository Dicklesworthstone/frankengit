...
fn put(objects: &mut Objects, format: GitHashAlgorithm, kind: ObjectKind, body: impl Into<Vec<u8>>) -> GitOid {
    let body = body.into();
...
fn audit(objects: &Objects, format: GitHashAlgorithm, references: &BTreeMap<RefName, GitOid>, limits: GraphLimits)
    -> Result<GraphReport, GraphRefusal>
{
    ...
    for (&id, (kind, bytes)) in objects { audit.observe(id, *kind, bytes, &mut || true)?; }
...
fn complete_graph_handles_both_native_domains_tag_chains_and_duplicate_parents() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let blob = put(&mut objects, format, ObjectKind::Blob, b"binary\0\xff".to_vec());
        let tree = put(&mut objects, format, ObjectKind::Tree,
            tree_body(&[(b"100644", b"file", blob), (b"120000", b"link", blob)]));
        let parent = put(&mut objects, format, ObjectKind::Commit, commit_body(tree, &[]));
        let commit = put(&mut objects, format, ObjectKind::Commit, commit_body(tree, &[parent, parent]));
        let tag = put(&mut objects, format, ObjectKind::Tag, tag_body(commit, "commit"));
        ...
        assert_eq!(result.payload_bytes, objects.values().map(|(_, body)| body.len() as u64).sum::<u64>());
    }
}
...
fn external_gitlinks_neither_require_nor_typecheck_local_objects_with_the_same_id() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let tree = put(&mut objects, format, ObjectKind::Tree, tree_body(&[
    ...
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        for (kind, body) in [
            ...
            assert_eq!(audit(&objects, format, &BTreeMap::new(), GraphLimits::default()),
                Err(GraphRefusal::MissingTarget { source: Some(source), target: absent }));
        }
        let mut objects = Objects::new();
        let tree = put(&mut objects, format, ObjectKind::Tree, Vec::new());
        let source = put(&mut objects, format, ObjectKind::Commit, commit_body(tree, &[absent]));
        assert_eq!(audit(&objects, format, &BTreeMap::new(), GraphLimits::default()),
            Err(GraphRefusal::MissingTarget { source: Some(source), target: absent }));
    }
}
...
fn all_local_edges_and_branch_roots_require_the_declared_object_kind() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        for (kind, expected, make) in [
            ...
            assert_eq!(audit(&objects, format, &BTreeMap::new(), GraphLimits::default()),
                Err(GraphRefusal::TargetKind { target: blob, expected, actual: ObjectKind::Blob }));
        }
        let mut objects = Objects::new();
        let tree = put(&mut objects, format, ObjectKind::Tree, Vec::new());
        put(&mut objects, format, ObjectKind::Tree, tree_body(&[(b"120000", b"link", tree)]));
        assert_eq!(audit(&objects, format, &BTreeMap::new(), GraphLimits::default()),
            Err(GraphRefusal::TargetKind { target: tree, expected: ObjectKind::Blob, actual: ObjectKind::Tree }));
        objects.retain(|id, _| *id == tree);
        let blob = put(&mut objects, format, ObjectKind::Blob, Vec::new());
        put(&mut objects, format, ObjectKind::Commit, commit_body(tree, &[blob]));
        assert_eq!(audit(&objects, format, &BTreeMap::new(), GraphLimits::default()),
            Err(GraphRefusal::TargetKind { target: blob, expected: ObjectKind::Commit, actual: ObjectKind::Blob }));
        objects.retain(|id, _| *id == blob);
        assert!(audit(&objects, format, &refs(b"refs/tags/blob", blob), GraphLimits::default()).is_ok());
        assert_eq!(audit(&objects, format, &refs(b"refs/heads/main", blob), GraphLimits::default()),
            Err(GraphRefusal::TargetKind { target: blob, expected: ObjectKind::Commit, actual: ObjectKind::Blob }));
    }
}
...
fn ambiguous_commit_and_tag_references_cannot_hide_edges() {
    ...
    for (kind, body) in [
        ...
        (ObjectKind::Commit, format!("tree {tree}\ntree {tree}\n\n").into_bytes()),
        (ObjectKind::Commit, format!("tree {tree}\n continuation\n\n").into_bytes()),
        (ObjectKind::Commit, format!("tree {tree}\nparent {}\n\n", "01".repeat(32)).into_bytes()),
        (ObjectKind::Tag, format!("object {tree}\nobject {tree}\ntype tree\n\n").into_bytes()),
        (ObjectKind::Tag, format!("object {tree}\ntype tree\ntype blob\n\n").into_bytes()),
    ] {
        let mut objects = base.clone();
        let id = put(&mut objects, format, kind, body);
        assert!(matches!(audit(&objects, format, &BTreeMap::new(), GraphLimits::default()),
...
fn unsupported_tree_modes_and_null_gitlinks_are_refused_not_silently_skipped() {
    ...
    for mode in [b"20000".as_slice(), b"170000", b"777777777777777777777"] {
        let mut objects = Objects::new();
        let id = put(&mut objects, format, ObjectKind::Tree, tree_body(&[(mode, b"bad", missing(format))]));
        assert_eq!(audit(&objects, format, &BTreeMap::new(), GraphLimits::default()),
            Err(GraphRefusal::InvalidTreeMode(id)));
    }
    let zero = GitOid::from_hex(format, &"0".repeat(40)).unwrap();
    let mut objects = Objects::new();
    put(&mut objects, format, ObjectKind::Tree, tree_body(&[(b"160000", b"bad", zero)]));
    assert_eq!(audit(&objects, format, &BTreeMap::new(), GraphLimits::default()), Err(GraphRefusal::ObjectFormat(zero)));
}
...
fn old_unordered_trees_and_unusual_nonreference_headers_are_not_normalized() {
    ...
    let tree = put(&mut objects, format, ObjectKind::Tree, tree_body(&[
        (b"100644", b"z", blob), (b"100644", b"a", blob),
    ]));
    let commit = put(&mut objects, format, ObjectKind::Commit,
        format!("tree {tree}\ngpgsig opaque\n continued signature\n\nmessage\0payload").into_bytes());
    assert!(audit(&objects, format, &refs(b"refs/heads/legacy", commit), GraphLimits::default()).is_ok());
}
...
fn observation_order_incomplete_reads_and_ignored_errors_cannot_yield_a_report() {
    ...
    let ids = objects.keys().copied().collect();
    let mut state = ObjectGraphAudit::new(&ids, format, GraphLimits::default(), &mut || true).unwrap();
    assert_eq!(state.observe(id, ObjectKind::Blob, b"wrong", &mut || true), Err(GraphRefusal::IdentityMismatch(id)));
    assert_eq!(state.observe(id, ObjectKind::Blob, b"real", &mut || true), Err(GraphRefusal::Failed));
    assert_eq!(state.finish(&BTreeMap::new(), &mut || true), Err(GraphRefusal::Failed));
    let state = ObjectGraphAudit::new(&ids, format, GraphLimits::default(), &mut || true).unwrap();
    assert_eq!(state.finish(&BTreeMap::new(), &mut || true), Err(GraphRefusal::Incomplete));
    let mut state = ObjectGraphAudit::new(&ids, format, GraphLimits::default(), &mut || true).unwrap();
    assert_eq!(state.observe(missing(format), ObjectKind::Blob, b"real", &mut || true), Err(GraphRefusal::ObjectOrder));
}

#[test]
fn missing_root_and_cross_format_selections_are_not_authorized_by_storage_existence() {
    let format = GitHashAlgorithm::Sha1;
    assert_eq!(audit(&Objects::new(), format, &refs(b"refs/tags/missing", missing(format)), GraphLimits::default()),
        Err(GraphRefusal::MissingTarget { source: None, target: missing(format) }));
    let foreign = missing(GitHashAlgorithm::Sha256);
    assert!(matches!(ObjectGraphAudit::new(&BTreeSet::from([foreign]), format, GraphLimits::default(), &mut || true),
...
fn exact_object_edge_reference_and_byte_limits_have_n_plus_one_refusal_twins() {
    ...
    let tree = put(&mut objects, format, ObjectKind::Tree, tree_body(&[(b"100644", b"a", blob)]));
    ...
    let limits = GraphLimits { max_objects: 2, max_edges: 1, max_references: 1,
        max_payload_bytes: payload, max_object_bytes: largest };
    audit(&objects, format, &references, limits).unwrap();
    for bounded in [
        GraphLimits { max_objects: 1, ..limits },
        GraphLimits { max_edges: 0, ..limits },
        GraphLimits { max_references: 0, ..limits },
        GraphLimits { max_payload_bytes: payload - 1, ..limits },
        GraphLimits { max_object_bytes: largest - 1, ..limits },
...
fn cancellation_at_every_checkpoint_prevents_completion() {
    ...
    let tree = put(&mut objects, format, ObjectKind::Tree, tree_body(&[(b"100644", b"file", blob)]));
    let references = refs(b"refs/tags/tree", tree);
    let run = |stop: usize| {
        let mut probes = 0;
        let mut live = || { probes += 1; probes < stop };
        let result = (|| {
            let mut state = ObjectGraphAudit::new(&objects.keys().copied().collect(), format, GraphLimits::default(), &mut live)?;
            for (&id, (kind, body)) in &objects { state.observe(id, *kind, body, &mut live)?; }
    ...
    result.unwrap();
    for stop in 1..=total { assert_eq!(run(stop).0, Err(GraphRefusal::Cancelled), "checkpoint {stop}"); }
}
...
fn iterative_cycle_walk_handles_deep_graphs_cycles_and_disconnected_components() {
    ...
    for index in 0..count {
        let start = edges.len();
        if index + 1 < count { edges.push(Edge { target: index + 1, expected: ObjectKind::Commit }); }
        nodes.push(Node { id, kind: Some(ObjectKind::Commit), start, end: edges.len() });
    }
    verify_acyclic(&nodes, &edges, &mut || true).unwrap();
    edges.push(Edge { target: 0, expected: ObjectKind::Commit });
