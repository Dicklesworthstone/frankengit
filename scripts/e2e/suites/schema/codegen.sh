...
# NOTE on `|| true`, which is load-bearing rather than sloppy:
# lib.sh documents that a bare `fge_run` whose command fails kills the script on
# that line under `set -e`, BEFORE FGE_LAST_EXIT is read and before the
# assertion meant to report it can run -- yielding `status=fail failed=0`, where
# the operator learns the suite died but not which check broke. Every fge_run
# here is followed by an assertion on its exit code, so every one is guarded.
set -euo pipefail
...
fge_run 'schema-gen check' \
  cargo run -q -p fgit-schema --bin fgit-schema-gen -- check "$SC_GENERATED" || true
...
fge_run 'schema-gen generate' \
  cargo run -q -p fgit-schema --bin fgit-schema-gen -- generate "$SC_WORK" || true
...
fge_run 'schema-gen check (corrupted copy)' \
  cargo run -q -p fgit-schema --bin fgit-schema-gen -- check "$SC_WORK" || true
...
if command -v python3 >/dev/null 2>&1; then
  ...
  fge_run 'python artifact imports' \
    python3 -c "import sys; sys.path.insert(0, '$SC_GENERATED'); import canonical_bodies as m; assert m.WIRE_ORDER" || true
