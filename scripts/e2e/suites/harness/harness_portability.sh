...
# ---------------------------------------------------------------------------
# frankengit-4k3x: a hyphenated suite FILENAME silently escapes the manifest
# set-equality gate.
#
# THE MECHANISM, traced through run_all.sh rather than assumed. A suite id is
# <area-dir>-<filename-stem> (ra_script_id, run_all.sh:298) and the area is
# recovered with `${d_id%-*}` (run_all.sh:414), which strips from the LAST
# hyphen. A hyphen in the DIRECTORY is therefore harmless -- suites/git-object/
# and suites/object-fabric/ both derive correctly, because the last hyphen
# still separates area from name. A hyphen in the FILENAME is not:
# suites/harness/foo-bar.sh derives the area `harness-foo`, which no profile
# claims. run_all.sh:416 files that phantom into S_MANIFEST_UNCOVERED_AREAS and
# takes the `continue`, SKIPPING the S_MANIFEST_UNREGISTERED check.
#
# WHY THAT IS A HOLE AND NOT A NUISANCE: unregistered is a HARD failure
# (run_all.sh:437, exit 1) and uncovered-areas is receipt-only -- it appears in
# no suite_status=fail condition. So the suite escapes a failing gate into a
# non-failing evidence bucket, and the release surface grows without the
# manifest approving it. That is precisely what the set check exists to stop.
#
# LATENT, NOT LIVE: measured across the corpus, 62/62 suites derive their true
# area and no hyphenated filename exists. This guard keeps it that way. It
# enforces the convention BoldMoose recorded on 1u5f (Agent Mail 2030) --
# "any hyphenated filename must either be covered by an explicit self-test or
# the parser changed before it lands" -- which until now lived only in a mail.
#
# SCOPE: this does NOT change the derivation. Whether the id format should
# admit hyphenated filenames is BoldMoose's call on run_all.sh.
#
# DELETION CONDITION: goes when the id parser handles hyphenated filenames.
# PORT-055 is that condition made executable -- it fails if the derivation
# changes, so this block cannot silently outlive its premise.
# ---------------------------------------------------------------------------
fge_phase action
hyphen_named=''
hyphen_checked=0
while IFS= read -r -d '' hy_f; do
  hyphen_checked=$((hyphen_checked + 1))
  hy_stem=${hy_f##*/}
  hy_stem=${hy_stem%.sh}
  case $hy_stem in
    *-*) hyphen_named="$hyphen_named${hy_f#"$E2E_ROOT"/} " ;;
  esac
done < <(find "$E2E_ROOT/suites" -type f -name '*.sh' -print0 | LC_ALL=C sort -z)

# Falsifiability and its permitted twin, computed with the SAME expression
# run_all.sh:414 uses. A planted hyphenated stem must derive an area that
# diverges from its true directory; the underscore twin must not.
hy_probe_id='harness-phantom-area'
hy_probe_area=${hy_probe_id%-*}
hy_twin_id='harness-phantom_area'
hy_twin_area=${hy_twin_id%-*}

fge_phase assert
fge_assert_eq FG-000A-PORT-051 '' "$hyphen_named" \
  'no suite filename contains a hyphen, which would derive a phantom area and skip the unregistered-suite gate'
fge_assert_cmd FG-000A-PORT-052 'the hyphen scan covered the whole suite corpus' \
  test "$hyphen_checked" -ge 60
fge_assert_ne FG-000A-PORT-053 harness "$hy_probe_area" \
  'a hyphenated filename stem demonstrably derives a phantom area, so PORT-051 is not vacuous'
fge_assert_eq FG-000A-PORT-054 harness "$hy_twin_area" \
  'the permitted twin -- an underscore stem in the same directory -- still derives its true area'
fge_assert_cmd FG-000A-PORT-055 'run_all.sh still derives the area by stripping the last hyphen, so this guard still guards what it claims' \
  grep -qF 'd_area=${d_id%-*}' "$E2E_ROOT/run_all.sh"
