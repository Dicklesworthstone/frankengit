...
# roots forward, so this campaign makes no redelivery, crash-recovery, or
# projection-rebuild claim.
set -euo pipefail
...
fge_context non_claim 'This lane proves one controlled merge-admission race, its typed stale loser, and the final canonical ref state. It does not claim forge-position advancement, outbox redelivery, crash recovery, or projection rebuild.'
...
fge_assert_exit FG-029B-E2E-006 0 "$lab_exit" \
  'the lab explores and replays real authority CAS schedules'
fge_assert_contains FG-029B-E2E-007 "$lab_output" \
...
fge_assert_exit FG-029B-E2E-009 0 "$admission_exit" \
  'the real admission mixed-generation corpus passes'
fge_assert_contains FG-029B-E2E-010 "$admission_output" \
