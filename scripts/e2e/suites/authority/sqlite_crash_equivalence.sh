...
fge_unsupported FG-005B-E2E-022 \
  'checkpoint under load: PERMANENTLY unreachable, not merely unexposed. My earlier wording -- FsqliteAuthorityStore publishes no checkpoint operation -- invited the wrong repair: add one. Adding one cannot help, because fsqlite 0.3.7 exposes no checkpoint command at all (no Command::Checkpoint variant, no public fn naming checkpoint; the only occurrences are an internal close_without_checkpoint_in_place). Every path that checkpoints also closes the connection, and closing it ends the load, so the cell is self-defeating rather than unbuilt. Recorded as disproven in NEG-022; verified independently at fsqlite-0.3.7/src/lib.rs rather than taken from the report'
