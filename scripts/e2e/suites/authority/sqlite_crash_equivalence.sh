...
fge_unsupported FG-005B-E2E-022 \
  'checkpoint under load: NOT covered here, and this reason has been wrong twice. It is NOT that no checkpoint exists: fsqlite-wal/src/checkpoint_executor.rs:114 execute_checkpoint does not close anything, and fsqlite-core/src/connection.rs maybe_run_adaptive_autocheckpoint runs at three NORMAL-OPERATION call sites, so checkpointing happens by default under sustained write load. The open question is OBSERVABILITY, not existence: whether a test can watch checkpoint behaviour through this profile surface. The previous text claimed permanent unreachability from a scan of one file of the facade crate, while the WAL lives in separate crates; see frankengit-g6s8 and the NEG-022 retraction'
