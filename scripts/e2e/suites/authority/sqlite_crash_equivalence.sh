...
fge_unsupported FG-005B-E2E-021 \
  'cancellation of a statement the VDBE is ACTIVELY STEPPING, plus the reply-lost cancellation cell. FG-005B-E2E-022 covers cancellation before dispatch, between retry attempts, and after dispatch before completion. What remains: the store statements are too short to reach a VDBE opcode checkpoint reliably (MEASURED -- 31 poll sites read, statements sized against the checkpoint interval), and reply-lost needs a lost response and a cancel arriving together, which the FG-005B-E2E-020 fault engine makes buildable but which is not written, so not claimed. CORRECTION, and it is why clause provenance is now labelled here: this cell previously also claimed commit-ambiguous, on the STRUCTURAL ASSERTION that it needed the same fault composition. That was wrong. Commit-ambiguous IS written -- the E2E-022 sweep cancels at scale-1, the post-commit window, which is exactly that cell and is the position that exposed w1ik. So it is not unwritten, it is DEFECT-BLOCKED, and it belongs to FG-005B-E2E-024. Right verdict for this cell, wrong reason inside it: the same shape RainyLotus found elsewhere, caught here only because the clause had been labelled unverified instead of left to look measured'
