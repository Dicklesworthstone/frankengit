...
main() {
  ...
  fge_assert_cmd FG-019C-PUSH-010 'a delete whose client omitted delete-refs is accepted by both, per the ee5663e ruling that the capability is a server advertisement' \
    grep -Fqx 'delete_without_negotiated_capability=match' "${verdict}"
  fge_assert_not_contains FG-019C-PUSH-017 "$(<"${verdict}")" \
    'delete_without_negotiated_capability=accepted-divergence' \
    'the ruled-compatible case is never quietly downgraded back to a divergence'
