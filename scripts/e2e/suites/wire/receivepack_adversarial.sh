...
main() {
  local wire_exit=0 admission_exit=0
  local wire_out='' admission_out=''
  local wire_passed=0 admission_passed=0
...
fge_context covered 'quarantine discard proven non-vacuously (real bytes buffered then asserted gone); cancellation contract exactly as its owner specified; quota bound refused past and accepted at; pre-seal refusal boundary with its documented delete-only twin'
...
fge_context not_yet 'admit_validated_receive needs an AuthorityStore and AdmissionProjection fixture; the concurrent-push race corpus uses TurquoiseDog FG-007b reproducers and is attributed to verifier 8fed725 unless the push path adds a distinct failure shape'
