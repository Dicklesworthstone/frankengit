...
- the **primary web UI**: a familiar, GitHub-like browser experience built in a DOM-oriented pure-Rust to WebAssembly framework (Leptos or Dioxus, an open ADR), server-side-rendered with hydration and styled with Tailwind. Because Leptos/Dioxus render the **real DOM** (not a canvas), text selection, find-in-page, screen-reader accessibility, international input, native scroll, and SSR / SEO for public repositories all behave like any HTML page; the text-selection/a11y objection applies to canvas-painted WASM (egui/ftui), not to this. Most users see this;
...
Staying in one Rust universe is the point: the canonical `fgit-codec` types, franken_markdown as the one source-spanned renderer (so text rendering and sanitization never drift from the server, plan 5.1/28.1), and the verified-read verifier (plan 18.7) are NATIVE code in the same WebAssembly build (not FFI'd into JavaScript), so the browser cryptographically checks Merkle inclusion proofs against a trusted head and a mirror or CDN cannot lie to the client, with no npm frontend toolchain in the first-party path. The honest cost, recorded with eyes open, is a smaller component ecosystem than React and a smaller contributor pool: a polished GitHub-like look is more first-party design/component work (achievable with Tailwind), and WebAssembly's performance edge is concentrated on the heavy forge screens (large diffs, big trees, blame, highlighting, rendering, verification) rather than the chrome. As a no-loss guarantee, a generated TypeScript client (from the schema registry, plan 42.4) plus a React reference front-end remain a permanently supported ALTERNATIVE, so third parties and JavaScript contributors keep a first-class path. The terminal TUI and the optional ftui web skin keep the ftui look; what all surfaces share is the Rust substrate, not one appearance.
...
- `fgit-web`: the PRIMARY browser UI, a familiar GitHub-like DOM-oriented pure-Rust to WebAssembly app (Leptos or Dioxus by ADR, SSR + Tailwind, real DOM), with franken_markdown rendering and the verified-read verifier as native Rust in the same build; a generated TypeScript client + a React reference are the supported alternative; NOT a canvas-painted UI;
