...
impl OneNode {
    pub(super) fn prepare_visible_upload_pack(
        ...
        let repository = AdmissionUploadPackRepository::from_snapshot(
            materialized.snapshot(), self.object_format, limits,
        ...
        let source = VerifiedFabricPackSource {
            ...
            maximum_object_bytes: self.selected_pack_limits.max_object_bytes
        ...
        let closure = project_visible_closure(
            &source, materialized.selected_closure().closure(),
            repository.advertised_refs().iter().map(|reference| reference.oid),
        ...
        if let Some(dimension) = exhaustion.get() {
            return Err(NodePackMaterializationRefusal::BudgetClassExhausted {
                class: BudgetClass::Database, dimension, operation: DISCLOSURE_OPERATION,
            }.into());
        }
        let closure = closure?;
        let repository = repository.with_closure_objects(closure.objects().clone());
        Ok(VisibleUploadPack { basis: materialized.basis().clone(), closure, repository })
    }
}
...
impl VisibilitySource for VerifiedFabricPackSource<'_> {
    fn format(&self) -> GitHashAlgorithm { self.object_format }
    fn parse_limits(&self) -> ParseLimits { VerifiedFabricPackSource::parse_limits(self) }
    fn checkpoint(&self) -> Result<(), NodePackMaterializationRefusal> {
        self.session_checkpoint()?;
        if !self.database_read_is_live() { return Err(PackWriteError::from(PackError::DeadlineExceeded).into()); }
        Ok(())
    }
    fn load(&self, id: GitOid) -> Result<(ObjectType, Vec<u8>), NodePackMaterializationRefusal> {
        self.read_object(&id).map_err(NodePackMaterializationRefusal::from)
    }
}
...
fn project_visible_closure(
    ...
    for root in roots {
        source.checkpoint()?;
        enqueue(source.format(), admitted, root, None, maximum, &mut required, &mut pending)?;
    }
    ...
    while let Some(id) = pending.pop_first() {
        ...
        if let Some(Some(expected)) = required.get(&id) { require_kind(kind, *expected)?; }
        // Blobs have no local edges. Avoid a second full-body parsed copy.
        let edges = if kind == ObjectType::Blob {
            Vec::new()
        } else {
            let parsed = parse_object_body(kind, &body, AcceptanceProfile::GitCompatibleImport, &parse_limits);
            source.checkpoint()?;
            let parsed = parsed.map_err(|_| disclosure_refusal(RefusalCode::ObjectHeaderInvalid))?;
            let mut stopped = None;
            let edges = crate::loose_import::graph::references(
                source.format(), &parsed, &body, &parse_limits, &mut edges_left,
                &mut || {
                    if stopped.is_some() { return false; }
            ...
            if let Some(error) = stopped { return Err(error); }
            source.checkpoint()?;
        ...
        for (child, expected) in edges {
            ...
            if let Some(actual) = known.get(&child) { require_kind(*actual, expected)?; }
            enqueue(source.format(), admitted, child, Some(expected), maximum, &mut required, &mut pending)?;
        }
    }
    source.checkpoint()?;
...
fn enqueue(
    ...
    if let Some(previous) = required.get_mut(&id) {
        if let (Some(first), Some(next)) = (*previous, expected) { require_kind(first, next)?; }
...
fn require_kind(actual: ObjectType, expected: ObjectType) -> Result<(), NodePackMaterializationRefusal> {
    if actual == expected { Ok(()) }
    else { Err(disclosure_refusal(RefusalCode::EvidenceInvalid)) }
}
