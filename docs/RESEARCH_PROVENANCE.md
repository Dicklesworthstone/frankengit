...
Three further FrankenSuite siblings are the settled product-surface stack. Source review confirmed their intended production paths are pure Rust and Asupersync-native, but their reviewed revisions are not yet one admissible version/path constellation: fastapi_rust and FrankenTUI still require Asupersync 0.3.x; sqlmodel_rust pins 0.4.4 and carries absolute local patches to an older FrankenSQLite; current FrankenSQLite accepts Asupersync 0.4.x. The owned sibling repositories will be updated before FrankenGit integration. The gate must see one runtime version and registry-resolvable sources; this prerequisite does not reopen the adoption decision.

### fastapi_rust
Mechanisms adopted: FastAPI-inspired typed routing, zero-copy request parsing, deterministic testing, and OpenAPI generation (`fastapi-openapi`)—all on Asupersync. FrankenGit application: the `fgit-gateway`/`fgit-api` layer; its OpenAPI generation feeds the schema registry (one schema source, no handwritten wire structs). Its upstream Asupersync dependency must advance to the selected 0.4.x contract before it enters the FrankenGit lockfile.

### sqlmodel_rust
Mechanisms adopted: SQLModel-style derive-macro models, compile-time-checked query builders, and migrations—via the `sqlmodel-frankensqlite` backend. FrankenGit application: the type-safe substrate for DERIVED PROJECTION read-models over FrankenSQLite. Boundary (critical): projections only, never canonical authority (the head-CAS decision stream is the sole source of truth); and ONLY the FrankenSQLite backend is admitted—the `sqlmodel-sqlite` (C `libsqlite3-sys`), `-postgres`, and `-mysql` backends are excluded. Before integration, the sibling removes its absolute path patches, aligns with the admitted FrankenSQLite release/runtime, and proves bounded connection-worker ownership, explicit close, whole-transaction retry, watermark, migration, cancellation, and deterministic rebuild behavior.
