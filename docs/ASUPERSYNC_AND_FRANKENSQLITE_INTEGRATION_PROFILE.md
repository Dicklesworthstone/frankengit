...
**Last source review:** 2026-08-20; §1 `fastapi_rust` and `sqlmodel_rust` rows re-audited against crates.io-published artifacts on 2026-08-23 (FG-093a)
...
| sqlmodel_rust | `23da90cdfe057162d436417bbc4af7ec206ed48e`; published family v0.4.0 (`aa1c6ec`) | converged: all 8 family crates request Asupersync `^0.4.9` with `default-features = false`, no `[patch]`/absolute paths, facade default features empty; NOT converged: published `sqlmodel-frankensqlite` requests `fsqlite`/`fsqlite-core`/`fsqlite-types` without `default-features = false`, so admission would union `extensions`/io_uring/json/fts5/rtree/icu/misc into every consumer (§3.2 violation; Cargo has no consumer-side downgrade) | adopted projection substrate; one precise prerequisite remains: publish a release requesting the fsqlite family with `default-features = false` + explicit minimal set (`async-api`, `native`). Gate: `check_sqlmodel_substrate_feature_profile` + `suites/checker/sqlmodel_admission_gate.sh` |
