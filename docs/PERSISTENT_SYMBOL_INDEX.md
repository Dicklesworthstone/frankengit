...
fg-symbol-index "$NODE_ROOT" "$TENANT_HEX" "$REPOSITORY_HEX" sha1 \
  refs/heads/main build "$INDEX_TOKEN"
fg-symbol-index "$NODE_ROOT" "$TENANT_HEX" "$REPOSITORY_HEX" sha1 \
...
registered `alg:CODE:LOWERCASE_HEX` form. `genesis` requires an uninitialized
symbol index; subsequent builds require its exact predecessor token. There is
no implicit `latest`, force, background maintenance, or retry with a new basis.
...
Every table and manifest is staged before the existing generation-root
conditional write. Namespace keys include tenant, repository, incarnation and
object format; each ref has its own generation head. This derived view is named
`source-rust-symbols`, with schema `source-symbol-index` 1.0 and a named parser/
index profile. Its document directory, declaration-table directory and source
metadata are one canonical manifest, so all four graph payload-root fields bind
that same manifest. They are not four independently materialized graphs, and no
call/reference graph is implied. Table and manifest frames use distinct schema
families in the existing generation identity domain.

Native `build_source_symbol_index_guarded_local_in` additionally accepts a
bounded synchronous write-ahead callback. It receives the original candidate
before any table/manifest put or root write; returning an error prevents those
effects. The unguarded API and simple CLI do not themselves create durable
controller progress. Publication errors after that barrier retain the original
candidate, including cancellation before the first put. Confirmed publication
has no following cancellation probe or await. The CLI shuts down the node on
operation success or failure; output and shutdown failures never imply rollback.
...
No indexed read builds or activates an index; there is no HTTP write route.
...
Even a forge-only write makes an old index stale; rebuilding is an explicit
operator action. An uninitialized index, stale index and unresolved checkpoint
return distinct HTTP 409 codes (`symbol_index_uninitialized`,
...
source refuse a build; they are not silently skipped. Full Rust or language-
server correctness is not claimed.

Tables have at most 20,000 declarations across the complete build, 128-byte
...
Builds currently rescan the complete Rust corpus. Identical per-blob payloads
can receive idempotent puts; incremental parser reuse, delta compaction and the
existing lexical maintenance worker are not integrated for symbols yet. There
is no browser symbol UI, multi-language symbol graph or semantic ranking.

At commit 6da56f30e9ce314be5d1d4ad5807e4e9e5a87a5a, the repository-owned standalone
lane compiled the actual production scanner/table modules on nightly-2026-08-31:
33 tests passed, 0 failed, 0 ignored (21 retained scanner tests plus 12 new table
tests). This is not execution of the canonical frame, TreeFS, node, HTTP or
FrankenSQLite integration. Six additional core index tests, eleven native
integration tests, three indexed HTTP protocol tests and three operator tests
are supplied separately. Native tests use actual node imports, patch admission,
storage/reopen, TCP listener and operator binaries, not substitute backends.
