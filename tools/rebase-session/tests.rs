...
struct Source {
    ...
    polls: Cell<usize>, stop: Cell<usize>,
}
impl Source {
    fn new(format: GitHashAlgorithm) -> Self {
        Self { format, commits: BTreeMap::new(), metadata: BTreeMap::new(), trees: BTreeMap::new(),
            blobs: BTreeMap::new(), polls: Cell::new(0), stop: Cell::new(usize::MAX) }
    }
    fn tree(&mut self, bytes: &[u8]) -> GitOid {
        ...
        let mut body = b"100644 file\0".to_vec(); body.extend_from_slice(oid.as_bytes());
        let tree = git_object_id(self.format, GitObjectKind::Tree, &body);
        self.trees.insert(tree, vec![MergeEntry { name: b"file".to_vec(), mode: 0o100644, oid }]);
        tree
    }
    fn commit(&mut self, bytes: &[u8], parents: &[GitOid], message: &[u8]) -> GitOid {
        let tree = self.tree(bytes);
        let data = RebaseCommitMetadata { author: b"Original <original@example.invalid> 17 -0430".to_vec(),
            encoding: Some(b"ISO-8859-1".to_vec()), message: message.to_vec() };
        let mut body = format!("tree {tree}\n").into_bytes();
        for parent in parents { body.extend_from_slice(format!("parent {parent}\n").as_bytes()); }
        body.extend_from_slice(b"author "); body.extend_from_slice(&data.author);
        body.extend_from_slice(b"\ncommitter Old <old@example.invalid> 20 +0000\nencoding ISO-8859-1\n\n");
        body.extend_from_slice(message);
        let id = git_object_id(self.format, GitObjectKind::Commit, &body);
        self.commits.insert(id, CommitInput { tree, parents: parents.to_vec() });
        self.metadata.insert(id, data); id
    }
}
impl MergeObjectSource for Source {
    fn checkpoint(&self) -> Result<(), MergeSourceError> {
        self.polls.set(self.polls.get()+1);
        if self.polls.get() >= self.stop.get() { Err(MergeSourceError::Cancelled) } else { Ok(()) }
    }
...
impl RebaseObjectSource for Source {
    fn rebase_metadata(&self, id: GitOid) -> Result<RebaseCommitMetadata, MergeSourceError> {
        self.metadata.get(&id).cloned().ok_or(MergeSourceError::Unavailable(id))
    }
}
fn committer() -> RebaseCommitter { RebaseCommitter { identity: "Rebaser <rebaser@example.invalid>".into(), timestamp: 100 } }
const BASE: &[u8] = b"a\nb\nc\nd\ne\nf\n";
const ONTO: &[u8] = b"A\nb\nc\nd\ne\nf\n";
fn fixture(format: GitHashAlgorithm) -> (Source, RebaseRequest, Vec<GitOid>) {
    ...
    (source, RebaseRequest { source_tip: third, upstream, onto, empty: EmptyCommitPolicy::Stop }, vec![first,second,third])
}
fn run(source: &Source, request: RebaseRequest, limits: PreparationLimits) -> Result<RebasePreparation, RebaseError> {
    prepare_rebase(source, source.format, request, &committer(), limits)
}
fn clean(source: &Source, request: RebaseRequest) -> PreparedRebase {
    let RebasePreparation::Clean(plan) = run(source, request, PreparationLimits::default()).unwrap() else { panic!("clean rebase expected"); };
    plan
}
...
fn successive_replays_read_generated_trees_and_blobs_and_preserve_native_metadata() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(plan.steps.iter().map(|s| s.original).collect::<Vec<_>>(), originals);
        let mut parent = request.onto;
        for step in &plan.steps {
            let object = plan.objects.iter().find(|o| o.id == step.rewritten).unwrap();
        ...
        assert!(plan.objects.iter().any(|o| o.id == blob && o.body == b"A\nb\nC\nd\ne\nFF\n"));
...
fn one_shared_content_tree_and_output_budget_covers_the_complete_series() {
    ...
    assert!(matches!(run(&source, request, PreparationLimits { max_content_merges:1,..PreparationLimits::default() }),
        Err(RebaseError::Preparation(PreparationError::Budget("content merges")))));
    let full = clean(&source, request);
    let total: usize = full.objects.iter().map(|o| o.body.len()).sum();
    assert!(matches!(run(&source, request, PreparationLimits { max_output_bytes:total-1,..PreparationLimits::default() }),
        Err(RebaseError::Preparation(PreparationError::Budget(_)))));
    let RebasePreparation::Clean(exact) = run(&source, request, PreparationLimits { max_output_bytes:total,..PreparationLimits::default() }).unwrap() else { panic!(); };
    assert_eq!(exact, full);
    assert!(matches!(run(&source, request, PreparationLimits { max_tree_entries:3,..PreparationLimits::default() }),
        Err(RebaseError::Preparation(PreparationError::Budget("tree entries")))));
}
...
fn original_empty_commits_survive_while_newly_empty_changes_require_a_policy() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let mut request = RebaseRequest { source_tip:empty,upstream,onto:already,empty:EmptyCommitPolicy::Stop };
        assert!(matches!(run(&source,request,PreparationLimits::default()).unwrap(),
...
fn later_conflict_returns_only_a_stop_and_not_a_partially_publishable_plan() {
    ...
    request.source_tip = source.commit(b"other top\nb\nc\nd\ne\nF\n",&[originals[0]],b"conflicting second");
    let result = run(&source,request,PreparationLimits::default()).unwrap();
    let RebasePreparation::Stopped { original,completed,reason:RebaseStop::Conflicted(conflicts),.. }=result else { panic!(); };
...
fn unrelated_upstream_merge_suffix_missing_history_and_cycles_refuse() {
    ...
    assert!(matches!(run(&source,request,PreparationLimits::default()),Err(RebaseError::UpstreamOutsideLinearHistory)));
    ...
    assert!(matches!(run(&source,request,PreparationLimits::default()),Err(RebaseError::MergeCommit { parents:2,.. })));
    request.source_tip = originals[2];
    source.commits.get_mut(&originals[0]).unwrap().parents = vec![originals[2]];
    assert!(matches!(run(&source,request,PreparationLimits::default()),Err(RebaseError::CyclicHistory(_))));
    source.commits.remove(&originals[0]);
    assert!(matches!(run(&source,request,PreparationLimits::default()),Err(RebaseError::Preparation(PreparationError::Source(MergeSourceError::Unavailable(_))))));
}

#[test]
fn commit_and_edge_bounds_apply_to_topology_before_merging() {
    let (source, request, _) = fixture(GitHashAlgorithm::Sha1);
    for limits in [PreparationLimits { max_commits:2,..PreparationLimits::default() },
        PreparationLimits { max_edges:1,..PreparationLimits::default() }] {
        assert!(matches!(run(&source,request,limits),Err(RebaseError::Preparation(PreparationError::Budget(_)))));
    }
}
...
fn every_checkpoint_cancellation_refuses_the_whole_rebase() {
    ...
    for stop in 1..=polls {
        source.polls.set(0);source.stop.set(stop);
        assert!(matches!(run(&source,request,PreparationLimits::default()),
            Err(RebaseError::Preparation(PreparationError::Source(MergeSourceError::Cancelled)))));
    }
}
...
fn empty_range_and_all_dropped_range_use_onto_without_manufacturing_a_commit() {
    ...
    let plan=clean(&source,RebaseRequest { source_tip:original,upstream,onto,empty:EmptyCommitPolicy::Drop });
...
fn malformed_original_headers_and_cross_hash_inputs_never_become_commits() {
    ...
    source.metadata.get_mut(&originals[0]).unwrap().author=b"Author <a@x> 1 +0000\nparent injected".to_vec();
    assert!(matches!(run(&source,request,PreparationLimits::default()),Err(RebaseError::InvalidOriginalMetadata(_))));
    let foreign=git_object_id(GitHashAlgorithm::Sha256,GitObjectKind::Commit,b"foreign");
    assert!(matches!(run(&source,RebaseRequest { onto:foreign,..request },PreparationLimits::default()),
