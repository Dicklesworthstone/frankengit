...
commands=[['cargo','test','--locked','-p','fgit-forge','-p','fgit-reference','-p','fgit-schema','--all-targets'],['cargo','check','--locked','-p','fgit-cli','--all-targets']]
with tempfile.TemporaryDirectory(prefix='fg-issue-verify-') as td:
    for index,args in enumerate(commands):
        ...
        print('\n'.join(lines[-180:] if code else [s for s in lines if 'issue::' in s or 'Finished ' in s]),flush=True)
