...
use fgit_wire::{AdvertisedRef, AnyGitOid, Capabilities, GitObjectFormat, LegacyUploadPack, PackRequest, Packet, UploadPackRepository, UploadPackVersion, V2UploadPack, WireError, WireEvent, WireLimits, encode_packets};
...
impl UploadPackRepository for Repository {
    ...
    fn contains_want(&self, id: AnyGitOid) -> bool { self.permitted.contains(&id) }
    fn is_common(&self, _: AnyGitOid) -> bool { false }
    fn supports_shallow(&self) -> bool { true }
    fn shallow_update(&self, _: &PackRequest) -> Result<ShallowUpdate, WireError> {
        self.calls.set(self.calls.get() + 1);
        if self.refuse { Err(WireError::PackSourceRefused) } else { Ok(self.update.clone()) }
    }
}
...
fn repository(format: GitObjectFormat) -> Repository {
    Repository {
        format, refs: vec![AdvertisedRef::new(oid(format, 1), b"refs/heads/main", &WireLimits::default()).unwrap()],
        permitted: (1..=3).map(|byte| oid(format, byte)).collect(),
        update: ShallowUpdate { shallow: vec![oid(format, 2)], unshallow: vec![oid(format, 1)] },
        refuse: false, calls: Cell::new(0),
    }
}
fn legacy(version: UploadPackVersion, limits: WireLimits, multi: bool) -> LegacyUploadPack {
    let tokens = if multi { b"shallow multi_ack_detailed".as_slice() } else { b"shallow" };
    LegacyUploadPack::new(version, Capabilities::parse_v1(tokens, &limits).unwrap(), limits).unwrap().with_shallow_updates()
}
fn legacy_request(machine: &mut LegacyUploadPack, repository: &Repository, multi: bool) {
    ...
    for packet in [line(format!("want {}{suffix}", oid(repository.format, 1))), line(format!("shallow {}", oid(repository.format, 1))), line("deepen 2")] {
        assert!(machine.push_packet(&packet, repository).unwrap().output.is_empty());
    }
}
fn v2(limits: WireLimits) -> V2UploadPack {
    let caps = Capabilities::parse_v2_advertisement(&[line("version 2"), line("fetch=shallow"), Packet::Flush], &limits).unwrap();
    V2UploadPack::new(caps, limits).unwrap().with_shallow_updates()
}
fn v2_request(machine: &mut V2UploadPack, repository: &Repository) {
    for packet in [line("command=fetch"), Packet::Delimiter, line(format!("want {}", oid(repository.format, 1))), line(format!("shallow {}", oid(repository.format, 1))), line("deepen 2"), line("done")] {
        assert!(machine.push_packet(&packet, repository).unwrap().output.is_empty());
    }
}
    ...
    for format in [GitObjectFormat::Sha1, GitObjectFormat::Sha256] {
        for version in [UploadPackVersion::V0, UploadPackVersion::V1] {
            for multi in [false, true] {
                ...
                assert_eq!(transition.output, vec![line(format!("shallow {}", oid(format, 2))), line(format!("unshallow {}", oid(format, 1))), Packet::Flush]);
                ...
                let [WireEvent::PackRequested(request)] = transition.events.as_slice() else { panic!("pack handoff is required"); };
...
fn legacy_empty_boundary_update_still_flushes_before_haves() {
    ...
    repository.update = ShallowUpdate { shallow: vec![], unshallow: vec![] };
    let mut machine = legacy(UploadPackVersion::V0, WireLimits::default(), false);
    legacy_request(&mut machine, &repository, false);
    assert_eq!(machine.push_packet(&Packet::Flush, &repository).unwrap().output, vec![Packet::Flush]);
}
...
fn v2_shallow_info_is_delimited_before_the_packfile_header() {
    for format in [GitObjectFormat::Sha1, GitObjectFormat::Sha256] {
        ...
        let transition = machine.push_packet(&Packet::Flush, &repository).unwrap();
        assert_eq!(transition.output, vec![line("shallow-info"), line(format!("shallow {}", oid(format, 2))), line(format!("unshallow {}", oid(format, 1))), Packet::Delimiter, line("packfile")]);
        assert_eq!(repository.calls.get(), 1);
        assert!(matches!(transition.events.as_slice(), [WireEvent::PackRequested(request)] if request.deepen == Some(2)));
    }
}
...
fn provider_failure_does_not_advance_the_legacy_machine_or_emit_success() {
    ...
    let mut machine = legacy(UploadPackVersion::V0, WireLimits::default(), false);
    legacy_request(&mut machine, &repository, false);
    assert_eq!(machine.push_packet(&Packet::Flush, &repository), Err(WireError::PackSourceRefused));
...
fn foreign_hidden_duplicate_overlapping_or_unrequested_boundaries_are_refused() {
    ...
    for update in [
        ShallowUpdate { shallow: vec![oid(format, 9)], unshallow: vec![] },
        ShallowUpdate { shallow: vec![oid(format, 2), oid(format, 2)], unshallow: vec![] },
        ShallowUpdate { shallow: vec![oid(format, 3), oid(format, 2)], unshallow: vec![] },
        ShallowUpdate { shallow: vec![oid(format, 1)], unshallow: vec![oid(format, 1)] },
        ShallowUpdate { shallow: vec![], unshallow: vec![oid(format, 2)] },
        ShallowUpdate { shallow: vec![oid(GitObjectFormat::Sha256, 2)], unshallow: vec![] },
    ] {
...
fn response_count_and_byte_limits_are_enforced_before_success() {
    for limits in [WireLimits { max_shallows: 1, ..WireLimits::default() }, WireLimits { max_outbound_bytes: 8, ..WireLimits::default() }] {
        let mut repository = repository(GitObjectFormat::Sha1);
...
fn fragmented_legacy_requests_resolve_once_and_keep_boundaries_before_nak() {
    ...
    let packets = [line(format!("want {}", oid(repository.format, 1))), line(format!("shallow {}", oid(repository.format, 1))), line("deepen 2"), Packet::Flush, line("done")];
    ...
    assert_eq!(repository.calls.get(), 1);
    assert_eq!(output, vec![line(format!("shallow {}", oid(repository.format, 2))), line(format!("unshallow {}", oid(repository.format, 1))), Packet::Flush, line("NAK")]);
...
fn parser_only_mode_does_not_pretend_to_have_resolved_graph_updates() {
    ...
    legacy_request(&mut machine, &repository, false);
    assert!(machine.push_packet(&Packet::Flush, &repository).unwrap().output.is_empty());
    assert_eq!(repository.calls.get(), 0);
    assert!(matches!(machine.push_packet(&line("done"), &repository).unwrap().events.as_slice(), [WireEvent::PackRequested(request)] if request.deepen == Some(2)));
}
