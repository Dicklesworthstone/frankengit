...
impl Graph {
    fn new(format: GitHashAlgorithm) -> Self { Self { format, objects: BTreeMap::new(), next: 1 } }
    fn add(&mut self, kind: ObjectType, edges: Vec<(GitOid, ObjectType)>) -> GitOid {
        ...
        self.next += 1;
        self.objects.insert(id, FilterObject { kind, size: 4, edges });
        id
    }
...
fn request(want: GitOid, depth: Option<u32>, shallows: Vec<GitOid>, haves: Vec<GitOid>) -> PackRequest {
    PackRequest {
        version: UploadPackVersion::V2, wants: vec![want], haves, shallows,
        deepen: depth, deepen_since: None, deepen_not: Vec::new(), filter: None, options: PackOptions::NONE,
    }
}
...
fn depth_one_contains_only_the_tip_and_its_tree_but_never_ancestor_bodies() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(update(&graph, &request), ShallowUpdate { shallow: vec![tip], unshallow: vec![] });
    }
}
...
fn a_have_at_the_old_boundary_does_not_subtract_the_ancestors_being_deepened() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        assert_eq!(ids(&graph, &request), BTreeSet::from([middle.0, middle.1, middle.2]));
        assert_eq!(update(&graph, &request), ShallowUpdate { shallow: vec![middle.0], unshallow: vec![tip.0] });
    }
}
...
fn unshallow_supplies_missing_history_and_removes_only_crossed_client_boundaries() {
    for format in [GitHashAlgorithm::Sha1, GitHashAlgorithm::Sha256] {
        ...
        let request = request(tip.0, Some(2_147_483_647), vec![middle.0, unrelated.0], vec![tip.0]);
        assert_eq!(ids(&graph, &request), BTreeSet::from([root.0, root.1, root.2]));
        assert_eq!(update(&graph, &request), ShallowUpdate { shallow: vec![], unshallow: vec![middle.0] });
    }
}
...
fn ordinary_fetch_preserves_existing_shallow_history_without_unshallowing_it() {
    ...
    assert_eq!(update(&graph, &request), ShallowUpdate { shallow: vec![], unshallow: vec![] });
}
...
fn a_natural_root_at_the_exact_depth_is_shallow_but_a_deeper_request_is_not() {
    let mut graph = Graph::new(GitHashAlgorithm::Sha1);
    let root = graph.commit(&[]);
    assert_eq!(update(&graph, &request(root.0, Some(1), vec![], vec![])).shallow, vec![root.0]);
    assert!(update(&graph, &request(root.0, Some(2), vec![], vec![])).shallow.is_empty());
}
...
fn annotated_tags_and_explicit_tree_or_blob_wants_survive_history_selection() {
    ...
    assert_eq!(ids(&graph, &request), BTreeSet::from([outer, inner, tip.0, tip.1, tip.2, root.1, root.2]));
...
fn unknown_client_markers_are_not_disclosed_and_never_authorize_wants() {
    ...
    assert!(matches!(select(&graph.objects, &request, &PackLimits::default(), &mut || true),
...
fn depth_selection_composes_with_blobless_and_treeless_filters_and_lazy_roots() {
    ...
    for (filter, expected) in [
        ...
        let mut selection = select(&graph.objects, &request, &PackLimits::default(), &mut || true).unwrap();
        request.deepen = None;
        request.filter = Some(filter);
        super::super::partial_clone::apply_selection(&graph.objects, &mut selection, &request, &PackLimits::default(), &mut || true).unwrap();
...
fn every_cancellation_checkpoint_refuses_without_returning_a_partial_selection() {
    ...
    let request = request(tip.0, Some(2), vec![tip.0], vec![tip.0]);
    let mut polls = 0;
    select(&graph.objects, &request, &PackLimits::default(), &mut || { polls += 1; true }).unwrap();
    for stop in 1..=polls {
        let mut seen = 0;
        assert!(matches!(select(&graph.objects, &request, &PackLimits::default(), &mut || {
            seen += 1; seen < stop
...
fn bounds_and_unsupported_controls_fail_before_returning_a_selection() {
    ...
    for limits in [PackLimits { max_entries: 2, ..PackLimits::default() }, PackLimits { max_delta_work: 1, ..PackLimits::default() }] {
        assert!(matches!(select(&graph.objects, &request, &limits, &mut || true),
            Err(NodePackMaterializationRefusal::DisclosureGraph(RefusalCode::ResourceBudgetExceeded))));
    }
    request.deepen_since = Some(1);
    assert!(matches!(select(&graph.objects, &request, &PackLimits::default(), &mut || true),
        Err(NodePackMaterializationRefusal::UnsupportedFetch("time/ref shallow boundaries"))));
    request.deepen_since = None;
    request.shallows.push(tip.2);
    assert!(matches!(select(&graph.objects, &request, &PackLimits::default(), &mut || true),
        Err(NodePackMaterializationRefusal::DisclosureGraph(RefusalCode::EvidenceInvalid))));
}
