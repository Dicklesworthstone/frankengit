...
impl ShallowProof {
    pub(super) fn new(
        ...
        Self { objects, limits, deadline }
    }

    pub(crate) fn update(&self, request: &PackRequest) -> Result<ShallowUpdate, WireError> {
        boundary_update(&self.objects, request, &self.limits, &mut || !self.deadline.expired())
...
impl<'a, F: FnMut() -> bool> Work<'a, F> {
    ...
    fn count(&self, count: usize) -> Result<(), NodePackMaterializationRefusal> {
        if count > self.maximum { Err(budget()) } else { Ok(()) }
    }
...
fn history(
    ...
    if request.deepen == Some(0) {
        return Err(NodePackMaterializationRefusal::UnsupportedFetch("zero shallow depth"));
    }
    if request.deepen_since.is_some() || !request.deepen_not.is_empty() {
        return Err(NodePackMaterializationRefusal::UnsupportedFetch("time/ref shallow boundaries"));
    }
    ...
    while let Some((depth, id)) = pending.pop_first() {
        ...
        match item.kind {
            ...
            ObjectType::Commit => {
                ...
                let boundary = request.deepen.map_or_else(
                    || old.contains(&id),
...
fn enqueue(
    ...
    work.count(depths.len().checked_add(usize::from(!depths.contains_key(&id))).ok_or_else(budget)?)?;
...
fn walk(
    ...
    for &id in roots {
        work.tick()?;
        if ignore_unknown_roots && !objects.contains_key(&id) { continue; }
        work.insert(&mut pending, id)?;
    }
    while let Some(id) = pending.pop_first() {
        work.tick()?;
        if !work.insert(&mut seen, id)? { continue; }
        let item = object(objects, id)?;
        for &(child, kind) in &item.edges {
            work.tick()?;
            if item.kind == ObjectType::Commit && kind == ObjectType::Commit && boundaries.contains(&id) {
                continue;
            }
            if !seen.contains(&child) { work.insert(&mut pending, child)?; }
        }
    }
    Ok(seen)
}
...
pub(super) fn select(
    ...
    let desired = walk(objects, &request.wants, &history.boundaries, false, &mut work)?;
    let known = walk(objects, &request.haves, &history.old, true, &mut work)?;
    let mut result = Vec::new();
    result.try_reserve_exact(desired.len()).map_err(|_| budget())?;
    for id in desired {
        work.tick()?;
        if !known.contains(&id) { result.push(id); }
    }
    work.tick()?;
    Ok(result)
}
