...
fn invalid(text: &[u8]) -> WireError {
    WireError::InvalidFilter { filter: text.to_vec() }
}
fn parts_limit(limits: &WireLimits) -> WireError {
    WireError::TooManyFilterParts { limit: limits.max_filter_parts }
}

fn parse_part(
    text: &[u8], format: GitObjectFormat, limits: &WireLimits,
    depth: usize, remaining: &mut usize,
) -> Result<ObjectFilter, WireError> {
    ...
    if let Some(compound) = text.strip_prefix(b"combine:") {
        if compound.is_empty() { return Err(invalid(text)); }
        let mut parts = Vec::new();
        for encoded in compound.split(|byte| *byte == b'+') {
            if encoded.is_empty() { return Err(invalid(text)); }
            if *remaining == 0 { return Err(parts_limit(limits)); }
            let decoded = decode_part(encoded)?;
            let part = parse_part(&decoded, format, limits, depth + 1, remaining)?;
            parts.try_reserve(1).map_err(|_| WireError::AllocationFailure)?;
            parts.push(part);
        }
        return Ok(ObjectFilter::Combine(parts));
    }
    *remaining = remaining.checked_sub(1).ok_or_else(|| parts_limit(limits))?;
    if text == b"blob:none" { return Ok(ObjectFilter::BlobNone); }
    if let Some(value) = text.strip_prefix(b"blob:limit=") {
        return scaled_size(value).map(ObjectFilter::BlobLimit).ok_or_else(|| invalid(text));
    }
    if let Some(value) = text.strip_prefix(b"tree:") {
        return parse_unsigned(value).ok().and_then(|value| u32::try_from(value).ok())
            .map(ObjectFilter::TreeDepth).ok_or_else(|| invalid(text));
    }
...
fn decode_part(encoded: &[u8]) -> Result<Vec<u8>, WireError> {
    ...
    decoded.try_reserve_exact(encoded.len()).map_err(|_| WireError::AllocationFailure)?;
    let mut offset = 0;
    while offset < encoded.len() {
        ...
        if byte == b'%' {
            ...
            let (Some(high), Some(low)) = (high, low) else { return Err(invalid(encoded)); };
