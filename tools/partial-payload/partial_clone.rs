...
impl Predicate {
    fn compile(
        filter: Option<&ObjectFilter>, work: &mut Work<'_, impl FnMut() -> bool>,
    ) -> Result<Self, NodePackMaterializationRefusal> {
        let mut result = Self::default();
        let mut pending = Vec::new();
        if let Some(filter) = filter { pending.push(filter); }
        while let Some(filter) = pending.pop() {
            work.tick()?;
            match filter {
                ObjectFilter::BlobNone => result.blob_limit = Some(0),
                ObjectFilter::BlobLimit(limit) => {
                    result.blob_limit = Some(result.blob_limit.map_or(*limit, |old| old.min(*limit)));
                }
                ObjectFilter::TreeDepth(depth) => {
                    result.tree_depth = Some(result.tree_depth.map_or(*depth, |old| old.min(*depth)));
                }
                ObjectFilter::Combine(parts) => {
                    ...
                    pending.try_reserve(parts.len()).map_err(|_| disclosure_refusal(RefusalCode::ResourceBudgetExceeded))?;
                    pending.extend(parts.iter().rev());
                }
                ObjectFilter::SparseObject(_) | ObjectFilter::SparsePath(_) => {
                    return Err(NodePackMaterializationRefusal::UnsupportedFetch("sparse filters"));
                }
            }
        }
        Ok(result)
    }
    fn permits(&self, object: &FilterObject, depth: Option<u32>) -> Result<bool, NodePackMaterializationRefusal> {
        if object.kind == ObjectType::Blob
            && self.blob_limit.is_some_and(|limit| u64::try_from(object.size).unwrap_or(u64::MAX) >= limit)
        { return Ok(false); }
        if matches!(object.kind, ObjectType::Blob | ObjectType::Tree)
            && let Some(maximum) = self.tree_depth
        {
            return depth.map(|depth| depth < maximum)
...
fn object(
    objects: &BTreeMap<GitOid, FilterObject>, id: GitOid,
) -> Result<&FilterObject, NodePackMaterializationRefusal> {
    objects.get(&id).ok_or_else(|| disclosure_refusal(RefusalCode::ObjectClosureIncomplete))
}
...
pub(super) fn apply_selection(
    objects: &BTreeMap<GitOid, FilterObject>, ids: &mut Vec<GitOid>,
    request: &PackRequest, limits: &PackLimits, live: &mut impl FnMut() -> bool,
) -> Result<(), NodePackMaterializationRefusal> {
    let mut work = Work {
        live, remaining: limits.max_delta_work,
        maximum: usize::try_from(limits.max_entries).unwrap_or(usize::MAX),
    };
    work.tick()?;
    if !request.shallows.is_empty() || request.deepen.is_some()
        || request.deepen_since.is_some() || !request.deepen_not.is_empty()
    { return Err(NodePackMaterializationRefusal::UnsupportedFetch("shallow history")); }
    ...
    for &id in &request.wants {
        ...
        if !objects.contains_key(&id) {
            return Err(NodePackMaterializationRefusal::RequestedWantOutsideClosure(id));
        }
        explicit.insert(id);
    }
    ...
    for &id in &explicit {
        ...
        if !direct_haves.contains(&id)
            && matches!(object(objects, id)?.kind, ObjectType::Blob | ObjectType::Tree)
        { frontier.insert(id); }
    }
    let mut restored = BTreeSet::new();
    while let Some(id) = frontier.pop_first() {
        work.tick()?;
        if !restored.insert(id) { continue; }
        ...
        for &(child, _) in &object(objects, id)?.edges {
            ...
            if !restored.contains(&child) {
                work.count(frontier.len().saturating_add(usize::from(!frontier.contains(&child))))?;
    ...
    result.try_reserve_exact(selected.len())
        .map_err(|_| disclosure_refusal(RefusalCode::ResourceBudgetExceeded))?;
    for id in selected {
        work.tick()?;
        if explicit.contains(&id) || predicate.permits(object(objects, id)?, depths.get(&id).copied())? {
            result.push(id);
        }
    }
    work.tick()?;
...
fn tree_depths(
    objects: &BTreeMap<GitOid, FilterObject>, selected: &BTreeSet<GitOid>,
    explicit: &BTreeSet<GitOid>, work: &mut Work<'_, impl FnMut() -> bool>,
    ...
    let seed = |id, depths: &mut BTreeMap<GitOid, u32>, frontier: &mut BTreeSet<(u32, GitOid)>| {
        if selected.contains(&id) && depths.insert(id, 0) != Some(0) { frontier.insert((0, id)); }
    };
    for &id in selected {
        work.tick()?;
        ...
        match item.kind {
            ObjectType::Commit => {
                for &(child, kind) in &item.edges {
                    work.tick()?;
                    if kind == ObjectType::Tree { seed(child, &mut depths, &mut frontier); }
                }
            }
            ...
            ObjectType::Tag => {
                ...
                loop {
                    ...
                    if !tag_seen.insert(target) { break; }
    ...
    while let Some((depth, id)) = frontier.pop_first() {
        ...
        let item = object(objects, id)?;
        if item.kind != ObjectType::Tree { continue; }
        let next_depth = depth.checked_add(1)
            .ok_or_else(|| disclosure_refusal(RefusalCode::ResourceBudgetExceeded))?;
        for &(child, _) in &item.edges {
            work.tick()?;
            if !selected.contains(&child) { continue; }
            if depths.get(&child).is_some_and(|known| *known <= next_depth) { continue; }
            if let Some(old) = depths.insert(child, next_depth) { frontier.remove(&(old, child)); }
            work.count(depths.len())?;
