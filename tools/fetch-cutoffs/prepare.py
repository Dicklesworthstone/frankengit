...
with tempfile.TemporaryDirectory(prefix='fg-cutoffs-source-') as td:
    ...
    assert git('rev-parse',f'{base}:{v}')=='2b601f33448b184f4316bcdc7982287738d30a68'
    literals=[]
    ...
    assert literals==[(v,1),(node+'upload_visibility/shallow/tests.rs',1)],literals
    ...
    edit(v,'            let mut stopped = None;', '            commit_time = shallow::cutoffs::committer_time(&parsed);\n            let mut stopped = None;')
...
''','')
    ...
    for path in sorted((work/node/'upload_visibility').rglob('*.rs')):
        text=path.read_text()
        if 'shallow deepen-relative filter' in text:path.write_text(text.replace('shallow deepen-relative filter','shallow deepen-relative deepen-since deepen-not filter'))
    ...
    path=work/'crates/fgit-wire/src/cutoff_ref.rs';path.write_bytes(path.read_bytes()+(root/'tools/fetch-cutoffs/cutoff_ref_tests.rs').read_bytes())
    changed=set(git('diff','--name-only',cwd=work).splitlines())|set(git('ls-files','--others','--exclude-standard',cwd=work).splitlines())
    allowed={v,s,p,w,node+'lib.rs',node+'upload_visibility/shallow/tests.rs',node+'upload_visibility/tests/partial_clone.rs',node+'upload_visibility/shallow/cutoffs.rs','crates/fgit-wire/src/cutoff_ref.rs'}
    assert changed<=allowed,(changed-allowed)
    for name in sorted(changed):
        if name.endswith('.rs') and not name.endswith('/src/lib.rs'):subprocess.run(['rustfmt','--edition','2024','--config','skip_children=true',name],cwd=work,check=True,timeout=180)
