...
with tempfile.TemporaryDirectory(prefix='fg-relative-verify-') as td:
    ...
    if all(item['exit']==0 for item in evidence):
        evidence.append(run(td,'pinned-relative',['cargo','test','--locked','-p','fgit-node','--lib','pinned_git_relative_','--','--ignored','--nocapture','--test-threads=1'],env,600))
    if all(item['exit']==0 for item in evidence):
    ...
    for path in sorted((oracle_root/'runs').glob('*/transcripts/*.json')):
        ...
        if item['exit']:
            ...
            trace=pathlib.Path(item['packet_transcript']);print('FAILED_TRACE',trace.read_text(errors='replace') if trace.exists() else 'not generated',flush=True)
